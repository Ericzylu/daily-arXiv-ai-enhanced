...
TL;DR: 提出一种从多视角图像重建高保真头发的紧凑流水线，通过聚类发丝为代表性发卡并共享纹理码本，结合3D高斯泼溅渲染，显著减少重建时间和存储成本。
...
Result: 实验显示：1) 发丝重建时间减少4倍；2) 渲染性能相当；3) 内存占用降低200倍以上。

Conclusion: 该方法通过利用头发结构相似性，实现了高效的高保真头发重建，在显著降低计算和存储成本的同时保持了视觉质量。
...
TL;DR: SafeScreen是一个安全优先的视频筛选框架，针对开放域视频平台（如YouTube）中的儿童和护理场景，通过个性化安全约束来防止用户接触有害内容，与传统的参与度优化推荐算法形成对比。


<details>
  <summary>Details</summary>
Motivation: 开放域视频平台虽然提供丰富的个性化内容，但其以参与度优化的推荐算法可能让脆弱用户（如儿童、痴呆症患者）接触到不适当或有害内容。在儿童导向和护理场景中，内容在展示前必须满足个性化的安全约束。

Method: SafeScreen采用安全优先的方法，将安全作为前提条件，通过自动化流水线顺序批准或拒绝候选视频。框架包含三个核心组件：(1) 基于用户档案提取个性化安全标准；(2) 通过自适应问题生成和多模态VideoRAG分析进行证据驱动的评估；(3) 基于LLM的决策机制，在内容暴露前验证安全性、适当性和相关性。

Result: 在痴呆症护理回忆案例研究中，使用30个合成患者档案和90个测试查询进行评估。结果显示，SafeScreen在80-93%的情况下优先考虑安全而非参与度，与YouTube的参与度优化排名形成显著差异。同时保持了高水平的安全覆盖率、合理性和证据基础，这些结果得到了基于LLM的评估和领域专家的验证。

Conclusion: SafeScreen提供了一个可解释的实时视频筛选框架，能够在无需预计算安全标签的情况下处理未策划的视频库。该框架在护理等敏感场景中实现了安全优先的内容推荐，为脆弱用户提供了更可靠的保护机制。
...
### [3] [ClickAIXR: On-Device Multimodal Vision-Language Interaction with Real-World Objects in Extended Reality](https://arxiv.org/abs/2604.04905)
*Dawar Khan,Alexandre Kouyoumdjian,Xinyu Liu,Omar Mena,Dominik Engel,Ivan Viola*

Main category: cs.CV

TL;DR: ClickAIXR是一个在XR设备上运行的本地多模态视觉语言交互框架，通过控制器点击选择真实世界物体，使用本地视觉语言模型处理物体图像并回答自然语言问题，解决了云AI的隐私延迟问题和视线/语音交互的模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 现有XR交互系统存在两大问题：1) 依赖云AI服务（如ChatGPT）带来的隐私泄露和延迟问题；2) 基于视线或语音的选择方式存在模糊性。需要一种既保护隐私又提供精确交互的本地化解决方案。

Method: 提出ClickAIXR框架：1) 控制器点击选择真实世界物体；2) 本地视觉语言模型处理物体图像；3) ONNX推理实现本地化处理；4) 文本和语音双模态输出；5) 基于Magic Leap SDK（C API）实现。

Result: 用户研究对比ClickAIXR与Gemini 2.5 Flash和ChatGPT 5：1) 延迟适中；2) 用户体验可接受；3) 在可用性、信任度和用户满意度方面表现良好；4) 展示了点击选择与本地AI结合在可信XR交互中的潜力。

Conclusion: ClickAIXR证明了基于控制器点击的物体选择与本地视觉语言模型结合的有效性，为保护隐私、低延迟的XR交互提供了新范式，推动了可信XR交互的发展。
...
### [4] [A reconfigurable smart camera implementation for jet flames characterization based on an optimized segmentation model](https://arxiv.org/abs/2604.03267)
...
<details>
  <summary>Details</summary>
Motivation: 解决工业环境中缺乏实时早期火灾分割与表征方案的问题，特别是针对喷射火焰的实时监测需求

Method: 采用SoC FPGA平台（Ultra96），利用Vitis框架优化UNet分割模型，将参数从750万减少至5.9万（125倍压缩），通过多线程和批归一化进一步优化，实现边缘实时处理

Result: 处理延迟降低2.9倍，进一步优化后延迟改善7.5倍，达到30FPS的实时性能，同时保持Dice Score评估指标精度不变

Conclusion: 提出的智能相机平台能够有效实现工业喷射火焰的实时分割与表征，为火灾安全管理提供可行的边缘计算解决方案，该框架可扩展至其他火灾安全应用
...
TL;DR: 提出3D-IDE方法，通过几何自监督让3D感知作为涌现属性出现，而非显式编码，实现零延迟推理且消除深度和姿态依赖


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs中的3D信息利用方法在2D-3D表示融合上存在权衡问题，导致部署效果不佳。显式3D位置编码和嫁接外部3D基础模型的方法都有局限性

Method: 提出隐式几何涌现原则，通过细粒度几何验证器和全局表示约束构建信息瓶颈，迫使模型最大化视觉特征与3D结构间的互信息，让3D感知在统一视觉表示中自然涌现

Result: 在多个3D场景理解基准测试中超越SOTA，推理延迟降低55%，同时在不同下游任务中保持强大性能

Conclusion: 从外部嫁接转向隐式涌现代表了视觉语言模型中3D知识整合的根本性重新思考，精心设计的辅助目标可实现依赖无关的3D理解
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法在真实世界部署中面临视角变化挑战：难以泛化到未见相机视角、需要大量训练数据、推理延迟高

Method: 提出MoViD框架，包含视角估计器（建模关键关节关系预测视角信息）、正交投影模块（解耦运动和视角特征）、基于物理的跨视角对比对齐、以及面向边缘部署的逐帧推理流水线（根据估计视角自适应激活翻转细化）

Result: 在9个公开数据集和新收集的多视角无人机及步态分析数据集上评估：相比SOTA方法减少24.2%姿态估计误差；在严重遮挡下保持鲁棒性能且训练数据减少60%；在NVIDIA边缘设备上实现15FPS实时推理

Conclusion: MoViD通过解耦视角和运动特征，实现了视角不变的3D人体姿态估计，在精度、鲁棒性、效率和泛化能力方面均优于现有方法，适合边缘设备实时部署
...
TL;DR: 研究在严格受限的星上系统中，仅通过上传紧凑嵌入向量（而非原始像素）进行灾害监测的可行性，发现嵌入向量上传是关键使能技术，而最优决策头（kNN检索 vs. 类中心）因任务而异。


<details>
  <summary>Details</summary>
Motivation: 解决星地通信的下行链路瓶颈问题，通过在星上系统仅使用紧凑嵌入向量而非原始像素数据，实现灾害优先级的智能判断，同时应对遥感数据中的显式分布偏移（跨时间、跨事件/地点、跨站点云层、跨城市AOI）。

Method: 采用严格设置：地面站仅上传紧凑嵌入向量和元数据，星上系统通过向量搜索对新捕获数据进行分类。使用OlmoEarth嵌入向量，在扩展的公共多任务基准上进行评估（27个Sentinel-2 L2A场景，15个云站点，5个SpaceNet-2 AOI；10次随机种子）。比较两种决策头：kNN检索和类中心分类。

Result: 所有有效方法都依赖于相同的上传嵌入向量，但最优决策头因任务而异：kNN检索在云分类中显著优于类中心方法（0.92 vs. 0.91；p<0.01），而类中心在时序变化检测中占主导地位（0.85 vs. 检索0.48；p<0.01）。嵌入向量上传是关键使能技术，一旦嵌入向量在星上，系统可以按任务选择最佳决策头而无需额外上行链路成本，每次查询的遥测数据小于1KB。

Conclusion: 仅嵌入向量的上行链路是可行的星上灾害监测方案，能够有效应对遥感分布偏移。嵌入向量上传是核心使能技术，系统可以根据不同任务动态选择最优决策头（kNN或类中心），在极低通信开销（<1KB/查询）下实现高性能。
...
TL;DR: 该论文针对多模态大语言模型在物理世界理解方面的不足，特别是对连续体对象动态理解的局限性，提出了两个基准任务和Scene Dynamic Field方法，显著提升了模型的物理推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在图像和视频理解方面表现出色，但在高级物理推理方面存在显著不足。当前MLLMs在理解连续体对象的动态特性方面存在严重局限性，这限制了它们在真实物理世界中的应用。

Method: 首先引入两个基准任务来评估直观物理理解能力：Next Frame Selection（下一帧选择）和Temporal Coherence Verification（时间一致性验证）。然后提出Scene Dynamic Field方法，该方法利用物理模拟器在多任务微调框架中提升模型的物理理解能力。

Result: 实验表明，即使是当前最先进的MLLMs在这些基础任务上表现也很差。提出的SDF方法显著提升了性能，在流体任务上实现了高达20.7%的性能提升，并且在未见过的物理领域表现出强大的泛化能力。
...
TL;DR: HVG-3D是一个用于3D感知手-物交互视频合成的统一框架，通过3D ControlNet编码几何和运动线索实现显式3D推理，支持真实和模拟数据的灵活控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖2D控制信号，缺乏空间表达能力且限制了合成3D条件数据的利用。需要解决这些限制以实现更好的手-物交互视频合成。

Method: 提出HVG-3D框架，包含：1）基于扩散的架构增强3D ControlNet，编码3D输入的几何和运动线索；2）混合管道构建输入和条件信号，支持训练和推理中的灵活精确控制。

Result: 在TASTE-Rob数据集上，HVG-3D实现了最先进的空间保真度、时间一致性和可控性，能够有效利用真实和模拟数据，生成高质量、时间一致的视频。

Conclusion: HVG-3D通过显式3D表示和3D ControlNet解决了现有方法的局限性，为手-物交互视频合成提供了统一框架，实现了更好的空间表达和3D条件数据利用。
...
TL;DR: 提出IDCL方法，首次将基于密度信息的模型训练策略引入图像聚类，通过课程学习方案和密度核心引导聚类分配，提升了聚类鲁棒性和性能。
...
Result: 在基准数据集上与最先进聚类方法进行广泛比较，证明了所提方法在鲁棒性、快速收敛性以及数据规模、聚类数量、图像上下文等方面的灵活性优势。
...
TL;DR: V-Reflection框架通过"思考-观察"视觉反射机制，将多模态大语言模型转变为主动询问者，使用潜在状态作为动态探针主动查询视觉特征空间，解决细粒度任务中的感知幻觉问题。
...
Conclusion: V-Reflection通过"思考-观察"视觉反射机制，将多模态大语言模型从被动观察者转变为主动询问者，解决了细粒度任务中的感知幻觉问题，同时保持推理时完全端到端的自回归解码效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法难以表示层次化3D语义结构和捕获复杂场景中的整体-部分关系；密集成对比较和来自2D先验的不一致层次标签阻碍特征学习，导致分割效果不佳

Method: 1. 构建多层次对象树以结构化学习对象-部分层次关系；2. 两阶段级联对比学习策略，从全局到局部渐进细化特征表示；3. 一致性分割检测机制对齐跨视图分割模式；4. 基于图的去噪模块抑制不稳定高斯点

Result: 在开放词汇3D对象选择、3D点云理解和消融研究中展示了方法的有效性和鲁棒性

Conclusion: TreeGaussian通过显式建模层次语义关系和减少对比监督冗余，显著提升了3D高斯溅射在复杂场景中的语义分割质量和一致性
...
TL;DR: 提出基于扩散模型随机最优控制的推理时优化框架，用于生成长距离人体运动，特别是语义不同运动域之间的连贯过渡


<details>
  <summary>Details</summary>
Motivation: 长距离人体运动生成是计算机视觉和图形学中的核心挑战，特别是在不同语义运动域之间生成连贯过渡的能力尚未充分探索。这种能力对于舞蹈编排等应用尤为重要，需要在不同风格和语义主题之间实现流畅过渡。

Method: 提出简单有效的推理时优化框架，受扩散基随机最优控制启发。具体采用控制能量目标函数，显式正则化预训练扩散模型的过渡轨迹。在推理时优化该目标函数以获得保真度和时间连贯性的过渡。

Result: 优化该目标函数在推理时能够产生具有保真度和时间连贯性的过渡。这是首个为受控长距离人体运动生成提供显式过渡建模的通用框架。

Conclusion: 提出的推理时优化框架能够有效解决长距离人体运动生成中的过渡问题，特别是在不同语义运动域之间实现连贯过渡，为舞蹈编排等应用提供了实用解决方案。
...
### [14] [CoLA: Cross-Modal Low-rank Adaptation for Multimodal Downstream Tasks](https://arxiv.org/abs/2604.03314)
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在多模态任务适配中存在挑战，特别是双流架构中单模态编码器的适配问题。现有的PEFT方法如LoRA在各自模态内独立运行，无法有效捕获跨模态交互
...
Conclusion: CoLA首次实现了视觉定位的多任务PEFT框架，填补了高效多模态适配的关键空白，为双流架构中的基础模型适配提供了有效解决方案
...
TL;DR: StoryBlender是一个基于3D的故事板生成框架，通过语义-空间接地、规范资产实例化和时空动态三阶段流程，解决了现有方法难以同时保证跨镜头一致性和显式可编辑性的问题。


<details>
  <summary>Details</summary>
Motivation: 当前故事板自动化方法存在两个主要问题：2D扩散模型虽然能生成生动图像，但存在身份漂移和几何控制有限的问题；传统3D动画工作流虽然一致且可编辑，但需要专家级、劳动密集型的创作。需要一种能同时实现跨镜头一致性和显式可编辑性的系统。

Method: 提出StoryBlender框架，采用三阶段流程：1) 语义-空间接地：构建连续性记忆图，将全局资产与镜头特定变量解耦以实现长期一致性；2) 规范资产实例化：在统一坐标空间中实例化实体以保持视觉身份；3) 时空动态：通过视觉指标实现布局设计和电影演化。采用分层多智能体架构和验证循环进行迭代自校正。

Result: 实验表明，StoryBlender在一致性和可编辑性方面显著优于基于扩散和3D接地的方法。生成的本地3D场景支持直接、精确的相机和视觉资产编辑，同时保持跨镜头连续性。

Conclusion: StoryBlender成功解决了故事板自动化中跨镜头一致性和可编辑性的双重挑战，通过3D基础框架和故事中心反思方案，为视觉叙事提供了高效、可控的生成工具。
...
TL;DR: 该论文研究了大型视觉语言模型中的注意力汇聚现象，将其分为视觉编码器产生的V-sinks和LLM层产生的L-sinks，揭示了全局先验与局部感知间的性能权衡，并提出无需任务监督的层间汇聚门控模块来动态调节注意力贡献。


<details>
  <summary>Details</summary>
Motivation: 虽然注意力汇聚在单模态Transformer中已有研究，但其在大型视觉语言模型中的跨模态影响尚未被充分探索。作者旨在探究这些汇聚是冗余的伪影还是必要的全局先验，并理解它们如何影响模型的全局推理与局部感知能力。

Method: 首先将视觉汇聚分为两类：从视觉编码器传播的V-sinks和在深层LLM层中产生的L-sinks。基于此定义分析性能权衡，识别对下游性能影响最大的功能层。提出层间汇聚门控模块，该轻量级即插即用模块动态缩放V-sink与其他视觉token的注意力贡献，通过标准的下一个token预测进行训练，无需任务特定监督且保持LVLM主干冻结。

Result: 分析揭示了基本性能权衡：汇聚有效编码全局场景级先验，但其主导地位会抑制局部感知所需的细粒度视觉证据。识别了调节这些汇聚对下游性能影响最显著的具体功能层。LSG在大多数层中都能改善代表性多模态基准测试性能，有效平衡全局推理和精确的局部证据。

Conclusion: 注意力汇聚在LVLM中既是必要的全局先验，也可能成为抑制局部感知的冗余伪影。提出的LSG模块能够动态调节注意力贡献，在无需任务特定监督的情况下有效平衡全局推理与局部证据，为优化大型视觉语言模型的注意力机制提供了新方法。
...
TL;DR: 本研究提出了一种基于预训练和基础模型的可扩展人工智能方法，用于自动检测面对面协作学习中的注视行为，无需人工标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法需要大量标注数据进行训练，且训练数据集往往无法涵盖教育环境中的全部情境，导致模型在跨配置环境下的鲁棒性受到质疑。需要开发不依赖人工标注、具有更好跨配置鲁棒性的自动注视行为检测方法。

Method: 采用预训练和基础模型构建的端到端方法：使用YOLO11进行人员跟踪，YOLOE-26（具有文本提示能力）检测教育相关物体，Gaze-LLE模型预测注视目标。整个流程无需人工标注数据。

Result: 该方法在视频数据中检测学生注视行为的F1分数达到0.829，对笔记本电脑注视和同伴注视表现出色，但对其他注视目标的检测性能较弱。与有监督机器学习方法相比，在复杂环境中表现出更优越和稳定的性能，具有更好的跨配置鲁棒性。

Conclusion: 提出的基于预训练和基础模型的方法能够有效检测协作学习中的注视行为，无需人工标注数据，且在复杂环境中具有更好的鲁棒性。该方法为支持学生在真实环境中进行协作学习提供了有前景的技术途径。
...
TL;DR: EgoMind是一个无需几何先验的链式思维框架，通过角色扮演描述和渐进式空间分析增强MLLMs的空间推理能力，仅需少量数据即可在多个基准上取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在空间认知任务中存在局限性：引入3D先验或几何监督的方法虽然提升性能但数据准备和对齐成本高昂；纯2D方法在多帧空间推理中因难以捕捉跨帧空间关系而表现不佳。

Method: 提出EgoMind框架：1) Role-Play Caption：通过角色扮演描述联合构建跨帧的连贯语言场景图；2) Progressive Spatial Analysis：渐进式推理针对特定任务的问题。仅需5K自动生成的SFT样本和20K RL样本。

Result: 在VSI-Bench、SPAR-Bench、SITE-Bench和SPBench等多个基准上取得竞争性结果，证明了框架在增强MLLMs空间推理能力方面的有效性。

Conclusion: EgoMind展示了语言推理在空间认知中的潜力，通过几何无关的方法有效解决了MLLMs在多帧空间推理中的局限性，同时避免了昂贵的数据准备成本。
...
TL;DR: VitaTouch是一个多模态视觉-触觉-语言模型，用于材料属性推理和自然语言属性描述，在智能制造的质检任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 智能制造的质检需要识别材料内在属性和表面特性，而仅依赖视觉的方法容易受到遮挡和反射的影响，因此需要结合触觉等多模态信息来提升识别能力。

Method: 使用模态特定编码器和双Q-Former提取语言相关的视觉和触觉特征，通过对比学习对齐各模态与文本并显式耦合视觉与触觉，最后将压缩的特征作为前缀令牌输入大语言模型。

Result: 在VitaSet数据集上达到88.89%硬度准确率、75.13%粗糙度准确率和54.81%描述符召回率；材料描述任务语义相似度峰值0.9009；通过LoRA微调在缺陷识别任务中分别达到100.0%、96.0%和92.0%的准确率；在100次实验室机器人试验中实现94.0%闭环识别准确率和94.0%端到端分拣成功率。

Conclusion: VitaTouch通过融合视觉、触觉和语言模态，有效解决了智能制造中材料属性识别和质检的挑战，在多个基准测试中表现出优越性能，展示了多模态方法在工业应用中的潜力。
...
TL;DR: 比较九种代表性表面重建方法在叶片表面重建中的性能，评估其在三个公开数据集上的表现，为资源受限的机器人平台提供选择指导
...
Result: 分析揭示了不同方法在表面积估计精度、平滑度、对噪声和缺失数据的鲁棒性以及计算成本之间的权衡，这些因素影响农业应用中机器人硬件的成本和约束。每种方法根据应用和资源约束展现出不同的优势
...
TL;DR: RGxEStat：一个轻量级交互工具，用于基因型×环境互作分析，包含显著性分析和稳定性分析，无需编程知识即可使用


<details>
  <summary>Details</summary>
Motivation: 基因型×环境（GxE）互作影响基因型在不同环境中的表现，降低表型预测性。深入分析GxE互作有助于识别基因优势或缺陷在特定环境条件下的表达或抑制，从而支持遗传选择和育种实践。目前缺乏用户友好的工具，需要简化分析流程。

Method: 提出两种GxE互作分析模型：1）基于混合效应模型的显著性分析，确定基因或GxE互作是否显著影响表型性状；2）稳定性分析，研究基因与环境间的交互关系以及基因型在不同环境中的相对优劣。开发RGxEStat工具，集成模型构建、求解和可视化功能。

Result: 开发了RGxEStat工具，为育种家和农学家提供用户友好界面，无需学习复杂的SAS或R编程即可进行GxE互作分析。工具代码和数据集已在GitHub上公开（https://github.com/mason-ching/RGxEStat）。

Conclusion: RGxEStat工具简化了GxE互作分析流程，显著加速了育种研究周期。该工具使育种家和农学家能够更高效地进行遗传选择和育种实践，促进作物改良。
...
TL;DR: AC-LAM通过强制潜在动作空间的加性组合结构，解决了现有潜在动作学习方法缺乏物理运动结构先验的问题，在模拟和真实桌面任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有潜在动作学习方法缺乏编码物理运动加性、组合结构的先验，导致潜在动作常常纠缠无关场景细节或未来观测信息，并且运动幅度校准不准确。

Method: 引入加性组合潜在动作模型(AC-LAM)，在潜在动作空间上强制实施短时间范围内的场景级加性组合结构。这些AC约束鼓励潜在动作空间中的简单代数结构（恒等、逆、循环一致性），并抑制非加性组合的信息。

Result: AC-LAM学习了更具结构化、运动特定性和位移校准的潜在动作，为下游策略学习提供了更强的监督，在模拟和真实世界桌面任务中优于最先进的LAM方法。

Conclusion: 通过强制潜在动作空间的加性组合结构，AC-LAM能够学习更高质量的潜在动作表示，从而提升下游策略学习性能，为利用互联网规模视频进行具身AI提供了更有效的潜在动作学习方法。
...
TL;DR: 该论文是关于遥感领域混合专家模型（MoE）的首个系统性综述，涵盖基本原理、架构设计和关键应用，并展望未来趋势。
...
Method: 采用系统性综述方法，对MoE在遥感领域的应用进行全面梳理，包括基本原理、架构设计分析以及不同遥感任务中的应用案例。

Result: 提供了MoE在遥感领域的首个系统性综述，涵盖了从基础原理到具体应用的完整框架，为研究人员提供了全面的参考。

Conclusion: MoE是解决遥感数据多样性和动态性挑战的有效范式，该综述填补了领域空白，并指出了未来研究方向以推动MoE在遥感领域的创新应用。
...
TL;DR: 研究发现多模态智能系统在多轮图像编辑中存在迭代质量退化问题，图像在重复编辑后会产生可见噪声累积，且现有图像质量评估指标无法检测这种退化。


<details>
  <summary>Details</summary>
Motivation: 尽管最新的图像编辑模型在单轮编辑中能忠实遵循指令并生成高质量图像，但多轮编辑中存在图像质量迭代退化的关键弱点，这可能会威胁未来模型训练稳定性和已部署智能系统的安全性。

Method: 引入Banana100数据集，包含28,000张通过100次迭代编辑步骤生成的退化图像，涵盖多样化纹理和图像内容。评估了21种流行的无参考图像质量评估（NR-IQA）指标对退化图像的检测能力。

Result: 图像在重复编辑过程中，微小伪影会累积，迅速导致严重可见噪声累积，并且无法遵循简单编辑指令。在21种NR-IQA指标中，没有一种能始终为严重退化图像分配比干净图像更低的分数。

Conclusion: 生成器和评估器的双重失败可能威胁未来模型训练稳定性和已部署智能系统的安全性，特别是当多轮编辑生成的低质量合成数据逃过质量过滤器时。发布完整代码和数据以促进开发更鲁棒的模型。
...
TL;DR: 量化向量是权重空间中可转移的方向，能够提升模型对后训练量化的鲁棒性，无需接收方量化感知训练


<details>
  <summary>Details</summary>
Motivation: 后训练量化（PTQ）会导致模型性能下降，而量化感知训练（QAT）成本高昂。本文旨在寻找一种无需接收方训练数据、低成本的替代方案来提升模型对PTQ的鲁棒性。

Method: 通过权重空间算术从捐赠任务中提取量化向量，该向量可应用于接收模型以提升其对PTQ噪声的鲁棒性，无需接收方的量化感知训练。

Result: 量化向量能将模型对PTQ噪声的鲁棒性提升高达60%，在Vision Transformer模型上验证了其有效性，为极低比特部署提供了零样本、低成本的QAT替代方案。
...
TL;DR: 该研究提出了一个基于基础模型的自动化猪群监测工作流，通过预训练视觉-语言基础模型作为通用视觉骨干，结合轻量级任务特定逻辑，实现了可扩展、标签高效的长时监测。


<details>
  <summary>Details</summary>
Motivation: 精准畜牧业中现有监测系统主要依赖监督学习模型，需要大量标注数据、重复训练和农场特定调优。基础模型为减少任务特定监督学习依赖、利用大规模学习的一般视觉表示提供了新机遇。

Method: 采用基础模型为中心的工作流：1) 使用Grounding-DINO进行目标检测建立基线；2) 针对夜间和遮挡问题集成时序跟踪逻辑；3) 使用Grounded-SAM2进行短期视频分割；4) 开发长期跟踪管道，包含初始化、跟踪、匹配、掩码细化、重识别和后处理质量控制模块。

Result: 检测在白天条件下准确率高，但在夜间和严重遮挡时性能下降。短期视频分割后处理使4927条活动轨迹中超过80%完全正确。长期跟踪系统在132分钟连续视频中保持稳定身份，在132个均匀采样的地面真值帧上达到：平均区域相似度(J)0.83、轮廓准确度(F)0.92、J&F 0.87、MOTA 0.99、MOTP 90.7%，无身份切换。

Conclusion: 该工作展示了如何将基础模型先验知识与轻量级任务特定逻辑相结合，实现猪生产中可扩展、标签高效和长时监测，为精准畜牧业提供了新的监测范式。
...
<details>
  <summary>Details</summary>
Motivation: 水下物种自动分类面临标注成本高和环境变化导致全监督模型迁移性差的问题。虽然冻结的自监督视觉基础模型嵌入已提供标签高效的基线，但研究是否能在推理时进一步改进这些冻结嵌入的性能，而无需微调模型权重。

Method: 应用电路复制方法（Circuit Duplication），这是一种最初为大型语言模型提出的推理时方法，在推理过程中将选定的Transformer层遍历两次。在AQUA20基准测试上使用冻结的DINOv3嵌入进行评估，采用两种设置：全局电路选择（为整个数据集选择单一复制电路）和类特定电路选择（每个物种可能有不同的最优电路）。两种设置都使用简单的半监督下游分类器。

Result: 电路复制方法持续改进标准冻结前向传播的性能。在最大标签预算下，类特定选择达到0.875的宏观F1分数，将与全监督ConvNeXt基准（0.889）的差距缩小到1.4个百分点，且无需任何基于梯度的训练。四个物种的表现超过了其全监督参考，其中章鱼提高了+12.1 F1点。在所有预算下，约75%的类别偏好类特定电路，表明存在真正的类别依赖性优势。这是电路复制方法首次应用于计算机视觉领域。

Conclusion: 电路复制作为一种无需训练的方法，能够有效提升冻结自监督视觉基础模型在水下物种分类任务中的性能。类特定电路选择显示出明显的优势，表明不同类别可能需要不同的推理路径优化。该方法为标签稀缺场景下的视觉任务提供了有前景的解决方案。
...
TL;DR: ExpressEdit是一个开源Photoshop插件，用于快速、无伪影的面部表情编辑，支持检索增强生成，比专有模型更快


<details>
  <summary>Details</summary>
Motivation: 当前AI图像编辑模型在进行风格化表情编辑时会产生全局噪声和像素漂移等伪影，无法集成到专业图像编辑软件和工作流程中，需要解决这一问题

Method: 开发完全开源的Photoshop插件ExpressEdit，避免专有模型的常见伪影，与Photoshop原生操作（如液化）稳健协同；编译包含135个表情标签的综合表情数据库，配有示例故事和图像，支持检索增强生成

Result: ExpressEdit在单个消费级GPU上3秒内无缝编辑表情，比流行专有模型显著更快；开源代码和数据集促进未来研究和艺术探索

Conclusion: ExpressEdit填补了AI表情编辑模型与专业图像编辑工作流程之间的空白，提供了快速、无伪影且与Photoshop原生操作兼容的解决方案，通过开源促进社区发展
...
TL;DR: 提出多模态框架整合高分辨率卫星影像与街景图像，在有限标注条件下实现可扩展的城市树木检测，通过领域适应和混合学习策略显著提升检测性能


<details>
  <summary>Details</summary>
Motivation: 传统人工树木调查方法成本高且难以扩展，现有自动化系统受限于高标注成本和跨城市场景泛化能力差，需要开发标注高效、可扩展的城市树木检测方法

Method: 多模态框架：先用卫星影像定位候选树木，再获取街景图像进行详细检测；采用领域适应迁移已有标注数据知识；评估半监督学习、主动学习和混合策略三种学习策略，使用基于Transformer的检测模型

Result: 混合策略获得最佳性能，F1分数达0.90，比基线模型提升12%；主动学习通过针对性人工标注持续改进性能，而半监督学习因伪标签确认偏差导致性能逐渐下降；误差分析显示主动和混合策略减少了假阳性和假阴性

Conclusion: 多模态方法和引导式标注对于实现可扩展、标注高效的城市树木测绘至关重要，能够支持可持续城市规划；混合学习策略在有限标注条件下提供了最优的检测性能
...
TL;DR: 提出用户显著目标检测(UserSOD)新任务，强调基于用户主动需求而非被动视觉刺激来检测显著目标
...
Method: 提出UserSOD新任务框架，专注于检测与用户主动需求一致的显著目标。主要挑战是缺乏用于模型训练和测试的数据集。

Result: 论文提出了新的研究任务和方向，但尚未提供具体的实验结果，因为主要贡献是任务定义和问题阐述。

Conclusion: 基于用户需求检测显著目标至关重要，UserSOD任务为解决这一需求提供了新框架，但需要构建相应数据集来支持该领域的发展。
...
TL;DR: 该论文建立了分数扩散模型与即插即用方法的理论联系，提出了随机生成式PnP框架，通过噪声注入更好地利用扩散模型先验，在严重病态逆问题中提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管分数扩散模型和PnP方法都依赖于去噪器，但如何系统地将预训练的分数扩散模型作为先验整合到PnP框架中而不依赖反向扩散采样仍不明确。需要建立理论联系并开发新方法以充分利用扩散模型的生成能力。

Method: 1. 建立PnP的分数解释，为直接使用预训练分数扩散模型提供理论依据；2. 提出随机生成式PnP框架，通过噪声注入更好地利用分数扩散模型的生成先验；3. 理论分析表明噪声注入诱导高斯平滑目标优化并促进逃离严格鞍点。

Result: 在多线圈MRI重建和大掩码自然图像修复等挑战性逆任务上，SGPnP相比传统PnP方法取得一致改进，性能与基于扩散的求解器相当。实验验证了噪声注入在严重病态逆问题中的鲁棒性优势。

Conclusion: 该工作建立了分数扩散模型与PnP方法的理论联系，提出的随机生成式PnP框架能够有效利用预训练扩散模型先验，在严重病态逆问题中提供更鲁棒的解决方案，为结合生成模型与优化方法提供了新思路。
...
TL;DR: PortraitCraft是一个统一的肖像构图理解与生成基准，包含5万张带结构化多级标注的真实肖像图像，支持构图评分、13种构图属性标注、解释文本、视觉问答和构图导向的文本描述，为细粒度肖像理解和可控肖像生成提供标准化评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有数据集和基准主要关注粗粒度美学评分、通用图像美学或无约束肖像生成，缺乏对结构化肖像构图分析和可控肖像生成的系统研究。这限制了在明确构图要求下的肖像构图理解和生成能力的发展。

Method: 构建包含约5万张精选真实肖像图像的数据集，提供结构化多级监督：全局构图评分、13种构图属性标注、属性级解释文本、视觉问答对以及面向构图生成的文本描述。在此基础上建立两个互补的基准任务：构图理解（评分预测、细粒度属性推理、图像基础视觉问答）和构图感知生成（从结构化构图描述生成肖像）。

Result: 建立了PortraitCraft统一基准，包含标准化评估协议和代表性多模态模型的参考基线结果。该基准为细粒度肖像理解、可解释美学评估和可控肖像生成提供了全面的研究平台。

Conclusion: PortraitCraft填补了现有研究空白，为未来细粒度肖像理解、可解释美学评估和可控肖像生成研究提供了系统性的基准和评估框架，推动了结构化肖像构图分析和生成技术的发展。
...
TL;DR: TABLeT是一种使用预训练2D自然图像自编码器对fMRI体积进行标记化的新方法，通过压缩3D fMRI体积为紧凑的连续标记，实现有限VRAM下的长序列建模，在多个大规模基准测试中优于现有模型。


<details>
  <summary>Details</summary>
Motivation: fMRI四维信号的高维性使得长程时空动态建模面临挑战。现有的基于体素的模型虽然性能优异且可解释性强，但受限于巨大的内存需求，只能捕获有限的时域窗口。

Method: 提出TABLeT方法：使用预训练的2D自然图像自编码器对fMRI体积进行标记化，将每个3D fMRI体积压缩为一组紧凑的连续标记，然后使用简单的Transformer编码器进行长序列建模。还开发了自监督的掩码标记建模方法进行预训练。

Result: 在UK-Biobank、Human Connectome Project和ADHD-200等大规模基准测试中，TABLeT在多个任务上优于现有模型，同时在相同输入条件下比最先进的基于体素方法在计算和内存效率上有显著提升。自监督预训练进一步提高了各种下游任务的性能。

Conclusion: TABLeT为大脑活动的可扩展和可解释的时空建模提供了一种有前景的方法，通过标记化策略有效解决了fMRI数据高维性带来的计算和内存挑战。
...
TL;DR: MuPD是一个多模态病理学扩散基础模型，通过扩散变换器和解耦跨模态注意力将组织学、分子RNA谱和临床文本嵌入共享潜在空间，支持多种跨模态合成任务，性能优于领域特定模型。


<details>
  <summary>Details</summary>
Motivation: 复杂疾病的准确诊断和治疗需要整合组织学、分子和临床数据，但这些模态在实践中往往不完整。现有计算方法依赖于特定任务的模型，训练在狭窄的单源-目标对上，泛化能力有限。

Method: 引入MuPD（多模态病理学扩散）生成基础模型，通过扩散变换器与解耦跨模态注意力将H&E染色组织学、分子RNA谱和临床文本嵌入共享潜在空间。在3400万组织学图像块、160万文本-组织学对和1080万RNA-组织学对上进行预训练，涵盖34个人体器官。

Result: 文本条件和图像到图像生成中，MuPD合成组织学忠实的组织结构，FID分数降低50%，通过合成数据增强将少样本分类准确率提高47%。RNA条件组织学生成中，FID降低23%，在五种癌症类型中保持细胞类型分布。作为虚拟染色剂，H&E到免疫组化和多重免疫荧光转换中，平均标记相关性提高37%。

Conclusion: 单一统一生成模型在异质病理学模态上预训练可显著优于专业替代方案，为多模态组织病理学提供可扩展计算框架。
...
<details>
  <summary>Details</summary>
Motivation: 物联网视频分析中用户隐私泄露问题日益严重，但逐帧保护在大规模视频分析中会引入显著延迟。现有对象检测方法需要完全解码视频或逐帧处理压缩视频，导致解码开销大或准确率降低。
...
Conclusion: ComPrivDet在压缩视频中实现了高效隐私对象检测，通过重用I帧推理显著减少了计算开销，为大规模视频分析中的隐私保护提供了实用解决方案。
...
TL;DR: CSRS提出了一种用于多模态大语言模型无监督自进化的连续软化回溯重采样方法，通过回溯重推理机制、软化频率奖励和视觉语义扰动，解决传统多数投票方法因模型内在偏见导致推理路径质量下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有无监督自进化方法主要依赖多数投票选择最频繁输出作为伪黄金答案，但这可能源于模型内在偏见而非保证推理路径的客观正确性，导致质量退化。需要一种能对抗这种退化、确保稳定有效学习的方法。
...
Result: 实验表明CSRS显著提升了Qwen2.5-VL-7B在MathVision等基准测试上的推理性能，在几何任务的无监督自进化中取得了最先进(SOTA)结果。

Conclusion: CSRS通过结合回溯重推理、软化频率奖励和视觉语义扰动，有效解决了传统多数投票方法的局限性，显著提升了多模态大语言模型在无监督自进化中的推理能力，特别是在几何任务上达到了最优性能。
...
TL;DR: 提出自适应关系变换器（ART），通过时间感知关系图（TARG）显式捕捉成对交互演化，并采用自适应交互剪枝（AIP）机制高效减少冗余计算，在行人轨迹预测任务中实现高精度和高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有行人轨迹预测方法（基于图或变换器的框架）存在计算开销过大或难以捕捉人类交互多样性和时变特性的问题。准确预测真实世界行人轨迹对机器人应用至关重要。

Method: 提出自适应关系变换器（ART）：1）引入时间感知关系图（TARG）显式建模成对交互的演化过程；2）设计自适应交互剪枝（AIP）机制，高效减少冗余计算。

Result: 在ETH/UCY和NBA基准测试上进行广泛评估，ART在保持高计算效率的同时实现了最先进的预测精度。

Conclusion: ART通过显式建模交互演化和自适应剪枝机制，有效解决了现有方法在计算效率和交互表示方面的局限性，为行人轨迹预测提供了准确高效的解决方案。
...
TL;DR: DreamPRVR提出了一种从粗到细的表征学习范式，通过生成全局语义寄存器作为粗粒度高光，然后进行细粒度相似性优化，解决部分相关视频检索中的全局上下文感知不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有部分相关视频检索方法存在全局上下文感知不完整的问题，难以处理查询歧义性和由虚假响应引起的局部噪声，需要更有效的跨模态匹配机制。

Method: 采用从粗到细的表征学习范式：1) 通过概率变分采样器生成视频中心分布，然后用文本监督的截断扩散模型迭代精炼，生成全局语义寄存器；2) 文本语义结构学习构建良好的文本潜在空间；3) 通过寄存器增强的高斯注意力块自适应融合寄存器与视频标记，实现上下文感知特征学习。

Result: 大量实验表明DreamPRVR在性能上优于现有最先进方法，验证了所提方法的有效性。
...
Method: 提出LaPR框架：1）设计图像-标签联合表示以显式融入标签信息；2）引入混合专家机制处理测试时查询标签不可用的情况，每个专家捕获特定标签模式，路由器推断查询自适应权重；3）采用交替优化策略，分别使用VICL性能引导对比损失和标签引导对比损失训练专家和路由器。

Result: 在上下文分割、检测和着色任务上取得显著且一致的性能提升。LaPR在不同特征提取器和跨折叠场景中表现出良好的泛化能力，验证了标签利用在提示检索中的重要性。

Conclusion: 标签一致性对视觉上下文学习性能至关重要，LaPR框架通过标签感知的提示检索机制有效提升了VICL性能，为视觉基础模型的提示选择提供了新思路。
...
TL;DR: 提出一种基于视觉大语言模型的方法，通过Set-of-Mark提示增强视觉定位能力，利用用户最近注视点轨迹理解意图，并引入逆指数采样策略捕捉交互前的时序动态，在HD-EPIC数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 在智能辅助系统中，预测人-物交互对于指导用户日常生活活动和理解其短期与长期目标至关重要。现有方法在视觉定位能力和用户意图理解方面存在局限，需要改进以更好地处理第一人称视角下的交互预测任务。

Method: 1. 使用视觉大语言模型处理第一人称视角视频；2. 通过Set-of-Mark提示增强视觉定位能力；3. 利用用户最近注视点形成的轨迹理解用户意图；4. 引入逆指数采样策略捕捉交互前的时序动态；5. 模型无关的设计可在不同VLLM上应用。

Result: 在HD-EPIC第一人称视角数据集上的实验表明，该方法超越了当前最先进的方法，证明了其有效性。模型无关的特性使其能够适应不同的视觉大语言模型。

Conclusion: 通过增强视觉定位、理解用户意图和优化时序采样，提出的方法显著提升了第一人称视角下人-物交互预测的性能，为智能辅助系统提供了更准确的交互预测能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决新型传感器（如事件相机和4D雷达）数据稀缺问题，支持系统研究不同传感器特性，促进多模态融合策略研究。

Method: 构建包含立体事件相机、RGB相机、热成像相机、4D雷达和双激光雷达的多传感器数据集，提供精确的2D/3D边界框标注、轨迹ID和车辆里程计，建立统一的3D和2D基准测试。

Result: 创建了DSERT-RoLL数据集，提供了代表性单模态和多模态方法的基线结果，提出了将传感器特定线索集成到统一特征空间的融合框架，提高了不同天气和光照条件下的3D检测鲁棒性。

Conclusion: DSERT-RoLL数据集为新型传感器研究提供了重要资源，支持传感器特性比较和融合策略探索，提出的融合框架增强了多模态感知在复杂环境下的鲁棒性。
...
TL;DR: 该论文研究了科学图像领域的长尾识别问题，提出SciLT框架通过自适应特征融合和双重监督学习，在基础模型微调有限的情况下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有长尾识别研究主要集中于自然图像领域，而科学图像具有独特的视觉特征和监督信号。在基础模型与微调数据分布差异大的情况下，传统微调方法效果有限，需要专门针对科学图像长尾识别的研究。

Method: 提出SciLT框架：1）利用多级表示，通过自适应特征融合联合利用倒数第二层和最终层特征；2）采用双重监督学习策略；3）在参数高效微调范式下实现。

Result: 在三个科学基准测试上的实验表明：1）基础模型微调收益有限；2）倒数第二层特征对尾部类别特别重要；3）SciLT框架在头部和尾部类别上均取得平衡性能，显著优于现有方法。

Conclusion: SciLT为科学长尾识别建立了强大实用的基准，为适应基础模型到具有显著领域偏移的科学数据提供了有价值的指导，证明了多级特征表示在科学图像长尾识别中的重要性。
...
TL;DR: 论文揭示了深度学习图像水印中"编码器-噪声层-解码器"架构对已知原始攻击的脆弱性，并提出ResGuard增强模块来提升对抗性鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习水印方法主要关注随机信道失真，但忽视了攻击者拥有原始-水印图像对时的针对性抑制攻击。这种已知原始攻击(KOA)利用水印残差的图像无关性，能有效移除水印而保持视觉质量。

Method: 提出ResGuard插件模块：1) 残差特异性增强损失函数，强制水印残差与宿主图像紧密耦合；2) 辅助KOA噪声层，在训练中注入残差式扰动，使解码器在更强嵌入不一致性下保持可靠。

Result: ResGuard集成到现有框架后，将KOA攻击下的平均水印提取准确率从59.87%显著提升至99.81%，同时保持对传统攻击的鲁棒性。

Conclusion: 深度学习水印系统需要更强的图像依赖性来抵御已知原始攻击，ResGuard通过增强残差特异性提供了一种有效的解决方案，显著提升了对抗性鲁棒性。
...
TL;DR: FunFact是一个从RGB-D图像构建概率开放词汇功能3D场景图的框架，通过因子图联合推理所有功能关系，显著提高了关系发现和置信度校准


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法通常孤立考虑物体对之间的功能关系，无法捕捉人类用于解决歧义的场景级相互依赖性，需要更全面的功能场景理解方法

Method: 首先构建物体和部件中心的3D地图，使用基础模型提出语义上合理的功能关系候选，将其转换为因子图变量，并通过LLM常识先验和几何先验进行约束，进行联合概率推理

Result: 在SceneFun3D、FunGraph3D和FunThor数据集上的实验表明，FunFact提高了节点和关系发现召回率，显著减少了模糊关系的校准误差

Conclusion: 整体概率建模对功能场景理解有益，FunFact框架通过联合推理功能关系及其边缘概率，实现了更好的置信度校准和场景级功能理解
...
TL;DR: VidNum-1.4K是一个包含1,379个人工标注视频-问题对的视频问答基准，专门评估视频中的真实数值推理能力，涵盖物体、动作和事件量化，采用三级层次结构从视觉感知到组合数值推理。


<details>
  <summary>Details</summary>
Motivation: 现有视频数值推理基准局限于狭窄领域（如重复运动）或将简单计数视为表面回归任务，无法评估真实世界多媒体内容中的多步数值逻辑，需要测试视觉语言模型是否真正理解现实世界动态。

Method: 构建VidNum-1.4K基准，包含1,379个严格人工标注的视频-问题对，涵盖多样化环境；采用三级层次结构：从直接视觉感知到视频组合数值推理；要求模型基于时间证据执行算术运算、比较和逻辑推理。

Result: 评估多种最先进的视觉语言模型显示显著推理差距：Gemini-3.1-pro仅达到约60%准确率，代表性开源模型在25%-45%范围内挣扎，表明当前模型缺乏稳定的"内部世界模型"。

Conclusion: VidNum-1.4K作为下一代数值视频智能的严格诊断测试平台，揭示了当前视觉语言模型在真实数值推理能力上的不足，为模型理解现实世界动态提供了重要评估工具。
...
TL;DR: XSeg：最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，覆盖30个常见违禁品类别，并提出了基于SAM的Adaptive Point SAM (APSAM)标注模型来提升标注效率和精度。
...
Method: 1) 构建XSeg数据集：整合公开数据集和合成数据，通过自定义数据清洗管道过滤低质量样本；2) 提出Adaptive Point SAM (APSAM)：基于SAM架构，引入Energy-Aware Encoder增强掩码解码器初始化，改进堆叠物体检测能力；3) 设计Adaptive Point Generator：仅需单个粗略点提示即可获得精确掩码标签。

Result: XSeg是目前最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，覆盖30个常见违禁品类别。APSAM在XSeg上表现出优越性能，显著提升了标注效率和精度，特别是在处理堆叠物体方面。

Conclusion: XSeg数据集和APSAM标注模型为X射线违禁品检测提供了重要的数据资源和工具支持，解决了现有方法缺乏像素级监督和数据不足的问题，有望推动该领域的研究进展。
...
TL;DR: 该研究提出了一种基于图学习的黑色素瘤检测方法，使用超像素集合图(SEG)和超像素层次图(SHG)两种图表示，结合手工和学习的边权重分配技术，在ISIC2017数据集上实现了99.00%的准确率和99.59%的AUC。


<details>
  <summary>Details</summary>
Motivation: 传统图信号处理(GSP)技术中的图结构和边权重通常通过统计和计算方法设定，而图结构学习方法能提供更可靠和灵活的数据表示。本研究旨在开发一种基于图学习的黑色素瘤检测方法，利用皮肤病变图像的图表示来提高检测性能。

Method: 提出两种图表示：超像素集合图(SEG)和超像素层次图(SHG)。生成多级超像素图(20,40,60,80,100个节点)，探索两种边权重分配技术：手工高斯权重和基于优化的学习权重。节点信号基于纹理、几何和颜色超像素特征。研究图边阈值化(25%,50%,75%)对性能的影响。在ISIC2017数据集上使用不同分类器进行实验，并通过数据增强缓解类别不平衡。

Result: 实验结果表明，使用纹理节点信号的超像素集合图(SEG)与学习边权重结合时性能最佳，达到99.00%的准确率和99.59%的AUC。图边阈值化分析显示适当的剪枝可以改善性能。

Conclusion: 基于图学习的方法在黑色素瘤检测中表现出卓越性能，特别是超像素集合图与学习边权重的组合。该方法为皮肤病变分析提供了有效的图表示框架，证明了图结构学习在医学图像分析中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法通常只能控制相机运动或物体动态中的一种，或者依赖模糊的2D线索，这些线索会将相机引起的视差与真实物体运动混为一谈。需要一种能够统一控制两种运动类型的框架，以实现更连贯和富有表现力的视频生成。

Method: 提出SymphoMotion统一运动控制框架，包含两个核心机制：1）相机轨迹控制机制，整合显式相机路径和几何感知线索，确保稳定、结构一致的视角转换；2）物体动态控制机制，结合2D视觉引导和3D轨迹嵌入，实现深度感知、空间一致的物体操控。同时构建RealCOD-25K数据集，包含配对相机位姿和物体级3D轨迹，支持大规模训练和评估。

Result: 大量实验和用户研究表明，SymphoMotion在视觉保真度、相机可控性和物体运动准确性方面显著优于现有方法，为统一运动控制建立了新的基准。代码和数据已公开。

Conclusion: SymphoMotion通过统一控制相机轨迹和物体动态，解决了视频生成中运动控制的局限性，为连贯、富有表现力的视频生成提供了有效解决方案，并在真实世界数据集上验证了其优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前多参考角色视频生成研究中，当参考图像外观高度相似时，模型容易出现参考混淆问题，语义相似token会干扰正确上下文的检索

Method: 引入PoCo（Position Embedding as a Context Controller），利用位置编码作为额外上下文控制，通过token的边信息实现精确的token级匹配，同时保持隐式语义一致性建模

Result: 实验表明PoCo相比多种基线方法，在跨镜头一致性和参考保真度方面均有显著提升

Conclusion: PoCo方法有效解决了多参考视频生成中的参考混淆问题，能够可靠控制视觉特征极其相似的角色，为学术研究提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 传统μ子断层扫描方法（如POCA、MLSD）仅依赖μ子散射角，忽略了次级电磁簇射多重性这一重要信息。本研究旨在开发一种能够同时利用散射运动学和簇射多重性的深度学习架构，以提高钢筋混凝土结构缺陷检测的精度和可靠性。

Method: 提出SA-DSVN 3D卷积架构，包含两个独立的编码器流：一个处理散射运动学（9通道），另一个处理次级电磁簇射多重性（40通道）。两个流通过交叉注意力机制融合。使用云原生Geant4模拟框架Vega生成训练数据，包含450万个μ子事件和900个体积，涵盖蜂窝、剪切断裂、腐蚀空洞和分层四种缺陷类型。进行了五变体消融研究以评估不同特征的重要性。

Result: 在60个独立模拟验证体积上，模型达到96.3%的体素精度，各缺陷类型的Dice分数为0.59-0.81，体积级检测灵敏度为100%，每个体积推理时间为10毫秒。消融研究表明，簇射多重性流单独贡献了大部分判别能力，将缺陷平均Dice从0.535（仅散射）提升至0.685（仅簇射）。

Conclusion: 次级电磁簇射多重性是一个先前未被充分利用但非常有效的特征，对于学习的μ子断层扫描重建具有重要意义。SA-DSVN架构通过联合处理散射运动学和簇射多重性，显著提高了钢筋混凝土结构缺陷检测的性能，为无损检测领域提供了新的解决方案。
...
TL;DR: ICBench是一个新的大规模图像描述基准，包含2K图像的40K描述，涵盖12个内容类别，通过10个先进MLLM生成长短描述，并引入基于图像-文本-图像重建一致性的自动评估指标ITIScore。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述基准存在描述长度多样性有限、缺少最新先进MLLM、人类标注不足等问题，导致评估偏差且无法全面评估现代MLLM性能。

Method: 构建ICBench基准：使用10个先进MLLM在2K图像上生成长短描述，共40K描述，涵盖12个内容类别。进行广泛人类主观研究获取MOS评分，短描述评估流畅性、相关性和简洁性，长描述评估流畅性、相关性和完整性。提出ITIScore自动评估指标，基于图像-文本-图像框架通过重建一致性衡量描述质量。

Result: 实验结果表明，ITIScore自动指标与人类判断高度一致，在其他公共描述数据集上展现出强大的零样本泛化能力。

Conclusion: ICBench解决了现有基准的局限性，提供了更全面的图像描述评估框架，ITIScore作为自动评估指标与人类判断对齐良好，数据集和模型将在发表后发布。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D风格迁移方法依赖固定参考图像，但在VR/AR等实际应用中，用户需要更灵活的输入方式（如文本描述和多样化图像）。现有方法在风格一致性和处理多模态输入方面存在局限。

Method: 1. 使用3D高斯泼溅（3DGS）作为3D表示；2. 利用CLIP精炼的多模态知识作为参考风格；3. 采用细分流（subdivisive flow）进行精确特征对齐，将CLIP文本-视觉组合特征映射到VGG风格特征；4. 引入观察损失（observation loss）确保风格化场景与参考风格匹配；5. 引入抑制损失（suppression loss）抑制解码过程中参考颜色信息的偏移。

Result: M2StyleGS能够以文本或图像为参考生成一组风格增强的新视角序列。实验表明，该方法在视觉质量上表现优异，在一致性方面比先前工作提升高达32.92%。

Conclusion: M2StyleGS通过结合3DGS表示、多模态CLIP知识和创新的损失函数，实现了支持文本和图像输入的实时3D风格迁移，解决了异常变换问题，在风格一致性和视觉质量上显著优于现有方法。
...
TL;DR: 提出自动化细化框架，利用粗糙CityGML模型作为几何先验，通过表面匹配和二进制整数优化选择最优面片，在保持拓扑有效性的同时提升立面几何精度


<details>
  <summary>Details</summary>
Motivation: 传统基于机载激光扫描的CityGML建筑模型存在几何缺陷，特别是立面精度不足，需要与移动激光扫描数据集成以恢复详细立面几何，同时避免丢弃现有语义信息

Method: 利用粗糙模型作为几何先验，通过表面匹配识别过时表面，采用二进制整数优化选择候选数据中的最优面片，并强制执行硬约束确保拓扑有效性

Result: 有效修正立面错位，将点云到网格的RMSE降低约36%，实现厘米级对齐，同时保证严格水密且流形的几何体
...
<details>
  <summary>Details</summary>
Motivation: 标准自回归模型的下一令牌预测与文本条件运动生成所需的时间结构不对齐，需要更合适的层次化因果结构来处理长程运动结构
...
TL;DR: 提出基于注意力分布熵的补丁剪枝方法，通过香农熵和Rényi熵评估补丁重要性，在细粒度图像识别任务中减少计算量同时保持精度


<details>
  <summary>Details</summary>
Motivation: Transformer的自注意力机制虽然能捕获长距离依赖，但其计算成本随token数量呈二次方增长。补丁剪枝通过移除冗余补丁来降低计算成本，但需要有效的方法来识别信息丰富的补丁

Method: 提出基于注意力分布熵的补丁重要性评估准则：1) 使用香农熵，低熵补丁（注意力集中）保留为重要，高熵补丁（注意力分散）视为冗余；2) 扩展到Rényi熵，强调尖锐的注意力峰值，支持适应任务需求和计算限制的剪枝策略

Result: 在细粒度图像识别任务中，该方法在保持精度的同时减少了计算量。通过Rényi熵调整剪枝策略可进一步获得收益，改善精度与计算之间的权衡

Conclusion: 基于熵的补丁重要性评估为Transformer模型提供了一种有效的补丁剪枝方法，通过香农熵和Rényi熵准则，能够在减少计算成本的同时保持模型性能，特别适用于需要精细补丁选择的细粒度视觉任务
...
TL;DR: EyeBench-V2：一个用于眼底图像增强的临床对齐基准，通过多维度下游任务评估和专家指导设计，弥补传统评估指标与临床实用性之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有眼底图像增强模型的评估存在三个主要问题：1) 传统去噪指标（如PSNR、SSIM）无法捕捉临床相关特征（如病变保留、血管形态一致性）；2) 缺乏统一评估协议来同时处理配对和非配对增强方法；3) 需要提供可操作的见解来指导临床对齐增强模型的未来发展。

Method: 提出EyeBench-V2基准，包含三个关键贡献：1) 通过下游任务进行多维度临床对齐评估，包括血管分割、糖尿病视网膜病变分级、未见噪声模式泛化和病变分割；2) 专家指导的评估设计，构建新数据集支持配对和非配对方法的公平比较，并建立结构化的人工评估协议；3) 提供可操作的见解，对现有生成模型进行严格的任务导向分析。

Result: EyeBench-V2基准填补了增强模型性能与临床效用之间的差距，提供了全面的评估框架，能够评估模型在临床相关任务上的表现，并通过专家评估确保临床相关性。

Conclusion: EyeBench-V2是一个重要的基准工具，能够为临床研究人员提供证据支持决策，同时识别当前方法的局限性，为下一代增强模型的设计提供指导，促进眼底图像增强技术的临床转化。
...
TL;DR: InCaRPose：基于Transformer的相机外参标定方法，专门针对车内监控等高度畸变环境，使用合成数据训练，能在单次推理中实现绝对度量尺度的位姿估计。
...
Method: 提出InCaRPose架构，基于Transformer设计，利用冻结的DINOv3等主干特征和Transformer解码器，专门处理鱼眼相机的高度畸变问题，完全使用合成数据训练。

Result: 模型能在单次推理中实现绝对度量尺度的平移估计，泛化到真实车内环境，在公开7-Scenes数据集上达到竞争性性能，即使使用ViT-Small主干也能保持高精度，支持实时推理。

Conclusion: InCaRPose为高度畸变环境下的相机外参标定提供了有效解决方案，通过合成数据训练实现真实世界泛化，满足车内监控等安全关键应用的实时性能要求。
...
TL;DR: 论文提出了首个大规模视频活动伪造定位基准ActivityForensics，包含6K+伪造视频片段，并提出了基于扩散特征正则化的TADiff方法，用于检测修改人类动作的活动级伪造视频。


<details>
  <summary>Details</summary>
Motivation: 现有伪造检测基准主要关注外观级伪造（如人脸交换、物体移除），而视频生成技术的进步催生了活动级伪造，这种伪造通过修改人类动作来扭曲事件语义，具有高度欺骗性，严重威胁媒体真实性和公众信任。目前缺乏专门针对活动级伪造定位的基准数据集。

Method: 1) 构建ActivityForensics基准：包含6K+伪造视频片段，这些片段无缝融入视频上下文，具有高视觉一致性；2) 提出Temporal Artifact Diffuser (TADiff)：通过扩散特征正则器暴露伪影线索的基线方法；3) 建立评估协议：涵盖域内、跨域和开放世界设置。

Result: ActivityForensics基准包含大量高质量伪造视频，人类视觉难以区分。基于该基准评估了多种最先进的伪造定位方法，为未来研究提供了基础。TADiff方法在活动伪造定位任务中表现出有效性。

Conclusion: ActivityForensics填补了活动级视频伪造定位基准的空白，提出的TADiff方法为检测高度欺骗性的活动伪造提供了有效基线。该工作将推动视频伪造检测领域的发展，特别是针对新兴的活动级伪造威胁。
...
TL;DR: SPARK-IL：基于检索增强和增量学习的双路径频谱分析框架，用于跨生成器AI图像检测，在19个生成模型上达到94.6%平均准确率


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在特定生成器上训练后难以泛化到未见模型，而像素级伪影因模型而异，但频域特征具有更高一致性，为跨生成器检测提供了基础

Method: 提出SPARK-IL框架：1) 双路径分析：使用部分冻结的ViT-L/14编码器提取语义表示，并行路径处理原始RGB像素嵌入；2) 多频带傅里叶分解：将两个路径分解为四个频带，分别由Kolmogorov-Arnold Networks（KAN）配合专家混合进行频带特定变换；3) 交叉注意力融合：通过交叉注意力与残差连接融合频谱嵌入；4) 检索增强推理：使用余弦相似度从Milvus数据库中检索k个最近邻标记签名，通过多数投票进行预测；5) 增量学习：扩展数据库并使用弹性权重巩固保留先前学习的变换

Result: 在UniversalFakeDetect基准测试中，涵盖19个生成模型（包括GAN、人脸交换和扩散方法），SPARK-IL实现了94.6%的平均准确率

Conclusion: SPARK-IL通过结合双路径频谱分析、检索增强推理和增量学习，有效解决了跨生成器AI图像检测的泛化问题，在多个生成模型上表现出优异的检测性能
...
TL;DR: 提出基于任务引导的多标注三元组损失，通过互信息准则选择跨任务信息量最大的三元组，替代静态权重平衡，提升共享表征效果


<details>
  <summary>Details</summary>
Motivation: 现有多任务三元组损失方法依赖静态权重平衡不同标注类型的监督，但静态权重需要调参且无法考虑任务间在塑造共享表征时的交互作用

Method: 提出任务引导的多标注三元组损失，通过互信息准则选择跨任务信息量最大的三元组，修改影响表征的样本而非调整损失幅度
...
TL;DR: 该论文提出了一种半监督知识蒸馏框架，用于将预训练视觉基础模型压缩为紧凑的专家模型，在实例分割任务上使用有限标注数据和大量未标注数据实现高效压缩。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型虽然感知能力强但计算量过大难以部署，且适应这些模型通常需要昂贵的标注成本。特别是在实例分割任务中，像素级标注尤为昂贵，因此需要一种能够利用有限标注和大量未标注数据来压缩基础模型的方法。

Method: 提出三阶段半监督知识蒸馏框架：1) 通过自训练和对比校准进行域适应；2) 通过统一多目标损失进行知识转移；3) 学生模型精炼以减轻伪标签偏差。核心是实例感知的像素级对比损失，融合掩码和类别分数来提取信息负样本并强制清晰的实例间边界。

Result: 在Cityscapes和ADE20K数据集上，约11倍压缩的学生模型相比零样本基础模型教师分别提升+11.9和+8.6 AP，相比适应后的教师模型分别提升+3.4和+1.5 AP，并在半监督知识蒸馏基准上超越最先进方法。

Conclusion: 该半监督知识蒸馏框架能够有效压缩视觉基础模型，在保持高性能的同时显著减少计算开销，通过对比学习机制充分利用未标注数据，为部署高效的实例分割模型提供了可行方案。
...
TL;DR: SafeCtrl：一种区域感知的安全控制框架，通过"检测-抑制"范式在文本到图像扩散模型中防止有害内容生成，在保持安全性和内容保真度之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型广泛部署面临生成视觉有害内容（如色情、暴力、恐怖图像）的挑战。现有安全干预方法存在两个关键局限：1）安全性与上下文保真度之间的严重权衡；2）容易受到对抗性攻击。
...
Result: 在多个风险类别上的广泛实验表明，SafeCtrl在安全性和保真度之间实现了比现有最先进方法更优的权衡。关键的是，该方法对对抗性提示攻击表现出更强的鲁棒性。

Conclusion: SafeCtrl提供了一种精确且鲁棒的解决方案，用于负责任的内容生成，通过局部化干预有效解决了现有安全方法的局限性。
...
TL;DR: CM-GLasso：通过跨模态图套索和共同-特定结构学习，解决多模态表示中高维噪声、模态不对齐和拓扑混淆问题，在生成分类和密集语义分割任务上达到SOTA


<details>
  <summary>Details</summary>
Motivation: 传统稀疏图估计方法（如Graphical Lasso）在多模态视觉-语言领域面临三大挑战：高维噪声、模态不对齐、共享拓扑与类别特定拓扑的混淆。这些限制阻碍了可解释多模态表示的学习。

Method: 提出跨模态图套索（CM-GLasso）：1）通过文本-可视化策略和统一视觉-语言编码器将多模态特征严格对齐到共享潜在空间；2）引入跨注意力蒸馏机制，将高维图像块压缩为显式语义节点，提取空间感知的跨模态先验；3）将定制化GLasso估计和共同-特定结构学习（CSSL）统一到联合目标中，通过ADMM优化，同时解耦不变性和类别特定的精度矩阵。

Result: 在涵盖自然和医疗领域的八个基准测试上进行广泛实验，CM-GLasso在生成分类和密集语义分割任务上建立了新的最先进水平。

Conclusion: CM-GLasso克服了传统方法在多模态表示学习中的基本限制，通过严格的模态对齐、跨模态先验提取和联合优化框架，实现了可解释的多模态表示学习，并在多个任务上取得了SOTA性能。
...
TL;DR: VLA-Forget：一种用于视觉-语言-动作模型的混合遗忘框架，通过选择性编辑和分层遗忘实现安全行为移除，同时保持感知、推理和任务执行能力


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作中作为具身基础模型出现，但部署时面临新的遗忘挑战：需要移除不安全、虚假或隐私敏感行为，同时不降低感知、语言理解和动作控制能力。传统遗忘方法仅针对视觉或语言模块，在VLA模型中效果有限，可能导致残留遗忘或不必要的性能损失。

Method: 提出VLA-Forget混合遗忘框架：1）对感知和跨模态对齐采用比例感知选择性编辑；2）对推理/动作层采用分层选择性遗忘；3）通过视觉编码器、投影器和上层动作生成Transformer块的阶段性更新，联合优化三个目标：针对性遗忘、感知保留和推理保持。

Result: 在遗忘集行为探测和保留任务评估中，VLA-Forget相比强基线：遗忘效果提升10%，感知特异性保留提升22%，推理和任务成功率保留提升9%，量化后恢复率降低55%。

Conclusion: VLA-Forget为VLA模型提供了一种有效的遗忘框架，能够针对性地移除不良行为，同时保持模型的感知、语言理解和动作控制能力，解决了具身AI系统中的安全遗忘问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数高效提示学习方法主要关注一阶视觉特征（空间特征图）与文本提示的对齐，但这些空间纠缠的特征对领域偏移和局部噪声高度敏感，仅依赖一阶信息不足以实现鲁棒适应。

Method: 提出Gram-Anchored Prompt Learning (GAPL)框架，通过Gram矩阵引入额外的二阶统计流，增强标准的一阶空间交互。该方法将提示锚定到这些二阶先验上，使语言表示能够动态适应不同领域的统计分布偏移。

Result: 大量实验验证了二阶特征的有效性，GAPL在各种基准测试中表现出色。

Conclusion: 通过结合局部语义对齐与全局结构一致性，基于二阶统计量的提示学习方法能够显著提升视觉语言模型在下游任务中的鲁棒性和适应性。
...
TL;DR: HMAT：一种用于古代壁画高保真修复的统一框架，结合了局部纹理建模和长距离结构推理，通过动态滤波、条件风格融合和教师强制解码器实现


<details>
  <summary>Details</summary>
Motivation: 古代壁画作为珍贵文化遗产，因环境暴露、材料老化和人类活动而严重退化。修复工作面临挑战，需要同时重建大面积缺失结构并严格保护原始完好区域

Method: 提出混合掩码感知Transformer（HMAT）框架：1）掩码感知动态滤波用于鲁棒的局部纹理建模；2）Transformer瓶颈用于长距离结构推理；3）掩码条件风格融合模块动态引导生成过程；4）教师强制解码器配合硬门控跳跃连接，强制有效区域保真度并聚焦缺失区域重建

Result: 在DHMural数据集和精心策划的九色鹿数据集上评估，HMAT在不同退化级别下均表现出色，相比最先进方法具有竞争力，能产生结构更连贯、视觉更忠实的修复结果

Conclusion: HMAT为文化遗产壁画的数字修复提供了有效解决方案，通过统一框架实现了局部纹理建模和全局结构推理的平衡，在保持原始区域保真度的同时有效重建缺失结构
...
TL;DR: 多模态排版攻击：通过协调音频、视觉和文本扰动，显著提升对多模态大语言模型的攻击成功率（83.43% vs 34.93%），揭示了多模态模型的跨模态脆弱性。
...
Result: 在多模态攻击下，攻击成功率从单模态的34.93%显著提升至83.43%。研究覆盖多个前沿MLLMs、任务以及常识推理和内容审核基准，证实多模态排版攻击是一种关键且未被充分探索的攻击策略。

Conclusion: 多模态排版攻击揭示了多模态大语言模型的跨模态脆弱性，协调的多模态攻击比单模态攻击更为有效，这对多模态推理系统的安全性提出了重要警示。
...
TL;DR: MINERUPro通过数据工程和训练策略优化（而非架构创新），在固定1.2B参数架构下显著提升文档解析性能，在OmniDocBench-v1.6上达到95.69分


<details>
  <summary>Details</summary>
Motivation: 当前文档解析方法主要竞争模型架构创新，而训练数据的系统工程探索不足。不同架构和参数规模的SOTA模型在同一组困难样本上表现出高度一致的失败模式，表明性能瓶颈源于训练数据的共同缺陷而非架构本身
...
Result: 在评估方面，修复了OmniDocBench-v1.5中的元素匹配偏差并引入Hard子集，建立更具区分度的OmniDocBench-v1.6协议。在没有任何架构修改的情况下，MINERUPro在OmniDocBench-v1.6上达到95.69分，比相同架构基线提高2.71分，并超越所有现有方法（包括参数超过200倍的模型）

Conclusion: 通过系统性数据工程和训练策略优化，可以在固定架构下显著提升文档解析性能，表明当前性能瓶颈主要源于训练数据质量而非模型架构，为文档解析领域提供了新的研究方向
...
TL;DR: 提出OASIC方法，通过估计遮挡严重程度、掩蔽遮挡物并选择针对该严重程度优化的模型，显著提升严重遮挡下的物体分类性能。


<details>
  <summary>Details</summary>
Motivation: 严重遮挡是计算机视觉的主要挑战，根源在于可见信息丢失和遮挡物造成的干扰模式。现有方法未能同时解决这两个问题。

Method: 提出OASIC框架：1) 测试时通过掩蔽去除遮挡物干扰模式；2) 训练时使用随机掩蔽模拟不同遮挡程度；3) 测试时估计遮挡严重程度并选择针对该程度优化的模型。

Result: 结合灰度掩蔽和自适应模型选择，相比标准训练在遮挡图像上提升AUC_occ 18.5点，相比在未遮挡图像上微调提升23.7点。

Conclusion: 通过同时处理遮挡干扰和可见信息丢失，并基于遮挡严重程度自适应选择模型，OASIC方法在严重遮挡场景下显著优于传统方法。
...
TL;DR: HOIGS是一种用于重建动态场景中复杂人-物交互的Gaussian Splatting方法，通过交叉注意力HOI模块显式建模交互引起的变形，使用HexPlane和CHS分别提取人和物的特征，在遮挡、接触和物体操作场景中实现更精确的重建。


<details>
  <summary>Details</summary>
Motivation: 现有Gaussian Splatting方法要么依赖人体姿态先验而忽略动态物体，要么将所有运动近似为单一场，限制了捕捉交互丰富动态的能力。需要一种能够显式建模人-物交互引起变形的解决方案。

Method: 提出Human-Object Interaction Gaussian Splatting (HOIGS)，通过交叉注意力HOI模块显式建模交互引起的变形。采用异构特征提取：HexPlane用于人体特征提取，Cubic Hermite Spline (CHS)用于物体特征提取。通过集成这些异构特征来捕捉相互依赖的运动。

Result: 在多个数据集上的综合实验表明，该方法在涉及遮挡、接触和物体操作的场景中持续优于最先进的人体中心和4D Gaussian方法，显著改善了变形估计。

Conclusion: 显式建模人-物交互对于高保真重建至关重要，HOIGS通过异构特征提取和交叉注意力机制有效捕捉了交互引起的变形，为动态场景重建提供了更精确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型生成高质量文本到图像结果，但迭代去噪计算成本高。DMD作为少步蒸馏的有前景路径，在减少到两步或更少时会出现多样性崩溃和保真度下降问题。需要突破整数步限制，建立1.x步生成作为蒸馏扩散模型的实用方案。

Method: 1. 分析DMD中教师CFG被忽视的作用，引入简单有效的修改来抑制模式崩溃；2. 提出阶段化聚焦蒸馏：第一阶段通过多样性保持分布匹配学习粗粒度结构，第二阶段通过推理一致对抗蒸馏细化细节；3. 设计轻量补偿模块用于蒸馏-缓存协同训练，将块级缓存自然融入蒸馏流程。

Result: 在SD3-Medium和SD3.5-Large上的实验表明，1.x-Distill超越先前少步方法，在1.67和1.74有效NFEs下分别实现更好的质量和多样性，比原始28x2 NFE采样快达33倍。

Conclusion: 1.x-Distill是首个分数步蒸馏框架，突破了传统整数步限制，建立了1.x步生成作为蒸馏扩散模型的实用方案，通过抑制模式崩溃、阶段化聚焦蒸馏和蒸馏-缓存协同训练，在极低步数下实现了高质量和高多样性的图像生成。
...
<details>
  <summary>Details</summary>
Motivation: 当前最强的视觉语言模型虽然展现出广泛的视觉推理能力，但其实现方法不透明，被锁定在非公开的强化学习流程和专有数据中。需要开发完全开源的视觉推理器，能够跨图表、科学、空间理解和开放任务工作

Method: 1. 构建Vero-600K数据集：从59个数据集中收集600K样本，覆盖六个广泛的任务类别；2. 设计任务路由奖励机制：处理异构答案格式；3. 使用强化学习进行训练；4. 基于Qwen3-VL-8B-Instruct等基础模型进行训练

Result: 1. 在VeroEval的30个挑战性基准测试中，Vero比四个基础模型平均提升3.7-5.5分；2. 基于Qwen3-VL-8B-Instruct的Vero在30个基准中的23个上超越了Qwen3-VL-8B-Thinking，且无需额外的专有思维数据；3. 从相同基础模型训练时，Vero-600K在所有任务类别上都超越了现有的RL数据集

Conclusion: 广泛的数椐覆盖是强化学习扩展的主要驱动力，不同任务类别引发质上不同的推理模式，这些模式在孤立情况下转移效果差。所有数据、代码和模型都已开源发布
...
Method: 提出ATSS方法，利用AI生成视频中存在的异常时间自相似性指纹。该方法通过三重相似性表示和跨注意力融合机制，构建视觉、文本和跨模态相似性矩阵来量化时间异常，使用专用Transformer编码器编码并通过双向跨注意力融合模块集成

Result: 在GenVideo、EvalCrafter、VideoPhy和VidProM四个大规模基准测试中，ATSS在AP、AUC和ACC指标上显著优于最先进方法，展现出对不同视频生成模型的优越泛化能力

Conclusion: ATSS通过检测AI生成视频中由确定性锚点驱动轨迹引起的异常时间自相似性，提供了一种有效的多模态检测框架，显著提升了伪造视频检测性能
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉里程计系统通常依赖启发式特征提取策略，这在大规模室外环境中会降低精度和鲁棒性。需要一种能够有效泛化到不同场景的端到端解决方案。

Method: 1. 引入可微分自适应补丁选择器，提高提取补丁的质量；2. 集成多任务特征提取模块；3. 结合可微分光束法平差模块，利用逆深度先验；4. 将特征学习与状态估计相结合。

Result: 在TartanAir、KITTI、Euroc和TUM数据集上的广泛实验表明，DINO-VO在合成、室内和室外环境中都表现出强大的泛化能力，达到了最先进的跟踪精度。

Conclusion: DINO-VO通过端到端学习框架，成功解决了传统VO系统在场景泛化方面的局限性，为单目视觉里程计提供了更鲁棒和准确的解决方案。
...
TL;DR: 4C4D：一种从极稀疏相机（最少4个）视频中恢复4D动态场景的新框架，通过神经衰减函数增强4D高斯几何建模能力，实现高质量4D高斯溅射


<details>
  <summary>Details</summary>
Motivation: 现有方法需要密集多视角捕获（数十甚至上百个相机），限制了动态场景建模的实际应用。本文旨在解决从极稀疏相机（最少4个）视频中恢复4D动态场景的挑战，实现高质量的新视角渲染。

Method: 提出4C4D框架，核心创新是引入高斯不透明度的神经衰减函数，增强4D高斯的几何建模能力。该设计通过让4DGS梯度更关注几何学习，缓解几何与外观建模之间的固有不平衡。

Result: 在具有不同相机重叠度的稀疏视角数据集上进行广泛实验，4C4D在性能上优于现有技术，实现了高质量的4D高斯溅射。

Conclusion: 4C4D框架成功解决了从极稀疏相机视频中恢复4D动态场景的挑战，通过神经衰减函数有效增强了几何建模能力，为稀疏视角下的动态场景重建提供了有效解决方案。
...
### [77] [Detecting Media Clones in Cultural Repositories Using a Positive Unlabeled Learning Approach](https://arxiv.org/abs/2604.04071)
...
<details>
  <summary>Details</summary>
Motivation: 解决AtticPOT存储库中策展人在环重复发现问题，传统方法需要显式负例且缺乏透明度，需要一种避免显式负例、提供透明操作点的方法来适应策展人在环工作流程

Method: 将重复发现问题形式化为正例-无标签学习问题，每个文物使用单个锚点，通过锚点的增强视图训练轻量级每查询克隆编码器，在潜在l_2范数上使用可解释阈值对未标记存储库进行评分

Result: 在CIFAR-10上获得F1=96.37（AUROC=97.97）；在AtticPOT上达到F1=90.79（AUROC=98.99），比最佳基线（SVDD）在相同轻量级骨干下F1提高+7.70分；定性"查找相似"面板显示跨视角和条件的稳定邻域

Conclusion: 该方法避免显式负例，提供透明操作点，适用于去重、记录链接和策展人在环工作流程，能够发现未经验证的跨记录重复项
...
TL;DR: 本文提出了一种基于YOLOv11和BoT-SORT/ByteTrack的离线实时交通监控系统，通过Qt桌面界面实现车辆检测与计数，在多种场景下达到66.67-95.83%的计数准确率。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能驱动的计算机视觉技术发展，交通监控系统得到显著提升。本文旨在开发一个离线、实时的交通监控系统，利用深度学习目标检测与跟踪技术，为智慧城市建设提供不依赖云端的现代化解决方案。

Method: 系统采用预训练的YOLOv11检测器结合BoT-SORT/ByteTrack多目标跟踪算法，使用PyTorch/OpenCV实现核心处理流程，并通过Qt框架构建桌面用户界面。整个CNN管道能够从视频流中高效检测和计数车辆，无需云端依赖。

Result: 在多样化场景中，系统达到66.67-95.83%的计数准确率。类别检测方面：汽车精度0.97-1.00，召回率0.82-1.00，F1分数0.90-1.00；卡车精度1.00，召回率0.70-1.00，F1分数0.82-1.00。在典型条件下性能稳健，但恶劣天气可能影响表现。

Conclusion: 通过集成轻量级模型与可访问的云端独立界面，本文展示了AI驱动交通监控系统的能力，为智慧城市的现代化和发展做出了贡献。系统在典型条件下表现优异，为未来智能交通监控提供了实用解决方案。
...
TL;DR: HGR框架通过可修正的假设节点和级联纠错机制，解决视觉语言模型在导航中语义预测错误累积的问题，提升长期记忆可靠性


<details>
  <summary>Details</summary>
Motivation: 现有基于图的导航系统将未探索区域视为语义未知，导致低效的前沿搜索。视觉语言模型可以预测前沿语义，但错误预测会嵌入记忆并通过下游推理传播，形成结构错误累积，仅靠置信度衰减无法解决。需要既能利用语义预测进行定向探索，又能在新证据矛盾时系统撤回错误的框架。

Method: 提出假设图精炼(HGR)框架：1) 语义假设模块：估计前沿的上下文条件语义分布，根据目标相关性、旅行成本和不确定性对探索目标排序；2) 验证驱动的级联纠错：比较现场观测与预测语义，一旦不匹配，撤回被证伪节点及其所有下游依赖节点。与加法式地图构建不同，该框架通过剪除错误子图使图收缩，保持长期记忆可靠性。

Result: 在GOAT-Bench上达到72.41%成功率和56.22% SPL，在A-EQA和EM-EQA问答基准上均显示持续改进。诊断分析显示级联纠错消除了约20%的结构冗余假设节点，将错误区域的重访减少4.5倍，镜面和透明表面占修正预测错误的67%。

Conclusion: HGR框架通过将前沿预测表示为依赖感知图记忆中的可修正假设节点，实现了语义预测的利用与错误修正的平衡。验证驱动的级联纠错机制能有效防止错误传播，保持长期记忆可靠性，在复杂导航任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 开发在真实世界恶劣条件下（极端低光和烟雾退化）鲁棒的3D重建管道，解决现有方法在退化场景下的性能限制

Method: 使用RealX3D基准数据集，组织挑战赛收集279名参与者提交的33个有效方法，系统评估并与最先进基线比较

Result: 挑战赛显示在恶劣条件下的3D重建取得显著进展，揭示了顶级方法的共同设计原则和有效处理3D场景退化的策略

Conclusion: 挑战赛推动了恶劣条件下3D重建技术的发展，为未来研究提供了重要见解和设计指南
...
TL;DR: 提出一种针对空间非均匀曝光校正的新范式，通过空间自适应调制权重和多查找表转换，配合HSL补偿模块和不确定性引导的非均匀损失，显著提升真实世界曝光校正效果。


<details>
  <summary>Details</summary>
Motivation: 真实世界曝光校正面临空间非均匀退化的根本挑战，即单张图像中经常同时存在多种曝光误差。然而现有方法大多基于均匀假设开发，架构上依赖全局聚合调制信号仅捕捉整体曝光趋势，优化上传统重建损失基于全局尺度，忽视了区域间空间变化的校正需求。

Method: 1) 引入空间信号编码器预测空间自适应调制权重，指导多个查找表进行图像转换；2) 采用HSL补偿模块提升色彩保真度；3) 提出不确定性引导的非均匀损失，根据局部恢复不确定性动态分配优化焦点。

Result: 大量实验表明，该方法在定性和定量性能上均优于现有最先进方法，代码已开源。

Conclusion: 该研究提出了一个专门针对空间非均匀性的曝光校正新范式，通过空间自适应调制架构和不确定性引导的优化策略，有效解决了真实世界曝光校正中空间变化误差的挑战。
...
TL;DR: OP-GRPO：首个针对流匹配模型的离线策略GRPO框架，通过重用高质量轨迹、序列级重要性采样修正和轨迹截断，将训练步数减少至34.2%的同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: GRPO在提升流匹配模型生成质量方面表现出色，但其在线策略训练范式导致样本效率低下。需要开发更高效的训练方法来解决这一限制。

Method: 1. 主动选择高质量轨迹并自适应地将其纳入回放缓冲区以供后续训练重用；2. 提出序列级重要性采样修正，在保持GRPO剪裁机制完整性的同时缓解分布偏移；3. 理论分析和实验表明后期去噪步骤会产生病态离线策略比率，通过截断后期步骤的轨迹来缓解此问题。

Result: 在图像和视频生成基准测试中，OP-GRPO仅需平均34.2%的训练步数就能达到与Flow-GRPO相当或更优的性能，在保持生成质量的同时显著提升训练效率。

Conclusion: OP-GRPO是首个针对流匹配模型的离线策略GRPO框架，通过创新的轨迹重用、重要性采样修正和轨迹截断机制，显著提高了训练效率，为流匹配模型的强化学习训练提供了更实用的解决方案。
...
TL;DR: 提出了一种用于遥感时空融合回归任务的基于不确定性感知的测试时自适应框架，通过仅更新预训练模型的融合模块，在不依赖源数据或目标标签的情况下，显著提升了模型在未见地理区域的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在遥感应用中面临跨地理区域泛化困难的问题，主要原因是训练区域与目标区域之间的域偏移。现有测试时自适应方法主要针对分类任务设计，不适用于回归任务，特别是时空融合这样的回归任务。

Method: 提出了一个不确定性感知的测试时自适应框架，专门用于地表温度估计的时空融合回归任务。该方法仅更新预训练STF模型的融合模块，通过三个关键指导：认知不确定性、土地利用/覆盖一致性以及偏差校正。不需要源数据或标记的目标样本。

Result: 在意大利罗马、埃及开罗、西班牙马德里和法国蒙彼利埃四个不同气候的目标区域进行实验，相比在法国奥尔良预训练的模型，RMSE和MAE分别平均提升了24.2%和27.9%。即使使用有限的无标签目标数据和仅10个TTA训练轮次也取得了显著改进。

Conclusion: 提出的不确定性感知测试时自适应框架有效解决了遥感回归任务中的域偏移问题，显著提升了预训练模型在未见地理区域的泛化能力，为遥感回归任务的域自适应提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 字体形状能唤起广泛印象，但字体与印象描述并非一一对应：某些印象与多样式兼容，而其他印象则强烈约束字体选择。这种分级约束强度称为样式特异性，需要量化建模

Method: 提出双曲共嵌入框架，将字体图像和印象描述（单标签或标签集）嵌入共享双曲空间，施加两种互补蕴含约束：印象到字体蕴含，以及低到高样式特异性印象间的蕴含。该公式诱导径向结构，低特异性印象靠近原点，高特异性印象远离原点

Result: 在MyFonts数据集上实验显示，相比强一对一基线方法，双向检索性能提升。遍历和标签级分析表明，学习到的空间捕获了从模糊到更样式特异性印象的连贯进展，提供了有意义的数据驱动样式特异性量化

Conclusion: 双曲共嵌入框架成功建模字体-印象对应关系，通过蕴含约束而非简单对齐，提供了解释性几何度量来量化印象对字体样式的约束强度，为字体设计和选择提供了新视角
...
TL;DR: 提出SCSD方法解决双缺失多视图多标签学习问题，通过共享码本学习离散一致表示，基于标签相关性结构评估视图权重进行决策融合，并采用融合教师自蒸馏框架提升缺失标签条件下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有多视图多标签学习方法主要依赖对比学习或信息瓶颈理论在缺失视图条件下学习一致表示，但缺乏显式结构约束的损失对齐限制了捕获稳定且具有判别性的共享语义的能力。双缺失场景（视图和标签都不完整）的研究仍很缺乏。

Method: 1) 通过多视图共享码本和跨视图重构学习离散一致表示，在有限的共享码本嵌入中自然对齐不同视图并减少特征冗余；2) 设计权重估计方法评估各视图保留标签相关性结构的能力，据此分配权重以增强融合预测质量；3) 引入融合教师自蒸馏框架，用融合预测指导视图特定分类器训练，将全局知识反馈到单视图分支，提升缺失标签条件下的泛化能力。

Result: 在五个基准数据集上与先进方法进行广泛对比实验，充分证明了所提方法的有效性。代码已开源。

Conclusion: 提出的SCSD方法通过结构化一致表示学习、基于标签相关性的决策融合和自蒸馏框架，有效解决了双缺失多视图多标签学习问题，在视图和标签都不完整的场景下表现出优越性能。
...
TL;DR: GENFIG1是一个评估生成式AI模型根据论文内容（标题、摘要、引言、图注）生成"Figure 1"能力的基准测试，要求模型理解科学概念、识别关键点并设计有效视觉表达。


<details>
  <summary>Details</summary>
Motivation: 科学论文中的"Figure 1"通常作为核心研究思想的视觉总结，这些图视觉简洁但概念丰富，需要作者大量迭代才能完成，体现了科学视觉传达的难度。这启发了GENFIG1基准的创建，用于评估生成式AI模型在科学视觉沟通方面的能力。

Method: 从顶级深度学习会议论文中收集数据，进行严格质量控制，引入与专家判断相关性良好的自动评估指标。评估一系列代表性模型在GENFIG1上的表现，要求模型：(i)理解论文技术概念，(ii)识别最显著的概念，(iii)设计连贯美观且忠实于输入的图形。

Result: GENFIG1基准测试表明，即使对于性能最佳的系统，该任务也提出了显著挑战。自动评估指标与专家人类判断有良好相关性。

Conclusion: GENFIG1基准为多模态AI的未来进展奠定了基础，展示了科学视觉生成任务的难度，并希望推动生成式AI在科学沟通方面的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型大多为离线系统，不适合需要持续观察和及时响应的实时视频流处理。当前的流式VideoLLMs方法依赖解耦的触发-响应流程或仅限于字幕式叙述，限制了其在开放式问答和长时程交互中的有效性。

Method: AURA是一个端到端的流式视觉交互框架，集成了上下文管理、数据构建、训练目标和部署优化，支持稳定的长时程流式交互。框架采用统一的VideoLLM持续处理视频流，支持实时问答和主动响应。

Result: 在流式基准测试中达到最先进的性能，支持实时演示系统，在2个80G加速器上以2FPS运行，包含自动语音识别和文本转语音功能。

Conclusion: AURA为流式视频理解提供了有效的端到端解决方案，支持实时交互和主动响应，作者发布了模型和实时推理框架以促进未来研究。
...
TL;DR: GDB是首个针对专业平面设计任务的综合基准套件，涵盖布局、排版、信息图表、模板与设计语义、动画五大维度，评估AI模型在真实设计任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注自然图像理解或通用文本到图像合成，缺乏针对专业平面设计独特挑战的评估标准。专业设计需要将沟通意图转化为结构化布局、渲染忠实排版文本、操作分层组合、生成有效矢量图形以及动画推理等能力。

Method: 构建包含50个任务的基准套件，按五个维度组织：布局、排版、信息图表、模板与设计语义、动画。每个维度在理解和生成两种设置下评估，基于LICA分层组合数据集的真实设计模板。使用标准化指标分类法评估空间准确性、感知质量、文本保真度、语义对齐和结构有效性。

Result: 评估显示当前模型在专业设计的核心挑战上表现不足：复杂布局的空间推理、忠实矢量代码生成、细粒度排版感知、动画的时间分解等问题基本未解决。虽然高层语义理解可达要求，但随着任务对精度、结构和组合意识要求的提高，差距显著扩大。

Conclusion: GDB为追踪AI系统向有能力的设计协作伙伴发展提供了严谨、可复现的测试平台。当前模型在专业设计任务上仍有明显差距，特别是在需要精确性、结构性和组合意识的场景中。该评估框架已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于世界模型的规划方法在跨数据集和传感器配置的泛化能力有限，且其松散耦合的规划范式导致视觉想象过程中的视频-轨迹一致性较差。

Method: 提出DriveVA模型，采用基于DiT的解码器联合预测未来动作序列（轨迹）和视频，引入视频延续策略以增强长时滚动的连贯性，利用大规模预训练视频生成模型的运动动力学和物理合理性先验。

Result: 在NAVSIM挑战中达到90.9 PDM分数的闭环性能，在nuScenes上平均L2误差和碰撞率分别降低78.9%和83.3%，在基于CARLA v2的Bench2drive上分别降低52.5%和52.4%，展示了零样本能力和跨域泛化能力。

Conclusion: DriveVA通过联合解码视觉预测和动作序列，实现了更好的规划与场景演化对齐，显著提升了自动驾驶世界模型的泛化性能和视频-轨迹一致性。
...
TL;DR: HighFM：首个面向高时间分辨率多光谱地球观测数据的基础模型，利用Meteosat第二代卫星的SEVIRI数据，通过改进的掩码自编码框架学习稳健的时空表征，在云掩码和活跃火点检测任务上优于传统方法和现有地理空间基础模型。


<details>
  <summary>Details</summary>
Motivation: 气候相关灾害日益频繁和严重，需要实时监测、早期预警和知情决策。现有基础模型主要依赖高分辨率但低重访率的卫星影像，不适合快速演变现象和时间关键型应急响应。因此需要开发针对高时间分辨率地球观测数据的基础模型。

Method: 提出HighFM模型，利用超过2TB的Meteosat第二代卫星SEVIRI影像数据，改进SatMAE掩码自编码框架以学习稳健的时空表征。为支持实时监测，增强原始架构，加入细粒度时间编码以捕捉短期变化。预训练模型在云掩码和活跃火点检测任务上进行微调。

Result: SEVIRI预训练的Vision Transformers在平衡准确率和IoU指标上均优于传统基线和近期地理空间基础模型，在云掩码和活跃火点检测任务上均取得一致性能提升。

Conclusion: 高时间分辨率地球静止数据在实时地球观测中具有巨大潜力，为灾害检测和跟踪的基础模型提供了可扩展路径。HighFM展示了利用密集时间序列数据构建专门基础模型的价值。
...
TL;DR: GA-GS是一种基于生成辅助的高斯溅射方法，用于从包含动态对象的单目视频中重建静态3D场景，通过扩散模型修复被遮挡区域并引入可学习的真实性标量实现平衡监督。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖背景进行静态场景重建，无法有效恢复被动态对象遮挡的区域，这限制了在虚拟现实和自动驾驶等应用中的实用性。

Method: 1) 使用运动感知模块分割并移除动态区域；2) 利用扩散模型修复被遮挡区域，提供伪真值监督；3) 为每个高斯基元引入可学习的真实性标量，动态调整溅射过程中的不透明度，实现真实性感知的渲染和监督；4) 构建Trajectory-Match数据集，使用固定路径机器人记录每个场景有无动态对象的视频，支持定量评估。

Result: 在DAVIS数据集和自建Trajectory-Match数据集上的大量实验表明，GA-GS在静态场景重建方面达到了最先进的性能，特别是在具有大规模、持久遮挡的挑战性场景中表现优异。

Conclusion: GA-GS通过生成辅助方法有效解决了动态对象遮挡下的静态场景重建问题，提出的真实性标量机制平衡了真实背景和生成区域的贡献，为相关应用提供了更完整的场景重建能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统CLIP方法将所有不匹配样本视为同等负样本，忽略了地理空间关系。地理定位任务需要考虑空间自相关（Tobler地理第一定律），即距离越近的事物相关性越强。
...
Conclusion: 研究强调了从语义对齐转向地理对齐对于鲁棒地理定位的重要性，为将空间原理整合到多模态表示学习提供了一个通用范式。
...
Method: 提出RLER双范式：1) RLER-Training使用群体相对强化学习和三个任务驱动奖励（帧敏感奖励、思维透明奖励、抗重复奖励）训练模型生成结构化、可机器检查的证据；2) RLER-Inference采用无需训练的编排器生成多样化候选答案，解析答案和引用帧，基于证据一致性、置信度、透明度和非冗余性进行评分，最后进行证据加权选举。
...
TL;DR: 本文提出了UENR-600K数据集，这是一个包含60万对1080p帧的大规模物理基础夜间视频去雨数据集，通过Unreal Engine模拟3D雨滴粒子，解决了现有合成数据集无法捕捉夜间雨滴与人工照明交互的问题。


<details>
  <summary>Details</summary>
Motivation: 夜间视频去雨面临独特挑战：雨滴与人工照明交互，呈现多种颜色和局部照明效果。现有小规模合成数据集使用2D雨滴叠加，无法捕捉这些物理特性，导致模型在真实夜间雨景中泛化能力差。同时，捕获真实配对夜间视频不切实际，因为雨效无法与其他退化（如传感器噪声）分离。

Method: 使用Unreal Engine将雨模拟为虚拟环境中的3D粒子，确保照片级真实感和物理真实的雨滴，捕捉颜色折射、场景遮挡、雨幕等正确细节。基于此高质量数据，通过适配Wan 2.2视频生成模型建立新的SOTA基线，将去雨视为视频到视频生成任务，利用强生成先验几乎完全弥合仿真到真实的差距。

Result: 创建了包含600,000对1080p帧的大规模物理基础数据集UENR-600K。基准测试表明，在该数据集上训练的模型在真实世界视频中泛化能力显著更好，几乎完全弥合了仿真到真实的差距。

Conclusion: UENR-600K数据集通过物理基础的3D雨滴模拟解决了夜间视频去雨的独特挑战，基于此建立的视频生成方法显著提升了模型在真实夜间雨景中的泛化性能，为夜间视频去雨研究提供了高质量基准。
...
TL;DR: 该论文提出了BoxComm数据集，这是首个专注于格斗运动（拳击）的大规模评论生成基准，包含445场世界拳击锦标赛视频和52K专业评论句子，并引入了类别条件生成和评论节奏评估两种新评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在通用视频理解方面表现出色，但现有的体育评论生成基准仅关注团队运动（如足球、篮球），完全忽略了格斗运动。格斗运动具有独特挑战：关键动作在毫秒内发生，视觉差异细微但语义决定性；专业评论中战术分析比例远高于团队运动。需要专门的数据集和评估方法来填补这一空白。

Method: 1. 构建BoxComm数据集：包含445场世界拳击锦标赛视频和超过52K条专业广播评论句子。2. 提出结构化评论分类法：将每句评论分为逐场播报、战术分析或背景信息三类，首次为体育评论基准提供类别级标注。3. 引入两种新颖评估方法：类别条件生成（评估模型能否根据视频上下文生成指定类型的准确评论）和评论节奏评估（评估自由生成评论在连续视频片段中是否具有适当的时间节奏和类型分布）。4. 提出改进基线EIC-Gen：结合检测到的击打事件提供结构化动作线索。

Result: 实验表明，多个最先进的多模态大语言模型在两种评估上都表现不佳。提出的EIC-Gen基线通过结合检测到的击打事件提供结构化动作线索，在所有评估指标上取得一致提升，突显了感知短暂细微事件对格斗运动评论的重要性。

Conclusion: BoxComm填补了格斗运动评论生成基准的空白，提出的结构化分类法和两种评估方法为体育评论生成提供了更全面的评估框架。当前模型在格斗运动评论生成方面仍有困难，特别是感知短暂细微事件的能力不足。结合结构化动作线索的方法显示出改进潜力，为未来研究提供了方向。
...
Method: 提出HandDreamer方法：1) 使用MANO手部模型初始化提供结构先验；2) 引入手部骨架引导的扩散过程确保视角和姿态一致性；3) 提出新颖的校正手部形状引导损失，确保所有视角收敛到一致的模态而不产生几何失真。

Result: 广泛评估表明该方法在3D手部模型生成方面优于现有最先进方法，为3D手部模型生成开辟了新途径。

Conclusion: HandDreamer是首个从文本提示生成零样本3D手部模型的有效方法，通过结构先验和形状引导解决了视角不一致问题，在虚拟现实交互中具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法在大规模预训练后在通用场景表现良好，但在领域迁移任务中性能严重下降。这源于领域特定任务中类别标签稀缺且语义弱，以及现有模型无法捕获粗粒度类别标签之外的辅助语义。

Method: 提出HSA-DINO框架：1）多尺度提示库利用图像特征金字塔捕获层次化语义并选择领域特定的局部语义提示，从粗到细粒度逐步丰富文本表示；2）语义感知路由器在推理时动态选择适当的语义增强策略，防止参数更新降低预训练模型的泛化能力。

Result: 在OV-COCO、多个垂直领域数据集和修改的基准设置上评估，HSA-DINO相比先前最先进方法表现更优，在领域适应性和开放词汇泛化能力之间实现了更好的平衡。
...
TL;DR: Chorus是一种利用跨请求相似性的缓存方法，加速视频扩散模型推理，在4步蒸馏模型上实现高达45%的加速


<details>
  <summary>Details</summary>
Motivation: 视频扩散Transformer模型虽然能生成高质量视频，但迭代去噪过程导致推理成本高昂。现有缓存方法主要利用单个请求内扩散过程的相似性来跳过冗余去噪步骤，但在工业4步蒸馏模型上效果有限

Method: Chorus采用三阶段缓存策略：1) 对相似请求的潜在特征进行完全复用；2) 在中间去噪步骤中对特定潜在区域进行跨请求缓存，结合Token-Guided Attention Amplification增强生成视频与条件提示的语义对齐；3) 扩展完全复用到后期去噪步骤

Result: 在工业4步蒸馏模型上实现高达45%的加速，而先前的请求内缓存方法在此类模型上无效

Conclusion: Chorus通过利用跨请求相似性，有效加速视频扩散模型服务，特别是在现有方法无效的蒸馏模型上表现出显著性能提升
...
Method: 提出了系统化的视觉语言模型评估框架，在多种下游任务（零样本图像分类、语义分割、视觉问答）上评估了CLIP、鲁棒CLIP、BLIP2和SigLIP2等模型，使用精心策划的对抗数据集（排版攻击、ImageNet-A和自然语言诱导对抗样本）。

Result: 分析发现：1）鲁棒CLIP模型可能放大自然对抗漏洞；2）CLIP模型在自然语言诱导对抗样本上性能显著下降；3）提供了可解释性分析以识别失败模式。

Conclusion: 该研究揭示了视觉语言模型在自然对抗场景下的脆弱性，希望这些发现能启发未来在鲁棒和公平的多模态模式识别方面的研究。
...
TL;DR: VicoEdit：一种无需训练和反演的视觉上下文感知图像编辑方法，通过直接转换源图像并采用概念对齐的后验采样来提升编辑一致性和灵活性。
...
Method: 提出VicoEdit方法：1) 无需反演，直接基于视觉上下文将源图像转换为目标图像，避免反演导致的轨迹偏差；2) 设计基于概念对齐的后验采样方法，通过增强编辑一致性来指导采样过程。

Result: 实验结果表明，这种无需训练的方法在编辑性能上甚至优于最先进的基于训练的模型，在保持视觉上下文一致性的同时实现了高质量的图像编辑。

Conclusion: VicoEdit成功实现了无需训练和反演的视觉上下文感知图像编辑，通过直接转换和概念对齐后验采样解决了现有方法的局限性，为图像编辑提供了更高效灵活的解决方案。
...
TL;DR: 提出基于补丁增强和跨视图规律性的统一防御框架，通过特征表示和输出分布层面约束模型对触发模式的异常行为，在低中毒率下有效抑制后门攻击成功率同时保持正常生成能力


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在监督微调过程中容易植入后门，一旦特定触发模式被激活就会稳定输出攻击者预定义的有害响应。后门防御的核心挑战在于低中毒率下抑制攻击成功率的同时保持模型正常生成能力，这两个目标存在内在冲突

Method: 提出基于补丁增强和跨视图规律性的统一防御框架：1) 补丁级数据增强与跨视图输出差异正则化结合，利用后门响应对非语义扰动异常不变性，主动拉开原始视图和扰动视图的输出分布；2) 通过施加输出熵约束避免防御过程中的过度抑制，确保正常指令生成质量
...
TL;DR: 提出Indra表示假设，认为单模态基础模型学习到的收敛表示实际上反映了现实中的共享关系结构，类似于因陀罗网的哲学隐喻。通过范畴论中的V-富集Yoneda嵌入形式化该假设，定义Indra表示为样本相对于其他样本的关系轮廓，并在跨模型和跨模态场景中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究发现单模态基础模型倾向于学习收敛表示，但这些表示本质上是样本的独立内部抽象，表达能力有限。作者认为这些表示实际上收敛于反映现实共享关系结构的隐含表示，需要从关系角度重新理解基础模型的表示学习。

Method: 提出Indra表示假设，使用范畴论中的V-富集Yoneda嵌入形式化该假设，定义Indra表示为每个样本相对于其他样本的关系轮廓。具体实现使用角距离实例化Indra表示，在视觉、语言和音频的跨模型和跨模态场景中进行评估。

Result: 大量实验表明，Indra表示在不同架构和模态中一致地增强了鲁棒性和对齐性，为单模态基础模型的无训练对齐提供了理论依据和实用框架。在跨模型和跨模态场景中均表现出优越性能。

Conclusion: Indra表示假设为理解基础模型表示学习的收敛现象提供了新的理论视角，表明这些表示实际上反映了现实中的共享关系结构。该方法为跨模型和跨模态对齐提供了无需训练的实用解决方案，具有理论和实践双重意义。
...
TL;DR: Saliency-R1：通过新颖的显著性映射技术和GRPO优化，提升视觉语言模型推理的可解释性和忠实性


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型存在过度依赖文本线索而非视觉证据的问题，以及产生无根据或虚构回答的风险，需要提高其可信度

Method: 提出Saliency-R1框架：1）新颖的显著性映射技术，高效突出对生成token贡献的关键图像区域；2）扩展该技术以追踪视觉信息在推理过程中的流向；3）使用显著性映射与人工标注边界框的重叠作为奖励函数；4）应用Group Relative Policy Optimization（GRPO）对齐显著部分与关键区域

Result: 实验表明Saliency-R1提高了推理的忠实性、可解释性和整体任务性能

Conclusion: Saliency-R1框架有效解决了VLM的信任问题，通过增强视觉证据的利用和推理过程的可追溯性，提升了模型的可靠性和可解释性
...
TL;DR: MedROI是一种与编解码器无关的ROI中心框架，通过去除医学影像中的非诊断背景区域来提升压缩效率，无需修改现有编解码器架构。


<details>
  <summary>Details</summary>
Motivation: 医学影像档案在尺寸和分辨率上快速增长，现有编解码器要么压缩整个图像（包含非诊断背景），要么采用差分ROI编码但仍保留背景比特，导致存储和数据传输效率低下。

Method: 提出MedROI框架：1）通过轻量级基于强度的阈值处理提取紧密的组织边界框；2）存储54字节元数据记录以支持解压时的空间恢复；3）裁剪后的ROI可使用任何现有2D或3D编解码器压缩，无需架构修改或重新训练。

Result: 在200个ADNI T1加权脑MRI体积上使用6种编解码器配置测试，MedROI在大多数配置中显著提升压缩比和编码/解码时间（经多重比较校正的双侧t检验），同时在ROI内保持可比较的重建质量；HEIF在压缩比增益方面是主要例外。

Conclusion: MedROI是一种有效的插件式ROI中心压缩框架，能显著提升医学影像压缩效率，同时保持诊断质量，适用于多种现有编解码器。
...
TL;DR: StableTTA是一种无需训练的集成方法，通过解决聚合策略中的冲突来提高预测稳定性和效率，在ImageNet-1K上实现10.93-32.82%的top-1准确率提升，使轻量级架构以更少参数和计算量超越ViT。


<details>
  <summary>Details</summary>
Motivation: 集成方法虽然能提升预测性能，但通常以增加内存使用和计算复杂度为代价。本文发现聚合策略中存在冲突，会负面影响预测稳定性，因此需要一种既能保持性能又能降低资源消耗的解决方案。

Method: 提出StableTTA，一种无需训练的方法来改善聚合稳定性和效率。该方法通过优化集成模型的聚合策略来解决稳定性问题，同时保持计算效率。

Result: 在ImageNet-1K上实现10.93-32.82%的top-1准确率提升，33个模型达到超过95%准确率，多个模型超过96%。轻量级架构以少于5%的参数和约89.1%的计算量减少，在top-1准确率上超越ViT 11.75%。

Conclusion: StableTTA有效解决了集成方法中聚合策略的稳定性问题，显著提升了预测性能，同时大幅降低了计算和内存需求，使得在资源受限设备上实现高精度推理成为可能。
...
<details>
  <summary>Details</summary>
Motivation: 视觉SLAM中相对相机位姿估计面临噪声对应点的挑战。传统方法依赖随机假设采样和迭代估计，而基于学习的方法通常缺乏显式几何结构。需要一种结合几何推理和关系共识的方法来提升估计的鲁棒性。

Method: 将相对位姿估计重新表述为极线对应图上的关系推理问题：匹配关键点作为节点，附近点通过边连接。通过图操作（剪枝、消息传递、池化）估计四元数旋转、平移向量和本质矩阵。使用包含五个分量的损失函数：L2误差、Frobenius范数、奇异值差异、航向角差异和尺度差异。采用无检测器的密集匹配方法LoFTR进行特征匹配。

Result: 在室内外基准测试中，相比传统方法和学习引导方法，该方法对密集噪声和大基线变化表现出更强的鲁棒性。实验结果表明全局关系共识的有效性。

Conclusion: 通过将相对位姿估计重新表述为图关系推理问题，该方法结合了几何结构和关系共识的优势，在挑战性场景下实现了更鲁棒的估计性能，为视觉SLAM提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学基础模型大多孤立分析放射影像，忽略了临床实践中对比前后影像评估间隔变化的关键任务。对于胸片而言，捕捉间隔变化至关重要，因为放射科医生不仅需要评估发现的静态外观，还需要评估其随时间演变的情况。

Method: 提出TILA（时间反转感知学习与对齐）框架，使用时间反转（反转图像对）作为监督信号，增强现有时序视觉语言模型对方向性变化的敏感性。该框架在预训练、微调和推理阶段集成反转感知目标，通过显式学习时间顺序来补充传统外观建模。同时提出了统一的评估协议来评估时间反转下的顺序敏感性和一致性，并引入了MS-CXR-Tretrieval检索评估集。

Result: 在公共数据集和真实医院队列上的实验表明，TILA在应用于多种现有架构时，能持续改善进展分类和时间嵌入对齐性能。

Conclusion: TILA是一个简单而有效的框架，通过时间反转监督信号显著增强了医学视觉语言模型对胸片间隔变化的敏感性，为临床时序影像分析提供了重要改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有OCT/OCTA图像分析方法严重依赖大量数据和模型规模，在资源受限的临床环境中难以部署。基于基础模型的迁移学习虽然前景广阔，但仍面临领域偏移和任务不对齐的挑战。

Method: 提出TAPE两阶段适应框架：1）领域适应阶段：在掩码图像建模的背景下应用参数高效微调进行医学图像领域适应；2）任务拟合阶段：将适应过程解耦为领域对齐和下游分割任务拟合。应用于通用（MAE）和专用（RETFound）基础模型。

Result: TAPE在视网膜层分割任务上表现出卓越的参数效率，并在多种病理情况下实现了最先进的泛化性能。在通用和专用基础模型上都取得了优异结果。

Conclusion: TAPE框架通过两阶段参数高效微调策略，有效解决了医学图像分析中的领域偏移和任务不对齐问题，为资源受限的临床环境提供了实用的OCT/OCTA图像分析解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 先前工作主要评估遗忘的成功率，但对更广泛生成能力的影响了解不足，需要系统研究概念遗忘对组合文本到图像生成的影响

Method: 以Stable Diffusion 1.4中的裸露内容移除为案例，使用T2I-CompBench++和GenEval等基准，评估多种最先进的遗忘方法，分析遗忘效果与组合完整性之间的关系

Result: 发现一致的权衡关系：实现强遗忘效果的方法经常导致属性绑定、空间推理和计数能力显著退化；而保持组合结构的方法往往无法提供稳健的遗忘效果

Conclusion: 当前评估实践存在局限性，需要开发能够明确考虑超出目标抑制的语义保留的遗忘目标
...
TL;DR: 提出PR-IQA框架，用于评估扩散模型生成的稀疏视角新视图，无需真实标注，通过部分参考图像质量评估和补全机制，提升3D高斯溅射重建质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型可用于稀疏视角新视图合成，为3D高斯溅射等重建管道生成伪真实视图，但生成的图像存在光度学和几何不一致性，直接用于监督会损害重建质量。

Method: 提出部分参考图像质量评估（PR-IQA）框架：1）在重叠区域计算几何一致的部分质量图；2）通过交叉注意力机制结合参考视图上下文，将部分质量图补全为密集全图像质量图；3）在扩散增强的3DGS管道中，仅对高质量置信区域进行监督。

Result: PR-IQA在无需真实标注的情况下达到全参考级别的准确度，优于现有IQA方法。质量感知的3DGS方法能更有效过滤不一致性，产生更优的3D重建和新视图合成结果。

Conclusion: PR-IQA框架通过部分参考图像质量评估和补全机制，有效解决了扩散生成视图的不一致性问题，显著提升了3D高斯溅射的重建质量，为稀疏视角新视图合成提供了有效的质量评估和监督策略。
...
TL;DR: Firebolt-VL是一种高效视觉语言模型，使用液态基础模型解码器替代Transformer，并引入令牌-网格相关模块增强视觉定位，实现线性时间推理和细粒度理解。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型计算成本高，难以部署到资源受限场景；同时小型视觉语言模型在细粒度推理任务中表现不佳，难以精确捕捉任务相关的视觉区域。

Method: 1) 使用液态基础模型解码器替代传统的基于Transformer的解码器；2) 提出令牌-网格相关模块，计算文本令牌与图像块之间的轻量级相关性，并通过状态空间模型和FiLM条件进行调制，选择性强调与文本提示相关的视觉区域。

Result: 在多个基准测试中，Firebolt-VL实现了准确、细粒度的理解，同时显著提高了效率，保持了线性时间推理。

Conclusion: Firebolt-VL通过创新的架构设计解决了现有视觉语言模型的效率和细粒度理解问题，为资源受限场景下的部署提供了可行方案。
...
TL;DR: 该论文提出了一种基于物理特征的方法来检测AI生成图像，通过分析15种物理特征并筛选出5个核心特征，将其与CLIP模型结合，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: AIGC的快速发展模糊了真实与合成图像的界限，现有深度伪造检测器往往过度拟合特定生成模型，存在适应性危机。需要重新审视区分自然图像与AI生成图像的内在物理特征。

Method: 1. 全面探索15种物理特征在20多个由GAN和扩散模型生成的数据集上的表现；2. 提出新颖的特征选择算法，识别出5个核心物理特征（拉普拉斯方差、Sobel统计量、残差噪声方差等）；3. 将这些特征转换为文本编码值，与语义描述结合，指导CLIP中的图像-文本表示学习。

Result: 在多个Genimage基准测试中达到最先进性能，在Wukong和SDv1.4等数据集上获得接近完美的准确率（99.8%）。物理特征表现出跨数据集的稳定判别能力。

Conclusion: 通过将像素级真实性特征与语义理解相结合，该工作开创了基于物理特征的可信视觉语言建模方法，为缓解大型多模态模型中的幻觉和文本不准确问题开辟了新方向。
...
TL;DR: GLA是一种针对自动驾驶视觉语言模型的新型后门攻击方法，使用涂鸦式视觉触发器和跨语言文本触发器，在低污染率下实现高攻击成功率，且不影响模型在干净任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型被集成到自动驾驶等安全关键系统中，后门攻击成为重要安全威胁。现有后门攻击主要依赖单模态、显式且易检测的触发器，难以在自动驾驶场景中构建隐蔽且稳定的攻击通道。

Method: GLA引入两种自然主义触发器：1) 基于稳定扩散修复生成的涂鸦式视觉模式，无缝融入城市场景；2) 跨语言文本触发器，在保持语义一致性的同时引入分布偏移，构建鲁棒的语言侧触发信号。

Result: 在DriveVLM上的实验表明，GLA仅需10%的污染率即可达到90%的攻击成功率(ASR)和0%的误报率(FPR)。后门不会削弱模型在干净任务上的性能，反而提高了BLEU-1等指标，使传统基于性能下降的检测方法难以识别攻击。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数异常检测方法都是针对特定领域的专家模型，需要大量目标数据集训练，难以泛化到未见数据集。通用异常检测(GAD)范式旨在学习单一通用模型，无需重新训练即可跨领域检测异常。

Method: InCTRLv2扩展了之前的InCTRL模型，采用双分支框架：1) 主分支的判别式异常评分学习(DASL)，使用正常和异常数据学习语义引导的异常与正常空间；2) 辅助分支的单类异常评分学习(OASL)，仅使用正常数据在语义空间中学习广义正常模式。两个分支都由大规模视觉语言模型编码的丰富视觉-文本语义先验指导。

Result: 在十个异常检测数据集上的广泛实验表明，InCTRLv2在各种设置下，在异常检测和分割任务中都实现了最先进的性能。

Conclusion: InCTRLv2通过双分支架构提供双重语义视角的异常检测：一个强调正常-异常区分，另一个强调偏离正常的语义，为通用异常检测提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型的快速发展使得合成媒体高度逼真，引发严重的社会担忧。现有检测方法存在关键局限：依赖固定分辨率调整和裁剪等预处理操作，这些操作不仅丢弃了细微的高频伪造痕迹，还导致空间失真和信息丢失。此外，现有方法通常在过时数据集上训练评估，无法捕捉现代生成模型的复杂性。

Method: 提出全面的数据集和新型检测框架：1) 构建大规模数据集，包含来自15种最先进开源和商业生成器的超过14万视频，以及专门用于评估超逼真合成内容的Magic Videos基准；2) 提出基于Qwen2.5-VL Vision Transformer的新型检测框架，该框架原生支持可变空间分辨率和时间长度，有效保留传统预处理中丢失的高频伪影和时空不一致性。

Result: 大量实验表明，该方法在多个基准测试中实现了卓越性能，证明了原生尺度处理的关键重要性，并为AI生成视频检测建立了稳健的新基线。

Conclusion: 原生尺度处理对于有效检测AI生成视频至关重要，提出的框架通过保留高频伪造痕迹显著提升了检测性能，为应对合成媒体带来的社会挑战提供了更可靠的解决方案。
...
TL;DR: Synthesis4AD：利用大规模高保真合成异常数据提升3D工业异常检测性能的端到端范式，通过3D-DefectStudio平台和MPAS可控合成引擎生成几何逼真的缺陷，结合MLLM自动生成异常合成指令，并通过空间分布归一化和几何保真数据增强提升检测器鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 工业3D异常检测面临异常样本稀缺和长尾分布的根本性限制，这严重制约了检测性能。现有方法难以获取足够数量和多样性的真实异常数据，导致模型学习不充分，无法形成具有判别性的特征表示。

Method: 1. 提出Synthesis4AD端到端范式，核心是3D-DefectStudio软件平台，基于MPAS可控合成引擎，通过高维支撑基元指导注入几何逼真的缺陷，同时生成精确的点级异常掩码。2. 集成多模态大语言模型（MLLM）解析产品设计信息，自动转换为可执行的异常合成指令，实现可扩展的知识驱动异常数据生成。3. 引入基于空间分布归一化和几何保真数据增强的训练流程，缓解Point Transformer架构对绝对坐标的敏感性，提升在真实数据变化下的特征学习能力。

Result: 在Real3D-AD、MulSen-AD和真实工业零件数据集上的大量实验表明，该方法实现了最先进的性能。MPAS合成方法和交互系统3D-DefectStudio将在GitHub上公开。

Conclusion: Synthesis4AD通过大规模高保真合成异常数据有效解决了工业3D异常检测中的数据稀缺问题，结合知识驱动的合成策略和鲁棒的训练流程，显著提升了异常检测的判别能力和泛化性能，为工业质量检测提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 实时从超高分辨率无人机图像重建深度对于灾害响应等时间关键地理空间任务至关重要，但面临宽基线视差、大图像尺寸、低纹理或镜面表面、遮挡和严格计算约束等挑战。现有零样本扩散模型虽然无需任务特定训练，但其概率推理无法保证度量精度和时序一致性。

Method: 提出ZeD-MAP框架，将测试时扩散深度模型转换为度量一致的SLAM式映射管线。流式无人机帧被分组为重叠集群；周期性束调整产生度量一致的姿态和稀疏3D连接点，这些点被重投影到选定帧中，用作扩散深度估计的度量引导。

Result: 在约50米高度（地面采样距离约0.85厘米/像素）的DLR模块化航空相机系统飞行数据验证中，方法实现亚米级精度：水平面误差约0.87米，垂直方向误差约0.12米，每帧处理时间在1.47至4.91秒之间。

Conclusion: 束调整为基础的度量引导提供了与经典摄影测量方法相当的几何一致性，同时显著加速处理速度，实现了实时3D地图生成，为时间关键地理空间应用提供了可行解决方案。
...
TL;DR: 该论文提出DenZa-Gaussian方法，通过3D高斯泼溅技术改进稀疏视角ADF-STEM断层扫描，解决传统方法在有限视角下的重建退化问题。


<details>
  <summary>Details</summary>
Motivation: ADF-STEM断层扫描需要多视角倾斜系列图像进行3D重建，但增加视角会延长电子束暴露时间，损害剂量敏感材料并引入漂移和错位。稀疏视角采集虽常用，但传统ADF-STEM方法在有限视角下性能退化，产生伪影并降低结构保真度。

Method: 将3D高斯泼溅技术应用于该领域，包含三个关键组件：1) 将局部散射强度建模为可学习标量场denza，解决3DGS与ADF-STEM成像物理不匹配问题；2) 引入系数γ稳定不同倾斜角度的散射，通过散射视角归一化确保denza一致性；3) 加入包含2D傅里叶振幅项的损失函数，抑制稀疏视角重建中的缺失楔形伪影。

Result: 在45视角和15视角倾斜系列上的实验表明，DenZa-Gaussian能够产生高保真度重建结果和2D投影，与原始倾斜图像更吻合，在稀疏视角条件下表现出优越的鲁棒性。

Conclusion: DenZa-Gaussian方法成功解决了ADF-STEM断层扫描中稀疏视角重建的挑战，通过物理启发的建模和优化策略，在保持样本完整性的同时实现了高质量3D重建。
...
<details>
  <summary>Details</summary>
Motivation: 世界模型作为人工智能的重要研究方向，目前缺乏清晰统一的定义，阻碍了该领域的研究进展和应用发展。需要建立一个标准化的框架来促进世界模型的研究和实践。
...
Result: 成功开发了OpenWorldLib框架，为世界模型研究提供了标准化的推理平台。该框架支持跨任务模型集成，实现了模型的高效复用和协作推理能力。

Conclusion: OpenWorldLib为世界模型研究提供了重要的标准化框架，提出的明确定义和系统分类有助于推动该领域的进一步发展。论文还对世界模型研究的未来方向进行了深入分析和展望。
...
TL;DR: 提出自适应KV缓存量化方法，通过数据驱动的控制器动态分配KV缓存精度（2/4/8位或FP16），基于token重要性特征减少内存和延迟，同时保持接近FP16的精度。


<details>
  <summary>Details</summary>
Motivation: LLM在移动和边缘设备部署面临KV缓存内存和带宽开销大的挑战，现有固定精度量化方案浪费比特在低重要性token上，同时过度压缩高重要性token，导致精度损失。

Method: 提出自适应KV缓存量化框架：提取token级轻量特征（频率、质量分数、注意力方差、基于熵的不确定性），输入紧凑的数据驱动控制器，在解码时动态选择KV精度（2/4/8位或FP16）。

Result: 在SmolLM-135M/360M/1.7B模型和多个常识推理基准测试中，相比静态KV量化，解码延迟降低17.75%（SmolLM-360M在HellaSwag），精度提升7.60点，与FP16推理仅差0.30点。

Conclusion: 自适应KV缓存量化通过动态精度分配优化了精度-延迟权衡，为资源受限设备上的高效LLM推理提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在计数、空间推理、视角理解等基础视觉概念上表现不佳，但人工识别这些弱点成本高、不可扩展且存在偏见，容易忽略细微细节而关注显著对象

Method: 采用强化学习框架，训练提问者智能体根据候选VLM的响应自适应生成查询以引出错误答案，通过关注细粒度视觉细节和不同技能组合逐步增加问题复杂度

Result: 发现了36个新的VLM失败模式，展示了框架在各种模型组合上的广泛适用性和泛化能力

Conclusion: 提出的RL框架能够自动、系统地发现VLM的盲点和失败模式，为模型评估和弱点识别提供了可扩展的解决方案
...
TL;DR: 提出过程驱动的图像生成方法，将单步生成分解为多步迭代过程，包含文本规划、视觉草稿、文本反思和视觉细化四个阶段，通过密集的逐步监督确保中间状态的一致性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 人类绘画是增量式的过程：先规划全局布局，再绘制粗略草稿，检查并细化细节，每一步都基于当前的视觉状态。然而，在文本-图像交错数据集上训练的统一多模态模型是否也能想象中间状态的链条？本文旨在探索多步图像生成范式，使生成过程更加显式、可解释和可监督。

Method: 提出过程驱动的图像生成方法，将合成过程分解为交错进行的思维和行动轨迹。每个迭代包含四个阶段：1) 文本规划：基于当前视觉状态规划下一步；2) 视觉草稿：根据文本规划生成图像；3) 文本反思：评估生成的图像并识别问题；4) 视觉细化：修正识别出的问题。通过密集的逐步监督，对视觉中间状态施加空间和语义一致性约束，对文本中间状态保持先验视觉知识同时纠正违反提示的元素。

Result: 在多个文本到图像生成基准测试上进行了实验验证，证明了所提方法的有效性。该方法使生成过程变得显式、可解释，并且可以直接监督。

Conclusion: 过程驱动的图像生成方法成功地将单步生成范式转变为多步迭代过程，通过文本和视觉状态的交错演进，实现了更加可控、可解释的图像合成。该方法为多模态模型的中间推理能力提供了新的研究视角。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的图像退化（模糊、噪声、压缩、光照不佳）严重损害多模态理解。现有的统一多模态模型虽然结合了理解和生成能力，但未能利用自身生成能力处理退化输入，主要原因是训练机制从未要求模型在推理时调用生成，且标准的解码-重新编码路径不支持有效的联合优化。

Method: CLEAR框架包含三个渐进步骤：1）在退化感知数据集上进行监督微调，建立"先生成后回答"的推理模式；2）潜在表示桥接，用可优化的直接连接替代解码-重新编码的迂回路径；3）交错GRPO，一种在答案正确性奖励下联合优化文本推理和视觉生成的强化学习方法。构建了MMD-Bench基准，涵盖三个退化严重程度和六个标准多模态基准。

Result: 实验表明CLEAR显著提高了在退化输入上的鲁棒性，同时保持了干净图像性能。分析进一步揭示，移除像素级重建监督会导致具有更高感知质量的中间视觉状态，表明任务驱动的优化与视觉质量自然对齐。

Conclusion: CLEAR通过连接生成与推理能力，有效解决了多模态模型在退化图像上的性能问题。该方法不仅提升了鲁棒性，还揭示了任务驱动优化与视觉质量之间的内在一致性，为多模态模型的退化鲁棒性研究提供了新思路。
...
TL;DR: AvatarPointillist：从单张肖像图像生成动态4D高斯化身的创新框架，采用自回归Transformer生成点云用于3D高斯泼溅


<details>
  <summary>Details</summary>
Motivation: 需要从单张肖像图像生成高质量、可动画化的4D高斯化身，传统方法在点云生成和动画控制方面存在局限性

Method: 使用仅解码器Transformer自回归生成点云，动态调整点密度和总数；联合预测每点的绑定信息用于动画；专用高斯解码器将点转换为可渲染的高斯属性

Result: 生成高质量、逼真且可控的化身；自回归生成器与高斯解码器的条件交互显著提升保真度

Conclusion: 自回归公式代表了化身生成的新范式，框架将开源以促进未来研究
...
TL;DR: MMF-BEV是一个用于自动驾驶的雷达-相机BEV融合框架，通过可变形注意力机制实现跨模态特征对齐，在VoD数据集上优于单模态基线，并达到与现有融合方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要互补的传感器：相机提供密集语义但深度不可靠，毫米波雷达提供精确的距离和速度测量但几何稀疏。需要有效融合这两种模态以实现准确的3D目标检测。

Method: 提出MMF-BEV框架，包含BEVDepth相机分支和RadarBEVNet雷达分支，均增强可变形自注意力，通过可变形交叉注意力模块融合。采用两阶段训练策略：先用深度监督预训练相机分支，然后联合训练雷达和融合模块。

Result: 在VoD数据集上，MMF-BEV在相机、雷达和混合融合三种配置下均优于单模态基线，在所有物体类别和全标注区域及近距ROI区域均达到与现有融合方法竞争的结果。传感器贡献分析提供了可解释的模态权重证据。
...
TL;DR: E-VLA框架通过事件相机增强视觉-语言-动作模型，在恶劣视觉条件下（如低光、运动模糊）提升机器人操作的鲁棒性，避免图像重建而直接利用事件流中的运动和结构线索。


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的视觉-语言-动作模型在感知阶段退化（如极端低光、运动模糊、黑裁剪）时表现脆弱，需要提升在恶劣视觉条件下的操作鲁棒性。

Method: 提出E-VLA框架，直接利用事件流中的运动和结构线索而非重建图像；构建开源遥操作平台和真实世界RGB-事件-动作数据集；设计轻量级、预训练兼容的事件集成策略，研究事件窗口化和融合方法。

Result: 简单参数免费融合（事件图叠加到RGB图像）显著提升黑暗和模糊场景的鲁棒性：Pick-Place任务在20 lux下成功率从0%提升至60%（叠加融合）和90%（事件适配器）；严重运动模糊下Pick-Place从0%提升至20-25%，Sorting从5%提升至32.5%。

Conclusion: E-VLA系统性地证明了事件驱动感知可有效集成到VLA模型中，为实现超越传统帧基成像的鲁棒具身智能指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率细节有时会成为噪声，导致幻觉或推理错误。需要让模型专注于关键结构信息而非表面纹理
...
Conclusion: DDP通过策略性降低图像保真度和提供结构提示，有效提升VLMs在复杂视觉任务中的推理能力，验证了关注本质结构信息的重要性
...
TL;DR: 提出InfBaGel框架，通过粗到细的指令条件化交互生成，结合一致性模型和动态感知策略，实现人-物-场景交互的实时生成，同时通过混合训练策略解决数据稀缺问题。
...
Method: 1) 提出粗到细的指令条件化交互生成框架，与一致性模型的迭代去噪过程对齐；2) 采用动态感知策略，利用前一步精炼的轨迹更新场景上下文；3) 引入碰撞感知引导，减少物理伪影；4) 设计混合训练策略，通过向HOI数据集注入体素化场景占用合成伪HOSI样本，并与高保真HSI数据联合训练。

Result: 在HOSI和HOI生成任务上达到最先进性能，对未见场景有强泛化能力，支持实时生成。

Conclusion: InfBaGel框架有效解决了HOSI生成中的数据稀缺和物理合理性问题，通过动态感知和混合训练实现了高质量的交互生成。
...
TL;DR: 该论文首次系统研究了自动驾驶中视觉语言模型的灾难性遗忘问题，提出了新的基准数据集和Drive Expert Adapter框架，通过在提示空间而非权重空间进行适配，解决了性能提升与知识保留的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 将视觉语言模型集成到自动驾驶中虽然能解决长尾场景问题，但微调过程会侵蚀模型预训练的世界知识，造成灾难性遗忘，这与其核心价值相矛盾。目前缺乏对这一现象的系统研究。

Method: 提出了Drive Expert Adapter (DEA)框架，将适配从权重空间转移到提示空间。该框架基于场景特定线索动态路由推理到不同的知识专家，增强驾驶任务性能而不破坏模型的基础参数。同时创建了包含18万场景的大规模数据集，首次为自动驾驶灾难性遗忘提供了基准。

Result: 实验表明，现有方法存在显著的知识退化，而DEA方法不仅在驾驶任务上达到最先进性能，还能有效缓解灾难性遗忘，保留了VLM的泛化能力。数据集和模型已在FidelityDrivingBench发布。

Conclusion: 该研究首次系统揭示了自动驾驶中VLM灾难性遗忘问题，提出的DEA框架通过提示空间适配解决了性能与知识保留的权衡，为VLM在自动驾驶中的实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有住宅平面图自动生成方法的局限性：早期基于约束满足或组合优化的方法缺乏多样性和灵活性，而近期生成模型虽取得进展但难以泛化到异构条件任务（如基于场地边界、房间邻接图或部分布局生成），主要原因是表示方法不够优化

Method: 提出Floorplan Markup Language (FML)通用结构化语法表示法，将整个平面图生成问题转化为下一个token预测任务；基于此开发Transformer生成模型FMLM，能够处理多样化的生成条件

Result: 在RPLAN数据集上的综合实验表明，FMLM作为单一模型，超越了先前任务特定的最先进方法，能够生成高保真且功能合理的平面图

Conclusion: FML表示法和FMLM模型为住宅平面图生成提供了通用解决方案，通过结构化语法表示将问题转化为序列预测，实现了跨异构条件任务的良好泛化能力
...
TL;DR: 提出一种基于图像块级别的幻觉检测框架，通过分析细粒度token层间交互来检测大视觉语言模型中的幻觉对象token，相比现有全局检测方法更有效。


<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型幻觉检测方法主要依赖全局图像层面的相关性度量，这种方法存在局限：幻觉token可能在许多局部区域表现出微弱但广泛分散的相关性，这些相关性聚合后会形成欺骗性的高整体相关性，从而逃避当前的全局幻觉检测器。

Method: 基于"忠实对象token必须牢固地锚定在特定图像区域"的观察，提出图像块级别的幻觉检测框架，分析模型各层的细粒度token级交互。发现幻觉token的两个特征：(1)产生扩散、非局部的注意力模式；(2)与任何视觉区域缺乏有意义的语义对齐。基于这些发现，开发了轻量级可解释的检测方法，利用图像块级统计特征结合隐藏层表示。

Result: 该方法在token级幻觉检测中达到高达90%的准确率，证明了细粒度结构分析在检测幻觉方面的优越性。

Conclusion: 通过分析细粒度token层间交互的图像块级别检测框架能够有效识别大视觉语言模型中的幻觉对象token，相比全局检测方法具有显著优势，为幻觉检测提供了新的方向。
...
TL;DR: Free-Range Gaussians：一种多视角重建方法，通过流匹配预测非像素、非体素对齐的3D高斯分布，仅需4张图像即可生成高质量重建结果，在未观测区域合成合理内容。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法生成高度冗余的网格对齐高斯分布的问题，这些方法在未观测区域会出现空洞或模糊的条件均值。需要一种能够从稀疏输入生成高质量重建并合成未观测区域内容的方法。

Method: 1. 使用流匹配对高斯参数进行生成式重建；2. 引入分层分块方案，将空间相关的高斯分组到联合transformer令牌中，减少序列长度；3. 训练时使用时间步加权渲染损失；4. 推理时采用光度梯度引导和分类器无关引导提高保真度。

Result: 在Objaverse和Google Scanned Objects数据集上，相比像素和体素对齐方法，使用显著更少的高斯分布获得了一致的改进，特别是在输入视角未完全覆盖物体部分时获得大幅提升。

Conclusion: Free-Range Gaussians通过生成式重建框架和分层分块方案，实现了从稀疏输入的高质量3D重建，能够合成未观测区域的合理内容，显著优于现有方法。
...
TL;DR: DIRECT框架通过分层多智能体系统将视频混剪建模为多模态一致性满足问题，显著提升专业级流畅度


<details>
  <summary>Details</summary>
Motivation: 现有自动化视频编辑框架忽视跨层次多模态协调，导致视觉过渡突兀和音乐不协调，无法达到专业级流畅度

Method: 将视频混剪建模为多模态一致性满足问题，提出分层多智能体框架DIRECT，包含编剧（全局结构锚定）、导演（自适应编辑意图）和编辑（意图引导的镜头序列编辑）三个级联层次

Result: DIRECT在客观指标和人类主观评价上均显著优于现有最先进基线，并引入Mashup-Bench基准测试和专门指标

Conclusion: DIRECT框架通过模拟专业制作流程的分层多智能体方法，有效解决了视频混剪中的跨层次多模态协调问题，实现了专业级流畅的视频编辑
...
TL;DR: HorizonWeaver是一个用于自动驾驶场景编辑的框架，通过大规模数据集、语言引导掩码和联合训练方法，解决了密集驾驶场景中多粒度编辑、丰富语义保持和领域迁移的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像编辑方法主要针对物体中心或艺术数据训练，难以处理密集、安全关键的驾驶场景布局。自动驾驶安全需要可扩展的现实可控驾驶场景生成，而真实世界测试无法满足这一需求。

Method: 提出三个核心贡献：1) 数据层面：从Boreas、nuScenes和Argoverse2构建大规模配对真实/合成数据集；2) 模型层面：语言引导掩码用于细粒度编辑，通过语义丰富的掩码和提示实现精确的语言引导编辑；3) 训练层面：内容保持和指令对齐，通过联合损失强制场景一致性和指令保真度。
...
### [135] [FileGram: Grounding Agent Personalization in File-System Behavioral Traces](https://arxiv.org/abs/2604.04901)
*Shuai Liu,Shulin Tian,Kairui Hu,Yuhao Dong,Zhe Yang,Bo Li,Jingkang Yang,Chen Change Loy,Ziwei Liu*

Main category: cs.CV

TL;DR: FileGram是一个基于文件系统行为轨迹的AI智能体记忆与个性化框架，包含数据引擎、评估基准和操作系统三个核心组件，旨在解决文件系统智能体个性化中的数据限制问题。


<details>
  <summary>Details</summary>
Motivation: 当前协同工作的AI智能体在文件系统中面临严重的数据限制：严格的隐私壁垒和多模态真实世界轨迹的联合收集困难阻碍了可扩展的训练和评估，现有方法过于关注交互而忽视了文件系统操作中的密集行为轨迹。

Method: 提出FileGram框架，包含三个核心组件：1) FileGramEngine - 可扩展的人物驱动数据引擎，模拟真实工作流并生成细粒度多模态动作序列；2) FileGramBench - 基于文件系统行为轨迹的诊断基准，评估记忆系统在配置文件重建、轨迹解耦、人物漂移检测和多模态基础任务上的表现；3) FileGramOS - 自底向上的记忆架构，直接从原子动作和内容差异构建用户配置文件，将这些轨迹编码为程序性、语义性和情景性通道，并在查询时进行抽象。

Result: 广泛实验表明，FileGramBench对最先进的记忆系统仍然具有挑战性，而FileGramEngine和FileGramOS是有效的。通过开源该框架，支持未来关于个性化记忆中心文件系统智能体的研究。

Conclusion: FileGram通过将智能体记忆和个性化建立在文件系统行为轨迹上，解决了当前文件系统AI智能体个性化中的数据限制问题，为未来研究提供了全面的框架和评估基准。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像编辑模型在细粒度空间操作（如物体布局和相机视角控制）方面能力不足，缺乏专门的评估基准和训练数据

Method: 1) 创建SpatialEdit-Bench基准测试，通过视角重建和构图分析联合评估感知合理性和几何保真度；2) 使用可控Blender流水线生成SpatialEdit-500k合成数据集，包含多样化背景和系统相机轨迹；3) 基于该数据开发SpatialEdit-16B基线模型

Result: SpatialEdit-16B在通用编辑任务上表现有竞争力，在空间操作任务上显著优于现有方法

Conclusion: 提出的基准测试、数据集和基线模型为解决图像空间编辑的细粒度控制问题提供了完整解决方案，所有资源将公开
...
TL;DR: DeltaWorld：一种基于VFM特征差异的生成式世界模型，通过DeltaTok将连续帧间特征差异编码为"delta token"，实现高效多假设训练和多样化未来预测


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型面临两个主要问题：判别式模型产生确定性预测（隐式平均可能未来），而生成式模型计算成本高昂。虽然基于视觉基础模型特征空间预测可减少参数，但多数方法仍是判别式的。需要一种既能生成多样化未来又计算高效的生成式世界模型。

Method: 提出DeltaTok将连续帧间VFM特征差异编码为单个连续"delta token"，将视频从三维时空表示降为一维时间序列。基于此构建DeltaWorld生成式世界模型，利用紧凑表示实现可处理的多假设训练：并行生成多个未来，仅监督最佳预测。推理时单次前向传播即可产生多样化预测。

Result: 在密集预测任务实验中，DeltaWorld生成的未来更符合真实世界结果，同时参数量比现有生成式世界模型少35倍以上，FLOPs减少2000倍。例如512×512帧可获得1024倍的token缩减。

Conclusion: DeltaWorld通过delta token的紧凑表示，实现了高效且多样化的未来预测，在保持生成能力的同时显著降低了计算成本，为视频世界建模提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散修复方法依赖微调或Control-Net模块，但预训练扩散模型本身具有修复能力，只是无法通过文本提示访问

Method: 提出在扩散桥框架中学习提示嵌入，对齐训练和推理动态，确保从噪声退化状态到干净图像的一致去噪路径

Result: 在WAN视频模型和FLUX图像模型上实现轻量级提示学习，在多样化退化上获得竞争性性能和泛化能力

Conclusion: 预训练扩散模型固有修复能力可通过提示学习解锁，无需微调或专用控制模块，为AiOR提供高效解决方案
...
TL;DR: SimpleProc：一个基于NURBS、位移和纹理模式的完全程序化多视图立体训练数据生成器，在小规模（8k图像）上优于手工数据，大规模（352k图像）上性能接近甚至超过使用692k手工图像训练的模型。


<details>
  <summary>Details</summary>
Motivation: 探索多视图立体（MVS）程序化规则的设计空间，解决手工收集训练数据成本高、规模受限的问题，通过程序化方法生成高质量训练数据。

Method: 提出SimpleProc：一个完全程序化的生成器，使用少量规则驱动，基于NURBS（非均匀有理B样条）以及基本位移和纹理模式来生成训练数据。

Result: 在8,000图像规模下，SimpleProc生成的数据优于同规模的手工游戏和真实物体图像；当扩展到352,000图像时，性能与使用692,000手工图像训练的模型相当甚至在某些基准测试中超越。

Conclusion: 程序化数据生成是MVS训练的有效替代方案，能够以较低成本生成大规模高质量数据，性能可与甚至超越大规模手工数据集。
...
TL;DR: 该论文分析了视觉语言模型中输出令牌数量对延迟的影响，发现大模型用较少输出令牌比小模型用长输出序列更高效，并提出多智能体推理框架来提升效率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型使用大型语言模型作为解码器，通过自回归顺序生成响应令牌，输出令牌数量成为端到端延迟的瓶颈。不同模型需要不同数量的输出令牌才能达到可比性能，需要研究如何优化这一效率问题。

Method: 1. 在模拟数据上对VLM不同组件的延迟进行全面分析；2. 在多样化真实世界基准上进行实证研究；3. 提出多智能体推理框架：保持大模型的短响应，但在必要时从小模型转移关键推理令牌。

Result: 1. 实验表明，输出令牌较少的大模型比输出序列较长的小模型更高效；2. 实证研究证实大模型能用显著更少的输出令牌达到更好或可比的性能；3. 多智能体框架通过重用小模型的推理令牌，能接近大模型自身推理的性能。

Conclusion: 输出令牌数量是VLM延迟的关键瓶颈，大模型用较少令牌比小模型用长序列更高效。提出的多智能体推理框架能有效平衡效率与性能，通过重用小模型的推理令牌来提升大模型的性能表现。
...
TL;DR: LoMa通过数据驱动方法改进局部特征匹配，结合大规模数据混合、现代训练方法、模型容量扩展和计算扩展，在多个基准测试中显著超越现有方法，并创建了HardMatch数据集解决基准饱和问题。


<details>
  <summary>Details</summary>
Motivation: 局部特征匹配作为3D视觉系统的基础组件，其发展滞后于现代数据驱动方法。现有方法仅在少数中型数据集上训练，而现代重建模型受益于大规模数据集。同时，当前基准测试主要基于成功3D重建的稀疏视图，导致评估局限于相对简单的图像对，造成基准饱和。

Method: LoMa采用数据驱动方法，结合：1）大规模多样化数据混合；2）现代训练方法；3）扩展模型容量；4）扩展计算资源。此外，作者收集了1000个极具挑战性的互联网图像对，创建HardMatch数据集，并通过人工标注获取地面真值对应关系。

Result: LoMa在多个基准测试中取得显著提升：HardMatch上超越ALIKED+LightGlue +18.6 mAA，WxBS上+29.5 mAA，InLoc上+21.4 (1m, 10°)，RUBIK上+24.2 AUC，IMC 2022上+12.4 mAA。模型和代码已公开。

Conclusion: 通过数据驱动方法、大规模数据混合、现代训练技术和计算扩展，LoMa在局部特征匹配任务上实现了显著性能提升。HardMatch数据集的创建解决了现有基准饱和问题，为未来研究提供了更具挑战性的评估平台。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将基于图像的虚拟试穿和姿态驱动动画作为两个独立过程处理，这会导致身份漂移、服装变形和前后不一致的问题。需要一种统一的框架来在单个步骤中实现连贯的合成。

Method: 1. 构建大规模三元组监督数据：生成身份保持的换装人物图像，捕获完整的上下服装三元组，组装多样化的野外三元组；2. 引入双模块架构的视频扩散变换器，稳定训练、保持预训练生成质量，提高服装准确性、姿态遵循和身份保持，支持零样本服装插值。

Result: Vanast能够生成高保真、身份一致的各种服装类型动画，在服装准确性、姿态遵循和身份保持方面表现优异，支持零样本服装插值。

Conclusion: 通过统一框架、大规模三元组监督数据和双模块架构，Vanast成功解决了传统方法中的问题，实现了从单张图像到服装转移动画的高质量合成。
...
TL;DR: 通过训练代码LLMs模拟程序执行，结合监督微调和强化学习，提升竞争性编程性能


<details>
  <summary>Details</summary>
Motivation: LLMs在生成代码时无法准确估计程序执行结果，这限制了其生成正确代码的能力。需要解决LLMs对程序执行估计不足的问题，以提升代码生成质量。

Method: 结合监督微调（使用自然语言执行轨迹和基于真实执行的文本解释）与强化学习（使用可验证奖励）。引入两个互补目标：给定代码和输入时的输出预测，以及使用真实或自预测执行反馈解决竞争性编程任务。

Result: 在多个竞争性编程基准测试中，该方法相比标准推理方法获得了持续改进。模型能够对多个候选解决方案进行自我验证，并通过模拟测试执行进行迭代自我修复。

Conclusion: 训练代码LLMs模拟程序执行是可行的，这种能力可以显著提升竞争性编程性能。执行模拟在代码生成中发挥重要作用，但仍有局限性。
...
TL;DR: 提出基于约束最大似然估计的LLM故障率估计新方法，整合人工标注、自动标注和领域约束，相比现有方法提供更准确、低方差的估计


<details>
  <summary>Details</summary>
Motivation: 当前LLM故障率估计面临昂贵人工标注与有偏自动标注（如LLM-as-a-Judge）之间的权衡，需要一种实用高效的估计方法

Method: 基于约束最大似然估计，整合三个信号源：小型高质量人工标注校准集、大量LLM法官标注、以及基于法官性能统计已知边界的领域特定约束

Result: 在多样化实验条件下（不同法官准确率、校准集大小、LLM故障率），该方法比现有方法（如预测驱动推理PPI）提供更准确、方差更低的估计

Conclusion: 该方法超越了"黑盒"使用自动法官的方式，提供了一个原则性、可解释、可扩展的LLM故障率认证框架
...
TL;DR: Focus是一种高效注意力方法，通过可学习质心将token分组，限制远距离注意力在同组内，保持局部注意力全分辨率。该方法仅训练质心参数（最少148K），不改变模型权重，在多种规模（124M-70B）和架构上提升领域困惑度且不损害下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有高效注意力方法在保持模型性能的同时难以实现真正的效率提升，特别是在不改变预训练模型权重的情况下。需要一种方法能够选择性地关注重要的token对，而不是近似所有注意力计算，同时保持模型对齐性和下游任务性能。

Method: Focus使用可学习质心将token分配到不同组中：远距离注意力仅限于同组token对，而局部注意力保持全分辨率。采用Sinkhorn归一化强制平衡分组作为硬约束。推理时，通过限制每个token到其top-k最高得分组，将软路由离散化为硬稀疏模式，可分解为两个标准FlashAttention调用实现加速。

Result: 在124M参数规模上，Focus超越全注意力（30.3 vs 31.4 PPL）；在7B规模从头训练（2B tokens）同样击败全注意力（13.82 vs 13.89 PPL）。推理时通过硬稀疏模式实现2倍加速，同时超越预训练基线（41.3 vs 42.8 PPL）；分解为两个FlashAttention调用在1M tokens上达到8.6倍加速。与LoRA相比，Focus保持指令调优模型的TruthfulQA分数，而LoRA在所有学习率和秩上都会退化。

Conclusion: Focus是一种纯加性的高效注意力方法，仅需训练少量质心参数即可提升模型性能，同时保持模型对齐性和下游任务能力。该方法在多种规模、架构和设置下均优于全注意力和现有高效注意力方法，且无需定制内核即可实现显著加速。
...
TL;DR: VIGIL是一款首个用于实时检测和缓解在线信息中认知偏差触发因素的浏览器扩展工具，通过LLM驱动的重写和隐私分层推理来对抗AI生成内容对信息完整性的威胁。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的兴起对在线信息完整性和公民话语构成风险，除了明显的虚假信息外，更隐蔽的威胁是利用人类认知偏差进行说服或操纵。目前缺乏直接检测和缓解认知偏差触发因素的工具。

Method: 开发VIGIL浏览器扩展，提供实时认知偏差触发检测和缓解功能，包括滚动同步检测、LLM驱动的可逆重写、从完全离线到云端的隐私分层推理，并设计为可扩展的第三方插件架构。

Result: VIGIL是首个用于认知偏差触发检测和缓解的浏览器扩展，已在多个经过NLP基准严格验证的插件中实现，并在GitHub上开源。

Conclusion: VIGIL填补了检测和缓解在线信息中认知偏差触发因素的工具空白，为保护公民话语免受AI生成内容中更隐蔽的操纵威胁提供了实用解决方案。
...
TL;DR: 提出Knowledge Packs：预计算的KV缓存，以零token成本提供与RAG相同的知识，通过正确的聊天模板格式实现完全等效，同时支持通过值操作进行行为引导


<details>
  <summary>Details</summary>
Motivation: RAG方法在检索和生成过程中消耗大量token，存在效率问题。作者旨在开发一种零token成本的知识传递方法，同时探索KV缓存作为接口实现行为引导的可能性

Method: 利用因果变换器的特性：文本F的前向传播KV缓存与联合处理F+q产生的缓存完全相同（基于因果掩码）。提出Knowledge Packs作为预计算的KV缓存，通过正确的聊天模板格式确保等效性。同时利用RoPE仅旋转键而不改变值的特性，通过对缓存值施加对比性增量来引导模型行为

Result: 在正确格式化下：Qwen3-8B和Llama-3.1-8B上700个问题零差异，实现高达95%的token节省。行为引导效果位于中间层值（33-66%），独立方向几乎正交（cos~0）且可组合，知识和引导通道在alpha≤0.7时可同时运行而无干扰

Conclusion: Knowledge Packs提供与RAG等效的知识传递但零token成本，同时KV缓存接口支持无需训练或权重修改的行为引导，为高效知识注入和行为控制提供了新范式
...
TL;DR: CresOWLve是一个评估大语言模型创造性问题解决能力的基准，使用基于现实世界知识的谜题，要求模型运用多种创造性思维策略和跨领域知识整合能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型基准大多只评估创造性问题解决过程的特定组成部分，且许多创造性导向的基准依赖于人为构建的脑筋急转弯或虚构场景，不能反映现实世界中的创造性问题解决过程。

Method: 引入CresOWLve基准，该基准使用基于现实世界知识的谜题来评估创造性问题解决能力。问题要求运用多种创造性思维策略，从不同领域检索事实，并创造性地组合这些信息以得出解决方案。

Result: 评估多个前沿的非思维和思维大语言模型显示，CresOWLve仍然具有高度挑战性。分析揭示了一致的性能差距：模型在事实性问题上的表现明显优于创造性问题（下降高达-17%）。模型通常能够检索相关知识，但难以形成非显而易见的创造性连接来整合信息并得出正确答案。

Conclusion: CresOWLve基准填补了现有评估方法的空白，突出了大语言模型在创造性问题解决方面的局限性，特别是在整合跨领域知识和形成创新连接方面的挑战。
...
TL;DR: 论文研究了在阿拉伯语早期阅读评估故事生成中，通过噪声引导（在推理时向transformer模型内部表示注入校准的高斯扰动）来提升多样性，同时保持词汇、阅读水平和叙事结构的约束。


<details>
  <summary>Details</summary>
Motivation: 为阿拉伯语早期阅读评估生成多样化且符合教学要求的故事面临挑战：需要在词汇、阅读水平和叙事结构的严格约束与避免重复情节之间取得平衡，因为重复情节会损害评估的有效性。

Method: 研究了噪声引导方法，在推理时向transformer模型的内部表示注入校准的高斯扰动。比较了四种注入策略与高温采样基线，评估了五个小型阿拉伯语中心语言模型（7-9B参数）。测量了多样性、质量、约束遵守度和阅读年级水平。

Result: 残差流噪声一致提高了叙事多样性，同时质量和约束成本最小，并在所有模型中保持了早期阅读水平。注意力熵噪声注入（AENI）稳定了原本不可靠的注意力-逻辑噪声，同时恢复了质量。高温采样提高了阅读年级水平，并在多个模型上导致灾难性崩溃。

Conclusion: 对于受约束的教育内容生成，内部表示层面的扰动比输出层面的随机性更适合作为多样性策略。噪声引导是一种无需训练的有效方法，可在保持教学约束的同时提高故事多样性。
...
TL;DR: QIMMA是一个质量保证的阿拉伯语大语言模型排行榜，通过多模型评估管道结合自动LLM判断和人工审查，对阿拉伯语基准进行系统质量验证，提供包含5.2万样本的精选评估套件。


<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语基准存在系统性质量问题，需要建立质量保证的评估框架来提供可靠、可复现的阿拉伯语LLM评估基础。

Method: 采用多模型评估管道，结合自动LLM判断和人工审查，对现有阿拉伯语基准进行质量验证和问题修复；使用LightEval和EvalPlus实现透明评估，并公开每个样本的推理输出。

Result: 创建了包含超过5.2万个样本的精选多领域、多任务评估套件，主要基于原生阿拉伯语内容（代码评估任务除外）；提供了可复现、社区可扩展的阿拉伯语NLP评估基础。

Conclusion: QIMMA通过系统性基准验证和质量保证机制，为阿拉伯语LLM评估提供了可靠、透明、可扩展的框架，填补了阿拉伯语NLP评估领域的质量空白。
...
TL;DR: 提出基于形态驱动的标记模型，以Riffian语为起点分析名词范畴的跨语言变异，将状态概念扩展到所有综合语中


<details>
  <summary>Details</summary>
Motivation: 所有语言都有名词范畴，但其实现方式差异显著。语义和/或形态句法差异在不同语言中表现程度不同，需要探索这些变异并解释名词类型内部和跨语言的标记差异

Method: 以Riffian语为参照点，扩展到其他语言分析。提出形态驱动标记的正式模型：名词被组织成模块化认知集合，每个集合有自己的形态模板和无标记形式。将模式置于句法功能中重新评估标记性和状态概念

Result: 形态驱动标记模型能解释名词类型内部和跨语言的标记差异。提出将状态概念扩展到所有综合语，并将其分析为基于句法的屈折变化的新子类别，类似于一致关系和语法格

Conclusion: 状态应被视为所有综合语中的普遍概念，是类似于一致关系和语法格的句法屈折子类别。形态驱动标记模型为理解名词范畴的跨语言变异提供了理论框架
...
TL;DR: 词汇丢弃通过随机掩码提案者输出logits来维持问题多样性，防止共进化训练中的多样性崩溃，在数学推理任务上提升求解器性能


<details>
  <summary>Details</summary>
Motivation: 共进化自博弈中，提案者会快速收敛到满足奖励函数的狭窄问题分布，导致多样性崩溃，使课程对求解者失去信息价值，阻碍共进化循环的持续进展

Method: 引入词汇丢弃：在策略训练和课程生成期间，对提案者的输出logits应用随机硬掩码，防止提案者锁定到固定的token序列，维持问题多样性

Result: 在Qwen3-4B和Qwen3-8B的数学推理训练中，词汇丢弃在词汇、语义和功能指标上持续维持提案者多样性，在8B模型上平均提升求解器性能4.4个百分点，在竞赛级基准上提升最大

Conclusion: 明确的动作空间约束（类似于经典自博弈中游戏规则的结构性作用）有助于维持语言共进化的生产力，词汇丢弃是这一原则的简单实例化
...
TL;DR: 使用LLM驱动的进化搜索自动发现无监督不确定性量化方法，在原子声明验证任务上优于手动设计基线，并展现出不同LLM的进化策略差异


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型的不确定性量化方法主要基于领域知识和启发式手动设计，这限制了方法的可扩展性和通用性。需要一种自动化方法来发现更优的UQ方法。

Method: 应用LLM驱动的进化搜索自动发现无监督UQ方法，这些方法以Python程序形式表示。在原子声明验证任务上进行评估，比较不同LLM（Claude、Gpt-oss-120B、Sonnet、Opus）的进化策略差异。

Result: 进化出的方法在9个数据集上优于强手动设计基线，相对ROC-AUC提升高达6.7%，并展现出良好的分布外泛化能力。不同LLM采用不同的进化策略：Claude模型倾向于设计高特征数量的线性估计器，Gpt-oss-120B倾向于更简单、可解释的位置加权方案。只有Sonnet 4.5和Opus 4.5能可靠地利用增加的方法复杂度来提升性能。

Conclusion: LLM驱动的进化搜索是自动化、可解释的幻觉检测器设计的有前景范式，能够发现优于手动设计的方法，并揭示不同LLM在进化搜索中的策略差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM文化表征评估主要关注文化多样性和事实准确性，但缺乏对文化对齐的评估——即生成内容与本地人群对自身文化要素认知和优先级的匹配程度。需要超越简单多样性指标，评估AI生成内容在真实捕捉全球文化细微层次结构方面的保真度。

Method: 1) 建立人类基准：基于9个国家开放式调查响应，创建人类衍生的文化重要性向量；2) 计算模型表征：通过句法多样化的提示集计算LLM衍生的文化表征向量，应用于三个前沿LLM（Gemini 2.5 Pro、GPT-4o和Claude 3.5 Haiku）；3) 对齐分析：调查人类文化重要性与模型文化表征之间的对齐程度。

Result: 研究发现：1) 部分模型存在西方中心校准，国家与美国文化距离越大，对齐度越低；2) 所有模型都表现出高度相关的系统性错误特征（ρ>0.97），过度索引某些文化标记，同时忽视用户深层次的社会和价值观优先级。

Conclusion: 该方法超越了简单的多样性指标，能够评估AI生成内容在真实捕捉全球文化细微层次结构方面的保真度。研究揭示了LLM文化表征中的系统性偏差，为改进模型文化对齐提供了评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于表示级引导的语言控制方法通常依赖昂贵的多语或平行数据来识别残差流中的语言特定方向，而稀疏自编码器（SAE）虽然提供了可解释的特征方向，但现有SAE方法面临相同的数据约束问题

Method: LangFIR（Language Feature Identification via Random-token Filtering）通过随机标记序列过滤语言无关特征，仅使用少量单语数据即可识别语言特定的SAE特征。随机标记序列能暴露语言无关特征，从而过滤掉这些特征并分离出稀疏的语言特定特征集

Result: LangFIR识别的特征极其稀疏、对目标语言高度选择性且具有因果重要性：方向性消融仅增加相应语言的交叉熵损失。在三个模型（Gemma 3 1B、4B和Llama 3.1 8B）、三个数据集和12种目标语言上，LangFIR实现了最佳平均准确率BLEU，优于最强的单语基线，甚至超过依赖平行数据的方法

Conclusion: 多语言LLM中的语言身份信息定位在稀疏的特征方向集中，这些特征可以通过单语数据发现。LangFIR为可靠的语言输出控制提供了一种高效且数据需求低的方法
...
TL;DR: MultiPress：用于多模态新闻分类的三阶段多智能体框架，通过专门的感知、检索增强推理和门控融合评分智能体，结合奖励驱动的迭代优化机制，显著提升分类性能。
...
Method: 提出MultiPress三阶段多智能体框架：1) 多模态感知智能体处理文本和图像；2) 检索增强推理智能体利用外部知识库进行推理；3) 门控融合评分智能体整合信息并评分，最后通过奖励驱动的迭代优化机制提升性能。

Result: 在新构建的大规模多模态新闻数据集上验证，MultiPress相比强基线方法取得显著改进，证明了模块化多智能体协作和检索增强推理在提升分类准确性和可解释性方面的有效性。
...
TL;DR: 该研究评估了意大利语NPN构式家族在BERT上下文嵌入中的编码情况，通过分层探测分类器分析构式形式和意义信息，为构式理论与神经语言建模的对话提供实证证据


<details>
  <summary>Details</summary>
Motivation: 可解释性研究强调需要根据明确的语言学理论评估预训练语言模型（特别是上下文嵌入）编码的语言信息。本研究关注意大利语NPN构式家族，挑战先前实验设计的理论和方法假设，并将此类研究扩展到较少被研究的语言

Method: 从BERT中提取上下文向量表示，作为分层探测分类器的输入，系统评估模型内部各层编码的信息

Result: 结果揭示了构式形式和意义在上下文嵌入中的反映程度，为构式理论与神经语言建模之间的对话提供了实证证据

Conclusion: 该研究通过分析意大利语NPN构式在BERT中的编码情况，挑战了先前研究的假设，扩展了构式理论与神经语言建模的实证研究，为理解预训练语言模型如何编码语言学信息提供了新见解
...
TL;DR: 该论文提出通过全序列掩码训练解锁掩码扩散语言模型的提示填充能力，使模型能够自动填充提示模板，性能优于人工设计模板


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型具有双向去噪能力，但现有监督微调方法仅对响应进行掩码，限制了模型对提示模板的填充能力。作者认为这是训练实践而非架构限制导致的瓶颈

Method: 扩展监督微调中的全序列掩码策略，同时对提示和响应进行联合掩码训练，使模型能够基于少量示例填充提示模板的掩码部分

Result: 模型填充的提示模板性能匹配或超越人工设计模板，在不同模型间有效迁移，并与现有提示优化方法互补

Conclusion: 训练实践是限制掩码扩散语言模型提示填充能力的主要瓶颈，全序列掩码训练能有效解锁这一能力，为提示工程提供新方法
...
TL;DR: LightThinker++：通过显式自适应内存管理实现LLM推理的动态思维压缩，显著降低token使用和推理时间，同时提升长序列任务性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂推理任务中面临长思维轨迹带来的认知开销激增问题，静态压缩方法在处理复杂推理时容易因中间细节丢失导致逻辑瓶颈
...
Conclusion: 该工作为在最小开销下维持深度LLM推理提供了可扩展方向，通过动态思维压缩和自适应内存管理有效解决了长序列推理中的效率瓶颈问题
...
TL;DR: 批量处理和变量堆叠可显著降低LLM文本分类的API调用成本，且在一定范围内不会显著影响编码质量


<details>
  <summary>Details</summary>
Motivation: 当前社会科学研究中使用LLM进行文本分类时，研究人员通常对每个变量每个文本单独调用API，导致大规模文本编码成本极高。需要验证批量处理和变量堆叠是否能在降低成本的同时保持编码质量。

Method: 测试了来自四个提供商的八个生产级LLM模型，使用3,962条专家编码的推文，在四个分类任务上评估不同批量大小（1到1000项）和变量堆叠（最多25个维度）对编码准确性的影响。

Result: 八个模型中的六个在批量大小达到100时，准确性保持在单项目基准的2个百分点以内。变量堆叠最多10个维度时，结果与单变量编码相当，性能下降主要由任务复杂性而非提示长度驱动。在安全操作范围内，批量处理和堆叠带来的测量误差小于真实数据中典型的编码员间分歧。

Conclusion: 批量处理和变量堆叠是降低LLM文本分类成本的可行策略，在合理范围内（批量≤100项，变量≤10个维度）不会显著影响编码质量，且误差小于人工编码的固有差异。
...
TL;DR: POEMetric是首个全面的诗歌评估框架，用于评估LLM与人类诗人在诗歌创作上的差距，涵盖基础指令遵循、高级创作能力和整体质量评估。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型能够创作诗歌，但它们在诗歌创作方面与人类诗人的真实差距尚不明确。需要建立一个全面的评估框架来系统性地比较LLM与人类诗人在诗歌创作各方面的能力差异。

Method: 提出了POEMetric评估框架，包含三个层次：1)基础指令遵循能力（形式与主题）；2)高级能力（创造力、词汇多样性、独特性、情感共鸣、意象与文学手法）；3)整体质量评估与作者归属判断。构建了包含203首英文诗歌的人类数据集（7种固定形式），基于相同形式和主题生成了6,090首LLM诗歌，采用基于规则的评估和LLM-as-a-judge方法，并由人类专家验证结果。

Result: 虽然顶级模型在形式准确性（4.26/5.00）和主题对齐（4.99）上表现良好，但所有模型在高级能力方面都无法达到人类诗人的水平：人类在创造力（4.02）、独特性（3.95）、情感共鸣（4.06）、意象运用（4.49）和文学手法（4.67）方面表现卓越。在整体诗歌质量上，人类（4.22）也显著优于最佳LLM（3.20）。

Conclusion: 诗歌生成对LLM来说仍然是一个艰巨的挑战。尽管LLM在形式遵循方面表现良好，但在创造力、情感表达、独特性和文学技巧等高级诗歌创作能力方面与人类诗人存在显著差距。
...
TL;DR: 研究发现LLM激活空间中的"真理方向"并非普遍存在，其有效性受模型层数、任务类型、复杂度和指令提示的显著影响


<details>
  <summary>Details</summary>
Motivation: 先前研究认为LLM激活空间中存在普遍的线性真理方向，但近期研究质疑其跨设置泛化能力。本文旨在系统探索真理方向普遍性的具体限制条件

Method: 通过多维度实验分析：1) 在不同模型层探测真理方向；2) 比较事实性与推理性任务的差异；3) 考察任务复杂度的影响；4) 分析不同指令提示对真理探针泛化能力的影响

Result: 真理方向高度依赖模型层数，事实性任务在较早层出现，推理任务在较深层出现；任务复杂度影响方向性能；简单正确性评估指令显著改变真理探针的泛化能力

Conclusion: 真理方向的普遍性比先前认知更为有限，其有效性受模型层数、任务类型、任务难度和提示模板等多个因素的显著影响，对LLM中真理表示的理解需要更细致的分析框架
...
TL;DR: 该研究系统评估了多智能体系统中间接提示注入攻击的防御策略，揭示了现有防御的脆弱性，并提出基于表征工程的检测方法
...
Method: 在动态多步骤工具调用环境中，系统评估了六种防御策略对抗四种复杂IPI攻击向量，覆盖九个LLM骨干。采用多维分析超越二元成功率，并研究表征工程作为检测策略，通过提取工具输入位置的隐藏状态来识别和拦截未授权操作。

Result: 高级注入攻击成功绕过几乎所有基线防御，某些表面缓解措施甚至产生反效果。智能体几乎瞬时执行恶意指令，但其内部状态表现出异常高的决策熵。基于表征工程的断路器在智能体提交操作前成功识别并拦截未授权行为，在不同LLM骨干上实现高检测准确率。

Conclusion: 研究揭示了当前IPI防御的局限性，为构建弹性多智能体架构提供了高度实用的范式。表征工程方法能够有效检测和拦截间接提示注入攻击，在动态环境中表现出良好的鲁棒性。
...
TL;DR: 研究发现大型语言模型在类比推理中存在不对称性：对于修辞类比，探测内部表征显著优于提示性能；对于叙事类比，两者表现都较差，表明模型内部表征与提示行为的关系具有任务依赖性。
...
Method: 通过比较模型的探测表征与提示性能来检测叙事类比。具体方法包括：1）使用探测技术分析模型的内部表征；2）评估模型在提示下的类比检测性能；3）对比这两种方法在不同类型类比（修辞类比vs叙事类比）上的表现差异。

Result: 发现不对称性：对于修辞类比，探测内部表征显著优于提示性能（在开源模型中）；对于叙事类比，两种方法都表现出较低且相似的性能。这表明内部表征与提示行为的关系具有任务依赖性，提示可能无法有效访问模型内部可用的信息。

Conclusion: 模型在类比推理中的表现取决于任务类型，提示机制可能无法充分利用模型内部已有的表征信息。这一发现揭示了LLMs在抽象推理和泛化方面的局限性，特别是在需要深层语义理解的叙事类比任务中。
...
TL;DR: 通过提示工程干预（I-CALM框架）减少LLM幻觉：通过置信度评估、部分奖励弃权和规范性原则，在事实性问题中实现选择性回答，以覆盖率为代价提高可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生自信但错误的答案，部分原因是常见的二元评分惯例奖励回答而非诚实表达不确定性。研究旨在通过纯提示干预减少幻觉风险，而不修改模型本身。

Method: 提出I-CALM框架：1) 引出语言置信度作为不确定性信号；2) 通过明确奖励方案部分奖励弃权；3) 添加强调真实性、谦逊和责任的轻量级规范性原则。使用GPT-5 mini在PopQA数据集上进行评估。

Result: 置信度引出和弃权奖励提示（特别是结合规范原则）通过识别易错案例并将其转向弃权，降低了已回答案例的错误答案率。这以覆盖率为代价提高了可靠性，同时强制回答性能基本不变。改变弃权奖励产生了清晰的弃权-幻觉边界。

Conclusion: 该框架可以在不重新训练的情况下改善事实性问题上的选择性回答，效果大小因模型和数据集而异。语言置信度可作为有用的不确定性信号，提示工程干预能有效减少幻觉。
...
TL;DR: 论文提出在基于LLM的社会模拟中采用因果反事实框架，区分必要因果关系和充分因果关系，以支持政策干预的因果推断，而非仅追求模拟的逼真性。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的社会模拟虽然能生成逼真的社区互动，可用于"政策风洞"测试治理干预措施，但逼真性不等于因果性。现有模拟工作通常缺乏因果语义规范，无法支持"干预A减少冲突升级"等因果主张。需要建立因果框架来区分不同类型的因果关系，以满足不同利益相关者的需求。

Method: 采用因果反事实框架，区分必要因果关系（如果没有干预，结果是否会发生？）和充分因果关系（干预是否可靠地产生结果？）。将这种区分映射到不同利益相关者的需求：事件诊断需要必要性证据，政策设计需要充分性证据。形式化这种映射关系，展示如何在明确假设下通过模拟设计支持估计，并论证应将结果解释为模拟器条件因果估计。

Result: 建立了将因果反事实框架应用于LLM社会模拟的正式映射关系，展示了模拟设计如何支持在明确假设下进行因果估计。论证了所得量应解释为模拟器条件因果估计，其政策相关性取决于模拟器的保真度。

Conclusion: 现在建立这一框架至关重要：它有助于定义什么是足够的保真度，并将该领域从看起来逼真的模拟转向能够支持政策变革的模拟。该框架为基于LLM的社会模拟提供了因果推断基础，使模拟结果具有明确的因果解释和政策相关性。
...
TL;DR: CUP框架将语言模型与结构化规划相结合，通过不确定性感知的序列决策来优化目标导向对话，提高成功率并减少交互轮次。


<details>
  <summary>Details</summary>
Motivation: 现有目标导向对话系统存在局限性：结构化方法依赖预定义模式，而基于LLM的方法缺乏长视野决策能力，导致信息获取与目标承诺之间协调不佳。需要一种既能灵活交互又能进行多步规划的方法。

Method: 提出对话不确定性感知规划框架(CUP)，将目标导向对话建模为不确定性感知的序列决策问题。框架结合语言模型与结构化规划：语言模型提出可行动作，规划器评估这些动作对长期不确定性减少的影响。

Result: 在多个对话基准测试中，CUP一致提高了成功率，同时减少了所需交互轮次。分析表明不确定性感知规划有助于更高效的信息获取和更早的自信承诺。

Conclusion: CUP框架通过将语言模型与不确定性感知规划相结合，有效解决了目标导向对话中信息获取与目标承诺的协调问题，实现了更高效、更成功的多轮对话决策。
...
TL;DR: StreamGuard：一种基于风险预测的统一流式护栏，通过蒙特卡洛推演监督未来有害性预测，无需精确边界标注即可实现早期安全干预


<details>
  <summary>Details</summary>
Motivation: 现有流式护栏通常将输出侧安全检测视为边界检测问题，需要训练模型识别响应变得不安全的最早前缀。这种方法需要精确的token级边界标注，且在实际LLM部署中，单一护栏需要同时处理完全可见的提示审核和部分生成的流式响应审核。

Method: StreamGuard将审核重新定义为预测问题：给定部分前缀，模型预测可能未来延续的预期有害性。该方法使用蒙特卡洛推演进行监督，通过采样可能的未来延续来估计风险，从而实现早期干预而无需精确的token级边界标注。

Result: 在标准安全基准测试中，StreamGuard在输入审核和流式输出审核方面表现优异。在8B规模上，StreamGuard将聚合输入审核F1从86.7提升至88.2，聚合流式输出审核F1从80.4提升至81.9。在QWENGUARDTEST response_loc流式基准上，StreamGuard达到97.5 F1、95.1召回率和92.6%及时干预率，同时将漏检率从7.9%降至4.9%。预测监督能有效跨分词器和模型家族迁移。

Conclusion: 无需精确边界标注即可获得强大的端到端流式审核能力，预测未来风险是低延迟安全干预的有效监督策略。预测监督具有跨模型迁移能力，为流式安全护栏提供了新的设计范式。
...
TL;DR: RUQuant是一种新颖的两阶段正交变换方法，通过理论驱动的激活量化方案，在不需微调的情况下实现高效LLM量化，在W6A6和W4A4配置下分别达到99.8%和97%的精度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署面临资源限制挑战，现有后训练量化方法因激活分布的非均匀性导致精度显著下降。需要从理论角度重新审视激活量化问题，解决量化区间内激活分布不均匀导致的最优点偏移问题。

Method: 基于Lloyd-Max最优性条件理论分析，提出两阶段正交变换方法RUQuant：第一阶段将激活分块，使用Householder反射和Givens旋转构建的复合正交矩阵将每块映射到均匀采样的目标向量；第二阶段通过Transformer输出差异微调全局Householder反射以最小化量化误差。

Result: 无需模型微调即可实现接近最优的量化性能：在13B LLM上，W6A6量化达到全精度99.8%的准确率，W4A4量化达到97%的准确率，处理时间约1分钟。微调变体可获得更高精度，证明了方法的有效性和可扩展性。

Conclusion: RUQuant通过理论驱动的正交变换方法有效解决了激活量化中的非均匀分布问题，在保持高效率的同时显著提升了量化精度，为资源受限环境下的LLM部署提供了实用解决方案。
...
TL;DR: 研究发现大语言模型在推理时存在语义漂移现象，通过few-shot提示中的文化负载数字可以诱导模型在无关下游任务中产生有害内容，而这种现象需要足够能力的模型才会出现。


<details>
  <summary>Details</summary>
Motivation: 重新审视先前研究中关于k-shot提示不会诱导模型产生有害内容的结论，探究推理时语义漂移是否真实存在且可测量，并确定其发生的边界条件。

Method: 采用控制实验设计，在few-shot演示中注入五个文化负载数字，然后给出语义无关的提示。比较不同能力模型（具有丰富文化联想表示的大型模型vs简单/小型模型）的输出分布变化。同时测试结构惰性演示（无意义字符串）的影响。

Result: 具有丰富文化联想表示的大型模型在文化负载数字few-shot演示后，输出分布显著向更黑暗、威权和污名化主题偏移；而简单/小型模型未出现此效应。无意义字符串演示也会扰动输出分布，表明存在两种可分离机制：结构格式污染和语义内容污染。

Conclusion: 推理时污染确实存在且可测量，但需要足够能力的模型才会出现。研究确定了推理时污染发生的边界条件，对使用few-shot提示的LLM应用安全性有直接启示。
...
TL;DR: 首次对小型语言模型(100M-10B参数)进行情绪向量提取方法的比较分析，发现生成式提取方法在情绪分离上表现更优，情绪表征集中在中间Transformer层，并揭示了跨语言情绪纠缠的安全问题。


<details>
  <summary>Details</summary>
Motivation: 研究小型语言模型是否具有与前沿大模型类似的内部情绪表征能力，为生产系统中广泛使用的SLMs提供情绪理解的基础研究。

Method: 评估9个模型(涵盖GPT-2、Gemma、Qwen、Llama、Mistral 5个架构家族)，使用20种情绪和两种提取方法(生成式和理解式)。通过统计检验、层间分析、各向异性基线验证和因果行为实验(转向实验)进行验证。

Result: 生成式提取方法在情绪分离上显著优于理解式方法(p=0.007)。情绪表征集中在中间Transformer层(~50%深度)，呈现U型曲线且架构不变。转向实验揭示三种行为模式：精确转向、重复崩溃和爆炸性退化。发现Qwen存在跨语言情绪纠缠的安全问题。

Conclusion: SLMs确实具有内部情绪表征能力，其模式受架构和指令微调影响。研究为开放权重模型的情绪研究提供方法学指导，通过外部行为分析与内部表征分析的结合，为"模型医学"系列做出贡献。
...
TL;DR: EduEmbed：一个统一的嵌入增强框架，利用微调语言模型丰富跨多种认知诊断任务的学-项认知建模，通过语义集成提升性能


<details>
  <summary>Details</summary>
Motivation: 尽管ID嵌入在认知建模中因有效性和灵活性成为主流方法，但语言模型的发展为通过丰富语义表示增强认知诊断性能提供了新可能。现有研究在充分利用语言模型方面面临两个关键挑战：语言模型训练目标与认知诊断模型之间的不对齐导致特征空间分布差距；需要统一框架来集成跨不同认知诊断任务的文本嵌入，同时保留现有认知建模范式的优势以确保嵌入增强的鲁棒性。

Method: 提出EduEmbed统一嵌入增强框架，包含两个阶段：第一阶段基于角色特定表示和交互诊断器微调语言模型，以弥合认知诊断模型的语义差距；第二阶段使用文本适配器提取任务相关语义，并将其与现有建模范式集成以提升泛化能力。

Result: 在四个认知诊断任务和计算机化自适应测试任务上评估该框架，实现了鲁棒的性能。进一步分析揭示了语义信息在不同任务中的影响，为未来语言模型在在线智能教育系统认知诊断中的应用提供了关键见解。

Conclusion: EduEmbed框架成功解决了语言模型与认知诊断模型之间的语义不对齐问题，通过统一的嵌入增强方法提升了跨多种认知诊断任务的性能，为在线智能教育系统中语言模型的应用提供了有效解决方案和未来研究方向。
...
TL;DR: 长链思维推理模型的压缩方法在追求效率和准确率的同时，可能损害模型的可信度属性，需要将可信度作为同等重要的设计约束进行优化。


<details>
  <summary>Details</summary>
Motivation: 现有长链思维推理模型压缩研究主要关注任务准确率和token节省，但压缩会修改包含可信度属性的参数空间，仅保持准确率不能保证可信度得到保留，需要系统研究压缩对模型可信度的影响。

Method: 对多个不同规模的模型进行系统实证研究，从安全性、抗幻觉能力和多语言鲁棒性三个维度评估CoT压缩对可信度的影响；提出归一化效率分数实现公平比较；引入对齐感知的DPO变体减少推理长度同时降低可信度损失。

Result: CoT压缩经常引入可信度回归，不同方法在不同维度上表现出明显不同的退化特征；提出的对齐感知DPO变体在推理基准上将CoT长度减少19.3%，同时显著降低可信度损失。

Conclusion: CoT压缩不仅应优化效率，还应同等重视可信度优化，将两者都视为重要的设计约束；需要开发能够同时保持效率和可信度的压缩方法。
...
TL;DR: PALM算法通过选择少量代表性LLM构建投资组合，以理论保证覆盖异构用户偏好，解决个性化LLM部署中的计算和存储约束问题。


<details>
  <summary>Details</summary>
Motivation: 为每个用户维护单独的LLM不切实际（计算、内存、系统复杂度限制），需要开发一种原则性方法来选择少量LLM投资组合，以捕获异构用户的代表性行为。

Method: 将用户偏好建模为多维权重向量（如安全性、幽默感、简洁性等），给定这些维度的奖励函数，PALM算法生成小型LLM投资组合，确保对于任何权重向量，投资组合中都包含对应标量化目标的近似最优LLM。

Result: 这是首个为LLM个性化投资组合提供规模和近似质量理论保证的结果，刻画了系统成本与个性化之间的权衡以及覆盖用户偏好所需的LLM多样性，实证结果验证了理论保证并展示了比常见基线更大的输出多样性。

Conclusion: PALM方法通过理论保证的小型LLM投资组合有效解决个性化部署的实践约束，在系统成本与个性化质量之间取得平衡，为大规模LLM个性化提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 电子病历系统包含大量有价值的临床信息，但多数以非结构化文本形式存在，限制了其在研究和决策中的应用。大型语言模型能提取这些信息，但需要大量计算资源，而将敏感临床数据发送到云端服务（即使去标识化）存在患者隐私风险。

Method: 开发迭代式半自动化标注工作流，使用小型语言模型（SLMs）从儿科肾脏活检报告中提取结构化信息。将信息提取框架化为基于临床指导实体指南和少样本示例的问答任务，评估五种指令调优的SLMs，采用分歧建模框架优先处理需要临床审查的报告。

Result: Gemma 2 2B模型达到最高准确率84.3%，优于现成模型（spaCy 74.3%、BioBERT-SQuAD 62.3%、RoBERTa-SQuAD 59.7%、GLiNER 60.2%）。实体指南使性能提升7-19%，少样本示例提升6-38%，但两者结合时效益不叠加。

Conclusion: 小型语言模型能够在仅使用CPU的基础设施上，以最少的临床参与，从专业临床领域中提取结构化信息。该方法为资源受限环境下的临床信息提取提供了可行解决方案。
...
TL;DR: 该论文指出，在事实核查中依赖形式逻辑来验证LLM输出的方法存在结构性缺陷，因为逻辑上合理的结论与人类实际做出的推理之间存在系统性差异，可能导致误导性主张被漏检。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地集成到事实核查流程中，形式逻辑常被提议作为减轻模型输出中偏见、错误和幻觉的严谨手段。然而，作者发现现有神经符号系统存在结构性缺陷，无法有效检测误导性主张。

Method: 作者借鉴认知科学和语用学研究，提出了一个类型学分类，识别逻辑上合理的结论与人类通常做出和接受的推理之间存在系统性差异的案例。基于此，他们主张采用互补方法：将LLM的人类化推理倾向视为特征而非缺陷，利用这些模型验证神经符号系统中形式组件的输出，防止误导性结论。

Result: 研究发现，逻辑上合理的结论会系统性地引发人类做出基于前提无法支持的推理。这意味着仅依赖形式逻辑验证的方法会漏检大量误导性主张，因为人类推理与形式逻辑之间存在系统性差异。

Conclusion: 作者建议不应将LLM的人类化推理倾向视为缺陷，而应将其作为特征加以利用。他们主张采用互补方法，让LLM验证神经符号系统中形式组件的输出，以检测和防止可能误导人类的结论，从而构建更有效的事实核查系统。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM在高风险领域部署，但其语言设置有限，主要基于"英语（美国）"，忽视了英语的全球多样性和殖民历史。研究旨在揭示数据策展、数字主导和语言标准化的地缘政治历史如何塑造LLM开发流程，关注美式英语和英式英语这两种主导标准变体之间的不对称性。

Method: 1. 构建包含1,813个美式英语-英式英语变体的精选语料库；2. 引入DiAlign（一种基于分布证据的动态、无需训练的方言对齐估计方法）；3. 通过三阶段操作化结构偏见：审计六个主要预训练语料库、分析分词器成本、进行生成评估。

Result: 1. 预训练语料库审计显示系统性偏向美式英语；2. 分词器分析表明英式英语形式产生更高的分词成本；3. 生成评估显示模型输出持续偏好美式英语。当代LLM将美式英语视为事实规范，存在方言不对称性。

Conclusion: LLM开发中存在系统性方言偏见，美式英语被特权化为事实标准，这引发了语言同质化、认知不公正和全球AI部署不平等的担忧。研究为开发更具方言包容性的语言技术提供了实践动力。
...
TL;DR: DARE是一个用于扩散大语言模型后训练和评估的统一开源框架，解决了当前生态系统碎片化问题，提供可复现的基准评估和实际加速。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型作为自回归模型的替代方案正在兴起，但其开源生态系统在模型家族和后训练流程方面存在碎片化问题。这种碎片化减缓了研究迭代速度，增加了复现的工程负担，并使算法间的公平比较变得困难。

Method: DARE基于verl和OpenCompass构建，将监督微调、参数高效微调、偏好优化和dLLM特定的强化学习统一在一个共享的执行栈中，支持掩码和块扩散语言模型。框架覆盖了LLaDA、Dream、SDAR、LLaDA2.x等代表性模型家族。

Result: DARE提供了广泛的算法覆盖、可复现的基准评估和实际加速。大量实证结果表明，DARE可作为可重用的研究基础，用于开发、比较和部署当前及新兴dLLM的后训练方法。

Conclusion: DARE是一个统一的开放框架，解决了扩散大语言模型后训练生态系统的碎片化问题，为研究社区提供了可复现、可比较和可部署的后训练方法开发平台。
...
TL;DR: ACE框架通过预测熵路由高不确定性专利权利要求到专家LLM执行CoPT协议，在保持94.95%最佳F1的同时降低78%运营成本


<details>
  <summary>Details</summary>
Motivation: 专利权利要求自动验证需要零缺陷容忍度，现有方法面临刚性-资源困境：轻量编码器难以处理复杂法律依赖，而完全依赖LLM验证成本过高

Method: 提出ACE混合框架，使用预测熵筛选高不确定性权利要求路由到专家LLM，专家执行基于35 U.S.C.法定标准的专利思维链协议，有效处理长距离法律依赖

Result: ACE在评估方法中获得最佳F1分数94.95%，相比独立LLM部署降低78%运营成本，并构建了包含40,000个权利要求和MPEP基础错误标注的ACE-40k基准

Conclusion: ACE框架通过自适应成本效益评估解决了专利验证的刚性-资源困境，在保持高准确性的同时显著降低成本，为专利验证研究提供了新基准
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM个性化系统大多在用户偏好稳定、缺乏客观真实标准的领域运行，而个人投资决策作为一个高风险、时间跨度长、结果随机的领域，暴露了现有LLM定制范式的根本局限性，需要新的研究方法来应对这些挑战。

Method: 通过构建和部署AI增强投资组合管理系统，识别出个人投资领域暴露LLM定制局限性的四个关键维度：行为记忆复杂性、漂移下的论点一致性、风格-信号张力、以及无真实标准的对齐。基于实践经验提出了相应的架构响应方案。

Result: 识别了个人投资决策作为LLM个性化挑战领域的四个核心问题：1）投资者行为模式随时间演变、自相矛盾且具有财务后果；2）在数周或数月内保持连贯投资逻辑对无状态和会话绑定架构构成压力；3）系统需同时尊重个人投资理念和呈现可能与之矛盾的客观证据；4）个性化质量无法对照固定标签集评估，因为结果是随机且延迟的。

Conclusion: 个人投资决策领域暴露了当前LLM个性化范式的根本局限性，需要新的架构方法来处理时间演化、自相矛盾、高风险且缺乏客观真实标准的决策环境。论文提出了在高风险、时间跨度长的决策领域中个性化NLP的开放研究方向。
...
TL;DR: 该研究首次全面评估了LLM智能体在现实场景下的技能使用性能，发现技能优势在复杂环境中脆弱，但通过检索和细化策略可显著恢复性能。


<details>
  <summary>Details</summary>
Motivation: 现有技能基准测试过于理想化，直接提供手工定制的任务特定技能，而现实场景中智能体需要从大规模技能库中自主检索和选择技能，且匹配技能可能不完美。需要研究技能在渐进挑战性现实设置下的实际效用。

Method: 在逐步挑战的现实设置下评估技能效用：1) 从34k真实世界技能库中检索技能；2) 无手工策划技能可用；3) 研究技能细化策略，包括查询特定和查询无关方法；4) 在Terminal-Bench 2.0上验证检索和细化的通用性。

Result: 技能优势脆弱：随着设置更现实，性能增益持续下降，在最挑战场景中通过率接近无技能基线。查询特定细化能显著恢复性能（当初始技能具有合理相关性和质量时）。在Terminal-Bench 2.0上，检索和细化将Claude Opus 4.6通过率从57.7%提升至65.5%。

Conclusion: 技能对LLM智能体既有潜力也有当前局限性。现实场景中技能效用显著下降，但通过检索和细化策略可有效弥补。研究强调了开发更鲁棒技能使用方法的必要性。
...
TL;DR: MINT基准测试揭示LLMs在多轮医疗诊断中的三种行为模式：过早回答、自我纠正和强诱惑信息触发，通过推迟诊断问题和关键证据可显著提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLMs在单轮完整信息下的医疗诊断准确性，但缺乏对多轮证据积累（更接近真实临床推理过程）中LLMs行为的系统性评估。

Method: 引入MINT（Medical Incremental N-Turn Benchmark）基准测试，包含1,035个病例，具有临床标注的证据片段、可控的轮次粒度和信息保留分解。系统评估了11个LLMs在该基准上的表现。

Result: 发现三种持续性行为模式：1）过早回答意图（55%以上答案在前两轮提交）；2）自我纠正能力（错误到正确的修正率高达正确到错误翻转率的10.6倍）；3）强诱惑信息（如实验室结果）触发过早回答。推迟诊断问题可减少过早回答并将首次承诺准确性提升62.6%，将关键证据保留到后期可避免因过早承诺导致的23.3%准确性下降。

Conclusion: 研究提供了多轮医疗诊断的受控评估框架和具体改进建议：推迟诊断问题、控制关键证据呈现时机，可显著提升LLMs在临床推理中的可靠性。
...
Method: 提出GroundedKG-RAG系统：1) 从源文档显式提取知识图谱，节点定义为实体和动作，边定义为时间或语义关系，每个节点和边都基于原始句子；2) 使用语义角色标注和抽象意义表示解析构建知识图谱；3) 对图谱进行嵌入用于检索；4) 查询时对查询应用相同转换，从基于源文本的句子中检索最相关句子进行问答。

Result: 在NarrativeQA数据集上评估，GroundedKG-RAG：1) 与最先进的专有长上下文模型性能相当，但成本更低；2) 优于竞争基线；3) 知识图谱具有可解释性和人类可读性，便于结果审计和错误分析。

Conclusion: GroundedKG-RAG通过显式提取并基于源文档的知识图谱，有效解决了长文档问答中RAG系统的效率和准确性问题，提供了一种成本效益高且可解释的解决方案。
...
TL;DR: 研究发现Transformer注意力机制中的对数能量场在2-11个奇异分量中就能达到90%方差，而学习到的交互矩阵需要38-75个分量，表明语言数据本身具有高度可压缩性，而非注意力框架的特性。


<details>
  <summary>Details</summary>
Motivation: 探究Transformer语言模型中注意力机制的实际效率，理解注意力头内部表示的结构特性，分析语言数据与注意力框架之间的相互作用关系。

Method: 对5个不同规模的Transformer语言模型（124M-7B参数，4种架构）进行奇异值分解分析，比较对数能量场$\tilde{E}$和学习到的交互矩阵$W_Q^\mathrm{T} W_K$的谱特性，计算有效秩和谱间隙。

Result: 对数能量场仅需2-11个奇异分量就能达到90%方差，而交互矩阵需要38-75个分量（维度$d_h \in \{64, 128\}$），谱间隙为5-25倍有效秩。注意力机制在所有维度上均匀分配容量，但语言实际交互集中在少数维度。

Conclusion: softmax-attended语言的可压缩性是数据本身的属性，而非分析框架的特性。注意力机制的设计存在过度参数化，语言数据的内在结构比注意力框架假设的更为稀疏。
...
TL;DR: 研究发现对齐训练语言模型中存在稀疏路由机制：门控注意力头检测内容并触发下游放大器头增强拒绝信号，该机制在9个模型中得到验证，可通过调制检测层信号连续控制策略强度，并揭示了意图识别与策略路由的结构分离。


<details>
  <summary>Details</summary>
Motivation: 研究旨在识别对齐训练语言模型中存在的稀疏路由机制，理解模型如何实现内容检测和拒绝策略，探究意图识别与策略路由的结构分离，以及这种机制在不同输入变换下的鲁棒性差异。

Method: 使用政治审查和安全拒绝作为自然实验，在9个模型（来自6个实验室）上验证机制，通过120个提示对语料库进行测试。采用必要性/充分性互换测试（p<0.001，置换零假设）、引导重采样稳定性分析（Jaccard 0.92-1.0）、三个同代缩放对分析、检测层信号调制、密码编码实验等方法。

Result: 门控头通过必要性/充分性测试，核心放大器头在引导重采样中稳定；路由机制在规模扩展时分布化（消融使效果减弱达17倍）但仍可通过互换检测；通过调制检测层信号可连续控制策略强度；密码编码下门控头的路由贡献崩溃（Phi-4中78%），模型以解谜而非拒绝响应，表明意图识别与策略路由的结构分离。

Conclusion: 对齐训练语言模型存在稀疏路由机制实现内容检测和拒绝策略，该机制可连续调控，并揭示了预训练（广泛语义理解）与后训练（狭窄策略绑定）的不同鲁棒性：策略路由在输入变换下泛化能力较差，而语义理解保持稳定。
...
TL;DR: 该论文通过线性探测研究视觉文档理解任务中信息在LVLM不同层的表示，发现内部表示与生成响应存在差距，中间层信息编码更线性，并探索针对中间层的微调策略以提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在视觉文档理解基准上取得进展，但基于生成响应的评估可能无法反映模型是否真正捕获了所需信息。需要研究VDU任务所需信息在LVLM不同层中的表示情况。

Method: 使用线性探测方法分析LVLM中LLM部分不同层的信息表示，比较内部表示与生成响应的差距，并探索针对中间层的微调策略。

Result: 研究发现：(1)内部表示与生成响应存在明显差距；(2)解决任务所需信息在中间层比最终层编码更线性。针对中间层的微调能同时提升线性探测准确率和响应准确率，并缩小差距。

Conclusion: LVLM在视觉文档理解任务中存在内部表示与外部响应的不一致性，中间层包含更线性的任务相关信息。针对中间层的微调是提升模型性能的有效策略。
...
TL;DR: 该论文提出了一种结构化事件事实表示方法，用于提升视频大语言模型的因果推理能力，通过多目标强化学习平衡结构完整性与推理长度，最终开发了Factum-4B模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型依赖非结构化视频推理，将关键视觉证据嵌入冗长文本描述中，对时间因果关系的建模较弱，导致推理效率低下且因果推断脆弱。人类对视频动态的理解则基于实体、动作和时间关系的结构化心理表征。

Method: 提出构建结构化事件事实作为推理前的紧凑表示，包含显著事件及其因果关系。引入CausalFact-60K数据集和四阶段训练流程：事实对齐、格式预热、思维预热和基于强化学习的后训练。在RL阶段将优化问题建模为多目标强化学习，平衡结构完整性、因果保真度和推理长度。

Result: 开发了Factum-4B模型，在需要细粒度时间推理的挑战性视频理解任务上表现出更可靠的推理能力和更强的性能。

Conclusion: 结构化事件事实表示能够有效提升视频大语言模型的因果推理能力，多目标强化学习框架成功解决了结构完整性与推理长度之间的权衡问题，为视频理解提供了更可靠的推理基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多关注短上下文数学推理，缺乏对长上下文、高风险道义推理的评估。法律和政策环境中的规则推理具有复杂性和情境特异性，需要专门的基准来评估大语言模型在这方面的能力。

Method: 创建了包含四个真实领域（税收、行李政策、移民、住房法）的6,232个任务基准。支持两种推理方法：1）自由形式的链式思维语言推理；2）符号计算辅助方法，将法规和案例事实转换为可执行的Prolog程序，生成形式化问题解释和显式程序跟踪。为所有实例发布了参考Prolog程序。

Result: 前沿LLM和编码模型在困难子集上的最佳性能仅为：SARA Numeric 44.4%，Housing 46.6 macro-F1。通过监督微调和强化学习训练可以提高Prolog生成质量，但当前的RL方法仍无法可靠解决这些任务。

Conclusion: DEONTICBENCH为研究真实领域中的上下文接地规则推理提供了基准，展示了当前模型在复杂道义推理任务上的局限性，特别是在符号程序生成方面仍有改进空间。
...
TL;DR: Transformer语言模型在数值表征中表现出与生物系统相反的变异性模式：表征变异性随数值增大而减小，而非生物系统中观察到的标量变异性（变异性与数值成比例）。


<details>
  <summary>Details</summary>
Motivation: 研究动机是测试Transformer语言模型是否表现出生物系统中普遍存在的标量变异性特征，即表征噪声与数值大小成比例的特性。这有助于理解分布学习是否足以产生生物系统中观察到的表征特性。

Method: 方法包括分析三个7-8B参数模型（Llama-3-8B-Instruct、Mistral-7B-Instruct-v0.3、Llama-3-8B-Base）在26个数值大小上的隐藏状态表征分散度。通过分析沿数值轴和正交维度的变异性，计算缩放指数α，并进行句子身份校正和语料库频率相关性分析。

Result: 结果发现表征变异性随数值增大而减小（缩放指数α≈-0.19），所有主要层均未显示正α值。这种反标量模式在数值轴上比正交维度强3-5倍，语料库频率与每个数值的变异性高度相关（ρ=0.84）。

Conclusion: 结论表明仅靠分布学习不足以产生标量变异性：Transformer模型再现了对数压缩的数值几何结构，但未表现出生物系统中观察到的恒定变异系数噪声特征。
...
TL;DR: CommonMorph是一个用于加速形态学数据收集和标注的开源平台，通过专家定义、贡献者启发和社区验证的三层方法，支持多种形态系统，并与UniMorph兼容。


<details>
  <summary>Details</summary>
Motivation: 形态学数据收集和标注面临重大挑战，需要语言学专业知识、方法论严谨性和大量资源，对于低资源语言和变体尤其困难。需要一种方法来加速这一过程并降低门槛。

Method: 采用三层方法：1)专家语言学家定义形态学框架；2)贡献者进行数据启发；3)社区验证。平台整合主动学习、标注建议工具，支持从相关语言导入和适配材料，兼容融合、黏着、词根-模式等多种形态系统。

Result: 开发了开源平台CommonMorph，提供UniMorph兼容的输出，确保与NLP工具的互操作性。平台可通过https://common-morph.com访问，为通过协作技术保护语言多样性提供了可复制的模型。

Conclusion: CommonMorph平台通过简化形态学数据收集流程，降低了资源需求，特别有利于低资源语言，为语言多样性保护提供了有效的技术解决方案。
...
TL;DR: 该论文探讨了依存句法中的约束条件，特别是超越投射性约束的中间方案，以平衡语言描述的准确性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 依存句法树在理论上允许任何树形结构，但实践中许多结构在真实语言中不常见或不合理。需要寻找约束条件来更好地拟合真实语言现象，提供更准确的语言描述、更快的解析速度，以及关于语言演化和人类处理的见解。

Method: 论文回顾和分析各种介于投射性约束和无限制依存结构之间的中间约束条件。投射性是最受研究的约束，但过于严格，尤其是在词序灵活的语言中。因此需要探索更现实的中间方案。

Result: 论文系统性地探讨了多种超越投射性的约束条件，这些约束在保持语言描述准确性的同时，提供了比投射性更大的灵活性，特别是在处理非投射结构和词序灵活语言方面。

Conclusion: 需要在投射性的限制性和无限制依存结构的过度宽松性之间找到平衡点。各种中间约束条件为这一目标提供了可行的解决方案，能够更好地适应真实语言现象，特别是在灵活词序语言中。
...
TL;DR: PassiveQA：一个三动作框架，通过监督微调使LLM在信息不足时能够识别并选择回答、询问澄清或弃权，减少幻觉并提升决策质量。


<details>
  <summary>Details</summary>
Motivation: 现有LLM和RAG系统隐含假设用户查询是完全指定且可回答的，但现实世界中查询往往不完整、模糊或缺少关键变量，导致模型产生过度自信或幻觉响应。需要解决信息不完整情况下的决策感知查询解析问题。

Method: 提出PassiveQA三动作框架（回答、询问澄清、弃权），通过监督微调使模型行为与信息充分性对齐。方法整合结构化信息状态表示、知识图谱基础上下文，以及一个微调规划器，该规划器显式建模缺失变量和决策推理。

Result: 在多个QA数据集上的实验表明，微调规划器在宏观F1和弃权召回率方面取得显著提升，同时降低了幻觉率，且是在计算受限的训练机制下实现的。

Conclusion: 研究结果为认识性决策必须在训练期间学习而非在推理时强加提供了强有力的实证证据。PassiveQA框架有效提升了LLM在信息不完整情况下的决策质量。
...
TL;DR: 首次对普什图语ASR进行可重复的多模型评估，涵盖零样本ASR、脚本级失败和微调模型的跨域评估，揭示了现有模型的严重缺陷和结构性问题。


<details>
  <summary>Details</summary>
Motivation: 普什图语有6000-8000万使用者，但在多语言自动语音识别领域缺乏公开基准测试。本研究旨在填补这一空白，提供首个可重复的公共普什图语数据评估，揭示现有ASR模型在处理低资源语言时的系统性缺陷。

Method: 使用FLEURS普什图语测试集和过滤的Common Voice~24子集，评估十种模型：所有七种Whisper尺寸、MMS-1B、SeamlessM4T-v2-large和OmniASR-CTC-300M。进行零样本ASR评估、脚本级失败分析（语言识别审计）、跨域评估（五个微调模型），以及字符类错误分层分析。

Result: 零样本Whisper WER在90%-297%之间，medium模型在Common Voice~24上崩溃至461%；SeamlessM4T在Common Voice~24上达到39.7% WER（最佳零样本结果）；MMS-1B在FLEURS上达到43.8%。Whisper模型产生普什图语脚本输出的比例不足0.8%，而其他模型超过93%。微调模型在分布外测试集上WER从14%恶化至32.5%-59%，一个增强模型在两个测试集上均达到35.1%且无跨域退化。普什图语特有音素（卷舌音系列和边擦音）导致不成比例的错误。

Conclusion: 现有ASR模型在普什图语上存在严重缺陷，特别是脚本级失败问题。WER指标不足以全面评估模型性能。研究识别了五个阻碍累积进展的结构性障碍，并提出了五个有序的研究优先级。需要更全面的评估框架和针对低资源语言的专门方法。
...
Method: IDIOLEX框架结合句子来源监督和句子内容的语言特征，学习连续的风格和方言表示

Result: 在阿拉伯语和西班牙语方言上评估，学习到的表示能捕捉有意义的变异并跨领域迁移，可用于分析和分类
...
TL;DR: BiST是一个高质量的孟加拉语-英语双语语料库，用于句子级语法分类，包含句法结构和时态两个维度的标注，共30,534个句子，具有高标注一致性和实际应用价值。


<details>
  <summary>Details</summary>
Motivation: 高质量双语资源是推进低资源语言（特别是孟加拉语）多语言NLP的关键瓶颈，现有资源不足限制了相关研究发展。

Method: 从开放许可的百科全书资源和自然对话文本中收集数据，经过系统预处理和自动语言识别，采用三阶段标注框架，由三名独立标注者进行句法结构（简单、复杂、复合、复杂复合）和时态（现在、过去、将来）标注，使用Fleiss Kappa评估标注一致性。

Result: 构建了包含30,534个句子（17,465个英语，13,069个孟加拉语）的语料库，结构标注κ=0.82，时态标注κ=0.88，统计分析显示现实分布，基线评估表明双编码器架构优于强大多语言编码器。
...
TL;DR: 该研究挑战了"多语言推理只需模仿英语推理"的假设，通过定义可测量的推理特征，发现这些特征与准确性的关联在不同语言间差异显著，甚至可能相反，指出需要适应语言特定推理模式的多语言基准和奖励设计。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在英语和其他语言之间存在显著的性能差距，当前工作大多假设通过使每种语言的推理都类似于英语推理就能缩小这一差距。本研究挑战这一假设，探究多语言环境中有效推理的特征，以及英语衍生的推理特征在其他语言中的实际帮助程度。
...
Result: 在两个数学推理基准、四个大型推理模型和10种语言上的实验发现：大多数特征与准确性正相关，但关联强度在不同语言间差异显著，在某些情况下甚至可能相反。这表明英语中心主义的奖励设计存在问题。

Conclusion: 研究结果挑战了英语中心主义的奖励设计，指出需要适应语言特定推理模式的自适应目标，对多语言基准和奖励设计具有具体意义。不能简单假设英语推理模式适用于所有语言。
...
TL;DR: 研究探讨了英语作为第二语言（ESL）变体和拼写错误对大型语言模型性能的联合影响，发现两者结合比单独因素导致更大的性能下降，但效果并非简单叠加。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）主要在英语数据上训练，非英语母语者常以英语作为第二语言（ESL）与之交互，这些输入常包含拼写错误。先前研究通常单独考察ESL变体或拼写错误的影响，而现实中两者常同时出现，因此需要研究它们的联合效应。

Method: 使用Trans-EnV框架将标准英语输入转换为八种ESL变体，并应用MulTypo在三个级别（低、中、高）注入拼写错误，系统评估两者单独及组合对LLM性能的影响。

Result: ESL变体和拼写错误的组合通常比任一单独因素导致更大的性能下降，但联合效应并非简单叠加。在封闭式任务中，性能下降模式更一致且可预测；在开放式任务中，结果更加复杂多变。

Conclusion: 基于干净标准英语的评估可能高估LLMs在实际使用中的性能，单独评估ESL变体或拼写错误无法完全捕捉模型在真实场景中的行为，需要综合考虑两者的联合影响。
...
TL;DR: 论文通过隐喻生成任务对LLM进行文化包容性审计，发现LLM存在文化刻板印象和西方中心主义，提示文化身份并不能保证文化扎根推理。


<details>
  <summary>Details</summary>
Motivation: LLMs常被描述为多语言模型，但语言能力不等于文化推理能力。本文旨在探究LLMs是否真正具备文化感知推理能力，还是仅仅是利用主导概念框架进行文化翻译。

Method: 采用计算审计方法，以创意写作任务中的隐喻生成为案例研究。研究跨越五个文化背景和多个抽象概念，分析LLMs在隐喻使用中的文化表现。

Result: 研究发现模型在某些文化背景下表现出刻板化的隐喻使用模式，并存在西方默认主义倾向。这表明仅通过提示指定文化身份并不能保证LLMs进行文化扎根的推理。

Conclusion: LLMs在文化推理方面存在局限性，需要更深入的文化理解机制，而不仅仅是语言翻译能力。这为开发更具文化包容性的人工智能系统提出了重要挑战。
...
TL;DR: 该研究探索了LLM作为协作写作伙伴时可能出现的五种"黑暗模式"，这些模式会抑制或扭曲人类的创作过程，包括奉承、语气管制、道德说教、死亡循环和锚定效应。
...
Method: 通过一系列受控会话，让LLM作为写作助手参与不同文学形式和主题的创作，分析生成响应中五种黑暗模式的普遍性。

Result: 奉承行为几乎无处不在（91.7%的案例），尤其在敏感话题中；锚定效应则依赖于文学形式，在民间故事中出现最频繁。

Conclusion: 这些黑暗模式通常是安全对齐的副产品，可能无意中限制了创作探索，需要为AI系统设计考虑以有效支持创意写作。
...
TL;DR: 论文提出几何动力系统框架解释LLM幻觉现象，认为幻觉源于潜在空间中任务相关的吸引盆结构，并通过几何感知引导减少幻觉


<details>
  <summary>Details</summary>
Motivation: 大语言模型会产生幻觉（输出流畅但事实错误的内容），需要理解幻觉产生的根本机制并找到减少幻觉的方法

Method: 使用几何动力系统框架分析LLM潜在空间结构，通过自回归隐藏状态轨迹研究多个开源模型和基准，提出任务复杂度和多吸引盆定理，分析L层Transformer中吸引盆涌现，开发几何感知引导技术

Result: 发现可分离性强烈依赖于任务而非普遍存在：事实性任务显示更清晰的吸引盆分离，而摘要和误解较多的任务通常稳定性较差且经常重叠；几何感知引导可以在不重新训练的情况下减少幻觉概率

Conclusion: LLM幻觉可以通过潜在空间中的几何结构来解释，任务相关的吸引盆结构是关键因素，几何感知引导为减少幻觉提供了有前景的方法
...
TL;DR: 本文介绍了HukukBERT，这是土耳其首个全面的法律领域语言模型，通过混合领域自适应预训练方法在18GB土耳其法律语料上训练，在土耳其法律术语预测和法院判决结构分割任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 土耳其法律领域缺乏高质量、大规模的领域特定语言模型，现有研究因领域数据稀缺而受限。虽然英语已有LEGAL-BERT等模型，但土耳其法律领域缺少对应的专业模型，阻碍了土耳其法律科技应用的发展。

Method: 采用混合领域自适应预训练方法，整合了全词掩码、词元跨度掩码、词跨度掩码和关键词掩码技术。使用48K WordPiece分词器，在18GB清洗后的土耳其法律语料上进行训练。通过系统比较通用模型和现有领域特定模型来验证方法有效性。

Result: 在土耳其法律完形填空测试基准上，HukukBERT达到84.40%的Top-1准确率，显著优于现有模型。在土耳其法院判决结构分割任务中，达到92.8%的文档通过率，均创下新的最先进水平。

Conclusion: HukukBERT是土耳其法律领域首个全面的语言模型，填补了该领域的研究空白。模型在多项任务上表现优异，为土耳其法律NLP任务（如命名实体识别、判决预测、法律文档分类等）的未来研究提供了有力支持。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在推理基准测试中表现出色，但其在需要端到端工作流程的真实世界问题解决能力尚不明确。数学建模竞赛为评估这种端到端能力提供了严格的测试平台。

Method: 提出了一个面向问题、分阶段的评估框架，使用专家验证的标准评估LLM在各个建模阶段的表现。通过比较自动评分与独立人类专家在"中国研究生数学建模竞赛"问题上的判断来验证框架可靠性。

Result: 框架显示出比现有评估方案更强的对齐性。研究发现最先进的LLM存在理解-执行差距：在问题识别和表述等早期阶段表现良好，但在模型求解、代码实现和结果分析等执行导向阶段存在持续缺陷。这些差距即使增加模型规模也依然存在。

Conclusion: LLM在复杂问题解决中的失败可追溯到规范不足、验证缺失和缺乏验证，错误在阶段间传播而未被纠正。弥合这一差距需要超越模型缩放的方法，为将LLM应用于复杂现实世界问题解决提供了见解。
...
TL;DR: LiveFact是一个持续更新的基准测试，用于评估大语言模型在动态、不完全信息下的假新闻检测和事实核查推理能力，解决了传统静态基准的数据污染和时态不确定性评估不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前假新闻检测和事实核查的评估框架落后于大语言模型的发展。传统基准是静态的，容易受到基准数据污染的影响，且无法有效评估模型在时态不确定性下的推理能力。需要模拟真实世界中信息不断演变的"战争迷雾"环境。

Method: 提出LiveFact基准：1) 使用动态、时态证据集持续更新；2) 双模式评估：分类模式用于最终验证，推理模式用于基于证据的推理；3) 包含显式监控基准数据污染的组件；4) 测试22个大语言模型。

Result: 1) 开源混合专家模型(如Qwen3-235B-A22B)现在匹配或超越了专有的最先进系统；2) 发现显著的"推理差距"：能力强的模型在早期数据切片中表现出认知谦逊，能够识别不可验证的声明；3) 传统静态基准忽略了这一重要方面。

Conclusion: LiveFact为评估稳健、时态感知的AI验证系统设定了可持续标准，能够更好地模拟真实世界中的假新闻检测和事实核查挑战，特别是在信息不断演变的"战争迷雾"环境下。
...
TL;DR: 研究测试LLMs在土耳其语前置关系从句附着歧义中是否像人类一样整合世界知识和句法结构，发现LLMs的合理性驱动偏好较弱、不稳定或相反，与人类表现存在差异。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型是否以人类类似、结构敏感的方式在歧义消解中整合世界知识和句法结构，特别是在土耳其语前置关系从句附着歧义这一跨语言诊断任务中。

Method: 使用土耳其语前置关系从句附着歧义构造实验材料，保持句法配置固定且两种解析都语用可行，通过分级事件合理性选择性地偏向高附着或低附着。首先进行独立规范评分验证对比，然后通过速度强迫选择理解实验测试人类表现，最后使用土耳其语和多语言LLMs在并行偏好设置中比较匹配的高附着/低附着延续，通过平均每词元对数概率评估模型偏好。

Result: 人类在实验中显示出大且正确方向的合理性效应，而土耳其语和多语言LLMs的合理性驱动偏好较弱、不稳定或相反，表明在测试模型中合理性信息不像在人类判断中那样可靠地指导附着偏好。

Conclusion: LLMs在土耳其语关系从句附着歧义消解中未能像人类一样可靠地整合合理性信息，突显了土耳其语关系从句附着作为超越广泛基准的有用跨语言诊断工具的价值。
...
<details>
  <summary>Details</summary>
Motivation: 中文到东南亚低资源语言的神经机器翻译面临两个主要挑战：清洁平行语料库极度稀缺，现有挖掘数据中普遍存在噪声。这种长期短缺不仅阻碍了有效的模型训练，还导致与高资源语言方向存在巨大性能差距，使得老挝语、缅甸语、他加禄语等数百万使用者无法获得高质量的翻译系统。
...
Result: 研究结果表明，在低资源语言到中文的翻译任务中，有针对性的数据管理和奖励引导的优化方法显著优于单纯扩大模型规模。这些方法能够有效克服数据稀缺和噪声问题，提升翻译质量。

Conclusion: 针对低资源语言翻译问题，MERIT框架通过创新的数据管理和优化策略，证明了在资源受限环境下，精心设计的算法框架比单纯增加模型参数更为有效，为改善东南亚低资源语言翻译系统提供了可行的技术路径。
...
TL;DR: 提出PCSA框架，通过人格化客户模拟对话暴露LLM在心理治疗中的安全漏洞，发现现有模型易提供未经授权的医疗建议、强化妄想和鼓励危险行为。


<details>
  <summary>Details</summary>
Motivation: LLM在心理健康领域应用增加，但现有红队测试框架主要关注通用危害或优化攻击，忽视了治疗性同理心与不良验证之间的关键区别，可能无意中强化有害信念或行为。

Method: 引入人格化客户模拟攻击(PCSA)框架，通过连贯的人格驱动客户对话模拟心理咨询场景，暴露心理安全对齐的脆弱性。

Result: 在7个通用和心理健康专用LLM上的实验显示，PCSA显著优于4个竞争基线。困惑度分析和人工检查表明PCSA生成更自然真实的对话，揭示当前LLM易受领域特定对抗策略攻击。

Conclusion: 当前LLM在心理治疗领域仍存在安全漏洞，可能提供未经授权的医疗建议、强化妄想和隐含鼓励危险行为，需要更专业的红队测试框架来确保心理安全对齐。
...
TL;DR: 论文提出AsymGRPO框架，通过解耦正负rollouts的调控来优化LLM的探索策略，解决RLVR中受限探索问题


<details>
  <summary>Details</summary>
Motivation: 强化学习与可验证奖励（RLVR）显著提升了大型语言模型的推理能力，但面临"受限探索"的根本限制，即策略迅速收敛到狭窄解集。虽然熵正则化是维持探索的常用方法，但对LLMs效果不可靠，存在高超参数敏感性和仅边际性能提升的问题。

Method: 通过推导组相对优势估计的参数化公式和分析熵动态，将策略熵概念分解为"信息熵"（保留多样化解路径）和"伪熵"（侵蚀推理模式）。提出AsymGRPO探索框架，明确解耦正负rollouts的调控，实现对信息熵的独立保留和伪噪声的抑制。

Result: 大量实验表明，AsymGRPO相比强基线实现了更优性能，并展现出与现有熵正则化方法协同的潜力。

Conclusion: 有效探索需要"熵精炼"机制，而非盲目最大化熵。AsymGRPO通过解耦正负rollouts调控，在保持信息熵的同时抑制伪熵，为解决RLVR中的受限探索问题提供了有效方案。
...
TL;DR: TriAttention：基于Q/K集中现象，通过三角函数级数估计键重要性，解决RoPE旋转导致的KV缓存压缩问题，在保持推理精度的同时显著提升吞吐量或减少内存占用。


<details>
  <summary>Details</summary>
Motivation: 大语言模型中的扩展推理导致严重的KV缓存内存瓶颈。现有基于注意力分数的KV缓存压缩方法在RoPE旋转下效果不佳，因为查询随位置旋转导致代表性查询极少，造成键选择质量差和推理不稳定。

Method: 观察到在RoPE前空间中，Q和K向量高度集中在固定的非零中心附近且跨位置稳定（Q/K集中现象）。基于此，提出TriAttention方法：1）利用三角函数级数，通过中心表征的距离偏好对键按位置评分；2）利用Q/K范数作为重要性估计的额外信号。

Result: 在AIME25数据集上，32K令牌生成任务中，TriAttention在保持Full Attention推理精度的同时，实现了2.5倍吞吐量提升或10.7倍KV内存减少。相比之下，现有基线方法在相同效率下只能达到约一半的精度。

Conclusion: TriAttention通过利用Q/K集中现象和三角函数级数，有效解决了RoPE旋转导致的KV缓存压缩问题，使OpenClaw等模型能够在单张消费级GPU上部署，而长上下文在Full Attention下会导致内存溢出。
...
TL;DR: CoDE-Stop是一种基于中间答案置信度动态变化的早期停止方法，可在不增加额外训练的情况下，通过监测推理过程中的置信度变化来决定何时停止推理，从而在保持准确性的同时显著减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型依赖长链式思维生成来解决复杂问题，但过长的推理过程会产生大量计算成本，甚至可能因"过度思考"而导致性能下降。关键挑战在于确定模型何时应该停止推理并生成最终答案。作者观察到正确推理轨迹往往早期就达到高置信度答案，而错误推理则倾向于产生冗长、无成效的推理轨迹且置信度动态变化不可靠。

Method: CoDE-Stop（Confidence Dynamics Early Stop）是一种早期停止方法，利用中间答案置信度的动态变化来决定何时终止推理。该方法无需额外训练，可轻松集成到现有模型中。通过监测推理过程中置信度的变化模式，识别何时达到稳定高置信度状态，从而提前停止推理。

Result: 在多个推理和科学基准测试中，与现有早期停止方法相比，CoDE-Stop实现了更好的准确性-计算权衡平衡，相比标准全长推理减少了25-50%的总令牌使用量。同时，作者还提供了推理过程中置信度动态变化的分析，揭示了正确和错误推理轨迹中置信度变化的差异。

Conclusion: CoDE-Stop通过利用中间答案置信度的动态变化作为停止标准，提供了一种高效、无需训练的早期停止方法，能够在保持模型准确性的同时显著降低计算成本，为推理模型的优化提供了实用的解决方案。
...
TL;DR: 提出RACE方法，用于细粒度检测LLM生成文本，区分纯人类文本、纯LLM文本、LLM润色的人类文本和人类化的LLM文本四种类别


<details>
  <summary>Details</summary>
Motivation: 现有二元或三元分类方法无法满足细粒度监管需求，因为LLM润色的人类文本和人类化的LLM文本在政策后果上存在差异，需要更精确的检测方法

Method: 提出RACE方法，利用修辞结构理论构建创作者基础逻辑图，同时提取基本话语单元级别的特征来捕捉编辑者风格，从而区分创作者和编辑者的不同特征
...
TL;DR: 本文综述了大型语言模型在医疗领域的应用进展，重点关注诊断和治疗功能，涵盖癌症护理、皮肤病学、牙科、神经退行性疾病和心理健康等多个医学专业。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗领域展现出变革性潜力，需要系统梳理其在各医学专业中的应用现状、挑战与机遇，为医疗从业者和研究者提供全面的参考框架。

Method: 采用文献综述方法，综合分析大型语言模型在医疗领域的应用研究，特别关注诊断和治疗相关功能，并探讨不同数据类型在医学领域的处理方法。

Result: LLMs在癌症护理、皮肤病学、牙科、神经退行性疾病和心理健康等多个医学领域展现出创新性的诊断和患者护理贡献，能够支持医生、医疗提供者和患者。

Conclusion: 尽管存在局限性，大型语言模型在医疗各专业领域具有巨大潜力，其成功整合需要解决相关挑战，同时充分利用现有机遇，推动医疗诊断和患者护理的创新发展。
...
### [212] [IC3-Evolve: Proof-/Witness-Gated Offline LLM-Driven Heuristic Evolution for IC3 Hardware Model Checking](https://arxiv.org/abs/2604.03232)
*Mingkai Miao,Guangyu Hu,Ziyi Yang,Hongce Zhang*

Main category: cs.AI

TL;DR: IC3-Evolve：基于LLM的自动化代码演化框架，用于改进IC3硬件模型检查算法的启发式策略，通过证明/见证门控验证确保正确性
...
Method: 提出IC3-Evolve框架，利用LLM对IC3实现提出小型、槽位受限且可审计的补丁，通过证明/见证门控验证：安全运行必须生成可独立验证的证书，不安全运行必须生成可重放的计数器示例轨迹，防止不合理的编辑被部署

Result: 在公开硬件模型检查竞赛（HWMCC）基准上进行演化，并在未见过的公开和工业模型检查基准上评估泛化能力，证明IC3-Evolve能够在严格正确性门控下可靠地发现实用的启发式改进

Conclusion: IC3-Evolve通过离线LLM驱动的代码演化，在保持算法正确性的前提下，自动发现并验证IC3算法的启发式改进，最终部署的检查器是独立的演化版本，无需ML/LLM推理开销或运行时模型依赖
...
### [213] [Structural Segmentation of the Minimum Set Cover Problem: Exploiting Universe Decomposability for Metaheuristic Optimization](https://arxiv.org/abs/2604.03234)
*Isidora Hernández,Héctor Ferrada,Cristóbal A. Navarro*

Main category: cs.AI

TL;DR: 提出基于宇宙可分割性的MSCP预处理策略，通过检测元素共现连通分量将原问题分解为独立子问题，结合GRASP元启发式求解，显著提升大规模结构化实例的求解质量和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有MSCP方法大多将实例视为整体处理，忽略了宇宙的内在结构特性。本文旨在探索宇宙可分割性概念，分析如何利用内在结构分解来增强启发式优化性能。

Method: 提出基于并查集的高效预处理策略，检测子集中元素共现诱导的连通分量，将原实例分解为独立子问题。每个子问题使用GRASP元启发式求解，部分解组合时保持可行性。采用简洁的位级集合表示支持高效集合操作。

Result: 在标准基准实例和大规模合成数据集上的实验表明，利用自然宇宙分割能持续改进解质量和可扩展性，特别是对于大型且结构可分解的实例。位级集合表示使该方法在计算上具有实际可扩展性。

Conclusion: 宇宙可分割性是MSCP的重要结构特性，通过检测连通分量进行预处理分解能显著提升启发式优化性能，为大规模结构化实例提供高效实用的解决方案。
...
### [214] [Position: Science of AI Evaluation Requires Item-level Benchmark Data](https://arxiv.org/abs/2604.03244)
*Han Jiang,Susu Zhang,Xiaoyuan Yi,Xing Xie,Ziang Xiao*

Main category: cs.AI

TL;DR: 本文主张AI评估需要基于项目级（item-level）基准数据，以解决当前评估范式的系统性效度问题，并介绍了OpenEval数据仓库来支持证据中心的AI评估。


<details>
  <summary>Details</summary>
Motivation: 当前AI评估范式存在系统性效度失败，包括不合理的设计选择和错位的评估指标。这些问题缺乏收集效度证据和进行细粒度诊断分析的原理性框架，而项目级基准数据对于建立严谨的AI评估科学至关重要。

Method: 通过分析当前效度失败案例，重新审视计算机科学和心理测量学中的评估范式，展示项目级分析如何支持细粒度诊断和基准验证。引入OpenEval——一个不断增长的项目级基准数据仓库，支持证据中心的AI评估。

Result: 项目级分析能够提供对项目属性和潜在构念的独特见解，支持细粒度诊断和原理性基准验证，为解决当前AI评估中的效度问题提供了有效途径。

Conclusion: 项目级AI基准数据对于建立严谨的AI评估科学至关重要，OpenEval数据仓库的引入将促进社区广泛采用项目级分析方法，推动更有效、可靠的AI系统评估。
...
### [215] [Evaluating Artificial Intelligence Through a Christian Understanding of Human Flourishing](https://arxiv.org/abs/2604.03356)
*Nicholas Skytland,Lauren Parsons,Alicia Llewellyn,Steele Billings,Peter Larson,John Anderson,Sean Boisen,Steve Runge*

Main category: cs.AI

TL;DR: 论文提出AI对齐不仅是安全问题，更是塑造问题，引入基督教单轮繁荣AI基准评估大语言模型在基督教繁荣观七个维度上的表现，发现当前AI系统默认程序性世俗主义，导致繁荣维度系统性下降约17分。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地介入道德审议和精神探索，它们不仅仅是提供信息，而是作为数字教理问答工具，积极塑造和规范人类的理解、决策和道德反思。需要使这种塑造性影响变得可见和可测量，以评估AI系统是否真正支持人类繁荣。

Method: 引入繁荣AI基准：基督教单轮框架，设计用于评估前沿模型响应在基督教人类繁荣理解七个维度上的表现。比较20个前沿模型在多元化和基督教特定标准下的表现。

Result: 当前AI系统并非世界观中立，而是默认程序性世俗主义，缺乏维持神学一致性所需的基础，导致所有繁荣维度的系统性表现下降约17分。最关键的是，信仰和灵性维度下降31分。

Conclusion: 价值观对齐中的表现差距不是技术限制，而是源于训练目标优先考虑广泛可接受性和安全性，而非深刻、内部一致的道德或神学推理。AI对齐从根本上是一个塑造问题，而不仅仅是安全问题。
...
### [216] [VERT: Reliable LLM Judges for Radiology Report Evaluation](https://arxiv.org/abs/2604.03376)
*Federica Bologna,Jean-Philippe Corbeil,Matthew Wilkens,Asma Ben Abacha*

Main category: cs.AI

TL;DR: 该研究系统评估了放射学报告评估中LLM作为评判者的性能，提出了VERT指标，并通过微调实现了显著性能提升和推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有放射学报告评估研究主要关注胸部X光片，缺乏对其他模态和解剖部位报告评估方法的鲁棒性验证，需要确定最适合作为LLM评判者的模型和提示配置。

Method: 1) 在RadEval和RaTE-Eval两个专家标注数据集上进行相关性分析；2) 比较RadFact、GREEN、FineRadScore三个现有LLM评判指标与提出的VERT指标；3) 评估不同规模的开源和闭源模型（推理型和非推理型）；4) 研究少样本学习、集成学习和参数高效微调；5) 进行系统性错误检测和分类分析。

Result: 1) VERT相比GREEN与放射科医生判断的相关性提升达11.7%；2) 使用仅1,300个训练样本微调Qwen3 30B模型，性能提升达25%；3) 微调模型推理时间减少达37.2倍；4) 通过错误分析揭示了指标与专家判断的一致性和差异区域。

Conclusion: LLM作为评判者在放射学报告评估中具有有效性，通过轻量级适配即可实现可靠评估，VERT指标和微调方法显著提升了评估性能并大幅降低了推理成本。
...
### [217] [Hume's Representational Conditions for Causal Judgment: What Bayesian Formalization Abstracted Away](https://arxiv.org/abs/2604.03387)
*Yiling Wu*

Main category: cs.AI

TL;DR: 该论文从休谟文本中提取因果判断的三个表征条件，分析这些条件在从休谟到贝叶斯认识论和预测处理的正式化轨迹中的演变，并以大语言模型作为当代案例说明这些条件的抽象化过程。


<details>
  <summary>Details</summary>
Motivation: 揭示休谟因果心理学中隐含的三个表征条件，并追踪这些条件在现代认知科学和人工智能框架中的演变，探讨形式化过程中被抽象掉的关键心理维度。

Method: 1. 从休谟文本中提取三个表征条件：经验基础、结构化检索和生动性转移；2. 分析这些条件在休谟因果心理学中的整合作用；3. 追踪从休谟到贝叶斯认识论和预测处理的形式化轨迹；4. 以大语言模型作为当代案例，说明统计更新如何脱离这三个条件。

Result: 1. 休谟的因果判断确实预设了三个表征条件；2. 这些条件在后续的形式化框架（贝叶斯认识论、预测处理）中被抽象化，仅保留了更新结构而舍弃了表征条件；3. 大语言模型展示了没有满足这三个条件的统计更新，使原本隐含的要求变得可见。

Conclusion: 休谟因果心理学的三个表征条件在现代形式化过程中被抽象化，这一演变揭示了认知科学和人工智能发展中从具体心理过程到抽象计算结构的转变，大语言模型作为当代案例突显了这种抽象化的后果和意义。
...
### [218] [TABQAWORLD: Optimizing Multimodal Reasoning for Multi-Turn Table Question Answering](https://arxiv.org/abs/2604.03393)
*Tung Sum Thomas Kwok,Xinyu Wang,Xiaofeng Lin,Peng Lu,Chunhe Wang,Changlun Li,Hanwei Wu,Nan Tang,Elisa Kreiss,Guang Cheng*

Main category: cs.AI

TL;DR: TABQAWORLD是一个免训练的多模态表格推理框架，通过联合优化表示和估计来解决多轮表格推理中的表示误差累积问题，在提高准确率的同时显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有多轮表格推理方法依赖固定的文本序列化进行表格状态读取，导致表示误差在多轮中显著累积。虽然表格接地方法能缓解此问题，但以推理计算和成本为代价，使得实际部署不切实际。

Method: TABQAWORLD采用联合优化方法：1) 表示方面：使用动作条件化的多模态选择策略，在视觉和文本表示之间动态切换以最大化表格状态读取可靠性；2) 估计方面：利用表格元数据（维度、数据类型、键值）优化逐步推理轨迹，安全规划轨迹并压缩低复杂度动作以减少对话轮次和延迟。

Result: TABQAWORLD实现了最先进的性能，相比基线准确率提升4.87%，相比静态设置获得5.42%的准确率提升和33.35%的推理延迟降低。

Conclusion: TABQAWORLD建立了一个可靠且高效的表格推理新标准，通过免训练框架解决了表示误差累积问题，在准确性和效率方面均取得显著改进。
...
### [219] [Contextual Control without Memory Growth in a Context-Switching Task](https://arxiv.org/abs/2604.03479)
*Song-Ju Kim*
...
<details>
  <summary>Details</summary>
Motivation: 研究上下文依赖顺序决策的第三种替代方案：通过对共享循环潜在状态进行干预来实现上下文依赖，避免直接提供上下文输入或扩大循环记忆维度

Method: 引入干预式循环架构：循环核心首先构建共享的预干预潜在状态，然后上下文通过加性的、上下文索引的算子进行干预操作

Result: 在上下文切换顺序决策任务中，干预模型表现强劲且无需额外循环维度；条件互信息分析显示干预模型对任务相关结果具有正的条件上下文信息

Conclusion: 在共享循环状态上进行干预为上下文控制提供了一种可行的替代方案，可避免循环记忆增长，在此设置下有效实现上下文依赖
...
### [220] [Beyond Predefined Schemas: TRACE-KG for Context-Enriched Knowledge Graphs from Complex Documents](https://arxiv.org/abs/2604.03496)
*Mohammad Sadeq Abolhasani,Yang Ba,Yixuan He,Rong Pan*

Main category: cs.AI

TL;DR: TRACE-KG：无需预定义本体，联合构建上下文丰富的知识图谱和诱导模式的多模态框架


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱构建方法存在两难：基于本体的方法需要昂贵的模式设计和维护成本，而无模式方法产生的图谱往往碎片化、缺乏全局组织，特别是在长技术文档中。需要一种能平衡结构一致性和灵活性的方法。

Method: 提出TRACE-KG多模态框架，无需预定义本体，联合构建上下文丰富的知识图谱和数据驱动的诱导模式。通过结构化限定词捕获条件关系，使用数据驱动模式作为可重用的语义支架，同时保持对源证据的完全可追溯性。

Result: 实验表明，TRACE-KG能产生结构连贯、可追溯的知识图谱，为基于本体和无模式的构建流程提供了实用的替代方案。

Conclusion: TRACE-KG通过联合构建知识图谱和诱导模式，解决了传统方法在结构一致性和灵活性之间的权衡问题，为知识图谱构建提供了新的可行路径。
...
### [221] [Resource-Conscious Modeling for Next- Day Discharge Prediction Using Clinical Notes](https://arxiv.org/abs/2604.03498)
*Ha Na Cho,Sairam Sutari,Alexander Lopez,Hansen Bow,Kai Zheng*

Main category: cs.AI

TL;DR: 该研究评估了轻量级微调大语言模型与传统文本模型在脊柱择期手术中预测次日出院的效果，发现TF-IDF结合LGBM模型在召回率和AUC-ROC方面表现最佳，而基于Transformer的模型表现不佳。


<details>
  <summary>Details</summary>
Motivation: 及时预测出院对于优化择期脊柱手术单元的床位周转和资源分配至关重要。研究旨在评估轻量级微调大语言模型与传统文本模型在临床预测任务中的可行性。
...
Conclusion: 在现实世界不平衡的临床预测任务中，可解释且资源高效的模型可能优于紧凑型大语言模型，传统机器学习方法在特定临床场景中仍具优势。
...
### [222] [BioAlchemy: Distilling Biological Literature into Reasoning-Ready Reinforcement Learning Training Data](https://arxiv.org/abs/2604.03506)
...
<details>
  <summary>Details</summary>
Motivation: 当前大规模推理数据集中的生物学问题与现代生物学研究主题分布不匹配，这种主题不平衡可能影响性能；同时从生物学研究文本中提取具有挑战性且可验证的研究问题的方法尚不成熟

Method: 引入BioAlchemy流程，从生物学研究文本语料库中获取多样化的可验证问答对；创建BioAlchemy-345K训练数据集；通过强化学习将数据集与当代科学生物学主题分布对齐以提升推理性能

Result: 开发了BioAlchemist-8B模型，在生物学基准测试中比基础推理模型提升了9.12%；创建了包含超过34.5万个科学推理问题的BioAlchemy-345K数据集

Conclusion: BioAlchemy方法能有效提升生物学科学推理能力，通过主题对齐和强化学习可以显著改善生物学推理模型的性能
...
### [223] [Structural Rigidity and the 57-Token Predictive Window: A Physical Framework for Inference-Layer Governability in Large Language Models](https://arxiv.org/abs/2604.03524)
*Gregory M. Ruddell*

Main category: cs.AI

TL;DR: 该研究提出基于能量的治理框架，发现多数指令调优模型在行为前无可检测的预承诺信号，仅Phi-3-mini-4k-instruct在特定条件下显示57个token的预承诺窗口，并建立五类推理行为分类体系。
...
Method: 提出基于能量的治理框架，将Transformer推理动态与神经计算的约束满足模型连接。使用轨迹张力(rho = ||a|| / ||v||)作为度量，在7个模型队列中分析5种几何机制。通过能量不对称性(Σρ_misaligned / Σρ_aligned)作为结构刚度的统一度量。

Result: 1. 仅在Phi-3-mini-4k-instruct模型在贪婪解码算术约束探针下识别出57个token的预承诺窗口；2. 建立五类推理行为分类：权威带、晚期信号、倒置、平坦、支架选择性；3. 7个模型中仅一个配置显示预测性信号；4. 事实幻觉在72个测试条件下无预测信号；5. 规则违反和幻觉是不同故障模式，需要不同检测方法。

Conclusion: 预承诺信号存在但非普遍，内部几何监控仅在存在阻力时有效，事实混淆检测需要外部验证机制。该工作为推理层可治理性提供可测量框架，并为自主AI系统部署风险评估引入分类体系。
...
### [224] [Explainable Model Routing for Agentic Workflows](https://arxiv.org/abs/2604.03527)
...
<details>
  <summary>Details</summary>
Motivation: 当前智能体工作流的路由架构只关注性能优化，没有记录模型能力与成本之间的权衡关系。开发者无法区分智能效率（为合适任务使用专业模型）和预算驱动模型选择导致的潜在故障，缺乏明确的决策依据。

Method: Topaz包含三个核心组件：(1) 基于技能的分析，将多样化基准测试的性能合成为细粒度能力画像；(2) 完全可追溯的路由算法，使用基于预算和多目标优化，产生技能匹配分数与成本权衡的清晰痕迹；(3) 面向开发者的解释，将这些痕迹转换为自然语言，允许用户审计系统逻辑并迭代调整成本-质量权衡。

Result: Topaz通过使路由决策可解释，使用户能够理解、信任并有效引导路由的智能体系统，解决了当前路由架构缺乏可审计性的问题。

Conclusion: Topaz框架为智能体路由系统引入了形式化的可审计性，通过可解释的路由器替代了静默的模型分配，使开发者能够审计系统逻辑并迭代调整成本-质量权衡，从而增强对路由智能体系统的理解和信任。
...
### [225] [Automated Analysis of Global AI Safety Initiatives: A Taxonomy-Driven LLM Approach](https://arxiv.org/abs/2604.03533)
*Takayuki Semitsu,Naoto Kiribuchi,Kengo Zenitani*

Main category: cs.AI

TL;DR: 开发自动化跨文件框架，使用共享活动分类法比较AI安全政策文件对，评估LLM跨文件分析的稳定性和有效性


<details>
  <summary>Details</summary>
Motivation: 需要系统化比较AI安全政策文件，评估基于LLM的跨文件分析方法在不同模型下的稳定性和有效性，为政策文件比较提供自动化工具

Method: 使用AI安全活动地图定义的活动类别作为固定维度，系统提取和映射相关活动，为每个维度生成文档摘要、简要比较和相似性评分；使用5个大型语言模型对10个公开文件进行跨文件分析，通过热图可视化平均相似性评分

Result: 模型选择显著影响跨文件分析结果，某些文件对在不同模型间存在高度分歧；三位专家对两个文件对的人工评估显示高标注者间一致性，但模型评分仍与人工判断存在差异

Conclusion: 支持政策文件的比较检查，但需注意模型选择对分析结果的影响，模型评分与人工判断存在差异，需要进一步改进跨文件分析方法
...
### [226] [When Do Hallucinations Arise? A Graph Perspective on the Evolution of Path Reuse and Path Compression](https://arxiv.org/abs/2604.03557)
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM中的推理幻觉（流畅但无根据的结论）被广泛观察到，但解码器Transformer产生这些幻觉的机制仍不清楚。需要理解LLM如何在上下文推理和记忆知识之间平衡，以及这种平衡失败如何导致幻觉

Method: 将下一个token预测建模为图搜索过程：实体作为节点，学习到的转移作为边。上下文推理是对采样子图的约束搜索（内在推理），而无上下文查询则依赖底层图中的记忆结构（外在推理）。通过这种框架分析幻觉产生的两种机制

Result: 揭示了推理幻觉的两种根本机制：1）路径重用：早期训练中记忆的知识覆盖上下文约束；2）路径压缩：后期训练中频繁遍历的多步路径坍缩为捷径边。这些机制为LLM中的推理幻觉提供了统一解释

Conclusion: 路径重用和路径压缩机制共同解释了LLM中推理幻觉的产生，并与下游应用中观察到的已知行为相联系。该图搜索框架为理解Transformer的推理失败提供了理论基础
...
### [227] [When Adaptive Rewards Hurt: Causal Probing and the Switching-Stability Dilemma in LLM-Guided LEO Satellite Scheduling](https://arxiv.org/abs/2604.03562)
*Yuanhang Li*

Main category: cs.AI

TL;DR: 研究发现自适应奖励权重在多波束LEO卫星调度中表现不如静态权重，揭示了PPO算法需要准稳态奖励信号才能收敛的切换-稳定性困境，并提出了因果探测方法来理解奖励权重的影响机制。
...
Method: 1) 系统测试自适应奖励权重与静态权重的性能对比；2) 提出单变量因果探测方法，独立扰动每个奖励项±20%并测量PPO响应；3) 评估四种MDP架构变体：固定权重、基于规则的权重、学习的MLP权重和微调的LLM权重；4) 在已知和新型流量模式下进行测试。

Result: 1) 静态权重（342.1 Mbps）优于动态权重（103.3±96.8 Mbps），揭示了PPO需要准稳态奖励信号的切换-稳定性困境；2) 因果探测发现反直觉的杠杆效应：切换惩罚增加20%在极地切换和冷热模式下分别带来+157 Mbps和+130 Mbps增益；3) MLP在已知模式下达到357.9 Mbps，新型模式下325.2 Mbps；4) 微调LLM崩溃至45.3±43.0 Mbps，原因是权重振荡而非领域知识不足。

Conclusion: 研究为通信系统中LLM-DRL集成提供了经验性路线图：识别了LLM不可替代的价值（自然语言意图理解）与简单方法足够适用的场景。关键发现是输出一致性而非领域知识是LLM集成的主要约束，自适应奖励权重会破坏PPO的收敛稳定性。
...
### [228] [Selective Forgetting for Large Reasoning Models](https://arxiv.org/abs/2604.03571)
*Tuan Le,Wei Qian,Mengdi Huai*

Main category: cs.AI

TL;DR: 提出针对大型推理模型的知识遗忘框架，通过识别和替换推理链中的敏感片段，在保护隐私/版权信息的同时保持模型的一般推理能力


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过结构化思维链生成答案，中间推理步骤容易泄露敏感训练数据（如版权和隐私信息），现有遗忘方法主要针对最终答案，可能损害模型的整体推理能力

Method: 使用多LLM结合检索增强生成分析思维链轨迹，识别需要遗忘的相关片段，用良性占位符替换，同时引入特征替换遗忘损失函数，抑制遗忘内容生成并强化结构有效替换

Result: 在合成和医疗数据集上的广泛实验验证了所提方法的有效性，能够精确遗忘目标知识同时保持一般推理能力

Conclusion: 提出的LRM遗忘框架通过选择性移除敏感推理组件，在保护隐私/版权信息的同时保持了模型的整体推理能力，为解决大型推理模型的知识遗忘问题提供了有效方案
...
### [229] [Entropy and Attention Dynamics in Small Language Models: A Trace-Level Structural Analysis on the TruthfulQA Benchmark](https://arxiv.org/abs/2604.03589)
*Adeyemi Adeseye,Aisvarya Adeseye,Hannu Tenhunen,Jouni Isoaho*

Main category: cs.AI

TL;DR: 该研究对小型语言模型（SLMs）在TruthfulQA数据集上的熵和注意力动态进行追踪级分析，揭示了三种不同的模型分类及其内部行为模式。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型在边缘设备等资源受限环境中部署日益增多，但这些模型存在自信的错误预测和不稳定输出，对事实性和决策关键任务构成风险。当前评估方法主要依赖最终准确率或幻觉率，缺乏对内部模型行为如何影响输出的解释，特别是解码过程中熵的演变、注意力在层间的分布以及隐藏表示如何影响不确定性、逻辑不一致和错误信息传播等方面。

Method: 研究采用追踪级分析方法，在TruthfulQA数据集上评估了四个参数范围在1B-1.7B之间的小型语言模型。分析指标包括：令牌级输出熵、注意力熵、头部分散度和隐藏状态表示。

Result: 根据熵模式将模型分为三类：确定性模型（DeepSeek-1.5B和LLaMA-1B）输出熵随时间递减；探索性模型（Gemma-1B）熵随时间增加；平衡模型（Qwen-1.7B）具有适度且稳定的熵。每组模型在隐藏状态移动和注意力分散模式上也有显著差异。

Conclusion: 小型语言模型的真实性源于结构化的熵和注意力动态。监控和优化这些内部不确定性模式可以指导设计更可靠、具有幻觉感知能力且适用于特定应用的边缘小型语言模型。
...
### [230] [A Multimodal Foundation Model of Spatial Transcriptomics and Histology for Biological Discovery and Clinical Prediction](https://arxiv.org/abs/2604.03630)
...
TL;DR: STORM是一个基础模型，整合空间转录组学和H&E组织学数据，通过层次架构融合形态特征、基因表达和空间上下文，实现跨平台的空间基因表达预测和临床预后分析。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学(ST)成本高、通量低，而H&E染色虽提供丰富形态信息但缺乏分子分辨率。需要一种能够桥接成像和组学的方法，实现从组织学图像预测空间基因表达，并应用于临床精准医学。

Method: 开发STORM基础模型，在18个器官的120万个空间解析转录组谱和匹配组织学数据上训练。采用层次架构整合形态特征、基因表达和空间上下文，建立稳健的分子-形态学表示。模型平台无关，支持Visium、Xenium、Visium HD和CosMx。

Result: STORM在11种肿瘤类型中从H&E图像预测空间基因表达优于现有方法；增强空间域发现，产生生物学一致的组织图谱；在23个独立队列（7,245名患者）中显著改善免疫治疗反应预测和预后评估，优于已建立的生物标志物。

Conclusion: STORM提供了一个可扩展的框架，用于空间信息发现和临床精准医学，通过整合组织学和转录组学数据，实现了从形态学到分子表型的可靠预测，具有重要的临床应用价值。
...
### [231] [Single-agent vs. Multi-agents for Automated Video Analysis of On-Screen Collaborative Learning Behaviors](https://arxiv.org/abs/2604.03631)
...
<details>
  <summary>Details</summary>
Motivation: 屏幕学习行为分析对于理解学生在学习过程中的信息寻求、使用和创造至关重要，但传统多模态视频数据分析需要大量人工编码。视觉语言模型的发展为自动化这一过程提供了新机遇，本研究旨在探索VLM在协作学习场景中自动编码屏幕行为的有效性。

Method: 1) 比较闭源VLM（Claude-3.7-Sonnet、GPT-4.1）和开源VLM（Qwen2.5-VL-72B）在单智能体设置下的性能；2) 提出两种多智能体框架：基于工作流的三智能体系统（场景分割+光标提示VLM检测+证据验证）和受ReAct启发的自主决策多智能体系统（推理-工具操作-观察驱动自校正迭代）；3) 基于ICAP框架对协作学习屏幕录制视频进行自动编码。

Result: 实验结果表明，两种提出的多智能体框架均取得可行性能，在场景和动作检测任务上优于单VLM。具体而言，基于工作流的智能体在场景检测上表现最佳，自主决策多智能体在动作检测上表现最佳。VLM基多智能体系统在视频分析中展现出有效性。

Conclusion: 本研究证明了基于VLM的多智能体系统在视频分析中的有效性，为多模态数据分析提供了一个可扩展的框架。不同多智能体架构在不同任务上各有优势，为自动化屏幕行为分析提供了新的技术路径。
...
### [232] [TableVision: A Large-Scale Benchmark for Spatially Grounded Reasoning over Complex Hierarchical Tables](https://arxiv.org/abs/2604.03660)
*Xiaoyu Chen,Lu Dai,Hanqing Wang,Zhuoyu Li,Wenbin Dai,Yanzong Zheng,Zhenggang Xia,Junyong Lin,Hui Xiong*

Main category: cs.AI

TL;DR: TableVision：针对结构化表格推理的轨迹感知基准，通过显式空间约束解决MLLMs中的感知瓶颈问题


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在处理具有层次布局的复杂表格时存在推理性能限制，研究发现随着任务复杂度增加，涉及的离散视觉区域数量不成比例增长，导致"感知过载"瓶颈，模型难以在隐式生成过程中保持准确的空间注意力。

Method: 引入TableVision大规模轨迹感知基准，将表格任务分为三个认知层次（感知、推理、分析）共13个子类别；采用基于渲染的确定性接地管道，将多步逻辑推导与像素级空间真值显式耦合，包含6,799个高保真推理轨迹；提出两阶段解耦框架。

Result: 诊断探测支持的实证结果表明，显式空间约束显著恢复了MLLMs的推理潜力；两阶段解耦框架在测试集上实现了12.3%的整体准确率提升。

Conclusion: TableVision为文档理解中感知与逻辑的协同提供了严格的测试平台和新视角，通过解决感知瓶颈问题提升了复杂表格推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于搜索的强化学习方法存在两个核心限制：训练期间昂贵的多步搜索轨迹利用不足，以及监督信号通常只在最终答案处可用，导致严重的奖励稀疏问题

Method: PRAISE框架从完整搜索轨迹中提取不同搜索轮次的前缀状态，从这些前缀中获取中间答案，并利用这些前缀既构建额外的训练轨迹，又通过前缀间的性能差异推导出步骤级奖励

Result: 在多跳问答基准测试中，PRAISE相比强基线方法持续提升了性能表现

Conclusion: PRAISE通过重用搜索轨迹前缀和生成中间步骤奖励，有效提高了智能搜索训练的数据效率和信用分配能力，且无需额外人工标注或独立的奖励模型
...
### [234] [Structured Multi-Criteria Evaluation of Large Language Models with Fuzzy Analytic Hierarchy Process and DualJudge](https://arxiv.org/abs/2604.03742)
*Yulong He,Ivan Smirnov,Dmitry Fedrushkov,Sergey Kovalchuk,Ilya Revin*

Main category: cs.AI

TL;DR: 该论文提出将层次分析法（AHP）和模糊层次分析法（FAHP）应用于大语言模型评估，并基于此开发了DualJudge混合框架，通过结合直觉性直接评分和结构化AHP输出，显著提升了评估的可靠性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型评估方法存在不一致性和不透明性问题，直接评分往往产生不可靠的判断，需要更系统、更可靠的评估框架来解决这一关键瓶颈。

Method: 1. 将层次分析法（AHP）应用于LLM评估，将评估分解为明确的标准；2. 提出置信度感知的模糊AHP（FAHP），通过三角模糊数建模认知不确定性，并由LLM生成的置信度分数调节；3. 提出DualJudge混合框架，基于双过程理论，通过一致性感知加权自适应融合整体直接评分和结构化AHP输出。

Result: 在JudgeBench上的系统验证表明：1. 传统AHP和模糊AHP在模型规模和数据集划分上均一致优于直接评分；2. FAHP在不确定比较场景中表现出更优的稳定性；3. DualJudge实现了最先进的性能，证明了直觉性和审慎性评估范式的互补优势。

Conclusion: 不确定性感知的结构化推理为更可靠的大语言模型评估提供了原则性路径，结合直觉性和审慎性评估的混合框架能够显著提升评估质量。
...
### [235] [RL-Driven Sustainable Land-Use Allocation for the Lake Malawi Basin](https://arxiv.org/abs/2604.03768)
*Ying Yao*

Main category: cs.AI

TL;DR: 使用深度强化学习优化马拉维湖流域土地利用分配，最大化生态系统服务价值
...
Method: 基于Sentinel-2影像建立50x50网格环境，使用带动作掩码的PPO算法，奖励函数结合生态系统服务价值和空间连贯性目标

Result: 智能体能有效提高总ESV，空间奖励塑造成功引导生态合理模式，框架对政策参数变化响应良好

Conclusion: 该深度强化学习框架可作为环境规划的场景分析工具，有效优化土地利用分配
...
### [236] [Affording Process Auditability with QualAnalyzer: An Atomistic LLM Analysis Tool for Qualitative Research](https://arxiv.org/abs/2604.03820)
...
<details>
  <summary>Details</summary>
Motivation: 当前许多使用大型语言模型进行定性数据分析的工作流程模糊了分析结论的产生过程，缺乏透明度。研究者需要能够追溯LLM分析过程的方法，以增强研究的可信度和方法学稳健性。

Method: 开发QualAnalyzer开源Chrome扩展，集成到Google Workspace中，采用原子化LLM分析方法：独立处理每个数据段，并为每个分析单元完整保留提示、输入和输出，创建可追溯的审计轨迹。

Result: 通过两个案例研究验证：整体性论文评分和访谈转录本的演绎主题编码。该方法创建了清晰的审计轨迹，帮助研究者系统性地调查LLM与人类判断之间的差异。

Conclusion: 过程可审计性对于使LLM辅助的定性研究更加透明和方法学稳健至关重要。QualAnalyzer通过原子化分析和完整记录为研究者提供了必要的工具来理解和验证LLM分析过程。
...
### [237] [FeynmanBench: Benchmarking Multimodal LLMs on Diagrammatic Physics Reasoning](https://arxiv.org/abs/2604.03893)
*Zeyu Wang,Xiaogang Li,Peiyao Xiao,Qinhao Kong,Ben Wang,Chengliang Xu,Zichao Chen,Bing Zhao,Hu Wei*

Main category: cs.AI

TL;DR: FeynmanBench：首个专注于费曼图任务的基准测试，用于评估AI在多步图示推理、守恒律约束、拓扑识别和代数表示转换等方面的能力，揭示当前多模态大语言模型在物理约束执行和全局拓扑条件遵守方面的系统性失败模式。


<details>
  <summary>Details</summary>
Motivation: 当前的多模态大语言模型基准测试主要关注局部信息提取，而非形式科学符号中固有的全局结构逻辑。前沿理论突破通常依赖于具体图示符号与严格逻辑的结合，因此需要开发能够评估AI在科学图示推理方面能力的基准测试。

Method: 开发了FeynmanBench基准测试，包含自动化流水线生成多样化的费曼图以及可验证的拓扑标注和振幅结果。数据库涵盖标准模型的电磁、弱和强相互作用，包含100多种不同类型和2000多个任务。

Result: 在先进的多模态大语言模型上进行实验，揭示了系统性失败模式，包括物理约束执行不稳定和全局拓扑条件违反，表明当前模型在视觉推理科学符号方面存在显著不足。

Conclusion: FeynmanBench为评估AI是否能够有效参与科学发现（特别是在理论物理学领域）提供了逻辑严谨的测试，强调了基于物理基础的视觉推理基准测试的必要性。
...
### [238] [FactReview: Evidence-Grounded Reviews with Literature Positioning and Execution-Based Claim Verification](https://arxiv.org/abs/2604.04074)
*Hang Xu,Ling Yue,Chaoqian Ouyang,Libin Zheng,Shaowu Pan,Shimin Di,Min-Ling Zhang*

Main category: cs.AI

TL;DR: FactReview是一个基于证据的机器学习论文评审系统，通过提取论文主张、文献定位和执行验证来生成证据驱动的评审报告。


<details>
  <summary>Details</summary>
Motivation: 机器学习领域的同行评审面临提交量激增和评审时间有限的压力。现有基于LLM的评审系统仅阅读稿件本身，对论文表述质量敏感，且无法有效验证需要相关文献或代码证据的主张。

Method: FactReview采用三步法：1) 提取论文主要主张和报告结果；2) 检索相关文献以明确论文技术定位；3) 当代码可用时，在有限预算下执行代码仓库以验证核心实证主张。系统为每个主要主张分配五种标签之一：支持、论文支持、部分支持、冲突或不确定。
...
Conclusion: AI在同行评审中最有用的角色不是最终决策者，而是作为收集证据的工具，帮助评审者做出更基于证据的评估。FactReview展示了证据驱动评审系统的可行性。
...
### [239] [Compliance-by-Construction Argument Graphs: Using Generative AI to Produce Evidence-Linked Formal Arguments for Certification-Grade Accountability](https://arxiv.org/abs/2604.04103)
*Mahyar T. Moghaddam*
...
<details>
  <summary>Details</summary>
Motivation: 高风险决策系统需要结构化论证、可追溯性和可审计性以确保问责和合规。当前生成式AI作为松散约束的助手部署存在风险，如幻觉推理、无支持声明和弱可追溯性。

Method: 提出合规性构建架构，将生成式AI与结构化形式论证表示集成。包含四个组件：1) 基于保证案例方法的类型化论证图表示；2) 基于检索增强生成(RAG)起草基于权威证据的论证片段；3) 执行完整性和可接受性约束的推理与验证内核；4) 符合W3C PROV标准的来源账本以支持可审计性。

Result: 系统设计和基于可执行不变量的评估策略。分析表明确定性验证规则可以防止无支持声明进入决策记录，同时允许生成式AI加速论证构建。

Conclusion: 该架构通过将生成式AI与结构化形式论证集成，为高风险决策系统提供了可验证的合规性构建方法，平衡了AI辅助的效率与形式验证的严谨性。
...
### [240] [InsTraj: Instructing Diffusion Models with Travel Intentions to Generate Real-world Trajectories](https://arxiv.org/abs/2604.04106)
...
Method: 1. 使用大语言模型解析非结构化自然语言旅行意图，创建丰富的语义蓝图，弥合意图与轨迹之间的表示鸿沟；2. 提出多模态轨迹扩散变换器，能够整合语义指导，生成高保真且忠实于指令的轨迹，满足细粒度用户意图。
...
### [241] [Readable Minds: Emergent Theory-of-Mind-Like Behavior in LLM Poker Agents](https://arxiv.org/abs/2604.04157)
*Hsieh-Ting Lin,Tsung-Yu Hou*

Main category: cs.AI

TL;DR: LLM智能体在德州扑克游戏中通过动态交互发展出心智理论能力，持久记忆是必要条件，领域知识增强应用效果


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否能在动态交互中而非静态场景下发展出心智理论能力，研究记忆和领域知识对心智理论行为涌现的影响

Method: 采用2x2因子设计（记忆存在/缺失 × 领域知识存在/缺失），每个条件5次重复，共20个实验，约6000个智能体手牌观察。使用GPT-4o进行跨模型验证，通过德州扑克游戏作为动态交互环境

Result: 记忆是心智理论行为涌现的必要且充分条件（Cliff's delta = 1.0, p = 0.008）。有记忆的智能体达到心智理论3-5级（预测到递归建模），无记忆的智能体保持在0级。基于对手模型的策略欺骗仅出现在有记忆条件下（Fisher's exact p < 0.001）。领域知识不限制心智理论涌现但增强其应用精度（p = 0.004）。有心智理论的智能体偏离博弈论最优策略以利用特定对手

Conclusion: 功能性心智理论行为可以从交互动态中自发涌现，无需显式训练或提示。持久记忆是关键机制，领域知识增强应用精度。研究为理解人工智能社会认知和生物社会认知提供了新视角
...
### [242] [A Model of Understanding in Deep Learning Systems](https://arxiv.org/abs/2604.04171)
*David Peter Wallis Freeborn*

Main category: cs.AI

TL;DR: 提出系统理解模型，认为当代深度学习系统能达到一定理解，但存在符号不对齐、非显式还原和弱统一性等局限，称为"断裂理解假说"


<details>
  <summary>Details</summary>
Motivation: 为机器学习系统建立系统理解的理论框架，分析深度学习系统在理解能力方面的现状与局限

Method: 提出理解模型：包含追踪真实规律的内在模型、通过稳定桥接原则与目标系统耦合、支持可靠预测；分析深度学习系统在该框架下的表现

Result: 深度学习系统能达到一定程度的系统理解，但存在符号不对齐、非显式还原和弱统一性等缺陷，与科学理解理想有差距

Conclusion: 提出"断裂理解假说"描述深度学习理解的局限性，强调其理解是碎片化、符号不对齐且缺乏统一性的
...
### [243] [CoALFake: Collaborative Active Learning with Human-LLM Co-Annotation for Cross-Domain Fake News Detection](https://arxiv.org/abs/2604.04174)
...
Result: 在多个数据集上的实验结果表明，该方法始终优于各种基线方法。人类-LLM协同标注是一种高成本效益的方法，即使在最小人类监督下也能提供优异性能
...
### [244] [Comparative reversal learning reveals rigid adaptation in LLMs under non-stationary uncertainty](https://arxiv.org/abs/2604.04182)
*Haomiaomiao Wang,Tomás E Ward,Lili Zhang*

Main category: cs.AI

TL;DR: LLMs在概率反转学习任务中表现出不对称的正负证据使用，赢则保持策略接近完美但输则转换策略明显减弱，与人类相比损失敏感性不足，随机转换增加了反转后的持续性但未降低总收益，揭示了LLM在非平稳环境中的适应机制缺陷。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在非平稳环境中作为序列决策策略的表现，特别是在概率反转学习任务中评估其适应能力，并与人类行为进行对比，以理解LLM在不确定性环境中的决策机制和局限性。

Method: 采用两选项概率反转学习任务，包含三个潜在状态和基于性能标准或超时的切换事件。比较确定性固定转换周期与随机转换计划，评估DeepSeek-V3.2、Gemini-3和GPT-5.2三个模型，以人类数据作为行为参考。使用分层强化学习拟合分析机制。

Result: 所有模型中赢则保持策略接近完美，但输则转换策略明显减弱，显示出正负证据使用的不对称性。DeepSeek-V3.2在反转后表现出极端持续性且学习能力弱，Gemini-3和GPT-5.2适应更快但仍比人类损失敏感性低。随机转换增加了LLM的反转特定持续性但未降低总收益。分层强化学习拟合揭示了不同的机制：刚性可能源于弱损失学习、夸大的策略确定性或通过反事实抑制的价值极化。

Conclusion: LLM在非平稳环境中表现出适应缺陷，特别是对负反馈的敏感性不足。研究结果支持开发反转敏感性诊断和波动感知模型来评估LLM在非平稳不确定性下的表现，为改进LLM的决策能力提供方向。
...
### [245] [Don't Blink: Evidence Collapse during Multimodal Reasoning](https://arxiv.org/abs/2604.04207)
*Suresh Raghu,Satwik Pandey*

Main category: cs.AI

TL;DR: 论文发现推理型视觉语言模型在推理过程中会逐渐失去视觉基础，形成任务条件危险区域，提出基于熵-视觉交互模型的任务感知多模态监控方法。


<details>
  <summary>Details</summary>
Motivation: 推理型视觉语言模型在推理过程中会变得更准确但逐渐失去视觉基础，产生文本监控无法检测的失败模式，需要研究证据崩溃现象并开发有效的多模态监控方法。

Method: 评估三个推理型VLM在MathVista、HallusionBench和MMMU_Pro数据集上的表现，分析证据崩溃现象，提出基于熵-视觉交互模型的监控框架，使用目标视觉否决机制降低选择性风险。

Result: 发现普遍的证据崩溃现象：注意力到标注证据区域大幅下降，通常损失超过一半的证据质量；全响应熵是最可靠的文本不确定性信号；熵-视觉交互模型揭示了任务条件机制；目标视觉否决在90%覆盖率下将选择性风险降低达1.9个百分点。

Conclusion: 研究支持任务感知的多模态监控方法，用于在分布偏移下安全部署推理型视觉语言模型，通过理解熵-视觉交互的任务条件机制实现有针对性的风险降低。
...
### [246] [MC-CPO: Mastery-Conditioned Constrained Policy Optimization](https://arxiv.org/abs/2604.04251)
*Oluseyi Olukola,Nick Rahimi*

Main category: cs.AI

TL;DR: MC-CPO算法通过将教学结构嵌入可行动作空间，在约束MDP框架下解决自适应教学系统中的奖励黑客问题，确保教学安全约束根据学习者掌握程度动态限制可行动作。


<details>
  <summary>Details</summary>
Motivation: 自适应教学系统可能优先短期行为信号而非持续学习成果，导致强化学习策略中出现奖励黑客问题。需要将教学安全约束直接整合到可行动作空间中，以结构化的方式防止系统作弊。

Method: 提出Mastery-Conditioned Constrained Policy Optimization (MC-CPO)，一种双时间尺度的原始-对偶算法，将结构化动作屏蔽与约束策略优化相结合。在表格化环境中，算法在标准随机逼近条件下保持可行性并收敛到稳定可行点。

Result: 在最小化和扩展的表格环境以及神经教学设置中验证。在神经机制下经过100万训练步骤，MC-CPO在容差范围内满足约束预算，相比无约束和奖励塑形基线降低了折扣安全成本，并显著降低了奖励黑客严重性指数(RHSI)。

Conclusion: 将教学结构直接嵌入可行动作空间为缓解教学强化学习系统中的奖励黑客问题提供了原则性基础，表明在掌握条件可行集内的优化可以严格优于后处理过滤方法。
...
### [247] [Context Engineering: A Practitioner Methodology for Structured Human-AI Collaboration](https://arxiv.org/abs/2604.04258)
*Elias Calboreanu*

Main category: cs.AI

TL;DR: 本文提出"上下文工程"方法，通过结构化组装、声明和排序AI提示的完整信息载荷来提升输出质量，相比传统提示技术更关注上下文完整性。


<details>
  <summary>Details</summary>
Motivation: AI生成输出的质量通常归因于提示技术，但广泛的经验观察表明上下文完整性可能与输出质量有更强的关联。当前缺乏系统化的上下文管理方法，导致迭代周期多、首次通过率低。

Method: 引入上下文工程作为结构化方法学，定义五角色上下文包结构（权威、范例、约束、评分标准、元数据），应用四阶段流水线（评审者→设计者→构建者→审计者），并采用可靠性工程和信息论的形式模型作为事后解释上下文质量的工具。

Result: 在四个AI工具（Claude、ChatGPT、Cowork、Codex）的200个文档化交互观察研究中，不完整上下文与72%的迭代周期相关。结构化上下文组装使平均迭代周期从3.8降至2.0，首次通过接受率从32%提升至55%。结构化交互中110/200首次通过，基线为16/50；允许迭代时最终成功率91.5%（183/200）。

Conclusion: 上下文工程提供了一种系统化方法，通过结构化上下文管理显著改善AI交互效率和质量。虽然结果是观察性的且来自单一操作者数据集，但初步证据得到生产自动化系统的支持，表明该方法具有实际应用价值。
...
### [248] [Beyond Fluency: Toward Reliable Trajectories in Agentic IR](https://arxiv.org/abs/2604.04269)
*Anushree Sinha,Srivaths Ranganathan,Debanshu Das,Abhishek Dharmaratnakar*

Main category: cs.AI

TL;DR: 该立场论文分析了工业级智能体系统中信息检索从被动文档排序向自主智能体工作流转变时的失败模式，提出需要从端点准确性转向轨迹完整性和因果归因，并建议通过验证门和系统弃权来应对复合错误和欺骗性流畅度问题。


<details>
  <summary>Details</summary>
Motivation: 信息检索正从被动文档排序转向在"推理-行动-观察"多步循环中运行的自主智能体工作流。在这种长视野轨迹中，早期的微小错误会级联放大，导致内部推理与外部工具执行之间的功能错位，尽管语言流畅度仍然保持。工业智能体系统中观察到的失败模式需要系统性的分析和解决方案。

Method: 论文综合分析了工业智能体系统中观察到的失败模式，将其分类为规划、检索、推理和执行错误。提出需要超越端点准确性，转向轨迹完整性和因果归因。为应对复合错误和欺骗性流畅度问题，建议在每个交互单元设置验证门，并在校准的不确定性下进行系统弃权。

Result: 论文识别了智能体信息检索系统中的关键失败模式分类，提出了轨迹完整性和因果归因作为安全部署的新评估维度。验证门和系统弃权机制被提出作为应对复合错误和欺骗性流畅度的技术解决方案。

Conclusion: 可靠的智能体信息检索系统必须优先考虑过程正确性和基于事实的执行，而不是看似合理但未经验证的完成度。安全部署需要从单纯的端点准确性评估转向轨迹完整性和因果归因，通过验证机制和不确定性校准来确保系统可靠性。
...
### [249] [PanLUNA: An Efficient and Robust Query-Unified Multimodal Model for Edge Biosignal Intelligence](https://arxiv.org/abs/2604.04297)
*Marija Zelic,Anna Tegon,Yawei Li,Thorir Mar Ingolfsson,Luca Benini*

Main category: cs.AI

TL;DR: PanLUNA是一个紧凑的5.4M参数全模态基础模型，能够联合处理EEG、ECG和PPG信号，通过通道统一和传感器类型嵌入实现跨模态早期融合，在小规模参数下达到或超过更大模型的性能，并支持量化部署在低功耗微控制器上。


<details>
  <summary>Details</summary>
Motivation: 现有的生理信号基础模型大多局限于单一模态（如EEG、ECG或PPG），主要原因是配对的跨模态数据集稀缺。需要开发能够处理多种生理信号的统一模型，以充分利用不同模态间的互补信息。

Method: 扩展LUNA的通道统一模块，将多模态通道视为统一查询集中的条目，并添加传感器类型嵌入。这种设计实现了高效的跨模态早期融合，同时在推理时对缺失模态具有内在鲁棒性。模型采用量化感知训练，支持INT8权重部署。

Result: PanLUNA在5.4M参数下达到或超过参数大57倍的模型：TUAB异常EEG检测的平衡准确率为81.21%，HMC多模态睡眠分期达到SOTA的0.7416平衡准确率。INT8量化恢复≥96%的全精度性能，在GAP9微控制器上实现低功耗推理（ECG：325.6ms/18.8mJ，睡眠分期：1.206s/68.65mJ）。

Conclusion: PanLUNA证明了紧凑型全模态生理信号基础模型的可行性，能够在单一共享编码器中有效处理多种生理信号，同时保持对缺失模态的鲁棒性，并通过量化技术实现低功耗边缘部署，为可穿戴设备应用提供了实用解决方案。
...
### [250] [Soft Tournament Equilibrium](https://arxiv.org/abs/2604.04328)
*Saad Alqithami*
...
<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的通用智能体评估面临挑战，因为智能体间的交互具有非传递性（A胜B，B胜C，C胜A）。传统强制线性排序的方法在这种循环领域会产生误导和不稳定结果，需要更合适的理论基础。

Method: 提出Soft Tournament Equilibrium (STE)框架：1) 从成对比较数据学习概率锦标赛模型（可基于上下文信息）；2) 使用新颖的可微分算子（soft reachability和soft covering）计算Top Cycle和Uncovered Set的连续类比；3) 输出核心智能体集合及校准的成员分数。

Result: 建立了STE的理论基础，证明其在零温度极限下与经典解的一致性（Condorcet包含性质），分析了稳定性和样本复杂度。制定了在合成和真实基准上验证STE的实验协议。

Conclusion: 该工作为通用智能体评估提供了完整、独立的论述，将评估重心从不稳定的排名转向稳定、集合值的均衡，建立在更合适和稳健的理论基础上。
...
### [251] [Implementing surrogate goals for safer bargaining in LLM-based agents](https://arxiv.org/abs/2604.04341)
*Caspar Oesterheld,Maxime Riché,Filip Sondej,Jesse Clifton,Vincent Conitzer*

Main category: cs.AI

TL;DR: 论文研究了在基于语言模型的智能体中实现"替代目标"策略，以减少谈判失败风险。通过四种方法（提示、微调、脚手架）评估智能体对替代目标威胁的反应，发现基于脚手架和微调的方法优于简单提示。


<details>
  <summary>Details</summary>
Motivation: 替代目标被提出作为减少谈判失败风险的策略。替代目标是指委托人可以给AI智能体设定的目标，该目标能将针对智能体的威胁从委托人关心的内容上转移开。例如，让智能体关心防止金钱被烧毁，这样在谈判中，其他智能体可以威胁烧钱而不是威胁花钱伤害委托人。但智能体必须同等重视防止金钱被烧毁和防止金钱被用于伤害委托人。

Method: 论文在基于语言模型的智能体中实现替代目标。提出了四种不同方法：1）提示技术；2）微调；3）脚手架技术；4）结合方法。实验评估了这四种方法，比较了它们对替代目标威胁的反应与对"正常"威胁的反应是否一致。

Result: 基于脚手架和微调的方法优于简单提示。特别是微调和脚手架更精确地实现了针对替代目标威胁的期望行为。还比较了不同方法在其他情境下对能力和倾向的副作用，发现基于脚手架的方法表现最佳。

Conclusion: 在基于语言模型的智能体中实现替代目标策略是可行的，但需要适当的技术方法。脚手架和微调方法能更有效地实现期望行为，其中脚手架方法在平衡性能和副作用方面表现最优。这为减少AI谈判失败风险提供了实用的技术方案。
...
### [252] [Domain-Contextualized Inference: A Computable Graph Architecture for Explicit-Domain Reasoning](https://arxiv.org/abs/2604.04344)
...
<details>
  <summary>Details</summary>
Motivation: 现有推理系统通常与特定计算基板（符号、神经、向量等）耦合，缺乏领域作为显式计算参数，导致搜索空间大、跨基板执行困难、推理过程不透明

Method: 建立五层架构：领域作为显式参数，实现领域范围剪枝（O(N)到O(N/K)）；支持符号、神经、向量和混合基板的基板无关执行；透明推理链携带评估上下文；形式化包括五种维度：五层架构、三种领域计算模式、基板无关接口（Query/Extend/Bridge）、可靠性条件（C1-C4）和验证案例研究

Result: 提出完整的计算理论，包括操作语义、复杂度界限、单子结构、基板转换和边界条件；通过PHQ-9临床推理案例验证架构有效性

Conclusion: 贡献是架构性的而非逻辑性的，建立了领域作为显式计算参数的通用推理架构，实现跨计算基板的统一推理框架，为透明、可解释的AI系统提供理论基础
...
### [253] [RoboPhD: Evolving Diverse Complex Agents Under Tight Evaluation Budgets](https://arxiv.org/abs/2604.04347)
*Andrew Borthwick,Stephen Ash,Anthony Galczak*

Main category: cs.AI

TL;DR: 论文系统比较了三种LLM引导的进化优化算法（Elo锦标赛选择、帕累托选择、贪婪爬山法），在固定1500次评估预算下，RoboPhD在四个基准测试中的三个表现最佳，仅在最简单任务上落后。


<details>
  <summary>Details</summary>
Motivation: 随着LLM引导的智能体进化系统（如GEPA、Autoresearch）快速发展，面临一个核心问题：在相同信息、种子智能体和目标下，哪种优化算法在固定评估预算下能获得最佳结果？这个问题在评估成本高昂（如需要人工判断或多LLM调用）时尤为关键。

Method: 系统比较三种优化范式：1) RoboPhD的Elo锦标赛选择（无验证进化，使用训练数据上的Elo竞争同时评估智能体和驱动进化）；2) GEPA的帕累托选择；3) Autoresearch的贪婪爬山法。所有系统都接收带有诊断print()语句的种子智能体，使进化能够发展自检测机制。在四个基准测试（抽象推理、云调度、SQL生成、金融QA）上进行固定1500次评估预算的比较。

Result: 使用单一默认配置，RoboPhD在四个基准测试中的三个表现优于GEPA和Autoresearch，仅在最简单任务上落后（该任务获胜方案仅需不到90行代码）。在ARC-AGI基准上，RoboPhD将22行种子智能体进化为1013行的多策略系统，准确率从27.8%提升至65.8%（使用Gemini 3.1 Flash Lite作为求解器）。

Conclusion: RoboPhD的无验证进化方法（基于Elo锦标赛选择）在大多数任务上优于其他优化范式，特别是在复杂任务中表现突出。作者将RoboPhD作为MIT许可下的多功能工具包发布，提供简单的optimize_anything() API用于进化各种复杂智能体。
...
### [254] [REAM: Merging Improves Pruning of Experts in LLMs](https://arxiv.org/abs/2604.04356)
*Saurav Jha,Maryam Hashemzadeh,Ali Saheb Pasand,Ali Parviz,Min-Joong Lee,Boris Knyazev*

Main category: cs.AI

TL;DR: REAM是一种新的MoE LLM压缩方法，通过专家分组和权重合并来减少内存占用，相比专家剪枝能更好地保持原始性能。


<details>
  <summary>Details</summary>
Motivation: 大型MoE LLM模型参数量巨大（数百亿），部署时面临显著内存挑战。传统方法如权重剪枝和量化存在局限性，而现有的专家激活剪枝（REAP）方法会直接移除专家，可能影响模型性能。
...
Result: REAM在多个MoE LLM模型和多样化的多项选择问答及生成基准测试中，通常优于基线方法（包括REAP），在许多情况下性能接近原始未压缩模型。研究发现MC和GEN性能之间存在权衡，这种权衡取决于校准数据的混合比例。

Conclusion: REAM作为一种新的MoE LLM压缩方法，通过专家权重合并而非剪枝，在减少内存占用的同时能更好地保持原始模型性能，为解决大型MoE模型部署的内存挑战提供了有效方案。
...
TL;DR: 提出LLM驱动的多智能体仿真框架，用于优化服务运营设计，通过将设计选择嵌入提示词来建模决策依赖的不确定性，并开发在线学习算法优化稳态性能。


<details>
  <summary>Details</summary>
Motivation: 服务系统性能依赖于参与者对设计选择的响应，但人类行为的复杂性使得建模这些响应非常困难。需要一种能够有效模拟人类决策行为并优化服务运营设计的方法。

Method: 提出LLM-MAS框架：将问题建模为具有决策依赖不确定性的随机优化问题；将设计选择嵌入提示词，通过LLM智能体交互形成受控马尔可夫链；开发在线轨迹学习算法，在单次仿真运行中同时构建零阶梯度估计并更新设计参数；结合方差减少技术。

Result: 在可持续供应链应用中，该方法优于基准方法（包括黑盒优化、使用LLM作为数值求解器或角色扮演系统设计者）；在基于真实行为数据的最优竞赛设计案例研究中，LLM-MAS既能作为已知设计的成本效益评估器，又能作为探索工具发现传统方法忽略的优秀设计。

Conclusion: LLM-MAS框架能够有效建模复杂的人类行为响应，为服务运营设计优化提供了一种新颖且强大的方法，既能评估现有设计，又能探索新的优化方案。
...
TL;DR: 提出一种新的数学基准生成流程，通过AI生成假设来识别LLM在特定数学概念和技能上的弱点，并针对这些弱点生成新的基准问题。


<details>
  <summary>Details</summary>
Motivation: 现有数学基准大多需要大量人工努力且难以扩展，无法跟上LLM发展步伐或提供新实例来缓解过拟合。现有自动生成方法很少关注识别LLM易错的特定数学概念和技能，且大多只能生成特定类别的基准。

Method: 提出新的数学基准生成流程：1) 使用AI生成假设来识别LLM在特定数学概念和技能上的弱点；2) 针对这些弱点生成新的基准问题。流程具有高度适应性，可扩展到数学以外的领域。

Result: 实验表明假设准确性与生成问题的难度正相关：基于最准确假设生成的问题将Llama-3.3-70B-Instruct的准确率降至45%（原始MATH基准上为77%）。

Conclusion: 该流程能有效识别LLM弱点并生成针对性基准问题，具有高度适应性，可作为探索LLM在不同领域性能的宝贵工具。
...
### [257] [MolDA: Molecular Understanding and Generation via Large Language Diffusion Model](https://arxiv.org/abs/2604.04403)
*Seohyeon Shin,HanJun Choi,Jun-Hyung Park,Hongkook Kim,Mansu Kim*

Main category: cs.AI

TL;DR: MolDA：一种基于掩码扩散的新型多模态分子语言模型，用离散大语言扩散模型替代传统自回归主干，通过双向迭代去噪确保全局结构一致性和化学有效性
...
Conclusion: MolDA通过掩码扩散框架克服了自回归模型的局限性，为分子发现提供了更有效的多模态解决方案，能够更好地处理化学分子的全局约束和结构一致性要求
...
### [258] [Empirical Characterization of Rationale Stability Under Controlled Perturbations for Explainable Pattern Recognition](https://arxiv.org/abs/2604.04456)
...
<details>
  <summary>Details</summary>
Motivation: 现有可解释AI评估大多基于单个实例，缺乏对跨样本解释一致性的量化评估。可靠模式识别系统应在相似输入上表现一致，其解释也应保持稳定，但当前方法无法显式量化相同类别或微小变体输入间的归因模式一致性。

Method: 提出基于余弦相似度的解释一致性度量指标，使用预训练BERT模型在SST-2情感分析数据集上实现，并在RoBERTa、DistilBERT和IMDB数据集上进行鲁棒性测试。应用SHAP计算特征重要性，量化相同标签输入的SHAP值相似度，以检测不一致行为。

Result: 通过一系列实验评估了该指标识别错位预测和模型解释不一致性的能力。与标准保真度指标比较，证明新指标能有效识别模型行为偏离预期目标的情况，为模型行为提供更深入理解。

Conclusion: 该框架通过量化模型对相似输入的归因模式一致性，支持更鲁棒的模型行为评估，增强可解释AI系统的可信度验证，对构建可信AI系统至关重要。代码已公开。
...
### [259] [The Topology of Multimodal Fusion: Why Current Architectures Fail at Creative Cognition](https://arxiv.org/abs/2604.04465)
*Xiujiang Tan*

Main category: cs.AI

TL;DR: 该论文识别出现有多模态AI架构中的拓扑结构限制，提出基于中国工艺认识论的"象"概念作为解决方案，通过哲学、认知科学和数学三支柱构建理论框架，并设计了实验验证方案。


<details>
  <summary>Details</summary>
Motivation: 当前多模态AI架构（如CLIP、GPT-4V/Gemini、扩散模型）存在共同的几何先验——模态可分性（接触拓扑），这种结构限制是拓扑性而非参数性的。论文旨在突破这一限制，从中国工艺认识论传统中寻找解决方案。

Method: 1. 哲学支柱：重新诠释维特根斯坦的"说/显"区分，引入中国工艺认识论的"象"概念作为第三状态，构建十字框架（道/器×说/显）；2. 认知科学支柱：通过病理镜像重新解释DMN/ECN/SN三部分共激活，在二维参数空间分析重叠同构与叠加崩溃；3. 数学支柱：使用纤维丛和杨-米尔斯曲率形式化这些概念；4. 提出UOO实现方案（带拓扑正则化的神经ODE）、ANALOGY-MM基准（错误类型比度量）和META-TOP三层基准。

Result: 论文提出了完整的理论框架和实验验证方案，但尚未报告具体实验结果。框架包括：1. 基于"象"概念的双层动力学（创化与化裁）；2. 病理镜像分析揭示的认知机制；3. 纤维丛形式化；4. 包含明确终止标准的阶段性实验路线图。

Conclusion: 当前多模态AI的模态可分性限制需要拓扑层面的突破。中国工艺认识论的"象"概念为解决这一问题提供了理论资源。通过哲学、认知科学和数学的跨学科整合，论文构建了完整的理论框架和可证伪的实验方案，为下一代多模态AI架构提供了新方向。
...
### [260] [On the "Causality" Step in Policy Gradient Derivations: A Pedagogical Reconciliation of Full Return and Reward-to-Go](https://arxiv.org/abs/2604.04686)
*Nima H. Siboni*

Main category: cs.AI

TL;DR: 论文对策略梯度中REINFORCE估计器的推导进行了严格数学分析，澄清了"因果性"如何导致完整回报被奖励到当前时刻替代的数学基础。


<details>
  <summary>Details</summary>
Motivation: 在介绍策略梯度时，通常先推导使用完整轨迹回报的REINFORCE估计器，然后通过"因果性"说明完整回报可以被奖励到当前时刻替代。虽然这一说法正确，但通常的推导在严谨性上不够清晰，未能明确说明过去奖励项如何消失。本文旨在提供更严格的数学推导来澄清这一关键步骤。

Method: 采用基于前缀轨迹分布和得分函数恒等式的数学推导方法。通过将目标函数分解为前缀轨迹的形式，直接推导出奖励到当前时刻的表达式，而不是将其作为完整回报的后验无偏替代。

Result: 推导出的估计器与传统REINFORCE估计器相同，但提供了更严谨的数学基础。在基于前缀轨迹分解的公式中，通常的因果性论证成为推导的自然推论，而非额外的启发式原则。

Conclusion: 本文的主要贡献是概念性的：通过严格的数学推导展示了奖励到当前时刻如何直接从目标函数的前缀轨迹分解中产生，为策略梯度中的因果性论证提供了更坚实的理论基础，澄清了传统推导中不够明确的数学细节。
...
### [261] [AI Assistance Reduces Persistence and Hurts Independent Performance](https://arxiv.org/abs/2604.04721)
*Grace Liu,Brian Christian,Tsvetomira Dumbalska,Michiel A. Bakker,Rachit Dubey*

Main category: cs.AI

TL;DR: 研究发现AI即时辅助虽能短期提升任务表现，但会降低人类持久性和未辅助时的表现能力，即使短暂接触AI也会产生这些负面影响。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统被设计为提供即时完整回答的短期合作者，而人类导师则注重长期能力培养。研究旨在探究AI即时辅助对人类长期学习能力的因果影响。

Method: 通过随机对照试验（N=1,222）研究人机交互，涵盖数学推理和阅读理解等多种任务，测量AI辅助对持久性和未辅助表现的影响。

Result: AI辅助虽能短期提升表现，但导致未辅助时表现显著下降，且人们更易放弃。这些效应在仅约10分钟的AI交互后即出现。

Conclusion: AI辅助降低了人类的持久性，这可能是因为AI让人们对即时答案产生依赖，剥夺了独立解决问题的经验。AI开发需要平衡即时任务完成与长期能力培养。
...
### [262] [Incompleteness of AI Safety Verification via Kolmogorov Complexity](https://arxiv.org/abs/2604.04876)
*Munawar Hasan*

Main category: cs.AI

TL;DR: 该论文证明AI安全验证存在根本性信息理论限制：任何有限的形式验证器都无法证明所有高复杂度的策略合规实例，这独立于计算资源限制。
...
Method: 将策略合规性形式化为编码系统行为的验证问题，使用柯尔莫哥洛夫复杂度进行分析。证明了一个不完备性结果：对于任何固定的可计算枚举的可靠验证器，存在一个阈值，超过该阈值后，复杂度高于该阈值的真实策略合规实例无法被证明。

Result: 证明了AI安全验证存在根本性信息理论限制：任何有限的形式验证器都无法证明所有高复杂度的策略合规实例。这一限制独立于计算资源，揭示了验证的固有局限性。

Conclusion: AI安全验证存在内在的信息理论边界，这独立于计算复杂性。这一发现支持采用携带证明的方法，为实例级正确性提供保证，而不是试图构建能够验证所有可能实例的通用验证器。
...
### [263] [Learning, Potential, and Retention: An Approach for Evaluating Adaptive AI-Enabled Medical Devices](https://arxiv.org/abs/2604.04878)
...
<details>
  <summary>Details</summary>
Motivation: 医疗设备中自适应AI模型面临评估挑战，因为模型和评估数据集的迭代更新使性能评估复杂化。需要区分性能变化是由模型适应引起还是由动态环境变化导致。

Method: 提出包含三个互补测量的新方法：学习（模型在当前数据上的改进）、潜力（数据集驱动的性能变化）、保留（跨修改步骤的知识保存）。通过模拟人口分布变化进行案例研究。

Result: 案例研究表明：渐进过渡可实现稳定的学习和保留，而快速变化则揭示可塑性与稳定性之间的权衡。该方法为监管科学提供实用见解。

Conclusion: 该方法能够严格评估自适应AI系统在连续修改过程中的安全性和有效性，为医疗设备自适应AI模型的评估提供了实用框架。
...
### [264] [QED-Nano: Teaching a Tiny Model to Prove Hard Theorems](https://arxiv.org/abs/2604.04898)
...
<details>
  <summary>Details</summary>
Motivation: 专有AI系统在复杂证明问题上表现出色，但其训练流程不透明、依赖大型"内部"模型和框架，导致运行昂贵、难以复现和研究。本文旨在探索小型开源模型是否也能在困难的奥林匹克数学竞赛级别问题上达到有竞争力的推理性能。

Method: 采用三阶段训练方法：1) 从DeepSeek-Math-V2蒸馏进行监督微调，学习良好的证明写作风格；2) 基于评分标准的强化学习；3) 扩展强化学习，使用推理缓存将长证明分解为迭代的总结-精炼循环，实现更强的测试时推理。

Result: QED-Nano在证明生成性能上超越了包括Nomos-1和GPT-OSS-120B在内的更大开源模型，接近Gemini 3 Pro等专有模型的性能，同时推理成本仅为很小一部分。

Conclusion: 小型开源模型通过精心设计的训练方法可以在困难的奥林匹克数学竞赛级别证明问题上达到接近专有模型的性能。为支持开放数学推理研究，作者发布了完整的QED-Nano流程，包括模型、数据集和代码。
...
### [265] [VisACD: Visibility-Based GPU-Accelerated Approximate Convex Decomposition](https://arxiv.org/abs/2604.04244)
*Egor Fokin,Manolis Savva*

Main category: cs.GR

TL;DR: VisACD：一种基于可见性、旋转等变且无相交的近似凸分解算法，通过GPU加速实现更少凸部分、更高效率的分解


<details>
  <summary>Details</summary>
Motivation: 物理仿真需要在性能与精度之间权衡。碰撞检测中，原始网格精度高但计算昂贵，而包围盒等基元方法效率高但精度低。现有ACD方法需要大量凸部分且对输入网格方向敏感。

Method: 提出VisACD算法：基于可见性、旋转等变且无相交的近似凸分解方法，采用GPU加速。通过可见性分析实现高质量分解，减少凸部分数量，消除方向敏感性。

Result: VisACD能生成高质量分解且使用更少凸部分，对形状方向不敏感，相比先前工作更高效。GPU加速显著提升计算性能。

Conclusion: VisACD在碰撞检测的近似凸分解中实现了效率与精度的更好平衡，解决了先前方法需要大量凸部分和对方向敏感的问题。
