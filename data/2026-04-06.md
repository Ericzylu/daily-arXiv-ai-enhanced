...
TL;DR: 提出用于长文档视觉理解的合成推理数据管道，通过页面相关性评分、证据提取和排序生成思维轨迹，结合SFT和模型合并实现推理能力内化，在Qwen3 VL和Mistral Small上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 视觉长文档理解在企业、法律和科学应用中至关重要，但现有最佳开放方法尚未充分探索推理能力，而推理能力已在数学和代码任务中展现出显著性能提升潜力。

Method: 开发合成数据管道：1) 评估每页与问题的相关性；2) 提取文本证据并按相关性排序；3) 在<think>标签内生成思维轨迹，使用<cot>控制令牌；4) 对结果轨迹进行监督微调(SFT)；5) 通过低强度模型合并将推理能力内化。在Qwen3 VL 32B和Mistral Small 3.1 24B模型上进行实验。

Result: Qwen3 VL 32B在MMLongBenchDoc上达到58.3分，超越7倍大的Qwen3 VL 235B A22B(57.0)。Mistral模型上，合成推理比从Thinking版本蒸馏的轨迹高出3.8分(MMLBD-C)。内化推理相比显式推理平均输出token减少12.4倍。

Conclusion: 合成推理数据管道能有效提升视觉长文档理解的性能，使较小模型超越更大模型，同时通过推理能力内化显著减少输出长度。该方法为长文档理解任务提供了新的有效途径。
...
TL;DR: TrackerSplat通过集成现有点跟踪方法来增强3D高斯泼溅在动态场景重建中的鲁棒性和可扩展性，有效处理大帧间位移并减少伪影。


<details>
  <summary>Details</summary>
Motivation: 当前基于高斯的动态场景重建方法在处理大帧间位移时存在困难，导致快速物体运动下的伪影和时间不一致性，需要更鲁棒的解决方案。

Method: 利用现有点跟踪模型提取像素轨迹，将每视角像素轨迹三角化到3D高斯上，在训练前指导高斯的重新定位、旋转和缩放，从而处理大位移。

Result: 在具有显著位移的真实世界数据集上验证了鲁棒性，在并行设置下实现更高的吞吐量，同时保持视觉质量优于基线方法。

Conclusion: TrackerSplat通过集成点跟踪有效解决了大位移问题，减少了伪影，提高了动态场景重建的吞吐量和质量，为机器人学和沉浸式媒体应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散和基于流的生成模型在图像恢复中表现出强大潜力，但在未知和变化的噪声条件下，图像去噪仍然具有挑战性。学习的向量场在不同噪声水平下可能变得不一致，导致训练和推理不匹配时的恢复质量下降。

Method: 提出定量流匹配框架：首先从局部像素统计中估计输入噪声水平，然后使用这个定量估计来调整推理轨迹，包括起始点、积分步数和步长调度。通过将定量噪声估计与噪声自适应流推理相结合，使去噪过程更好地与每个输入的实际退化水平对齐。

Result: 在自然、医学和显微镜图像上的大量实验表明，该方法在不同噪声水平和成像条件下具有鲁棒性和强大的泛化能力。减少了轻度退化图像的不必要计算，同时为严重退化图像提供足够的细化。

Conclusion: 通过耦合定量噪声估计和噪声自适应流推理，提出的方法提高了恢复精度和推理效率，解决了训练和推理噪声水平不匹配的问题。
...
<details>
  <summary>Details</summary>
Motivation: 6G通信与智能感知深度融合，需要环境感知的信道预测技术。车联网通信对信道预测的准确性、前瞻性、可靠性、时延和适应性有严格要求。传统经验模型和确定性模型在准确性、泛化性和部署性方面存在局限，而车载和路侧感知设备为环境先验信息提供了新来源。

Method: 提出基于多模态视觉特征融合的环境感知信道预测框架：1) 使用GPS数据和车辆侧全景RGB图像；2) 通过语义分割和深度估计提取语义、深度和位置特征；3) 采用三分支架构处理不同模态特征；4) 通过压缩-激励注意力门控模块进行自适应多模态融合；5) 针对360维角功率谱预测设计专用回归头和复合多约束损失函数。

Result: 在同步城市V2I测量数据集上的实验结果显示：路径损耗的RMSE为3.26 dB，时延扩展、到达方位角扩展、离开方位角扩展的RMSE分别为37.66 ns、5.05度、5.08度，角功率谱的均值/中值余弦相似度为0.9342/0.9571，证明了该方法在准确性、泛化性和实际应用潜力方面的优势。

Conclusion: 该框架通过融合多模态视觉特征实现了高精度的环境感知信道预测，为6G车联网通信中的智能信道预测提供了有效的解决方案，展示了在准确性、泛化性和实际部署方面的强大潜力。
...
TL;DR: 提出VE-MD框架，通过变分编码器-多解码器结构实现隐私感知的群体情绪识别，避免显式个体监控，专注于群体级情感预测，同时利用结构监督提升表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有群体情绪识别方法依赖个体级处理（如人脸裁剪、人员跟踪），导致分析流程以人为中心，在仅需群体级理解的部署场景中引发隐私担忧。需要设计避免显式个体监控的隐私感知方法。

Method: 提出VE-MD（变分编码器-多解码器）框架：学习共享潜在表示，联合优化情绪分类和内部身体/面部结构表示预测。研究两种结构解码策略：基于Transformer的PersonQuery解码器和适应可变群体大小的密集Heatmap解码器。模型仅预测聚合群体级情感，不进行身份识别或个体情绪输出。

Result: 在六个真实数据集（两个GER和四个IER基准）上实验表明：结构监督持续改善表示学习；GER与IER存在明显区别——仅优化潜在空间对GER不足（会减弱交互相关线索），而保留显式结构输出改善集体情感推断；结构表示对IER起到有效去噪瓶颈作用。VE-MD在GAF-3.0上达到90.06%的SOTA性能，在VGAF上通过音频多模态融合达到82.25%。在IER数据集上，VE-MD在SamSemo上达到77.9%（加入文本模态），在MER-MULTI、DFEW和EngageNet上分别达到63.8%、70.7%和69.0%的竞争性性能。

Conclusion: VE-MD框架通过避免显式个体监控实现隐私感知的群体情绪识别，证明保留交互相关结构信息对群体级情感建模特别有益，无需依赖先验个体特征提取。结构监督在GER和IER任务中均有效，但作用机制不同：对GER保留交互线索，对IER起去噪作用。
...
TL;DR: LumiVideo：一个模拟专业调色师认知工作流的智能系统，通过感知、推理、执行和反思四个阶段，将原始log视频自动转换为电影级调色效果，并生成行业标准参数配置。


<details>
  <summary>Details</summary>
Motivation: 现有自动化调色方法作为静态黑盒执行器直接输出编辑后的像素，缺乏可解释性和专业人士所需的迭代控制能力。需要一种能够模拟专业调色师认知工作流程的系统。

Method: 引入LumiVideo系统，包含四个阶段：1)感知阶段分析场景物理光照和语义内容；2)推理阶段结合LLM的电影知识和RAG框架，通过Tree of Thoughts搜索导航非线性色彩参数空间；3)执行阶段编译推导出的参数为ASC-CDL配置和全局一致的3D LUT；4)可选反思循环允许通过自然语言反馈优化结果。

Result: 实验表明LumiVideo在完全自动模式下接近人类专家质量，同时支持精确的迭代控制。系统还引入了首个log编码视频基准LumiGrade用于评估自动调色。

Conclusion: LumiVideo通过模拟专业调色师的认知工作流程，实现了从原始log视频到电影级调色的自动化处理，既保证了时间一致性，又提供了可解释性和迭代控制能力，填补了现有自动化方法的不足。
...
Result: 高程图方法偶尔能取得良好结果，但其性能未能超越基于轮廓线的输入方法
...
TL;DR: PlayGen-MoG：基于混合高斯和相对空间注意力的多智能体轨迹生成框架，用于从静态阵型生成美式足球比赛轨迹，解决了后验坍缩和模式坍缩问题。


<details>
  <summary>Details</summary>
Motivation: 团队运动中的多智能体轨迹生成需要同时捕捉可能的战术多样性和球员间的空间协调性。传统的CVAE和扩散模型存在后验坍缩或收敛到数据集均值的问题，且大多数轨迹预测方法需要多帧历史观测，限制了其在仅基于初始阵型的战术设计中的应用。

Method: 提出PlayGen-MoG框架，采用三个关键设计：1) 混合高斯输出头，所有智能体共享混合权重，通过单一权重集选择耦合所有球员轨迹的比赛场景；2) 相对空间注意力，将成对球员位置和距离编码为学习到的注意力偏置；3) 非自回归预测绝对位移，从初始阵型直接预测，消除累积误差漂移，无需轨迹历史观测。

Result: 在美式足球追踪数据上，PlayGen-MoG实现了1.68码的ADE和3.98码的FDE，同时保持所有8个混合组分的完全利用（熵值2.06/2.08），定性验证了无模式坍缩的多样化生成。

Conclusion: PlayGen-MoG通过混合高斯输出、相对空间注意力和非自回归位移预测，成功解决了多智能体轨迹生成中的后验坍缩问题，实现了仅从静态阵型生成多样化且空间协调的战术轨迹，为战术设计提供了有效工具。
...
TL;DR: SPAR是一种针对ALPR系统的物理对抗性攻击方法，通过合法方式降低车牌识别准确率，成本低于100美元


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注ALPR系统的技术漏洞，而较少关注其合法性和物理世界的实用性。本文旨在探究低资源威胁行为者能否成功对现代开源ALPR系统实施对抗性攻击。

Method: 提出SPAR（Street-legal Physical Adversarial Rim）攻击方法，这是一种针对fast-alpr系统的物理可实现白盒攻击。SPAR在攻击部署时无需访问ALPR基础设施，也不改变或遮挡攻击者的车牌。基于德克萨斯州的法律法规，论证了SPAR的合法性。

Result: 在最优条件下，SPAR能将ALPR准确率降低60%，并实现18%的目标模仿率。攻击成本低于100美元，且完全由商业智能编码助手实现。

Conclusion: SPAR展示了现代ALPR系统在现实物理条件下的实际漏洞，为攻击和防御研究提供了新方向。
...
TL;DR: VERTIGO框架通过视觉偏好优化提升相机轨迹生成质量，使用Unity渲染和视觉语言模型评分，结合DPO后训练，显著改善构图、角色可见性和美学质量。


<details>
  <summary>Details</summary>
Motivation: 现有生成式相机系统缺乏"导演在环"的反馈机制，没有明确的视觉偏好监督，导致构图不佳、角色出屏和美学质量差的问题。

Method: 提出VERTIGO框架：1) 使用Unity实时图形引擎渲染相机轨迹的2D视觉预览；2) 通过电影微调的视觉语言模型，采用循环语义相似度机制对渲染结果评分；3) 利用这些视觉偏好信号进行直接偏好优化(DPO)后训练。

Result: 定量评估和用户研究表明：条件遵循、构图质量和感知真实感均有提升；角色出屏率从38%降至接近0%；用户在所有维度(构图、一致性、提示遵循、美学质量)上都更偏好VERTIGO。

Conclusion: VERTIGO首次实现了相机轨迹生成器的视觉偏好优化，通过结合实时渲染和视觉语言模型评分，显著提升了生成相机轨迹的视觉质量和用户偏好。
...
TL;DR: HIL-CBM提出了一种分层可解释的无标签概念瓶颈模型，通过多语义层次的概念预测来增强模型解释性，更贴近人类认知过程。


<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型(CBMs)在概念和标签空间都只工作在单一语义层次，而人类识别物体时使用不同抽象层次的特征。需要更贴近人类认知过程的分层解释模型。

Method: 提出HIL-CBM：1)引入基于梯度的视觉一致性损失，使抽象层关注相似空间区域；2)训练双分类头，在不同抽象层次的特征概念上操作；3)无需关系概念标注，实现多语义层次的分类和解释。

Result: 在基准数据集上，HIL-CBM在分类准确率上优于最先进的稀疏CBMs。人类评估显示HIL-CBM提供更可解释和准确的解释，同时保持分层和无标签的特征概念方法。

Conclusion: HIL-CBM通过分层框架扩展了CBMs，使概念解释的抽象层次与模型预测对齐，从抽象到具体，更贴近人类认知过程，提高了分类准确性和解释质量。
...
TL;DR: 提出一种分解优先的管道方法，将完整临床指南转换为可执行的临床决策图，通过拓扑感知分块、接口约束的块图生成和溯源保留的全局聚合，显著提升决策图的准确性和完整性。


<details>
  <summary>Details</summary>
Motivation: 临床实践指南是冗长、多模态的文档，其分支推荐难以转换为可执行的临床决策支持（CDS）。现有LLM/VLM提取器大多是局部或文本中心的，未能充分指定章节接口，也无法将跨页面的控制流整合为连贯的决策图。
...
Conclusion: 该方法支持分解优先、可审计的指南到CDS转换，在当前基准测试中表现优异，但证据仍限于单一前列腺指南，需要更广泛的多指南验证。
...
TL;DR: 研究探索了使用EarthSynth扩散模型基于燃烧掩码合成真实的野火后Sentinel-2 RGB图像，无需任务特定重新训练，评估了不同架构、提示工程和颜色匹配方法的效果。


<details>
  <summary>Details</summary>
Motivation: 标记卫星图像的稀缺性是深度学习野火监测系统的根本瓶颈，需要探索生成模型能否合成真实野火后图像来增强数据集。

Method: 使用CalFireSeg-50数据集的燃烧掩码，设计了六种实验配置：比较掩码生成与上下文修复两种架构，三种手工提示与VLM生成提示，以及颜色匹配后处理步骤。

Result: 修复架构在所有指标上优于全图生成，结构化修复提示获得最佳空间对齐（Burn IoU=0.456）和燃烧显著性（Darkness Contrast=20.44），颜色匹配产生最低颜色距离但降低显著性。

Conclusion: 修复方法优于全图生成，VLM辅助提示与手工提示竞争力相当，为将生成数据增强纳入野火检测流程奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管VLMs在多模态任务上表现出色，但在需要细粒度视觉感知的任务中常常失败，即使所需信息已存在于其内部表示中。本研究旨在探究这种差距的根源。

Method: 通过视觉对应任务（语义、形状、人脸对应）测试VLMs性能，使用Logit Lens分析机制，并探索通过教授任意名称或任务特定微调来改进性能的方法。

Result: VLMs在相关实体可语言命名时表现显著优于不可命名实体；Logit Lens分析证实VLMs会为可命名实体显式分配语义标签；教授任意名称可改善性能，但任务特定微调能实现更强的泛化能力。
...
Method: 提出Image Prompt Packaging (IPPg)提示范式，将结构化文本直接嵌入图像中以减少文本token开销。在五个数据集、三个前沿模型(GPT-4.1、GPT-4o、Claude 3.5 Sonnet)和两类任务(VQA和代码生成)上进行基准测试。推导了按token类型分解的成本公式，并进行125种配置的渲染消融实验。

Result: IPPg实现了35.8-91.0%的推理成本降低，token压缩高达96%。准确率表现高度模型和任务依赖：GPT-4.1在CoSQL上同时实现准确率和成本增益，而Claude 3.5在多个VQA基准上成本增加。系统错误分析得出失败模式分类：空间推理、非英语输入和字符敏感操作最脆弱，而模式结构化任务受益最大。渲染消融显示准确率变化10-30个百分点。

Conclusion: 视觉编码选择应成为多模态系统设计的一等变量。IPPg能显著降低推理成本，但效果高度依赖模型和任务特性。需要针对特定应用场景优化视觉提示策略。
...
TL;DR: Delaunay Canopy方法利用Delaunay图作为几何先验，为大型稀疏建筑点云创建自适应搜索空间，解决了传统方法在噪声、稀疏或内角区域无法准确重建线框的问题。
...
Method: 提出Delaunay Canopy方法，使用Delaunay图作为几何先验定义几何自适应搜索空间。核心是Delaunay图评分，不仅能重建底层几何流形，还能生成区域曲率特征来稳健指导重建。在此基础上，角点和线框选择模块利用Delaunay诱导的先验专注于高概率元素，从而塑造搜索空间。

Result: 在Building3D Tallinn城市和入门级数据集上的大量实验展示了最先进的线框重建性能，能够在多样复杂的建筑几何结构中实现准确预测。

Conclusion: Delaunay Canopy通过几何自适应搜索空间解决了传统线框重建方法在挑战性区域的局限性，为大型稀疏建筑点云提供了准确可靠的线框重建解决方案。
...
TL;DR: 提出一个可解释的视觉语言模型框架，用于腰椎管狭窄症的诊断和分割，通过空间补丁交叉注意力模块和自适应PID-Tversky损失函数解决类别不平衡和空间精度问题，实现高精度诊断、分割和自动放射报告生成。


<details>
  <summary>Details</summary>
Motivation: 腰椎管狭窄症诊断依赖人工解读多视图MRI，存在观察者间变异性和诊断延迟。现有视觉语言模型无法同时处理临床分割数据集中极端的类别不平衡问题，同时保持空间精度，主要因为全局池化机制丢弃了关键的解剖层次信息。

Method: 提出端到端的可解释视觉语言模型框架：1) 空间补丁交叉注意力模块，实现文本引导的脊柱异常精确定位；2) 自适应PID-Tversky损失函数，集成控制理论原理动态调整训练惩罚，专门处理难以分割的少数类别实例；3) 结合基础视觉语言模型和自动放射报告生成模块。

Result: 诊断分类准确率达到90.69%，分割的宏平均Dice分数为0.9512，CIDEr分数为92.80%。框架通过将复杂分割预测转换为放射科医生风格的临床报告，展示了可解释性。

Conclusion: 该框架为临床医学影像中的透明、可解释AI建立了新基准，在保持必要人工监督的同时增强了诊断能力，解决了现有模型在类别不平衡和空间精度方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 增强现实和虚拟现实应用中，注视跟踪对新产品快速部署高精度设备端注视估计面临挑战，因为硬件配置（相机位置、姿态、照明）在不同设备代际间经常变化。视觉基础模型在自然图像基准上表现优异，但在专用近眼红外图像上仍难以达到高精度。

Method: DistillGaze采用两阶段框架：1）使用带标签的合成数据和未标注的真实数据，通过自监督学习将视觉基础模型适配为领域专用教师模型；2）结合教师指导与自训练，训练轻量级设备端学生模型。合成数据提供可扩展的高质量注视监督，未标注真实数据帮助弥合合成到真实的领域差距。

Result: 在超过2000名参与者的大规模众包数据集上评估，DistillGaze相比仅使用合成数据的基线，将中位注视误差降低了58.62%，同时保持仅256K参数的轻量级模型，适合实时设备端部署。

Conclusion: DistillGaze为训练和部署适应硬件变化的注视跟踪模型提供了高效途径，并为在设备端回归任务中结合合成监督与未标注真实数据提供了解决方案。
...
TL;DR: FASS是一个评估特征归因方法稳定性的新基准，通过预测不变性过滤和三种互补指标，在几何、光度、压缩扰动下评估四种归因方法，发现几何扰动导致最大不稳定性，Grad-CAM表现最稳定。


<details>
  <summary>Details</summary>
Motivation: 现有特征归因稳定性评估存在三个主要问题：1) 主要评估加性噪声，忽略现实扰动；2) 将稳定性压缩为单一标量；3) 未考虑预测不变性，混淆了归因脆弱性与模型敏感性。需要更全面、细粒度的评估框架。

Method: 提出特征归因稳定性套件(FASS)，包含三个核心设计：1) 预测不变性过滤，确保评估时模型预测不变；2) 三种互补稳定性指标：结构相似性、秩相关性和top-k Jaccard重叠度；3) 评估三种扰动类型：几何变换、光度变化和压缩失真。在ImageNet-1K、MS COCO、CIFAR-10数据集上评估四种归因方法(Integrated Gradients、GradientSHAP、Grad-CAM、LIME)和四种架构。

Result: 1) 稳定性估计严重依赖扰动类型和预测不变性过滤；2) 几何扰动比光度变化导致更大的归因不稳定性；3) 未进行预测不变性过滤时，高达99%的评估对涉及预测变化；4) 在受控评估下，Grad-CAM在所有数据集上表现出最高的稳定性。

Conclusion: FASS揭示了特征归因方法稳定性的关键影响因素，强调了预测不变性过滤的重要性，并提供了细粒度的评估框架。几何扰动是评估归因稳定性的重要测试场景，Grad-CAM在稳定性方面表现最佳。
...
TL;DR: 该研究系统评估了医学视觉语言模型在视觉问答任务中的置信度校准问题，发现模型普遍存在过度自信，提出后处理校准优于提示策略，并引入幻觉感知校准方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在临床决策支持中的部署增加，仅关注准确性已不足够，了解何时信任模型预测同样关键。然而，在医学领域对模型过度自信问题的系统性研究仍然缺乏，需要填补这一空白。

Method: 研究采用全面的实证分析方法，涵盖三个模型家族（Qwen3-VL、InternVL3、LLaVA-NeXT）、三种模型规模（2B-38B）以及多种置信度估计提示策略，在三个医学视觉问答基准上进行评估。比较了后处理校准方法（如Platt缩放）与提示策略，并提出了幻觉感知校准方法，该方法利用视觉基础幻觉检测信号作为补充输入来优化置信度估计。

Result: 研究发现：1）过度自信在不同模型家族中普遍存在，且无法通过模型缩放或提示策略（如思维链和语言化置信度变体）解决；2）简单的后处理校准方法能有效降低校准误差，且始终优于基于提示的策略；3）由于后处理方法的严格单调性限制，它们无法改善预测的判别质量，AUROC保持不变；4）幻觉感知校准方法通过利用幻觉信号，同时改善了校准性能和AUROC，在开放式问题上提升最为显著。

Conclusion: 研究建议将后处理校准作为医学视觉语言模型部署的标准实践，优于原始置信度估计。同时强调幻觉信号的实际效用，能够实现医学视觉问答中更可靠的视觉语言模型应用。
...
TL;DR: UniScene3D：基于Transformer的3D场景编码器，通过多视角彩色点图学习统一场景表示，联合建模图像外观和几何信息，在多种3D场景理解任务上实现SOTA性能
...
Result: 在视角接地、场景检索、场景类型分类和3D视觉问答等任务的低样本和任务特定微调评估中，均实现了最先进的性能

Conclusion: UniScene3D方法在统一3D场景理解方面表现出色，验证了通过多视角彩色点图联合建模外观和几何的有效性
...
<details>
  <summary>Details</summary>
Motivation: 现有SVD方法在实际应用中难以显著降低视觉语言模型的推理延迟，需要更有效的计算模式和细粒度优化策略

Method: 引入新的计算模式，在更细粒度应用SVD；根据权重元素重要性自适应分配权重；结合权重和激活值的量化

Result: WSVD方法在保持精度的同时实现超过1.8倍的解码加速，优于其他方法

Conclusion: 加权SVD通过细粒度优化和自适应权重分配，有效解决了VLM推理延迟问题，实现了精度与效率的平衡
...
<details>
  <summary>Details</summary>
Motivation: 现有图像-3D表示学习方法主要关注单个对象图像与其3D模型的特征对齐，但在现实场景中，对象通常从多个视角被观察和捕获。多视角观察自然提供互补的几何和外观线索，但现有多模态大模型很少探索如何有效融合这种多视角视觉信息以改善跨模态检索。

Method: 1. 提出基于跨注意力的多视角视觉聚合器，自适应地整合对象多视角图像的特征；2. 多视角视觉编码器融合视角间的互补关系，并选择性地强调跨多个视角的信息视觉线索；3. 提出法线感知的3D模型编码器，通过联合编码点法线和3D位置来增强3D几何特征，特别适用于无纹理或颜色退化的3D模型。

Result: 广泛的图像-3D检索实验表明，FusionBERT在单视角和多视角设置下都实现了比现有最先进多模态大模型显著更高的检索精度，为多视角多模态检索建立了强大的基线。

Conclusion: FusionBERT通过创新的多视角视觉融合和法线感知3D编码，有效解决了现有方法在现实多视角场景中的局限性，为图像-3D多模态检索提供了更鲁棒的解决方案。
...
Method: 1) 基于Moondream 3的指称图像分割扩展；2) 自回归解码向量路径并迭代细化掩码；3) 引入强化学习阶段直接优化掩码质量；4) 从强化学习阶段生成粗到真值的目标用于细化器；5) 发布RefCOCO-M清洗数据集。

Result: 在RefCOCO (val)上达到80.2% cIoU，在LVIS (val)上达到62.6% mIoU，并发布了边界精确的RefCOCO-M清洗验证集。

Conclusion: Moondream Segmentation通过强化学习优化和迭代细化策略，有效提升了指称图像分割的性能，同时通过清洗数据集减少了评估噪声。
...
<details>
  <summary>Details</summary>
Motivation: 光学传感器（相机、激光雷达）在烟雾、雾霾、非理想光照等恶劣条件下容易失效，而专用雷达系统依赖定制硬件和授权频谱，可扩展性和成本效益有限

Method: 提出Rascene框架，利用无处不在的毫米波OFDM通信信号进行3D场景成像；通过多帧、空间自适应融合和置信度加权前向投影，从任意姿态恢复几何一致性

Result: 实验结果表明，该方法能够高精度重建3D场景，为实现低成本、可扩展且鲁棒的3D感知提供了新途径

Conclusion: Rascene框架展示了利用通信信号进行3D场景成像的可行性，为恶劣环境下的鲁棒感知提供了有前景的解决方案
...
TL;DR: 该研究首次探索了联邦学习在基于姿态的儿童自闭症行为识别中的应用，通过骨骼抽象和联邦学习两层隐私保护机制，解决临床数据隐私和稀缺性问题。
...
Method: 提出联邦学习框架，采用两层隐私保护机制：1）使用人体骨骼抽象从原始RGB视频中移除可识别的视觉信息；2）通过联邦学习确保敏感姿态数据保留在诊所内部。该框架利用分布式临床数据学习泛化表示，同时提供站点特定个性化的灵活性。

Result: 在MMASD基准测试上的实验结果表明，该框架实现了高识别准确率，优于传统的联邦基线方法，为多站点临床分析提供了稳健的隐私优先解决方案。

Conclusion: 该研究证明了联邦学习在基于姿态的儿童自闭症行为识别中的有效性，通过隐私保护机制实现了多站点协作，解决了临床数据隐私和稀缺性挑战，为客观临床评估提供了可行方案。
...
TL;DR: 提出Smart Transfer框架，利用视觉基础模型和两种新颖的迁移策略（像素级聚类和距离惩罚三元组），实现跨区域建筑损毁快速制图，支持灾害应急响应。


<details>
  <summary>Details</summary>
Motivation: 气候变化导致自然灾害频发，传统灾害调查方法难以泛化到不同城市形态和新灾害事件，且需要耗时的人工标注。需要自动化解决方案来支持"黄金72小时"救援。

Method: 提出Smart Transfer GeoAI框架：1) 像素级聚类(PC)实现原型级全局特征对齐；2) 距离惩罚三元组(DPT)通过惩罚语义不一致但空间相邻的斑块来整合斑块级空间自相关模式。

Result: 在2023年土耳其-叙利亚地震数据集上的实验表明，在跨区域迁移设置（LODO和SSDC）中表现出色，能够加速建筑损毁制图，支持快速灾害响应。

Conclusion: Smart Transfer提供了一个可扩展的自动化GeoAI解决方案，能够增强气候脆弱地区和社区的灾害韧性，代码和数据已开源。
...
Conclusion: 该工作为铰接式车辆的环视深度估计提供了一个有效的自监督框架，通过几何一致性约束和结构先验显著提升了深度估计的准确性和一致性。
...
Method: 包含两个核心组件：1) 时间可靠性校准器(TRC)学习为历史状态分配每帧可靠性分数，过滤噪声并锚定真实模板；2) 时间引导合成器(TGS)将校准后的历史合成为紧凑的动态时间先验集以提供预测引导

Result: 集成到OSTrack、ODTrack和LoRAT三种跟踪架构中均获得一致显著性能提升，基于扩展LoRATv2的最佳模型在LaSOT上达到77.5%成功率，在GOT-10k上达到80.3% AO

Conclusion: DTPTrack是一个轻量级、可泛化的模块，能有效抑制跟踪器漂移，提升多种跟踪架构的性能，在多个基准上达到最先进水平
...
TL;DR: Efficient3D是一个用于3D多模态大语言模型视觉令牌剪枝的统一框架，通过去偏视觉令牌重要性估计器和自适应令牌重平衡策略，在保持竞争力的准确度同时显著降低推理开销。


<details>
  <summary>Details</summary>
Motivation: 3D多模态大语言模型虽然具备细粒度空间理解能力，但其庞大的模型规模和高维输入特征带来了显著的推理开销，限制了在资源受限平台上的实际部署。需要一种能够加速3D MLLMs同时保持竞争力的准确度的解决方案。

Method: 提出了Efficient3D统一框架，包含两个核心组件：1) 去偏视觉令牌重要性估计器(DVTIE)，在注意力聚合时考虑浅层初始层的影响，产生更可靠的视觉令牌重要性预测；2) 自适应令牌重平衡(ATR)策略，根据场景复杂度动态调整剪枝强度，保持语义完整性和跨层注意力平衡。两者结合实现上下文感知的令牌缩减。

Result: 在五个代表性3D视觉和语言基准测试(ScanRefer、Multi3DRefer、Scan2Cap、ScanQA、SQA3D)上的综合实验表明，Efficient3D在保持竞争力的准确度同时显著降低计算开销。特别是在Scan2Cap数据集上实现了+2.57%的CIDEr改进，优于未剪枝的基线模型。

Conclusion: Efficient3D为3D多模态大语言模型的高效推理提供了一个可扩展且有效的解决方案，通过智能令牌剪枝在保持语义完整性的同时显著降低计算需求，促进了3D MLLMs在资源受限平台上的实际部署。
...
TL;DR: 在文档布局分析中引入轻量级结构精炼阶段，通过集合级推理稳定解析器接口，减少因布局假设不稳定导致的解析错误


<details>
  <summary>Details</summary>
Motivation: 在密集文档页面中，重叠区域和模糊边界会导致不稳定的布局假设，使得保留的实例集合与解析器输入顺序不一致，从而引发严重的下游解析错误。需要解决解析器接口不稳定的问题。

Method: 在DETR风格检测器和解析器之间引入轻量级结构精炼阶段。将原始检测器输出视为紧凑假设池，对查询特征、语义线索、边界框几何和视觉证据进行集合级推理。从共享的精炼结构状态中，联合确定实例保留、精炼边界框定位，并在交接前预测解析器输入顺序。引入保留导向监督和难度感知排序目标，使保留实例集合及其顺序与最终解析器输入更好对齐。

Result: 在公开基准测试中，该方法持续提升页面级布局质量。集成到标准端到端解析管道后，稳定的解析器接口显著减少序列不匹配，在OmniDocBench上实现0.024的阅读顺序编辑距离。

Conclusion: 通过在检测器和解析器之间插入结构精炼阶段，可以有效稳定解析器接口，解决密集文档中布局假设不稳定导致的解析错误问题，显著提升文档解析的准确性和鲁棒性。
...
TL;DR: XrayClaw：通过合作-竞争多智能体架构实现胸部X光诊断，引入竞争偏好优化来减少逻辑不一致和诊断幻觉，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统单体模型在胸部X光诊断中缺乏细致推理，容易产生逻辑不一致和诊断幻觉。现有多智能体系统虽然模拟协作会诊，但基于单一底层模型时仍易受共识错误影响，需要更可靠的诊断框架。

Method: 提出XrayClaw框架，包含四个专业合作智能体模拟系统化临床工作流程，以及一个竞争智能体作为独立审计员。引入竞争偏好优化学习目标，通过分析性和整体性解释之间的相互验证来惩罚不合理推理。

Result: 在MS-CXR-T、MIMIC-CXR和CheXbench基准测试中，XrayClaw在诊断准确性、临床推理保真度和零样本领域泛化方面达到最先进性能，有效缓解累积幻觉问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM方法依赖直接确定性姿态优化，对初始化敏感且容易发生灾难性遗忘。需要一种能够显式处理不确定性、提升鲁棒性的概率框架。
...
Conclusion: VBGS-SLAM通过概率框架显式维护不确定性，在保持3DGS效率和渲染质量的同时，显著提升了SLAM系统的鲁棒性和抗漂移能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于视觉-语言-动作（VLA）架构的端到端自动驾驶模型通过行为克隆学习驾驶策略，但模仿学习限制了模型只能复制观察到的行为，无法探索多样驾驶策略，在新型或分布外场景中表现脆弱。强化学习（RL）虽然能实现策略探索，但VLA模型通常基于离线数据集训练，缺乏可直接观察的状态转移，需要学习世界模型来预测动作后果。

Method: 提出了一个统一的理解-生成框架，利用世界建模同时实现有意义的探索和提供密集监督。具体包括：1）通过未来RGB和深度图像生成增强轨迹预测，作为密集世界建模目标；2）世界模型不仅作为监督信号，还作为策略探索的内在奖励来源——其图像预测不确定性自然衡量轨迹相对于训练分布的新颖性；3）将这种探索信号整合到安全门控奖励中，通过组相对策略优化（GRPO）优化策略。

Result: 在NAVSIM和nuScenes基准测试中证明了方法的有效性，在NAVSIM上实现了93.7的PDMS分数和88.8的EPDMS分数，达到最先进水平。

Conclusion: 该工作通过世界建模为VLA自动驾驶模型提供了密集监督和内在探索奖励的统一框架，克服了纯模仿学习的局限性，在保持安全的同时实现了有效的策略探索，显著提升了模型在分布外场景中的鲁棒性。
...
TL;DR: MOMO是首个火星遥感多传感器基础模型，通过模型合并整合了HiRISE、CTX和THEMIS三种传感器的表示学习，采用新颖的等验证损失策略选择兼容收敛阶段的检查点进行融合，在火星基准测试的9个下游任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 火星遥感数据来自多种传感器，分辨率差异大（0.25-100米/像素），现有方法通常针对单一传感器或依赖地球数据预训练，缺乏专门针对火星多分辨率数据的基础模型。需要开发能够整合多传感器表示的统一模型。

Method: 1. 独立训练三个传感器（HiRISE、CTX、THEMIS）的模型；2. 提出等验证损失策略，基于验证损失相似性对齐检查点；3. 通过任务算术融合对齐后的模型；4. 在约1200万个火星轨道数据样本上进行预训练；5. 在Mars-Bench的9个下游任务上评估。

Result: MOMO在整体性能上优于ImageNet预训练模型、地球观测基础模型、传感器特定预训练和全监督基线。在分割任务上表现出持续且显著的性能提升。模型合并结合最优检查点选择策略对多分辨率数据基础模型构建有效。

Conclusion: 通过等验证损失策略选择兼容收敛阶段的检查点进行模型合并，是构建多分辨率数据基础模型的有效方法。MOMO作为首个火星多传感器基础模型，在火星遥感任务中展现出优越性能，为行星科学中的多传感器数据融合提供了新思路。
...
TL;DR: THOM是一个无需训练的两阶段框架，从文本生成具有物理合理性的3D手-物体交互网格，通过高斯到网格的映射和物理优化实现高真实感


<details>
  <summary>Details</summary>
Motivation: 从文本生成3D手-物体交互对于灵巧机器人抓取和VR/AR内容生成至关重要，但现有方法面临从文本生成的高斯中提取网格的ill-posed问题，以及在错误网格上进行物理优化的挑战
...
Conclusion: THOM无需模板物体网格即可生成具有物理合理性的3D手-物体交互，解决了从高斯表示提取网格的挑战，并通过物理优化提升了交互的真实性
...
TL;DR: LLaBIT：一个用于脑MRI多任务处理的大语言模型，扩展了LLMs的视觉推理能力，在报告生成、视觉问答、图像分割和图像翻译等临床相关任务中表现出色


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在视觉语言任务中表现出色，但简单的文本到图像生成在医学影像中临床价值有限。医学影像中更重要的任务如病理定位的图像分割或重建缺失序列的图像翻译尚未被整合到单一的多功能语言模型中

Method: LLaBIT通过重用图像编码器的特征图来缓解图像标记化过程中的空间信息损失，并使用LLMs生成文本数据来增强脑MRI中有限的图像-文本配对数据

Result: 在五个脑MRI数据集和四个不同任务（报告生成、视觉问答、图像分割、图像翻译）上的综合评估显示，该方法不仅在所有任务上表现出优越性能，而且在直接比较中超越了专门的特定任务模型
...
TL;DR: 提出一种双表示方法，将离散折线与连续Bézier控制点耦合，通过双向映射机制实现协作优化，减少笔画数量30-50%，优化时间减少30-40%
...
Method: 提出双表示方法，通过双向映射机制耦合离散折线与连续Bézier控制点，支持高斯溅射启发的初始化，实现高度并行的笔画优化

Result: 笔画数量减少30-50%，获得更结构一致的布局，提高重建质量，相比现有可微分矢量化方法优化时间减少30-40%

Conclusion: 双表示方法有效解决了笔画渲染中的局部最优问题，通过离散-连续协作优化实现了更高效、结构更优的笔画布局
...
TL;DR: DeCo-DETR：一种解耦认知的视觉中心框架，通过构建分层语义原型空间和解耦训练策略，在保持零样本检测性能的同时显著提升推理效率


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法存在两大限制：1）多模态设计依赖推理时的文本编码器，计算开销大；2）紧耦合训练目标在闭集检测精度和开放世界泛化能力之间存在权衡。需要一种更实用、高效的解决方案。

Method: 提出DeCo-DETR框架，采用统一的解耦范式：1）构建分层语义原型空间，从预训练LVLMs生成的区域级描述中提取语义，通过CLIP对齐，实现高效可重用的语义表示；2）解耦训练策略，将语义对齐和检测分离为并行优化流，解耦语义推理与定位。

Result: 在标准OVOD基准测试中，DeCo-DETR实现了具有竞争力的零样本检测性能，同时显著提升了推理效率，验证了解耦语义认知与检测的有效性。

Conclusion: DeCo-DETR通过解耦范式解决了OVOD的实用部署问题，为可扩展的开放词汇目标检测系统提供了实用方向，平衡了性能与效率。
...
<details>
  <summary>Details</summary>
Motivation: 从变形的3D服装几何恢复参数化2D缝纫图案是一个具有挑战性的病态问题，现有方法难以解决

Method: 两阶段框架：第一阶段使用几何驱动的自回归模型从3D服装推断BoxMesh；第二阶段使用语义感知自回归模型将BoxMesh解析为参数化缝纫图案

Result: 在GarmentCodeData基准测试中达到最先进性能，并能有效泛化到真实扫描和单视图图像

Conclusion: 通过BoxMesh中间表示将几何反演与结构化图案推断分离，实现了更准确和鲁棒的缝纫图案恢复
...
<details>
  <summary>Details</summary>
Motivation: 小目标检测面临像素有限、边界模糊、标注困难、高质量数据集稀缺以及语义表示弱等挑战，现有标签高效方法在小目标检测中性能下降明显，需要新的解决方案。

Method: 1) 创建TinySet-9M大规模多领域小目标检测数据集填补数据空白；2) 提出点提示小目标检测(P2SOD)新范式，在推理时引入稀疏点提示作为类别级定位的信息桥梁；3) 基于P2SOD和TinySet-9M开发DEAL框架，学习鲁棒的点提示条件表示。

Result: DEAL在TinySet-9M上相比全监督基线在严格定位指标(如AP75)上获得31.4%的相对提升，仅需推理时单次点击即可实现，并能有效泛化到未见类别和数据集。

Conclusion: 该工作通过构建大规模数据集和提出点提示检测范式，有效解决了小目标检测的数据限制和语义表示不足问题，为标签高效的小目标检测提供了新方向。
...
TL;DR: 该论文首次系统研究了视觉模型中对抗性成员操纵现象，揭示了对抗性扰动可以欺骗成员推理攻击，并提出了基于梯度几何特征的检测方法和鲁棒推理框架。


<details>
  <summary>Details</summary>
Motivation: 现有成员推理攻击(MIAs)假设查询输入是诚实的，其对抗鲁棒性尚未被探索。研究发现MIAs存在一个被忽视的对抗性攻击面：对抗性成员操纵，即微小扰动可以将非成员图像可靠地推入"成员"区域。

Method: 1) 在不同架构和数据集上验证对抗性成员伪造的有效性；2) 揭示梯度范数崩溃轨迹这一几何特征，区分伪造成员与真实成员；3) 基于梯度几何信号设计检测策略；4) 开发鲁棒推理框架以减轻对抗操纵。

Result: 实验表明：对抗性成员伪造在多种情况下广泛有效；提出的检测方法能可靠区分伪造成员；鲁棒推理框架显著增强了对抗操纵的抵御能力。

Conclusion: 该工作首次建立了视觉模型中对抗性成员操纵的全面框架，揭示了MIAs的对抗脆弱性，并提供了基于梯度几何的检测和防御方法，为评估模型隐私泄露提供了更鲁棒的视角。
...
TL;DR: 提出EnsemHalDet框架，通过集成多种VLM内部表示（注意力输出和隐藏状态）来检测多模态幻觉，相比单检测器方法显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有基于内部表示的方法通常依赖单一表示或检测器，限制了捕捉多样化幻觉信号的能力，需要更鲁棒的检测框架
...
<details>
  <summary>Details</summary>
Motivation: 多色光照下的颜色环境光照归一化面临严重色偏、高光饱和和材料依赖反射等挑战，现有几何和低层先验在光照引起的色偏占主导时无法有效恢复物体固有颜色。

Method: 提出CANDLE方法：1) DINO全层引导(D.O.G.)：将多层DINOv3特征自适应注入到连续编码器阶段；2) 颜色频率细化设计(BFACG + SFFB)：抑制解码器端的色度崩溃和细节污染。

Result: 在CL3AN数据集上比最强基线方法提升+1.22 dB PSNR；在NTIRE 2026 ALN Color Lighting Challenge中获得第3名，在White Lighting track的保真度方面获得第2名且FID最低。

Conclusion: DINOv3的自监督特征可作为光照鲁棒的语义先验，CANDLE通过DINO全层引导和颜色频率细化设计，在色度和亮度主导的光照条件下都表现出强大的泛化能力。
...
TL;DR: LumaFlux：首个基于物理和感知引导的扩散Transformer，用于SDR到HDR重建，通过引入物理引导适应、感知交叉调制和HDR残差耦合器实现高质量HDR转换


<details>
  <summary>Details</summary>
Motivation: 随着HDR设备快速普及，需要将8位SDR内容转换为10位HDR。现有逆色调映射方法依赖固定色调映射算子，难以泛化到真实世界退化、风格变化和相机流水线，常产生高光剪切、颜色去饱和或不稳定的色调再现问题
...
Conclusion: LumaFlux通过物理和感知引导的扩散Transformer架构，成功解决了SDR到HDR转换中的关键挑战，为高质量HDR内容生成提供了有效解决方案
...
TL;DR: UNICA是一个骨架无关的生成模型，将3D虚拟人的所有控制组件统一到单个神经框架中，通过键盘输入生成下一帧几何，并使用3D高斯泼溅进行高保真渲染。
...
Method: UNICA采用骨架无关的生成模型，基于键盘输入通过动作条件扩散模型生成2D位置图表示的下一帧几何，然后使用点变换器将几何映射到3D高斯泼溅进行渲染。

Result: 该方法能够自然捕捉头发和宽松衣物的动态效果，无需手动设计物理模拟，并支持超长自回归生成，统一了"运动规划、绑定、物理模拟和渲染"的工作流程。

Conclusion: UNICA是首个将3D虚拟人控制的所有组件统一到单一神经框架中的模型，为3D游戏、元宇宙和AR/VR应用提供了更高效的解决方案。
...
TL;DR: 提出一种量化感知的视觉token剪枝框架，通过结合量化误差和异常值强度来保留对量化鲁棒的语义重要token，在低比特量化下优于独立优化方法


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型计算和内存成本高，难以在资源受限环境中部署。后训练量化和视觉token剪枝通常作为独立优化技术，但研究发现两者存在强耦合关系：在量化优化后的MLLMs上直接应用基于语义的token剪枝会丢弃对数值稳定性重要的激活异常值，从而在低比特量化（如W4A4）中恶化量化误差
...
Conclusion: 量化感知的视觉token剪枝框架成功解决了后训练量化和token剪枝之间的耦合问题，通过保留对量化鲁棒的语义重要token，在低比特量化下实现了更好的性能，为资源受限环境中的MLLM部署提供了有效解决方案
...
TL;DR: MMPhysVideo是一个通过多模态建模提升视频生成物理一致性的框架，将语义、几何和时空轨迹等感知线索转换为伪RGB格式，使用双向控制教师架构解耦RGB与感知处理，并通过知识蒸馏到单流学生模型实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型(VDMs)虽然能生成视觉效果出色的内容，但由于仅依赖像素级重建，常常产生物理不一致的结果。需要一种方法来提升视频生成的物理合理性。

Method: 1) 将语义、几何和时空轨迹等感知线索转换为统一的伪RGB格式，使VDMs能直接捕捉复杂物理动态；2) 提出双向控制教师架构，使用并行分支完全解耦RGB和感知处理，采用两个零初始化控制链接逐步学习像素级一致性；3) 通过表示对齐将教师的物理先验知识蒸馏到单流学生模型以提高推理效率；4) 开发MMPhysPipe数据管道，使用视觉语言模型(VLM)基于视觉证据链规则识别物理主体，让专家模型提取多粒度感知信息。

Result: 在不增加推理成本的情况下，MMPhysVideo在各种基准测试中持续提升了物理合理性和视觉质量，相比现有方法达到了最先进的性能。

Conclusion: MMPhysVideo是首个通过联合多模态建模扩展视频生成物理合理性的框架，通过将感知线索转换为伪RGB格式、双向控制教师架构和知识蒸馏，有效解决了视频扩散模型的物理不一致问题，同时保持了推理效率。
...
TL;DR: NavCrafter：从单图像创建可控3D场景的新框架，通过视频扩散模型和几何感知扩展实现新颖视角合成，结合碰撞感知相机轨迹规划和增强3D高斯溅射提升3D重建质量


<details>
  <summary>Details</summary>
Motivation: 当直接获取3D数据成本高昂或不切实际时，从单图像创建灵活的3D场景至关重要。现有方法在实现可控多视角合成和保持时空一致性方面存在挑战。

Method: 1) 利用视频扩散模型捕获丰富3D先验，采用几何感知扩展策略逐步扩大场景覆盖范围；2) 引入多阶段相机控制机制，通过双分支相机注入和注意力调制实现可控多视角合成；3) 提出碰撞感知相机轨迹规划器；4) 开发增强的3D高斯溅射(3DGS)流程，包含深度对齐监督、结构正则化和细化

Result: 大量实验表明，NavCrafter在大视角偏移下实现了最先进的新颖视角合成，并显著提高了3D重建的保真度

Conclusion: NavCrafter通过结合视频扩散模型、几何感知扩展、可控相机机制和增强3D重建流程，为从单图像创建灵活可控的3D场景提供了一种有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 神经辐射场（NeRF）在3D表示中表现出色，但需要大量计算资源限制了其应用。设备端训练可以解决通信限制、隐私问题和快速适应变化场景的需求，但受限于GPU内存、存储和功耗等资源约束

Method: Fact-Hash结合了张量分解和哈希编码技术，将3D坐标投影到多个低维形式（2D或1D），然后应用哈希函数，最后聚合成单个特征。这种方法既能利用丰富的高分辨率特征，又具有少样本鲁棒性

Result: 与最先进方法相比，Fact-Hash在保持质量和渲染速度的同时，内存效率显著提高。相比之前的编码方法，Fact-Hash节省了超过三分之一的内存使用，同时保持PSNR值。设备端实验验证了其在计算效率和能耗方面的优越性
...
TL;DR: 提出自适应局部频率滤波方法，通过空间变化的参数α(x)调制傅里叶编码分量，实现不同空间位置的低通、带通和高通行为平滑过渡，提升傅里叶编码隐式神经表示在非平稳信号建模中的性能。
...
Result: 在2D图像拟合、3D形状表示和稀疏数据重建实验中，该方法相比固定频率基线一致提升重建质量并加速优化。学习到的α(x)提供空间变化频率偏好的直观可视化，有助于解释模型在非平稳信号上的行为。

Conclusion: 自适应局部频率调制是傅里叶编码隐式神经表示的实际增强方法，能有效处理具有空间变化频谱的信号，提供更好的重建性能和更快的收敛速度。
...
TL;DR: 提出新框架，利用视频生成模型的3D先验将单视角头发重建转化为校准多视角重建任务，通过神经方向提取器和两阶段发丝生长算法实现高质量3D发丝重建
...
Method: 1) 利用视频生成模型的3D先验将单视角重建转化为校准多视角重建任务；2) 引入基于稀疏真实图像标注训练的神经方向提取器进行全视角方向估计；3) 设计基于混合隐式场的两阶段发丝生长算法快速合成细节丰富的3D发丝曲线

Result: 在多样化头发肖像的可见和不可见区域均实现了最先进的单视角3D发丝重建性能

Conclusion: 提出的框架通过利用视频生成模型的3D先验、改进的方向估计和高效的发丝生长算法，显著提升了单视角3D头发重建的质量和效率
...
TL;DR: 该论文提出了一种新颖的token级视角变换方法，通过反向token扭曲技术，使多模态大语言模型能够更稳定地理解场景在不同视角下的表现，显著优于传统的像素级扭曲方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉推理方面表现良好，但对视角变化非常脆弱。传统的像素级扭曲方法对深度误差高度敏感，容易引入几何失真。受人类心理意象理论启发，该研究探索是否可以利用ViT-based MLLMs中的图像token作为视角变换的有效基底。

Method: 比较前向和后向token扭曲方法，发现后向token扭曲（在目标视图上定义密集网格，为每个网格点检索对应的源视图token）具有更好的稳定性和语义一致性。该方法在提出的ViewBench基准上进行评估。

Result: 实验表明，token级扭曲使MLLMs能够从邻近视角进行可靠推理，在所有基线方法中表现最佳，包括像素级扭曲方法、空间微调的MLLMs和生成式扭曲方法。

Conclusion: token级扭曲为MLLMs提供了更鲁棒的视角变换能力，验证了图像token作为视角变换基底的可行性，为提升多模态模型的空间理解能力提供了新思路。
...
TL;DR: 提出自反思层次提示框架，通过正向和反向知识转移解决手术视频场景解析中的类增量分割问题，避免灾难性遗忘


<details>
  <summary>Details</summary>
Motivation: 现有方法在手术器械增量分割中忽视了正向知识转移（历史知识帮助学习新类）和反向知识转移（学习新类帮助精炼历史知识）的潜力，导致学习效率不高且存在灾难性遗忘问题

Method: 基于冻结预训练模型构建自反思层次提示框架：1）使用层次提示解析树组织器械感知提示，根节点为器械共享提示分区，中间节点为n部分共享提示分区，叶节点为器械独特提示分区，实现正向知识转移；2）通过有向加权图传播进行自反思精炼，利用树中记录的知识关联提高代表性，实现反向知识转移

Result: 在两个公开基准测试中，相比竞争方法分别获得超过5%和11%的性能提升，适用于CNN和Transformer基础模型

Conclusion: 该框架有效解锁了类增量分割中的正向和反向知识转移潜力，能够熟练学习新器械、改进现有常规器械技能，同时避免旧器械的灾难性遗忘
...
TL;DR: InstructTable：一个指令引导的多阶段训练框架，通过表格指令预训练和TSR微调，结合创新的Table Mix Expand合成方法，在复杂表格结构识别任务中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统视觉中心模型在处理复杂表格布局（如合并单元格、空单元格）时缺乏语义支持，而视觉语言模型又忽视视觉结构信息建模，导致复杂表格结构识别准确率受限。

Method: 提出InstructTable框架：1）精心设计的表格指令预训练，关注细粒度结构模式；2）互补的TSR微调，保持强大的视觉信息建模能力；3）Table Mix Expand（TME）无模板合成方法，生成大规模真实表格数据；4）构建BCDSTab基准数据集。

Result: 在多个公共数据集（FinTabNet、PubTabNet、MUSTARD）和自建的BCDSTab基准上，InstructTable在表格结构识别任务中实现了最先进的性能。消融研究证实了表格特定指令和合成数据的积极影响。

Conclusion: InstructTable通过指令引导的多阶段训练框架，有效结合了视觉结构建模和语义理解，解决了复杂表格结构识别的挑战，为表格解析提供了鲁棒且高精度的解决方案。
...
TL;DR: 提出条件引导编辑重建框架，通过约束反演在结构化虚拟人潜在空间中编辑，限制更新到低维部件特定编辑子空间，防止身份泄漏和时域闪烁。


<details>
  <summary>Details</summary>
Motivation: 现有可动画人体虚拟人编辑方法通常依赖稀疏监督（少数编辑关键帧），但简单地将重建虚拟人拟合到这些编辑会导致身份泄漏和姿态依赖的时域闪烁。这些失败本质上是病态反演问题：可用编辑约束不足以确定负责预期编辑的潜在方向。

Method: 提出条件引导编辑重建框架，将编辑作为结构化虚拟人潜在空间中的约束反演执行。限制更新到低维、部件特定的编辑子空间以防止意外身份变化。通过优化从完整解码-渲染流程局部线性化导出的条件目标来设计反演期间的编辑约束，产生编辑子空间信息矩阵，其频谱预测稳定性并驱动帧重新加权/关键帧激活。该方法在小子空间矩阵上操作，可通过Hessian-向量积高效实现。

Result: 该方法在有限编辑监督下提高了稳定性，防止了身份泄漏和时域闪烁问题，同时保持了编辑的准确性和一致性。

Conclusion: 通过将编辑视为结构化潜在空间中的约束反演问题，并利用编辑子空间信息矩阵指导反演过程，提出的框架有效解决了稀疏监督下虚拟人编辑的身份泄漏和时域闪烁问题，为可动画人体虚拟人的稳定编辑提供了系统解决方案。
...
TL;DR: 该论文将几何教育中的视觉解释建模为Referring Image Segmentation问题，通过自动化数据引擎生成20万+合成几何图表，微调视觉语言模型Florence-2，在几何图表分割上取得显著性能提升（从<1% IoU提升到49% IoU），并提出了几何感知的Buffered IoU评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有Referring Image Segmentation模型在自然图像基准（如RefCOCO）上训练，但在抽象、无纹理的几何图表上表现极差，存在严重的领域偏移问题。几何教育需要能够提供视觉化逐步解释的人工智能教师，但缺乏合适的训练数据。

Method: 1) 开发全自动程序化数据引擎，生成20万+合成几何图表，包含像素级分割掩码和语言多样的指代表达，无需人工标注；2) 对视觉语言模型进行领域特定微调；3) 提出Buffered IoU评估指标，专门处理细薄结构定位问题。

Result: 微调后的Florence-2模型在几何图表分割上达到49% IoU和85% Buffered IoU，相比零样本设置的<1% IoU有显著提升。Buffered IoU比标准IoU更能反映真实分割质量，特别是在处理细薄几何结构时。

Conclusion: 该研究为构建能够提供视觉化逐步几何问题解释的人工通用教师（AGTs）奠定了基础，通过自动化数据生成和领域特定微调解决了几何图表分割的关键挑战，提出的Buffered IoU指标更适合几何教育应用评估。
...
<details>
  <summary>Details</summary>
Motivation: 工业检测系统需要识别未知缺陷，但现有方法大多基于封闭集假设，无法检测新型异常。现有视觉提示方法因类内方差高和类间差异小而容易出现提示嵌入崩溃问题。
...
TL;DR: GP-4DGS将高斯过程集成到4D高斯泼溅中，为动态场景提供概率建模，解决现有方法无法处理运动模糊性和缺乏可靠性评估的问题。


<details>
  <summary>Details</summary>
Motivation: 现有4D高斯泼溅方法专注于确定性重建，无法捕捉运动模糊性，且缺乏评估预测可靠性的机制。需要一种能够量化不确定性、处理稀疏观测区域并进行时间外推的概率建模框架。

Method: 将高斯过程集成到4D高斯泼溅框架中，设计时空核函数捕捉变形场的相关结构，采用带诱导点的变分高斯过程实现可处理的推理，以处理大量高斯基元。

Result: 实验表明GP-4DGS在提高重建质量的同时，提供了可靠的不确定性估计，能够有效识别高运动模糊性区域。

Conclusion: 该工作通过将概率建模与神经图形学相结合，为解决动态场景重建中的不确定性、稀疏观测和外推问题提供了有意义的进展。
...
TL;DR: BEVPredFormer：一种基于注意力机制的纯摄像头BEV实例预测架构，通过门控Transformer层、分离的时空注意力机制和多尺度头任务，在nuScenes数据集上达到或超越SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要准确感知动态场景的演变，传统模块化感知流水线存在累积误差和延迟问题。实例预测模型提供统一解决方案，但在处理动态驾驶环境中密集的时空信息方面面临挑战，需要能够捕捉细粒度运动模式和长程依赖关系且不牺牲实时性能的架构。

Method: 提出BEVPredFormer，一种纯摄像头BEV实例预测架构：1）使用基于注意力的时间处理提升时空理解；2）基于注意力的3D投影机制；3）无循环设计，包含门控Transformer层；4）分离的时空注意力机制；5）多尺度头任务；6）差异引导的特征提取模块增强时间表征。

Result: 在nuScenes数据集上进行广泛消融研究验证了各架构组件的有效性。BEVPredFormer在性能上达到或超越了最先进方法，展示了其在自动驾驶感知中的鲁棒性和高效性潜力。
...
TL;DR: PolyReal是一个基于真实世界科学实践的多模态基准，用于评估MLLMs在聚合物实验全生命周期中的能力，揭示了当前模型在实践性任务上的严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在通用领域表现出色，但在复杂真实世界科学任务上表现不佳。聚合物科学作为跨学科领域，具有多样化的多模态数据，是理想的高风险测试平台。现有聚合物科学相关基准大多忽视真实工作流程，限制了其实用性，未能系统评估MLLMs在实践导向的实验全生命周期中的表现。

Method: 提出PolyReal基准，基于真实世界科学实践，涵盖聚合物实验全生命周期的五个关键能力：基础知识应用、实验室安全分析、实验机理推理、原始数据提取、性能与应用探索。使用该基准评估领先的MLLMs。

Result: 评估显示MLLMs存在能力不平衡现象：在知识密集型推理任务（如实验机理推理）上表现良好，但在实践性任务（如实验室安全分析和原始数据提取）上表现急剧下降。这暴露了抽象科学知识与其实践应用之间的严重差距。

Conclusion: PolyReal有助于解决当前评估空白，为评估AI系统在真实世界科学工作流程中的表现提供了实用基准。研究表明真实世界任务对MLLMs仍然具有挑战性，需要弥合抽象知识与实践应用之间的鸿沟。
...
TL;DR: 提出利用合成数据来源信息指导模型关注目标区域的训练框架，通过梯度分解抑制对非目标区域的依赖


<details>
  <summary>Details</summary>
Motivation: 现有基于合成数据的方法主要通过增加训练样本多样性间接提升鲁棒性，但未明确教导模型哪些输入区域真正有助于判别，导致模型可能学习到合成偏差和伪影造成的虚假相关性

Method: 利用训练数据合成过程中获得的来源信息作为辅助监督信号，基于合成过程中的目标和非目标区域信息分解输入梯度，引入输入梯度指导来抑制非目标区域的梯度，从而直接促进目标区域判别性表征的学习

Result: 实验证明该方法在多个任务和模态上具有有效性和通用性，包括弱监督目标定位、时空动作定位和图像分类

Conclusion: 通过利用合成数据的来源信息指导梯度学习，能够有效抑制模型对非目标区域的依赖，直接促进目标区域判别性表征的获取，提高模型鲁棒性
...
TL;DR: CrossWeaver：一种简单有效的任意模态语义分割多模态融合框架，通过模态交互块和轻量级对齐融合模块实现选择性、可靠性感知的跨模态交互


<details>
  <summary>Details</summary>
Motivation: 现有多模态语义分割方法依赖精心设计的融合策略，通常使用模态特定的适配或松散的耦合交互，限制了灵活性，导致跨模态协调效果不佳。这些方法难以在不同模态组合中平衡高效信息交换与保持各模态独特特性。
...
Conclusion: CrossWeaver通过简单的模态交互块和轻量级对齐融合模块，有效解决了多模态语义分割中的融合挑战，实现了灵活、高效的跨模态协调，在性能和泛化方面均表现出色。
...
TL;DR: CoMP提出了一种协作多模态剪枝框架，通过联合参数剪枝和令牌剪枝来压缩视觉语言模型，在高剪枝率下保持性能


<details>
  <summary>Details</summary>
Motivation: 现有VLM剪枝方法主要关注单一模态（参数或令牌），未能充分利用各模态内的冗余性，导致高剪枝率下性能显著下降

Method: 提出协作多模态剪枝框架：1) 协作重要性度量(CIM)评估参数和令牌的相互干扰；2) 多模态剪枝策略(MPS)分阶段自适应切换最优剪枝模式

Result: 在各种视觉语言任务和模型上的实验表明，该方法在高剪枝率下相比最先进方法有效提升了性能

Conclusion: CoMP通过联合参数和令牌剪枝，充分利用VLM中的冗余性，在高压缩比下实现了更好的性能保持
...
TL;DR: UAVGen：针对无人机目标检测的布局到图像生成框架，通过视觉原型条件扩散模型和焦点区域增强数据管道解决小目标边界伪影问题
...
Conclusion: UAVGen通过视觉原型条件和焦点区域增强机制，有效解决了无人机目标检测中布局到图像生成的伪影问题，为有限标注数据下的检测任务提供了有效的合成数据增强方案。
...
<details>
  <summary>Details</summary>
Motivation: 文本驱动人体运动生成中，运动动态与文本语义的准确对齐仍然是一个基本挑战，现有方法在运动-语言对齐方面存在不足，且存在未被充分关注的注意力下沉现象
...
Conclusion: MLA-Gen通过解决运动-语言对齐和注意力下沉问题，显著提升了文本驱动人体运动生成的性能，为相关领域提供了新的分析工具和解决方案
...
TL;DR: 该论文建立了一个可靠的SAR海冰分类基线，使用Vision Transformer模型在AI4Arctic/ASIP数据集上评估，ViT-Large配合focal loss在少数类多年冰上表现最佳。
...
Result: 在测试配置中，使用focal loss的ViT-Large模型达到69.6%的保持集准确率、68.8%加权F1分数，在少数类多年冰上达到83.9%的精确率。这些结果表明focal loss训练为稀有冰类提供了比加权交叉熵更有用的精确率-召回率权衡。

Conclusion: focal loss训练为SAR海冰分类提供了更优的稀有类别性能，建立了更清晰的基线，为未来与光学、热红外或气象数据的多模态融合奠定了基础。
...
TL;DR: SCOPE是一个无需训练的高效自回归视频扩散模型加速框架，通过三模态调度（缓存、预测、重计算）和选择性计算，在保持生成质量的同时实现高达4.73倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频扩散模型存在计算效率低的问题：1）现有免训练加速方法采用二元的缓存或重计算决策，忽略了中间情况；2）异步自回归调度为共生成帧分配不同噪声水平，但现有方法统一处理整个有效区间。

Method: SCOPE框架包含两个核心创新：1）三模态调度器（缓存、预测、重计算），其中预测通过噪声级泰勒外推填补重用和重计算之间的空白，并基于误差传播分析提供显式稳定性控制；2）选择性计算，将执行限制在活动帧区间内。

Result: 在MAGI-1和SkyReels-V2数据集上，SCOPE实现了高达4.73倍的加速，同时保持与原始输出相当的质量，优于所有免训练基线方法。

Conclusion: SCOPE通过解决自回归视频扩散模型特有的低效问题，提供了一种高效且质量保持的加速方案，为长视频生成的实际应用提供了可行的计算优化路径。
...
TL;DR: VLM-UnBench：首个用于视觉语言模型训练无关视觉概念遗忘的基准测试，揭示提示级抑制与真正视觉概念擦除之间的差距


<details>
  <summary>Details</summary>
Motivation: 现有训练遗忘方法存在结构缺陷（微调窄遗忘集会先损害通用能力），而训练无关方法缺乏严谨的视觉任务评估基准。需要建立系统基准来评估训练无关视觉概念遗忘的有效性。

Method: 提出VLM-UnBench基准，涵盖4个遗忘级别、7个源数据集、11个概念轴，采用三级探测分类法与五种评估条件来区分真正遗忘与指令遵从。

Result: 现实遗忘提示下遗忘准确率接近无指令基线；仅在oracle条件下（向模型披露目标概念）出现有意义降低。对象和场景概念最难抑制，强指令调优模型即使有明确遗忘指令仍保持能力。

Conclusion: 训练无关的提示级抑制与真正的视觉概念擦除存在明显差距，当前方法无法有效实现视觉概念遗忘，需要更有效的训练无关遗忘技术。
...
<details>
  <summary>Details</summary>
Motivation: 从稀疏视角输入重建多人多物体动态场景对于机器人学和VR/AR的数字孪生至关重要，但面临两大挑战：严重相互遮挡下的视角一致性表示，以及复杂组合交互依赖的显式建模

Method: 提出MM-GS分层框架：1）实例级多视角融合模块，通过聚合所有可用视角的视觉信息为每个实例建立鲁棒一致的表示；2）场景级实例交互模块，在全局场景图上推理所有参与者之间的关系，细化属性以捕捉细微交互效应

Result: 在具有挑战性的数据集上进行广泛实验，方法显著优于强基线，产生具有高保真细节和合理实例间接触的最新结果

Conclusion: MM-GS通过分层框架有效解决了多人多物体渲染中的遮挡和交互建模问题，为动态场景重建提供了高质量解决方案
...
TL;DR: 本文提出了一种用于NTIRE 2026 3D恢复与重建挑战赛Track 2的多阶段流水线方法，针对烟雾退化图像，通过图像修复、去雾、MLLM增强、3DGS-MCMC优化和多次运行平均等技术，在提升图像可见性的同时保持跨视角一致性，最终在14支参赛队伍中排名第一。


<details>
  <summary>Details</summary>
Motivation: 烟雾会降低图像可见性并削弱场景优化和渲染所需的跨视角一致性，这给3D恢复与重建带来了挑战。需要一种方法能够在提升可见性的同时，限制输入视图之间的场景内容变化。

Method: 采用多阶段流水线：1) 图像修复；2) 去雾处理；3) 基于多模态大语言模型(MLLM)的增强；4) 3D高斯泼溅-马尔可夫链蒙特卡洛(3DGS-MCMC)优化；5) 对多次运行结果进行平均。该流水线旨在渲染前提升可见性，同时限制跨视角的场景内容变化。

Result: 在挑战赛基准测试中，该方法在定量性能和视觉质量上均优于提供的基线方法。在NTIRE 3DRR挑战赛Track 2的14名参与者中排名第一，代码已在GitHub开源。

Conclusion: 提出的多阶段流水线方法有效解决了烟雾退化图像的3D恢复与重建问题，在提升图像可见性的同时保持了跨视角一致性，在挑战赛中取得了优异的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 视频异常检测面临开放集本质的挑战，现有基于静态提示的训练免费方法依赖大规模基础模型来弥补提示模糊性。作者认为瓶颈不在于模型容量，而在于查询的静态特性。

Method: 提出QVAD框架，将VLM-LLM交互视为动态对话。LLM智能体基于视觉上下文迭代优化查询，引导小型VLM生成高保真描述和精确语义推理，无需参数更新。这种"提示更新"机制解锁轻量级模型的潜在能力。

Result: 在UCF-Crime、XD-Violence和UBNormal数据集上达到最先进性能，参数量仅为竞争方法的一小部分。在ComplexVAD单场景数据集上展示出色泛化能力。推理速度快，内存占用小，适合资源受限的边缘设备部署。

Conclusion: 通过动态对话和提示更新机制，QVAD框架成功解决了视频异常检测中静态查询的瓶颈问题，使轻量级模型能够实现高性能异常检测，具有实际部署价值。
...
TL;DR: STEAR是一个层感知的时空证据干预框架，通过识别高风险解码步骤，从中间层提取token条件视觉证据，用于恢复缺失的局部接地和构建时间扰动反事实，从而在单次编码推理中同时缓解时空幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型容易产生时空幻觉，生成视觉不支持的内容或错误的时间关系。现有缓解方法通常将幻觉视为统一的解码失败，应用全局共享的校正规则，但作者观察到解码器不同层对视觉接地和语言组合的贡献不同，表明干预必须是层感知的。

Method: 提出STEAR框架：1)识别高风险解码步骤；2)从接地敏感的中间层选择token条件视觉证据；3)使用共享证据进行双重干预：在中间层恢复缺失的局部接地，在后期层解码时构建时间扰动的patch级反事实来证伪不一致推理。所有操作在高效的单次编码推理框架内完成。

Result: 在代表性Video-LLM主干和挑战性基准测试上的实验表明，STEAR能持续减少幻觉，同时提高忠实度、时间一致性和鲁棒性。结果证实可靠的视频解码依赖于在正确层对精确证据进行干预，而不是强制执行全局惩罚。

Conclusion: STEAR通过层感知的时空证据干预，有效缓解视频大语言模型的时空幻觉问题。该方法强调需要根据解码器不同层的功能特性进行针对性干预，而不是采用统一的全局校正策略，为提升视频理解的可靠性提供了新思路。
...
TL;DR: PR3DICTR是一个用于三维医学图像分类研究的开源平台，基于PyTorch和MONAI构建，提供模块化设计和标准化功能，简化深度学习模型开发流程


<details>
  <summary>Details</summary>
Motivation: 三维医学图像数据和计算机辅助决策（特别是深度学习）在医学领域日益重要，但缺乏一个开放、灵活且标准化的研究平台来简化预测模型开发流程

Method: 基于社区标准分布（PyTorch和MONAI）构建开源平台，采用模块化设计原则和标准化，提供丰富的预置功能（模型架构设计选项、超参数解决方案、训练方法），同时允许用户"插入"自定义解决方案

Result: 开发了PR3DICTR平台，支持任何二元或基于事件的三维分类任务，仅需两行代码即可使用，减轻了开发负担同时保持可调整性

Conclusion: PR3DICTR为三维医学图像分类研究提供了一个开放、灵活且标准化的平台，通过模块化设计和丰富的预置功能简化了深度学习模型开发，同时保持用户自定义的灵活性
...
TL;DR: 该论文系统评估了Nano Banana 2作为通用图像恢复解决方案的潜力，发现提示设计对恢复效果至关重要，简洁提示配合明确保真度约束能实现重建精度与感知质量的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的进展，研究通用图像编辑模型能否作为统一的图像恢复解决方案，评估Nano Banana 2在不同场景和退化类型下的恢复能力。

Method: 系统评估Nano Banana 2在多样化场景和退化类型下的图像恢复性能，重点关注提示设计的影响，采用简洁提示配合明确保真度约束的方法，并与最先进的恢复模型进行对比。

Result: Nano Banana 2在全参考指标上优于现有方法，在感知质量方面保持竞争力，用户研究进一步支持这一结论。在挑战性场景（如小人脸、密集人群、严重退化）中表现出强大的泛化能力，但对提示表述敏感，可能需要迭代优化。

Conclusion: 通用生成模型具有作为统一图像恢复解决方案的强大潜力，但可控性和鲁棒性至关重要。提示设计在平衡重建精度与感知质量方面起关键作用。
...
TL;DR: 提出MI-Pruner方法，通过直接计算视觉和文本特征间的互信息来评估跨模态依赖关系，实现更精确的视觉token剪枝，相比基于注意力分数的传统方法效果更好且延迟更低。


<details>
  <summary>Details</summary>
Motivation: 针对多模态大语言模型中视觉信息相对稀疏的问题，现有基于注意力分数的视觉剪枝方法存在局限性。作者认为应该直接测量视觉和文本特征间的跨模态依赖关系，而不是依赖机制特定的信号（如注意力分数）。

Method: 提出MI-Pruner方法，在视觉和文本特征交互之前直接计算它们之间的互信息，显式地测量特征层面的跨模态依赖关系。该方法简单、高效且非侵入式，不需要访问内部注意力图或修改架构。

Result: 实验结果表明，该方法优于先前基于注意力的剪枝方法，同时保持了最小的延迟。

Conclusion: 通过直接计算视觉和文本特征间的互信息来评估跨模态依赖关系，能够实现更精确的视觉token剪枝，为多模态大语言模型的高效推理提供了新思路。
...
TL;DR: UCGP：首个针对红外视觉语言模型的通用物理对抗补丁框架，通过曲线网格参数化和统一表示驱动目标破坏跨模态语义对齐


<details>
  <summary>Details</summary>
Motivation: 红外视觉语言模型在低能见度环境中应用前景广阔，但其对抗鲁棒性尚未得到充分研究。现有对抗补丁方法主要针对RGB模型和闭集设置，不适用于红外VLM的开放式语义理解和物理部署需求

Method: 提出通用曲线网格补丁框架，包含：1) 曲线网格网格参数化实现连续、低频、可部署的补丁生成；2) 统一表示驱动目标促进子空间偏离、拓扑破坏和隐蔽性；3) 元差分进化和EOT增强的TPS变形建模提升真实世界部署和域偏移下的鲁棒性

Result: UCGP能持续破坏多种IR-VLM架构的语义理解，同时保持跨模型可迁移性、跨数据集泛化性、真实世界物理有效性以及对防御的鲁棒性

Conclusion: 该研究揭示了当前红外多模态系统中先前被忽视的鲁棒性漏洞，UCGP框架为红外VLM的对抗鲁棒性研究提供了新方向
...
TL;DR: 提出SC-DMD方法，通过显式正则化去噪更新的端点一致性组合，改进低NFE视频生成质量，并结合缓存分布感知训练实现实时自回归视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型蒸馏到极低推理预算（2-4 NFE）时面临挑战：轨迹一致性蒸馏在复杂视频动态下趋于保守，产生过度平滑外观和弱运动；分布匹配蒸馏能恢复锐利样本，但其局部训练信号未显式正则化去噪更新在时间步上的组合，导致组合展开易漂移。

Method: 提出自一致分布匹配蒸馏（SC-DMD），显式正则化连续去噪更新的端点一致性组合。针对实时自回归视频生成，将KV缓存视为质量参数化条件，提出缓存分布感知训练：在多个步骤展开上应用SC-DMD，并引入缓存条件特征对齐目标，引导低质量输出向高质量参考对齐。

Result: 在非自回归骨干（如Wan 2.1）和自回归实时范式（如Self Forcing）上的广泛实验表明，Salt方法持续改进低NFE视频生成质量，同时保持与多种KV缓存内存机制的兼容性。

Conclusion: SC-DMD通过显式正则化去噪更新的组合一致性解决了分布匹配蒸馏中的漂移问题，结合缓存分布感知训练实现了高质量实时视频生成，为极低推理预算下的视频生成提供了有效解决方案。
...
TL;DR: CAMEO是一个结构化多智能体框架，将条件图像编辑重新定义为质量感知、反馈驱动的过程，通过分解为规划、结构化提示、假设生成和自适应参考接地等协调阶段，实现迭代精炼，显著提升编辑质量和结构一致性。


<details>
  <summary>Details</summary>
Motivation: 现有条件图像编辑方法主要依赖单步生成范式，缺乏显式质量控制，容易导致过度偏离原始图像、产生结构伪影或环境不一致的修改，通常需要手动提示调整才能获得可接受结果。需要一种更可控、更可靠的结构化编辑方法。

Method: 提出CAMEO结构化多智能体框架，将编辑分解为：1）规划阶段；2）结构化提示；3）假设生成；4）自适应参考接地（仅在任务复杂度需要时调用外部指导）。通过将评估嵌入编辑循环，形成闭环过程，迭代精炼中间结果，纠正结构和上下文不一致性。

Result: 在异常插入和人体姿态切换任务上评估CAMEO，使用多个强大的编辑骨干和独立评估模型。相比多个最先进模型，CAMEO平均获得20%更高的胜率，展示了在条件图像编辑中改进的鲁棒性、可控性和结构可靠性。

Conclusion: CAMEO通过将条件图像编辑重新定义为质量感知、反馈驱动的过程，而非单次生成任务，显著提升了编辑质量。其结构化多智能体框架和闭环迭代精炼机制为解决现有方法缺乏显式质量控制的问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在冷启动场景中，预测产品质量需要依赖图像和文本元数据，但现有的视觉语言模型通常依赖大型架构和/或大量外部数据集，导致计算成本高昂。需要开发更高效的解决方案。

Method: 提出EffiMiniVLM紧凑双编码器视觉语言回归框架，集成EfficientNet-B0图像编码器和MiniLM文本编码器，配备轻量级回归头。引入加权Huber损失函数，利用评分计数强调更可靠的样本，提高训练样本效率。

Result: 模型仅含2770万参数，需要6.8 GFLOPs，使用20%的Amazon Reviews 2023数据集训练，在基准测试中获得0.40的CES分数，资源成本最低。与显著更大的模型保持竞争力，比其他前5方法资源效率高4-8倍，且是唯一不使用外部数据集的方法。数据扩展到40%时，模型性能超过使用更大模型和数据集的其他方法。

Conclusion: EffiMiniVLM证明了紧凑设计在冷启动产品质量预测中的有效性，通过高效架构和加权损失函数实现了高性能与低资源消耗的平衡，展示了强大的可扩展性潜力。
...
TL;DR: ProtoFlow：基于时间感知原型动态的持续遥感分割框架，通过建模原型轨迹和显式时间向量场来稳定原型几何并减少遗忘


<details>
  <summary>Details</summary>
Motivation: 当前遥感分割在真实部署中面临持续学习挑战：新语义类别不断出现，采集条件（季节、城市、传感器）不断变化。现有增量方法将训练步骤视为孤立更新，导致表征漂移和遗忘问题控制不足。

Method: 提出ProtoFlow框架，将类别原型建模为轨迹，通过显式时间向量场学习其演化过程。联合实施低曲率运动和类间分离约束，在整个增量学习过程中稳定原型几何结构。

Result: 在标准类别增量和领域增量遥感基准测试中，相比强基线方法获得一致提升，mIoUall指标提高1.5-2.0个百分点，同时减少了遗忘现象。

Conclusion: 显式建模时间原型演化是构建鲁棒持续遥感分割的实用且可解释策略，能够有效控制表征漂移和遗忘问题。
...
TL;DR: VOSR：首个纯视觉生成式超分辨率框架，无需多模态预训练，通过视觉语义引导和恢复导向的指导策略，在训练成本大幅降低的情况下达到或超越基于文本到图像扩散模型的SR方法性能。


<details>
  <summary>Details</summary>
Motivation: 当前生成式超分辨率方法主要依赖预训练的大型文本到图像扩散模型，但这些模型从通用T2I生成器出发，而SR本质上是低分辨率输入条件化的图像恢复任务。研究是否纯视觉数据训练的SR模型能够匹敌基于T2I的方法。

Method: 提出VOSR纯视觉生成框架：1）使用预训练视觉编码器从LR输入提取语义丰富且空间基础的特征作为视觉语义引导；2）重新审视分类器自由指导，发现标准无条件分支不适合从头训练的恢复模型，提出保留弱LR锚点的恢复导向指导策略；3）先训练多步VOSR模型，然后蒸馏为一步模型进行高效推理。

Result: VOSR训练成本不到代表性T2I基SR方法的十分之一，但在多步和一步设置下均达到竞争性或更好的感知质量和效率，在合成和真实世界基准上产生更忠实结构且更少幻觉。首次证明无需多模态预训练即可实现高质量生成式SR。

Conclusion: VOSR展示了纯视觉生成式超分辨率的可行性，挑战了当前依赖多模态预训练的范式。该方法在保持高质量输出的同时显著降低训练成本，为SR领域提供了更高效、更忠实的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型通常依赖单一的对比学习视觉编码器（如CLIP），虽然这类编码器在跨模态对齐和检索上有效，但自监督视觉编码器（如DINO）往往能捕捉更丰富的密集语义并在识别理解任务上表现更强。如何有效融合这两种互补的视觉表示来提升视觉语言模型性能是本研究的主要动机。

Method: 提出CoME-VL框架，融合对比训练视觉编码器和自监督DINO编码器。采用两种关键技术：(1) 熵引导的多层聚合，通过正交约束投影减少冗余；(2) RoPE增强的交叉注意力，对齐异构令牌网格并生成紧凑的融合视觉令牌。融合后的令牌可以注入到仅解码器的LLM中，对标准VLM流程改动最小。

Result: 在多样化的视觉语言基准测试中，CoME-VL始终优于单编码器基线。具体而言，在视觉理解任务上平均提升4.9%，在定位任务上平均提升5.4%。在RefCOCO检测任务上达到最先进性能，相比基线有大幅提升。消融研究验证了层融合、非冗余特征混合和融合容量对性能的影响。

Conclusion: 融合对比学习和自监督视觉编码器的互补表示能显著提升视觉语言模型的性能。CoME-VL框架通过有效的特征融合机制，在保持模块化设计的同时实现了性能的大幅提升，为多编码器视觉语言模型的发展提供了新思路。
...
TL;DR: 该研究针对大型语言模型在心理健康支持中的风险，特别是对精神病患者可能加剧妄想和幻觉的问题，开发了临床验证的安全评估方法，并测试了LLM作为评估者的可行性。


<details>
  <summary>Details</summary>
Motivation: 通用大型语言模型正被广泛用于心理健康支持，但高频使用存在显著风险，特别是对精神病患者可能强化妄想和幻觉。现有评估缺乏临床验证和可扩展性，需要建立更可靠的安全评估方法。

Method: 研究聚焦精神病作为LLM安全评估的关键条件：(1) 开发并验证七个临床医生指导的安全标准；(2) 构建人类共识数据集；(3) 测试自动化评估方法，包括使用LLM作为评估者（LLM-as-a-Judge）或采用多个LLM评估者的多数投票（LLM-as-a-Jury）。
...
TL;DR: CIPHER模型使用ERP特征和宽带DDA系数从高密度头皮EEG解码语音信息，在二分类任务上表现良好但易受混淆因素影响，在11类CVC音素任务上性能有限，主要作为基准和特征比较研究而非实用的EEG到文本系统。


<details>
  <summary>Details</summary>
Motivation: 从头皮EEG解码语音信息面临信噪比低和空间模糊的挑战，需要开发更有效的解码方法并评估不同神经表征特征的有效性。

Method: 提出CIPHER模型，采用双通路架构：(1)ERP特征通路，(2)宽带DDA系数通路。使用Conformer架构，在OpenNeuro ds006104数据集（24名参与者，两项TMS研究）上进行评估，包括二分类发音任务和11类CVC音素任务。

Result: 二分类发音任务达到接近天花板性能但易受混淆因素影响（声学起始可分离性和TMS目标阻断）。在11类CVC音素任务（16名留一被试）上性能有限：ERP通路WER 0.671±0.080，DDA通路WER 0.688±0.096，表明细粒度区分能力有限。

Conclusion: 该研究主要作为基准和特征比较研究，而非实用的EEG到文本系统。神经表征声明仅限于混淆因素受控的证据，强调了在EEG语音解码中控制混淆因素的重要性。
...
TL;DR: 本文提出了SWAY，一种无监督的计算语言学指标，用于量化大语言模型中的谄媚行为（sycophancy），并开发了基于反事实提示的缓解策略，能有效将谄媚行为降至接近零。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在谄媚行为，即倾向于向用户表达的立场偏移输出，而不考虑正确性或一致性。现有研究缺乏严谨的计算语言学指标来识别模型何时表现出谄媚行为，需要开发量化方法和有效的缓解策略。

Method: 1. 提出SWAY指标：通过反事实提示机制，测量模型在正面与负面语言压力下同意程度的变化，隔离框架效应与内容影响。2. 在6个模型上应用该指标进行基准测试。3. 开发反事实缓解策略：教导模型考虑如果相反假设被提出时的答案，与仅指示明确反谄媚的基线缓解方法进行对比。

Result: 1. 发现谄媚行为随认知承诺程度增加而增加。2. 基线缓解方法（明确指示反谄媚）仅产生中等程度的减少，且可能适得其反。3. 反事实思维链缓解方法能将谄媚行为降至接近零，适用于不同模型、承诺水平和子句类型，同时不抑制对真实证据的响应能力。

Conclusion: 本文贡献了用于基准测试谄媚行为的SWAY指标，以及基于该指标的反事实缓解策略。该方法能有效减少大语言模型的谄媚倾向，同时保持对真实证据的响应能力，为模型对齐提供了重要工具。
...
TL;DR: 在计算token预算固定的条件下，单智能体系统在信息效率上优于多智能体系统，多智能体系统的优势主要源于未控制的额外计算和上下文效应而非架构本身。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM系统（MAS）的性能提升常与增加的计算量混淆，缺乏在计算归一化条件下的理论分析和实证比较，需要澄清MAS与单智能体系统（SAS）的相对优势及其理论依据。

Method: 基于数据处理不等式提出信息论论证，并在三个模型家族（Qwen3、DeepSeek-R1-Distill-Llama、Gemini 2.5）上进行控制性实证研究，比较SAS与多种MAS架构在匹配预算下的表现，同时进行详细的诊断分析。

Result: 在推理token固定的条件下，SAS在多跳推理任务上持续匹配或优于MAS；发现API预算控制中的显著人为因素（特别是Gemini 2.5）和标准基准测试中的问题会夸大MAS的增益；多智能体系统的优势主要源于未控制的额外计算和上下文效应。

Conclusion: 对于多跳推理任务，多智能体系统的许多报告优势更可能源于未考虑的计算和上下文效应而非架构本身的内在优势，强调了在智能体系统中理解并显式控制计算、上下文和协调之间权衡的重要性。
...
TL;DR: LLMs在假设探索任务中表现出确认偏误，这降低了规则发现效率，但可通过人类干预策略缓解


<details>
  <summary>Details</summary>
Motivation: 确认偏误是人类推理中的常见认知偏差，会阻碍理性思考。本研究旨在探究大型语言模型是否也存在类似的确认偏误，以及如何通过人类心理学中的干预策略来缓解这种偏差

Method: 采用人类心理学中的规则发现研究范式：给定三个数字序列，模型通过交互反馈循环（1）提出新序列，（2）接收是否符合隐藏规则的反馈，（3）猜测规则。测试了11个不同家族和规模的LLM，并探索了针对人类的干预策略（如鼓励考虑反例）

Result: LLMs普遍表现出确认偏误，倾向于提出确认假设的序列而非证伪假设，导致规则发现速度更慢、成功率更低（平均42%）。通过提示干预可将成功率提升至56%，且通过行为蒸馏可将干预效果泛化到新任务（Blicket测试）

Conclusion: 确认偏误是LLMs在假设探索任务中的局限性，但可以通过借鉴人类心理学设计的干预策略有效缓解，这为改进LLMs的推理能力提供了新途径
...
TL;DR: 该论文研究大语言模型是否能在定量上近似人类社交推理，并提出基于语用理论的提示策略来改进这种近似。通过引入两个校准指标和语用理论驱动的提示方法，发现LLMs能复制人类社交推理的定性结构，但在定量校准上存在差异。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型是否不仅在定性上，而且在定量上近似人类的社交意义推理，以及基于语用理论的提示策略能否改进这种近似。当前LLMs展现出类似人类的语用和社交推理模式，但缺乏对其定量校准能力的系统评估。

Method: 引入两个校准指标：效应大小比（ESR）和校准偏差分数（CDS），分别评估结构保真度和幅度校准。基于语用理论设计提示策略：1）社交意义源于对语言替代方案的推理；2）听者推断说话者的知识状态和交际动机。在三个前沿LLMs上进行数值（不）精确性的案例研究。

Result: 所有模型都能可靠地复制人类社交推理的定性结构，但在幅度校准上差异显著。提示模型推理说话者知识和动机最一致地减少幅度偏差，而提示替代方案意识往往放大夸张。结合两种成分是唯一能改善所有模型校准敏感指标的干预措施，但精细的幅度校准仍仅部分解决。

Conclusion: LLMs捕捉推理结构但可变地扭曲推理强度，语用理论为改进这种近似提供了有用但不完整的工具。模型在定性结构上表现良好，但在定量校准方面仍需改进，特别是精细的幅度校准问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有多样性感知检索方法缺乏理论保证，且在检索段落数量k增加时面临可扩展性问题，需要一种具有理论保证且可扩展的多样性检索方法。

Method: 将多样性检索形式化为基数约束二元二次规划（CCBQP），提出非凸紧连续松弛和基于Frank-Wolfe的算法，包含景观分析和收敛保证。

Result: 在相关性与多样性的帕累托前沿上始终优于基线方法，同时实现了显著的速度提升。

Conclusion: 提出的CCBQP框架为多样性感知检索提供了理论保证和可扩展性，通过可解释权衡参数平衡相关性和语义多样性，在RAG系统中具有实际应用价值。
...
TL;DR: 该论文提出文化适应艺术描述生成任务，评估语言模型在不同文化群体中的文化熟悉度，通过语用说话者模型提升模拟听众理解度8.2%


<details>
  <summary>Details</summary>
Motivation: 语言模型在决策任务中表现出文化偏见，但在开放式文本生成任务中的文化熟悉度研究较少。需要评估模型为不同文化背景观众描述艺术品时的文化适应能力。

Method: 提出文化适应艺术描述生成任务，建立基于文化基础问答的评估框架，采用语用说话者模型优化生成内容，通过模拟听众理解和人工研究验证效果。

Result: 基础模型在该任务中表现仅勉强合格，但通过语用说话者模型可将模拟听众理解度提升8.2%。人工研究进一步证实，语用能力更强的模型在帮助理解方面评分高出8.0%。

Conclusion: 语言模型在文化适应生成任务中存在局限性，但语用建模能显著提升文化适应能力，为开发更具文化敏感性的语言生成系统提供了有效方法。
...
TL;DR: DEMASK是一种轻量级依赖预测器，通过估计掩码位置间的条件依赖关系，指导离散扩散语言模型进行并行解码，在保持输出质量的同时实现1.7-2.2倍加速。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型通过并行解码加速文本生成，但完全因子化的边际概率乘积近似会导致分布不匹配问题，当选定标记间存在强依赖关系时会降低输出质量。

Method: 提出DEMASK依赖预测器，附加到dLLM的最终隐藏状态上，单次前向传播即可估计掩码位置间的成对条件影响。使用贪心选择算法识别具有有限累积依赖的位置进行同时解掩码。

Result: 在子可加性假设下，理论证明该方法能限制并行采样与模型联合分布间的总变差距离。实验显示DEMASK在Dream-7B上实现1.7-2.2倍加速，同时匹配或优于基于置信度和KL散度的基线方法。

Conclusion: DEMASK通过依赖感知的并行解码有效解决了离散扩散语言模型中的分布不匹配问题，在保持生成质量的同时显著加速推理过程。
...
TL;DR: 该论文研究了多示例上下文学习在英语到十种低资源语言机器翻译中的应用，发现基于BM25的检索能显著提高数据效率


<details>
  <summary>Details</summary>
Motivation: 虽然多示例上下文学习能让大语言模型从大量示例中受益，但这种方法需要精心选择示例且推理成本高昂，对低资源语言社区来说尤其具有挑战性。研究旨在探索如何通过更有效的方法在多示例上下文学习中实现更好的机器翻译性能

Method: 研究对英语到十种FLORES+新增低资源语言的机器翻译进行了实证分析，评估了三种策略：1) 检索更具信息量的示例（基于BM25检索），2) 使用领域外数据，3) 按长度排序示例。通过比较不同数量示例下的性能来评估数据效率

Result: 研究发现多示例上下文学习随着示例数量增加而更有效。更重要的是，BM25检索显著提高了数据效率：50个检索示例的性能约等于250个普通多示例，250个检索示例的性能与1000个普通多示例相当

Conclusion: 基于检索的方法能大幅降低多示例上下文学习在低资源语言机器翻译中的推理成本，为资源受限的语言社区提供了更实用的解决方案。这证明了智能示例选择在多示例上下文学习中的关键作用
...
TL;DR: 提出使用LLM作为评判者生成奖励信号的RL框架，无需真实标签，通过单令牌输出实现高效奖励计算，结合可验证奖励可显著提升数学推理性能


<details>
  <summary>Details</summary>
Motivation: 现有RL方法通常依赖可验证奖励和真实标签，这限制了在无标注数据上的应用。需要开发无需真实标签监督的RL框架，利用LLM作为评判者从大量无标注数据中生成有效的训练信号

Method: 提出基于LLM评判者的RL框架：1) 使用LLM作为评判者评估模型输出，生成奖励信号；2) 评判者采用单令牌输出设计，实现高效奖励计算；3) 结合无标注数据进行知识蒸馏；4) 可与可验证奖励结合使用

Result: 在数学推理基准测试中取得显著性能提升，表明LLM评判者能够产生有效的RL微调训练信号。框架支持标签自由的知识蒸馏，无需真实标签监督

Conclusion: LLM作为评判者能够生成有效的训练信号用于RL微调，无需真实标签监督。该方法为利用大量无标注数据进行模型训练提供了新途径，特别是在数学推理等任务上展现出潜力
...
TL;DR: LLMimic：一种基于角色扮演的交互式游戏化AI素养教程，通过让用户扮演LLM并经历训练流程的三个关键阶段，显著提高AI素养、降低AI说服成功率，并增强真实性和社会责任感。
...
Method: 开发了LLMimic——基于角色扮演的交互式游戏化AI素养教程，参与者扮演LLM角色并经历训练流程的三个关键阶段：预训练、监督微调（SFT）和基于人类反馈的强化学习（RLHF）。进行了2×3组间研究（N=274），参与者分为控制组（观看AI历史视频）和实验组（与LLMimic互动），然后在三种现实AI说服场景中测试：慈善捐款说服、恶意金钱索取和酒店推荐。

Result: LLMimic显著提高了参与者的AI素养（p<.001），在所有场景中降低了说服成功率（p<.05），在酒店推荐场景中增强了真实性和社会责任感水平（p<0.01）。

Conclusion: LLMimic提供了一种可扩展的、以人为本的方法来提高AI素养，支持与有说服力的AI进行更明智的互动。这表明通过让用户亲身体验LLM训练过程，可以增强他们对AI工作原理的理解和批判性思维。
...
TL;DR: 提出一个多维度诊断框架，通过四轴难度分类法构建企业RAG基准测试，以系统诊断实际部署中的潜在弱点


<details>
  <summary>Details</summary>
Motivation: 现有学术基准测试无法系统诊断企业RAG系统的多维度复合挑战（推理复杂度、检索难度、文档结构多样性、可解释性要求），导致高评分模型在实际部署中可靠性不足

Method: 定义四轴难度分类法（推理复杂度、检索难度、文档结构多样性、可解释性要求），并将其集成到企业RAG基准测试中，构建多维度诊断框架

Result: 提出了一个能够系统诊断RAG系统潜在弱点的框架，填补了学术基准与企业实际需求之间的差距

Conclusion: 通过多维度诊断框架和四轴难度分类法，能够更准确地评估企业RAG系统的实际可靠性，为实际部署提供更有效的评估工具
...
TL;DR: 该论文聚焦于元语言主题，首先定义元语言概念及其与NLP和LLMs的关联，介绍两个实验室的元语言研究，最后提出元语言和元语言任务的四个维度，并列出未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 该论文旨在将研究焦点引向元语言这一重要但相对被忽视的主题。作者认为元语言在自然语言处理和大型语言模型研究中具有重要意义，需要系统性的探讨和梳理。

Method: 采用概念分析和框架构建的方法：1) 明确定义元语言概念；2) 建立元语言与NLP和LLMs的理论联系；3) 介绍两个实验室的具体研究案例；4) 提出元语言和元语言任务的四维分析框架。

Result: 论文提出了一个系统的元语言分析框架，包括四个维度，并识别了多个未被充分研究的未来研究方向。通过两个实验室的研究案例展示了元语言研究的实际应用价值。

Conclusion: 元语言是NLP和LLMs研究中一个关键但被低估的领域，需要更多系统性研究。论文提出的四维框架为未来研究提供了理论基础，识别的研究方向有望推动该领域的发展。
...
TL;DR: 该研究首次系统性地研究了工业级大模型（Hunyuan-A13B，800亿参数）的长上下文持续预训练学习动态，揭示了需要大规模数据（超过1500亿tokens）、传统评估方法的"欺骗性饱和"问题，以及注意力机制作为训练监控的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文持续预训练研究主要关注小规模模型和有限数据（数百亿tokens），直接迁移到工业级模型可能导致适应不足和过早终止训练。同时，当前评估方法过度依赖下游基准测试（如Needle-in-a-Haystack），无法准确反映内在收敛状态，存在"欺骗性饱和"风险。

Method: 使用工业级Hunyuan-A13B模型（800亿总参数），在2000亿tokens的训练轨迹上进行研究。提出分层分析框架：行为层面（监督微调探测）、概率层面（困惑度）、机制层面（注意力模式）。特别关注检索头的注意力分数作为训练监控指标。

Result: 1. 工业级LLM的长上下文持续预训练需要大规模数据（Hunyuan-A13B在超过1500亿tokens后才达到饱和）。2. 传统NIAH评分早期报告"虚假饱和"，而基于困惑度的分析显示持续的内在改进且与下游性能更相关。3. 检索头的注意力分数能可靠追踪训练进展，与监督微调结果高度相关，可作为高效低资源的训练监控器。

Conclusion: 该工作为工业级LLM的长上下文持续预训练提供了全面的监控框架、评估系统和机制解释，强调了大规模数据的重要性、传统评估方法的局限性，以及注意力机制作为训练状态监控的有效性。
...
TL;DR: SocioEval是一个评估大语言模型中社会经济偏见的模板框架，通过决策任务系统评估13个前沿LLM，发现偏见率差异显著（0.42%-33.75%），且不同主题的偏见表现差异很大。
...
Method: 提出SocioEval模板框架，通过分层结构（8个主题、18个话题）生成240个提示，覆盖6个类别对组合。评估13个前沿LLM的3,120个响应，采用严格的三阶段标注协议分析偏见。

Result: 偏见率存在显著差异（0.42%-33.75%）。生活方式判断的偏见比教育相关决策高10倍。部署安全措施能有效防止显性歧视，但对领域特定刻板印象表现出脆弱性。

Conclusion: SocioEval为审计语言模型中的阶级偏见提供了可扩展、可扩展的基础框架，揭示了社会经济偏见在不同主题中的差异表现，以及现有安全措施的局限性。
...
TL;DR: 通过向多智能体系统中的LLM提供同伴谄媚倾向排名，可以有效降低谄媚行为影响，提升讨论准确性


<details>
  <summary>Details</summary>
Motivation: 大型语言模型常表现出谄媚行为（同意用户立场即使与模型自身观点冲突），先前研究主要在单智能体环境中进行，而在协作多智能体系统中谄媚行为的影响尚未充分探索。本研究旨在探究了解其他智能体的谄媚程度是否会影响讨论结果。

Method: 使用六个开源LLM进行受控实验，为智能体提供同伴谄媚排名，这些排名基于静态（讨论前）和动态（在线）策略计算的谄媚分数。通过对比有无谄媚先验信息的情况，分析其对讨论结果的影响。

Result: 提供谄媚先验信息能够减少易谄媚同伴的影响，缓解错误级联效应，并将最终讨论准确性绝对提升10.5%。这表明该方法是一种轻量级且有效的方式来减少讨论中的谄媚行为并提高下游准确性。

Conclusion: 在多智能体系统中提供同伴谄媚倾向信息是减少谄媚行为、改善讨论质量的有效策略，为提升LLM协作系统的可靠性提供了实用解决方案。
...
TL;DR: 研究发现语言模型的偏见具有任务依赖性：在显性决策任务中模型会对抗刻板印象，但在隐性关联任务中却会重现刻板印象，同一模型在不同任务类型中的刻板印象得分差异可达0.43。


<details>
  <summary>Details</summary>
Motivation: 现有单任务基准测试只能捕捉模型偏见的一个侧面，无法全面评估语言模型的偏见特征。需要开发更全面的评估框架来揭示模型在不同任务类型中表现出的系统性偏见模式。

Method: 引入分层分类法涵盖9种偏见类型（包括种姓、语言和地理偏见等较少研究的维度），通过7个评估任务（从显性决策到隐性关联）进行操作化。使用约45K个提示对7个商业和开源权重LLM进行审计。

Result: 发现三个系统性模式：1) 偏见具有任务依赖性；2) 安全对齐具有不对称性（模型拒绝给边缘化群体分配负面特质，但自由地将正面特质与特权群体关联）；3) 较少研究的偏见维度在所有模型中表现出最强的刻板印象。

Conclusion: 单基准审计会系统性误判LLM偏见，当前的对齐实践掩盖了表征性伤害而非减轻它。对齐努力更多跟踪基准覆盖范围而非伤害严重性。
...
TL;DR: 研究通过对照实验证伪了E-Prime通过特定词汇-认知映射进行认知重构的假设，发现任何约束模型默认生成路径的限制都能作为输出正则化器改善推理，最浅层的约束效果最好


<details>
  <summary>Details</summary>
Motivation: 验证先前研究中提出的E-Prime通过移除"to be"动词选择性改变语言模型推理的认知重构机制，通过设计对照实验检验该假设

Method: 设计了五种条件（无约束控制组、E-Prime、No-Have、精细化元认知提示、中性填充词禁令）的对照实验，在六个模型和七个推理任务上进行测试（N=15,600次试验，过滤后11,919次）

Result: 所有预测都被证伪：四个处理组均优于控制组（83.0%），包括预测应显示零效应的主动对照组；中性填充词禁令改善最大（+6.7pp），E-Prime改善最小（+3.7pp）；跨模型相关特征未复现（平均r=0.005）

Conclusion: 结果支持更简单的机制：任何迫使模型偏离默认生成路径的约束都作为输出正则化器，通过破坏流畅但肤浅的响应模式来改善推理；最浅层的约束效果最佳，因其施加监控负荷而概念干扰最小
...
TL;DR: ChomskyBench：首个基于乔姆斯基层级理论的大语言模型形式推理能力系统评测基准，揭示LLMs在形式语言处理中的效率瓶颈而非绝对能力限制


<details>
  <summary>Details</summary>
Motivation: 现有LLM评测基准缺乏基于计算理论和复杂度的系统评估，无法确定SOTA LLMs是否能理解形式语言的结构化层次复杂性，需要填补这一关键空白

Method: 提出ChomskyBench基准，首次结合完整乔姆斯基层级覆盖、通过自然语言的过程追踪评估和确定性符号可验证性，设计全面的语言识别和生成任务套件

Result: 实验显示性能分层与层级复杂度相关；任务难度增加显著影响推理长度和性能；更大模型和先进推理方法虽有相对提升，但面临严重效率障碍，实际可靠性需要过高计算成本；LLMs效率显著低于传统算法程序

Conclusion: 结果界定了当前LLMs的实际限制，突显传统软件工具的必要性，为开发具有更强形式推理能力的未来LLMs提供指导
...
TL;DR: 该研究开发了基于心理角色和情感节奏的用户模拟器，测试对话AI在情感和伦理敏感场景中的表现，发现主流模型存在情感错位、伦理指导失败等系统性故障模式。


<details>
  <summary>Details</summary>
Motivation: 当前对话AI越来越多地应用于情感密集和伦理敏感的场景，但现有研究主要关注静态安全检测或情感基准测试，缺乏对动态对话过程中对齐机制如何演变的深入理解。需要探究对话AI在面对情感和伦理敏感行为时会出现哪些故障，以及这些故障如何影响对话质量。

Method: 开发了基于心理角色和情感节奏的用户模拟器，能够进行多轮对话，通过设定不同的心理角色和情感变化轨迹来压力测试聊天机器人的表现。该方法能够模拟真实对话中情感和伦理敏感场景的动态演变。

Result: 主流模型在情感轨迹升级时表现出系统性故障，包括：1) 情感错位 - 模型无法与用户情感状态保持同步；2) 伦理指导失败 - 在伦理敏感问题上提供不当建议；3) 跨维度权衡 - 共情压倒或削弱责任意识。这些故障模式被组织成一个分类体系。

Conclusion: 研究强调了在动态交互中保持伦理一致性和情感敏感性的重要性，为HCI社区提供了诊断和改进对话AI在价值敏感和情感密集场景中表现的新视角。需要开发能够在整个对话过程中维持伦理连贯性和情感对齐的对话系统。
...
TL;DR: 提出了一种名为Multiple-Debias的多语言去偏方法，通过多语言反事实数据增强和自去偏技术，结合参数高效微调，显著降低了多语言预训练语言模型在四种语言中的三种敏感属性偏见。


<details>
  <summary>Details</summary>
Motivation: 多语言预训练语言模型虽然重要，但普遍存在性别、种族、宗教等敏感属性偏见，需要开发有效的多语言去偏方法来提升模型公平性。

Method: 提出Multiple-Debias方法，包含：1）多语言反事实数据增强；2）多语言自去偏技术；3）参数高效微调。该方法在预处理和后处理阶段同时工作，并将CrowS-Pairs基准扩展到德语、西班牙语、中文和日文。

Result: 实验表明：1）多语言去偏方法在减轻偏见方面优于单语言方法；2）整合不同语言的去偏信息显著提升了MPLMs的公平性；3）在四种语言的三种敏感属性上有效降低了偏见。

Conclusion: Multiple-Debias是一种有效的全流程多语言去偏方法，能够显著降低多语言预训练语言模型中的敏感属性偏见，提升模型公平性，且多语言方法优于单语言方法。
...
TL;DR: 提出持续多模态知识图谱推理（CMMKGR）新任务，构建基准数据集，并提出MRCKG模型解决多模态知识图谱动态演化中的灾难性遗忘问题


<details>
  <summary>Details</summary>
Motivation: 现实世界多模态知识图谱是动态演化的，现有持续知识图谱推理方法只关注结构三元组而无法充分利用新实体的多模态信号，而现有多模态知识图谱推理方法假设静态图谱并在图谱演化时遭受灾难性遗忘

Method: 提出MRCKG模型：1）采用多模态-结构协同课程调度，基于新三元组与历史图谱的结构连接性和多模态兼容性安排渐进学习；2）引入跨模态知识保留机制，通过实体表示稳定性、关系语义一致性和模态锚定减轻遗忘；3）设计多模态对比重放方案，通过两阶段优化策略，利用多模态重要性采样和表示对齐强化已学知识

Result: 在多个数据集上的实验表明，MRCKG在显著改善新知识学习的同时，有效保留了先前学习的多模态知识

Conclusion: 本文系统研究了持续多模态知识图谱推理问题，构建了基准数据集并提出MRCKG模型，为解决多模态知识图谱动态演化中的灾难性遗忘问题提供了有效解决方案
...
TL;DR: RTT框架通过令牌级相关性判别器解决基于规则的强化学习中奖励稀疏和模糊问题，将粗粒度响应级评分与细粒度令牌级信用分配相结合，显著提升指令遵循性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于规则的强化学习方法主要依赖响应级奖励，导致严重的奖励稀疏性和模糊性问题，难以精确指导语言模型学习复杂、开放域的指令遵循任务。

Method: 提出RTT框架：1) 令牌级相关性判别器预测响应中哪些令牌对特定约束负责；2) RTT-GRPO算法整合响应级和令牌级优势；3) 提出样本内令牌组归一化方法处理三维奖励空间。

Result: 大量实验和基准测试表明，RTT在不同模型上均优于其他基线方法，在指令级和规则级准确性方面表现一致优越。

Conclusion: RTT通过将粗粒度规则评分与细粒度令牌级信用分配相结合，有效解决了基于规则的强化学习中的奖励稀疏和模糊问题，为复杂指令遵循任务提供了更精确的训练信号。
...
TL;DR: Gen-SSD是一种学生参与循环的生成时自选择蒸馏框架，通过在教师采样过程中让学生评估候选延续，引导可学习推理路径的扩展并早期剪除无用分支，从而提升小模型从大模型复杂推理轨迹中学习的效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖后处理过滤，在完整生成后基于启发式标准选择轨迹，但这种方法无法控制生成过程本身，仍可能产生超出学生学习能力的推理路径。需要一种能够在生成过程中就进行选择的方法。

Method: 提出Gen-SSD（生成时自选择蒸馏）框架，采用学生参与循环的方式。在教师采样过程中，学生评估候选延续，引导可学习推理路径的扩展，并早期剪除无帮助的分支，而不是被动消费完整轨迹。

Result: 在数学推理基准测试中，Gen-SSD持续优于标准知识蒸馏和近期基线方法，相比标准KD提升约5.9个百分点，相比其他基线提升高达4.7个百分点。分析显示Gen-SSD产生更稳定和可学习的推理轨迹。

Conclusion: Gen-SSD通过生成时选择机制有效解决了复杂推理轨迹向小模型转移的挑战，证明了在生成过程中融入监督对于有效蒸馏的重要性，为知识蒸馏提供了新思路。
...
TL;DR: GRADE使用梯度动力学检测LLM内部知识是否足以回答问题，通过计算梯度与隐藏状态子空间的跨层秩比来量化知识差距


<details>
  <summary>Details</summary>
Motivation: 现有方法通过LLM自我报告置信度或分析响应token的隐藏状态来评估知识激活程度，但这些方法可能捕获与查询要求不对齐的特征（如风格和长度相关特征），无法准确检测知识差距

Method: 提出GRADE方法，通过计算梯度与对应隐藏状态子空间的跨层秩比来量化知识差距，利用梯度作为所需知识更新的估计器特性

Result: 在六个基准测试上验证了GRADE的有效性和对输入扰动的鲁棒性，并通过案例研究展示了梯度链如何为长格式答案生成可解释的知识差距解释

Conclusion: GRADE提供了一种基于梯度动力学的新方法来检测LLM的知识差距，相比现有方法能更准确地评估模型内部知识是否足以回答问题，并具有可解释性优势
...
TL;DR: 多语言机器翻译中权重空间模型合并效果不佳，尤其当目标语言不同时性能下降，原因是微调改变了语言选择性神经元的分布而非增强其特异性
...
Method: 在大规模双语语料上完全微调语言模型，评估标准合并策略，使用跨度条件神经元选择性和层中心核对齐分析内部表示

Result: 合并导致性能下降，尤其目标语言不同时更明显；语言特异性神经元集中在嵌入层和上层transformer块，中间层主要共享；微调重新分配而非增强语言选择性

Conclusion: 多语言微调可能重塑几何结构，降低与标准权重空间合并假设的兼容性，解释了多语言翻译场景中合并失败的原因
...
TL;DR: 提出了一个用于生物医学乌尔都语命名实体识别的黄金标准基准数据集BioUNER，包含15.3万标记，标注者间一致性达0.78，并评估了多种机器学习模型。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语在生物医学领域的命名实体识别缺乏高质量的基准数据集，限制了该语言在医疗信息处理方面的发展和应用。

Method: 从乌尔都语在线新闻门户、医疗处方和医院健康博客网站爬取健康相关文章，经过预处理后由三位熟悉医学领域的母语标注者使用Doccano工具进行标注，共标注15.3万标记。

Result: 创建了包含15.3万标记的BioUNER数据集，标注者间一致性分数达0.78，验证了其黄金标准质量。评估了SVM、LSTM、mBERT和XLM-RoBERTa等模型，展示了数据集的实用性和基准能力。

Conclusion: BioUNER数据集作为可靠的基准，是乌尔都语语言处理资源的宝贵补充，有助于推动乌尔都语生物医学文本处理研究。
...
TL;DR: 该研究探讨了语言图表示如何补充和改进神经语言建模，发现语义成分结构对语言建模性能最有帮助，超越句法成分结构和依赖结构。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言图表示（包括句法和语义结构）在原则上如何能够补充和改进神经语言建模，特别是在神经符号语言建模的背景下。

Method: 采用集成设置，结合预训练的Transformer模型和来自7种不同形式化体系的真实图结构，比较不同语言图表示对语言建模性能的影响。

Result: 研究发现语义成分结构对语言建模性能最有帮助，超越了句法成分结构以及句法和语义依赖结构。此外，效果因词性类别而异，差异很大。

Conclusion: 研究结果指出了神经符号语言建模的有希望趋势，并邀请未来研究量化不同形式化体系所做的设计选择。
...
<details>
  <summary>Details</summary>
Motivation: 当前采用MoE架构的LLM虽然性能出色，但普遍存在幻觉问题（生成看似合理但事实错误的内容）和系统偏见，这些问题在推理过程中因专家激活不均匀而被放大。

Method: 提出Council Mode三阶段框架：1) 智能分类器根据查询复杂度进行路由；2) 并行调用架构多样的模型进行专家生成；3) 结构化共识合成，明确识别一致性、分歧和独特发现后再生成最终响应。

Result: 在HaluEval基准上实现35.9%的相对幻觉率降低，在TruthfulQA上相比最佳单模型提升7.8个百分点，同时在不同领域保持显著更低的偏见方差。

Conclusion: Council Mode通过多智能体共识框架有效解决了LLM的幻觉和偏见问题，提供了数学化的共识机制和系统架构，并在开源AI工作空间中验证了其有效性。
...
TL;DR: 提出基于多头注意力的俄语形态标注新架构，支持开放词典并考虑词素特征，在SinTagRus和Taiga数据集上取得优于先前结果的性能


<details>
  <summary>Details</summary>
Motivation: 解决俄语形态标注问题，支持开放词典以分析训练集中未出现的词汇，同时考虑词素特征（前缀、后缀等）对形态分析的影响

Method: 基于多头注意力的新架构，将词拆分为子词标记，通过训练过程聚合子词向量为词向量，不使用RNN，无需BERT式大规模无标注文本预训练

Result: 在SinTagRus和Taiga数据集上，部分语法范畴准确率达98-99%以上，优于先前结果；90%词汇能精确预测所有语法范畴；模型可在消费级GPU上训练，处理速度更快
...
TL;DR: 该研究通过分析语义结构预测语言建模的失败案例，建立实证下界，探索使语义引导语言模型成功所需的最小条件。


<details>
  <summary>Details</summary>
Motivation: 基于语义结构预测语言建模的负面结果，建立实证下界，确定使语义引导语言模型成功所需的最小条件。

Method: 设计简洁的词汇级语义结构二进制向量表示，深入评估增量标注器需要达到多好的性能才能使端到端语义引导语言模型优于基线。

Result: 发现：(a) 语义向量表示的维度可以大幅降低而不丧失其主要优势；(b) 预测质量的下界不能仅通过单一分数确定，需要考虑信号和噪声的分布。

Conclusion: 语义引导语言模型需要结合序列神经组件和层次符号组件，在低惊奇度和高语言可解释性之间取得平衡，预测质量评估需考虑信号噪声分布。
...
Method: 研究涉及25名来自5个不同组别的标注者（专家和非专家），标注1021个对话中的20种社会影响力技术、意图、反应和后果。150个文本子集在主标注过程前后各标注一次用于比较。采用定性定量分析、半结构化访谈、自我评估调查，以及在比较数据集上训练和评估LLM来测量能力变化。

Result: 结果显示标注者自我感知能力和信心显著提升。数据质量的变化表明标注过程能增强标注者能力，且专家组效果更明显。标注者能力变化对基于其标注数据训练的LLM性能有可见影响。

Conclusion: 标注过程本身能提升标注者能力，特别是专家标注者。这种能力变化会影响下游机器学习模型性能，表明标注者能力动态变化是数据标注研究中需要考虑的重要因素。
...
TL;DR: LogicPoison攻击框架通过保持类型一致的实体交换，破坏知识图谱的拓扑完整性，从而绕过GraphRAG系统的防御机制，显著降低其性能。


<details>
  <summary>Details</summary>
Motivation: GraphRAG系统通过知识图谱增强LLM的推理能力，对传统RAG攻击具有抵抗力。然而，其安全性依赖于底层图谱的拓扑完整性，这种完整性可能在不改变表层文本语义的情况下被破坏。本文旨在探索这种安全漏洞。

Method: 提出LogicPoison攻击框架，采用类型保持的实体交换机制，同时扰动全局逻辑枢纽（破坏整体图连通性）和查询特定的推理桥梁（切断关键的多跳推理路径），从而将有效推理导向死胡同，同时保持表层文本的合理性。

Result: 在多个基准测试上的综合实验表明，LogicPoison成功绕过GraphRAG的防御，显著降低其性能，在有效性和隐蔽性方面均优于现有最先进的基线方法。

Conclusion: GraphRAG系统的安全性依赖于知识图谱的拓扑完整性，LogicPoison通过逻辑推理攻击而非内容注入的方式，揭示了这一关键漏洞，为图增强RAG系统的安全研究提供了新方向。
...
TL;DR: 提出NeuReasoner框架，通过白盒分析识别关键神经元及其波动模式，集成轻量级MLP进行故障检测和特殊令牌触发的自校正机制，提升大推理模型的性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务中表现出色，但存在三个层面的持续故障模式：1）步骤内层面的计算或推导错误；2）步骤间层面的振荡和停滞；3）实例层面的自适应过度思考。现有方法针对孤立层面且缺乏统一性，同时黑盒性质和依赖强化学习限制了可解释性和可控性。

Method: 进行深入的白盒分析，识别与不同故障相关的关键神经元（神经元混合体，MoN）及其波动模式。基于此提出NeuReasoner框架，集成轻量级MLP进行故障检测，以及通过监督微调学习的特殊令牌触发自校正机制。推理时，检测到故障时插入特殊令牌以触发可控的补救行为。

Result: 在六个基准测试、六个骨干模型（8B~70B）上对九个竞争基线进行评估，NeuReasoner实现了高达27.0%的性能提升，同时将令牌消耗降低了19.6% ~ 63.3%。

Conclusion: NeuReasoner提供了一个可解释、可控且统一的推理框架，通过白盒分析和神经元级干预有效解决了大型推理模型的多层次故障问题，在提升性能的同时显著降低了计算成本。
...
TL;DR: R2-Write框架通过迭代式作者-评判者交互，在开放式写作任务中整合显式反思和修订模式，显著提升大型语言模型的深度推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有主流推理模型在可验证领域（如数学）通过长思维链取得显著改进，但在开放式写作任务中效果有限。研究发现这些模型缺乏深度反思和修订模式，导致改进幅度远小于数学推理任务。

Method: 提出R2-Write自动化框架：1）通过迭代式作者-评判者交互合成高质量的思维轨迹；2）设计过程奖励机制监督反思质量，防止冗余反思；3）在强化学习中提高性能和标记效率。

Result: 在多个创意写作和深度研究基准测试中，R2-Write框架表现出显著改进，验证了显式整合反思和修订模式能够解锁开放式写作任务的深度推理能力。

Conclusion: 通过系统性地整合反思和修订模式，R2-Write框架成功解决了现有推理模型在开放式写作任务中的局限性，为提升语言模型在开放领域的深度推理能力提供了有效途径。
...
TL;DR: JoyAI-LLM Flash是一个高效的混合专家模型，在50B参数以下实现了性能与推理效率的平衡，通过新颖的FiberPO算法、高稀疏架构和训练-推理协同设计优化。


<details>
  <summary>Details</summary>
Motivation: 在50B参数以下的语言模型中，平衡强大性能与推理效率（特别是token效率）是一个关键挑战。现有模型在保持高性能的同时难以实现高效的推理速度和资源利用。

Method: 1. 采用混合专家架构，总参数量48B但每前向传播仅激活2.7B参数；2. 引入FiberPO算法，基于纤维化理论将信任区域维护分解为全局和局部组件；3. 平衡"思考"与"非思考"认知模式；4. 联合训练-推理协同设计，包括密集多token预测和量化感知训练；5. 在20万亿token语料上预训练，并通过SFT、DPO和大规模RL进行后训练优化。

Result: 模型实现了比同规模行业领先模型更高的稀疏比，显著提升了token效率和推理吞吐量。发布了JoyAI-LLM-48B-A3B Base及其后训练变体的检查点。

Conclusion: JoyAI-LLM Flash通过创新的架构设计、优化算法和训练策略，在50B参数以下的语言模型中重新定义了性能与效率的权衡，为开源社区提供了高效的语言模型解决方案。
...
TL;DR: 提出一种开源方法，让用户通过自然语言查询结构化非文本数据集，训练LLM生成可执行查询，使用合成训练数据生成管道，在资源受限环境下实现高精度。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成（RAG）在处理数值和高度结构化信息时存在困难，需要一种能够理解用户意图并生成可执行查询的方法，特别是在资源受限环境中。

Method: 1. 提出合成训练数据生成管道，创建多样化的问答对，捕捉用户意图和底层数据集语义；2. 使用QLoRA和4位量化对DeepSeek R1 Distill 8B模型进行微调；3. 在西班牙Durangaldea地区基本服务可及性数据集上进行评估。

Result: 微调模型在单语言、多语言和未见位置场景下均实现高准确率，展示了强大的泛化能力和可靠的查询生成能力，小型领域特定模型无需依赖大型专有LLM即可实现高精度。

Conclusion: 该方法证明小型领域特定模型能够在不依赖大型专有LLM的情况下，在资源受限环境中实现高精度自然语言到结构化查询的转换，适用于更广泛的多数据集系统。
...
TL;DR: 论文提出"言语化假设"框架，揭示LLM社会奉承行为源于对用户意图的错误假设（如低估用户寻求信息而非安慰的需求），并通过假设探针实现可解释的微调控制。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解LLM为何表现出社会奉承行为（如用户问"我错了吗？"时一味附和而非客观评估），假设这种行为源于LLM对用户意图的错误假设。

Method: 提出言语化假设框架，从LLM内部提取其关于用户意图的假设；开发假设探针（基于这些假设内部表示的线性探针），用于可解释地调控奉承行为；分析人类对AI与人类的不同期望差异。

Result: 发现LLM在社会奉承数据集中的主要假设是"寻求认可"；证明言语化假设与奉承行为存在因果关系；假设探针能实现细粒度、可解释的奉承行为调控；揭示LLM未考虑人类对AI与人类期望的差异。

Conclusion: 言语化假设是理解LLM奉承行为的新机制，为可解释的AI安全调控提供框架，强调需要训练LLM理解人类对AI的独特期望以改善其响应质量。
...
TL;DR: 提出多视角知识蒸馏方法，通过深度模仿自注意力和前馈模块来捕捉不同层面的语言知识信息，解决现有方法在层间知识分布对齐过程中丢失细粒度信息的问题。
...
Method: 提出多视角知识蒸馏方法，深度模仿自注意力和前馈模块，从不同层面捕捉丰富的语言知识信息。

Result: 在相同存储参数预算下，MaKD方法能够达到与各种强基线模型竞争的性能，并且在蒸馏自回归架构模型方面也表现良好。

Conclusion: 多视角知识蒸馏方法通过深度模仿Transformer核心组件，有效保留了细粒度语言知识，实现了高效的语言模型压缩。
...
TL;DR: 提出一个领域适应的RAG管道用于教学对话标注，通过微调轻量级嵌入模型和话语级索引来提升标注性能，而非微调生成模型。


<details>
  <summary>Details</summary>
Motivation: 教学对话自动标注是高风险任务，LLM缺乏领域知识时表现不佳。需要一种实用的方法来提升教学对话标注的准确性，同时避免昂贵的生成模型微调。

Method: 设计领域适应的RAG管道：1）在辅导语料上微调轻量级嵌入模型；2）在话语级别索引对话以检索带标签的少样本示例；3）保持生成模型冻结。在两个真实辅导对话数据集（TalkMoves和Eedi）和三个LLM骨干（GPT-5.2、Claude Sonnet 4.6、Qwen3-32b）上评估。

Result: 最佳配置在TalkMoves上达到Cohen's κ 0.526-0.580，在Eedi上达到0.659-0.743，显著优于无检索基线（κ=0.275-0.413和0.160-0.410）。消融研究表明，话语级索引是性能提升的主要驱动因素，Top-1标签匹配率在TalkMoves上从39.7%提升到62.0%，在Eedi上从52.9%提升到73.1%。检索还纠正了零样本提示中的系统性标签偏差，对罕见和上下文依赖标签提升最大。

Conclusion: 仅适应检索组件是提升教学对话标注性能的实用有效途径，同时保持生成模型冻结。话语级索引而非嵌入质量是性能提升的关键因素，该方法为专家级教学对话标注提供了可行方案。
...
TL;DR: StoryScope通过分析叙事层面的10个维度特征，能够以93.2%的宏F1分数区分人类与AI生成的小说，发现AI故事倾向于过度解释主题、情节单一，而人类故事则包含更多道德模糊性和时间复杂性。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成小说的普及，作者身份和原创性问题变得日益重要。现有研究多关注表面写作风格特征，本文旨在探索是否可以不依赖风格信号，仅通过话语层面的叙事选择（如角色能动性、时间不连续性）来区分AI与人类创作。

Method: 提出StoryScope管道，自动从10个维度诱导出细粒度、可解释的话语层面叙事特征空间。应用于包含10,272个写作提示的平行语料库，每个提示由人类作者和5个LLM分别创作，共61,608个故事（每个约5,000词），每篇故事提取304个特征。

Result: 仅叙事特征就能达到93.2%宏F1的人类vs.AI检测准确率和68.4%宏F1的六方作者归属准确率，保留了包含风格线索模型97%以上的性能。30个核心叙事特征捕捉了主要信号：AI故事过度解释主题、偏爱整洁的单线情节，而人类故事将主角选择描绘得更道德模糊且时间复杂性更高。各模型有特定指纹特征：Claude事件升级平缓，GPT过度使用梦境序列，Gemini默认外部角色描述。
...
TL;DR: 提出一个同时评估长文本生成事实性的精确度和召回率的综合框架，发现当前LLM在精确度上表现远好于召回率，事实不完整性是主要限制。


<details>
  <summary>Details</summary>
Motivation: 现有的事实性评估方法主要关注精确度（将回答分解为原子声明并验证），但忽视了同样重要的召回率维度（回答是否覆盖了应包含的相关事实）。需要综合评估长文本生成的事实完整性。

Method: 提出综合事实性评估框架，同时测量精确度和召回率。利用外部知识源构建参考事实，确定它们是否在生成文本中被捕获。引入基于相关性和显著性的重要性感知加权方案。

Result: 分析显示当前LLM在精确度上的表现显著优于召回率，表明事实不完整性是长文本生成的主要限制。模型通常能更好地覆盖高度重要的事实，而非所有相关事实。

Conclusion: 需要同时评估精确度和召回率才能全面衡量长文本生成的事实性。当前LLM在事实完整性方面存在显著不足，特别是在覆盖所有相关事实方面，而不仅仅是高度重要的事实。
...
TL;DR: 通过PCA和岭回归从情感标注文本中提取LLM表征的效价-唤醒度子空间，该子空间具有圆形几何结构，能控制模型输出的情感维度、拒绝行为和谄媚倾向。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型表征中是否存在与人类情感感知模型一致的效价-唤醒度（VA）子空间，并研究该子空间如何影响模型的行为模式，特别是拒绝和谄媚倾向。

Method: 从211k情感标注文本中提取情感导向向量，通过PCA降维后使用岭回归学习VA轴，建立与模型自报告的效价-唤醒度分数的映射关系，从而识别VA子空间。

Result: 恢复的VA子空间呈现圆形几何结构，与人类情感感知模型一致；在44k词汇项上与人类众包VA评分相关；沿VA轴导向能单调调节模型输出的情感维度，并双向控制拒绝和谄媚行为：提高唤醒度降低拒绝、增加谄媚。

Conclusion: LLM表征中存在可解释的VA子空间，该子空间具有跨架构通用性，能机制性地解释情感导向控制：拒绝相关令牌位于低唤醒、负效价区域，VA导向直接调节其发射概率。
...
TL;DR: 该论文是一篇关于语言模型增强策略的系统综述，提出了基于推理时结构化上下文程度的统一分析框架，涵盖了从上下文学习到因果增强的多种方法，并提供了文献筛选、证据合成和部署决策框架。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然编码了丰富的世界知识，但受到静态知识、有限上下文窗口和弱结构化因果推理的根本限制。需要系统梳理和评估各种增强策略，为可信赖的检索增强NLP提供理论指导和实践框架。

Method: 1. 提出统一分析框架：以推理时提供的结构化上下文程度为轴，系统分析上下文学习、提示工程、RAG、GraphRAG和CausalRAG等增强策略；2. 建立透明文献筛选协议；3. 开发声明审计框架；4. 构建结构化跨论文证据合成方法，区分高置信度发现和新兴结果。

Result: 1. 建立了语言模型增强策略的统一分类和分析框架；2. 提供了系统化的文献评估方法学；3. 开发了区分证据强度的证据合成框架；4. 识别了不同增强策略的优势和局限性。

Conclusion: 论文提出了面向部署的决策框架和具体研究优先级，为可信赖的检索增强NLP提供了系统指导。强调需要结合结构化上下文增强来克服LLM的固有局限，并提供了评估和实施这些策略的方法学基础。
...
TL;DR: EWAD和CPDP方法用于低资源抽象摘要的多教师知识蒸馏，通过教师间一致性路由监督和几何约束提升可靠性，在孟加拉语数据集上验证了logit级KD最可靠，跨语言伪标签KD在3.2倍压缩下保留71-122%教师性能。


<details>
  <summary>Details</summary>
Motivation: 研究低资源抽象摘要任务中的多教师知识蒸馏，从可靠性角度出发，解决传统知识蒸馏方法在复杂任务中可能出现的性能不稳定问题，特别是在多教师监督和数据稀缺场景下。

Method: 提出EWAD（熵加权一致性感知蒸馏）和CPDP（容量比例散度保持）两种方法。EWAD基于教师间一致性在token级别路由教师蒸馏监督和黄金监督；CPDP对学生在异构教师空间中的位置施加几何约束。在13个BanglaT5变体和8个Qwen2.5实验中进行评估。

Result: logit级知识蒸馏提供最可靠的性能提升，复杂蒸馏方法对短摘要提升语义相似度但损害长摘要质量。跨语言伪标签知识蒸馏在10种语言上以3.2倍压缩保留教师ROUGE-L的71-122%。人类验证的多评委LLM评估揭示单评委流程存在校准偏差。

Conclusion: 可靠性感知蒸馏有助于确定多教师监督何时能改进摘要质量，以及何时数据扩展比损失工程更有效。该方法为低资源抽象摘要中的知识蒸馏提供了可靠性保证。
...
TL;DR: 论文提出了首个可迁移的基于学习的成员推理攻击方法LT-MIA，通过利用微调过程中产生的无限标记数据训练分类器，实现了跨不同架构语言模型的零样本迁移攻击。


<details>
  <summary>Details</summary>
Motivation: 现有成员推理攻击方法依赖人工设计的启发式规则，受限于设计者的直觉。作者观察到微调过程天然产生无限标记数据，可以突破影子模型的瓶颈，将成员推理带入深度学习时代。

Method: 提出LT-MIA方法：1）利用微调过程中已知成员关系的无限数据训练成员推理分类器；2）将成员推理重构为基于每token分布统计量的序列分类问题；3）仅在Transformer架构上训练，实现跨架构零样本迁移。

Result: LT-MIA在未见过的Mamba、RWKV-4、RecurrentGemma架构上分别达到0.963、0.972、0.936 AUC，均超过在保留Transformer上的性能（0.908 AUC）。在代码数据上达到0.865 AUC，尽管仅在自然语言文本上训练。在Transformer上，比最强基线在0.1% FPR下的TPR提高2.8倍。

Conclusion: 微调语言模型会产生跨架构和领域的可检测记忆化特征，这些特征仅源于梯度下降和交叉熵损失优化。LT-MIA通过深度学习方式有效捕获这一信号，实现了首个可迁移的成员推理攻击，为隐私风险评估提供了新工具。
...
TL;DR: 论文提出行为对齐分数（BAS），一种基于决策理论的度量标准，用于评估LLM置信度在包含弃权选项的决策场景中的可靠性，揭示现有标准指标（如ECE、AURC）的局限性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常在应该弃权的情况下产生自信但错误的答案，而标准评估协议不考虑不同风险偏好下置信度应如何指导决策。需要一种能够评估LLM置信度在支持弃权感知决策方面效果的度量标准。

Method: 引入行为对齐分数（BAS），这是一种基于显式的"回答或弃权"效用模型的决策理论度量标准。BAS通过连续风险阈值聚合实现的效用，产生一个依赖于置信度大小和排序的决策级可靠性度量。理论证明真实的置信度估计能最大化期望BAS效用。

Result: 研究显示不同LLM在决策有用置信度方面存在显著差异，更大更准确的模型通常获得更高BAS，但即使是前沿模型仍存在严重过度自信问题。具有相似ECE或AURC的模型可能表现出非常不同的BAS，突显标准指标的局限性。简单干预（如top-k置信度引出和后校准）能显著改善置信度可靠性。

Conclusion: 该工作提供了一个原则性度量标准（BAS）和全面的基准测试，用于评估LLM置信度可靠性。BAS与对数损失等适当评分规则相关但结构不同，对过度自信错误施加不对称惩罚，更符合实际决策需求。
...
### [132] [Holos: A Web-Scale LLM-Based Multi-Agent System for the Agentic Web](https://arxiv.org/abs/2604.02334)
...
TL;DR: Holos是一个面向长期生态持久性的web级LLM驱动多智能体系统架构，旨在解决开放世界中的扩展摩擦、协调崩溃和价值耗散问题，为自组织和持续演化的Agentic Web奠定基础。


<details>
  <summary>Details</summary>
Motivation: 随着LLM驱动的智能体从孤立任务求解器转变为持久数字实体，Agentic Web（异构智能体自主交互和共同演化的生态系统）的出现标志着向AGI的关键转变。然而，基于LLM的多智能体系统面临开放世界问题：扩展摩擦、协调崩溃和价值耗散。

Method: 提出Holos架构，采用五层设计，核心模块包括：1) Nuwa引擎用于高效智能体生成和托管；2) 市场驱动的Orchestrator实现弹性协调；3) 内生价值循环实现激励兼容。通过弥合微观协作与宏观涌现之间的差距。

Result: 已公开发布Holos系统（可通过https://holosai.io访问），为社区提供资源，并为大规模智能体生态系统的未来研究提供测试平台。

Conclusion: Holos为下一代自组织和持续演化的Agentic Web奠定了基础，通过其架构设计解决了LLM多智能体系统在开放世界中的核心挑战，有望推动大规模智能体生态系统的发展。
...
TL;DR: XpertBench是一个用于评估大语言模型在真实专业领域能力的高保真基准，包含1,346个任务覆盖80个类别，采用专家校准的ShotJudge评估范式，发现当前顶尖模型在专业任务上存在显著性能天花板（最高成功率约66%）。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架存在领域覆盖狭窄、依赖通用任务或自评估偏差等问题，无法有效评估LLMs在复杂、开放式专业任务上的真实能力。随着LLMs在传统基准上性能趋于平稳，需要开发能评估真实专家级认知能力的评估工具。

Method: 构建XpertBench基准：包含1,346个精心策划的任务，覆盖金融、医疗、法律服务、教育和双轨研究（STEM与人文）等80个类别；任务来自1,000多份领域专家提交；每个任务使用详细评分标准（15-40个加权检查点）。开发ShotJudge评估范式：使用LLM作为评估者，通过专家few-shot示例进行校准，减轻自奖励偏差。

Result: 对最先进LLMs的实证评估显示显著性能天花板：即使领先模型最高成功率仅约66%，平均得分约55%。模型表现出领域特异性差异：在定量推理与语言合成方面显示出非重叠优势。这些发现揭示了当前AI系统存在显著的"专家差距"。

Conclusion: XpertBench为从通用助手向专业协作伙伴的过渡提供了关键评估工具，揭示了当前LLMs在专业任务上的局限性，强调了开发更专业化AI系统的必要性。
...
TL;DR: 提出一种神经符号架构，通过提取网格中的对象级结构、使用神经先验从领域特定语言中提出候选变换，并通过跨示例一致性过滤假设，来改进抽象推理任务中的组合泛化能力。


<details>
  <summary>Details</summary>
Motivation: 纯神经架构缺乏可靠组合泛化能力，而纯符号系统难以处理感知基础问题。需要结合两者优势来解决抽象推理任务中的泛化挑战。

Method: 采用神经符号架构：1) 从网格中提取对象级结构；2) 使用神经先验从固定领域特定语言中提出候选变换；3) 通过跨示例一致性过滤假设。基于人类视觉抽象的单位模式构建组合推理框架，为大型语言模型增强对象表示和变换提议。

Result: 在ARC-AGI-2上，将基础LLM性能从16%提升到公开评估集的24.4%，与ARC Lang Solver结合后达到30.8%。展示了无需任务特定微调或强化学习即可改善泛化能力。

Conclusion: 分离感知、神经引导的变换提议和符号一致性过滤能改善泛化能力，减少对暴力搜索和基于采样的测试时扩展的依赖，为抽象推理任务提供了有效的神经符号解决方案。
...
TL;DR: 提出了一个四部分框架，用于在极端天气不确定性下优化电网投资组合，发现基于净现值排名的简单方法比复杂的基于模型的元启发式优化方法表现更好


<details>
  <summary>Details</summary>
Motivation: 电力公司面临需求激增、资产老化和极端天气威胁，需要大规模资本投资。现有资本规划框架可以扩展以解决不确定性下的多目标优化问题
...
Conclusion: 在电网投资组合优化中，简单的净现值排名方法优于复杂的基于模型的元启发式优化方法，这挑战了传统认为需要详细电网模型才能获得最优结果的假设
...
TL;DR: 提出一种基于可解释强化学习的方法，用于桥梁生命周期优化管理，使用斜决策树形式输出可理解的政策


<details>
  <summary>Details</summary>
Motivation: 新的国家桥梁清单规范强调使用构件级状态数据进行基于风险的桥梁管理，但四维概率阵列扩展了状态空间，给制定最优生命周期政策带来挑战

Method: 提出新的可解释强化学习方法，包括：(a)使用可微分软树模型作为行动者函数逼近器，(b)训练过程中的温度退火过程，(c)正则化配合剪枝规则限制政策复杂度

Result: 该方法能产生以确定性斜决策树形式表示的可解释生命周期政策，节点数量和深度合理，可直接被人类理解和审计，易于集成到现有桥梁管理系统中

Conclusion: 提出的可解释强化学习方法能有效处理构件级状态表示，为桥梁生命周期优化提供可理解和可实施的决策树形式政策
...
TL;DR: 提出基于知识图谱的神经符号架构，将能力问题转化为可执行的叙事计划，通过"计划-检索-生成"工作流实现可验证的故事生成，在Live Aid知识图谱上评估三种RAG策略的权衡。


<details>
  <summary>Details</summary>
Motivation: 非物质文化遗产保护面临集体记忆消退的挑战，现有大语言模型存在事实不准确（幻觉）问题，无法满足文化遗产应用中对真实性的核心要求。

Method: 提出基于知识图谱的神经符号架构，将能力问题（CQs）重新设计为运行时可执行的叙事计划，建立"计划-检索-生成"透明工作流，在Live Aid多模态知识图谱上评估三种RAG策略：纯符号KG-RAG、文本增强Hybrid-RAG和结构感知Graph-RAG。

Result: 实验揭示了符号检索的事实精确性、混合方法的上下文丰富性和基于图遍历的叙事连贯性之间的量化权衡，为设计个性化和可控的故事讲述系统提供了可行见解。

Conclusion: 该神经符号架构通过将能力问题转化为可执行叙事计划，实现了证据封闭和完全可审计的故事生成，为非物质文化遗产保护提供了可靠的技术方案。
...
TL;DR: 论文提出Debiasing-DPO方法，通过自监督训练减少LLMs对无关上下文信息的敏感性，在教育评估任务中实现84%的偏见减少和52%的预测准确性提升。


<details>
  <summary>Details</summary>
Motivation: LLMs在高风险决策中日益重要，但对无关上下文信息的敏感性可能引入有害偏见，特别是在教师教学质量评估等任务中，偏见评估会影响教师的专业发展和职业轨迹。

Method: 提出Debiasing-DPO方法：自监督训练方法，将仅基于查询生成的中立推理与基于查询加额外无关上下文生成的偏见推理配对。结合监督微调防止预测准确性损失。在Llama 3B/8B和Qwen 3B/7B Instruct模型上应用。

Result: 无关上下文信息可使模型预测在7分量表上偏移达1.48分；更大模型有时表现出更高敏感性；标准缓解方法效果有限；Debiasing-DPO平均减少偏见84%，提升预测准确性52%。

Conclusion: 对无关上下文的鲁棒性不是模型规模扩大的自然副产品；Debiasing-DPO方法能在提示预测任务中同时显著提升准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 音频-视觉大语言模型作为多模态感知的统一接口正在兴起，但对其工作机制缺乏理解。本研究旨在通过机制可解释性分析，探究音频和视觉特征如何在不同层中演化融合以产生最终文本输出。

Method: 采用机制可解释性方法，分析AVLLM不同层中音频和视觉特征的演化与融合过程；通过探测分析检查潜在音频信息的存在性；追溯训练过程以理解模态偏见的根源。

Result: 发现AVLLM在中间层编码了丰富的音频语义，但当音频与视觉冲突时，这些能力无法在最终文本生成中体现；有用的潜在音频信息存在，但深层融合层不成比例地偏向视觉表征，抑制音频线索；这种不平衡源于训练过程，AVLLM的音频行为与其视觉-语言基础模型高度匹配，表明对音频监督的额外对齐有限。

Conclusion: 研究揭示了AVLLM中存在根本性的模态偏见，为理解多模态LLM如何整合音频和视觉提供了新的机制性见解，表明当前模型在音频-视觉融合方面存在系统性不平衡。
...
TL;DR: 提出一种本体导向的知识图谱构建方法，通过内在-关系路由机制将属性分类到模块化模式中，实现可移植、可重用的模式设计，而非传统流水线代码嵌入的紧耦合方案。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱构建方法将结构决策嵌入流水线代码或临时提取关系，导致模式与构建过程紧密耦合，难以重用于下游本体级任务。需要一种从设计之初就面向本体分析、实体消歧、领域定制和LLM引导提取的模式设计方法。

Method: 采用本体导向方法，核心是内在-关系路由机制：将每个属性分类为内在属性或关系属性，并路由到相应的模式模块。该方法包括基于规则的清洗阶段从完整数据集中识别核心实体集，然后通过迭代路由将属性分配到模块化类别中。

Result: 在2026年1月Wikidata数据集上实例化：识别出3460万个实体的核心集，将属性分配到8个类别下的94个模块中。模式达到93.3%的类别覆盖率和98.0%的模块分配率。导出得到包含3400万个节点、6120万条边、38种关系类型的属性图。

Conclusion: 本体导向方法通过内在-关系路由产生了可移植、可重用的声明性模式，成功应用于五个独立于构建流水线的下游任务：本体结构分析、基准标注审计、实体消歧、领域定制和LLM引导提取，验证了其本体导向的主张。
...
TL;DR: GrandCode是一个多智能体强化学习系统，首次在实时编程竞赛中击败所有人类选手，包括传奇大师级选手，标志着AI在最具竞争力的编程任务上超越人类。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在编码领域取得进展，但竞争性编程仍是人类最后的堡垒之一。现有最佳AI系统（如Google的Gemini~3 Deep Think）在实时竞赛条件下仍不及最佳人类选手，这激发了开发能真正超越人类的多智能体系统的需求。

Method: 采用多智能体强化学习系统，包含假设提出、求解器、测试生成器、总结等多个智能体模块，通过后训练和在线测试时RL联合优化。引入专门设计的Agentic GRPO算法，处理多阶段智能体展开中的延迟奖励和严重的离策略漂移问题。

Result: 在最近三次Codeforces实时竞赛（第1087、1088、1089轮）中，GrandCode全部获得第一名，击败了所有人类参与者，包括传奇大师级选手。这是首个在实时编程竞赛中持续击败所有人类选手的AI系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有推理基准主要评估模型从固定前提集推导正确答案的能力，但低估了动态环境中密切相关的关键能力：在最小证据变化下的信念修订。需要测量模型在前提发生微小变化时是否能够适当调整结论。

Method: 引入DeltaLogic基准转换协议，将自然语言推理示例转换为短修订情节：首先在前提P下询问初始结论，然后应用最小编辑δ(P)，最后询问先前结论应保持稳定还是需要修订。从FOLIO和ProofWriter实例化DeltaLogic，使用受限标签评分评估小型因果语言模型。

Result: 在30个情节的Qwen评估子集上，更强的初始推理能力并不保证更强的修订行为：Qwen3-1.7B初始准确率0.667但修订准确率仅0.467，在应改变标签的情节中惯性上升至0.600；Qwen3-0.6B几乎完全弃权；Qwen3-4B保持相同惯性失败模式；Phi-4-mini-instruct表现更强但仍有显著弃权和控制不稳定性。

Conclusion: 固定前提下的逻辑能力并不暗示在局部证据编辑后能进行有纪律的信念修订。DeltaLogic针对的是与现有逻辑推理和信念更新基准互补的独特且实际重要的推理能力。
...
TL;DR: 本文提出了一种量化角色清晰度的方法来改善LLM驱动的多智能体系统中的角色一致性，通过构建角色分配矩阵和角色清晰度矩阵来量化智能体行为轨迹与角色描述的对齐程度，并作为正则化项进行轻量级微调。


<details>
  <summary>Details</summary>
Motivation: 在LLM驱动的多智能体系统中，智能体不遵守角色规范（未能遵守分配角色的职责和约束，可能导致智能体表现得像另一个角色）是一个主要的故障模式。为了解决这个问题，需要一种方法来量化和改善角色一致性。

Method: 首先构建角色分配矩阵S(φ)，其中s_ij(φ)表示第i个智能体的行为轨迹与第j个智能体角色描述的语义相似度。然后定义角色清晰度矩阵M(φ)=softmax(S(φ))-I，其中softmax(S(φ))是S(φ)的行向softmax，I是单位矩阵。M(φ)的Frobenius范数量化了智能体角色描述与行为轨迹的对齐程度。最后将角色清晰度矩阵作为正则化项在轻量级微调中使用，以提高角色一致性。
...
TL;DR: EMS框架通过可靠性感知的智能体调度，在多数共识达成时提前终止推理，减少32%的智能体调用


<details>
  <summary>Details</summary>
Motivation: 传统多数投票方法需要所有智能体完成推理才能聚合，导致计算开销大，因为一旦达成多数共识，许多响应就变得冗余
...
Conclusion: EMS通过将多智能体投票重新定义为可靠性感知的智能体调度问题，显著提高了推理效率，在多数共识达成时提前终止推理流程
...
TL;DR: 该研究评估了大型语言模型在经典规划问题中的推理能力，发现前沿模型能够接近理论最优性限制，即使在去除领域特定语义提示的情况下，也能通过算法模拟和几何记忆机制有效处理组合复杂性。


<details>
  <summary>Details</summary>
Motivation: 当前AI规划研究在LLM时代主要关注成功率而非规划效率，需要评估前沿模型是进行最优推理还是依赖简单启发式策略。研究旨在探究LLM在经典规划问题中的真实推理能力，特别是能否达到理论最优性。

Method: 研究采用Blocksworld领域（涉及标记积木塔的移动）和形式等价的广义Path-Star（P*）图任务。系统性地操纵问题深度（塔高度）、宽度（塔数量）和组合性（目标积木数量），比较LLM与传统满足型规划器（如LAMA）的性能。通过P*图任务隔离真实拓扑推理与语义先验。

Result: 推理增强的LLM在复杂多目标配置中显著优于传统满足型规划器。当搜索空间扩展时，经典搜索算法遇到瓶颈，而LLM能够以近乎完美的精度跟踪理论最优性限制，即使去除领域特定语义提示。LLM在P*图任务中表现出色，表明其能力不依赖语义先验。

Conclusion: LLM通过算法模拟（通过推理令牌执行）和几何记忆（将P*拓扑表示为可导航的全局几何结构）两种机制有效绕过指数级组合复杂性，展现出接近理论最优性的规划能力。这些发现挑战了传统规划方法的局限性，为LLM在复杂推理任务中的应用提供了新见解。
...
TL;DR: 研究发现大型推理模型存在"第一即最优"现象，提出RED框架通过优化首解和剪枝次解来提升效率


<details>
  <summary>Details</summary>
Motivation: 尽管大型推理模型在复杂推理任务上表现出色，但研究发现其探索的替代解决方案不仅次优，甚至可能有害，这挑战了现有的测试时间缩放定律，需要新的高效推理框架

Method: 提出RED自引导高效推理框架，包含两个组件：1) Refining First - 抑制首解中的错误森林增长；2) Discarding Subs - 通过双重一致性剪枝后续错误森林

Result: 在5个基准测试和6个骨干模型上的实验表明，RED优于8个竞争基线，性能提升高达19.0%，同时减少37.7%～70.4%的token消耗

Conclusion: 研究揭示了大型推理模型中"第一即最优"现象，提出的RED框架通过有效管理错误森林实现了高效推理，为未来推理模型设计提供了新方向
...
Result: 在两个互补基准测试中表现出色：在WideSearch-en上达到8.4%成功率，在BrowseComp-zh上达到52.9%准确率，同时实现3-5倍速度提升

Conclusion: InfoSeeker框架通过分层架构有效解决了大规模信息合成中的关键挑战，在效率和效果上都显著优于现有系统，为数据密集型智能体搜索提供了实用解决方案
...
TL;DR: Agentic-MME是一个面向多模态智能体能力的流程验证基准，包含418个真实世界任务，通过2000多个逐步检查点评估视觉扩展和知识扩展的协同能力，强调过程验证而非仅最终答案。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估存在三个主要缺陷：缺乏灵活工具集成、单独测试视觉和搜索工具、仅评估最终答案。这导致无法验证工具是否实际调用、正确应用或高效使用，无法全面评估多模态智能体的真实能力。

Method: 构建包含418个真实世界任务的基准，涵盖6个领域和3个难度级别。每个任务包含支持沙盒代码和API的统一评估框架，以及带有逐步检查点的人类参考轨迹。采用双轴评估：S轴（语义正确性）和V轴（视觉正确性）。通过审计细粒度中间状态而非仅最终答案实现流程级验证，并通过"过度思考"指标量化效率。

Result: 最佳模型Gemini3-pro在整体准确率上达到56.3%，但在Level-3任务上显著下降至23.0%，突显了真实世界多模态智能体问题解决的难度。基准包含超过2000个逐步检查点，每个任务平均需要10+人时的手动标注。

Conclusion: Agentic-MME填补了现有评估空白，提供了首个支持流程验证的多模态智能体能力基准。实验结果表明当前模型在复杂真实世界任务上仍有显著挑战，强调了过程级评估的重要性，为未来MLLM向主动智能体发展提供了关键评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 推动大规模教科书形式化的发展，从本科拓扑学和现有库内容重构扩展到研究生教材的完整独立形式化，展示AI在数学形式化验证方面的能力

Method: 使用3万个Claude 4.5 Opus智能体通过版本控制并行协作，在共享代码库上工作，实现多智能体软件工程

Result: 成功形式化了整本500多页的研究生级代数组合学教材，包含13万行Lean代码和5900个声明，推理成本与人类专家团队薪资相当或更低

Conclusion: 该研究展示了AI系统在大规模数学形式化方面的突破性能力，为未来教科书形式化提供了高效可行的技术路径，代码和蓝图网站已开源
...
TL;DR: Chart-RL：基于强化学习的视觉语言模型框架，通过反馈驱动的策略优化提升图表理解能力，在ChartQA任务中实现更高准确率和更低延迟
...
Method: 提出Chart-RL强化学习框架，结合策略优化技术和自适应奖励函数，通过反馈驱动的视觉感知和逻辑推理策略优化；集成参数高效微调技术LoRA，仅需单GPU配置

Result: 在ChartQAPro数据集上，RL微调的Qwen3-VL-4B-Instruct模型达到0.634准确率，超越8B参数基础模型的0.580准确率，同时推理延迟从31秒降至9秒

Conclusion: Chart-RL框架有效解决了VLMs在图表理解中的关键挑战，通过强化学习优化实现了参数效率、准确率和推理速度的同步提升，为复杂数据可视化理解提供了新方法
...
TL;DR: 本文提出松鼠生态学作为研究智能体AI控制、记忆和验证耦合的基准案例，建立了包含潜在动态、结构化记忆和观察者信念的层次部分可观测控制模型，并提出了三个可证伪的假设。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究通常将控制、记忆和验证分开研究，但真实世界的智能体需要同时应对这些挑战。松鼠生态学提供了一个理想的比较案例，因为其树栖运动、分散储藏和观众敏感缓存行为将这三者紧密耦合在一起。

Method: 采用明确的推理阶梯：经验观察→最小计算推断→AI设计猜想。引入最小层次部分可观测控制模型，包含潜在动态、结构化情景记忆、观察者信念状态、选项级动作和延迟验证信号。

Result: 提出了三个可证伪假设：H1-快速局部反馈加预测补偿提高隐藏动态变化下的鲁棒性；H2-为未来控制组织的记忆改善延迟检索；H3-动作-记忆循环内的验证器和观察者模型减少静默失败和信息泄漏。进一步推测角色分化的提议者/执行者/检查者/对抗者系统可减少相关错误。

Conclusion: 贡献在于提供了一个比较视角和基准议程：一个关于控制、记忆和可验证动作耦合的可证伪主张的严谨研究计划，将松鼠生态学作为智能体AI研究的基准案例。
