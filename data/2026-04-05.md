...
Method: 采用预训练/后训练范式：首先在100万野外视频上进行预训练，学习外观和几何的广泛先验；然后在高质量策划数据上进行后训练，增强表达性和保真度。模型采用编解码器架构，支持前馈推理。

Result: LCA能够泛化到各种发型、服装和人口统计特征，同时提供精确的细粒度面部表情和手指级关节控制，具有强身份保持能力。模型展现出对重新光照和宽松服装支持的涌现泛化能力，以及对风格化图像的零样本鲁棒性。

Conclusion: LCA首次展示了大规模3D化身建模的预训练/后训练范式，成功解决了保真度与泛化性的权衡，实现了高质量全身3D化身的世界级泛化，为3D化身建模开辟了新方向。
...
TL;DR: DOne框架通过解耦结构理解和元素渲染来解决VLMs在Design-to-Code中的"整体瓶颈"问题，在复杂布局数据集上显著提升视觉相似度和代码生成质量
...
Conclusion: DOne通过解耦结构理解和元素渲染有效解决了Design-to-Code中的整体瓶颈问题，在复杂布局场景下实现了更好的视觉保真度和代码生成质量
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在回答需要结合视觉理解和外部知识的查询时，往往难以识别最相关的视觉和文本证据。模型需要整合视觉线索与检索到的文本证据（这些证据通常有噪声或仅部分相关），同时定位图像中的细粒度视觉信息。

Method: LoT利用模型注意力模式估计哪些视觉区域和检索到的文本元素与查询相关，然后基于这些突出显示的证据生成答案。通过轻量级的提示级标记突出选定的线索，鼓励模型在生成过程中重新关注相关证据。

Result: 在多个基于知识的VQA基准测试中，相比零样本MLLMs取得了一致的改进。在视觉中心和幻觉导向的基准测试中，仅视觉证据突出显示就能在没有文本上下文的情况下提升模型性能，且无需额外训练或架构修改。

Conclusion: LoT是一个有效的推理时框架，通过注意力引导的证据突出显示机制，显著提升了预训练多模态大语言模型在知识密集型视觉问答任务中利用多模态证据的能力。
...
TL;DR: MedQwen是一个参数高效的医学视觉语言模型，通过谱路由混合专家系统结合理论推导的缩放规则，在保持基础架构不变的情况下实现接近全参数微调的性能，同时显著减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在医学影像领域缺乏鲁棒性，存在跨数据集干扰和数据机制敏感性。临床工作流程中数据和任务按顺序到达，传统持续训练会导致灾难性遗忘。需要开发参数高效的医学VLM来解决这些问题。

Method: 提出MedQwen模型，结合谱路由混合专家系统与理论推导的缩放规则。从预训练权重的非重叠奇异值分解段初始化专家，引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致性路由，保持基础架构不变。

Result: 在23个医学数据集上（涵盖视觉问答、报告生成、放射学分类和幻觉缓解），MedQwen实现强大可靠性能：零样本分类接近全参数微调，可训练参数减少339倍；顺序遗忘降至约5%，而强基线退化20-50%。

Conclusion: MedQwen通过参数高效的谱路由MoE设计，有效解决了医学VLM中的跨数据集干扰、数据机制敏感性和灾难性遗忘问题，为临床工作流程中的持续学习提供了实用解决方案。
...
TL;DR: 基于视觉Transformer的视频分析方法，通过不平衡感知训练，在扩展的733个美式足球擒抱视频数据集上实现危险擒抱检测，相比基线提升8%以上召回率


<details>
  <summary>Details</summary>
Motivation: 早期识别接触性运动中的危险动作能够及时干预并提高运动员安全，需要开发可靠的检测方法来识别美式足球训练中的危险擒抱动作

Method: 使用基于视觉Transformer的模型，采用不平衡感知训练策略，在扩展的733个单运动员-假人擒抱视频数据集上进行训练，每个视频都标注了标准化擒抱技术评估(SATT-3)的撞击区域组件

Result: 在交叉验证中获得了0.67的危险召回率和0.59的危险F1分数，相比先前在较小数据集上的基线(危险召回率0.58，危险F1 0.56)，危险召回率提升了超过8个百分点
...
<details>
  <summary>Details</summary>
Motivation: 解决现有姿态估计模型在蹦床体操这种极端人体姿态和非常规视角下的性能不足问题

Method: 从动作捕捉数据生成合成蹦床姿态数据集(STP)，通过参数化人体模型拟合噪声数据，生成多视角真实图像，用此数据微调ViTPose模型

Result: 在2D姿态估计上达到最先进水平，在3D姿态估计中MPJPE降低12.5mm（相对改进19.6%），显著缩小了常规姿态与极端姿态之间的性能差距

Conclusion: 合成数据微调能有效提升姿态估计模型在极端姿态场景下的性能，为处理非常规视角和复杂姿态提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: ViT的注意力分数通常非零，导致注意力图噪声大、扩散性强，限制了可解释性。需要量化注意力分数的统计显著性，去除噪声产生的虚假注意力。

Method: 采用统计学习中的自助法，通过对输入特征重采样生成注意力分数的基线分布，基于该分布估计注意力分数的显著性和后验概率，实现注意力正则化。

Result: 在自然图像和医学图像上，该方法能有效去除噪声产生的虚假注意力，显著提高注意力图的稀疏性和收缩性。通过模拟和真实数据集进行了定量评估。

Conclusion: 自助法可作为实用的正则化工具，当使用注意力分数作为ViT解释时，能够提高注意力的统计显著性和可解释性。
...
TL;DR: 研究发现CNN模型作为视觉系统模型的质量与其纹理感知与人类对齐程度之间没有关联，表明纹理感知机制不同于基于物体识别训练的CNN常用建模方法。
...
TL;DR: AffordTissue：用于胆囊切除术中预测工具-动作特定组织可操作区域的密集热图多模态框架，通过时空视觉编码、语言条件化和DiT解码器实现，在首个组织可操作性基准上显著优于视觉语言模型基线。


<details>
  <summary>Details</summary>
Motivation: 当前手术动作自动化方法在临床部署中存在局限性：缺乏对器械在组织表面交互位置的预测能力，缺少强制执行工具-动作特定安全交互区域的显式条件输入。需要一种能够预测工具-动作特定组织可操作区域的方法，为安全手术自动化提供空间推理。

Method: 提出AffordTissue多模态框架：1）时空视觉编码器捕捉多视角下的工具运动和组织动态；2）语言条件化实现不同器械-动作对的泛化；3）DiT风格解码器进行密集可操作性预测。构建首个组织可操作性基准数据集，包含103个胆囊切除术中的15,638个视频片段，涵盖4种器械（钩、抓钳、剪刀、夹钳）和6种工具-动作对。

Result: 实验显示显著优于视觉语言模型基线：平均对称表面距离（ASSD）为20.6像素 vs. Molmo-VLM的60.2像素。任务特定架构在密集手术可操作性预测方面优于大规模基础模型，能够准确预测工具-动作特定组织可操作区域。

Conclusion: AffordTissue通过预测工具-动作特定组织可操作区域，为安全手术自动化提供了显式空间推理，有望实现针对适当组织区域的显式策略指导，并在器械偏离预测安全区域时提供早期安全停止机制。
...
TL;DR: GRAZE是一个无需训练的框架，用于在美式足球训练视频中定位首次接触点(FPOC)，通过目标检测、运动感知时间推理和像素级接触验证实现鲁棒的时空定位。


<details>
  <summary>Details</summary>
Motivation: 美式足球训练产生大量视频，但关键交互只占长视频中的短暂窗口。可靠的生物力学分析需要精确的时空定位来识别交互实体和接触起始点。现有方法面临相机运动、杂乱场景、多个相似装备运动员以及冲击时快速姿态变化等挑战。

Method: GRAZE采用无训练流水线：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知时间推理进行细化；3) 使用SAM2作为显式像素级接触验证器，不依赖检测置信度。候选发现与接触确认的分离使方法对杂乱场景和冲击附近的不稳定定位具有鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的视频中产生有效输出，在77.5%的视频中将FPOC定位在±10帧内，在82.7%的视频中定位在±20帧内。结果表明无需任务特定训练即可在真实训练视频中实现帧级准确的接触起始定位。

Conclusion: GRAZE证明了在无约束美式足球训练视频中实现首次接触点帧级准确定位的可行性，无需标记的擒抱接触示例或任务特定训练。分离候选发现和接触确认的方法对杂乱场景和冲击附近的不稳定定位具有鲁棒性。
...
TL;DR: 提出SVRaster框架解决3DGS在开放词汇3D场景理解中的空间模糊性和多级语义模糊性问题，通过结构化几何表示和确定性特征注册实现最先进性能
...
Method: 提出SVRaster框架：1) 使用稀疏体素光栅化作为结构化、不相交的几何表示；2) 通过单目深度和法线先验正则化SVRaster建立稳定几何基础；3) 实现确定性、置信感知的特征注册过程；4) 利用基础模型AM-RADIO的密集对齐特性解决多级语义模糊性，避免分层训练的计算开销

Result: 在开放词汇3D对象检索和点云理解基准测试中达到最先进性能，特别是在细粒度查询任务上表现优异，而传统注册方法通常在这些任务上失败

Conclusion: SVRaster框架通过结构化几何表示解决了3DGS在开放词汇3D场景理解中的关键限制，实现了更精确的特征注册和细粒度语义理解，为3D场景理解提供了更稳健的解决方案
...
TL;DR: EgoFlow：基于流匹配的框架，从多模态第一人称视角观测生成物理一致的6DoF物体轨迹，通过混合Mamba-Transformer-Perceiver架构和梯度引导推理实现物理约束，在多个数据集上超越现有方法。
...
Result: 在HD-EPIC、EgoExo4D和HOT3D数据集上实验表明，EgoFlow在准确性、泛化性和物理真实性方面超越基于扩散和Transformer的基线方法，碰撞率降低达79%，对未见场景有强泛化能力。

Conclusion: 流匹配生成模型为可扩展和物理基础的自我中心运动理解提供了前景，EgoFlow无需后处理滤波或额外监督即可生成连贯可控的运动轨迹。
...
TL;DR: 提出一个通用的人体姿态数据分析流程，用于从复杂的姿态时间序列中提取有意义的运动模式，适用于面部和全身运动、2D/3D数据以及个体与多智能体行为分析。
...
Method: 提出一个通用分析流程，结合原则性预处理、降维和基于递归的时间序列分析，量化运动动态的时间结构，支持线性和非线性特征描述。
...
TL;DR: UniRecGen是一个统一框架，将前馈重建和扩散生成相结合，通过共享规范空间和分离式协作学习，从稀疏观测中创建完整一致的3D模型。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模面临重建保真度与生成合理性之间的基本矛盾：前馈重建方法效率高且输入对齐好，但缺乏全局先验导致结构不完整；而基于扩散的生成能提供丰富几何细节，但多视图一致性差。

Method: 提出UniRecGen统一框架，将重建和生成两种范式整合到单一协作系统中。通过将两种模型对齐到共享规范空间，采用分离式协作学习保持训练稳定性，在推理时实现无缝协作。重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和补全几何结构。

Result: 实验结果表明，UniRecGen在保真度和鲁棒性方面表现优异，在从稀疏观测创建完整一致的3D模型方面超越了现有方法。

Conclusion: UniRecGen成功解决了稀疏视图3D建模中重建与生成之间的固有冲突，通过统一框架实现了更好的结构完整性和多视图一致性。
...
TL;DR: 提出TAG热成像框架，通过非参数纹理恢复解决材料非均匀性问题，实现高保真夜视并超越现有HADAR技术


<details>
  <summary>Details</summary>
Motivation: 传统热成像存在鬼影效应，现有HADAR技术仅适用于均匀材料场景，而真实世界中材料非均匀性普遍存在，需要通用抗鬼影成像方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在动态区域初始化不完整，导致重建和运动估计不稳定，通常需要依赖外部预计算的光流等密集运动指导，这增加了复杂性和错误传播风险。
...
TL;DR: VideoZeroBench：用于长视频问答的分层基准测试，通过时空证据验证严格评估模型能力，揭示当前模型在基于证据的推理方面存在显著不足


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型的评估存在两个关键局限：(1) 夸大的分数掩盖了细粒度视觉理解和推理的缺陷；(2) 答案正确性评估通常不验证模型是否识别支持其预测的精确时空证据。需要更严格的基准来评估基于证据的视频理解能力。

Method: 提出VideoZeroBench分层基准，包含500个手动标注的问题，涵盖13个领域，每个问题配有时间区间和空间边界框作为证据。引入五级评估协议，逐步收紧证据要求，分离答案生成、时间定位和空间定位能力。

Result: 实验显示，即使在标准端到端QA设置下（Level-3），Gemini-3-Pro的正确回答率也低于17%。当施加定位约束时，性能急剧下降：在要求正确回答和准确时空定位时（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。

Conclusion: 结果揭示了表面答案正确性与真正基于证据的推理之间存在显著差距，表明基于证据的视频理解仍然是长视频QA的瓶颈。基准测试为未来基于证据的视频推理研究提供了重要见解。
...
Conclusion: GAAL范式通过梯度对齐有效解决多模态融合中的梯度冲突问题，提供有效的单模态辅助，提升整体融合性能。
...
TL;DR: 提出卫星图像免费训练框架SFT，通过无人机多视角序列进行三维重建生成伪正射影像，实现无需卫星数据训练的无人机视角地理定位。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角地理定位方法依赖卫星图像进行训练，但在卫星数据不可用或受限的实际部署场景中存在局限性。需要开发不依赖卫星图像的训练方法。

Method: 三阶段框架：1) 使用3D高斯溅射从多视角无人机图像重建密集三维场景；2) 通过PCA引导的正射投影生成几何归一化的伪正射影像，并用轻量级几何引导修复完善纹理；3) 从生成的伪正射影像提取DINOv3特征，仅使用无人机数据学习Fisher向量聚合模型，测试时复用该模型编码卫星瓦片进行跨视角检索。

Result: 在University-1652和SUES-200数据集上的实验表明，SFT框架显著优于卫星免费泛化基线方法，并缩小了与使用卫星图像训练方法的性能差距。

Conclusion: 提出的卫星免费训练框架通过无人机多视角序列的三维重建和伪正射影像生成，实现了不依赖卫星数据的无人机视角地理定位，为实际部署提供了可行的解决方案。
...
TL;DR: SHOE是一个新的开放词汇人-物交互检测评估框架，通过语义相似度而非精确字符串匹配来评估预测结果，使用LLM计算动词和对象组件的语义相似度，比传统mAP指标更符合人类判断。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标（如mAP）将HOI类别视为离散分类标签，无法对语义有效但词汇不同的预测给予适当评价（如"lean on couch" vs. "sit on couch"），限制了开放词汇预测的评估能力。

Method: SHOE将每个HOI预测分解为动词和对象组件，使用多个大型语言模型（LLMs）的平均值估计它们的语义相似度，并将这些相似度组合成一个综合评分，以评估超出精确字符串匹配的对齐程度。

Result: SHOE评分比现有指标（包括基于LLM和嵌入的基线）更接近人类判断，与平均人类评分的吻合度达到85.73%。该框架能够在标准基准（如HICO-DET）上灵活评估现有HOI检测方法和开放生成模型。
...
<details>
  <summary>Details</summary>
Motivation: 开集测试时适应（OSTTA）面临协变量偏移ID样本（csID）和协变量偏移OOD样本（csOOD）同时存在的挑战。现有方法结合熵最小化（保持ID性能）和熵最大化（增强OOD检测）存在内在冲突，导致csID分类和csOOD检测之间的权衡问题。

Method: 提出ROSETTA方法：1）分析熵最大化在OSTTA中的局限性；2）引入角度损失调节特征范数大小；3）设计特征范数损失抑制csOOD的logits。这些目标共同提升OOD检测能力。

Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C上实现了强大的OOD检测，同时保持高ID分类性能。在Cityscapes数据集上验证了真实世界语义分割的有效性，在HAC数据集上展示了不同开集TTA设置下的适用性。

Conclusion: ROSETTA通过角度损失和特征范数损失有效解决了熵最小化与最大化之间的冲突，在开集测试时适应场景中实现了ID分类性能和OOD检测能力的平衡，具有实际应用价值。
...
TL;DR: F3DGS是一个联邦3D高斯溅射框架，用于去中心化的多智能体3D重建，通过共享几何骨架和可见性感知聚合解决分布式系统中的几何一致性和部分可观测性问题。
...
Method: 首先通过注册多个客户端本地合并的LiDAR点云构建共享几何骨架来初始化全局3DGS模型。联邦优化期间固定高斯位置以保持几何对齐，每个客户端仅更新外观相关属性（协方差、不透明度和球谐系数）。服务器使用可见性感知聚合更新，根据每个客户端观察每个高斯的频率加权贡献。

Result: 实验表明F3DGS在实现与集中训练相当的重建质量的同时，支持跨智能体的分布式优化。收集了包含同步LiDAR、RGB和IMU测量的多序列室内数据集进行评估。

Conclusion: F3DGS为分布式多智能体3D重建提供了有效的联邦学习框架，解决了现有集中式方法的局限性，数据集、开发工具包和源代码将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 形态特征是理解生物与环境相互作用的关键，但传统人工标注方法效率低下，限制了大规模生态研究。主要瓶颈在于缺乏高质量的生物图像与特征级标注数据集。

Method: 1) 在基础模型特征上训练稀疏自编码器，获得单语义、空间定位的神经元；2) 利用这些神经元定位显著区域；3) 通过视觉-语言提示生成可解释的特征描述；4) 构建包含8万标注的BIOSCAN-Traits数据集。

Result: 1) 创建了包含8万特征标注的BIOSCAN-Traits数据集，涵盖1.9万张昆虫图像；2) 人类评估确认了生成形态描述的生物合理性；3) 通过消融研究系统评估了设计选择对标注质量的影响。

Conclusion: 该研究提供了一种可扩展的形态特征标注方法，替代了昂贵的人工标注，为大规模形态分析提供了可能，并弥合了生态相关性与机器学习实用性之间的差距。
...
Method: 1. 渐进构建全局一致运动场；2. 引入几何感知对齐模块解决视图间的方向和尺度模糊；3. 使用基于哈希的紧凑运动场表示，支持高效查询和稳定动态传播；4. 支持渲染时的双向运动传播，无需昂贵视频细化。

Result: 在单张RTX 5090 GPU上，每个新场景扩展步骤生成需9秒，运动对齐和运动场更新需3秒，实现了具有全局一致环境动态的交互式4D世界生成。

Conclusion: LivingWorld框架成功解决了从单张图像生成4D世界中环境动态的挑战，通过渐进运动场构建和紧凑表示实现了交互式生成，为动态场景建模提供了新方向。
...
TL;DR: 提出用于稀疏标注单目3D目标检测的新框架，包含道路感知补丁增强和原型过滤两个核心模块，通过几何保持增强和原型引导伪标注解决稀疏监督问题
...
Result: 广泛的实验证明了所提方法的有效性。该方法在稀疏监督下实现了鲁棒的检测性能。

Conclusion: 提出的框架通过几何保持增强和原型引导伪标注相结合的训练策略，有效解决了稀疏标注单目3D目标检测问题，为现实场景中的稀疏监督检测提供了实用解决方案。
...
TL;DR: 提出基于莫尔效应的物理认证签名，利用真实相机自然产生但生成模型无法忠实复现的莫尔运动不变量来区分真实视频与AI生成视频。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI合成内容越来越难以与真实视频区分，需要开发可靠的物理基础认证方法来检测AI生成视频。

Method: 利用莫尔效应：当相机观察紧凑双层光栅结构时形成的干涉条纹。推导莫尔运动不变量，证明条纹相位与光栅图像位移通过光学几何线性耦合，与观察距离和光栅结构无关。验证器从视频中提取两种信号并测试其相关性。

Result: 在真实拍摄和多个最先进生成器产生的AI生成视频上验证不变量，发现真实视频和AI生成视频产生显著不同的相关特征，表明该方法能有效区分两者。

Conclusion: 确定性光学现象可作为物理基础的可验证签名来对抗AI生成视频，为视频认证提供了新的物理基础方法。
...
TL;DR: DynaVid：利用合成运动数据训练视频扩散模型，通过两阶段生成框架（运动生成+运动引导视频生成）解决动态运动合成和精细运动控制的难题


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在合成高度动态运动或需要精细运动控制的视频时仍存在困难，主要原因是常用训练数据集中此类示例稀缺。需要一种方法既能获得多样运动模式和精确控制信号，又能避免合成视频的不自然外观。

Method: 提出DynaVid框架，利用计算机图形管道渲染的合成运动数据（表示为光流）进行训练。采用两阶段生成：1）运动生成器合成运动；2）运动引导的视频生成器基于该运动生成视频帧。光流仅编码运动，与外观解耦，避免模型学习合成视频的不自然外观。

Result: 在剧烈人体运动生成和极端相机运动控制这两个具有挑战性的场景中进行了验证，现有数据集在这些场景中特别有限。大量实验表明，DynaVid在动态运动生成和相机运动控制方面提高了真实感和可控性。

Conclusion: 通过利用合成运动数据并采用解耦的两阶段生成框架，DynaVid能够从合成数据中学习动态运动模式，同时从真实世界视频中保持视觉真实感，有效解决了视频扩散模型在动态运动合成和精细控制方面的局限性。
...
TL;DR: 提出一种无需域标识和样本记忆的增量学习框架，用于具身多媒体系统在开放物理空间中的连续环境适应，通过解耦表示机制和权重融合策略解决分布漂移和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 具身感知系统在开放物理空间连续交互时面临动态环境分布漂移的挑战。现有域增量感知方法通常依赖测试阶段预先获取的域标识，限制了在未知交互场景中的实用性。同时，模型容易过拟合到特定上下文感知噪声，导致泛化能力不足和灾难性遗忘。

Method: 提出域标识和样本记忆免费的增量学习框架：1）设计解耦表示机制，去除非必要的环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征；2）采用权重融合策略，在参数空间动态整合新旧环境知识，无需存储历史数据，在适应新分布的同时最大限度保留旧环境的判别能力。

Result: 在多个标准基准数据集上的大量实验表明，该方法在完全无样本记忆和无域标识的设置下显著减少了灾难性遗忘，其准确率优于现有最先进方法。

Conclusion: 该方法为具身多媒体系统提供了鲁棒的连续环境适应能力，通过解耦表示和参数空间知识融合，有效解决了开放物理空间中动态分布漂移带来的挑战，实现了无需域标识和样本记忆的增量学习。
...
TL;DR: 该论文提出频率域适应(FDA)和谐波约束最优传输(HOT)框架，用于解决远程光电容积描记术(rPPG)在跨域应用中的性能下降问题，通过转移低频光谱成分来学习对外观变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 远程光电容积描记术(rPPG)虽然能够从面部视频进行非接触式生理测量，但在实际部署中常因域偏移导致性能显著下降。现有深度学习方法虽然在单个数据集上表现良好，但容易过拟合于光照、相机特性、颜色响应等外观相关因素，这些因素在不同域间变化很大。

Method: 提出频率域适应(FDA)作为建模rPPG外观变化的原理性策略，通过转移编码域依赖外观特征的低频光谱成分，使rPPG模型学习对外观变化的不变性，同时保留心脏诱导信号。进一步提出谐波约束最优传输(HOT)，利用心脏信号的谐波特性来指导原始表示和FDA转移表示之间的对齐。

Result: 广泛的跨数据集实验表明，提出的FDA和HOT框架有效增强了rPPG模型在不同数据集间的鲁棒性和泛化能力。

Conclusion: 频率域适应和谐波约束最优传输为解决rPPG跨域性能下降问题提供了有效框架，通过建模外观变化和利用心脏信号的谐波特性，显著提升了模型的鲁棒性和泛化性能。
...
Method: 1) 使用时序对齐的实例掩码和MLLM生成的句子嵌入监督高斯语义特征；2) 通过2D光流桥接4D高斯并微调运动；3) 引入几何感知SDF约束和正则化项增强表面连续性。

Result: 实验表明Director实现了时序一致的4D重建，同时支持实例分割和开放词汇查询，在动态场景理解方面表现出色。

Conclusion: 嵌入实例一致性语义自然地补充了4D建模，实现了更准确的场景分解和鲁棒的动态场景理解，为联合建模外观、几何和语义提供了统一框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的视频对象移除方法难以消除对象引起的副作用（如阴影、反射、光照变化），同时保持整体连贯性。这源于对目标对象及其与场景交互的物理和语义理解不足。

Method: 1. 外部：提出知识蒸馏方案，将视觉基础模型中对象与其诱导效应之间的关系转移到视频扩散模型中；2. 内部：提出帧间上下文交叉注意力机制，使每个去噪块基于目标区域周围的信息化未掩码上下文进行推理。

Result: 实验证明该方法达到最先进性能，并建立了首个真实世界视频对象移除基准，促进未来研究和社区发展。代码、数据和模型已开源。

Conclusion: 外部和内部引导共同使模型能够理解目标对象、其诱导效应和全局背景上下文，实现清晰连贯的对象移除，解决了现有方法的局限性。
...
Method: 采用两步处理：1）基于个体注视注意力热图和群体推断中估计的群体成员标量生成共享注意力热图；2）考虑初始共享注意力热图细化初始群体成员关系，并最终预测共享注意力热图。
...
Method: 提出SteerFlow框架，包含三个核心组件：1) 摊销定点求解器，通过强制连续时间步的速度一致性隐式拉直前向轨迹；2) 轨迹插值，自适应混合目标编辑和源重建速度；3) 自适应掩码机制，通过概念引导分割和速度差异空间约束编辑信号。

Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的实验表明，SteerFlow相比现有方法在编辑质量上表现更优，并能自然扩展到多轮编辑范式而不累积漂移。

Conclusion: SteerFlow为基于流的生成模型提供了具有理论保证的模型无关编辑框架，在保持源图像保真度的同时实现高质量文本引导编辑，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 密集人群实例分割在监控、交通等领域有广泛应用，但现有数据集多为点标注，区域标注（如边界框）稀缺且不准确。直接应用SAM等基础模型在密集人群中效果不佳，需要专门的方法来处理点标注并生成准确的实例分割。

Method: 1. 提出Dense Point-to-Mask Optimization (DPMO)：结合SAM与最近邻互斥圆约束，从点标注生成密集实例分割掩码；2. 通过DPMO和人工校正，将现有数据集的点标注转换为掩码标注；3. 提出Reinforced Point Selection (RPS)框架，使用Group Relative Policy Optimization (GRPO)训练，从初始点预测采样中选择最佳预测点；4. 设计基于掩码监督的新损失函数。

Result: 在ShanghaiTech、UCF-QNRF、JHU-CROWD++和NWPU-Crowd数据集上实现了最先进的密集人群实例分割性能。新设计的掩码监督损失函数在不同模型上均提升了计数性能，证明了掩码标注对提升计数精度的重要作用。

Conclusion: DPMO方法能有效将点标注转换为掩码标注，RPS框架在密集人群实例分割任务中表现出色。掩码标注不仅改善了分割质量，还能显著提升人群计数精度，为密集人群分析提供了更准确的标注和监督信号。
...
TL;DR: 提出几何感知的无人机跨视角地理定位框架，通过3D场景重建和虚拟鸟瞰图渲染统一粗粒度地点识别与细粒度姿态估计


<details>
  <summary>Details</summary>
Motivation: 现有方法将透视畸变视为外观噪声而非显式几何变换，采用解耦的检索-姿态估计流程，难以处理无人机倾斜视角与卫星正交地图间的严重几何差异
...
Conclusion: 通过显式建模3D场景几何，将透视畸变转化为几何变换，统一了跨视角地理定位的检索和姿态估计，为GNSS拒止环境下的无人机定位提供了有效解决方案
...
TL;DR: 本文提出了Ultrasound-CLIP框架，通过构建大规模超声图像-文本数据集US-365K和超声诊断分类学(UDT)，结合语义感知对比学习和异构图推理，显著提升了超声影像的跨模态理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言预训练模型（如CLIP）主要针对其他模态设计，难以直接应用于超声数据，因为超声图像具有异质性解剖结构和多样化诊断属性。需要专门针对超声影像的跨模态理解框架。

Method: 1. 构建US-365K大规模超声图像-文本数据集（36.5万配对样本，52个解剖类别）；2. 建立超声诊断分类学(UDT)，包含超声分层解剖分类学和超声诊断属性框架；3. 提出Ultrasound-CLIP框架，引入语义软标签和语义损失进行样本细粒度区分；4. 基于UDT文本表示构建异构图模态，实现病变-属性关系的结构化推理。

Result: 在患者级数据划分的实验表明，该方法在分类和检索基准上达到最先进性能，同时在零样本学习、线性探测和微调任务上表现出强大的泛化能力。

Conclusion: 通过构建专门的大规模超声数据集和结构化知识框架，结合语义感知对比学习，Ultrasound-CLIP显著提升了超声影像的跨模态理解能力，为超声诊断的自动化分析提供了有效解决方案。
...
TL;DR: 提出一种轻量级架构和训练策略，利用自监督学习特征作为预训练视频扩散模型的通用条件信号，实现视频域迁移和3D到视频生成，通过解耦外观与其他特征实现鲁棒控制。
...
Result: 能够实现视频域迁移（如风格化、重光照）和从3D到视频的生成任务。该方法在保持几何和语义信息的同时，实现了对外观变化的鲁棒控制。

Conclusion: 自监督学习特征可以作为预训练视频扩散模型的通用条件信号，通过适当的架构设计实现特征解耦，显著提升视频生成任务的可控性和灵活性。
...
TL;DR: 论文提出了RebusBench基准测试，用于评估大视觉语言模型在解决需要神经符号推理的谜语谜题方面的能力，发现当前模型在此类任务上表现严重不足。


<details>
  <summary>Details</summary>
Motivation: 当前大视觉语言模型在显式视觉识别方面表现出色，但在需要复杂多步推理的任务中存在认知鸿沟。模型难以处理视觉输入仅作为线索而非答案的情况，特别是在解决需要整合感知和先验知识的谜语谜题时。

Method: 提出了RebusBench基准测试，包含1,164个谜语谜题，专门用于评估模型整合视觉感知和语言知识的神经符号推理能力。评估了包括Qwen、InternVL和LLaVA在内的先进模型，并测试了模型缩放和上下文学习的影响。

Result: 当前最先进模型在RebusBench上表现严重不足：精确匹配率低于10%，语义准确率低于20%。模型缩放和上下文学习均未带来显著改进，表明模型缺乏连接视觉和语言组件的认知推理能力。

Conclusion: 大视觉语言模型虽然具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理"胶水"。解决谜语谜题需要神经符号推理能力，这是当前模型架构的一个关键缺陷。
...
TL;DR: DriveDreamer-Policy：一个统一的驾驶世界-动作模型，将深度生成、未来视频生成和运动规划集成到单一模块化架构中，通过几何感知的世界表示提升自动驾驶的规划鲁棒性和未来预测质量。


<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型（WAM）主要关注2D外观或潜在表示建模，缺乏对物理世界操作至关重要的几何基础。自动驾驶系统需要几何感知的世界表示来产生连贯的未来想象和明智的驾驶动作。

Method: 使用大型语言模型处理语言指令、多视角图像和动作，然后通过三个轻量级生成器分别产生深度、未来视频和动作。模型学习几何感知的世界表示，并在统一框架内指导未来预测和规划，保持模块化和可控延迟。
...
Conclusion: DriveDreamer-Policy通过集成几何感知的世界表示，在统一框架中实现了更连贯的未来想象和更明智的驾驶动作，证明了显式深度学习对自动驾驶世界模型的重要性，为具身系统提供了更强大的基础。
...
TL;DR: FSKD：基于知识蒸馏的LiDAR到RGBI框架，使用多模态教师融合LiDAR与RGBI数据，训练仅需RGBI的SegFormer学生模型，实现零样本高精度森林结构监测


<details>
  <summary>Details</summary>
Motivation: 机载LiDAR成本高、采集频率低，但却是森林结构监测（如CHM、PAI、FHD）的金标准。需要开发仅使用低成本RGBI图像就能获得高分辨率森林结构数据的方法。

Method: 提出FSKD知识蒸馏框架：多模态教师模型通过交叉注意力融合RGBI图像与LiDAR衍生的平面指标和垂直剖面；RGBI-only的SegFormer学生模型学习复现这些输出。在德国萨克森州384 km²森林数据上训练，使用20 cm地面采样距离。

Result: 学生模型在8个地理分布不同的测试区域实现SOTA零样本CHM性能：MedAE 4.17 m，R²=0.51，IoU 0.87；MAE比HRCHM/DAC基线提升29-46%（5.81 m vs. 8.14-10.84 m），相关系数更强（0.713 vs. 0.166-0.652）。多模态融合比仅RGBI训练提升10-26%性能。

Conclusion: FSKD框架能够仅使用RGBI图像联合预测CHM、PAI和FHD，突破了现有单目CHM估计器的局限。方法在时间不匹配（冬季LiDAR、夏季RGBI）下仍有效，消除了严格同步采集限制，为德国数字孪生和国家数字正射影像计划等提供了可扩展的20 cm操作监测方案。
...
TL;DR: 提出GardenDesigner框架，通过编码江南园林美学原则和基于程序化建模的智能体链，实现江南园林的自动生成与交互式编辑。


<details>
  <summary>Details</summary>
Motivation: 江南园林作为重要的数字资产，在影视游戏制作和数字旅游中具有巨大潜力，但传统手工建模依赖专家经验且耗时耗力，需要自动化解决方案。

Method: 提出GardenDesigner框架：1) 编码江南园林建造的美学原则；2) 基于程序化建模构建智能体链，包括地形分布、道路生成、资产选择和布局优化智能体；3) 引入GardenVerse知识库增强资产布置；4) 在Unity中开发交互界面，支持文本输入快速生成。

Result: 实验和人工评估表明，GardenDesigner能够生成多样且美观的江南园林，非专业用户可通过文本输入在1分钟内完成园林构建。

Conclusion: GardenDesigner通过编码美学原则和智能体链，实现了江南园林的高效自动生成，为数字内容创作提供了实用工具。
...
TL;DR: 提出一种利用轮式里程计实现时序一致性的单目深度估计框架，通过光流三角化估计相机位姿和稀疏深度，使用贝叶斯递归估计度量尺度，对预训练深度估计基础模型的相对深度进行重标定。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在连续帧间缺乏时序一致性，导致深度估计抖动，且在深度范围突变时容易失败。这些问题在自动驾驶和移动机器人感知系统中尤为关键，需要稳定连贯的深度预测。

Method: 1) 利用连续帧间的光流进行三角化，估计相机位姿和稀疏深度；2) 使用稀疏深度更新度量尺度的递归贝叶斯估计；3) 将估计的度量尺度应用于预训练深度估计基础模型输出的相对深度，进行重标定；4) 结合轮式里程计信息增强时序一致性。

Result: 在KITTI、TartanAir、MS2和自建数据集上评估，展示了鲁棒且准确的深度估计性能，特别是在保持时序一致性方面表现优异。

Conclusion: 提出的基于轮式里程计的一致性感知单目深度估计框架有效解决了时序一致性问题，通过融合稀疏深度估计和度量尺度标定，实现了稳定连贯的深度预测，适用于自动驾驶和移动机器人应用。
...
TL;DR: STRIVE是一个用于视频问答的时空强化学习框架，通过构建多个视频变体并进行联合归一化来解决奖励方差低的问题，提升策略更新的稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于群体的策略优化方法在视频问答中面临奖励方差低的问题，当多个回答具有相似正确性时，会导致优势估计弱或不稳定，限制了强化学习在视频推理任务中的效果。

Method: STRIVE通过构建输入视频的多个时空变体，在文本生成和视觉变体上进行联合归一化；引入重要性感知采样机制，优先选择与问题最相关的帧同时保持时间覆盖；通过结构化视觉扰动扩展群体比较，丰富奖励信号。

Result: 在六个具有挑战性的视频推理基准测试（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench、PerceptionTest）上，相比现有强化学习基线方法，在多个大型多模态模型上都取得了持续改进。

Conclusion: 结构化时空探索是稳定多模态强化学习和提升视频推理性能的有效机制，STRIVE框架通过丰富奖励信号和促进稳定策略更新，为视频问答提供了更稳健的推理能力。
...
TL;DR: SafeRoPE：针对MMDiT的轻量级安全生成框架，通过分析注意力机制发现不安全语义集中在可解释的低维子空间，提出基于RoPE扰动和潜在风险评分的头级安全抑制方法


<details>
  <summary>Details</summary>
Motivation: 基于整流流变换器的T2I模型（如SD3、FLUX）虽然生成保真度高，但对不安全语义（特别是多令牌交互触发）仍然脆弱。现有缓解方法依赖微调或注意力调制，计算开销大且针对U-Net设计，难以直接适配变换器扩散模型（如MMDiT）

Method: 1. 分析MMDiT注意力机制，发现不安全语义集中在头级的可解释低维子空间；2. 构建头级不安全子空间，计算输入向量在子空间投影的潜在风险评分；3. 引入头级RoPE扰动，对查询和关键向量嵌入执行风险特定的头级旋转，精确抑制不安全输出

Result: SafeRoPE在平衡有害内容缓解和效用保持方面达到SOTA性能，有效抑制不安全语义生成同时保持生成保真度，计算开销小且无需微调

Conclusion: 通过分析MMDiT注意力机制发现不安全语义的集中特性，提出基于RoPE扰动的轻量级安全生成框架SafeRoPE，实现了对变换器扩散模型的高效安全防护
...
TL;DR: 提出随机标签桥接训练方法，无需人工标注即可将大语言模型参数适配到视觉基础任务，发现部分桥接训练效果更佳


<details>
  <summary>Details</summary>
Motivation: 语言预训练模型和视觉预训练模型的异常参数比例差异显著，跨模态（语言和视觉）比跨域适应更具挑战性。先前研究多关注跨域迁移而非跨模态桥接，认为语言预训练模型不适合下游视觉任务。本文挑战这一假设，证明通过桥接训练可以对齐LLM参数与视觉任务。

Method: 提出随机标签桥接训练方法：1）添加桥接训练阶段作为模态适应学习器；2）无需人工标注；3）帮助LLM参数适应视觉基础任务；4）采用部分桥接训练策略，保留LLM某些层的强基础特性。

Result: 证明桥接训练能有效对齐LLM参数与视觉任务，部分桥接训练通常更有利，因为LLM某些层具有强基础特性，即使不针对视觉任务微调也保持有益。

Conclusion: 为直接利用语言预训练参数到视觉模型开辟新途径，强调部分桥接训练作为跨模态适应的实用路径的潜力，挑战了语言预训练模型不适合视觉任务的假设。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中存在域偏移问题，现有半监督域自适应方法在处理严重程度分类时面临挑战，因为严重程度类别具有自然顺序性，导致类别边界模糊不清，难以实现有效的域对齐
...
Conclusion: 该方法通过利用类别顺序信息进行跨域排序和对齐，有效解决了医学图像严重程度分类中的域自适应问题，为处理有序类别分类任务提供了新思路
...
Method: 提出纹理感知Transformer，直接从每个网格面关联的原始像素学习，采用新的分层学习方案进行多尺度特征聚合。包含纹理分支将面级像素汇总为可学习token，与几何描述符融合后通过两阶段Transformer块处理，实现局部和全局信息流。

Result: 在Semantic Urban Meshes基准上达到81.9% mF1和94.3% OA，在新构建的文化遗产数据集（带损坏类型三角形级标注的纹理屋顶瓦片）上达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 该方法有效利用纹理3D网格的丰富信息，通过纹理感知Transformer和分层学习方案，在城市场景和文化遗产语义分割任务上取得显著性能提升，证明了同时利用几何和纹理信息的重要性。
...
TL;DR: PI-VQ是一种位置无关的向量量化自编码器，通过消除潜在代码中的位置信息来捕获全局语义特征，并引入匹配量化算法提高信息容量，支持单次前向传播的图像合成。
...
Method: 提出置换不变向量量化自编码器（PI-VQ），约束潜在代码不携带位置信息。引入匹配量化算法，基于最优二分匹配进行向量量化，相比朴素最近邻量化将有效瓶颈容量提高3.5倍。支持基于插值的采样，实现单次前向传播的图像合成。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得了具有竞争力的精度、密度和覆盖率指标。学习的代码具有组合结构，支持直接图像插值而无需学习先验。

Conclusion: 位置无关表示在可分离性和可解释性方面存在权衡，但能够捕获全局语义特征并实现高效图像合成，为未来工作指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解方面表现出色，但输入视频令牌数量巨大导致计算负担重。现有方法主要在输入级剪枝视频令牌，忽略了视频和大语言模型固有的信息结构。

Method: 基于视频具有分段-帧结构和LLM内部单向传播多模态信息的观察，提出三层剪枝：1)分段级：视频令牌先时间分段后空间合并；2)帧级：同一分段内相似帧联合剪枝以保持多样性；3)层级：随着LLM层数增加，冗余逐渐减少而不影响性能。

Result: 在四个广泛使用的视频理解基准上进行了广泛实验。仅保留30%令牌时，HieraVid实现了新的SOTA性能，同时保持了LLaVA-Video-7B和LLaVA-OneVision-7B分别超过98%和99%的性能。

Conclusion: HieraVid通过分层剪枝框架有效减少了视频大语言模型的计算负担，在显著减少令牌数量的同时保持了高性能，为视频理解模型的部署提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅在CT重建中表现良好，但其优势不足以促使从成熟算法转向。现有GS方法仍存在显著局限性，特别是在速度和灵活性方面需要改进

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流水线，实现快速CT重建。改进的体素化能够快速将高斯拟合到现有体积数据，可作为先验或压缩表示

Result: 在标准512×512投影上比现有最佳GS CT重建快4倍以上，在2k投影上快13倍以上。方法具有良好的可扩展性，能适应不同投影和输出体积尺寸

Conclusion: FaCT-GS解决了GS在CT重建中的主要限制，提供了快速、灵活且可扩展的解决方案，可作为现有重建算法的有竞争力替代方案
...
<details>
  <summary>Details</summary>
Motivation: 研究当前最先进的视觉语言模型在基本几何变换下的脆弱性，揭示其虽然擅长语义任务但在空间不变性和等变性方面的系统性缺陷

Method: 通过系统评估方法，在不同视觉领域（符号草图、自然照片、抽象艺术）测试VLMs对旋转、缩放和恒等变换的鲁棒性，涵盖多种架构、模型容量和提示策略

Result: VLMs在语义内容稀疏时性能急剧下降，这种脆弱性在不同架构、模型容量和提示策略中普遍存在，表明当前模型缺乏可靠的几何推理能力

Conclusion: 当前VLMs在语义理解与空间推理之间存在系统性差距，未来多模态系统需要更强的几何基础来提升对基本变换的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 当前时序动作分割方法依赖复杂架构，不利于实际部署。需要一种轻量级方法，通过简单损失设计而非复杂架构或推理时优化来提升分割质量

Method: 提出双损失训练框架：1）边界回归损失：通过单通道边界预测提升时序定位精度；2）CDF段级正则化损失：通过匹配预测段和真实段的累积分布来鼓励段内结构一致性。该框架与架构无关，可集成到现有TAS模型中

Result: 在三个基准数据集上，该方法提高了段级一致性和边界质量，在三种不同模型（MS-TCN、C2F-TCN、FACT）上均获得更高的F1和Edit分数。帧级准确率基本保持不变

Conclusion: 通过简单的损失设计而非复杂架构或推理时优化，可以实现精确的时序动作分割。该方法轻量、架构无关，易于集成到现有模型中，具有实际部署价值
...
TL;DR: MAR-MAER：一种结合度量感知嵌入正则化和概率潜在模型的层次自回归框架，用于提升文本到图像生成的质量和模糊提示处理能力。


<details>
  <summary>Details</summary>
Motivation: 自回归文本到图像生成模型面临两个主要挑战：1）生成图像质量可能不符合人类期望标准；2）难以处理具有多种有效解释的模糊提示。需要同时提升生成质量和对模糊语义的处理能力。
...
Result: 在COCO和新开发的Ambiguous-Prompt Benchmark上的实验表明，MAR-MAER在度量一致性和语义灵活性方面表现优异。相比基线Hi-MAR模型，CLIPScore提升+1.6，HPSv2提升+5.3。对于模糊输入，能产生更广泛的输出多样性。结果通过人工评估和自动化指标验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将可供性推理视为静态场景观察下的单次预测问题，假设已有足够证据。但在复杂3D场景中，许多失败案例并非源于预测能力不足，而是固定观察下任务相关证据不完整。需要将细粒度可供性推理重新定义为序列证据获取过程

Method: 提出A3R框架：1）将可供性推理重新定义为序列证据获取过程，通过互补的3D几何和2D语义证据逐步减少模糊性；2）基于MLLM的策略迭代选择证据获取动作并更新可供性信念；3）引入GRPO策略学习方法优化序列决策，提高证据获取效率和推理准确性

Result: 在场景级基准测试上的广泛实验表明，A3R始终优于静态单次预测基线方法，证明了在复杂3D高斯场景中，智能体跨维度证据获取对细粒度可供性推理的优势

Conclusion: 通过将可供性推理重新定义为序列证据获取过程，A3R框架能够更有效地处理复杂3D场景中的模糊性，智能体跨维度证据获取方法在细粒度可供性推理任务中展现出显著优势
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）在实时渲染方面表现优异，但大量高斯点导致高内存消耗。现有剪枝方法虽能减少内存，但会损害空间一致性并产生渲染伪影，需要一种既能压缩表示又能保持高质量渲染的解决方案

Method: 1) 基于证据下界（ELBO）的自适应致密化策略，自动控制高斯点生成过程；2) 透明度感知渐进剪枝策略，动态移除低透明度高斯点；3) 基于图的特征编码模块，通过特征引导的点位移调整空间分布

Result: GS^2在仅使用约12.5%高斯点的情况下，相比原始3DGS实现了更高的PSNR。在渲染质量和内存效率方面均优于所有基线方法，实现了紧凑高斯表示与高质量渲染的平衡

Conclusion: 提出的图基空间分布优化方法（GS^2）有效解决了3DGS内存消耗高的问题，通过自适应致密化、渐进剪枝和特征引导分布调整，在显著减少高斯点数量的同时提升了渲染质量，为紧凑3D高斯表示提供了有效解决方案
...
TL;DR: 研究发现文本到图像生成模型存在低成本的提示词越狱攻击漏洞，仅需自然语言提示即可绕过安全过滤器，攻击成功率最高达74.47%


<details>
  <summary>Details</summary>
Motivation: 尽管文本到图像生成模型已广泛部署并采用安全过滤机制，但现有系统仍存在安全漏洞。本研究旨在揭示这些模型对低成本提示词越狱攻击的脆弱性，这些攻击仅需自然语言提示，无需模型访问、优化或对抗训练。

Method: 提出系统性的提示词越狱攻击研究，引入视觉越狱技术分类学，包括：艺术重构、材料替换、伪教育框架、生活方式美学伪装和模糊动作替换。这些策略通过在良性语义上下文中隐藏不安全意图来利用提示词审核和视觉安全过滤的弱点。

Result: 在多个最先进的文本到图像系统上评估这些攻击，证明简单的语言修改能可靠地规避现有安全措施并生成受限图像。在所有测试模型和攻击类别中，观察到攻击成功率最高达74.47%。

Conclusion: 研究结果凸显了表面级提示词过滤与检测生成媒体系统中对抗意图所需的语义理解之间的关键差距。现有安全措施无法有效识别隐藏在良性上下文中的恶意意图。
...
TL;DR: ProVG是一个用于遥感视觉定位的新框架，通过将语言表达解耦为全局上下文、空间关系和物体属性，并采用渐进式跨模态调制器实现从粗到细的视觉-语言对齐，显著提升了遥感图像中物体定位的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法通常依赖句子级的视觉-语言对齐，难以利用空间关系和物体属性等细粒度语言线索，而这些线索对于区分相似特征物体至关重要。不同定位阶段需要不同的语言线索指导，需要更明确的引导机制。

Method: 1. 将语言表达解耦为全局上下文、空间关系和物体属性三个部分；2. 采用渐进式跨模态调制器，通过"调查-定位-验证"方案动态调制视觉注意力；3. 引入跨尺度融合模块处理遥感图像的大尺度变化；4. 设计语言引导校准解码器在预测时细化跨模态对齐；5. 统一多任务头同时支持指称表达理解和分割任务。

Result: 在RRSIS-D和RISBench两个基准测试上的广泛实验表明，ProVG持续优于现有方法，实现了新的最先进性能。

Conclusion: ProVG通过解耦语言表达和渐进式跨模态调制，有效利用了细粒度语言线索，显著提升了遥感视觉定位的准确性。该框架能够处理遥感图像的大尺度变化，并支持多种任务，为遥感视觉定位提供了有效的解决方案。
...
TL;DR: SHARC是一个新颖的框架，通过球谐函数表示的距离场集合来合成任意、属不可知的形状，在重建精度和时间效率方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的形状表示方法在几何保真度、计算效率和模型简洁性之间难以取得平衡。SHARC旨在通过一种新的表示框架，在保持模型简洁的同时提高重建精度和时间效率。

Method: 1. 在表面内部体积中优化放置参考点，通过联合最大化稀疏性、中心性和表面可见性的成本函数来选择位置；2. 对每个参考点，通过光线投射采样到表面的可见距离场，并使用快速球谐变换计算SH系数；3. 应用可配置的低通滤波器增强几何保真度；4. 基于邻近性使用局部一致性约束细化输出。

Result: SHARC在重建精度和时间效率方面均优于最先进的方法，同时不牺牲模型的简洁性。该方法能够有效合成任意、属不可知的形状。

Conclusion: SHARC框架通过球谐函数表示的距离场集合提供了一种高效、准确的形状合成方法，在几何保真度、计算效率和模型简洁性之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法存在局限性：要么专注于帧级增强而时间建模有限，要么依赖沉重的时空聚合而牺牲高频细节。如何在保持时间稳定性的同时保留空间细节是核心挑战。

Method: 提出FTPFusion方法：1）将特征表示分解为高频和低频分量进行协同建模；2）高频分支执行稀疏跨模态时空交互以捕捉运动相关上下文和互补细节；3）低频分支引入时间扰动策略增强对复杂视频变化（闪烁、抖动、局部错位）的鲁棒性；4）设计偏移感知时间一致性约束，在时间扰动下显式稳定跨帧表示。
...
Method: 1) 修改ResNet，用KAN卷积替换传统卷积，实现SAR图像的自适应特征提取；2) 使用Gram多项式作为激活函数，适合SAR数据捕捉复杂非线性关系；3) 采用参数共享策略：每个核按通道共享参数，在保留独特特征的同时减少参数和FLOPs。

Result: 在MSTAR、FUSAR-Ship和SAR-ACD数据集上分别达到99.09%、93.01%和97.26%的准确率。在1024×1024尺寸的MSTAR数据集上，相比VGG16减少82.90倍FLOPs和163.78倍参数。

Conclusion: Light-ResKAN为边缘SAR图像识别提供了一个高效的解决方案，在保持高精度的同时显著降低了计算复杂度和参数量。
...
TL;DR: 提出一种基于几何的角膜反射点检测与匹配方法，将多个光点视为结构化星座而非独立斑点，提高多LED眼动追踪的鲁棒性和可复现性。


<details>
  <summary>Details</summary>
Motivation: 当前角膜反射点检测通常作为启发式方法嵌入大型系统，难以在不同硬件设置间复现。需要一种透明、可评估的方法来处理多光点检测与匹配问题。

Method: 采用2D几何驱动的星座式流水线，提出相似性-布局对齐(SLA)方法，结合受控过检测、自适应候选回退、外观感知评分和可选语义布局先验，将检测与对应关系明确分离。

Result: 在公开多LED数据集上评估，系统在噪声条件下提供稳定的身份保持对应关系，实现了可复现的检测与匹配。

Conclusion: 该方法为多光点眼动追踪提供了透明、可复现的解决方案，通过代码、预设和评估脚本的发布支持透明复现、比较和数据集标注。
...
TL;DR: KnowMVG通过知识增强提示和全局-局部注意力机制提升医学视觉定位的精度，在四个基准测试中优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在医学视觉定位中空间定位精度不足，主要原因是缺乏显式的定位先验知识，仅依赖潜在嵌入无法提供足够的空间感知

Method: 提出KnowMVG框架：1) 知识增强提示策略，将医学知识编码为紧凑嵌入；2) 全局-局部注意力机制，联合利用粗粒度全局信息和细粒度局部线索指导精确区域定位

Result: 在四个MVG基准测试中，KnowMVG持续优于现有方法，AP50提升3.0%，mIoU提升2.6%。定性和消融研究验证了各组件有效性

Conclusion: KnowMVG通过显式增强解码过程中的空间感知，在医学视觉定位中实现了更精确的区域定位，弥合了高级语义理解和细粒度视觉感知之间的差距
...
TL;DR: 该研究探索了能否直接从波束形成前的天线级距离-多普勒张量中学习空间结构，而非依赖传统的波束形成角度域表示。
...
Method: 使用6发8收（48个虚拟天线）商用汽车雷达，采用A/B啁啾序列FMCW发射方案，其中有效发射孔径在啁啾间变化。采用双啁啾共享权重编码器，以端到端数据驱动方式处理波束形成前的天线级距离-多普勒张量。使用鸟瞰图占用率作为几何探针进行评估，监督信号来自激光雷达，采用可见性感知和跨模态方法，明确建模雷达视场和基于射线的可见性。
...
TL;DR: 该研究针对早期儿童教育图像描述任务，提出了ECAC大规模数据集和RSRS混合训练框架，开发了KinderMM-Cap-3B模型，显著提升了专业物体识别精度。


<details>
  <summary>Details</summary>
Motivation: 现有早期儿童教育图像描述方法面临两大挑战：缺乏大规模领域特定数据集导致无法捕捉细粒度语义概念；传统训练范式在提升专业物体描述能力方面存在局限，监督学习偏向高频表达，强化学习在困难样本上优化不稳定。

Method: 1) 构建ECAC基准数据集：包含256,121张真实世界图像，配备专家级描述和细粒度标签；2) 提出TTS评估指标：专门衡量专业玩具识别准确率；3) 设计RSRS混合训练框架：动态切换强化学习和监督微调，将零奖励困难样本重定向到监督学习；4) 开发KinderMM-Cap-3B：领域适应的多模态大语言模型。

Result: KinderMM-Cap-3B模型在TTS指标上达到51.06分，显著优于现有最先进基线模型，同时保持优异的描述质量，展示了在专业教育应用中的潜力。

Conclusion: ECAC数据集和RSRS训练框架有效解决了早期儿童教育图像描述中的领域适应问题，通过混合训练策略稳定了困难样本优化，显著提升了专业物体识别能力，为专门化教育应用提供了有力工具。
...
TL;DR: 论文提出AMIMV方法解决医学图像分类中的数据稀缺和类别不平衡问题，通过非对称多图像多视图增强策略，并在11个MedMNIST数据集上评估了8种SSL方法在长尾分布下的表现。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析面临两个主要问题：1）缺乏大量标注训练数据；2）数据不平衡问题，即常见类别数据丰富而罕见类别数据稀缺。自监督学习（SSL）方法在一定程度上解决了第一个问题，但SSL在医学图像分类中对不平衡数据的鲁棒性研究很少。

Method: 1）扩展了先前提出的MIMV方法，引入新的增强策略构建非对称多图像多视图（AMIMV）对，以同时解决数据稀缺和数据集不平衡问题；2）进行数据分析评估AMIMV在不同程度类别不平衡下的鲁棒性；3）在11个MedMNIST数据集上评估8种代表性SSL方法在长尾分布和有限监督下的表现。

Result: 在MedMNIST数据集上的实验结果显示：retinaMNIST提升4.25%，tissueMNIST提升1.88%，DermaMNIST提升3.1%。AMIMV方法在医学图像分类中有效改善了数据不平衡问题下的性能。

Conclusion: AMIMV方法通过非对称多图像多视图增强策略，有效解决了医学图像分类中的数据稀缺和类别不平衡问题，在多个医学图像数据集上取得了显著性能提升，为不平衡医学图像分类提供了有效的自监督学习解决方案。
...
Method: 提出MAVFusion端到端视频融合框架，采用运动感知稀疏交互机制：1）利用光学流识别多模态序列中的动态区域；2）对动态区域自适应分配计算密集的跨模态注意力，捕捉显著过渡并促进模态间信息交换；3）对静态背景区域使用轻量级弱交互模块保持结构和外观完整性；4）通过解耦动态和静态区域处理，同时保持时间一致性和细粒度细节。

Result: 在多个红外与可见光视频基准测试中达到最先进性能，在640×480分辨率下实现14.16 FPS的推理速度，显著加速了推理过程。

Conclusion: MAVFusion通过运动感知稀疏交互机制，在保持高质量融合结果的同时显著提升了计算效率，为红外与可见光视频融合提供了有效的解决方案。
...
Method: 使用nnU-Net v2框架构建深度学习模型，利用多模态mpMRI数据（T2加权成像、扩散加权成像DWI、表观扩散系数ADC图）提取互补组织信息，在PI-CAI数据集的981个病例上进行训练，采用5折交叉验证和外部验证评估性能。

Result: 模型在交叉验证中达到平均Dice分数0.96±0.00，在外部测试集（Hospital La Fe的54名患者）上达到0.82，表现出良好的泛化能力；相比之下，通用分割工具TotalSegmentator仅获得0.15的Dice分数，主要由于腺体欠分割。

Conclusion: 任务特异性多模态分割策略对于前列腺分割至关重要，提出的方法具有强大的泛化能力，适合集成到临床研究流程中；模型已容器化并作为即用推理工具提供，促进可重复性和部署。
...
TL;DR: MyEgo是首个评估多模态大语言模型在自我中心视频中个性化问答能力的基准数据集，揭示当前模型在自我定位和长期记忆方面的严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在理解自我中心视频中相机佩戴者（"我"）的能力尚未得到系统评估。需要建立基准来评估模型对"我的物品"、"我的活动"和"我的过去"的理解、记忆和推理能力。

Method: 创建MyEgo数据集，包含541个长视频和5K个个性化问题。系统评估多种MLLMs变体：开源vs专有、思考vs非思考、小规模vs大规模模型。分析模型在显式提供证据和时间跨度下的表现变化。

Result: 顶级闭源和开源模型（如GPT-5和Qwen3-VL）准确率仅约46%和36%，远低于人类表现（差距约40%和50%）。显式推理和模型缩放未带来一致改进。提供相关证据可提升表现，但随时间推移增益下降，表明模型在跟踪和记忆"我"及"我的过去"方面存在局限。

Conclusion: 自我定位和长期记忆是实现自我中心视频个性化问答的关键能力。MyEgo数据集和分析将推动自我中心个性化辅助领域的进一步发展。
...
Method: 1. 多视角场景先验增强：通过聚合多视角结构知识丰富欠指定的文本输入；2. 功能感知布局定位：利用区域功能定位作为隐式空间锚点，进行分层布局推理；3. 迭代反射-修正方案：通过自修正实现渐进式结构合理性优化

Result: 实验表明该方法在短文本条件下的3D室内场景生成任务上优于现有方法

Conclusion: SDesc3D通过整合多视角结构先验和区域功能含义，有效提升了短文本条件下3D室内场景生成的物理合理性和语义一致性
...
TL;DR: 提出交互式跟踪新范式，构建首个大规模交互式跟踪基准InteractTrack，评估发现现有跟踪器在交互场景中表现不佳，并提出基于动态记忆的基线方法IMAT。


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要在非交互、发射后不管的模式下运行，不适用于需要人在回路适应的现实场景。需要克服这一限制，实现用户通过自然语言指令随时指导跟踪器的新范式。

Method: 1) 构建InteractTrack基准：包含150个视频，带有密集边界框标注和时间戳语言指令；2) 提出综合评估协议，评估25个代表性跟踪器；3) 提出交互式记忆增强跟踪(IMAT)基线方法，采用动态记忆机制从用户反馈中学习并更新跟踪行为。

Result: 评估显示最先进的跟踪方法在交互场景中失败，传统基准上的强性能无法迁移到交互场景。IMAT作为新基线为开发更智能、自适应和协作的跟踪系统奠定了基础。

Conclusion: 交互式跟踪新范式、基准、协议和基线方法为开发更智能、自适应和协作的跟踪系统奠定了基础，弥合了自动感知与人工指导之间的差距。
...
TL;DR: Curia-2：首个十亿参数级多模态CT/MRI基础模型，通过改进预训练策略提升放射影像表征质量，并扩展评估基准为2D切片和3D体素双轨评测


<details>
  <summary>Details</summary>
Motivation: 医学影像快速增长导致放射科医生工作量不可持续，现有基础模型在CT和MRI分析方面仍有优化空间，需要更好地学习复杂放射学体积数据

Method: 基于Curia框架改进预训练策略和表征质量，首次将架构扩展到十亿参数级Vision Transformers，支持多模态CT和MRI；将CuriaBench扩展为2D切片和3D体素双轨评估体系

Result: Curia-2在视觉任务上优于所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型表现相当；模型权重将公开以促进研究

Conclusion: Curia-2通过优化的预训练策略和十亿参数架构显著提升了放射影像基础模型的性能，为医学影像分析提供了更强大的工具，并通过标准化评估框架推动领域发展
...
TL;DR: 论文提出IVE方法解决多模态大语言模型中视觉注意力的惯性问题，该惯性阻碍了认知推理所需的组合理解能力


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型存在视觉注意力惯性问题，注意力一旦在早期解码步骤中稳定后就基本保持静态，无法支持认知推理所需的组合理解。现有的幻觉缓解方法主要针对感知幻觉，对需要对象间关系推理的认知幻觉效果有限。

Method: 提出无需训练的惯性感知视觉激发方法，通过将认知推理建模为视觉注意力的动态响应来打破惯性模式。具体包括：选择相对于历史注意力趋势动态出现的视觉token，区分表现出惯性行为的token；引入惯性感知惩罚机制，防止注意力过度集中和局部区域持续关注。

Result: 在多种基础MLLM和多个幻觉基准测试上的广泛实验表明，IVE方法对认知幻觉特别有效，能够显著改善模型在需要组合推理的任务上的表现。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的关键因素，提出的IVE方法通过打破注意力惯性模式，有效提升了模型在需要关系推理的认知任务上的性能，为改善MLLM的认知推理能力提供了新思路。
...
Method: 提出Resonance4D框架，将3D高斯泼溅与物质点法通过轻量级物理表达监督耦合。核心创新包括：1）双域运动监督（DMS），结合空间结构一致性（局部变形）和频域谱一致性（振荡和全局动态模式）；2）零样本文本提示分割与模拟引导初始化，自动将高斯分解为对象部件级区域，支持全材料参数的联合优化

Result: 在合成和真实场景实验中，Resonance4D实现了强物理保真度和运动一致性，同时将峰值GPU内存从超过35GB降低到约20GB，使高保真物理驱动4D模拟能在单消费级GPU上运行

Conclusion: Resonance4D通过双域运动监督和全参数物理恢复，解决了现有4D动态模拟方法的计算成本和物理真实性限制，为复杂材料和动态场景的高保真物理模拟提供了高效解决方案
...
TL;DR: ProDiG：一种扩散引导的渐进式框架，通过几何感知因果注意力和距离自适应高斯模块，将航拍3D表示逐步转换为地面级逼真度，无需额外地面真值数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从纯航拍图像生成地面视角和连贯3D场景模型时面临挑战：极端视角变化、中间观测缺失和大尺度变化。现有方法要么后处理渲染导致几何不一致，要么依赖多高度地面真值（通常不可得）。高斯溅射和扩散细化在小变化下有效，但在大航拍-地面差距下失败。

Method: 提出ProDiG（渐进高度高斯溅射）框架：1）合成中间高度视角；2）使用几何感知因果注意力模块将极线结构注入参考视图扩散，细化每个阶段的高斯表示；3）距离自适应高斯模块根据相机距离动态调整高斯尺度和不透明度，确保大视角差距下的稳定重建。

Result: 在合成和真实数据集上的广泛实验表明，ProDiG能生成视觉逼真的地面级渲染和连贯3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法。

Conclusion: ProDiG通过渐进式、几何基础化的细化，无需额外地面真值视角，成功解决了从纯航拍图像生成地面视角和3D场景模型的挑战，实现了视觉逼真度和几何一致性的显著提升。
...
TL;DR: DR-Seg提出了一种解耦与校正框架，通过将CLIP特征分解为语义主导和结构主导子空间，实现有针对性的结构增强，从而在遥感开放词汇语义分割中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 遥感领域的开放词汇语义分割需要语言对齐识别和细粒度空间划分。虽然CLIP提供强大的语义泛化能力，但其全局对齐的视觉表示难以捕捉结构细节。现有方法引入RS预训练的DINO特征进行补偿，但将CLIP表示视为单一语义空间，无法定位需要结构增强的区域，导致边界划分效果不佳且可能破坏CLIP的语义完整性。

Method: DR-Seg框架基于CLIP特征通道存在功能异质性的关键观察，将CLIP特征解耦为语义主导和结构主导子空间，使DINO能够进行有针对性的结构增强而不扭曲语言对齐语义。随后，先验驱动的图校正模块在DINO指导下注入高保真结构先验形成精炼分支，不确定性引导的自适应融合模块动态集成精炼分支与原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征并针对性增强结构信息，有效解决了遥感开放词汇语义分割中语言对齐与空间细粒度划分的平衡问题，实现了更好的边界划分效果同时保持了语义完整性。
...
TL;DR: LinkS²Bench是首个评估视觉语言模型在无人机与卫星协同空间智能方面能力的基准，包含17.9k高质量问答对，覆盖12个细粒度任务，揭示现有模型在跨视图动态对齐方面的显著差距。
...
Method: 构建LinkS²Bench基准，将1022分钟动态无人机视频与覆盖200km²的高分辨率卫星图像关联；通过LMM辅助流水线和严格人工标注，创建17.9k高质量问答对，涵盖感知、定位、关系和推理四个维度的12个细粒度任务；设计跨视图对齐适配器来改善模型性能。

Result: 对18个代表性视觉语言模型的评估显示，与人类基线相比存在显著差距，准确跨视图动态对齐是关键瓶颈；跨视图对齐适配器显著提升了模型性能；微调实验证明了LinkS²Bench在推进视觉语言模型适应复杂空间推理方面的潜力。

Conclusion: LinkS²Bench填补了评估视觉语言模型在无人机-卫星协同空间智能方面能力的空白，揭示了跨视图动态对齐的关键挑战，提出的对齐适配器有效改善了性能，为推进复杂空间推理的视觉语言模型发展提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 在医学成像、生物学和物理学等领域，图像内容的空间采样通常不均匀，信息丰富的模式只在特定坐标上罕见出现，而背景在这些位置占据主导。这种空间不平衡导致自编码器偏向于重建主要模式，丢失细粒度细节，对罕见空间输入产生模糊重建。

Method: 提出两个互补组件：(1) 基于自熵的损失函数，对统计上不常见的空间位置进行加权；(2) 样本传播机制，这是一种重放机制，在训练过程中有选择地重新暴露模型难以重建的样本。

Result: 在受控空间不平衡条件的模拟数据集和三个真实世界数据集（物理、生物、天文领域）上验证。该方法在各种重建指标上优于基线方法，特别是在空间不平衡分布下表现更佳。

Conclusion: 该方法通过针对空间不平衡问题，鼓励模型关注统计上罕见的位置，提高了重建一致性。结果强调了批次中数据表示的重要性，并在无监督图像重建中突出了罕见样本的价值。
...
Method: 在四个校园位置（ACS-EC、ACS-EG、IE-Central、R-Central）收集31个视频（9,913帧，5fps），包含人工验证的每实例分割掩码。使用620帧控制子集评估三个基础模型自动标注器（SAM3、GroundingSAM、EfficientGroundingSAM），使用Cohen's κ、AP、精确率、召回率和掩码IoU指标。2,552帧子集支持多目标跟踪，采用MOTChallenge格式。使用YOLOv8n、YOLOv26n、RT-DETR-L与ByteTrack、BoT-SORT、OC-SORT建立基线。

Result: 场景分析显示难度变化显著，受人群密度、尺度和遮挡影响：ACS-EC场景（79.3%密集帧，平均实例尺度60.8像素）最具挑战性。自动标注器评估提供了基础模型在室内人群场景中的性能基准。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群数据集的空白，为检测、分割和跟踪任务提供了基准，并揭示了室内场景中人群密度、尺度和遮挡对算法性能的重要影响。
...
TL;DR: WISE提出了一种高效的多模态推理范式，通过"思考两次"原则（一次学习，一次加速），在训练时生成结构化序列（简洁推理→答案→详细解释），推理时仅使用简洁推理，实现5倍推理长度缩减，同时保持SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有思维链推理方法在语言引导分割中性能显著，但生成冗长推理导致计算成本过高，限制了实际应用。需要一种既能保持推理能力又能大幅降低计算开销的高效方法。
...
Result: 在ReasonSeg基准测试中实现58.3 cIoU的零样本SOTA性能，同时将平均推理长度从112个token减少到23个token，缩减近5倍，显著降低计算成本。

Conclusion: WISE通过"思考两次"范式成功解决了思维链推理的计算效率问题，在保持高性能的同时大幅降低推理成本，为实际应用提供了可行方案。结构化序列训练和简洁性指令提示是关键技术。
...
<details>
  <summary>Details</summary>
Motivation: 构建高质量多语言和非英语视觉语言模型面临的主要障碍是其他语言中视觉问答数据集规模有限、领域覆盖不足。英语通常通过聚合现有VQA资源构建数据集，但这一策略难以扩展到其他语言。
...
Conclusion: Jagle数据集有效解决了日语VLM训练数据稀缺问题，通过创新的数据生成策略构建了大规模多模态数据集，显著提升了日语VLM性能，同时保持了英语能力。作者发布了数据集、训练模型和代码以促进可重复性和未来研究。
...
TL;DR: 提出了一种全自动、可扩展的方法来评估虚拟人渲染中肤色保真度，通过肤色提取、纹理重着色、实时渲染和定量色彩分析，发现较深肤色存在更高的色度误差


<details>
  <summary>Details</summary>
Motivation: 虚拟人渲染中肤色准确再现对于真实感、身份保持和公平性至关重要，但现有可访问的虚拟人创建流程依赖未经色彩校准的摄影输入，可能导致不一致性和偏见

Method: 提出全自动工作流程，整合肤色和光照提取、纹理重着色、实时渲染和定量色彩分析；使用芝加哥面部数据库，比较基于脸颊区域采样和多维掩码的肤色提取策略，结合TRUST框架进行光照隔离；将提取肤色应用于MetaHuman纹理并在多种光照配置下渲染

Result: 分析了约19,848个渲染实例，结果显示提取策略表现出表型依赖性行为，较深肤色始终存在更高的色度误差

Conclusion: 提出的方法无需人工干预，计算成本低，支持大规模评估，揭示了虚拟人生成流程中肤色保真度的系统性偏差，特别对较深肤色存在不公平的色彩再现问题
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中多个物体可能具有相同的功能特性，但只有特定物体适合特定任务上下文（称为混淆对）。现有3D功能推理方法主要评估孤立单个物体，无法处理多物体场景中的隐式意图理解问题。

Method: 提出CompassNet框架：1) Instance-bounded Cross Injection (ICI)模块约束语言-几何对齐在物体边界内，防止跨物体语义泄漏；2) Bi-level Contrastive Refinement (BCR)模块在几何组和点级别强制区分，增强目标与混淆表面之间的辨别能力。

Result: 构建CompassAD基准数据集，包含30个混淆物体对、16种功能类型、6,422个场景和88K+查询-答案对。实验显示在已见和未见查询上均达到最先进性能，并在机器人操作器上部署验证了在真实世界混淆多物体场景中的有效抓取转移。

Conclusion: 该研究形式化了意图驱动的多物体功能推理新任务，通过构建基准数据集和提出专用框架，有效解决了多物体场景中混淆对的隐式意图理解问题，展示了向真实机器人应用的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于显式思维链的通用多模态嵌入方法存在两个主要问题：1）显式思维链产生大量推理开销；2）丰富的多模态证据被压缩到狭窄的文本瓶颈中。需要一种既能保持推理优势又能提高效率的方法。

Method: PLUME采用潜在推理框架，用连续潜在状态的自回归展开替代显式思维链。引入语义锚点引导的转换适配器，在固定计算预算下引导不同推理轨迹。采用渐进式显式到潜在课程学习，使用显式推理作为临时训练支架，逐步将推理行为转移到隐藏状态计算中，在推理时消除显式思维链。

Result: 在78任务的MMEB-v2基准测试中，PLUME优于强显式思维链基线，同时将推理从数百个生成token减少到少于10个潜在步骤，实现超过30倍的推理加速。在视频和视觉文档检索等证据密集、结构复杂、难以通过显式中间推理组织的检索场景中表现尤为出色。

Conclusion: 结构化潜在计算可以在不产生显式推理生成开销的情况下保留中间推理的优势，为实际检索系统提供了更强大、更高效的范式。这表明显式思维链并非实现复杂推理的必要条件，潜在状态计算可以更高效地实现类似功能。
...
TL;DR: GroundVTS：一种用于视频大语言模型的视觉token采样方法，通过查询引导的细粒度机制筛选关键时间片段，提升视频时序定位性能
...
Result: 在三个标准VTG基准测试中优于现有方法，时刻检索mIoU提升7.7点，亮点检测mAP提升12.0点

Conclusion: GroundVTS通过聚焦信息最丰富的时间片段，有效保留了时空信息并维持时序连贯性，显著提升了视频大语言模型的时序定位能力
...
TL;DR: LatentUM提出了一种在共享语义潜在空间中统一表示所有模态的模型，消除了视觉理解与生成之间像素空间中介的需求，实现了高效的交错跨模态推理与生成。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型（UMs）需要像素解码作为视觉理解与生成之间的桥梁，因为它们在理解和生成任务上使用分离的视觉表示，这种方法既低效又低效。研究者希望开发一种能够直接在语义层面进行跨模态推理和生成的统一模型。

Method: LatentUM将所有模态（包括视觉模态）表示在共享的语义潜在空间中，无需像素空间中介。这种设计允许模型在同一个语义空间中同时进行视觉理解和生成，支持灵活的交错跨模态推理和生成任务。

Result: LatentUM在计算效率上显著提升，减轻了编解码器偏差，增强了跨模态对齐。在Visual Spatial Planning基准测试中达到最先进性能，通过自反思提升了视觉生成质量，并支持在共享语义潜在空间中预测未来视觉状态的世界建模能力。

Conclusion: LatentUM通过在共享语义潜在空间中统一表示所有模态，有效解决了现有统一模型中视觉理解与生成分离的问题，为交错跨模态推理和生成提供了更高效、更强大的框架，在多个任务上展现出优越性能。
...
TL;DR: 提出CoRegOVCD框架，一种无需训练的开集词汇变化检测方法，通过后验校准和一致性正则化解决跨时间概念响应直接比较的噪声问题


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法假设固定标签空间，无法回答用户定义的任意查询。开集词汇变化检测(OVCD)旨在检测查询概念的变化掩码，但在完全无需训练的场景下，跨时间直接比较密集概念响应存在困难：外观变化、弱跨概念竞争以及土地覆盖类别的空间连续性导致噪声、碎片化和语义不可靠的变化证据

Method: 提出一致性正则化开集词汇变化检测(CoRegOVCD)框架，包含：1)竞争性后验校准(CPC)将原始概念响应转换为竞争感知的查询概念后验；2)语义后验差异(SPD)量化跨时间后验差异；3)几何令牌一致性门(GeoGate)通过几何感知结构验证抑制不支持响应；4)区域共识差异(RCD)通过区域共识提高空间一致性

Result: 在四个基准测试中(涵盖建筑导向和多类设置)，CoRegOVCD相比先前最强的无需训练基线持续提升2.24到4.98 F1_C点，在SECOND数据集上达到六类平均47.50% F1_C

Conclusion: CoRegOVCD通过后验校准和一致性正则化有效解决了开集词汇变化检测中跨时间概念响应比较的挑战，无需训练即可获得可靠的变化检测结果
...
TL;DR: 交叉验证本身引入可测量的随机方差，在BP4D+数据集上3折交叉验证产生±0.065的平均F1噪声，留一数据集交叉验证能提供更稳定和可解释的结果


<details>
  <summary>Details</summary>
Motivation: 揭示面部动作单元检测中标准评估协议（主题排他交叉验证）存在的问题，证明交叉验证本身引入可测量的随机方差，导致报告的小幅改进可能落在协议方差范围内

Method: 使用BP4D+数据集进行重复3折主题排他分割，测量经验噪声；采用留一数据集交叉验证协议在五个AU数据集上评估跨数据集鲁棒性

Result: 3折交叉验证产生±0.065的平均F1噪声，低流行度AU方差更大；操作点指标（如F1）比阈值无关指标（如AUC）波动更大；留一数据集交叉验证暴露了单数据集交叉验证不可见的域级不稳定性

Conclusion: 交叉验证本身引入显著方差，报告的小幅改进可能落在协议方差范围内；留一数据集交叉验证提供更稳定和可解释的发现，是更可靠的评估协议
...
TL;DR: 该论文提出了一种反射生成方法，通过向基础扩散模型注入反射位置和外观的先验信息，并采用类型感知模型设计，构建了首个大规模物体反射数据集DEROBA，实现了物理一致且视觉逼真的反射效果。
...
Conclusion: 该研究提出了一个有效的反射生成框架，通过注入先验信息和类型感知设计，结合大规模数据集DEROBA，成功实现了高质量的反射合成，填补了反射生成领域的研究空白。
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但在视觉场景中理解模型如何工作仍然困难。现有可解释性工具通常关注孤立组件或专家导向分析，缺乏对完整推理流程的引导式端到端理解。
...
Conclusion: ViT-Explainer填补了Vision Transformer端到端可解释性工具的空白，通过交互式可视化系统促进了模型行为的理解。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在自动驾驶应用中面临空间感知与语义推理的冲突困境：直接采用2D VLM空间感知有限，而增强3D空间表示又会损害VLM的原生推理能力。这种困境源于空间感知和语义推理在共享模型参数中的耦合优化。
...
Result: 在nuScenes数据集的开环评估和Bench2Drive的闭环评估中均达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等广泛任务中表现出色，展示了作为自动驾驶统一模型的广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在文本和图像理解与生成方面表现优异，但扩展到3D领域面临数据稀缺的挑战。现有方法通常依赖间接的2D编辑和优化提升到3D的流程，牺牲了几何一致性。需要开发能够直接处理3D内容并保持几何一致性的原生3D基础模型。

Method: 提出Omni123模型，采用自回归框架统一文本到2D和文本到3D生成。核心洞察是利用图像和3D之间的跨模态一致性作为隐式结构约束。通过将文本、图像和3D表示为共享序列空间中的离散标记，模型利用丰富的2D数据作为几何先验来改善3D表示。引入交错X-to-X训练范式，在异构配对数据集上协调多样化的跨模态任务，无需完全对齐的文本-图像-3D三元组。通过在自回归序列中遍历语义-视觉-几何循环（如文本到图像到3D到图像），联合强制执行语义对齐、外观保真度和多视图几何一致性。

Result: 实验表明，Omni123在文本引导的3D生成和编辑方面显著改进，展示了向多模态3D世界模型扩展的可扩展路径。

Conclusion: Omni123通过统一的3D原生框架解决了现有方法在几何一致性方面的局限性，利用2D数据作为几何先验，为构建更强大的多模态3D世界模型提供了有前景的方向。
...
TL;DR: SCALE：用于零样本骨架动作识别的轻量级确定性语义和置信度感知列表能量框架，通过类条件能量排序解决语义混淆问题


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖显式的骨架-文本对齐，在动作名称未充分指定细粒度动态或未见类别语义混淆时表现脆弱，需要更鲁棒的框架

Method: 提出SCALE框架：1) 构建文本条件变分自编码器，冻结文本表示参数化先验和解码器；2) 引入语义和置信度感知列表能量损失，强调语义相似的困难负样本并纳入后验不确定性；3) 使用潜在原型对比目标对齐后验均值与文本派生的潜在原型

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE持续优于先前的VAE和对齐基线方法，同时与基于扩散的方法保持竞争力

Conclusion: SCALE通过类条件能量排序、语义感知能量损失和潜在原型对比，为ZSAR提供了更鲁棒的解决方案，避免了显式对齐的脆弱性，实现了更好的未见类别识别性能
...
<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法擅长处理对象"背后"的内容修复和外观层面的伪影（如阴影和反射），但当移除的对象与其他对象有更显著的物理交互（如碰撞）时，现有模型无法正确修正这些交互，导致结果不真实。

Method: 1. 使用Kubric和HUMOTO生成新的配对数据集，包含需要改变下游物理交互的反事实对象移除场景；2. 推理时，使用视觉语言模型识别受移除对象影响的场景区域；3. 使用视频扩散模型生成物理一致的反事实结果，以识别区域为引导。

Result: 在合成数据和真实数据上的实验表明，与先前的视频对象移除方法相比，VOID方法在对象移除后能更好地保持一致的场景动态。

Conclusion: VOID框架展示了如何通过高层次因果推理使视频编辑模型成为更好的世界模拟器，为处理复杂物理交互的视频对象移除提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中存在局限性，因为其固定的预训练分辨率和粗糙的patch级表示。在密集预测任务（如开放词汇分割）中，高分辨率输入对像素级推理至关重要，但现有滑动窗口方法计算成本高昂。
...
TL;DR: 提出可操控视觉表示（Steerable Visual Representations），通过早期融合文本到视觉编码器层，使视觉特征能够用自然语言引导关注图像中的任意对象，同时保持基础表示质量。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉Transformer（如DINOv2、MAE）的特征倾向于关注最显著视觉线索，无法引导关注次要概念；而多模态大语言模型可通过文本提示引导，但其表示偏向语言中心化，在通用视觉任务中效果下降。需要一种既能用自然语言引导，又保持视觉表示质量的解决方案。

Method: 提出可操控视觉表示新类别，通过轻量级交叉注意力在视觉编码器层直接注入文本（早期融合），而非像CLIP等模型在编码后融合文本和视觉特征（后期融合）。这种方法使全局和局部特征都能用自然语言操控。

Result: 开发了衡量表示可操控性的基准测试，证明该方法能够引导视觉特征关注图像中任意所需对象，同时保持底层表示质量。在异常检测和个性化对象识别任务上匹配或优于专用方法，并展示了对分布外任务的零样本泛化能力。

Conclusion: 可操控视觉表示通过早期文本融合实现了自然语言对视觉特征的精确引导，解决了现有方法在可操控性和表示质量之间的权衡问题，为通用视觉表示提供了新的研究方向。
...
TL;DR: 提出一种基于推理时引导的文本到图像生成安全控制框架，利用冻结预训练基础模型的梯度反馈进行引导，无需修改生成器本身。
...
Method: 利用视觉语言基础模型作为现成的监督信号，通过梯度反馈在采样步骤中注入清洁潜在估计，将安全引导公式化为基于能量的采样问题。

Result: 在NSFW红队基准测试中表现出最先进的鲁棒性，有效实现多目标引导，同时在良性非目标提示上保持高生成质量。

Conclusion: 该框架提供了一种原则性方法，将基础模型用作语义能量估计器，为文本到图像生成实现可靠且可扩展的安全控制。
...
TL;DR: ActionParty：首个可同时控制最多7个智能体的多主体视频世界模型，通过引入主体状态令牌解决现有视频扩散模型中动作绑定的基本问题
...
Method: 提出ActionParty模型，引入主体状态令牌（潜在变量）持续捕捉场景中每个主体的状态；通过空间偏置机制联合建模状态令牌和视频潜在表示，实现全局视频帧渲染与个体动作控制主体更新的解耦

Result: 在Melting Pot基准测试中评估，展示了首个能够同时在46个多样化环境中控制最多7个玩家的视频世界模型；结果显示在动作跟随准确性和身份一致性方面有显著改进，同时能够通过复杂交互实现鲁棒的自回归主体跟踪

Conclusion: ActionParty成功解决了多主体视频世界模型中的动作绑定问题，实现了对多个智能体的同时控制，为生成式视频游戏提供了有效的多主体世界建模方法
...
TL;DR: 提出一种快速表面配准方法，将表面网格表示为概率测度，使用切片Wasserstein距离衡量差异，并开发AdamFlow优化算法在概率空间进行优化。
...
Method: 将表面网格表示为概率测度，将表面配准转化为分布优化问题。使用计算复杂度为对数线性的切片Wasserstein距离衡量网格差异。提出AdamFlow优化方法，将Adam优化器从欧几里得空间推广到概率空间，用于最小化切片Wasserstein距离。

Result: 从理论上分析了AdamFlow的渐近收敛性，并通过实验证明其在各种解剖结构的仿射和非刚性表面配准中具有优越性能。

Conclusion: 该方法通过将表面配准转化为分布优化问题，使用高效的切片Wasserstein距离和AdamFlow优化算法，实现了既高效又鲁棒的表面配准，在医学图像解剖形状分析中具有重要应用价值。
...
TL;DR: 提出场景化视觉定位新任务，引入RSC基准测试，包含段落级场景描述查询，需要基于角色、意图和关系推理而非显式命名进行目标定位，并提出ScenGround课程学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型可通过匹配显著命名类别成功。需要探索更具挑战性的场景化视觉定位设置，其中目标必须从角色、意图和关系上下文推断而非显式命名。
...
Result: RSC包含约31k训练样本、4k域内测试样本和3k分布外分割。实验显示场景化查询暴露当前模型系统性失败，而标准基准无法揭示；课程训练提升挑战性切片性能并迁移到标准基准。

Conclusion: 场景化视觉定位是现有基准的补充且更具挑战性，RSC基准支持细粒度分析，ScenGround方法为这一设置提供参考点，显示场景理解对深度视觉推理的重要性。
...
TL;DR: ModMap：一种原生多视角多模态的3D异常检测与分割框架，通过跨模态特征映射和视角相关调制实现多视角信息融合，在SiM3D基准上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 现有3D异常检测方法通常独立处理不同视角，缺乏对多视角和多模态信息的有效融合。工业场景中需要同时利用深度图和RGB图像等多模态数据，并建模视角间的依赖关系以提高检测精度。

Method: 1. 提出跨模态特征映射范式，学习跨模态和跨视角的特征映射；2. 通过特征级调制显式建模视角依赖关系；3. 引入跨视角训练策略，利用所有可能的视角组合；4. 通过多视角集成和聚合进行异常评分；5. 为处理高分辨率3D数据，训练并公开了针对工业数据集的基础深度编码器。

Result: 在SiM3D基准测试（首个多视角多模态3D异常检测与分割基准）上，ModMap取得了最先进的性能，以较大优势超越了先前方法。

Conclusion: ModMap通过原生多视角多模态框架有效解决了3D异常检测中的视角融合问题，其跨模态特征映射和视角相关调制方法为多视角工业检测提供了有效解决方案。
...
TL;DR: 提出一个从AAA游戏提取的大规模动态数据集，包含4M连续帧的RGB和G-buffer数据，用于提升双向渲染的域泛化能力，并引入基于VLM的无真值评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集在真实性和时序一致性上存在局限，导致生成式逆渲染和前向渲染在真实场景中应用时存在域差距。需要更真实、大规模、动态的数据集来弥合这一差距。

Method: 1) 采用新颖的双屏拼接捕获方法从视觉复杂的AAA游戏中提取数据；2) 收集4M连续帧(720p/30FPS)的同步RGB和五个G-buffer通道；3) 提出基于视觉语言模型(VLM)的评估协议，从语义、空间和时序一致性三个维度评估逆渲染性能；4) 提供工具包支持基于G-buffer的文本提示编辑。

Result: 1) 数据集包含多样场景、视觉效果和环境条件(包括恶劣天气和运动模糊变体)；2) 在数据集上微调的逆渲染器展现出优异的跨数据集泛化能力和可控生成性能；3) VLM评估结果与人类判断高度相关；4) 前向渲染器支持用户通过文本提示编辑AAA游戏的G-buffer风格。

Conclusion: 该大规模动态数据集有效弥合了合成数据与真实场景之间的域差距，显著提升了逆渲染和前向渲染的性能。提出的VLM评估协议为无真值评估提供了可靠方法。整体方案推动了双向渲染在真实世界应用的发展。
...
TL;DR: 该论文研究了在线交流中常见的重复延长形式（RLF）在情感分析中的重要性，创建了首个多领域RLF数据集Lengthening，并提出了ExpInstruct两阶段指令微调框架来提升LLM对RLF的理解和可解释性。


<details>
  <summary>Details</summary>
Motivation: 在线交流中人们经常使用非正式表达方式（如表情包、表情符号），但一种独特且强调的重复延长形式（RLF）多年来一直被忽视。论文旨在探索两个研究问题：1）RLF对情感分析是否重要？2）语言模型能否理解RLF？

Method: 1）基于先前语言学研究，创建了首个专注于RLF的多领域数据集Lengthening（85万个样本）；2）提出了ExpInstruct两阶段指令微调框架，旨在提升LLM对RLF的性能和可解释性；3）提出了一种新颖的统一方法来量化语言模型对非正式表达的理解。

Result: 1）RLF句子是富有表现力的表达方式，可以作为文档级情感的签名；2）RLF对在线内容分析具有潜在价值；3）微调的预训练语言模型在性能上可以超越零样本GPT-4，但在解释性上不如GPT-4；4）ExpInstruct能够用有限样本提升开源LLM，使其在RLF的性能和可解释性上匹配零样本GPT-4。

Conclusion: RLF在情感分析中具有重要意义，语言模型能够通过专门的数据集和微调框架来理解和处理这种非正式表达形式。ExpInstruct框架有效提升了LLM对RLF的理解能力和可解释性，为在线内容分析提供了新的工具和方法。
...
TL;DR: 通过强化学习和并行思考两种方法扩展推理令牌预算，用于竞争性编程任务，在AetherCode的456个难题上超越GPT-5-high


<details>
  <summary>Details</summary>
Motivation: 解决竞争性编程中单次生成推理在强化学习训练时计算成本过高的问题，同时探索如何有效扩展推理令牌预算以提高模型性能

Method: 1. 训练时强化学习：采用验证RL预热和随机剪枝策略优化训练轨迹；2. 测试时并行思考：设计多轮并行思考管道，将令牌预算分配到多个线程和轮次，进行生成、验证和精炼

Result: 基于Seed-OSS-36B模型，完整系统使用16线程和每线程16轮，平均每个问题消耗760万令牌，在pass@1指标上匹配了基础RL模型的oracle pass@16，并在AetherCode的456个难题上超越了GPT-5-high

Conclusion: 结合训练时强化学习和测试时并行思考的混合方法能有效扩展推理令牌预算，显著提升竞争性编程任务的性能，为大规模推理系统提供了可行的技术路径
...
TL;DR: M2-Verify是一个大规模多模态数据集，用于评估科学主张与多模态证据之间的一致性，包含超过46.9万个实例，覆盖16个领域，模型在复杂场景下表现不佳且存在幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏规模、领域多样性和视觉复杂性，无法真实评估科学主张与多模态证据之间的一致性，需要创建更全面的评估数据集。

Method: 从PubMed和arXiv收集数据构建M2-Verify数据集，包含超过469K个实例，覆盖16个领域，通过专家审核进行严格验证，并进行广泛的基线实验评估模型性能。

Result: 最先进模型在保持一致性方面表现不佳：在低复杂度医学扰动上达到85.8% Micro-F1，但在高复杂度挑战（如解剖结构变化）上性能降至61.6%，专家评估还发现模型在生成科学解释时存在幻觉。

Conclusion: M2-Verify填补了现有基准测试的空白，揭示了当前模型在科学主张一致性评估方面的局限性，提供了数据集使用指南，并展示了其实用价值。
...
Method: 提出混合方法：在文本表示基础上增强可解释信号特征，包括响应长度、拒绝指示器、毒性分数和提示-响应语义相似度，以显式捕捉有用性、安全性和相关性等关键方面。使用SHAP和LIME提供细粒度可解释性分析。

Result: 特征增强框架在所有测试的十个大型语言模型上均带来一致改进，最高达到0.84 ROC AUC和显著更高的成对准确率，其中DeBERTav3Large表现最佳。可解释性分析显示模型决策依赖于情境化安全和支持性框架而非孤立关键词。

Conclusion: 通过结合文本和结构化特征的多维方法能有效改进偏好学习，揭示了模型决策机制和特征交互对偏好学习的影响，为构建更可靠、可解释的偏好学习系统提供了方向。
...
Method: 引入Reasoning Memory框架：1）从逐步推理轨迹语料库中分解出3200万个自包含的子问题-子程序对；2）推理时通过轻量级思维内提示让模型表达核心子问题，检索相关子程序作为隐式程序先验。

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory始终优于基于文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时缩放基线。在更高推理预算下，相比无检索提升达19.2%，相比最强计算匹配基线提升7.9%。

Conclusion: 性能提升源于两个关键因素：源轨迹的广泛程序覆盖度，以及分解和检索设计，共同实现了程序性知识的有效提取和重用。
...
TL;DR: 该研究通过因果干预实验发现，语言模型在回答实体相关事实问题时，存在稀疏的、可因果操作的MLP神经元，这些神经元在早期层集中，能够实现紧凑的实体检索而非纯粹的深度渐进丰富。


<details>
  <summary>Details</summary>
Motivation: 语言模型能够回答许多以实体为中心的事实问题，但其内部机制尚不明确。研究旨在探究语言模型在实体相关事实检索过程中的内部工作机制，特别是是否存在稀疏的、可因果操作的神经元结构。

Method: 使用模板化提示定位实体选择性MLP神经元，在PopQA基准的200个实体上进行因果干预验证。通过负向消融（产生实体特异性遗忘）和受控注入（在占位符标记处注入）来测试神经元的因果效应，并与平均实体和错误细胞对照进行比较。

Result: 定位的神经元集中在早期层；负向消融导致实体特异性遗忘，受控注入能提高答案检索准确性；许多实体仅需激活单个神经元即可恢复实体一致性预测；神经元对别名、缩写、拼写错误和多语言形式具有鲁棒性，支持规范化解释；效应强但不普遍，流行实体覆盖率更高。

Conclusion: 研究识别出稀疏的、可因果操作的访问点，用于分析和调节实体条件化的事实行为，表明语言模型通过紧凑的实体检索机制而非纯粹的深度渐进丰富来处理实体相关事实问题。
...
<details>
  <summary>Details</summary>
Motivation: 翻译过程中的打字停顿通常被用来区分自动化翻译过程与需要认知处理的翻译难题，但如何确定区分这两种过程的停顿阈值一直存在争议

Method: 比较三种最近的计算停顿阈值方法，并提出和评估一种新的生产单元中断计算方法

Result: 未在摘要中明确说明具体结果，但暗示了对不同方法的比较评估

Conclusion: 需要更可靠的方法来确定翻译过程中的停顿阈值，以区分自动化翻译与认知处理过程
...
TL;DR: MiCP是首个用于多轮推理的保形预测框架，通过在不同轮次分配错误预算，使模型能在保持总体覆盖保证的同时提前停止，减少推理轮次和成本。


<details>
  <summary>Details</summary>
Motivation: 现有LLM多轮推理方法（如自适应RAG和ReAct）依赖启发式停止规则或固定轮次预算，缺乏形式化保证，无法平衡准确性与效率。在高风险领域（金融、医疗）中，过早停止可能导致错误决策，而过晚停止则增加成本和延迟。

Method: 提出Multi-Turn Language Models with Conformal Prediction (MiCP)框架，将保形预测扩展到多轮推理场景。MiCP在不同推理轮次间分配错误预算，使模型能够自适应地决定何时停止，同时保持总体覆盖保证。该方法适用于自适应RAG和ReAct等场景。

Result: 在单跳和多跳问答基准测试中，MiCP实现了目标覆盖保证，同时显著减少了推理轮次、推理成本和预测集大小。还提出了新的联合评估指标，同时衡量覆盖有效性和回答效率。

Conclusion: MiCP是首个为多轮推理提供形式化覆盖保证的保形预测框架，解决了现有方法在停止决策上的局限性，在保持准确性的同时提高了效率，特别适用于高风险应用场景。
...
TL;DR: 提出基于预测效度的基准测试质量评估方法，通过WILD数据集分析不同技术在预算约束下预测模型在未见任务上性能的能力，展示改进的多维IRT模型结合自适应项目选择能高效预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试数量庞大但冗余，研究表明LLM性能可由少量潜在能力解释，需要更高效、原则性的基准测试方法。当前缺乏比较不同基准测试方法质量的统一框架，因此提出以预测效度为基础，评估基准测试框架在预测模型在未见任务上性能的效率。

Method: 收集WILD数据集（65个模型在163个任务的109,564个独特项目上的响应对），采用改进的多维项目反应理论（IRT）模型，结合基于最优实验设计的自适应项目选择策略，并引入成本感知折扣因子优化评估成本。

Result: 改进的多维IRT模型结合自适应项目选择能在观察仅16个项目后，以平均绝对误差（MAE）小于7%预测112个保留基准任务的性能。引入成本感知折扣因子后，达到7% MAE所需总token数从141,000降至22,000，评估成本降低85%。

Conclusion: 基于预测效度的基准测试评估框架能显著提高评估效率，改进的多维IRT模型结合自适应项目选择策略在有限预算下能准确预测模型在未见任务上的性能，为高效、原则性的基准测试提供了可行方案。
...
TL;DR: 该研究使用词嵌入和随机森林模型，成功仅基于分布数据区分了印地语中梵语和波斯-阿拉伯语来源的同义词，表明使用模式保留了词源信号，同义词反映了与起源相关的系统差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证一个语言学假设：即使同义词表示相同概念，它们可能反映不同视角或携带不同的文化关联。印地语中梵语和波斯-阿拉伯语借词共存形成了大量同义词对，为定量测试这一假设提供了理想环境。研究旨在探索仅基于分布数据（不考虑语义内容）能否区分这些词的词源。

Method: 研究方法包括：1）收集印地语中梵语和波斯-阿拉伯语来源的同义词对；2）为这些词生成词嵌入；3）使用随机森林分类器训练模型，仅基于词嵌入的分布特征来分类词的起源；4）测试模型在语义无关的同义词上的表现。

Result: 随机森林模型成功区分了梵语和波斯-阿拉伯语来源的印地语同义词，即使在语义无关的情况下也能准确分类。这表明使用模式保留了词源信号，上下文编码了词源信息。同义词确实反映了与起源相关的系统差异，词源相关的词可能形成不同的概念子空间。

Conclusion: 研究结论是：1）上下文能够捕捉超越传统语义相似性的细微区别；2）同义词可以提供不同视角；3）词源相关的词形成由历史起源塑造的新型语义框架；4）分布数据单独就能编码词源信息，为"同义词反映不同视角"的假设提供了定量证据。
...
TL;DR: 研究发现引用粒度对模型性能有显著影响，段落级引用在归因质量上表现最佳，而句子级引用会破坏语义依赖关系，导致归因质量下降16-276%


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索引用粒度（句子级、段落级、文档级）对归因生成模型性能的影响。虽然细粒度引用通常被认为便于人类验证，但其对模型性能的具体影响尚未得到充分研究。

Method: 方法包括分析四个不同规模的模型（8B-120B），比较不同引用粒度下的归因质量。研究评估了句子级、段落级和多段落级引用对模型性能的影响，并分析了模型规模与最优引用粒度之间的关系。

Result: 结果显示：1）归因质量在中间粒度（段落级）达到峰值；2）细粒度（句子级）引用使归因质量下降16-276%；3）过粗的引用（多段落级）引入干扰噪声；4）性能差距随模型规模非单调变化，细粒度约束对更大模型影响更严重；5）最优引用粒度在提升归因质量的同时保持甚至改善答案正确性。

Conclusion: 结论是单纯为人类验证优化细粒度引用会忽视模型约束，损害归因忠实性和生成可靠性。有效的归因需要将引用粒度与模型的自然语义范围对齐，段落级引用在模型性能和人类验证需求之间取得了最佳平衡。
...
TL;DR: 该论文对LLM中过度自信的言语表达进行了电路级机制分析，发现特定MLP块和注意力头在中间到后期层中写入置信度膨胀信号，并可通过针对性干预改善校准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生事实错误的答案，并且会过度自信地表达这些错误答案，这种言语化的过度自信会误导用户并削弱置信度分数作为可靠不确定性信号的可靠性，但其内部机制尚不清楚。

Method: 采用电路级机制分析方法，围绕三个轴展开：将言语化置信度捕获为可微分的内部信号，识别导致其膨胀的电路，并利用这些见解进行针对性推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验。

Result: 发现一组紧凑的MLP块和注意力头（集中在中间到后期层）在最终标记位置一致地写入置信度膨胀信号。针对性推理时干预这些电路显著改善了校准效果。

Conclusion: LLM中的言语化过度自信由可识别的内部电路驱动，可通过针对性干预得到缓解。这为理解LLM不确定性表达的内部机制提供了新的见解。
...
<details>
  <summary>Details</summary>
Motivation: 传统计算分析往往将波斯诗歌材料扁平化为孤立的词语或宽泛的文档语义，忽略了波斯诗学中一个实用的组织单位：相关形式以家族形式传播，并通过重复关系获得力量。研究旨在捕捉这种动态的象征系统演变。

Method: 使用包含129,451首诗歌的语料库，将重复形式整合为可追溯的家族，将意象材料与神圣和宫廷参考分离，并在多层图中映射它们的关系。按11个伊斯兰世纪进行分箱分析。

Result: 象征核心相对稀疏，指称成分更密集，两者之间的连接区域是选择性的而非扩散的。某些家族（如Shab、Ruz、Khaak）广泛分布，而酒器、花园空间、火焰和抒情声音在后期增强，声望编码和英雄宫廷词汇在早期更重。模块性增加，跨范围连接减少，宫廷桥梁减弱，神圣桥梁增强。枢纽位置也发生变化。

Conclusion: 波斯诗歌象征系统不是固定的剧目，而是一个长期存在的系统，其内部权重和连接随时间变化，展现了动态的演变过程。
...
TL;DR: 论文提出了Magic, Madness, Heaven, Sin框架，将大语言模型输出变化建模为同质-异质轴，基于任务规范性目标进行分类，并分析了跨上下文交互的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于输出变化的术语分散且混乱，主要原因是任务的规范性目标很少被明确说明。研究者需要统一的框架来理解不同任务背景下输出变化的评估标准。

Method: 提出Magic, Madness, Heaven, Sin框架，将输出变化建模为同质-异质轴，并根据规范性目标将任务组织为四个上下文：认知性（事实性）、交互性（用户效用）、社会性（代表性）和安全性（鲁棒性）。分析每个上下文的失败模式和术语，并应用框架分析所有成对的跨上下文交互。

Result: 框架揭示了优化一个目标（如提高安全性）可能无意中损害人口统计代表性或创造性多样性。跨上下文分析显示了不同规范性目标之间的权衡关系。

Conclusion: 输出变化应被视为由任务目标塑造的属性，而非模型的内在特性。需要基于上下文感知的评估方法，明确任务的规范性目标来指导模型设计和评估。
...
<details>
  <summary>Details</summary>
Motivation: 研究指令遗忘范式是否适用于扩散模型等非语言生成模型，验证基于自然语言指令的遗忘方法在图像生成模型中的有效性

Method: 通过控制实验测试多个概念和提示变体，分析CLIP文本编码器和去噪过程中的交叉注意力动态，测量目标概念token的注意力变化

Result: 扩散模型在仅依赖自然语言遗忘指令时系统性地无法抑制目标概念，遗忘指令未能持续减少对目标概念token的注意力，导致目标概念表征在整个生成过程中持续存在
...
<details>
  <summary>Details</summary>
Motivation: 先前研究将HTML的冗长视为性能障碍，普遍采用观察缩减作为标准做法。本研究重新审视这一趋势，探索不同观察表示对Web智能体性能的影响，特别是考虑模型能力和思考token预算的因素。

Method: 通过实验分析不同模型在不同观察表示（HTML vs 无障碍树）下的性能表现，考虑模型能力差异和思考token预算的影响。进行错误分析以理解模型如何利用不同表示，并探索观察历史的影响以及基于差异的表示方法。

Result: 发现两个关键结果：(1) 紧凑观察适合低能力模型，详细HTML对高能力模型更有利，增加思考token会放大HTML的优势；(2) 错误分析显示高能力模型利用HTML中的布局信息进行更好的动作定位，而低能力模型在长输入下出现更多幻觉。还发现观察历史能提升大多数模型和设置下的性能，基于差异的表示提供了token高效的替代方案。

Conclusion: 提出实用指南：基于模型能力和思考token预算自适应选择观察表示，并使用基于差异的表示来整合观察历史。挑战了先前普遍采用观察缩减的做法，强调需要根据具体情境优化观察表示策略。
...
TL;DR: 提出模型合并框架，通过合并临床基础模型与通用指令模型来解决医学领域微调中的灾难性遗忘问题，在保持指令跟随能力的同时提升临床任务性能
...
Result: 合并模型能有效缓解灾难性遗忘，保留临床领域专业知识，保持指令跟随能力；在严重受限监督下（如64-shot vs 256-shot）达到与完全微调基线相当的性能
...
TL;DR: DeltaMem：基于强化学习的单智能体记忆管理系统，通过记忆编辑距离和强化学习优化人设记忆管理，在多个长期记忆基准测试中超越现有产品级基线


<details>
  <summary>Details</summary>
Motivation: 现有的人设中心记忆系统多为多智能体框架，存在信息丢失和场景适应性差的问题，导致性能不佳。需要更稳健、端到端的记忆管理解决方案。

Method: 提出DeltaMem单智能体记忆管理系统，将人设记忆管理建模为端到端任务。构建用户-助手对话数据集及操作级记忆更新标签，引入基于记忆的Levenshtein距离作为记忆更新奖励，设计定制化强化学习框架优化记忆管理能力。

Result: 在LoCoMo、HaluMem和PersonaMem等多个长期记忆基准测试中，DeltaMem（包括无训练和RL训练版本）均优于所有产品级基线方法。

Conclusion: DeltaMem通过单智能体端到端框架和强化学习优化，有效解决了多智能体记忆系统的信息丢失和脆弱性问题，在长期记忆管理任务中表现出色。
...
TL;DR: Swift-SVD：一种激活感知的闭式压缩框架，用于大语言模型的权重和KV缓存压缩，保证理论最优、实践高效和数值稳定，相比现有方法实现3-70倍加速。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的部署受到静态权重和动态键值缓存的内存与带宽需求的限制。现有的SVD压缩方法存在两个关键问题：一些方法在重构误差上不是最优的，而另一些方法虽然理论最优但实践效率低下。

Method: 提出Swift-SVD框架：1）增量聚合给定输入批次下输出激活的协方差，在聚合后执行单次特征值分解，实现免训练、快速且最优的逐层低秩近似；2）使用有效秩分析局部逐层可压缩性；3）设计动态秩分配策略，联合考虑局部重构损失和端到端层重要性。

Result: 在6个大语言模型和8个数据集上的广泛实验表明，Swift-SVD优于现有最先进基线，在实现最优压缩精度的同时，端到端压缩时间加速3-70倍。

Conclusion: Swift-SVD提供了一个硬件友好的解决方案，同时保证理论最优性、实践效率和数值稳定性，有效解决了大语言模型部署中的内存和带宽限制问题。
...
TL;DR: 对印度尼西亚349名K-12教师的全国性调查显示，AI在教学、内容开发和教学媒体中的应用正在增加，但采用不均衡，存在基础设施、情境适配等障碍。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在印度尼西亚课堂中开始应用，但缺乏大规模、以教师为中心的实证证据，这阻碍了开发适合当地情境的AI系统和政策。

Method: 对印度尼西亚小学、初中和高中349名K-12教师进行全国性调查，收集关于AI使用情况、需求和挑战的数据。

Result: AI在教学法、内容开发和教学媒体中的应用增加，但采用不均衡：小学教师使用更一致，高中教师参与较少；中年教师更重视AI，东部地区教师认为AI价值更大。教师主要用AI减少教学准备工作量，但通用输出、基础设施限制和情境适配不足阻碍有效整合。

Conclusion: 需要针对不同教育阶段、职业阶段和地区的差异化支持策略，以及解决基础设施和情境适配问题，才能促进AI在印度尼西亚课堂的有效整合。
...
TL;DR: 该研究分析了9个声明验证数据集的推理模式，发现现有基准主要测试证据提取能力，而多句子综合和数值推理严重不足，高基准分数主要反映检索+蕴含能力而非复杂推理。


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对现有基准实际测试的推理能力的系统理解。研究者希望揭示这些数据集真正评估的推理类型，识别其局限性，并为构建更具挑战性的评估套件提供指导。

Method: 使用GPT-4o-mini为9个数据集的24K个声明验证示例生成结构化推理轨迹，分析推理模式分布。使用紧凑的1B参数推理验证器识别五种错误类型，并分析不同领域的错误分布特征。

Result: 发现直接证据提取占主导地位（约80%），多句子综合（约15%）和数值推理（约5%）严重不足。数据集层面存在显著偏差：某些数据集几乎只测试词汇匹配，而其他数据集约一半案例需要信息综合。错误分析显示：通用领域验证主要受词汇重叠偏差影响，科学验证由过度谨慎主导，数学验证则因算术推理失败。

Conclusion: 当前声明验证基准的高分数主要反映检索+蕴含能力，而非复杂推理能力。需要构建更具挑战性的评估套件，更好地测试验证系统所需的推理能力，特别是多句子综合和数值推理方面。
...
TL;DR: 研究发现监督微调中不同来源的CoT轨迹对模型泛化能力有显著影响：DeepSeek-R1数据训练损失更低但泛化更差，因其轨迹发散且分支多；GPT-oss-120b轨迹收敛且演绎性强，泛化更好。通过过滤频繁分支轨迹可提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究不同来源的验证CoT轨迹如何影响大语言模型的泛化性能，特别是当训练损失与泛化能力出现矛盾时（训练损失低但泛化差），需要深入理解这一现象背后的机制。

Method: 对比研究使用DeepSeek-R1-0528和GPT-oss-120b生成的CoT轨迹进行SFT，控制问题集相同。进行多层面分析：token级SFT损失和步骤级推理行为分析。提出过滤频繁分支轨迹的简单有效方法。

Result: 发现训练损失与泛化能力的矛盾：DeepSeek-R1数据训练损失更低但泛化显著更差。分析揭示DeepSeek-R1轨迹发散、分支多，GPT-oss-120b轨迹收敛、演绎性强。过滤频繁分支轨迹后，DeepSeek-R1子集训练在AIME25提升5.1%，BeyondAIME提升5.5%，五个基准平均提升3.6%。

Conclusion: CoT轨迹的质量不仅取决于正确性，推理模式（收敛vs发散）对SFT泛化能力有决定性影响。过滤频繁分支轨迹是提升SFT泛化性能的有效策略，为高质量CoT数据筛选提供了新视角。
...
TL;DR: EndoASR：针对胃肠内窥镜场景优化的领域自适应语音识别系统，通过两阶段适应策略显著提升专业术语识别准确率和实时性能，在多中心临床验证中表现稳健。


<details>
  <summary>Details</summary>
Motivation: 传统ASR系统在胃肠内窥镜临床环境中可靠性有限，主要受限于领域专业术语和复杂声学条件，需要开发专门针对内窥镜工作流程优化的实时ASR系统。

Method: 采用两阶段适应策略：基于合成内窥镜报告进行领域特定语言建模和噪声鲁棒性优化；系统设计紧凑（220M参数），实现高效边缘部署和实时处理（RTF 0.005）。

Result: 在回顾性评估中，字符错误率从20.52%降至14.14%，医学术语准确率从54.30%提升至87.59%；前瞻性多中心研究中，CER从16.20%降至14.97%，Med ACC从61.63%提升至84.16%；实时性能显著优于Whisper-large-v3。

Conclusion: 领域自适应的ASR系统可作为胃肠内窥镜人机协作的可靠接口，在多中心真实临床环境中表现一致，改进的ASR质量直接增强了下游结构化信息提取和临床医生-AI交互。
...
TL;DR: 该论文系统综述并比较了基于大语言模型的智能体中的记忆方法，提出了统一框架，在标准基准上全面评估现有方法，并设计出优于现有方法的新记忆方法。


<details>
  <summary>Details</summary>
Motivation: 记忆在基于大语言模型的智能体中对于处理长时程复杂任务（如多轮对话、游戏、科学发现）至关重要，但目前各种记忆方法缺乏在相同实验设置下的系统比较。

Method: 1. 从高层次视角总结了一个包含所有现有智能体记忆方法的统一框架；2. 在两个知名基准上广泛比较代表性记忆方法；3. 通过利用现有方法中的模块设计新的记忆方法。

Result: 1. 提供了对现有记忆方法的全面分析；2. 设计的新记忆方法在实验中优于现有最先进方法；3. 识别了现有方法的有效性和局限性。

Conclusion: 通过对现有记忆方法的深入理解，为未来研究提供了有价值的见解和方向，并展示了通过组合现有模块可以设计出更优的记忆方法。
...
Method: 提出人机协作框架，核心是LLM推理机制。使用声学特征模型提供置信度和特征级证据，通过置信度路由区分简单和模糊样本。模糊样本交由LLM进行深度推理，推理过程遵循从人类标注行为中提取的结构化规则。采用迭代优化策略，通过错误分析和规则更新持续改进系统。

Result: 在包含2,764个样本的越南语语音数据集上进行实验，涵盖平静、愤怒、恐慌三类情感，标注者间一致性高（Fleiss Kappa = 0.8574）。最佳准确率达到86.59%，Macro F1分数在0.85-0.86之间，有效处理了模糊和难以分类的样本。
...
<details>
  <summary>Details</summary>
Motivation: 当前在线内容毒性检测方案经常误判有价值信息，包括医学术语和少数群体相关内容。需要一种更细致的方法来识别保加利亚语文本中的毒性内容，同时保护重要信息的访问。
...
Conclusion: 研究提出了一种有效的保加利亚语毒性内容检测方法，通过本体建模和BERT分类器实现了高精度检测，同时避免了有价值信息的误判，具有广泛的应用潜力。
...
TL;DR: LiveMathematicianBench：基于arXiv最新论文构建的动态多选基准，用于评估LLMs在研究生水平数学推理上的能力，通过证明草图引导的干扰项和替换抵抗机制防止数据污染和表面匹配。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准存在合成设置和数据污染的局限性，无法真实评估LLMs在研究生水平数学推理上的能力。随着LLMs越来越多地集成到科学工作流程中，需要建立一个能够反映真实研究场景、抵抗数据污染的评估基准。

Method: 1) 基于arXiv最新论文（发表于模型训练截止日期后）构建动态多选基准；2) 引入13类逻辑分类法对定理类型进行分类；3) 采用证明草图引导的干扰项生成管道，利用高级证明策略构建看似合理但无效的答案选项；4) 引入替换抵抗机制区分答案识别与实质推理；5) 实施双模式协议评估证明草图访问对推理的影响。

Result: 基准远未饱和：最佳模型Gemini-3.1-pro-preview仅达到43.5%准确率。在替换抵抗评估下，准确率大幅下降：GPT-5.4最高为30.6%，Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）。证明草图访问带来一致的准确率提升，表明模型能够利用高级证明策略进行推理。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的研究级数学推理测试平台，能够更真实地评估LLMs的数学推理能力，揭示当前模型在实质性数学理解方面仍存在显著局限性。
...
TL;DR: 本文提出基于指令微调和离散控制令牌的领域无关可控自动文本简化框架，发现小模型（1-3B）在充分数据变化下可达到竞争性表现，但现有语料库中压缩控制信号不足，且标准评估指标无法有效衡量控制效果。


<details>
  <summary>Details</summary>
Motivation: 当前可控自动文本简化（CATS）通常被视为解码问题，且评估指标无法有效反映控制效果。研究发现CATS的可控性受到数据和评估方法的显著限制，需要更系统的框架来提升可控性评估。
...
Result: 小模型（1-3B）在训练数据包含足够目标属性变化时可达到竞争性表现；可读性控制学习一致，但压缩控制因现有语料库信号变化有限而表现不佳；标准简化和相似性指标不足以衡量控制效果，需要基于误差的度量方法；朴素数据分割会导致分布不匹配问题。

Conclusion: 可控自动文本简化的有效性取决于训练数据中目标属性的变化程度，需要更合适的评估指标来衡量控制效果，数据分割策略对训练和评估有重要影响，压缩控制需要更丰富的数据信号。
...
TL;DR: DEFT框架通过数据筛选和分布引导，提升LLM对齐效率和性能，同时保持泛化能力


<details>
  <summary>Details</summary>
Motivation: RLHF使用PPO等方法对齐LLM与人类价值观，但成本高、不稳定。现有替代方法需要大量数据学习偏好，可能削弱LLM泛化能力。需要更高效的对齐方法，同时保持泛化能力。

Method: 提出Distribution-guided Efficient Fine-Tuning (DEFT)框架，包含数据筛选和分布引导。基于语言模型输出分布和偏好数据差异分布计算差分分布奖励，从原始数据中筛选高质量子集，并整合到现有对齐方法中引导模型输出分布。

Result: DEFT增强的方法在对齐能力和泛化能力上均优于原始方法，且训练时间显著减少。

Conclusion: DEFT框架能有效提升LLM对齐效率和性能，同时缓解泛化能力损失问题，为高效对齐提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM偏好学习方法存在性能提升有限、计算成本高、超参数敏感以及全局token级关系建模不足等问题，需要更有效的对齐方法

Method: PLOT通过最优传输问题形式化偏好学习，推导出token级损失函数，利用token嵌入捕捉语义关系，实现全局感知的优化，同时保持LLM原始分布

Result: 在人类价值观与逻辑问题解决两个偏好类别、七个子偏好的实验中，PLOT持续提升对齐性能，同时保持流畅性和连贯性

Conclusion: 最优传输为LLM偏好学习提供了理论基础，建立了理论严谨的框架，为LLM偏好学习提供了新见解
...
TL;DR: 提出自适应占位符补全(APC)框架，通过在高熵位置输出显式占位符来替代硬补全(HC)范式，降低代码补全的预期编辑成本


<details>
  <summary>Details</summary>
Motivation: 分析300万真实交互发现硬补全(HC)范式存在局限：61%的生成建议在被接受后仍需编辑或虽与用户后续代码相似度超过80%但仍被拒绝，表明模型在特定token位置常做出错误预测

Method: 1) 将代码补全形式化为不确定性下的成本最小化问题；2) 从过滤的真实编辑日志构建训练数据；3) 设计基于成本的奖励函数进行强化学习；4) 提出自适应占位符补全(APC)框架，在高熵位置输出显式占位符

Result: 在1.5B-14B参数模型上的广泛评估表明，APC将预期编辑成本降低了19%到50%，同时保持了标准HC性能
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法过于关注融合而忽视了噪声特征的不确定性和细粒度推理，需要同时提升鲁棒性和上下文建模能力
...
Method: 在法国心衰队列上评估多种方法：1）仅文本模型；2）仅结构化数据模型；3）多模态融合模型；4）大语言模型方法。比较了CLS嵌入与实体级表示，以及不同模态融合策略

Result: 1）临床文本中增加实体级表示比仅用CLS嵌入提升预测性能；2）文本与结构化变量的监督多模态融合获得最佳整体性能；3）大语言模型在不同模态和解码策略下表现不一致，仅文本提示优于结构化或多模态输入

Conclusion: 实体感知多模态transformer为心衰短期结局预测提供最可靠解决方案，而当前LLM提示方法在临床决策支持方面仍有局限
...
<details>
  <summary>Details</summary>
Motivation: 宣传新闻特别隐蔽，因为它们将导向性信息与看似可靠的事实报道混合。现有基于BERT等语言模型的方法因数据收集偏差容易过拟合训练数据集，需要提高分类鲁棒性和对新来源的泛化能力。

Method: 提出神经符号方法，结合非上下文文本嵌入（fastText）与符号概念特征，包括体裁、主题和说服技巧，形成混合模型来检测宣传新闻。

Result: 结果显示该方法优于等效的纯文本方法，消融研究和可解释性分析证实了添加特征的益处，提高了分类鲁棒性和泛化能力。

Conclusion: 神经符号方法通过结合文本嵌入与符号概念特征，能有效提高宣传新闻检测的鲁棒性和泛化能力，避免过拟合训练数据偏差。
...
TL;DR: 提出一种测量词序变异相对于交换距离最小化最优程度的新数学框架，发现跨语言手势至少77%最优，并将二次分配问题引入语言研究作为统一多种优化问题的框架。


<details>
  <summary>Details</summary>
Motivation: 研究语言中词序变异是否遵循交换距离最小化原则，即给定源顺序时，在置换多面体中距离更近的词序应该成本更低、更可能出现。需要开发测量词序变异相对于交换距离最小化最优程度的数学方法。

Method: 提出测量词序变异相对于交换距离最小化最优程度的新数学框架。使用置换多面体表示序列所有排列的结构，其中顶点是排列，相邻元素交换产生新排列的顶点相连。将二次分配问题引入语言研究作为统一多种优化问题的框架。

Result: 跨语言手势至少77%最优，多次达到最优的情况不太可能是偶然的。建立了研究词序或手势顺序相对于交换距离最小化最优性的理论基础。

Conclusion: 提出了一个统一的最优分配一般原则，将交换距离最小化等多种语言原则统一起来。二次分配问题为语言研究提供了涵盖多种优化问题的统一框架。
...
TL;DR: 论文提出了一种三步自动化质量保证方法来评估机器翻译基准数据集的质量，发现低COMET分数数据集在span级别有更多准确性/误译错误，并发布了清洗后的EU20数据集


<details>
  <summary>Details</summary>
Motivation: 机器翻译的基准数据集虽然降低了成本并提供了规模，但噪声、结构丢失和质量不均等问题削弱了对其的信任。研究旨在开发可扩展的翻译可靠性测量和验证方法，而不仅仅是能否翻译

Method: 采用三步自动化质量保证方法：1) 结构语料库审计与针对性修复；2) 使用神经度量COMET（无参考和基于参考）进行质量分析，并与DeepL/ChatGPT/Google翻译服务比较；3) 基于LLM的span级别翻译错误分析

Result: 趋势一致：COMET分数较低的数据集在span级别显示出更高比例的准确性/误译错误（特别是HellaSwag；ARC相对干净）。基于参考的COMET在MMLU上针对人工编辑样本也指向相同方向。发布了清洗/校正后的EU20数据集和可复现代码
...
TL;DR: SAFE是一个动态基准测试框架，通过可验证的实体序列替代无根据的思维链，用于多跳问答任务，包含训练时和推理时验证两个阶段。


<details>
  <summary>Details</summary>
Motivation: 现有多跳问答基准测试经常奖励大型语言模型虚假的正确性，掩盖了无根据或有缺陷的推理步骤，需要转向更严谨的推理评估。

Method: 提出SAFE框架：1) 训练时验证：建立原子错误分类法和基于知识图谱的验证流程，消除标准基准中的噪声监督；2) 推理时验证：基于验证数据集训练反馈模型，实时检测无根据的推理步骤。

Result: 识别出高达14%的实例为不可回答；在推理时显著优于标准基线，平均准确率提升8.4个百分点，同时保证可验证的推理轨迹。

Conclusion: SAFE框架不仅暴露了现有基准测试的关键缺陷，还能在推理时提供可验证的推理轨迹，显著提升多跳问答的准确性和可靠性。
...
TL;DR: 扩散模型在连续域生成建模中表现优异，但在离散数据上应用困难。研究发现高斯扩散模型在采样离散分布时存在临界区间问题，导致样本质量下降，提出了结合自条件和q采样的改进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为连续域生成建模的标准方法，但应用于离散数据仍面临挑战。需要理解为什么高斯扩散模型（使用DDPM求解器）难以从离散分布中采样，这些分布在连续空间中表示为δ分布的混合。

Method: 使用随机层次模型作为玩具模型，识别出噪声数据密度变为多模态的临界采样区间。分析DDPM在该区间内进入低密度区域的问题，评估现有启发式方法（包括自条件和q采样）的效果。提出将自条件与在临界区间内从DDPM切换到q采样相结合的方法。

Result: 研究发现DDPM在临界区间内偶尔进入模态间的低密度区域，产生模型分布外的输入，从而降低样本质量。自条件和q采样能缓解此问题。结合自条件与在临界区间切换采样策略的方法，在文本、编程代码和蛋白质等多个领域的条件和非条件任务中提高了生成质量。

Conclusion: 高斯扩散模型在离散数据采样中存在临界区间问题，导致样本质量下降。通过结合自条件技术和在关键采样阶段使用q采样策略，可以有效改善离散数据的生成质量，为扩散模型在文本、代码和蛋白质等离散领域的应用提供了改进方案。
...
TL;DR: 研究分析了自监督语音模型训练过程中语言结构表征的出现时机，发现不同层次的语言结构在层间分布和学习轨迹上存在显著差异，这些差异与结构的抽象程度和时间尺度有关，且预训练目标定义层次对语言结构的组织方式有重要影响。


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型能学习有效的语音表征，但这些语言结构在训练过程中何时出现尚不清楚。研究旨在探究不同语言结构在模型训练中的编码时机和模式，理解预训练目标对语言结构学习的影响。

Method: 研究分析了六个在荷兰语语音上训练的Wav2Vec2和HuBERT模型，考察了不同检查点和各层对广泛语言结构的编码能力。通过分析层间模式和学习轨迹，探究了语言结构抽象程度、时间尺度整合以及预训练目标定义层次的影响。

Result: 发现不同层次的语言结构表现出显著不同的层间模式和学习轨迹。这些差异部分可由其与声学信号的抽象程度差异以及输入信息整合的时间尺度来解释。预训练目标的定义层次强烈影响语言结构的层间组织和学习轨迹，高阶预测任务（如迭代细化的伪标签）能诱导更大的并行性。

Conclusion: 自监督语音模型中语言结构的编码具有层次特异性，其出现时机和模式受结构抽象程度、时间尺度以及预训练目标定义层次的影响。理解这些因素有助于设计更有效的语音表征学习方法和模型架构。
...
TL;DR: 提出BidirLM方法，将因果生成式语言模型转换为双向编码器，通过系统消融实验确定关键适应因素，引入线性权重合并和轻量级多领域数据混合策略，并融合专业因果模型，在文本、视觉和音频表示基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成式语言模型转换为双向编码器的方法存在三个主要限制：缺乏对最优训练目标的共识、在大规模应用时出现灾难性遗忘、以及无法灵活整合专业生成模型生态系统。需要开发更有效、可扩展且灵活的转换方法。

Method: 1. 在Gemma3和Qwen3模型家族上进行系统消融实验，识别成功适应的关键因素，特别是常被忽略的先验掩码阶段的重要性；2. 引入双重策略：线性权重合并结合轻量级多领域数据混合，在不依赖原始预训练数据的情况下缓解灾难性遗忘；3. 通过合并专业因果模型来增强编码器，实现模态和领域特定能力的无缝迁移。

Result: 开发出BidirLM家族，包含五个编码器模型，在文本、视觉和音频表示基准测试中超越了现有替代方法。该方法开源且适用于任何因果解码器LLM。

Conclusion: 提出了一个系统性的方法将因果生成式语言模型转换为高性能双向编码器，解决了现有方法的局限性，实现了更好的表示学习性能，并为整合专业模型能力提供了灵活框架。
...
TL;DR: GOOSE提出了一种训练自由的推测解码框架，通过构建自适应脊柱树（深链高接受率上下文匹配令牌+宽分支低接受率替代令牌）来优化令牌接受率，实现1.9-4.3倍无损加速。


<details>
  <summary>Details</summary>
Motivation: 现有训练自由的推测解码方法在构建候选令牌树时未区分不同来源令牌的质量差异，而上下文匹配令牌和统计预测令牌的接受率存在显著差距（中位数约6倍），这为优化树结构提供了机会。

Method: GOOSE构建自适应脊柱树：将高接受率的上下文匹配令牌组织成深链（脊柱），在每个节点上为低接受率的统计预测令牌创建宽分支。这种各向异性结构突破了平衡树的深度限制，确保每步接受的令牌数至少与任一单独源相同。

Result: 在五个LLM（7B-33B）和五个基准测试上，GOOSE实现了1.9-4.3倍无损加速，在相同预算下比平衡树基线性能提升12-33%。

Conclusion: 通过识别不同令牌源的接受率差异并构建各向异性树结构，GOOSE显著提升了推测解码的效率，证明了区分令牌质量并相应调整树结构的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前重排序模型基于静态人工标注的相关性标签进行优化，与下游生成过程解耦，导致信息检索指标认为相关的文档往往无法为LLM提供生成精确答案所需的实际效用

Method: 提出重排序偏好优化(RRPO)框架，将重排序建模为顺序决策过程，使用LLM反馈优化上下文效用，并引入参考锚定确定性基线确保训练稳定性

Result: 在知识密集型基准测试中，RRPO显著优于包括强大的列表式重排序器RankZephyr在内的基线模型。框架具有良好泛化性，可无缝适配不同读者模型，与查询扩展模块正交集成，且在噪声监督下保持鲁棒

Conclusion: RRPO通过强化学习直接对齐重排序与LLM生成质量，有效解决了当前重排序模型与下游生成过程脱节的问题，为检索增强生成提供了更有效的文档排序方法
...
<details>
  <summary>Details</summary>
Motivation: 自监督语音模型已知对音位对比敏感，但对韵律对比的敏感性尚未直接测量。需要一种无需大量示例和显式标签的方法来评估模型对韵律特征的敏感性

Method: 1. 提出prosodic ABX框架，扩展传统ABX判别任务来评估韵律对比；2. 构建并发布英语和日语最小对数据集；3. 使用该数据集及已有普通话数据集评估英语重音、日语音高重音和普通话声调的对比敏感性；4. 分析模型和层级排名在不同实验条件下的稳定性

Result: 1. 成功开发prosodic ABX框架，仅需少量示例即可评估韵律对比；2. 模型和层级排名在多种实验条件下通常保持稳定，使其适用于低资源环境；3. 提供了英语、日语和普通话韵律对比的评估结果

Conclusion: prosodic ABX框架能够有效评估自监督语音模型对韵律对比的敏感性，且模型和层级评估结果具有跨条件稳定性，为低资源环境下的模型评估提供了实用工具
...
TL;DR: 该论文提出了一种稳定性过滤方法，通过识别和保留行为稳定的边界来构建更有效的推理行为转向向量，显著提升了数学推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于关键词匹配检测推理行为边界的方法存在严重缺陷，大多数检测到的边界（93.3%）行为不稳定，无法在相同前缀下重现目标行为，这导致转向信号被稀释，影响转向向量的有效性。

Method: 提出稳定性过滤方法，仅保留模型能一致重现目标行为的边界；结合内容子空间投影去除残差问题特定噪声；将内在推理行为形式化为具有上下文依赖触发概率的随机事件。

Result: 在MATH-500上达到0.784准确率（比最强基线提升5.0%）；生成的转向向量可在同一架构家族的不同模型间迁移，无需重新提取，分别提升Nemotron-Research-Reasoning-1.5B（+5.0）和DeepScaleR-1.5B-Preview（+6.0）。

Conclusion: 稳定性过滤和内容子空间投影能有效构建高质量的推理行为转向向量，解决了当前方法中行为边界不稳定导致的信号稀释问题，显著提升了转向向量在数学推理任务中的性能。
...
TL;DR: GaelEval是首个针对苏格兰盖尔语的多维度基准测试，包含形态句法MCQA、文化翻译和文化知识问答任务，评估19个LLM发现前沿模型在盖尔语语法任务上超越人类基准，专有模型优于开源模型，盖尔语提示有稳定优势。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在未官方支持的语言中表现出"影子"能力，但性能不均衡且缺乏测量，特别是对于形态句法丰富的少数民族语言如苏格兰盖尔语，现有翻译基准无法捕捉结构能力。

Method: 引入GaelEval基准：1)专家编写的形态句法多项选择题任务；2)文化基础的翻译基准；3)大规模文化知识问答任务。评估19个LLM，与30名流利说话者的人类基线对比，分析专有模型与开源模型差异，以及盖尔语提示效果。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超越人类基线(78.1%)。专有模型持续优于开源系统，盖尔语提示带来稳定优势(+2.4%)。在文化任务上，领先模型超过90%准确率，但多数系统在盖尔语提示下表现更差，绝对分数相对于人工基准偏高。

Conclusion: GaelEval显示前沿模型在盖尔语语法的多个维度上达到超人类表现，证明了盖尔语提示的效果，并揭示了专有模型相对于开源模型的持续性能优势，为少数民族语言评估提供了新基准。
...
TL;DR: 该论文在SOMD 2026软件提及共指消解任务中取得第二名，比较了两种免微调方法：模糊匹配和上下文感知表示，分析了它们在噪声环境和不同规模语料下的性能差异。


<details>
  <summary>Details</summary>
Motivation: 软件名称具有高度表面规律性，可能不需要复杂的语义推理，因此研究免微调方法在软件提及共指消解任务中的有效性，并分析不同方法在噪声环境和规模扩展下的表现。

Method: 采用两种免微调方法：1) 模糊匹配(FM)：基于词汇字符串相似度的方法；2) 上下文感知表示(CAR)：结合提及级和文档级嵌入的方法。通过噪声注入实验（边界噪声和提及替换）和推理时间分析来评估方法性能。

Result: 两种方法在所有子任务中都取得竞争性性能（CoNLL F1 0.94-0.96），CAR在官方测试集上始终比FM高1分。噪声实验显示互补的失败模式：边界噪声下CAR更稳健（F1下降0.07 vs 0.20），提及替换下FM更稳健（F1下降0.52 vs 0.63）。CAR的推理时间随语料规模近似线性增长，而FM呈超线性增长。

Conclusion: 系统选择应同时考虑上游提及检测器的噪声特征和目标语料的规模。软件名称的表面规律性降低了复杂语义推理的需求，CAR在大规模语料下是更高效的选择。作者发布了代码以支持这一未充分探索任务的研究。
...
TL;DR: Neuro-RIT是一种通过神经元引导的鲁棒指令调优框架，通过神经元级稀疏性处理检索增强语言模型中的噪声上下文问题


<details>
  <summary>Details</summary>
Motivation: 检索增强语言模型在知识密集型任务中表现出潜力，但对不相关或噪声检索上下文敏感。现有方法通常在层或模块级别进行粗粒度参数更新，忽略了大型语言模型固有的神经元级稀疏性。

Method: 提出Neuro-RIT框架，从密集适应转向精确驱动的神经元对齐。方法包括：1) 使用基于归因的神经元挖掘明确分离处理相关与不相关上下文的神经元；2) 引入两阶段指令调优策略，实现双重噪声鲁棒性能力：通过功能失活不相关上下文专属神经元实现直接噪声抑制，同时优化目标层进行证据蒸馏。

Result: 在多样化QA基准测试中的广泛实验表明，Neuro-RIT始终优于强基线方法和鲁棒性增强方法。

Conclusion: Neuro-RIT通过神经元级精确对齐有效解决了检索增强语言模型对噪声上下文的脆弱性问题，为鲁棒性增强提供了新范式。
...
TL;DR: L3TR是一个基于大语言模型的列表式人才推荐框架，通过块注意力机制、局部位置编码和ID采样方法，解决了传统点式方法的token消耗高、位置偏见和中间信息丢失问题。


<details>
  <summary>Details</summary>
Motivation: 现有的人才推荐系统大多采用点式范式，需要LLM重复处理文本，无法捕捉候选人之间的关系，导致token消耗高且推荐效果不佳。此外，LLM在处理多项选择题和长文档时存在位置偏见和中间信息丢失问题。

Method: 提出L3TR框架：1) 块注意力机制和局部位置编码方法，增强文档间处理能力并缓解位置偏见和并发token偏见；2) ID采样方法解决训练和推理阶段候选集大小不一致问题；3) 设计位置偏见和token偏见的检测方法及无需训练的去偏见方法。

Result: 在两个真实世界数据集上的广泛实验验证了L3TR的有效性，相比现有基线方法取得了持续改进。

Conclusion: L3TR框架通过列表式处理、块注意力机制和去偏见方法，有效解决了传统人才推荐系统中的效率问题和LLM偏见问题，为人才推荐任务提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语语音命名实体识别研究不足，主要由于阿拉伯语的形态复杂性、缺少短元音以及有限的标注资源。现有研究已证明端到端方法在英语、法语和中文上优于级联管道，但阿拉伯语领域仍待探索。

Method: 1. 创建CV-18 NER数据集：基于阿拉伯语Common Voice 18语料库，采用Wojood细粒度模式（21种实体类型）进行人工NER标注。2. 对比评估级联系统（ASR+文本NER）和端到端模型（基于Whisper和AraBEST-RQ）。3. 分析阿拉伯语特定自监督预训练与多语言弱监督对任务的影响。

Result: 端到端系统在测试集上显著优于最佳级联配置：AraBEST-RQ 300M达到37.0% CoER，Whisper-medium达到38.0% CVER。分析表明：阿拉伯语特定自监督预训练在ASR上表现良好，而多语言弱监督在联合语音到实体学习上转移更有效，大模型在低资源环境下可能更难适应。

Conclusion: 端到端方法在阿拉伯语语音NER任务上具有优势，特别是结合多语言弱监督时。CV-18 NER数据集和模型的公开为阿拉伯语语音NER研究提供了首个开放基准，促进了该领域的发展。
...
TL;DR: 论文研究如何从多个LLM中生成全面的有效回答集，提出多样性覆盖率指标，开发路由器选择最佳模型，在开放域提示上提升回答多样性


<details>
  <summary>Details</summary>
Motivation: 当提示允许大量有效答案时，全面生成这些答案是满足多样化用户需求的第一步。现有LLM在生成全面回答集方面表现不一，需要系统方法来选择最适合特定查询的模型。

Method: 提出多样性覆盖率指标评估回答集质量；评估18个LLM的多样性表现；基于评估结果开发路由器模型，预测每个查询的最佳生成模型；在NB-Wildchat数据集上训练路由器。

Result: 发现没有单一模型在所有开放域提示上表现最优，但每个提示都存在显著优于其他模型的特定模型；训练的路由器在NB-Wildchat上超越最佳单一模型基线（26.3% vs 23.8%）；在NB-Curated数据集和不同生成策略上显示出良好的泛化能力。

Conclusion: 该研究为在拥有多个模型时生成全面回答集奠定了基础，证明了基于查询的路由策略在提升回答多样性方面的有效性，为多模型系统设计提供了新思路。
...
TL;DR: 论文分析了语言模型扩展新词汇时均值初始化的缺陷，提出基于语言基础的初始化方法GTI，在生成推荐任务中表现优于传统方法


<details>
  <summary>Details</summary>
Motivation: 当前语言模型扩展新词汇时采用均值初始化策略，但作者通过谱分析和几何诊断发现这种方法会导致新词嵌入坍缩到退化子空间，消除词间差异，使得后续微调难以完全恢复，表明词初始化是词汇扩展的关键瓶颈

Method: 提出"基于语言的词初始化假设"，并实现为GTI方法：在微调前，仅使用配对的语言监督将新词映射到预训练嵌入空间中具有语义意义的独特位置，进行轻量级的基础化阶段

Result: GTI在大多数评估设置中优于均值初始化和现有辅助任务适应方法，覆盖工业规模和公共数据集的多项生成推荐基准测试；分析显示基于语言的嵌入产生更丰富的词间结构，并在微调过程中持续存在

Conclusion: 初始化质量是词汇扩展的关键瓶颈，基于语言的词初始化假设得到验证，GTI方法通过改善新词嵌入的初始结构，使模型能更好地利用通用知识处理新词领域任务
...
<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法依赖不可观测的反事实数据，通过假设（如可忽略性、平行趋势等）填补缺失信息，但都无法直接产生反事实结果本身。需要一种新框架将因果主张转化为可检验的形式。

Method: 提出数字孪生反事实框架：1) 将数字孪生模拟器形式化为潜在结果框架中的随机映射；2) 建立孪生保真度假设层次结构（边际保真度、联合保真度、结构保真度）；3) 设计五层验证架构将不可证伪的反事实主张转化为可检验的数据测试；4) 形式化分解因果量，区分边际可验证量（ATE、CATE、QTE）和依赖copula的量（ITE分布、获益/损害概率、处理效应方差）；5) 提供边界、敏感性和不确定性量化工具。

Result: 该框架不解决因果推断的根本问题，但提供：1) 边际因果主张变得可检验；2) 联合因果主张明确依赖假设索引；3) 两者之间的差距被形式化表征。将不可证伪的因果主张转化为可通过观测数据检验的形式。

Conclusion: 数字孪生反事实框架通过模拟而非估计反事实，建立了分层验证体系，使边际因果主张可检验，联合因果主张假设透明化，并形式化表征了可检验与不可检验因果量之间的差距，为因果推断提供了更严谨的验证基础。
...
TL;DR: IDEA2是一个集成大语言模型的半自动化工作流，通过专家在环的协作过程来加速本体工程中的能力问题提取，提高CQ的接受度和相关性。


<details>
  <summary>Details</summary>
Motivation: 本体工程中的能力问题提取是一个关键但资源密集的瓶颈阶段，存在领域专家（拥有知识）和本体工程师（形式化知识）之间的沟通鸿沟，需要更高效的协作方法。

Method: IDEA2采用半自动化工作流，核心是迭代循环：从需求文档中LLM初始提取CQ → 领域专家在协作平台上进行共创式评审和反馈 → LLM基于反馈迭代重新表述被拒绝的CQ直至达成共识。使用溯源模型跟踪每个CQ的完整生命周期。

Result: 在2个真实场景（科学数据、文化遗产）中验证，IDEA2能加速需求工程过程，提高最终CQ的接受度和相关性，在领域专家中表现出高可用性和有效性。

Conclusion: IDEA2成功解决了本体工程中CQ提取的瓶颈问题，通过LLM与专家协作的混合方法，在保持透明度和可重复性的同时，显著提高了CQ提取的效率和质量。
...
Method: 1. 提出世界中心多智能体系统（WMAS）概念框架；2. 从本体显性、规范性等维度对世界进行分类；3. 提出语义模型作为表示此类世界的数学形式化方法；4. 开发Ontobox平台作为WMAS的实现。

Result: 建立了WMAS的理论框架，提供了世界分类维度，提出了语义模型作为形式化表示方法，并实现了Ontobox平台作为WMAS的具体实现。

Conclusion: WMAS通过共享世界模型而非孤立的智能体局部表示，为结构化领域提供了语义一致性、可解释性和长期稳定性的解决方案，语义模型和Ontobox平台为其提供了理论和实践基础。
...
TL;DR: 该研究提出AI自动化是一个连续体，包含"崩溃浪潮"（AI能力在少数任务上突然激增）和"上升潮汐"（AI能力持续广泛提升）。基于对3000多个文本任务的评估，研究发现上升潮汐是主要形式，AI在广泛任务上表现良好且快速改进。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解AI自动化的发展模式，区分两种可能的路径：崩溃浪潮式的突然突破与上升潮汐式的持续广泛改进。这有助于预测AI对经济和劳动力市场的影响时间线。

Method: 使用美国劳工部O*NET分类中的3000多个文本任务进行评估，这些任务都是基于文本且LLM可处理的。通过17000多名相关职业工作者的评估，测量AI模型在不同任务上的表现。

Result: 发现很少证据支持崩溃浪潮模式（与METR近期研究相反），但大量证据表明上升潮汐是主要形式。AI在广泛任务上表现良好且快速改进：2024年第二季度AI模型能以约50%成功率完成人类需3-4小时的任务，预计到2025年第三季度将提升至65%。如果趋势持续，到2029年LLM在大多数文本任务上的平均成功率将达到80%-95%。

Conclusion: AI自动化主要通过上升潮汐模式发展，即能力在广泛任务上持续改进而非少数任务上的突然突破。这种渐进式改进意味着到2029年AI将能胜任大多数文本任务，但组织采用AI并影响经济和劳动力市场可能需要更长时间。
...
TL;DR: 该论文研究了LLM中的认知偏见，开发了LLM CogBias基准来评估四种偏见类型，发现偏见在激活空间中线性可分，并通过激活导向技术实现了26-32%的偏见减少。
...
Method: 1) 定义LLM认知偏见为系统性的、可复现的偏离正确答案的行为；2) 引入LLM CogBias基准，围绕四种认知偏见家族组织：判断偏见、信息处理偏见、社会偏见和反应偏见；3) 评估三种LLM；4) 使用对比设计下的线性探针分析偏见在激活空间中的表征；5) 应用激活导向技术来调节偏见行为。

Result: 1) 认知偏见在所有四种家族中系统性出现，其程度和去偏见响应强烈依赖于偏见家族；2) 提示级去偏见显著减少反应偏见，但对判断偏见产生反效果；3) 偏见在模型激活空间中编码为线性可分的方向；4) 激活导向实现了26-32%的偏见分数减少，同时在25个基准测试中保持下游能力；5) 尽管不同模型的偏见表征几乎正交，但导向在不同架构中以相似速率减少偏见，表明共享的功能组织。

Conclusion: LLM认知偏见在激活空间中具有可识别的线性表征，可以通过激活导向技术有效缓解，且这种缓解在不同模型架构间具有一致性，表明认知偏见在LLM中具有共享的功能组织基础。
...
TL;DR: 提出RIFT框架，用于系统化诊断评估大语言模型时使用的评分量规（rubric）的质量问题，包含8种失败模式，分为3大类，并通过人工标注和自动化指标验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 评分量规广泛用于大语言模型基准测试和训练中，但现有方法仅通过下游信号（如强化学习结果）评估量规质量，缺乏系统化诊断量规设计缺陷的方法。

Method: 基于扎根理论开发RIFT分类法，从五个多样化基准中迭代标注量规直至饱和；测量人工标注者间一致性；提出自动化量规质量指标并与人工标注对齐。

Result: RIFT包含8种失败模式（可靠性、内容效度、后果效度三类）；人工标注一致性达87%配对一致性和0.64平均Cohen's kappa；自动化指标与人工标注对齐，F1最高达0.86。

Conclusion: RIFT为系统化诊断评分量规质量提供了框架，支持可扩展的质量评估，自动化指标与人工标注良好对齐，有助于改进大语言模型评估和训练中的量规设计。
...
<details>
  <summary>Details</summary>
Motivation: POMDP在状态和转移不确定性下的规划具有理论优势，但维度灾难和历史灾难使得在有限规划时间内获得高性能策略仍具挑战性。许多POMDP问题中，信息价值在信念空间上变化，这为优化计算资源分配提供了机会。

Method: 提出动态规划框架，基于每个信念点的信息价值有条件地处理观测。在此基础上提出VOIMCP算法，这是一种蒙特卡洛树搜索算法，当VOI较低时选择性忽略观测信息，避免不必要的观测分支，从而更高效地分配计算努力。

Result: 提供了VOI推理框架的近最优性理论保证，推导了VOIMCP的非渐近收敛界。仿真评估表明，VOIMCP在多个POMDP基准测试中优于基线方法。

Conclusion: 通过利用信息价值在信念空间上的变化性，提出的VOIMCP框架能够更高效地分配计算资源，在有限规划时间内获得更好的POMDP策略性能，为解决维度灾难和历史灾难提供了有效途径。
...
TL;DR: 该论文评估AI辅助用药系统的可靠性，关注系统错误类型及其临床后果，而非仅依赖传统性能指标


<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药学领域应用日益广泛，但传统性能指标无法充分反映其在真实临床决策中的可靠性。在用药管理等高风险领域，单个错误推荐可能导致严重患者伤害，因此需要深入理解AI系统的失败行为及其实际影响。

Method: 通过一系列受控模拟场景，分析药物相互作用和剂量决策中的系统失败类型，包括漏检相互作用、错误风险标记和不适当剂量推荐。研究关注错误发生机制及AI系统产生错误输出时的后果。

Result: 研究发现AI在用药相关场景中的错误可能导致药物不良反应、治疗无效或延误护理，特别是在缺乏充分人工监督的情况下。研究还揭示了过度依赖AI推荐的风险以及决策过程透明度不足带来的挑战。

Conclusion: 该研究为医疗AI评估提供了可靠性视角，强调理解失败行为和实际影响的重要性。在药房实践等安全关键领域，需要将传统性能指标与风险感知评估方法相结合，并加强人工监督和系统透明度。
...
<details>
  <summary>Details</summary>
Motivation: 科学发现因碎片化文献而减缓，现有AI工具缺乏结构化多步骤方法，LLMs存在幻觉和不完整提取问题，需要更可靠的文献分析系统

Method: 多智能体人机协同框架，分配任务给专门智能体进行论文筛选、数据提取、模型拟合和结果总结，人类监督确保可靠性，生成结构化报告并支持迭代精炼

Result: 在材料科学中应用于钨在氦离子辐照下的研究，发现氦气泡生长与辐照剂量和温度呈指数相关，为聚变反应堆等离子体面向材料提供见解

Conclusion: AI辅助文献综述能够揭示科学模式并加速发现，Elhuyar系统展示了整合LLMs、结构化AI和人类监督的有效性
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM方法主要关注可行实例的解决方案生成，而未能明确处理不可行性检测问题。需要开发能够同时处理可行解生成和不可行性检测的框架

Method: 1) 提出不可行性感知框架，包括可验证数据集构建、监督微调和LLM辅助下游搜索；2) 针对minor-embedding问题引入新的数学规划公式和可证明的零相位不可行性筛选；3) 通过精确优化流程生成训练数据，对8B参数LLM进行微调；4) 利用LLM输出作为下游局部搜索的暖启动

Result: 微调模型比GPT-5.2整体准确率提升高达30%；LLM引导的暖启动在下游局部搜索中相比从零开始提供高达2倍的加速

Conclusion: 提出的不可行性感知框架有效解决了LLM在组合优化中不可行性检测的不足，通过可验证数据生成和微调显著提升性能，LLM输出作为暖启动能加速下游优化过程
...
TL;DR: P-COD是一种通过文档间关系检测异常值的新方法，利用语料库中已验证的同行信息来减少LLM幻觉，提高科学文献数据提取的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前减少LLM幻觉的方法（如提示工程和思维链提示）主要关注单个文档，未能考虑语料库中文档间的关系。在科学文献总结领域，相似实验设置的论文应得出相似结论，但现有方法无法有效利用这种关系来提高数据提取准确性。

Method: 提出Peer Context Outlier Detection (P-COD)方法，通过比较提取的数据与语料库中已验证的同行信息来调整置信度分数，标记低置信度结果供专家审查，而高置信度且得到同行验证的结果被视为可靠。

Result: 实验在6个科学领域展示了高达98%的异常值检测精度，证明该方法能有效减少幻觉，增强对自动化系统的信任，并让研究人员专注于模糊案例，从而简化数据提取工作流程。

Conclusion: P-COD通过利用文档间关系显著提高了LLM数据提取的准确性，减少幻觉，为科学文献总结等应用提供了更可靠的数据提取解决方案。
...
TL;DR: 提出基于角色的大型语言模型框架，用于健康食品政策信息提取，通过分配专门角色解决政策文档结构多样性导致的信息提取问题
...
Method: 提出角色化LLM框架，分配三个专门角色：LLM政策分析师（元数据和机制分类）、LLM法律策略专家（识别复杂法律方法）、LLM食品系统专家（分类食品系统阶段），通过结构化领域知识增强角色特定提示

Result: 在Healthy Food Policy Project数据库的608个健康食品政策上评估，与零样本、少样本和思维链基线相比，该框架在复杂推理任务中表现出优越性能

Conclusion: 该框架为健康政策信息提取提供了可靠且透明的方法论，能够自动化处理非结构化政策数据，模仿专家分析工作流程
...
TL;DR: 该论文研究了将高维表示自编码器（RAE）整合到连续自回归（AR）模型中的挑战，提出了RAE-AR方法，通过分布归一化和高斯噪声注入解决了复杂分布建模和曝光偏差问题，使表示自编码器在AR模型中达到与传统VAE相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统上，生成模型的潜在空间由VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法显示了表示自编码器的潜力，但将其整合到连续自回归模型中的挑战尚未充分探索。

Method: 提出了RAE-AR方法，针对AR模型的两个主要挑战：1）复杂的分词级分布建模，通过分布归一化进行分词简化；2）高维度放大的训练-推理差距（曝光偏差），通过训练时注入高斯噪声来增强预测鲁棒性。

Result: 实验结果表明，这些修改显著缩小了性能差距，使表示自编码器在AR模型中能够达到与传统VAE相当的结果，为视觉理解和生成建模的统一架构铺平了道路。

Conclusion: 该工作成功解决了将高维表示自编码器整合到自回归模型中的关键挑战，通过分布归一化和噪声注入技术，使表示自编码器在生成建模中达到与VAE编码器竞争的性能，促进了视觉理解和生成建模架构的统一。
...
TL;DR: 研究发现LLM训练中归一化层与优化器存在负交互：Derf归一化与Muon优化器组合时性能显著下降，而DyT无此问题，主要原因为erf在Muon快速谱范数增长下的饱和与尺度盲区问题。


<details>
  <summary>Details</summary>
Motivation: 传统上LLM训练中将归一化层和优化器视为独立设计选择，但本研究通过实验发现这种假设可能不成立，需要系统性地研究不同归一化方法与优化器之间的交互效应。

Method: 采用3×2因子设计实验，在10亿参数规模、1000训练步数下，测试Derf、RMSNorm、DyT三种归一化方法与AdamW、Muon两种优化器的组合效果，分析性能差异及原因。

Result: Derf与Muon组合存在显著负交互：性能差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats（约3倍）。DyT作为有界归一化控制组无此惩罚。发现erf在Muon快速谱范数增长下存在饱和（有损压缩）和尺度盲区（丢弃激活幅度）两种失效模式。

Conclusion: 归一化层与优化器选择存在重要交互效应，不能独立设计。Derf与Muon组合的失效容易被短试验忽略，通过EMA混合重新引入运行尺度估计可恢复约84%差距，降低Derf的alpha参数至0.3（非默认0.5）可恢复约80%差距。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在线性运筹学问题中表现良好，但在现实世界非线性场景中性能严重下降，主要原因是数学公式与求解器代码之间的语义不对齐以及信息提取不稳定

Method: NED-Tree框架采用：(a)逐句提取策略确保参数映射的鲁棒性和可追溯性；(b)递归树结构自适应地将复杂非线性项分解为求解器兼容的子元素。同时提出了NEXTOR基准测试专门用于复杂非线性、多约束运筹学问题

Result: 在10个基准测试中，NED-Tree实现了72.51%的平均准确率，建立了新的最先进水平。这是首个通过元素分解驱动LLM解决非线性建模困难的框架，实现了建模语义与代码语义的对齐

Conclusion: NED-Tree成功解决了LLM在非线性运筹学问题建模中的语义对齐挑战，通过系统化框架显著提升了自然语言到可执行模型的翻译准确性，为自动化运筹学建模提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要正确性信号或批判注释来训练LLM的自我精炼能力，这限制了训练效率和可扩展性。研究者希望开发一种无需额外监督信号的联合训练框架，同时优化推理和精炼能力。

Method: 提出ThinkTwice两阶段框架：1) 第一阶段优化模型解决推理问题；2) 第二阶段优化模型精炼自己的解决方案。两个阶段都使用相同的二元正确性奖励，无需正确性信号或批判注释。基于Group Relative Policy Optimization (GRPO)实现。
...
Conclusion: ThinkTwice建立了推理和自我精炼联合训练作为RLVR的原则性和有效方法，通过简单的两阶段框架和相同的二元奖励信号，实现了显著的性能提升，揭示了训练动态中的隐含课程学习模式。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在教学决策时是采用基于学习者知识的推理策略，还是使用简单的经验法则，以理解其教学机制

Method: 使用带奖励标注的有向图任务，让LLMs作为模拟教师，观察假设学习者的轨迹并揭示单一边缘以改善学习者的路径规划；采用与人类研究相同的认知模型进行拟合：贝叶斯最优教师、弱贝叶斯变体、启发式基准和非心智化效用模型

Result: 在基线实验中，大多数LLMs表现良好，策略变化小，图间表现与人类相似；模型比较显示贝叶斯最优教学最能解释大多数模型的决策；支架干预中模型遵循推理或奖励导向提示，但这些干预不能可靠改善启发式不一致测试图上的教学，有时反而降低性能

Conclusion: 认知模型拟合揭示了LLMs的教学策略本质上是贝叶斯最优推理，提示合规性不能保证更好的教学决策，需要更深入理解LLMs的教学机制
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖监督微调，缺乏代码执行环境暴露，且即使最先进的MLLMs也难以有效进行自校正。需要一种能够通过环境交互和迭代自校正来生成更准确、可执行代码的方法。
...
TL;DR: CRaFT：基于电路引导的拒绝特征选择框架，通过边界附近提示评估特征对拒绝-顺从决策的影响，相比基于激活强度的传统方法能更有效地识别因果性拒绝特征
...
Method: 提出CRaFT框架，通过评估特征在拒绝边界附近提示上对模型拒绝-顺从决策的影响来对特征进行排序，使用电路引导方法识别因果性拒绝特征。

Result: 在Gemma-3-1B-it模型上，CRaFT将攻击成功率从6.7%提升至48.2%，在多个越狱基准测试中均优于基线方法。

Conclusion: 电路影响是比激活强度更可靠的特征选择标准，能更有效地识别真正介导拒绝行为的因果特征，为理解LLM拒绝机制提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统存在协调开销大、上下文碎片化和脆弱的阶段排序等问题，将其蒸馏为单智能体技能可以绕过这些成本，但缺乏原则性指导来确定何时以及蒸馏什么，导致实证结果不一致（技能提升从28%改善到2%退化）。

Method: 引入Metric Freedom (F)作为技能效用的先验预测指标，通过Mantel检验量化输出多样性与得分方差之间的耦合关系来度量评分地形的拓扑刚性。提出两阶段自适应蒸馏框架：阶段1作为选择性提取机制，在"自由"度量上提取工具和知识同时丢弃限制性结构以保持探索；阶段2专门针对"刚性"度量（F≲0.6）进行计算密集的迭代优化以消除轨迹局部过拟合。

Result: 在4个任务、11个数据集和6个度量上的评估显示，F强烈预测技能效用（ρ=-0.62，p<0.05）。相同的智能体轨迹在刚性与自由度量下产生截然相反的性能提升，表明技能效用本质上是度量层面的属性。自适应智能体匹配或超越原始MAS，同时将成本降低高达8倍，延迟降低高达15倍。

Conclusion: 技能效用不是由任务决定，而是由评估度量决定。Metric Freedom (F)作为先验预测指标能够有效指导多智能体系统向单智能体技能的蒸馏，自适应蒸馏框架在保持或提升性能的同时显著降低了计算成本和延迟。
...
TL;DR: GraphWalk：一种免训练、基于工具的框架，让现成LLM能够通过顺序图导航进行推理，显著提升大规模知识图谱上的复杂查询性能
...
Method: 提供最小正交图操作集合，让LLM通过顺序工具调用进行图遍历，每个工具调用代表可验证的推理步骤，形成透明执行轨迹

Result: 在所有测试模型家族中，基于工具的遍历方法相比上下文基线均获得显著一致提升，且随着规模增大增益更明显，而上下文方法在大型图谱上完全失效

Conclusion: GraphWalk框架通过最小正交图操作实现免训练、任务无关的图导航，为LLM在大规模知识图谱上进行复杂多跳推理提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在评估生产导向分类框架中大型语言模型在收据项目分类任务上的表现，特别关注模型在准确性、响应稳定性和令牌级成本方面的差异，并探究零样本或少样本提示方法在准确性和成本方面的适用性。

Method: 采用系统性、成本感知的评估方法，比较了AWS Bedrock平台上的四种指令调优模型：Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct和Mistral 7B Instruct。研究评估了准确性、响应稳定性和令牌级成本，并对比了零样本和少样本提示方法的效果。

Result: 实验结果表明，Claude 3.7 Sonnet在分类准确性和成本效率之间取得了最有利的平衡。研究还揭示了不同提示方法在准确性和成本方面的具体表现。

Conclusion: Claude 3.7 Sonnet是收据项目分类任务中最具成本效益的选择，研究为生产环境中LLM的实际应用提供了成本感知的评估框架和实用指导。
...
TL;DR: OSCAR：基于扩散语言模型的训练无关推理时幻觉缓解框架，通过并行去噪链和交叉链熵检测不确定性位置，结合检索证据进行针对性重掩码


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（DLMs）暴露其去噪轨迹，为推理时控制提供天然接口。理想的幻觉缓解框架应利用这一模型原生信号进行干预，而非依赖外部训练的幻觉分类器

Method: 提出承诺不确定性定位：给定去噪轨迹，识别交叉链熵超过无监督阈值的token位置。引入OSCAR框架：运行N个并行去噪链（随机揭示顺序），计算交叉链香农熵检测高不确定性位置，然后基于检索证据进行针对性重掩码

Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA上使用LLaDA-8B和Dream-7B，OSCAR显著减少幻觉内容并提高事实准确性。消融实验确认定位和校正贡献互补增益，在N∈{4,8,16}时保持鲁棒性

Conclusion: OSCAR展示了扩散语言模型通过原生熵基不确定性信号识别事实不确定性的内在能力，超越了专门训练的检测器。该框架支持未来在DLMs中进行定位和不确定性感知生成的研究
...
<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理很有吸引力，因为它们可以外部化操作知识并在异构数据和不断发展的规范中扩展数据管理。然而，用概率性工作流替换确定性组件会改变故障模式：LLM管道可能生成看似合理但错误的输出，这些输出通过表面检查并传播到不可逆操作（如DOI分配和公开发布）。
...
Result: 在两个生产部署中进行了比较：大学GIS中心生态档案（849个数据集）作为单智能体基线；SF2Bench复合洪水基准（2,452个监测站和8,557个文件）验证了多智能体工作流。多智能体方法提高了效率（单个操作员在两天内完成，跨部署重复使用工件）和可靠性：审计交接检测并阻止了影响所有2,452个站的坐标转换错误。代表性事件（ISS-004）展示了基于边界的遏制，检测延迟10分钟，零用户暴露，80分钟解决。

Conclusion: EnviSmart通过架构方法解决了LLM驱动数据管理中的可靠性问题，将可靠性作为系统属性而非事后检查，成功部署在生产环境中，防止了错误传播到不可逆操作，并提高了效率和可靠性。
...
TL;DR: CORAL：首个面向开放问题的自主多智能体进化框架，通过持久记忆、异步执行和心跳干预实现智能体自主探索，在数学、算法和系统优化任务上取得SOTA结果
...
Method: CORAL框架采用长期运行的智能体，通过共享持久记忆、异步多智能体执行和基于心跳的干预实现探索、反思和协作。提供隔离工作空间、评估器分离、资源管理、智能体会话和健康管理等安全保障机制。

Result: 在多样化的数学、算法和系统优化任务上，CORAL在10个任务上创造了新的SOTA结果，相比固定进化搜索基线，改进率提高3-10倍且评估次数更少。在Anthropic内核工程任务中，四个协同进化智能体将最佳得分从1363周期提升到1103周期。

Conclusion: 机制分析表明这些改进源于知识重用和多智能体探索与通信。结果表明更大的智能体自主性和多智能体进化能显著改善开放发现过程，为自主进化系统提供了新范式。
...
TL;DR: 提出七种面向临床AI系统的本体感知设计模式，用于构建抵抗本体扭曲的AI管道，包括数据验证、信号保留、漂移监控、双本体层、反馈中断、术语版本管理和合规适配等模式。


<details>
  <summary>Details</summary>
Motivation: 临床AI系统训练数据受到文档工作流、计费激励和术语碎片化的结构性扭曲，现有理论分析（如三力模型、物化反馈循环、术语治理失败）尚未转化为可实施的软件架构，需要设计模式来解决这一开放问题。

Method: 采用Gang-of-Four模式语言提出七种本体感知设计模式：本体检查点、休眠感知管道、漂移哨兵、双本体层、物化断路器、术语版本门、监管合规适配器。每种模式按问题、力量、解决方案、后果、已知用途和相关模式进行规范，并通过初级保健AI系统的参考架构和糖尿病风险预测场景进行说明。

Result: 建立了七种设计模式的完整规范，其中三种模式在现有系统中有部分先例，四种模式首次正式描述。提供了参考架构和场景演练，但未进行实证验证，缺乏运行时基准测试，且局限于德国和欧盟监管环境。

Conclusion: 本文提供了基于理论分析的设计词汇表，为构建抵抗本体扭曲的临床AI系统提供了架构模式，这些模式需要在生产系统中进行未来评估，填补了理论洞察到可实施软件架构之间的转化空白。
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在长程推理中面临上下文大小限制，受部署因素（内存、延迟、成本）约束形成上下文预算限制。随着交互历史增长，需要在保留过去信息和遵守上下文限制之间进行权衡，现有方法难以有效管理这一权衡。
...
Result: 在组合多目标QA和长程网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法，在高复杂度设置中达到基线1.6倍以上的性能提升。在预算缩减时保持明显优势，而大多数方法呈现性能下降趋势。

Conclusion: BACM-RL通过将上下文管理形式化为序列决策问题并采用强化学习优化压缩策略，有效解决了LLM智能体在有限上下文预算下的长程推理挑战，为实际部署中的上下文管理提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 长期记忆对智能体保持历史一致性至关重要，但大量交互数据的积累会导致性能瓶颈：存储扩展增加检索噪声和计算延迟，在受限的个人设备上会压垮模型的推理能力。

Method: 提出分层记忆编排（HMO）框架，将交互历史组织为三层目录：紧凑的主缓存（耦合近期和关键记忆与动态用户画像）、高优先级次级层，以及全局完整历史档案。用户画像驱动记忆在层级间的重新分配，将映射到长期模式的记录提升到更活跃层级，同时降级不相关信息。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要针对同构模型（相同预训练主干），但在开放模型生态中，有用专家模型常基于不同架构（如Llama、Qwen、Mistral），直接权重空间融合因架构不匹配、潜在基不对齐和跨源冲突而不可行

Method: HeteroFusion包含两个关键组件：1）基于拓扑的对齐，通过匹配功能模块结构而非原始张量坐标在异构主干间传递知识；2）冲突感知去噪，在融合过程中抑制不兼容或噪声传递信号。该方法保持目标适配器基的同时预测结构化更新

Result: 在异构迁移、多源融合、噪声源鲁棒性和跨家族泛化等设置中，HeteroFusion始终优于强大的融合、合并和集成基线方法

Conclusion: HeteroFusion为异构语言模型融合提供了有效解决方案，通过拓扑对齐和冲突感知机制克服了架构不匹配问题，在开放模型生态中实现了跨家族模型的互补优势集成
...
<details>
  <summary>Details</summary>
Motivation: AIGC的快速扩散正在重构在线内容生态，需要系统研究其行为和分布影响，以理解AIGC与HGC的差异及其对平台生态的影响。

Method: 利用中国领先视频分享平台的纵向数据集，包含数千万用户数据，对比分析AIGC与HGC的创作和消费行为特征。

Result: 发现"规模优于偏好"现象：AIGC创作者通过高产量获得与HGC相当的总体参与度，但消费者明显偏好HGC；算法分发机制能调节AIGC相关的竞争利益。
...
TL;DR: LiteInception：面向边缘部署的轻量级可解释故障诊断框架，采用两阶段级联架构，通过多方法融合压缩和知识蒸馏，在NGAFID数据集上实现81.92%故障检测准确率和77.00%故障识别准确率。


<details>
  <summary>Details</summary>
Motivation: 通用航空故障诊断和维护对飞行安全至关重要，但在资源受限的边缘设备上部署深度学习模型面临计算能力和可解释性的双重挑战。需要开发既轻量又具有可解释性的故障诊断框架。
...
Result: 在NGAFID数据集上：故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%。模型压缩效果：CPU推理加速超过8倍，F1损失小于3%。验证了框架在效率、准确性和可解释性之间的良好平衡。

Conclusion: LiteInception框架成功解决了边缘设备上深度学习模型部署的计算和可解释性挑战，通过两阶段架构、模型压缩和知识蒸馏实现了高效、准确且可解释的故障诊断，适用于通用航空的安全关键和辅助诊断场景。
...
TL;DR: 开发AnIML本体，通过OWL 2形式化AnIML语义并与Allotrope数据格式对齐，以解决XML模式表达性导致的语义不一致问题，提升跨系统互操作性。
...
Method: 采用专家在环方法，结合LLM辅助的需求获取与协作本体工程，开发AnIML本体。通过多层验证方法：将真实AnIML文件转化为知识图谱的数据驱动转换、通过SPARQL进行能力问题验证，以及基于对抗性负面能力问题的新验证协议，这些问题映射到已建立的本体反模式并通过SHACL约束强制执行。

Result: 提出了AnIML本体，这是一个OWL 2本体，形式化了AnIML的语义并与Allotrope数据格式对齐，支持未来的跨系统和跨实验室互操作性。

Conclusion: AnIML本体通过形式化语义和与Allotrope数据格式对齐，解决了AnIML XML模式表达性导致的语义不一致问题，为提升跨系统、跨实验室的数据互操作性提供了有效解决方案。
...
Method: 提出基于SAT的框架：1）将零件类型按需求扩展为多个副本；2）为每个副本分配板材变量；3）仅对分配到同一板材的副本激活非重叠约束；4）引入不可行方向消除规则，当零件只有一个方向能放入板材时固定旋转变量。比较三种最小化板材数量的方法：非增量SAT（二分搜索）、增量SAT（跨迭代重用子句）和加权部分MaxSAT。

Result: 在Cui-Zhao基准测试套件上，最佳SAT配置证明为最优的实例数量是OR-Tools、CPLEX和Gurobi的2-3倍，且获得更低的优化间隙。不同SAT方法的相对性能取决于旋转约束：无旋转时增量SAT最强，旋转增加公式大小时非增量SAT更有效。

Conclusion: SAT框架在解决2D-CSSP问题上优于传统优化求解器，通过创新的建模技术和约束处理策略有效应对组合爆炸问题。方法选择应根据旋转约束情况：无旋转时使用增量SAT，旋转复杂时使用非增量SAT。
...
TL;DR: 提出AeroTherm-GPT，首个面向高超音速热防护系统设计的LLM智能体，通过约束闭环生成框架解决多约束违反问题，相比通用LLM提升12.5%端到端成功率。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在生成可执行仿真工件时面临级联约束违反问题，无法满足安全关键工程工作流中的顺序多门约束，限制了LLM在高超音速热防护系统设计中的应用。

Method: 提出约束闭环生成框架，包含生成、验证、CDG引导修复、执行和审计的迭代工作流。约束依赖图编码约束类别间的经验共解结构，基于生命周期排序先验和共解概率指导上游故障修复。

Result: 在HyTPS-Bench评估中达到88.7%端到端成功率（95%置信区间：87.5-89.9），比无CDG消融基线提升12.5个百分点，根本原因修复效率达4.16（扁平清单修复为1.76），且在科学推理和代码生成任务上无灾难性遗忘。

Conclusion: AeroTherm-GPT通过约束依赖图引导的闭环修复机制，有效解决了工程设计中多约束违反问题，为安全关键领域的LLM应用提供了可扩展框架。
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱能高效存储大量关系，但难以处理概念在不同领域中语义漂移的问题。现有系统通常将领域信息作为元数据、限定符或图级组织来处理，这些机制仅用于过滤和溯源，不改变断言的形式状态。当领域被当作外部信息处理时，会导致知识系统的实际应用失败。

Method: 提出Domain-Contextualized Concept Graph (DCG)框架，将领域信息直接写入关系中，形式化为(C, R at D, C')结构，其中"at D"标记标识关系成立的世界。采用Kripke风格语义学，通过领域索引的必然性算子解释关系，使真值、推理和冲突检查都在相关世界范围内进行。开发了紧凑谓词系统、Prolog实现，并映射到RDF、OWL和关系数据库。

Result: DCG框架实现了三个重要结果：1) 模糊概念可以在表示层面消歧；2) 无效断言可以根据其领域进行质疑；3) 跨领域关系可以通过显式谓词连接。为领域信息提供了结构和可计算的角色，解决了传统知识系统中领域信息处理不足的问题。

Conclusion: 领域应被视为知识表示的内在组成部分而非补充性标注。DCG通过将领域信息结构化和可计算化地融入表示中，解决了知识系统中因领域信息外部化处理而导致的实践失败问题。这是对领域本身的一种表征性重新解释。
...
TL;DR: PGPO提出了一种细粒度的信用分配框架，通过量化token级别的视觉依赖关系，动态重塑优势函数，增强视觉依赖token的学习信号，抑制语言先验的梯度噪声，从而提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习框架存在方法学缺陷：它们在所有生成的token上分配相同的优势值，这稀释了多模态推理中关键视觉基础步骤的学习信号，无法有效优化视觉依赖的推理过程。

Method: 提出Token Visual Dependency概念，通过KL散度量化视觉输入对token预测的因果信息增益；基于此引入Perception-Grounded Policy Optimization框架，采用阈值门控的质量守恒机制，在token级别动态重塑优势函数，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声。

Result: 在Qwen2.5-VL系列模型上，基于七个具有挑战性的多模态推理基准测试，PGPO平均提升模型性能18.7%。理论和实证分析证实PGPO有效降低梯度方差，防止训练崩溃，并作为强大的正则化器实现稳健的感知基础多模态推理。

Conclusion: PGPO通过细粒度的token级别信用分配，解决了现有RLVR框架中学习信号稀释的问题，显著提升了多模态推理中视觉依赖步骤的优化效果，为感知基础的多模态推理提供了有效的训练框架。
...
TL;DR: 本文提出了AWARE框架，通过监督嵌入学习和轻量级适配器改进表格上下文学习在临床预测中的表现，特别是在数据异质性和类别不平衡情况下。


<details>
  <summary>Details</summary>
Motivation: 临床电子健康记录预测面临高维度、异质性、类别不平衡和分布偏移等挑战。虽然表格上下文学习和检索增强方法在通用基准上表现良好，但在临床环境中的行为尚不清楚，需要系统评估和改进。

Method: 提出了AWARE框架：1）使用监督嵌入学习进行任务对齐的检索；2）采用轻量级适配器；3）在多队列EHR基准上比较经典模型、深度表格模型和TICL模型；4）评估不同数据规模、特征维度、结果稀有性和跨队列泛化能力。

Result: PFN-based TICL模型在低数据情况下样本效率高，但在朴素距离检索下随着异质性和不平衡增加而性能下降。AWARE在极端不平衡情况下将AUPRC提高了12.2%，且数据复杂度越高增益越大。
...
<details>
  <summary>Details</summary>
Motivation: 传统SSP算法使用Bellman备份时，即使启发式函数显示某些动作代价过高，仍会考虑所有适用动作，导致计算资源浪费在无帮助的动作上。
...
Conclusion: 通过约束生成技术利用启发式信息避免昂贵动作，能显著提升SSP求解效率，CG-iLAO*在减少计算量的同时保持解的质量。
...
<details>
  <summary>Details</summary>
Motivation: 在多类分类任务中，传统监督学习通常使用硬标签或概率分布，但实际应用中可能存在不确定性或模糊性。可能性分布能够表达类别的分级可信度，需要一种方法将这种定性信息转化为可训练的监督信号。

Method: 1. 从可能性分布构造可容许概率分布集：结合概率兼容性（与可能性测度和必然性测度兼容）和线性形状约束（保持可能性分布的定性结构）
2. 使用Dykstra算法计算模型输出的概率向量到可容许集的KL投影
3. 最小化预测与其投影之间的散度作为训练目标，量化满足约束所需的最小调整

Result: 1. 投影算法在实践中有足够效率
2. 在合成数据和真实自然语言推理任务（基于ChaosNLI数据集）上，基于投影的学习目标能够提升预测性能

Conclusion: 该方法成功将可能性监督转化为有效的多类分类训练框架，通过构造可容许概率集和KL投影，能够利用分级可信度信息提升模型性能，且计算效率满足实际应用需求。
...
TL;DR: Qiana是一个用于在特定上下文中进行推理的逻辑框架，支持对公式和上下文进行量化，允许上下文内包含矛盾，基于一阶逻辑且可有限公理化，兼容现有一阶逻辑定理证明器。


<details>
  <summary>Details</summary>
Motivation: 需要一种能够在特定上下文中进行推理的逻辑框架，能够表达复杂的知识关系（如"每个人都知道Alice说的一切"），同时支持上下文内的矛盾（paraconsistent逻辑），并与现有的一阶逻辑工具兼容。

Method: 基于一阶逻辑构建Qiana逻辑框架，使其具有有限公理化特性；支持对公式和上下文进行量化；允许在上下文中使用paraconsistent逻辑来处理矛盾；设计兼容现有一阶逻辑定理证明器的理论形式。

Result: 成功开发了Qiana逻辑框架，能够表达复杂的上下文相关推理；展示了Qiana在表示时间性、事件演算和模态逻辑方面的应用能力；讨论了不同的设计替代方案。

Conclusion: Qiana提供了一个强大而灵活的逻辑框架，能够在特定上下文中进行推理，支持复杂的量化表达和矛盾处理，同时保持与现有一阶逻辑工具的兼容性，为上下文相关推理提供了实用的解决方案。
...
TL;DR: 该研究创建了NeuroPose-AHM数据集，这是一个基于知识的多条件异常头部运动数据集，包含57种神经疾病的2,756条患者组记录，并展示了其在颈肌张力障碍分析中的应用价值。
...
Method: 采用多LLM提取框架处理1,430篇同行评审文献，构建NeuroPose-AHM数据集。通过四任务框架分析颈肌张力障碍：1）多标签AHM类型分类；2）构建头颈严重程度指数（HNSI）；3）用真实患者数据验证HNSI；4）运动类型概率与HNSI分数的桥接分析。

Result: 数据集包含57种神经疾病的2,756条记录，LLM提取可靠性高（kappa=0.822）。颈肌张力障碍分析中，AHM类型分类F1=0.856；HNSI与真实数据严重带比例一致（6.7%）；运动类型与HNSI显著相关（p<0.001）。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的神经性异常头部运动研究资源，具有分析实用性。数据集已在Zenodo公开，为AI驱动诊断工具开发提供了重要基础。
...
TL;DR: SenseMath基准测试评估LLM的结构敏感数值推理能力，发现模型能使用数值捷径但缺乏人类数感的结构理解，无法判断捷径适用性或生成有效捷径问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型常默认逐步计算，即使存在高效数值捷径。本研究旨在探究LLM是否具有类似人类的数感——识别数值结构、适当应用捷径并在不适用时避免使用的能力。

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字尺度，匹配强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用（模型能否在适用捷径问题上应用捷径）、适用性判断（能否识别捷径何时适用或误导）、问题生成（能否生成正确包含给定类型捷径的新问题）。评估五个LLM，从GPT-4o-mini到Llama-3.1-8B。

Result: 当明确提示时，模型能采用捷径策略并在适用捷径项目上获得显著准确率提升（高达15%），但在标准思维链提示下，即使具备所需能力，自发使用策略的情况少于40%。这种能力仅限于使用层面；模型系统性地过度泛化捷径到不适用的问题，且无法从头生成有效的捷径问题。

Conclusion: 当前LLM表现出程序性捷径流畅性，但缺乏支撑人类数感的结构理解——即理解捷径何时有效及为何有效的深层认知。模型能执行捷径但无法理解其适用条件。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于监督学习的步态分析方法需要疾病标签数据，难以泛化到异质性病理表现。需要一种无需疾病标签、能够定位个体特异性步态异常并矫正运动学轨迹的方法

Method: 使用Transformer掩码自编码器，仅用150名成年人的正常步态序列训练。推理时采用两阶段方法：1)通过遮挡单个关节并测量与学习到的正常先验的偏差来估计关节不一致性分数；2)从编码器输入中排除标记的异常关节，从剩余时空上下文重建完整骨架，生成矫正后的运动学轨迹

Result: 在10名模拟7种步态异常的参与者上验证，能够准确定位生物力学不一致的关节，显著减少所有分析关节的角度偏差（大效应量），并保持正常运动学特征
...
<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能增强大语言模型的推理能力，但在多轮智能体任务中面临挑战，因为这类任务具有长时程交互特性和环境反馈的随机性。研究发现智能体探索中存在结构性失败模式：次优动作会引入噪声观察形成误导性上下文，进一步削弱后续决策，形成错误累积的恶性循环，使标准探索策略失效。
...
TL;DR: ATBench是一个轨迹级基准测试，用于评估LLM智能体在多步交互中的安全性风险，通过三维风险分类、异构工具池和长上下文延迟触发协议构建多样化的现实场景。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败可观测性粗糙、长时域真实性弱等局限，而现实部署中的风险往往在多步交互中显现，而非孤立提示或最终响应。

Method: 基于三维风险分类（风险来源、失败模式、现实危害）构建轨迹，使用异构工具池和长上下文延迟触发协议捕捉多阶段风险涌现，包含1000个轨迹（503安全/497不安全），平均9.01轮对话和3.95k令牌，涉及1954个工具调用（来自2084个可用工具池）。

Result: 实验表明ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性，同时支持分类分层分析、跨基准比较和长时域失败模式诊断。

Conclusion: ATBench为结构化、多样化和现实化的智能体安全评估提供了有效的轨迹级基准，能够捕捉多步交互中涌现的安全风险，支持更全面的安全分析和诊断。
...
TL;DR: 该论文是一篇关于语言模型中潜在空间的系统性综述，从基础、演化、机制、能力和展望五个视角全面梳理了该领域的研究现状，旨在将潜在空间确立为下一代智能系统的通用计算范式。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，研究者逐渐认识到许多关键内部过程在连续潜在空间中比在人类可读的显式标记空间中更自然。显式空间计算存在语言冗余、离散化瓶颈、序列效率低下和语义损失等结构限制，这推动了向潜在空间计算的范式转变。本文旨在提供语言模型中潜在空间的统一且最新的研究图景。

Method: 采用系统性综述方法，从五个连续视角组织调查：基础（界定潜在空间范围）、演化（追溯从早期探索到当前大规模扩展的发展历程）、机制（从架构、表示、计算和优化四个主要发展路线分析）、能力（展示潜在空间如何支持推理、规划、建模、感知、记忆、协作和具身等广泛能力谱系）、展望（讨论开放挑战和未来研究方向）。

Result: 论文构建了一个全面的分类框架，系统性地整理了语言模型中潜在空间的研究现状。从机制角度识别了四大发展路线，从能力角度展示了潜在空间支持的七类核心能力，为理解潜在空间作为下一代智能系统的通用计算范式提供了理论基础。

Conclusion: 潜在空间正在成为语言模型的原生计算基质，代表了从显式标记空间向连续潜在空间的范式转变。该综述不仅为现有研究提供了参考框架，更重要的是为理解潜在空间作为下一代智能的通用计算和系统范式奠定了基础。未来研究需要解决该领域的开放挑战，推动潜在空间计算的发展。
...
TL;DR: ARQuest框架使用LLMs和替代数据源创建个性化自适应问卷，改进保险核保流程，减少问题数量并提升用户体验。
...
Method: 使用大型语言模型（LLMs）和替代数据源（社交媒体图像分析、地理数据分类、检索增强生成RAG）提取用户洞察，生成个性化自适应问卷。

Result: 传统问卷风险评估准确度略高，但GPT驱动的自适应版本问题更少，用户体验更流畅、更具吸引力，用户偏好自适应版本。

Conclusion: ARQuest有潜力提升用户满意度、简化保险流程，通过进一步开发可能在风险准确性上超越传统方法，推动保险行业创新。
...
TL;DR: Diff-KD：通过扩散生成式精炼和教师-学生知识蒸馏提升多智能体协同感知在传感器和通信损坏下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有方法将损坏视为静态扰动或被动适应损坏输入，无法主动恢复底层清洁语义，导致多智能体协同感知在真实世界传感器和通信损坏下性能严重下降

Method: 提出Diff-KD框架，包含两个核心组件：1)渐进知识蒸馏(PKD)，将局部特征恢复视为条件扩散过程，从损坏观测中恢复全局语义；2)自适应门控融合(AGF)，在融合过程中基于自身可靠性动态加权邻居贡献

Result: 在OPV2V和DAIR-V2X数据集上评估七种损坏类型，Diff-KD在检测精度和校准鲁棒性方面均达到最先进性能

Conclusion: Diff-KD通过集成扩散生成式精炼到知识蒸馏中，有效解决了多智能体协同感知中的损坏恢复问题，显著提升了系统的鲁棒性
...
TL;DR: 该研究提出了一种无参考的LLM评估方法，用于验证时间序列数据生成的文本解释的事实正确性，构建了包含350个案例的合成基准，发现LLM作为解释生成器表现不稳定但作为评估器更可靠。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估LLM生成的时间序列文本解释事实正确性的通用方法。现有方法要么需要真实解释作为参考，要么只能处理数值信号而无法评估自由形式的文本推理，因此需要开发无需预定义参考或特定任务规则的评估框架。

Method: 研究将大语言模型同时作为时间序列解释的生成器和评估器，在无参考设置下，给定时间序列、问题和候选解释，评估器基于模式识别、数值准确性和答案忠实性分配三元正确性标签。构建了包含350个时间序列案例的合成基准，涵盖七种查询类型，每种都配有正确、部分正确和错误的解释。

Result: 结果显示明显的不对称性：解释生成高度依赖模式，在某些查询类型上存在系统性失败（季节性下降和波动率转移的准确率仅为0.00-0.12，而结构性断裂为0.94-0.96）；评估则更稳定，即使模型自身输出不正确，也能正确排序和评分解释。模型在四个任务上进行了评估：解释生成、相对排序、独立评分和多异常检测。

Conclusion: 研究证明了基于数据的大语言模型评估时间序列解释的可行性，突显了它们作为时间序列领域数据驱动推理可靠评估器的潜力。LLM作为评估器比作为生成器表现更稳定，这为构建无需真实参考的自动评估系统提供了基础。
...
TL;DR: TRACE-Bot：用于检测LLM驱动社交机器人的统一双通道框架，通过联合建模隐式语义表示和AIGC增强的行为模式，在两个公开数据集上分别达到98.46%和97.50%的准确率。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的社交机器人通过生成类人内容规避传统检测方法，现有方法存在三个主要问题：过度依赖单模态信号、对AIGC生成模式敏感度不足、未能充分建模语言模式与行为动态的交互作用。
...
Conclusion: 联合利用隐式语义表示和AIGC增强的行为模式对于新兴LLM驱动社交机器人检测是有效的，TRACE-Bot框架在准确性和鲁棒性方面均表现出色。
...
TL;DR: 提出了一种用于验证AI/ML系统在安全关键领域（如航空）中操作设计域（ODD）完整覆盖的结构化工程方法，以满足EASA认证标准。
...
Method: 提出了一种集成参数离散化、基于约束的过滤和基于关键性的维度约减的结构化多步骤ODD覆盖验证流程，基于先前AI空中防撞研究的仿真数据进行验证。
...
TL;DR: 研究评估LLM生成的胸部CT报告日文翻译的教育适用性，发现放射科医生与LLM评估者之间几乎无一致性，LLM评估者强烈偏好LLM输出，专家评审仍不可或缺。


<details>
  <summary>Details</summary>
Motivation: 放射学报告的多语言翻译对研究、临床沟通和教育很重要，但LLM评估的有效性尚不明确。需要评估LLM生成的翻译质量及其在教育应用中的适用性。

Method: 分析150份胸部CT报告，比较人工编辑的日文翻译与DeepSeek-V3.2生成的翻译。由放射科医生和放射科住院医师进行盲法配对评估（术语准确性、可读性、整体质量、放射科医生风格真实性），同时使用3个LLM评估者进行并行评估。使用QWK和百分比一致性评估一致性。

Result: 放射科医生与LLM评估者之间一致性接近零（QWK=-0.04至0.15），放射科医生之间一致性也较差（QWK=0.01至0.06）。放射科医生1认为59%的病例术语等效，在可读性（51%）和整体质量（51%）上偏好LLM翻译。放射科医生2认为75%的病例可读性等效，在整体质量上偏好人工编辑翻译（40% vs 21%）。所有3个LLM评估者强烈偏好LLM翻译（70%-99%），并在>93%的病例中认为其更具放射科医生风格。

Conclusion: LLM生成的翻译常被认为自然流畅，但放射科医生评估差异显著。LLM作为评估者表现出对LLM输出的强烈偏好，与放射科医生几乎无一致性。对于放射学报告翻译的教育应用，仅依赖自动化LLM评估不足够，专家放射科医生评审仍然重要。
...
Method: 基于扰动的策略结合三矩阵分析框架：角度偏差矩阵（捕捉语义方向变化）、幅度偏差矩阵（测量语义强度变化）、维度重要性矩阵（评估各向量维度贡献）。通过系统移除每个token并测量这三个互补维度的影响，得到综合重要性分数

Result: 开发了开源实现，代码和资源在https://github.com/Infosys/Infosys-Responsible-AI-Toolkit公开可用，支持可复现性和广泛采用

Conclusion: 提出了一种轻量级、模型无关的token重要性可视化技术，为理解生成式AI系统如何感知和优先处理输入文本信息提供了数学基础的方法，无需额外计算成本
...
TL;DR: 提出Trace Inversion方法，通过分析推理模型的推理轨迹来重建其回答的问题，通过比较原始问题与重建问题的相似度来判断模型是否应弃权，显著提升了推理模型的弃权能力。


<details>
  <summary>Details</summary>
Motivation: 推理模型在复杂任务上表现出色，但弃权能力较差。当模型产生幻觉导致弃权失败时，这可以重新解释为模型回答了错误的问题，而不是错误地回答了问题。基于这一观察，需要开发新的方法来提升推理模型的弃权能力。

Method: 提出查询错位框架和Trace Inversion方法：1) 生成模型的推理轨迹；2) 仅基于推理轨迹重建模型最可能回答的查询问题；3) 比较初始查询与重建查询的相似度。相似度低表明模型可能回答了错误的问题，应标记为弃权。

Result: 在4个前沿LLM和9个弃权QA数据集上的广泛实验表明，Trace Inversion在36个设置中的33个中击败了竞争基线，有效提升了弃权性能。

Conclusion: Trace Inversion通过分析推理轨迹来检测模型是否回答了错误的问题，为提升推理模型的弃权能力提供了一种新的有效方法，显著改善了模型在不确定时的弃权决策。
...
<details>
  <summary>Details</summary>
Motivation: 监管文档包含LLM系统必须遵守的法律义务，但将密集、层次结构的法律文本转换为机器可读规则仍然是一个昂贵且需要专家参与的过程，需要自动化解决方案来降低成本并提高可扩展性。

Method: De Jure采用四阶段管道：1) 将源文档规范化为结构化Markdown；2) 使用LLM进行语义分解为结构化规则单元；3) 通过LLM-as-a-judge在19个维度上进行多标准评估；4) 在有限再生预算内迭代修复低分提取，优先修复上游组件再评估规则单元。

Result: 在金融、医疗和AI治理三个监管语料库上评估，De Jure在金融领域实现了一致且单调的提取质量改进，在三个法官引导迭代内达到峰值性能。在医疗和AI治理领域也保持高性能，在开源和闭源模型上都表现良好。在下游合规问答评估中，基于De Jure提取规则的RAG响应在73.8%的单规则检索情况下优于先前工作，在更广泛检索下达到84.0%。

Conclusion: De Jure证明了明确的、可解释的评估标准可以在复杂监管领域中替代人工标注，为基于监管的LLM对齐提供了可扩展且可审计的路径，提取保真度直接转化为下游实用性。
...
Result: 静态情感前缀通常只产生小的准确率变化；情感效应在社会基础任务中更可变；更强的情感词汇仅引起适度额外变化；自适应情感选择比固定情感提示产生更可靠的性能增益
...
### [211] [Non-Rigid 3D Shape Correspondences: From Foundations to Open Challenges and Opportunities](https://arxiv.org/abs/2604.01274)
...
TL;DR: 该报告综述了变形形状对应估计的三种主要方法：基于功能映射的谱方法、施加离散约束的组合方法、以及直接恢复全局对齐的变形方法，并讨论了新兴挑战和机会。


<details>
  <summary>Details</summary>
Motivation: 变形形状实例之间的对应估计是计算机图形学中长期存在的问题，从纹理传输到统计建模等众多应用都依赖于恢复准确的对应映射。由于不同下游应用的需求，出现了多种方法，需要系统梳理该领域的最新进展和趋势。

Method: 将发展分为三个范式：1) 基于功能映射的谱方法；2) 施加离散约束的组合方法；3) 直接恢复全局对齐的变形方法。报告系统分析每种方法的优缺点，强调各领域最新进展，并提出潜在研究方向。

Result: 报告提供了该领域的全面综述，帮助研究人员、从业者和学生理解当前趋势。通过分类分析，揭示了不同方法的适用场景和局限性，为选择合适方法提供了指导。

Conclusion: 形状对应估计领域正在快速发展，新兴挑战包括使用视觉基础模型进行零样本对应，以及部分形状匹配等特别困难的任务。报告为未来研究指明了方向，包括跨范式融合和应对实际应用中的复杂场景。
...
### [212] [ColorGradedGaussians: Palette-Based Color Grading for 3D Gaussian Splatting via View-Space Sparse Decomposition](https://arxiv.org/abs/2604.01551)
*Cheng-Kang Ted Chao,Yotam Gingold*

Main category: cs.GR

TL;DR: 提出用于3D高斯泼溅的实时交互式色彩编辑框架，支持基于调色板的重新着色、每调色板色调曲线和像素级约束，通过视图空间调色板分解解决现有方法在alpha混合后稀疏性丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 专业色彩编辑需要精确控制色彩（色调和饱和度）和明度，理想情况下应通过独立控制实现。现有基于调色板的3DGS方法在原始级别分解颜色，优化每个高斯的调色板权重后进行泼溅，但稀疏的原始级别权重在alpha混合后无法保证像素级分解的稀疏性，导致调色板编辑影响非目标区域，降低编辑质量。

Method: 采用视图空间调色板分解方法，泼溅权重而非颜色以优化场景的可观察外观。引入使用逆重心坐标的几何损失来强制执行一致的稀疏模式，确保相似颜色具有相似分解。框架支持基于调色板的重新着色、每调色板色调曲线和像素级约束。

Result: 相比原始空间方法，该方法实现了更优的编辑质量，能够为3DGS场景提供专业色彩分级工作流程，并支持实时交互。

Conclusion: 提出的视图空间调色板分解方法解决了现有3DGS色彩编辑框架的关键限制，通过确保像素级分解的稀疏性，实现了高质量、可控的色彩编辑，为3D高斯泼溅场景提供了专业级的实时交互色彩编辑能力。
...
### [213] [Topology-First B-Rep Meshing](https://arxiv.org/abs/2604.02141)
*YunFan Zhou,Daniel Zint,Nafiseh Izadyar,Michael Tao,Daniele Panozzo,Teseo Schneider*

Main category: cs.GR

TL;DR: 提出一种基于B-Rep拓扑不变性的鲁棒网格生成方法，确保输出网格拓扑正确性，仅几何精度随容差变化，无需后处理修复。


<details>
  <summary>Details</summary>
Motivation: 传统CAD参数化边界表示(B-Rep)模型网格化过程脆弱，高次曲面曲线交点近似为低次基元时，几何实现常违反B-Rep编码的精确拓扑，导致网格邻接关系错误或缺失。现有方法依赖启发式特征合并和修复策略，缺乏拓扑保证且对复杂模型经常失败。

Method: 将B-Rep拓扑视为网格化过程的不变量，算法强制保持精确B-Rep拓扑，同时允许用户定义单个容差控制网格与底层参数曲面的偏差。拓扑正确性独立于几何精度，消除后处理修复需求。

Result: 在数千个ABC和Fusion 360存储库的真实CAD模型上评估，包括标准网格工具失败的实例。结果表明算法层面的拓扑保证能够实现适用于下游应用的可靠网格生成。

Conclusion: 通过将B-Rep拓扑作为网格化过程的不变量，实现了拓扑正确性与几何精度的解耦，为复杂CAD模型提供了鲁棒的网格生成方法，无需后处理修复即可保证拓扑正确性。
