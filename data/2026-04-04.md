...
TL;DR: LCA提出了一种高保真全身3D虚拟人模型，通过预训练/后训练范式解决保真度与泛化性的权衡问题，能够在保持高质量的同时泛化到大规模人群。


<details>
  <summary>Details</summary>
Motivation: 解决3D虚拟人建模中保真度与泛化性的关键权衡问题：多视角工作室数据能实现高保真但泛化性差，而大规模野外数据训练的模型泛化性好但质量低。

Method: 采用预训练/后训练范式：首先在100万野外视频上进行预训练学习外观和几何的广泛先验，然后在高质量策划数据上进行后训练以增强表现力和保真度。

Result: LCA能够泛化到不同发型、服装和人口统计特征，同时提供精确的细粒度面部表情和手指级关节控制，具有强身份保持能力。观察到对重光照、宽松服装支持的无监督泛化能力，以及对风格化图像的零样本鲁棒性。

Conclusion: LCA通过大规模预训练与高质量后训练相结合，成功实现了高保真3D虚拟人模型在大规模人群中的泛化，为3D虚拟人建模提供了新的范式。
...
TL;DR: DOne框架通过解耦结构理解和元素渲染来解决VLMs在设计到代码生成中的"整体瓶颈"问题，在复杂布局数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在设计到代码生成中存在"整体瓶颈"问题，无法同时处理高层次结构层次和细粒度视觉细节，导致布局失真或使用通用占位符。

Method: 提出DOne端到端框架：1) 学习布局分割模块分解复杂设计，避免启发式裁剪限制；2) 专用混合元素检索器处理UI组件的极端宽高比和密度；3) 模式引导生成范式连接布局和代码。

Result: 在HiFi2Code基准测试中，DOne在高层视觉相似度(如GPT Score提升超过10%)和细粒度元素对齐方面均优于现有方法。人工评估显示生产力提高3倍且视觉保真度更高。

Conclusion: DOne通过解耦结构理解和元素渲染有效解决了VLMs在设计到代码生成中的瓶颈问题，在复杂布局场景下实现了显著性能提升。
...
TL;DR: Look Twice (LoT)是一个无需训练、在推理时改进预训练多模态大语言模型利用多模态证据的框架，通过注意力模式估计相关视觉区域和文本元素，并用提示标记突出显示这些证据来引导模型重新关注。
...
Result: 在多个基于知识的VQA基准测试中相比零样本MLLMs取得一致改进；在纯视觉中心和幻觉导向基准测试中，仅视觉证据突出显示也能提升模型性能。

Conclusion: LoT框架有效改进了预训练MLLMs对多模态证据的利用，通过推理时的证据突出显示机制提升模型性能，无需额外训练成本。
...
TL;DR: MedQwen是一个参数高效的医学视觉语言模型，通过频谱路由的专家混合架构和理论驱动的缩放规则，在保持基础架构不变的情况下，实现了接近全参数微调的性能，同时大幅减少可训练参数并缓解持续学习中的灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在通用基准上表现优异，但在医学影像领域缺乏鲁棒性，主要面临三个问题：1) 异构监督导致的跨数据集干扰；2) 对数据机制（监督信号混合方式）敏感；3) 临床工作流中数据和任务顺序到达时，朴素持续训练会导致灾难性遗忘。

Method: 提出MedQwen模型，核心方法包括：1) 频谱路由的专家混合架构，从预训练权重的非重叠奇异值分解片段初始化每个专家；2) 理论驱动的缩放规则，将低秩更新与全秩、全参数微调的MoE对齐；3) 残差补偿和缩放方案，确保在分布偏移下实现稳定的专家专业化和一致路由。

Result: 在23个医学数据集（涵盖视觉问答、报告生成、放射学分类和幻觉缓解）上的评估显示：1) 在零样本分类任务上，仅用1/339的可训练参数就接近全参数微调性能；2) 将顺序遗忘降低至约5%，而强基线方法性能下降20-50%。

Conclusion: MedQwen通过参数高效的专家混合架构和理论驱动的缩放规则，成功解决了医学视觉语言模型中的跨数据集干扰、数据机制敏感性和灾难性遗忘问题，为临床工作流中的持续学习提供了有效的解决方案。
...
TL;DR: 提出基于视觉Transformer的模型，通过不平衡感知训练，在扩展的美式足球擒抱数据集上检测危险动作，相比基线提升8%以上的危险召回率。


<details>
  <summary>Details</summary>
Motivation: 早期识别接触性运动中的危险动作能够及时干预并提高运动员安全性。现有数据集规模有限，需要更有效的方法来检测美式足球训练中的危险擒抱动作。

Method: 使用视觉Transformer模型，结合不平衡感知训练策略，在扩展的数据集（733个单运动员-假人擒抱视频片段）上进行危险动作检测，每个片段围绕首次接触点进行时间定位，并标注SATT-3标准评估的打击区域组件。

Result: 在交叉验证中获得危险召回率0.67和危险F1分数0.59。相比先前在较小数据集上的基线（危险召回率0.58，危险F1分数0.56），危险召回率提升超过8个百分点，在更大数据集上表现更优。

Conclusion: 视觉Transformer视频分析结合类别不平衡的精心处理，能够可靠检测罕见但安全关键性的擒抱模式，为教练中心的伤害预防工具提供了实用途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有姿态估计模型在蹦床体操的极端姿态和非常规视角下表现不佳，需要专门解决方案

Method: 从动作捕捉数据生成合成蹦床姿态数据集(STP)，通过参数化人体模型拟合噪声数据并生成多视角逼真图像，用此数据微调ViTPose模型

Result: 在2D姿态估计上达到SOTA，显著缩小常规与极端姿态的性能差距；3D三角化中MPJPE降低12.5mm，相比预训练模型提升19.6%

Conclusion: 合成数据微调能有效提升姿态估计模型在极端姿态场景下的性能，为特定领域应用提供可行方案
...
Method: 提出Attention Regularization方法：在统计框架下，通过自助法对输入特征重采样，生成注意力分数的基线分布，用于估计注意力分数的显著性和后验概率，去除噪声产生的虚假注意力。
...
TL;DR: 研究发现CNN特征相关性（Gram矩阵）与人类纹理感知之间无关联，且CNN作为视觉系统模型的质量与其纹理感知对齐度无关
...
Method: 比较不同CNN计算的特征相关性（Gram矩阵）捕获的感知内容，并将其与Brain-Score测量的模型与哺乳动物视觉系统的感知对齐度进行比较
...
TL;DR: AffordTissue：用于胆囊切除术中预测工具-动作特定组织可操作性区域的密集热图的多模态框架，通过时空视觉编码、语言条件化和DiT解码器实现，在首个组织可操作性基准上显著优于视觉语言模型基线。


<details>
  <summary>Details</summary>
Motivation: 当前手术动作自动化方法在临床部署中存在局限性：缺乏对器械在组织表面交互位置的预测能力，缺少明确的工具-动作特定安全交互区域的条件输入。需要开发能够预测组织可操作性区域的系统来确保手术自动化安全。

Method: 提出AffordTissue多模态框架：1) 时空视觉编码器捕捉多视角下的工具运动和组织动态；2) 语言条件化实现不同器械-动作对的泛化；3) DiT风格解码器进行密集可操作性预测。建立了首个组织可操作性基准数据集，包含103个胆囊切除术的15,638个视频片段，涵盖6种工具-动作对。

Result: 实验显示显著优于视觉语言模型基线：ASSD指标从Molmo-VLM的60.2像素提升到20.6像素。任务特定架构在大规模基础模型上表现出色，能够准确预测工具-动作特定的组织可操作性区域。

Conclusion: AffordTissue通过预测工具-动作特定的组织可操作性区域，为安全手术自动化提供了明确的空间推理能力，有望实现向适当组织区域的明确策略指导，并在器械偏离预测安全区域时提供早期安全停止机制。
...
TL;DR: GRAZE是一个无需训练的管道，用于在美式足球训练视频中定位首次接触点（FPOC），无需标记的擒抱接触示例，通过候选发现和接触确认分离实现鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 美式足球训练产生大量视频，但关键交互只占长未剪辑片段中的短暂窗口。可靠的生物力学分析需要时空定位来识别交互实体和接触开始时间。研究旨在解决无约束训练视频中的FPOC定位问题，这些视频存在相机运动、杂乱场景、多个相似装备运动员以及冲击时快速姿态变化等挑战。

Method: GRAZE采用训练免费的管道：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知的时间推理进行细化；3) 使用SAM2作为显式的像素级接触验证器，而非仅依赖检测置信度。候选发现和接触确认的分离使方法对杂乱场景和冲击附近不稳定的接地具有鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的片段中产生有效输出，在77.5%的所有片段中在±10帧内定位FPOC，在82.7%的所有片段中在±20帧内定位FPOC。

Conclusion: 结果表明，在真实世界训练视频中进行帧级准确的接触开始定位是可行的，无需任务特定训练。分离候选发现和接触确认的方法对杂乱场景具有鲁棒性，为生物力学分析提供了可靠基础。
...
TL;DR: 提出SVRaster框架解决3DGS在开放词汇3D场景理解中的空间模糊性和多级语义模糊性问题，通过结构化几何表示和确定性特征注册实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的开放词汇3D场景理解方法存在两个关键限制：1）非结构化重叠高斯分布导致的空间模糊性，需要概率特征注册；2）基于物体级掩码池化特征导致的多级语义模糊性，稀释了细粒度细节

Method: 提出SVRaster框架，利用稀疏体素光栅化作为结构化、不相交的几何表示，通过单目深度和法线先验进行正则化，建立稳定几何基础。实现确定性、置信感知的特征注册，并利用AM-RADIO基础模型的密集对齐特性解决多级模糊性，避免分层训练的计算开销

Result: 在开放词汇3D物体检索和点云理解基准测试中达到最先进性能，特别是在细粒度查询任务上表现优异，而传统注册方法通常在此类任务上失败

Conclusion: SVRaster框架通过结构化几何表示解决了3DGS在开放词汇3D场景理解中的核心限制，实现了更精确的特征注册和语义理解，为细粒度3D场景分析提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 从第一人称视频理解和预测物体运动是具身感知和交互的基础，但现有生成模型缺乏显式物理推理，难以生成物理一致的6DoF轨迹，主要挑战包括遮挡、快速运动和物理一致性不足
...
Result: 通过三个案例研究（面部和全身运动、2D和3D数据、个体与多智能体行为）展示了流程的灵活性，证明相同分析工作流可以适应不同实验环境
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模存在重建保真度与生成合理性之间的基本矛盾：前馈重建方法效率高且输入对齐好，但缺乏全局先验导致结构不完整；扩散生成方法能提供丰富几何细节，但多视图一致性差。

Method: 提出UniRecGen统一框架，将重建和生成模型集成到共享规范空间中。采用解耦协作学习，保持训练稳定性并在推理时实现无缝协作。重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和补全几何结构。

Result: 实验结果表明，UniRecGen在保真度和鲁棒性方面表现优异，在从稀疏观测创建完整一致的3D模型方面超越了现有方法。

Conclusion: 通过将重建和生成范式整合到统一协作系统中，UniRecGen成功解决了稀疏视图3D建模中的基本矛盾，实现了更好的结构完整性和多视图一致性。
...
TL;DR: TAG框架通过非参数纹理恢复解决热成像中的鬼影效应，首次实现幽灵人脸的高保真表情恢复，超越现有HADAR技术并揭示其有效性边界。


<details>
  <summary>Details</summary>
Motivation: 热成像在夜视中至关重要，但受到鬼影效应（杂散光子流中纹理细节丢失）的根本限制。现有HADAR技术仅适用于均匀材料场景，而现实世界中材料非均匀性普遍存在，因此需要开发通用的抗鬼影热成像框架。

Method: 提出TAG（热抗鬼影）通用计算热成像框架，利用高光谱光子流进行非参数纹理恢复，解决材料非均匀性问题。该方法通过实验演示了在幽灵人脸（典型的鬼影现象）中前所未有的表情恢复能力。

Result: TAG在多种场景中普遍优于HADAR技术，揭示了材料非均匀性的影响并阐明了HADAR的有效性边界。首次实现了昼夜面部纹理和表情恢复、热3D拓扑对齐和情绪检测。

Conclusion: 该工作为高保真计算夜视建立了通用基础，在自主导航、侦察、医疗保健和野生动物监测等领域具有潜在应用价值，解决了长期存在的鬼影现象挑战。
...
<details>
  <summary>Details</summary>
Motivation: 无人机低空图像语义分割面临极端尺度变化、复杂物体边界和边缘设备计算资源有限的挑战。现有transformer方法计算开销大，轻量级方法难以捕捉高分辨率航拍场景的精细细节。

Method: 提出PBSeg框架：1）原型交叉注意力（PBCA）利用特征冗余降低计算复杂度；2）高效多尺度特征提取模块结合可变形卷积（DConv）和上下文感知调制（CAM）捕捉局部细节和全局语义。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在动态区域初始化不完整，导致重建和运动估计不稳定，通常需要依赖外部预计算光流等密集运动引导，这会引入额外复杂性和误差传播。
...
Result: 实验显示，即使在标准端到端QA设置（Level-3）下，Gemini-3-Pro的正确回答率也低于17%。当施加定位约束时，性能急剧下降：在要求正确答案和精确时空定位时（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。这揭示了表面答案正确性与真实证据推理之间的显著差距。

Conclusion: 基于证据的视频理解仍然是长视频QA的瓶颈。VideoZeroBench暴露了当前模型在细粒度视觉理解和时空推理方面的严重不足，为未来基于证据的视频推理研究提供了重要见解。
...
TL;DR: 提出卫星无关训练(SFT)框架，通过无人机多视角序列重建3D场景并生成伪正射影像，实现无需卫星数据训练的无人机-卫星跨视角地理定位


<details>
  <summary>Details</summary>
Motivation: 现有无人机地理定位方法依赖卫星图像进行训练，但在卫星数据不可用或受限的实际部署场景中受到限制。需要开发无需卫星数据的训练方法。

Method: 1) 使用3D高斯溅射从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影生成几何归一化的伪正射影像；3) 轻量级几何引导修复完善纹理；4) 从无人机数据学习Fisher向量聚合模型，测试时重用编码卫星瓦片进行跨视角检索

Result: 在University-1652和SUES-200数据集上，SFT框架显著优于卫星无关泛化基线，缩小了与使用卫星图像训练方法的性能差距

Conclusion: 提出的卫星无关训练框架通过无人机侧3D重建和伪正射影像生成，实现了无需卫星数据的地理定位训练，为实际部署提供了可行方案
...
<details>
  <summary>Details</summary>
Motivation: 开集测试时适应（OSTTA）面临的主要挑战是：在存在分布偏移的环境中，模型需要同时处理协变量偏移的ID样本（csID）和协变量偏移的OOD样本（csOOD）。现有方法通常结合熵最小化（保持ID性能）和熵最大化（增强OOD检测），但这两种目标存在内在冲突，导致在csID分类和csOOD检测之间需要权衡。

Method: ROSETTA方法包含两个关键组件：1）角度损失：用于调节特征范数的大小；2）特征范数损失：用于抑制csOOD样本的logits值。这些目标共同作用，在保持ID分类性能的同时提升OOD检测能力，避免了熵最小化与最大化之间的直接冲突。

Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C等基准数据集上，ROSETTA在保持高ID分类性能的同时实现了强大的OOD检测能力。在Cityscapes数据集上的实验验证了该方法在真实世界语义分割任务中的有效性，在HAC数据集上的结果证明了其在不同开集TTA设置中的适用性。

Conclusion: ROSETTA通过引入角度损失和特征范数损失，有效解决了开集测试时适应中熵最小化与最大化之间的冲突，在多个基准数据集和实际应用场景中均表现出色，为处理协变量偏移环境下的开集适应问题提供了一种有效的解决方案。
...
TL;DR: F3DGS是一个用于去中心化多智能体3D重建的联邦3D高斯溅射框架，通过共享几何骨架和可见性感知聚合解决分布式机器人场景中的通信开销和几何不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS流水线假设集中访问所有观测数据，这在分布式机器人设置中受限，因为智能体独立操作且集中数据聚合可能受限。直接将集中训练扩展到多智能体系统会引入通信开销和几何不一致问题。

Method: 首先通过注册多个客户端本地合并的LiDAR点云构建共享几何骨架来初始化全局3DGS模型。联邦优化期间固定高斯位置以保持几何对齐，每个客户端仅更新外观相关属性（协方差、不透明度和球谐系数）。服务器使用可见性感知聚合这些更新，根据每个客户端观察每个高斯的频率加权其贡献。

Result: 实验表明F3DGS在实现与集中训练相当的重建质量的同时，支持跨智能体的分布式优化。收集了包含同步LiDAR、RGB和IMU测量的多序列室内数据集进行评估。

Conclusion: F3DGS为分布式机器人环境中的3D重建提供了一种有效的联邦学习方法，解决了多智能体探索中的部分可观测性挑战，同时保持重建质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型对物理可实现的对抗攻击鲁棒性研究不足。现有攻击方法（语言扰动和2D视觉攻击）要么不具代表性，要么物理真实性有限。相比之下，对抗性3D纹理更具物理合理性和破坏性，但标准3D仿真器缺乏从VLA目标函数到物体外观的可微分优化路径，难以进行端到端优化。

Method: 提出Tex3D框架，包含两个核心技术：1) 前景-背景解耦(FBD)：通过双渲染器对齐实现可微分纹理优化，同时保留原始仿真环境；2) 轨迹感知对抗优化(TAAO)：优先处理行为关键帧，并使用基于顶点的参数化稳定优化过程。

Result: 在仿真和真实机器人环境中，Tex3D显著降低了VLA模型在多个操作任务中的性能，任务失败率高达96.7%。实验结果表明VLA系统对物理基础的3D对抗攻击存在严重脆弱性。

Conclusion: 该研究首次展示了VLA系统对物理可部署3D对抗纹理的脆弱性，提出了首个端到端优化框架Tex3D，并强调了需要鲁棒性感知训练来增强VLA系统的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 形态特征对理解生物与环境相互作用至关重要，但传统人工标注方法耗时且专家依赖性强，限制了大规模生态研究。缺乏高质量图像-特征标注数据集是主要瓶颈。

Method: 在基础模型特征上训练稀疏自编码器，获得单语义、空间定位的神经元；构建特征标注流程：定位显著区域，使用视觉语言提示生成可解释的特征描述；创建Bioscan-Traits数据集（8万标注，1.9万昆虫图像）。

Result: 人类评估确认生成形态描述具有生物学合理性；通过全面消融研究评估设计敏感性，系统分析关键设计选择对特征描述质量的影响。

Conclusion: 通过模块化流水线而非昂贵人工标注，提供了可扩展的生物学监督注入基础模型的方法，支持大规模形态分析，弥合生态相关性与机器学习实用性之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法主要关注静态几何重建，而忽略了场景尺度的环境动态（如云、水、烟雾）。建模这些动态具有挑战性，因为运动需要在扩展场景中保持连贯，同时支持低延迟的用户反馈。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测在密集标注数据集上表现良好，但在现实场景中通常只有部分目标被标注，因为3D标注成本高昂。稀疏标注设置在实际应用中很常见，但现有方法在此设置下表现不佳。

Method: 提出包含两个关键模块的框架：1) 道路感知补丁增强(RAPA)：利用稀疏标注，将分割的目标补丁增强到道路区域，同时保持3D几何一致性；2) 原型过滤(PBF)：通过原型相似性和深度不确定性过滤预测结果生成高质量伪标签，维护全局2D RoI特征原型，选择与学习原型特征一致且深度估计可靠的伪标签。

Result: 大量实验证明了所提方法的有效性。源代码已在GitHub上公开。

Conclusion: 提出的结合几何保持增强和原型引导伪标注的训练策略，在稀疏监督下实现了鲁棒的3D目标检测，解决了现实场景中标注稀疏的问题。
...
Method: 利用莫尔效应：相机拍摄双层光栅结构时产生的干涉条纹。推导莫尔运动不变量，证明条纹相位与光栅图像位移通过光学几何线性耦合，与观看距离和光栅结构无关。验证器从视频中提取两个信号并测试其相关性

Result: 在真实拍摄和多个最先进AI生成器的视频上验证不变量，发现真实视频和AI生成视频产生显著不同的相关特征，表明该方法能有效区分两者
...
TL;DR: DynaVid：利用合成运动数据增强视频扩散模型动态运动生成能力的框架
...
Method: 采用两阶段生成框架：1）运动生成器合成光学流表示的运动；2）运动引导的视频生成器基于运动条件生成视频帧。使用计算机图形流水线渲染合成运动数据，仅编码运动信息而不包含外观

Result: 在剧烈人体运动生成和极端相机运动控制两个具有挑战性的场景中，DynaVid显著提升了动态运动生成的逼真度和可控性

Conclusion: 通过合成运动数据与真实视频数据的解耦学习，DynaVid能够从合成数据学习动态运动模式，同时从真实数据保持视觉逼真度，有效解决了现有数据集在动态运动方面的局限性
...
Method: 1. 设计解耦表示机制，去除非必要的环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征；2. 采用权重融合策略，在参数空间中动态整合新旧环境知识，无需存储历史数据，同时最大化保留旧环境的判别能力。

Result: 在多个标准基准数据集上的广泛实验表明，该方法在完全无示例和无领域ID的设置下显著减少了灾难性遗忘，其准确率优于现有的最先进方法。

Conclusion: 提出的框架解决了具身多媒体系统中连续环境适应的关键问题，通过解耦表示和权重融合实现了鲁棒的增量学习，在未知交互场景中具有更好的实用性和泛化能力。
...
TL;DR: 该论文提出频率域适应(FDA)和谐波约束最优传输(HOT)框架，用于解决远程光电容积描记(rPPG)在跨域部署时的性能下降问题，通过转移低频光谱成分来学习对光照、相机特性等外观变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 远程光电容积描记(rPPG)技术虽然能够从面部视频进行非接触式生理测量，但在实际部署中面临严重的域偏移性能下降问题。现有的深度学习方法虽然在单个数据集上表现良好，但容易过度拟合光照、相机特性、颜色响应等外观相关因素，这些因素在不同域间变化显著。
...
Conclusion: 频率域适应和谐波约束最优传输为解决rPPG跨域性能下降问题提供了有效的解决方案，通过建模外观变化并利用心脏信号的谐波特性，显著提升了模型的泛化能力和实际部署的可行性。
...
Conclusion: 嵌入实例一致的语义自然补充了4D建模，实现了更准确的场景分解，同时支持鲁棒的动态场景理解，为联合建模外观、运动和语义提供了统一框架。
...
TL;DR: 提出UnderEraser方法，通过外部蒸馏和内部跨注意力机制，在视频对象移除中同时消除目标对象及其引发的副作用（阴影、反射等），保持时空一致性
...
Method: 提出双视角理解方法：1）外部：通过蒸馏方案将视觉基础模型中对象与副作用的关系迁移到视频扩散模型；2）内部：提出帧间上下文跨注意力机制，使每个去噪块基于目标区域周围未掩码的上下文信息

Result: 实验证明该方法达到最先进性能，并建立了首个真实世界视频对象移除基准，促进未来研究和社区发展

Conclusion: 外部和内部引导联合使模型能够理解目标对象、其引发的副作用以及全局背景上下文，实现清晰连贯的对象移除
...
Conclusion: 提出的端到端方法通过同时进行群体检测和共享注意力估计，有效解决了现有方法的局限性，提高了共享注意力检测的实用性和性能。
...
Conclusion: SteerFlow提供了一个具有强理论保证的模型无关编辑框架，在保持源保真度的同时实现高质量的文本引导图像编辑，并能扩展到复杂多轮编辑场景。
...
TL;DR: 提出DPMO方法将点标注转换为掩码标注，并结合RPS框架实现密集人群实例分割，在多个数据集上达到SOTA性能，同时证明掩码监督能提升计数精度。


<details>
  <summary>Details</summary>
Motivation: 当前人群数据集主要使用点标注，而区域标注（如边界框）稀缺且不准确。直接应用SAM等基础模型在密集人群场景效果不佳，需要专门的方法从点标注生成实例分割掩码。
...
Result: 在ShanghaiTech、UCF-QNRF、JHU-CROWD++和NWPU-Crowd数据集上实现了最先进的密集人群实例分割性能。掩码监督的损失函数显著提升了不同模型的计数精度。

Conclusion: DPMO方法能有效将点标注转换为掩码标注，RPS框架在密集人群实例分割中表现优异，掩码标注在提升计数精度方面发挥重要作用，为人群分析任务提供了新的解决方案。
...
TL;DR: 提出几何感知的无人机地理定位框架，通过3D场景重建和虚拟鸟瞰图渲染统一粗粒度位置识别与细粒度姿态估计
...
Method: 提出几何感知框架：1) 使用视觉几何基础变换器(VGGT)从多视角无人机图像序列重建局部3D场景；2) 渲染虚拟鸟瞰图(BEV)表示，将无人机视角正交校正以对齐卫星图像；3) 引入卫星级注意力块处理多个位置假设，隔离每个卫星候选与重建场景的交互；4) 发布重新校准的University-1652数据集，包含精确坐标标注和空间重叠分析。

Result: 在重新校准的University-1652基准和SUES-200数据集上的实验表明，该方法显著优于现有最先进基线，实现鲁棒的米级定位精度，在复杂城市环境中具有更好的泛化能力。

Conclusion: 通过显式建模3D场景几何，将粗粒度位置识别和细粒度姿态估计统一到单个推理流程中，有效解决了无人机地理定位中的几何差异问题，为GNSS拒止环境提供了实用的解决方案。
...
TL;DR: 该研究构建了US-365K超声图像-文本数据集和超声诊断分类体系(UDT)，提出了Ultrasound-CLIP框架，通过语义感知对比学习和异构图推理提升超声图像理解性能。
...
Method: 1) 构建US-365K大规模超声图像-文本数据集(36.5万配对样本，52个解剖类别)；2) 建立超声诊断分类体系(UDT)，包含超声层次解剖分类和超声诊断属性框架(9个诊断维度)；3) 提出Ultrasound-CLIP框架，引入语义软标签和语义损失进行样本细粒度区分；4) 构建基于UDAF文本表示的异构图模态，支持病变-属性关系的结构化推理。

Result: 在患者级数据划分下，该方法在分类和检索基准测试中达到最先进性能，同时在零样本学习、线性探测和微调任务中表现出强大的泛化能力。

Conclusion: 该研究通过构建专门的大规模超声数据集和诊断分类体系，结合语义感知对比学习和结构化推理，有效解决了超声图像理解中的模态适配问题，为超声AI诊断提供了有力工具。
...
TL;DR: 提出一种使用自监督学习特征作为预训练视频扩散模型条件信号的方法，通过轻量级架构解耦外观特征，实现视频域迁移和3D到视频生成等任务。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型主要用作生成渲染器，通常通过感知、几何或简单语义信号进行条件控制。自监督学习获得的高维特征虽然包含丰富的场景信息，但风格、光照和语义信息纠缠在一起，限制了其生成能力。需要一种方法能够利用这些特征作为更通用的条件信号，同时解耦外观特征以实现更好的控制。

Method: 引入轻量级架构和训练策略，将外观特征与其他需要保留的特征解耦。通过补偿低空间分辨率与高特征维度，提高从显式空间表示进行生成渲染的可控性。

Result: 该方法能够实现视频域迁移和从3D到视频的生成任务。通过解耦外观特征，实现了对风格化和重新光照等外观变化的鲁棒控制，提高了生成渲染的可控性。

Conclusion: 自监督学习特征可以作为预训练视频扩散模型的通用条件信号。通过解耦外观特征的方法，能够有效利用这些特征进行视频生成和控制任务，为视频生成和世界模拟提供了新的可能性。
...
TL;DR: 本文介绍了RebusBench基准测试，用于评估大视觉语言模型在解决需要神经符号推理的谜语谜题方面的能力，发现现有模型表现严重不足。


<details>
  <summary>Details</summary>
Motivation: 当前大视觉语言模型在显式视觉识别方面表现出色，但在需要复杂多步推理的问题上存在认知鸿沟，特别是当视觉输入仅作为线索而非答案时。模型缺乏将视觉感知与先验知识整合的神经符号推理能力。

Method: 作者构建了RebusBench基准测试，包含1,164个谜语谜题，用于评估模型整合视觉感知和语言知识的能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，测试了模型缩放和上下文学习的影响。

Result: 评估显示模型表现严重不足：精确匹配率低于10%，语义准确率低于20%。模型缩放和上下文学习均未带来显著改进，表明模型虽然具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理能力。

Conclusion: 大视觉语言模型在需要神经符号推理的任务上存在根本性缺陷，缺乏将视觉感知与先验知识整合的认知推理能力。RebusBench为评估这一关键能力提供了基准，揭示了当前模型的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型（WAM）主要关注2D外观或潜在表示建模，缺乏几何基础，而几何基础对于在物理世界中运行的具身系统至关重要。需要开发一个既具备推理和指令跟随能力，又能进行时空世界建模的几何感知驾驶模型。

Method: 提出统一的驾驶世界-动作模型，采用大型语言模型处理语言指令、多视角图像和动作，然后通过三个轻量级生成器分别生成深度、未来视频和动作。模型学习几何感知的世界表示，并用其在统一框架内指导未来预测和规划。

Result: 在Navsim v1和v2基准测试中表现优异：Navsim v1达到89.2 PDMS，Navsim v2达到88.7 EPDMS，优于现有基于世界模型的方法，同时生成更高质量的深度和未来视频预测。消融研究表明显式深度学习为视频想象提供互补优势并提升规划鲁棒性。
...
TL;DR: FSKD是一个LiDAR到RGB-红外(RGBI)的知识蒸馏框架，通过多模态教师融合LiDAR数据和RGBI图像，训练仅使用RGBI的学生模型来预测森林结构指标，实现高分辨率零样本监测。


<details>
  <summary>Details</summary>
Motivation: 虽然机载LiDAR是森林结构监测的黄金标准，但成本高昂且采集频率低。需要开发仅使用更易获取的RGBI图像就能准确预测森林结构指标的方法，以实现可扩展的高分辨率森林监测。

Method: 提出FSKD知识蒸馏框架：多模态教师模型通过交叉注意力融合RGBI图像与LiDAR导出的平面指标和垂直剖面，训练仅使用RGBI的SegFormer学生模型来复现这些输出。在德国萨克森州384平方公里森林数据上训练，使用20厘米地面采样距离数据。

Result: 学生模型在八个地理分布不同的测试区域实现零样本CHM预测SOTA性能：MedAE 4.17米，R²=0.51，IoU 0.87。相比HRCHM/DAC基线，MAE改善29-46%（5.81米 vs. 8.14-10.84米），相关系数更强（0.713 vs. 0.166-0.652）。多模态融合比仅RGBI训练提升10-26%性能，能联合预测CHM、PAI和FHD。

Conclusion: FSKD框架通过知识蒸馏实现了仅使用RGBI图像就能准确预测森林结构指标，消除了严格的同步采集限制，支持时间不匹配数据（冬季LiDAR，夏季RGBI），为德国数字孪生和国家数字正射影像计划等应用提供了可扩展的20厘米分辨率操作监测方案。
...
TL;DR: 提出GardenDesigner框架，通过智能体链和程序化建模自动生成江南园林数字资产，解决人工建模耗时问题，实现非专家用户通过文本输入快速创建园林
...
Method: 提出GardenDesigner框架，编码江南园林美学原则，基于程序化建模构建智能体链：地形分布智能体应用水中心地形规则，道路生成智能体应用探索路径规则，资产选择和布局优化智能体遵循美学文化约束选择安排对象，引入GardenVerse知识库增强资产布置

Result: 开发Unity交互界面和工具，非专家用户可通过文本输入在一分钟内构建江南园林；实验和人工评估表明系统能生成多样且美观的江南园林

Conclusion: GardenDesigner成功实现江南园林的自动化生成，为数字内容创作提供高效解决方案，项目页面已公开
...
TL;DR: 提出一种利用轮式里程计实现时间一致性的单目深度估计框架，通过三角测量和贝叶斯估计更新尺度，在多个数据集上验证了性能


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在连续帧间缺乏时间一致性，导致抖动和深度范围突变时的估计失败，需要解决自动驾驶和移动机器人感知系统中的这一关键问题

Method: 利用移动机器人的轮式里程计，通过连续帧间的光流进行三角测量估计相机位姿和稀疏深度，使用稀疏深度更新度量尺度的递归贝叶斯估计，然后将该尺度应用于预训练深度估计基础模型预测的相对深度进行重缩放

Result: 在KITTI、TartanAir、MS2和自有数据集上进行了评估，展示了鲁棒且准确的深度估计性能

Conclusion: 提出的基于一致性的单目深度估计框架通过融合轮式里程计信息，有效解决了时间一致性问题，实现了稳定连贯的深度预测
...
TL;DR: STRIVE：一种用于视频问答的时空强化学习框架，通过构建多个视频变体和跨文本生成与视觉变体的联合归一化，解决组策略优化中奖励方差低的问题，提升视频推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于组的策略优化方法在大型多模态模型中表现良好，但当回答具有相似正确性时，往往面临奖励方差低的问题，导致优势估计弱或不稳定。这限制了视频问答中强化学习的效果。

Method: STRIVE框架包含三个核心组件：1）构建输入视频的多个时空变体；2）在文本生成和视觉变体上进行联合归一化；3）引入重要性感知采样机制，优先选择与问题最相关的帧，同时保持时间覆盖。该方法通过结构化视觉扰动扩展组比较，丰富奖励信号。

Result: 在六个具有挑战性的视频推理基准测试（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench、PerceptionTest）上，STRIVE在多个大型多模态模型中相比强强化学习基线取得了一致的性能提升。

Conclusion: 结构化时空探索作为一种原则性机制，能够稳定多模态强化学习并提升视频推理性能。STRIVE通过将组比较从语言多样性扩展到结构化视觉扰动，促进了跨互补视觉视角的鲁棒推理，而非过拟合单一时空配置。
...
TL;DR: SafeRoPE：针对MMDiT的轻量级安全生成框架，通过头部级不安全子空间分析和RoPE扰动，有效抑制不安全语义同时保持生成质量


<details>
  <summary>Details</summary>
Motivation: 现有基于U-Net的扩散模型安全缓解方法计算开销大，且难以直接适配基于transformer的扩散模型（如MMDiT）。研究发现不安全语义集中在可解释的低维子空间中，特定安全关键头部负责不安全特征提取，而扰动RoPE可以有效修改生成图像中的特定概念。

Method: 1. 分析MMDiT注意力机制，识别安全关键头部；2. 通过分解不安全嵌入构建头部级不安全子空间；3. 计算潜在风险分数（LRS）评估输入向量风险；4. 引入头部级RoPE扰动，在不影响良性内容的情况下抑制不安全语义；5. 结合LRS和RoPE扰动对查询和键向量嵌入进行风险特定的头部级旋转。

Result: 实验表明SafeRoPE在平衡有害内容缓解和效用保持方面达到SOTA性能，有效抑制不安全输出同时保持生成保真度。代码已开源。

Conclusion: SafeRoPE为MMDiT提供了一种轻量级、细粒度的安全生成框架，通过头部级不安全子空间分析和RoPE扰动，实现了精确的不安全语义抑制，解决了现有方法计算开销大且难以适配transformer架构的问题。
...
TL;DR: 论文提出随机标签桥接训练方法，通过添加桥接训练阶段使大语言模型参数适应视觉任务，无需人工标注，并发现部分桥接训练效果更佳


<details>
  <summary>Details</summary>
Motivation: 语言预训练模型和视觉预训练模型的异常参数比例差异显著，使得跨模态（语言和视觉）比跨域适应更具挑战性。先前研究多集中于跨域迁移而非桥接语言和视觉模态，认为语言预训练模型不适合下游视觉任务。本文挑战这一假设，探索语言预训练参数在视觉模型中的直接应用潜力。

Method: 提出随机标签桥接训练方法：添加桥接训练阶段作为模态适应学习器，无需人工标注。该方法通过随机标签训练帮助LLM参数适应视觉基础任务。研究发现部分桥接训练（仅调整部分层）往往更有优势，因为LLM中的某些层表现出强大的基础属性，即使不针对视觉任务进行微调也保持有益。

Result: 研究表明桥接训练阶段能有效对齐大语言模型参数与视觉任务。随机标签桥接训练方法简单而强大，无需手动标注。部分桥接训练通常更有利，某些LLM层的基础属性在视觉任务中仍然有益。

Conclusion: 语言预训练参数可直接应用于视觉模型，挑战了传统假设。部分桥接训练为跨模态适应提供了实用途径，为利用语言预训练参数进行视觉任务开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像严重程度分类存在域偏移问题，现有半监督域自适应方法在处理有序类别标签时效果不佳，因为类别边界模糊且类别具有自然顺序

Method: 提出基于秩分数的对齐方法：1) 跨域排序：利用类别顺序信息对跨域样本对进行排序；2) 连续分布对齐：对齐源域和目标域的秩分数分布

Result: 在溃疡性结肠炎和糖尿病视网膜病变分类任务上验证了方法的有效性，成功实现了类别特定秩分数分布的对齐

Conclusion: 该方法通过利用类别顺序信息学习秩分数，有效解决了严重程度分类中的域自适应问题，为医学图像分析提供了新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 纹理3D网格同时包含几何、拓扑和外观信息，但其不规则结构给深度学习语义分割带来挑战。现有方法通常直接处理网格但忽略了丰富的纹理信息，需要一种能有效利用纹理数据的解决方案。

Method: 引入纹理感知Transformer：1）纹理分支将每个面的像素汇总为可学习token；2）与几何描述符融合；3）通过两阶段Transformer块（TSTB）处理，支持局部和全局信息流；4）采用新的分层学习方案进行多尺度特征聚合。

Result: 在Semantic Urban Meshes（SUM）基准上达到81.9% mF1和94.3% OA；在新构建的文化遗产数据集（带纹理的屋顶瓦片）上达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 提出的纹理感知Transformer模型能有效利用纹理3D网格中的丰富纹理信息，通过结合几何和纹理特征以及多尺度学习，在城市场景和文化遗产场景的语义分割任务中表现出色。
...
TL;DR: PI-VQ提出位置无关的离散表示方法，通过排列不变性约束和匹配量化算法，实现无需位置信息的图像表示与单次前向传播的图像合成。


<details>
  <summary>Details</summary>
Motivation: 传统向量量化方法（VQ-VAE、VQ-GAN）学习的离散表示具有位置依赖性：编码在空间上排列且上下文纠缠，需要自回归或扩散先验来建模其依赖关系。本文探讨位置信息是否对空间对齐数据的离散表示是必要的。

Method: 提出排列不变向量量化自编码器（PI-VQ），约束潜在编码不携带位置信息。为应对排列不变表示的信息容量减少问题，引入基于最优二分图匹配的匹配量化算法，相比朴素最近邻量化提高3.5倍有效瓶颈容量。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得具有竞争力的精度、密度和覆盖度指标。学习到的编码具有组合结构，支持基于插值的采样，允许在单次前向传播中合成新图像。

Conclusion: 位置无关表示在分离性和可解释性方面存在固有权衡，为未来工作指明方向。PI-VQ能够捕获全局语义特征，实现无需学习先验的直接图像插值。
...
TL;DR: FaCT-GS是一种基于高斯泼溅的快速灵活CT重建框架，通过优化体素化和光栅化流程，在速度和可扩展性上显著超越现有方法，并支持从现有体积快速拟合高斯作为先验。


<details>
  <summary>Details</summary>
Motivation: 高斯泼溅(GS)在CT重建中表现优异但优势不够显著，未能推动从传统算法的过渡。现有GS方法在速度、可扩展性和灵活性方面仍有重要限制需要解决。

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流水线，实现快速CT重建。改进的体素化支持从现有体积快速拟合高斯，可作为先验用于热启动重建或作为压缩表示。

Result: 在标准512×512投影上比现有最佳GS CT重建快4倍以上，在2k投影上快13倍以上。方法具有良好的可扩展性，能适应不同投影和输出体积大小。

Conclusion: FaCT-GS解决了GS在CT重建中的主要限制，提供了快速、灵活且可扩展的解决方案，支持从现有体积快速拟合高斯作为先验，有望推动高斯泼溅在CT重建中的实际应用。
...
TL;DR: HieraVid是一个分层剪枝框架，通过分段级、帧级和层级三个层次逐步动态减少视频冗余，在仅保留30%令牌的情况下实现SOTA性能，同时保持LLaVA-Video-7B和LLaVA-OneVision-7B超过98%和99%的性能。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解方面表现出色，但输入视频令牌数量巨大导致计算负担重。现有方法主要在输入级别剪枝视频令牌，忽略了视频和大语言模型固有的信息结构。

Method: 基于视频具有分段-帧结构和LLM内部单向传播多模态信息的观察，提出分层剪枝框架：1)分段级：视频令牌先按时间分段和空间合并；2)帧级：同一分段内相似帧联合剪枝以保持多样性；3)层级：随着LLM层数增加，冗余逐渐减少而不影响性能。

Result: 在四个广泛使用的视频理解基准上进行了广泛实验。仅保留30%令牌的情况下，HieraVid实现了新的最先进性能，同时保持LLaVA-Video-7B超过98%的性能和LLaVA-OneVision-7B超过99%的性能。
...
TL;DR: 现代视觉语言模型在基本几何变换下存在系统性脆弱性，缺乏鲁棒的空间不变性和等变性，导致在简单旋转、缩放和恒等变换下无法可靠识别物体


<details>
  <summary>Details</summary>
Motivation: 研究最先进的视觉语言模型在基本几何变换下的根本脆弱性。虽然现代VLM在语义任务上表现出色，但在更基础的空间推理层面存在系统性缺陷，需要更强的几何基础

Method: 通过系统评估方法，在符号草图、自然照片和抽象艺术等多样化视觉领域中测试VLM对旋转、缩放和恒等变换的鲁棒性，涵盖不同架构、模型容量和提示策略

Result: 随着语义内容变得稀疏，性能急剧下降；这种行为在不同架构、模型容量和提示策略中普遍存在，揭示了当前VLM在语义理解和空间推理之间的系统性差距

Conclusion: 当前视觉语言模型在语义理解和空间推理之间存在系统性差距，突显了未来多模态系统需要更强的几何基础来弥补这一缺陷
...
Method: 提出双损失训练框架：1) 边界回归损失通过单通道边界预测提升时间定位精度；2) 基于CDF的片段级正则化损失通过匹配预测和真实片段的累积分布来增强片段内结构一致性。框架与架构无关，可集成到现有TAS模型中
...
Method: 提出MAR-MAER框架，包含两个核心组件：1）度量感知嵌入正则化方法，使用轻量级投影头和自适应核回归损失函数，将模型内部表示与人类偏好度量（如CLIPScore和HPSv2）对齐；2）条件变分模块，在分层令牌生成过程中引入受控随机性，处理模糊语义。

Result: 在COCO和新开发的模糊提示基准测试中，MAR-MAER在度量一致性和语义灵活性方面表现出色，相比基线Hi-MAR模型，CLIPScore提升+1.6，HPSv2提升+5.3，对模糊输入能产生更广泛的输出多样性。

Conclusion: MAR-MAER通过结合度量感知正则化和概率潜在建模，有效解决了自回归文本到图像生成中的质量控制和模糊提示处理问题，在人类评估和自动化指标上都取得了显著改进。
...
TL;DR: A3R：一个用于3D高斯场景中细粒度可供性推理的智能体框架，通过跨维度证据获取的序列决策过程解决静态观察下证据不足的问题


<details>
  <summary>Details</summary>
Motivation: 现有方法将可供性推理视为静态场景观察下的一次性预测，假设已有足够证据进行推理。但在复杂3D场景中，许多失败案例并非源于预测能力弱，而是固定观察下任务相关证据不完整。需要将细粒度可供性推理重新定义为序列证据获取过程

Method: 提出A3R框架：1）将可供性推理重新定义为序列证据获取过程，通过互补的3D几何和2D语义证据逐步减少歧义；2）基于MLLM的策略迭代选择证据获取动作并更新可供性信念；3）引入GRPO-based策略学习方法优化序列决策，提高证据获取效率和推理准确性

Result: 在场景级基准测试上的广泛实验表明，A3R始终优于静态一次性基线方法，证明了在复杂3D高斯场景中，智能体跨维度证据获取对细粒度可供性推理的优势

Conclusion: 通过将可供性推理重新定义为序列证据获取过程，并采用智能体跨维度证据获取框架，能够有效解决复杂3D场景中固定观察下证据不足的问题，显著提高细粒度可供性推理的准确性
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）在实时渲染方面表现出色，但大量高斯点导致高内存消耗。现有剪枝方法虽然节省内存，但会损害空间一致性并产生渲染伪影，需要在保持高质量的同时实现紧凑表示。

Method: 1. 基于证据下界（ELBO）的自适应致密化策略，自动控制高斯点生成过程；2. 透明度感知渐进剪枝策略，动态移除低透明度高斯点；3. 基于图的特征编码模块，通过特征引导的点位移调整空间分布。

Result: GS^2仅使用约12.5%的高斯点就实现了比原始3DGS更高的PSNR，在渲染质量和内存效率方面均优于所有基线方法，实现了紧凑的高斯表示和卓越的渲染质量。

Conclusion: GS^2通过优化高斯点的空间分布，在显著减少内存消耗的同时提升了渲染质量，解决了3DGS内存效率与渲染质量之间的权衡问题，为紧凑3D高斯表示提供了有效解决方案。
...
TL;DR: 本文系统研究了文本到图像生成模型的安全漏洞，发现仅通过自然语言提示即可绕过安全过滤器，攻击成功率高达74.47%。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型被广泛部署于创意工具和在线平台，依赖安全过滤器和审核管道来阻止有害内容。然而，这些系统仍然存在安全漏洞，需要研究其脆弱性。

Method: 提出了一种基于提示的越狱攻击分类法，包括艺术重构、材料替换、伪教育框架、生活方式美学伪装和模糊动作替换等策略。这些方法不需要模型访问、优化或对抗训练，仅通过自然语言提示修改来绕过安全过滤。

Result: 在多个最先进的文本到图像系统上评估这些攻击，发现简单的语言修改可以可靠地规避现有安全措施并生成受限图像。所有测试模型和攻击类别的攻击成功率（ASR）高达74.47%。

Conclusion: 研究结果突显了表面级提示过滤与检测生成媒体系统中对抗意图所需的语义理解之间存在关键差距。现有安全措施无法有效识别隐藏在良性语义上下文中的不安全意图。
...
TL;DR: ProVG是一个用于遥感视觉定位的新框架，通过将语言表达解耦为全局上下文、空间关系和对象属性，并采用渐进式跨模态调制器实现从粗到细的视觉语言对齐，显著提升了定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法主要依赖句子级的视觉语言对齐，难以利用空间关系和对象属性等细粒度语言线索，而这些线索对于区分相似特征的对象至关重要。不同线索在不同定位阶段应发挥不同作用，需要相应地被利用以提供更明确的指导。

Method: 1. 将语言表达解耦为全局上下文、空间关系和对象属性三个部分；2. 设计渐进式跨模态调制器，通过"调查-定位-验证"方案动态调制视觉注意力，实现从粗到细的视觉语言对齐；3. 引入跨尺度融合模块处理遥感图像的大尺度变化；4. 使用语言引导的校准解码器在预测时细化跨模态对齐；5. 统一的多任务头支持指称表达理解和分割任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在合成任意形状时可能无法充分学习表面的精细细节，或者在效率和精度之间需要权衡。SHARC旨在通过优化放置参考点来最大化表面细节的学习，同时保持模型简洁性。

Method: 1. 在表面内部体积中优化放置参考点，通过联合最大化稀疏性、中心性和表面可见性的成本函数；2. 对每个参考点，通过光线投射采样到表面的可见距离场；3. 使用快速球谐变换计算SH系数；4. 应用可配置的低通滤波器增强几何保真度；5. 基于邻近性使用局部一致性约束细化输出。

Result: SHARC在重建精度和时间效率方面均优于现有最先进方法，且不牺牲模型简洁性。评估表明该方法在多个指标上表现优异。

Conclusion: SHARC通过球谐函数表示距离场和优化参考点放置，提供了一种高效、精确的形状合成方法，在保持模型简洁性的同时显著提升了性能。源代码已公开。
...
TL;DR: FTPFusion：一种基于时序扰动和稀疏跨模态交互的频率感知红外与可见光视频融合方法，在保持时空稳定性的同时保留高频细节


<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法存在局限性：要么侧重于帧级增强而缺乏充分的时序建模，要么依赖于繁重的时空聚合而牺牲高频细节。如何在保持时序稳定性的同时保留空间细节是一个根本性挑战。

Method: 1. 将特征表示分解为高频和低频分量进行协同建模；2. 高频分支执行稀疏跨模态时空交互以捕捉运动相关上下文和互补细节；3. 低频分支引入时序扰动策略以增强对复杂视频变化（如闪烁、抖动和局部错位）的鲁棒性；4. 设计偏移感知时序一致性约束，在时序扰动下显式稳定跨帧表示。

Result: 在多个公共基准测试上的广泛实验表明，FTPFusion在空间保真度和时序一致性方面均优于最先进的方法，并在多个指标上取得一致优势。

Conclusion: FTPFusion通过频率分解、稀疏跨模态交互和时序扰动策略，有效解决了红外与可见光视频融合中的时空稳定性与细节保留难题，为智能监控和低光监测应用提供了高质量的融合解决方案。
...
TL;DR: 提出Light-ResKAN模型，通过KAN卷积替换传统卷积、使用Gram多项式激活函数和参数共享策略，在保持SAR图像识别精度的同时大幅降低计算量和参数量，适用于边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: SAR图像识别在灾害监测、军事侦察等领域至关重要，但大尺寸SAR图像和深度学习的计算需求限制了在资源受限边缘设备上的部署。现有轻量化模型难以平衡高精度特征提取与低计算要求之间的矛盾。

Method: 1. 基于KAN思想改进ResNet，用KAN卷积替换传统卷积，实现自适应特征提取；2. 使用Gram多项式作为激活函数，更适合SAR数据捕捉复杂非线性关系；3. 采用参数共享策略：每个核按通道共享参数，在保留独特特征的同时减少参数和FLOPs。
...
TL;DR: 提出了一种基于几何和星座匹配的多重角膜反射检测与匹配方法，专注于可重复性和清晰评估


<details>
  <summary>Details</summary>
Motivation: 角膜反射检测在P-CR眼动追踪中很重要，但实践中常作为启发式方法嵌入大系统，导致不同硬件设置下难以复现

Method: 采用2D几何驱动的星座式管道，将反射点视为结构化星座而非独立斑点；提出相似性-布局对齐(SLA)程序，将星座匹配适配到多LED眼动追踪的特定约束；结合受控过检测、自适应候选回退、外观感知评分和可选语义布局先验

Result: 在公开多LED数据集上评估，系统在噪声条件下提供稳定的身份保持对应关系

Conclusion: 该方法实现了可重复的多重角膜反射检测与匹配，通过发布代码、预设和评估脚本支持透明复现、比较和数据集标注
...
TL;DR: KnowMVG提出了一种知识先验和全局-局部注意力增强框架，通过编码医学知识嵌入和联合利用粗粒度全局信息与细粒度局部线索，显著提升医学视觉定位的空间精度。
...
Method: 提出KnowMVG框架：1）知识增强提示策略，将短语相关的医学知识编码为紧凑嵌入；2）全局-局部注意力机制，联合利用粗粒度全局信息和细粒度局部线索来指导精确区域定位，桥接高级语义理解和细粒度视觉感知。

Result: 在四个MVG基准测试上的实验表明，KnowMVG始终优于现有方法，在AP50指标上提升3.0%，在mIoU指标上提升2.6%，超越了先前的最先进方法。定性和消融研究进一步验证了各组件有效性。

Conclusion: KnowMVG通过知识先验和全局-局部注意力增强，显著提升了医学视觉定位的空间精度，为临床决策提供更可靠的视觉证据，且不引入额外的文本推理开销。
...
TL;DR: 该研究探索了直接从波束形成前的单天线距离-多普勒张量学习空间结构的可行性，替代了传统的先波束形成再处理的方法。
...
Method: 使用6发8收（48个虚拟天线）商用汽车雷达，采用A/B啁啾序列FMCW发射方案，其中有效发射孔径在啁啾间变化。构建双啁啾共享权重编码器，以端到端数据驱动方式处理波束形成前的单天线距离-多普勒张量。使用鸟瞰图占据作为几何探针进行监督，监督信号来自激光雷达，并显式建模雷达视场和基于射线的可见性感知。

Result: 通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线评估，结果表明可以直接从波束形成前的单天线距离-多普勒张量学习空间结构，无需显式的角度域构建或手工信号处理阶段。

Conclusion: 空间结构可以直接从波束形成前的单天线距离-多普勒张量中学习，这为汽车雷达感知提供了新的表示学习范式，避免了传统角度域构建的局限性。
...
TL;DR: 提出ECAC大规模早教图像描述基准和RSRS混合训练框架，开发KinderMM-Cap-3B模型，显著提升专业物体识别精度


<details>
  <summary>Details</summary>
Motivation: 现有早教图像描述方法面临两大挑战：缺乏大规模领域特定数据集导致语义概念捕捉不足；传统训练范式在提升专业物体描述能力方面存在局限，监督学习偏向高频表达，强化学习在困难样本上优化不稳定

Method: 1) 构建ECAC基准：包含256,121张真实世界图像，配备专家级标注和细粒度标签，引入领域导向评估协议TTS；2) 提出RSRS混合训练框架：动态切换强化学习和监督微调，将零奖励困难样本重定向到监督微调；3) 开发KinderMM-Cap-3B：基于ECAC和RSRS训练的领域适应多模态大语言模型

Result: KinderMM-Cap-3B模型在TTS评估中达到51.06分，显著超越现有基准方法，同时保持优越的描述质量，展示了在专业教育应用中的潜力

Conclusion: ECAC基准和RSRS框架有效解决了早教图像描述中的领域适应问题，提出的模型在专业物体识别精度和描述质量方面均取得显著提升，为专业教育应用提供了有力工具
...
TL;DR: 该论文提出AMIMV方法解决医学图像分类中的数据稀缺和类别不平衡问题，评估了8种自监督学习方法在11个医学影像数据集上的表现，在长尾分布和有限监督下取得了性能提升。


<details>
  <summary>Details</summary>
Motivation: 医学影像分析面临两个主要问题：1）缺乏大量标注训练数据；2）数据不平衡问题，即常见类别数据丰富而罕见类别数据极度稀缺。虽然自监督学习（SSL）方法在一定程度上解决了第一个问题，但SSL方法在医学图像分类中对不平衡数据的鲁棒性研究很少。

Method: 1）扩展了先前提出的MIMV方法，引入新的增强策略构建非对称多图像多视图（AMIMV）对，以同时解决数据稀缺和数据集不平衡问题；2）进行数据分析评估AMIMV在不同程度类别不平衡下的鲁棒性；3）在11个医学影像数据集（MedMNIST）上评估8种代表性SSL方法在长尾分布和有限监督下的表现。

Result: 在MedMNIST数据集上的实验结果显示：retinaMNIST提升4.25%，tissueMNIST提升1.88%，DermaMNIST提升3.1%。AMIMV方法在医学图像分类中有效应对了数据稀缺和类别不平衡的挑战。

Conclusion: AMIMV方法成功解决了医学图像分类中的数据稀缺和类别不平衡问题，通过非对称多图像多视图对构建和增强策略，在多个医学影像数据集上取得了显著性能提升，为处理医学影像中的长尾分布问题提供了有效解决方案。
...
TL;DR: MAVFusion：一种基于运动感知稀疏交互机制的红外与可见光视频融合框架，通过光流识别动态区域并自适应分配计算资源，在保持融合质量的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有红外与可见光视频融合方法大多针对静态图像设计，无法有效处理视频中的帧间运动；当前视频融合方法虽通过跨帧交互提升时间一致性，但计算成本高昂。

Method: 提出端到端视频融合框架MAVFusion，采用运动感知稀疏交互机制：利用光流识别多模态序列中的动态区域，对稀疏动态区域分配计算密集的跨模态注意力以捕捉显著过渡和模态间信息交换；对静态背景区域使用轻量级弱交互模块保持结构和外观完整性。
...
TL;DR: 基于nnU-Net v2框架的深度学习模型，利用多模态mpMRI数据（T2加权、DWI、ADC）实现前列腺腺体自动分割，在PI-CAI数据集上训练，在外部验证中表现优于通用分割工具。
...
Method: 使用nnU-Net v2框架构建深度学习模型，利用多模态mpMRI数据（T2加权成像、扩散加权成像DWI、表观扩散系数ADC图）进行训练，采用PI-CAI数据集的981个病例进行全腺体标注训练，通过5折交叉验证和外部验证评估性能。

Result: 模型在交叉验证中平均Dice得分为0.96±0.00，在外部测试集（Hospital La Fe的54名患者）上Dice得分为0.82，表现出良好的泛化能力。相比之下，通用方法TotalSegmentator的Dice得分仅为0.15，主要由于腺体欠分割。

Conclusion: 研究强调了任务特异性、多模态分割策略的重要性，证明了所提方法可可靠集成到临床研究工作流程中。模型已完全容器化，可作为即用推理工具使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要理解相机佩戴者（自我接地）的个性化问答任务中表现未知，缺乏系统评估。自我中心视频中的个性化问答需要模型理解"我的物品"、"我的活动"和"我的过去"，这对未来个性化辅助系统至关重要。

Method: 构建MyEgo数据集：包含541个长视频和5K个个性化问题，涵盖"我的物品"、"我的活动"和"我的过去"三类。系统评估多种MLLMs变体：开源vs.专有、思考vs.非思考、小规模vs.大规模模型，分析显式推理和模型缩放的影响。

Result: 顶级闭源和开源模型（如GPT-5和Qwen3-VL）准确率仅约46%和36%，分别落后人类表现近40%和50%。显式推理和模型缩放未带来一致改进。提供相关证据时模型有所提升，但增益随时间下降，表明模型在跟踪和记忆"我"和"我的过去"方面存在局限。

Conclusion: 自我接地和长期记忆在自我中心视频的个性化问答中至关重要。现有MLLMs在这方面存在严重不足，MyEgo数据集和分析将推动该领域发展，为未来自我中心个性化辅助系统奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本条件3D场景生成方法在短文本描述下存在物理合理性差和细节丰富度不足的问题，主要原因是过度依赖显式的组合对象语义关系和空间关系线索，需要增强3D推理能力，特别是先验整合和空间锚定能力
...
Conclusion: SDesc3D通过整合多视角结构先验和区域功能含义，实现了在稀疏文本指导下的有效3D布局推理，显著提升了短文本条件3D场景生成的物理合理性和语义一致性
...
TL;DR: 提出交互式跟踪新范式，构建首个大规模基准InteractTrack，评估现有跟踪器在交互场景下的不足，并提出基于动态记忆的基线方法IMAT


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器以非交互式、发射后不管的方式工作，不适用于需要人机协同的现实场景，需要引入人类指导的交互能力

Method: 1) 构建InteractTrack基准：包含150个视频、密集边界框标注和时间戳语言指令；2) 提出评估协议并评估25个代表性跟踪器；3) 提出IMAT基线：采用动态记忆机制从用户反馈中学习并更新跟踪行为

Result: 评估显示最先进方法在交互场景中失败，传统基准上的强性能无法迁移；IMAT作为新基线为交互式跟踪提供了可行方案
...
TL;DR: Curia-2是一个用于医学影像分析的十亿参数级基础模型，通过改进预训练策略和表示质量，在CT和MRI分析任务上超越现有视觉基础模型，并与视觉语言模型在临床复杂任务上竞争。


<details>
  <summary>Details</summary>
Motivation: 医学影像的快速增长给放射科医生带来了不可持续的工作负担，需要开发基础模型来减轻这种压力。虽然现有基础模型在CT和MRI分析中显示出大规模预训练的力量，但在如何从复杂的放射学体积数据中学习方面仍有很大优化空间。

Method: 基于Curia框架，提出了Curia-2，改进了原始预训练策略和表示质量，更好地捕捉放射学数据的特性。方法支持将架构扩展到十亿参数级的Vision Transformers，这是多模态CT和MRI基础模型的首创。同时将CuriaBench扩展和重构为两个独立轨道：面向切片视觉模型的2D轨道和面向体积基准测试的3D轨道。

Result: Curia-2在视觉聚焦任务上超越了所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型竞争激烈。模型权重将公开以促进进一步研究。

Conclusion: Curia-2通过改进的预训练策略和十亿参数级架构，显著提升了医学影像基础模型的性能，为放射学数据分析提供了更强大的工具，并通过标准化评估框架推动了该领域的发展。
...
TL;DR: 本文提出了一种训练自由的惯性感知视觉激发（IVE）方法，用于解决多模态大语言模型中视觉注意力的惯性问题，该问题导致模型在认知推理中难以进行组合性理解。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中的视觉注意力表现出明显的惯性现象，一旦在早期解码步骤中稳定下来，就会保持静态，无法支持认知推理所需的组合性理解。现有幻觉缓解方法主要针对感知幻觉（对象存在或属性），但对需要对象间关系推理的认知幻觉效果有限。

Method: 提出惯性感知视觉激发（IVE）方法：1）通过标记级注意力分析识别视觉惯性；2）选择相对于历史注意力趋势动态出现的视觉标记，同时区分表现出惯性行为的标记；3）引入惯性感知惩罚机制，防止注意力过度集中并限制在局部区域的持久性。

Result: 广泛的实验表明，IVE在各种基础MLLM和多个幻觉基准测试中均有效，特别是在认知幻觉方面表现突出。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的关键因素，IVE方法通过打破这种惯性模式，显著提升了模型在组合性推理任务中的性能。
...
TL;DR: Resonance4D：一种基于物理的4D动态模拟框架，通过双域运动监督和全参数物理恢复，在单张消费级GPU上实现高保真4D模拟，显著降低计算成本和内存占用。


<details>
  <summary>Details</summary>
Motivation: 现有物理驱动的4D动态模拟方法存在两个主要问题：1）运动监督依赖计算成本高昂的在线视频扩散或光流管道；2）仅优化部分材料参数限制了复杂材料和动态场景的真实性。需要一种轻量级但物理表达力强的监督方法来实现高效且真实的4D模拟。

Method: Resonance4D框架将3D高斯泼溅与材料点方法耦合，通过双域运动监督（DMS）实现动态一致性。DMS结合空间结构一致性（用于局部变形）和频域谱一致性（用于振荡和全局动态模式）。此外，结合零样本文本提示分割与模拟引导初始化，自动将高斯分解为对象部分级区域，支持全材料参数的联合优化。

Result: 在合成和真实场景上的实验表明，Resonance4D实现了强大的物理保真度和运动一致性，同时将峰值GPU内存从超过35GB降低到约20GB，使高保真物理驱动的4D模拟能够在单张消费级GPU上运行。

Conclusion: Resonance4D通过创新的双域运动监督和全参数物理恢复方法，解决了现有4D动态模拟中的计算效率和真实性问题，为复杂材料和动态场景的高效物理模拟提供了可行方案。
...
TL;DR: ProDiG：一种扩散引导的渐进式框架，通过合成中间高度视图和几何感知的因果注意力模块，将航拍3D表示逐步转换为地面级保真度，解决航拍到地面极端视角变化下的3D重建问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从纯航拍图像生成地面视图和连贯3D场景模型时面临挑战，包括极端视角变化、缺少中间观测和大尺度变化。现有方法要么后处理渲染导致几何不一致，要么依赖多高度地面真值（通常难以获取），而高斯泼溅和扩散细化在小变化下有效但在大视角差距下失效。

Method: 提出ProDiG（渐进式高度高斯泼溅）框架：1）渐进式转换航拍3D表示到地面级保真度；2）合成中间高度视图；3）使用几何感知因果注意力模块在每个阶段细化高斯表示，将极线结构注入参考视图扩散；4）距离自适应高斯模块根据相机距离动态调整高斯尺度和不透明度，确保大视角差距下的稳定重建。

Result: 在合成和真实数据集上的广泛实验表明，ProDiG能生成视觉逼真的地面级渲染和连贯3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法。

Conclusion: ProDiG通过渐进式、几何基础化的细化框架，无需额外地面真值视角，成功解决了航拍到地面极端视角变化下的3D重建问题，实现了高质量的视觉渲染和几何一致性。
...
TL;DR: DR-Seg提出了一种用于遥感图像开放词汇语义分割的解耦-校正框架，通过分离CLIP特征为语义主导和结构主导子空间，实现针对性结构增强而不破坏语言对齐语义。


<details>
  <summary>Details</summary>
Motivation: 遥感开放词汇语义分割需要语言对齐识别和细粒度空间划分。CLIP虽然提供强大的语义泛化能力，但其全局对齐的视觉表示难以捕捉结构细节。现有方法引入遥感预训练的DINO特征进行补偿，但将CLIP表示视为单一语义空间，无法定位需要结构增强的区域，既不能有效划分边界，又可能破坏CLIP的语义完整性。

Method: DR-Seg是一个解耦-校正框架。首先观察到CLIP特征通道具有功能异质性而非均匀语义空间，据此将CLIP特征解耦为语义主导和结构主导子空间。然后通过先验驱动的图校正模块在DINO指导下注入高保真结构先验形成精炼分支，最后通过不确定性引导的自适应融合模块动态集成精炼分支和原始CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征为语义和结构子空间，实现了针对性的结构增强而不破坏语言对齐语义，为遥感开放词汇语义分割提供了一种有效的解决方案。
...
Method: 构建LinkS²Bench基准，将1022分钟动态无人机视频与覆盖200+ km²的高分辨率卫星图像关联；通过LMM辅助流程和严格人工标注，创建17.9k高质量问答对，涵盖感知、定位、关系和推理四个维度的12个细粒度任务；设计跨视图对齐适配器来改善模型性能。

Result: 评估18个代表性视觉语言模型显示，与人类基线相比存在显著差距，准确跨视图动态对齐是主要瓶颈；跨视图对齐适配器显著提升模型性能；微调实验证明LinkS²Bench在推进复杂空间推理的VLM适应方面具有潜力。

Conclusion: LinkS²Bench填补了评估视觉语言模型在无人机-卫星协同空间智能方面的空白，揭示了跨视图动态对齐的关键挑战，提出的对齐适配器有效提升性能，为复杂空间推理的VLM发展提供了重要基准和方向。
...
<details>
  <summary>Details</summary>
Motivation: 自编码器在处理空间非均匀采样的图像数据时面临挑战，特别是在医学影像、生物学和物理学领域，信息丰富的模式往往出现在特定坐标位置，而背景在这些位置占据主导地位，导致重建偏向多数外观，丢失细粒度细节，对罕见空间输入产生模糊重建。

Method: 提出了两个互补组件：1）基于自熵的损失函数，对统计上不常见的空间位置进行加权；2）样本传播机制，这是一种重放机制，在训练过程中选择性地将难以重建的样本重新暴露给模型。该方法专门针对空间不平衡问题，鼓励模型关注统计上罕见的位置。

Result: 在受控空间不平衡条件的模拟数据集以及三个不受控制的真实世界数据集（涵盖物理、生物和天文领域）上验证了方法的有效性。该方法在各种重建指标上优于基线，特别是在空间不平衡分布下表现更佳。

Conclusion: 该方法强调了批次中数据表示的重要性，并在无监督图像重建中突出了罕见样本的价值。研究结果展示了针对空间不平衡问题的专门方法在提升重建一致性方面的优势，代码和相关数据将公开提供。
...
TL;DR: IndoorCrowd是一个大规模室内人群数据集，包含31个视频（9,913帧），支持人体检测、实例分割和多目标跟踪任务，在四个校园场景中收集，并评估了基础模型自动标注器的性能。
...
Method: 在四个校园位置（ACS-EC、ACS-EG、IE-Central、R-Central）收集31个视频（9,913帧，5fps），提供人工验证的每实例分割掩码。使用620帧控制子集评估三个基础模型自动标注器（SAM3、GroundingSAM、EfficientGroundingSAM），使用Cohen's κ、AP、精度、召回率和掩码IoU指标。2,552帧子集支持MOTChallenge格式的多目标跟踪。建立检测、分割和跟踪基线，使用YOLOv8n、YOLOv26n、RT-DETR-L与ByteTrack、BoT-SORT、OC-SORT配对。

Result: 按场景分析显示，由于人群密度、尺度和遮挡的差异，难度变化显著：ACS-EC场景最具挑战性，包含79.3%的密集帧，平均实例尺度为60.8像素。基础模型自动标注器的性能通过人类标签进行了基准测试。

Conclusion: IndoorCrowd数据集填补了大规模真实世界室内人群数据集的空白，为计算机视觉任务提供了有价值的资源，并揭示了不同室内场景中人群分析任务的难度差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有思维链推理在语言引导分割中显著提升了性能，但其生成冗长推理过程带来的过高计算成本限制了实际应用。需要一种既能保持推理能力又能大幅降低计算开销的方法。

Method: WISE训练模型生成结构化序列：简洁推理→最终答案→详细解释。通过将简洁推理置于首位，利用自回归条件约束确保简洁推理能充分支撑详细解释的生成。采用自蒸馏目标联合奖励语义保真度和简洁性，使模型将详细推理内化为紧凑形式。推理时使用WISE-S策略，通过简洁性指令提示激活学习到的简洁策略。

Result: 在ReasonSeg基准测试中，WISE-S实现了58.3 cIoU的零样本SOTA性能，同时将平均推理长度从112个token减少到23个token，缩减近5倍。

Conclusion: WISE通过"思考两次"范式成功解决了思维链推理的计算效率问题，在保持高性能的同时大幅降低推理成本，为大规模多模态模型的实际部署提供了可行的解决方案。
...
TL;DR: Jagle：迄今为止最大的日语多模态后训练数据集，包含约920万个实例，通过多种策略生成VQA对，用于训练高性能日语视觉语言模型。


<details>
  <summary>Details</summary>
Motivation: 构建高质量多语言和非英语视觉语言模型面临的主要障碍是其他语言的视觉问答数据集在规模和领域覆盖上都有限。英语可以通过聚合现有VQA资源构建大规模训练数据集，但这一策略难以扩展到其他语言。

Method: 收集异构源数据（图像、图文对、PDF文档），通过多种策略生成VQA对：基于VLM的QA生成、翻译和文本渲染。构建了包含约920万个实例的日语多模态后训练数据集。

Result: 使用Jagle训练的2.2B模型在日语任务上表现优异，在十个日语评估任务中平均得分超过InternVL3.5-2B，接近Qwen3-VL-2B-Instruct（差距在5分以内）。将Jagle与FineVision结合不仅不会降低英语性能，反而比单独使用FineVision训练效果更好。

Conclusion: Jagle作为最大的日语多模态后训练数据集，有效解决了日语VLM训练数据不足的问题。通过创新的数据生成策略，成功构建了高质量日语VLM，同时保持了英语性能。研究团队发布了数据集、训练模型和代码以促进可重复性和未来研究。
...
TL;DR: 提出了一种全自动、可扩展的虚拟人皮肤色调保真度评估方法，通过集成肤色提取、纹理重着色、实时渲染和定量分析，系统评估虚拟人生成流程中的肤色保真度。


<details>
  <summary>Details</summary>
Motivation: 虚拟人渲染中准确再现面部肤色对于真实感、身份保持和公平性至关重要，但大多数可访问的虚拟人创建流程依赖未经色彩校准的摄影输入，可能引入不一致性和偏见。

Method: 提出全自动工作流程，集成肤色和光照提取、纹理重着色、实时渲染和定量色彩分析。使用芝加哥面部数据库图像，比较基于脸颊区域采样和全脸分析的多维掩码两种肤色提取策略，结合TRUST框架进行光照隔离。将提取的肤色应用于MetaHuman纹理并在多种光照配置下渲染，在CIELAB色彩空间中使用ΔE指标和个体类型学角度(ITA)客观评估肤色一致性。

Result: 生成并分析了约19,848个渲染实例，结果显示提取策略表现出表型依赖性行为，深色肤色的色彩误差始终更高。

Conclusion: 该方法无需人工干预，除预训练的光照补偿模块外不包含学习或训练阶段，计算成本低且支持大规模评估，揭示了肤色提取策略的表型依赖性差异和深色肤色保真度较低的普遍问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D功能推理方法主要评估孤立单物体，通常需要查询中提供明确类别名称，忽略了现实场景中多个物体可能共享相同功能但只有一个是任务上下文下合适的（混淆对）这一挑战

Method: 提出CompassNet框架：1) Instance-bounded Cross Injection (ICI)模块将语言-几何对齐约束在物体边界内，防止跨物体语义泄漏；2) Bi-level Contrastive Refinement (BCR)模块在几何组和点级别强制执行区分，增强目标与混淆表面之间的区别

Result: 构建包含30个混淆物体对、16种功能类型、6,422个场景、88K+查询-答案对的CompassAD基准；在已见和未见查询上均达到最先进性能；在机器人机械臂上部署证实了在混淆多物体场景中有效转移到真实世界抓取

Conclusion: 形式化了意图驱动指令下的多物体功能推理新任务，通过CompassAD基准和CompassNet框架有效解决了混淆多物体场景中的功能推理挑战，实现了从模拟到真实世界的有效转移
...
TL;DR: PLUME提出一种潜在推理框架，用连续潜在状态的自回归展开替代显式思维链，在保持推理优势的同时大幅减少推理开销，实现30倍加速
...
Method: PLUME采用潜在推理框架：1) 用连续潜在状态的自回归展开替代显式思维链；2) 引入语义锚点引导的转换适配器，在固定计算预算下引导不同推理轨迹；3) 采用渐进式显式到潜在课程学习，使用显式推理作为临时训练支架并逐步转移到隐藏状态计算

Result: 在78任务的MMEB-v2基准测试中，PLUME优于强显式思维链基线，同时将推理从数百个生成token减少到少于10个潜在步骤，实现超过30倍的推理加速

Conclusion: 结构化潜在计算可以在不产生显式推理生成开销的情况下保留中间推理的优势，为实际检索系统提供更强更高效的范式，特别适用于证据密集、结构复杂且难以通过显式中间推理组织的检索场景
...
TL;DR: FlowSlider是一种无需训练的方法，可在Rectified Flow中实现连续图像编辑，通过正交分解编辑更新为保真项和转向项，仅缩放转向项即可平滑控制编辑强度


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的滑块编辑方法依赖辅助模块和合成监督，需要额外训练开销，且滑块行为受限于训练分布，在分布偏移时可靠性降低

Method: 将FlowEdit的更新分解为保真项（作为源条件稳定器）和转向项（驱动语义转换），几何分析和实证测量显示这两项近似正交，通过仅缩放转向项实现稳定强度控制

Result: FlowSlider无需后训练即可提供平滑可靠的控制，在多样化任务中提升了连续编辑质量

Conclusion: FlowSlider通过正交分解编辑更新，实现了无需训练的连续图像编辑，解决了现有方法训练开销大和分布依赖性强的问题
...
Conclusion: GroundVTS通过聚焦信息最丰富的时序片段，有效保留时空信息并维持时间连贯性，显著提升了视频大语言模型的时间定位能力
...
TL;DR: LatentUM提出了一种在共享语义潜在空间中表示所有模态的统一模型，消除了视觉理解与生成之间像素空间中介的需求，实现了灵活的交错跨模态推理与生成。
...
Result: LatentUM在视觉空间规划基准上达到最先进性能，通过自反思推动视觉生成的极限，并在共享语义潜在空间内支持预测未来视觉状态的世界建模。

Conclusion: 共享语义潜在空间的统一表示显著提高了计算效率，减轻了编解码器偏差，增强了跨模态对齐，为实现灵活的交错跨模态推理与生成提供了有效框架。
...
TL;DR: 提出CoRegOVCD框架，通过后验校准和一致性正则化解决开放词汇变化检测中的噪声和碎片化问题，实现无需训练的高质量语义变化检测。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法假设固定的标签空间，无法响应用户自定义查询。开放词汇变化检测(OVCD)虽然能查询任意概念，但在无需训练的设置下，原始概念响应存在噪声、碎片化和语义不可靠的问题，难以直接进行跨时间比较。
...
Conclusion: CoRegOVCD通过后验校准和一致性正则化机制，有效解决了开放词汇变化检测中的噪声、碎片化和语义不可靠问题，在无需训练的情况下实现了高质量的语义变化检测。
...
TL;DR: 该研究揭示面部动作单元检测中标准交叉验证协议存在显著随机方差，提出留一数据集交叉验证作为更稳定的评估方法


<details>
  <summary>Details</summary>
Motivation: 面部动作单元检测研究中，基于被试排他的交叉验证是标准评估协议，但报告的性能提升往往很小。研究者怀疑交叉验证本身引入了可测量的随机方差，导致结果不稳定且难以解释

Method: 1) 在BP4D+数据集上进行重复3折被试排他分割，量化交叉验证的随机方差；2) 提出并实施留一数据集交叉验证协议，在五个动作单元数据集上进行跨数据集鲁棒性评估

Result: 1) 重复3折分割产生平均F1的±0.065经验噪声底限，低流行度动作单元的方差更大；2) 操作点指标如F1比阈值无关指标如AUC波动更大；3) 留一数据集交叉验证消除了分割随机性，暴露出单数据集交叉验证不可见的领域级不稳定性

Conclusion: 交叉验证本身引入显著随机方差，文献中报告的性能提升可能落在协议方差范围内。留一数据集交叉验证提供更稳定、可解释的结果，应作为面部动作单元检测的推荐评估协议
...
TL;DR: 该论文提出了一种反射生成方法，通过向基础扩散模型注入反射位置和外观的先验信息，并采用类型感知模型设计，在构建的大规模物体反射数据集DEROBA上进行训练，实现了物理一致且视觉逼真的反射效果。
...
Conclusion: 该研究填补了反射生成领域的空白，通过创新的先验信息注入和类型感知设计，结合大规模数据集训练，实现了高质量的反射生成效果。
...
TL;DR: ViT-Explainer是一个基于Web的交互式系统，用于可视化Vision Transformer的完整推理过程，从图像分块到最终分类，通过动画、注意力叠加和视觉适应的Logit Lens等工具帮助用户理解模型行为。


<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但理解这些模型在视觉设置中的运作仍然具有挑战性。现有可解释性工具通常关注孤立组件或专家导向分析，缺乏对完整推理管道的引导式端到端理解。

Method: 开发了ViT-Explainer系统，提供Vision Transformer推理的集成可视化，包括：1) 动画逐步演示；2) 补丁级注意力叠加；3) 视觉适应的Logit Lens；4) 引导和自由探索两种模式。

Result: 6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能够帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了现有可解释性工具的空白，通过交互式可视化系统提供了对Vision Transformer完整推理管道的端到端理解，有助于提升模型透明度和用户理解。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶VLA模型面临空间感知与语义推理的权衡困境：直接采用2D VLM空间感知有限，而增强3D空间表示又会损害VLM的推理能力。这种冲突源于空间感知和语义推理在共享模型参数中的耦合优化。
...
Result: 在nuScenes开环评估和Bench2Drive闭环评估中达到最先进性能。在3D检测、在线建图、运动预测和驾驶导向VQA等广泛任务上表现优异，展示了作为自动驾驶统一模型的广泛适用性。

Conclusion: UniDriveVLA通过专家解耦架构成功解决了自动驾驶VLA模型中空间感知与语义推理的冲突，实现了感知与推理能力的协同提升，为自动驾驶系统提供了强大的统一基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖显式的骨架-文本对齐，当动作名称无法充分描述细粒度动态特征或未见类别语义混淆时，这种方法较为脆弱。需要一种更鲁棒的方法来处理语义模糊性和类别混淆问题。

Method: 提出SCALE框架：1) 构建文本条件变分自编码器，冻结文本表示参数化先验和解码器；2) 引入语义和置信度感知列表能量损失，强调语义相似的困难负样本，结合后验不确定性调整决策边界；3) 使用潜在原型对比目标，将后验均值与文本导出的潜在原型对齐，改善语义组织和类别可分性。

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE在VAE和对齐基线方法上持续改进，同时与基于扩散的方法保持竞争力。

Conclusion: SCALE通过将ZSAR建模为类别条件能量排序问题，结合语义感知的困难负样本挖掘和不确定性加权，有效处理了语义模糊性和类别混淆问题，为轻量级确定性框架提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在文本和图像理解与生成方面表现优异，但扩展到3D领域面临挑战，主要原因是高质量3D数据稀缺导致3D合成约束不足。现有方法通常依赖间接流程，在2D中编辑后通过优化提升到3D，牺牲了几何一致性。

Method: 将文本、图像和3D表示为共享序列空间中的离散标记，利用丰富的2D数据作为几何先验来改善3D表示。引入交错X到X训练范式，在异构配对数据集上协调多样跨模态任务，无需完全对齐的文本-图像-3D三元组。通过在自回归序列中遍历语义-视觉-几何循环（如文本到图像到3D到图像），联合强制执行语义对齐、外观保真度和多视图几何一致性。

Result: 实验表明，Omni123显著改善了文本引导的3D生成和编辑，展示了向多模态3D世界模型扩展的可扩展路径。

Conclusion: Omni123通过统一的3D原生基础模型框架，利用跨模态一致性作为隐式结构约束，有效解决了3D数据稀缺问题，为多模态3D世界模型的发展提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法擅长修复对象"背后"的内容和校正外观层面的伪影，但在处理具有显著物理交互（如碰撞）的对象移除时，无法校正这些交互并产生不合理的结果。

Method: 1) 使用Kubric和HUMOTO生成包含反事实对象移除的配对数据集；2) 推理时使用视觉语言模型识别受移除对象影响的场景区域；3) 使用视频扩散模型生成物理一致的反事实结果。

Result: 在合成和真实数据上的实验表明，与现有视频对象移除方法相比，VOID能更好地保持对象移除后场景动态的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中效果有限，因为其固定的预训练分辨率和固有的粗粒度patch级表示。在密集预测场景（如开放词汇分割）中，高分辨率输入对精确的像素级推理至关重要。现有方法通常使用滑动窗口策略处理大分辨率图像，虽然通过更细的步幅提高了精度，但带来了显著的计算成本。

Method: 提出SPAR：单次推理任意分辨率ViT，一种分辨率无关的密集特征提取器。使用特征回归损失将细步幅滑动窗口教师模型的空间推理能力蒸馏到单次推理学生模型中，无需架构修改或像素级监督。

Result: 在开放词汇分割任务中，SPAR将单次推理基线提升了高达10.5 mIoU，甚至超过了教师模型的性能，证明了在高分辨率推理效率方面的有效性。

Conclusion: SPAR通过特征蒸馏方法实现了高效的高分辨率推理，解决了基础ViT在细粒度空间理解任务中的局限性，为密集预测任务提供了一种计算效率高的解决方案。
...
TL;DR: 提出一种基于推理时引导的文本到图像生成安全控制框架，利用冻结的预训练基础模型提供梯度反馈，无需微调生成器即可实现模块化、免训练的安全控制。
...
Method: 提出推理时引导框架，利用冻结的视觉-语言基础模型作为现成的监督信号，通过基于能量的采样问题将安全引导公式化，在每个采样步骤通过干净的潜在估计注入梯度反馈。
...
TL;DR: 提出可操控视觉表示方法，通过早期融合文本指导ViT特征，使其能聚焦于图像中任意指定对象


<details>
  <summary>Details</summary>
Motivation: 现有预训练ViT特征聚焦于最显著视觉线索，无法针对特定概念进行引导；而多模态LLM虽可文本引导但特征偏向语言中心，损失通用视觉任务能力

Method: 通过轻量级交叉注意力在视觉编码器层中直接注入文本（早期融合），创建可操控视觉表示，使全局和局部特征能通过自然语言引导

Result: 方法能在保持底层表示质量的同时聚焦于图像中任意指定对象，在异常检测和个性化对象区分任务上匹配或超越专用方法，并展示对分布外任务的零样本泛化能力

Conclusion: 提出的可操控视觉表示结合了ViT的通用视觉能力与文本引导的灵活性，为视觉表示学习提供了新的方向
...
TL;DR: ActionParty：首个可同时控制多达7个智能体的视频世界模型，通过引入主体状态令牌解决现有视频扩散模型中动作绑定的基本问题
...
Conclusion: ActionParty解决了多主体视频世界模型中的动作绑定问题，实现了对多个智能体的同时控制，为生成式视频游戏提供了有效的解决方案
...
Method: 将表面网格表示为概率测度，将表面配准转化为分布优化问题。使用计算复杂度为对数线性的切片Wasserstein距离度量网格差异。提出AdamFlow优化方法，将Adam优化器从欧几里得空间推广到概率空间，用于最小化切片Wasserstein距离。

Result: 从理论上分析了AdamFlow的渐近收敛性，并通过实验证明其在各种解剖结构的仿射和非刚性表面配准中具有优越性能。

Conclusion: 提出的方法解决了表面配准中效率与鲁棒性的权衡问题，通过概率测度表示和AdamFlow优化实现了高效且鲁棒的配准，适用于医学图像中的解剖形状分析。
...
TL;DR: 论文提出RSC基准测试，用于评估基于场景的视觉定位，要求模型从角色、意图和关系上下文中推断目标，而非简单的字面匹配，比现有基准更具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指代表达式的对齐，模型可通过匹配显著命名类别获得成功。需要探索更具挑战性的基于场景的视觉定位设置，要求从角色、意图和关系上下文中推断目标。

Method: 引入Referring Scenario Comprehension (RSC)基准，包含段落长度的查询文本，描述对象角色、用户目标和上下文线索，包括对干扰对象的故意引用。每个实例标注可解释的难度标签（唯一性、杂乱度、大小、重叠、位置）。提出ScenGround课程推理方法，结合监督预热和难度感知强化学习。

Result: RSC包含约31k训练样本、4k域内测试样本和3k域外分布样本（未见对象类别）。实验表明，基于场景的查询暴露了当前模型在标准基准中未揭示的系统性失败，课程训练提高了在挑战性切片上的性能，并能迁移到标准基准。

Conclusion: 基于场景的视觉定位比字面指代表达式更具挑战性，RSC基准揭示了现有模型的系统性失败模式，课程推理方法ScenGround能有效提升性能并实现迁移学习。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法独立处理不同视角，缺乏对多视角和多模态信息的有效整合，需要一种能够同时处理跨模态和跨视角特征映射的框架

Method: 采用跨模态特征映射范式，通过特征级调制显式建模视角依赖关系；引入跨视角训练策略利用所有可能的视角组合；训练并公开了针对工业数据集的基础深度编码器

Result: 在SiM3D基准测试中取得最先进性能，大幅超越先前方法

Conclusion: ModMap通过原生多视角多模态设计、跨视角训练和特征调制，为3D异常检测与分割提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有合成数据集在真实感和时域一致性方面存在局限，导致生成式逆渲染和正向渲染在真实世界场景中的应用受到瓶颈。需要高质量、大规模、动态的数据集来弥合这一领域差距。

Method: 1. 使用新颖的双屏拼接捕获方法从视觉复杂的AAA游戏中提取数据；2. 收集了400万连续帧（720p/30 FPS）的同步RGB和五个G-buffer通道数据；3. 包含多样场景、视觉效果和环境条件（恶劣天气、运动模糊变体）；4. 提出了基于视觉语言模型（VLM）的评估协议，用于测量语义、空间和时域一致性。

Result: 1. 在该数据集上微调的逆渲染器实现了优异的跨数据集泛化能力和可控生成；2. VLM评估结果与人类判断高度相关；3. 结合工具包，正向渲染器能够使用文本提示从G-buffer编辑AAA游戏的风格。

Conclusion: 该大规模动态数据集显著提升了双向渲染（逆渲染和正向渲染）的性能，提出的VLM评估协议为无地面真值情况下的逆渲染性能评估提供了有效方法，推动了生成式渲染在真实世界场景中的应用。
...
TL;DR: 该论文研究了在线交流中常见的重复延长形式（RLF）在情感分析中的重要性，创建了首个多领域RLF数据集Lengthening，并提出了ExpInstruct两阶段指令调优框架来提升LLM对RLF的理解和可解释性。


<details>
  <summary>Details</summary>
Motivation: 在线交流中人们经常使用非正式表达方式（如表情包、表情符号），但一种独特且强调性的重复延长形式（RLF）多年来一直被忽视。论文旨在探索两个研究问题：1）RLF对情感分析是否重要？2）语言模型能否理解RLF？

Method: 1）基于先前语言学研究，创建了首个多领域RLF数据集Lengthening（85万样本）；2）提出了ExpInstruct两阶段指令调优框架，旨在提升LLM对RLF的性能和可解释性；3）提出了统一方法来量化语言模型对非正式表达的理解。

Result: 1）RLF句子是表达性强的表达方式，可作为文档级情感的特征；2）RLF对在线内容分析具有潜在价值；3）微调的预训练语言模型在性能上可以超越零样本GPT-4，但在解释性上不如GPT-4；4）ExpInstruct能够用有限样本将开源LLM提升到与零样本GPT-4相当的性能和可解释性水平。

Conclusion: RLF在情感分析中具有重要性，语言模型需要专门优化才能更好地理解和解释RLF。ExpInstruct框架为提升LLM对非正式表达的理解提供了有效方法，特别是在数据和计算资源有限的情况下。
...
TL;DR: 该研究通过训练时强化学习和测试时并行思考两种方法扩展推理token预算，用于竞争性编程任务。训练时观察到验证准确率与生成推理token数之间的近似对数线性关系，并引入验证RL预热和随机剪枝优化训练轨迹。测试时采用多轮并行思考管道，将token预算分配到多个线程和轮次中，最终系统在AetherCode的456个难题上超越了GPT-5-high。


<details>
  <summary>Details</summary>
Motivation: 竞争性编程需要大量推理token，但单次生成推理在RL训练中成本高昂。需要探索如何有效扩展推理token预算，同时保持计算效率。

Method: 1. 训练时强化学习：采用验证RL预热提高起点，随机剪枝产生更陡峭的训练轨迹；2. 测试时并行思考：设计多轮并行思考管道，将token预算分配到16个线程，每个线程16轮生成-验证-精炼循环；3. 端到端训练：使训练目标与测试时结构匹配。

Result: 基于Seed-OSS-36B模型，完整系统（16线程×16轮）平均每个问题使用760万token，在pass@1指标上匹配了底层RL模型的oracle pass@16性能，并在AetherCode的456个硬竞争编程问题上超越了GPT-5-high。

Conclusion: 结合训练时RL优化和测试时并行思考管道，能够有效扩展推理token预算，在竞争性编程任务中实现高性能，同时保持计算效率。该方法为大规模推理任务提供了可行的解决方案。
...
TL;DR: M2-Verify是一个用于评估科学主张与多模态证据一致性的新型大规模数据集，包含超过46.9万个实例，覆盖16个领域，旨在解决现有基准在规模、领域多样性和视觉复杂性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有评估科学主张与多模态证据一致性的基准缺乏足够的规模、领域多样性和视觉复杂性，无法真实评估这种对齐关系。需要一个新的数据集来更准确地评估模型在科学论证中的一致性检查能力。

Method: 从PubMed和arXiv收集数据，构建包含超过469K实例的M2-Verify数据集，涵盖16个不同领域。通过专家审核进行严格验证，并进行广泛的基线实验评估现有模型性能。

Result: 最先进模型在一致性检查方面表现不佳：在低复杂度医学扰动上最高达到85.8% Micro-F1，但在高复杂度挑战（如解剖结构变化）上性能降至61.6%。专家评估还发现模型在生成科学解释时存在幻觉问题。

Conclusion: M2-Verify填补了科学主张一致性评估的空白，揭示了当前模型在处理复杂多模态科学证据时的局限性，为未来研究提供了有价值的基准和实用指南。
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型学习人类偏好的方法面临根本性挑战，因为奖励建模依赖于微妙、主观的比较而非明确标签。现有方法在标准成对偏好设置下性能有限（ROC AUC低于0.74），难以捕捉人类判断的多维特性。

Method: 提出混合特征增强框架：在文本表示基础上添加可解释信号，包括响应长度、拒绝指标、毒性分数和提示-响应语义相似度。使用Anthropic HHRLHF数据集评估10个不同大语言模型，采用标准成对偏好设置。结合SHAP和LIME进行细粒度可解释性分析。

Result: 特征增强方法在所有模型上带来一致改进，最高达到0.84 ROC AUC，显著提高成对准确率。DeBERTav3Large表现最佳。可解释性分析显示模型决策依赖于情境化安全性和支持性框架而非孤立关键词。偏差放大分析表明特征交互影响偏好学习。

Conclusion: 通过增强文本表示与可解释信号，可以更有效地捕捉人类偏好多维特性，显著提升语言模型偏好学习性能。特征交互在偏好学习中起重要作用，情境化安全性和支持性框架是关键决策因素。
...
TL;DR: 提出Reasoning Memory框架，通过检索和重用程序性知识来增强推理模型，在数学、科学和编程任务上显著提升性能
...
Method: 从逐步推理轨迹语料库中分解出自包含的子问题-子程序对，构建包含3200万条程序性知识条目的数据存储；推理时通过轻量级思维内提示让模型表达核心子问题，检索相关子程序作为隐式程序先验

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory始终优于基于文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时缩放基线；在更高推理预算下，相比无检索提升达19.2%，相比最强计算匹配基线提升7.9%

Conclusion: 性能提升主要来自两个关键因素：源轨迹的广泛程序覆盖范围，以及分解和检索设计，共同实现了程序性知识的有效提取和重用
...
TL;DR: 该研究发现语言模型通过稀疏的MLP神经元实现实体检索，这些神经元在早期层集中，能够因果性地影响实体相关事实的预测


<details>
  <summary>Details</summary>
Motivation: 研究语言模型回答实体中心事实问题的内部机制，探索模型如何检索和利用实体信息进行事实推理

Method: 使用模板化提示定位实体选择性MLP神经元，通过因果干预验证，在PopQA数据集上分析200个实体，进行负向消融和受控注入实验

Result: 实体选择性神经元集中在早期层；负向消融导致实体特异性遗忘，受控注入提高答案检索；单个神经元激活足以恢复实体一致预测；对别名、缩写等具有鲁棒性

Conclusion: 语言模型通过稀疏、因果可操作的MLP神经元实现实体检索，支持规范化解释，为分析和调节实体条件事实行为提供了具体切入点
...
<details>
  <summary>Details</summary>
Motivation: 翻译过程中的打字停顿通常被认为是翻译问题或困难的指标，但如何确定区分自动化与反思性翻译过程的停顿阈值一直存在争议，需要更可靠的方法来识别生产单元中断

Method: 比较了三种现有的停顿阈值计算方法，并提出了一种新的生产单元中断计算方法进行评估

Result: 通过比较不同方法，验证了新提出的生产单元中断计算方法的有效性

Conclusion: 新提出的生产单元中断计算方法为更准确地识别翻译过程中的认知活动提供了有效工具
...
TL;DR: 该论文提出了基于预测效度的基准测试质量评估框架，通过收集大规模项目级数据集WILD，结合改进的多维项目反应理论和自适应项目选择，显著降低了模型性能评估成本。


<details>
  <summary>Details</summary>
Motivation: 现有大规模语言模型基准测试数量庞大但效率低下，研究表明模型性能可由少量潜在能力因子解释，需要更高效、原则性的基准测试方法。当前缺乏评估不同基准测试方法质量的统一标准。

Method: 1. 收集WIDE-scale Item Level Dataset (WILD)：包含65个模型在163个任务（来自27个数据集）的109,564个独特项目上的响应数据；2. 提出基于预测效度的基准测试质量评估框架；3. 结合改进的多维项目反应理论模型和基于最优实验设计的自适应项目选择；4. 引入成本感知折扣因子优化选择标准。

Result: 1. 提出的方法在仅观察16个项目后，能在112个保留基准测试任务上实现平均绝对误差小于7%的性能预测；2. 通过成本感知折扣因子，将达到7% MAE所需的总token数从141,000减少到22,000，评估成本降低85%。

Conclusion: 该研究证明了基于预测效度的基准测试框架的有效性，通过结合多维项目反应理论和自适应项目选择，能够以极低的成本准确预测模型在未见任务上的性能，为高效、原则性的模型评估提供了新方法。
...
TL;DR: 该研究使用词嵌入和随机森林分类器，成功仅基于分布数据区分印地语中梵语和波斯-阿拉伯语来源的同义词，证明使用模式保留了词源信号，支持同义词反映不同视角的观点。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证同义词理论：绝对同义词理论上不应存在，因为它们不会扩展语言的表达能力。但即使同义词表示相同概念，它们可能反映不同视角或携带不同的文化关联。这一主张很少被定量测试。印地语中梵语和波斯-阿拉伯语借词共存形成众多同义词对，为研究提供了理想案例。

Method: 研究方法包括：1）使用印地语中的梵语和波斯-阿拉伯语来源的同义词对作为研究对象；2）基于分布数据训练词嵌入模型；3）使用随机森林分类器仅基于词嵌入特征（不考虑语义内容）对单词按词源（梵语或波斯-阿拉伯语）进行分类；4）评估分类性能。

Result: 随机森林分类器成功区分了梵语和波斯-阿拉伯语来源的印地语同义词，即使这些词在语义上无关。这表明使用模式保留了词源痕迹，上下文编码了词源信号。同义词可能反映与起源相关的微妙但系统的区别，词源相关的词可能形成不同的概念子空间。

Conclusion: 研究结论：1）上下文能够捕捉超越传统语义相似性的细微区别；2）同义词可以提供不同视角；3）词源相关的词可能形成由历史起源塑造的新型语义框架；4）使用模式保留了词源信号，为"同义词反映不同视角"的观点提供了定量证据。
...
TL;DR: 研究发现引用粒度对模型性能有显著影响，中等粒度（段落级）表现最佳，而细粒度（句子级）引用会损害归因质量，特别是对大模型影响更大。


<details>
  <summary>Details</summary>
Motivation: 在归因生成中，引用粒度（引用句子、段落还是文档）是关键设计选择。虽然细粒度引用通常更便于人工验证，但其对模型性能的影响尚未充分研究。

Method: 分析了四种模型规模（8B-120B），比较不同引用粒度（句子级、段落级、多段落级）对归因质量的影响，评估了归因质量和答案正确性。

Result: 强制细粒度引用会使归因质量下降16-276%；中等粒度（段落级）表现最佳；细粒度引用会破坏证据归因所需的语义依赖，而过度粗粒度引用会引入干扰噪声；大模型受细粒度约束的影响更大。

Conclusion: 仅为了人工验证而优化细粒度引用会忽视模型约束，损害归因忠实性和生成可靠性。有效的归因需要将引用粒度与模型的自然语义范围对齐。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常表现出"过度自信"的问题：当产生事实错误的答案时，它们倾向于表达过高的置信度而非不确定性。这种过度自信的表达会误导用户并削弱置信度分数作为可靠不确定性信号的价值，但其内部机制尚不清楚。

Method: 采用电路级机制分析方法，围绕三个轴展开：1) 将语言化置信度捕获为可微分的内部信号；2) 识别导致置信度膨胀的因果电路；3) 利用这些见解进行有针对性的推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验。

Result: 发现一组紧凑的MLP块和注意力头（主要集中在中间到后期层）持续在最终token位置写入置信度膨胀信号。进一步证明，对这些电路进行有针对性的推理时干预能显著改善校准效果。

Conclusion: LLM中的语言化过度自信是由可识别的内部电路驱动的，可以通过有针对性的干预来缓解。这一发现为理解LLM不确定性表达的内部机制和改善校准提供了新的见解。
...
TL;DR: 该研究通过计算分析129,451首波斯诗歌，识别诗歌中反复出现的象征形式家族，构建多层图展示象征核心与指涉成分的关系及历时变化。


<details>
  <summary>Details</summary>
Motivation: 传统计算分析将波斯诗歌材料扁平化为孤立词语或宽泛文档语义，忽略了波斯诗学中相关形式以家族形式传播并通过重复关系获得力量这一实际组织单元。

Method: 使用129,451首诗歌语料库，将反复出现的形式整合为可追溯的家族，将意象材料与神圣及宫廷指涉分离，并在多层图中映射它们的关系，分析11个回历世纪分箱中的变化。

Result: 象征核心相对稀疏，指涉成分更密集，两者间的连接区域具有选择性而非弥散性。某些家族（如夜、日、土）广泛分布，酒器、花园空间、火焰、抒情声音后期增强，而声望编码和英雄宫廷词汇早期权重更高。模块性上升，跨范围连接减少，宫廷桥梁减弱，神圣桥梁增强。

Conclusion: 波斯象征主义并非固定剧目，而是一个长期存在的系统，其内部权重和连接随时间变化，展现了动态演变的诗学传统。
...
### [117] [Magic, Madness, Heaven, Sin: LLM Output Diversity is Everything, Everywhere, All at Once](https://arxiv.org/abs/2604.01504)
*Harnoor Dhingra*

Main category: cs.CL

TL;DR: 论文提出了Magic, Madness, Heaven, Sin框架，将大语言模型输出变化建模为同质-异质轴，根据任务规范目标评估变化，并分析了四个规范上下文及其相互影响。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于"多样性"的术语碎片化，主要原因是任务背后的规范目标很少被明确说明。需要建立一个统一框架来系统分析输出变化，并理解不同规范目标之间的权衡关系。

Method: 提出Magic, Madness, Heaven, Sin框架，将输出变化建模为同质-异质轴。将任务组织到四个规范上下文中：认知性（事实性）、交互性（用户效用）、社会性（代表性）和安全性（鲁棒性）。分析每个上下文的失败模式和词汇，并应用框架分析所有成对的跨上下文交互。

Result: 框架揭示了优化一个目标（如提高安全性）可能无意中损害人口统计代表性或创造性多样性。不同规范上下文之间存在权衡关系，需要上下文感知的评估方法。

Conclusion: 输出变化应被重新定义为由任务目标塑造的属性，而非模型的内在特征。需要上下文感知的评估方法，认识到不同规范目标之间的权衡关系，避免单一优化导致其他方面的损害。
...
### [118] [Why Instruction-Based Unlearning Fails in Diffusion Models?](https://arxiv.org/abs/2604.01514)
*Zeliang Zhang,Rui Sun,Jiani Liu,Qi Wu,Chenliang Xu*

Main category: cs.CL

TL;DR: 扩散模型无法通过自然语言指令有效遗忘特定概念，指令式遗忘方法在图像生成模型中存在根本性限制


<details>
  <summary>Details</summary>
Motivation: 研究指令式遗忘范式是否适用于扩散模型等非语言生成模型，探索基于自然语言指令的遗忘方法在图像生成中的有效性

Method: 通过多个概念和提示变体的受控实验，分析CLIP文本编码器和去噪过程中的交叉注意力动态，评估扩散模型对目标概念的抑制能力

Result: 扩散模型仅通过自然语言遗忘指令无法有效抑制目标概念，注意力机制中目标概念表示在整个生成过程中持续存在
...
TL;DR: 提出模型融合框架，通过合并临床基础模型与通用指令模型，解决医学领域微调中的灾难性遗忘问题，在保持指令跟随能力的同时提升临床任务性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医学领域应用时，使用特定医学数据集微调会导致显著的指令跟随能力遗忘，这是临床应用中采用通用LLMs的关键挑战。

Method: 提出模型融合框架，通过插值融合方法将临床基础模型(GatorTronLlama)与通用指令模型(Llama-3.1-8B-Instruct)合并，获得领域适应模型。

Result: 融合模型能有效缓解灾难性遗忘，保持临床领域专业知识，保留指令跟随能力，在有限监督下(如64-shot vs 256-shot)达到与完全微调基线相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有角色中心记忆框架存在信息丢失和跨场景脆弱性问题，导致性能不佳，需要更鲁棒的解决方案

Method: 提出DeltaMem系统，将记忆管理作为单智能体端到端任务；构建用户-助手对话数据集和操作级记忆更新标签；引入基于记忆的Levenshtein距离形式化记忆更新奖励；设计定制强化学习框架

Result: DeltaMem在LoCoMo、HaluMem和PersonaMem等多个长期记忆基准测试中均优于所有产品级基线模型，包括无训练版本和RL训练版本

Conclusion: DeltaMem通过单智能体端到端任务框架和强化学习优化，有效解决了现有角色中心记忆管理系统的信息丢失和脆弱性问题，在长期记忆任务中表现出色
...
TL;DR: Swift-SVD：一种激活感知的闭式压缩框架，通过增量聚合输出激活协方差和单次特征值分解，实现训练无关、快速且理论最优的层级低秩近似，在LLM压缩中达到最优精度并加速3-70倍。
...
TL;DR: 对印度尼西亚349名K-12教师的全国性调查显示，AI在教学、内容开发和教学媒体中的应用正在增加，但采用不均衡，主要障碍包括通用输出、基础设施限制和情境适应性不足。
...
Method: 对印度尼西亚349名K-12教师（涵盖小学、初中和高中）进行了全国性调查，收集教师使用AI的实践数据和支持需求。

Result: 发现AI在教学法、内容开发和教学媒体中的应用正在增加，但采用不均衡：小学教师使用更一致，高中教师参与较少；中年教师更重视AI，东印度尼西亚教师认为AI价值更大。教师主要使用AI减轻教学准备工作量（如评估、课程规划和材料开发），但通用输出、基础设施限制和情境适应性不足阻碍了有效的课堂整合。

Conclusion: 需要开发更符合当地情境的AI系统，解决基础设施限制，并提供针对不同教师群体（不同教育阶段、职业阶段和地区）的差异化支持，以促进AI在印度尼西亚教育中的有效整合。
...
TL;DR: 该论文分析了9个声明验证数据集的推理模式，发现当前基准主要测试证据提取能力，而多句合成和数值推理严重不足，高基准分数主要反映检索+蕴含能力而非复杂推理。


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对这些基准实际测试的推理能力的系统性理解。需要了解当前验证数据集主要测试哪些推理类型，以及系统在哪些方面存在不足。

Method: 使用GPT-4o-mini为9个数据集的24K声明验证示例生成结构化推理轨迹，分析推理模式分布。使用紧凑的1B参数推理验证器识别五种错误类型，并分析不同领域的错误分布特征。

Result: 发现直接证据提取占主导地位（约60%），多句合成和数值推理严重不足（分别约20%和5%）。数据集层面存在显著偏差：某些数据集几乎只测试词汇匹配，而其他数据集约一半情况需要信息合成。错误分析显示：通用领域验证主要受词汇重叠偏差影响，科学验证由过度谨慎主导，数学验证则因算术推理失败。
...
<details>
  <summary>Details</summary>
Motivation: 现有情感支持对话方法在深度上下文理解方面存在不足，难以有效缓解个体情感困扰，需要更有效的框架来增强上下文认知理解。
...
Conclusion: PRCCF通过结合人物角色引导的检索和因果感知的认知过滤，有效提升了情感支持对话中的上下文理解和情感推理能力。
...
TL;DR: 研究发现监督微调中思维链轨迹来源对模型泛化能力有重要影响：DeepSeek-R1数据训练损失更低但泛化更差，因其发散式推理模式导致模型继承低效探索行为；通过过滤频繁分支轨迹可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探究不同来源的思维链轨迹如何影响大语言模型的泛化性能。尽管监督微调在构建大型推理模型中至关重要，但不同模型生成的思维链轨迹对最终模型性能的影响机制尚不明确。

Method: 研究方法包括：1）使用DeepSeek-R1-0528和gpt-oss-120b生成的已验证思维链轨迹进行对比研究；2）控制问题集相同以消除数据偏差；3）多层面分析包括词元级训练损失和步骤级推理行为；4）提出过滤频繁分支轨迹的简单有效方法。

Result: 研究结果揭示了一个悖论：使用DeepSeek-R1数据训练获得更低的训练损失，但在推理基准测试中表现出更差的泛化性能。分析发现gpt-oss-120b采用收敛式演绎推理，而DeepSeek-R1偏好发散式分支探索模式。过滤频繁分支轨迹后，DeepSeek-R1子集训练使AIME25性能提升5.1%，BeyondAIME提升5.5%，五个基准平均提升3.6%。

Conclusion: 结论表明思维链轨迹的质量不仅取决于最终答案正确性，更受推理模式影响。收敛式演绎推理轨迹比发散式探索轨迹更适合监督微调。通过轨迹过滤可有效提升模型泛化能力，这为优化思维链数据选择提供了重要指导。
...
TL;DR: EndoASR是一个针对胃肠道内窥镜领域优化的自动语音识别系统，通过两阶段适应策略显著提升转录准确性和临床可用性，在多中心真实世界临床环境中验证了其鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 胃肠道内窥镜中ASR作为人机交互的关键接口，在真实临床环境中因领域特定术语和复杂声学条件而可靠性受限，需要开发专门优化的系统。

Method: 开发了两阶段适应策略：基于合成内窥镜报告进行领域特定语言建模和噪声鲁棒性优化，构建了紧凑模型（220M参数）并实现实时部署。

Result: 在回顾性评估中，CER从20.52%降至14.14%，Med ACC从54.30%提升至87.59%；前瞻性多中心研究中，CER从16.20%降至14.97%，Med ACC从61.63%提升至84.16%；RTF达到0.005，显著优于Whisper-large-v3。

Conclusion: 领域适应的ASR可作为胃肠道内窥镜中人机协作的可靠接口，在多中心真实临床环境中验证了其一致性能，改进的ASR质量直接增强了下游结构化信息提取和临床医生-AI交互。
...
TL;DR: 该论文系统比较了LLM智能体中的各种记忆方法，提出了统一框架，在基准测试上全面评估现有方法，并设计出优于SOTA的新记忆方法。


<details>
  <summary>Details</summary>
Motivation: 记忆是LLM智能体处理长期复杂任务（如多轮对话、游戏、科学发现）的核心模块，能实现知识积累、迭代推理和自我进化。现有记忆方法缺乏在相同实验设置下的系统全面比较。

Method: 1. 从高层次视角总结现有智能体记忆方法的统一框架；2. 在两个知名基准上广泛比较代表性记忆方法；3. 利用现有方法模块设计新的记忆方法；4. 基于发现提出未来研究方向。

Result: 1. 提供了现有记忆方法的系统性分析；2. 新设计的记忆方法超越了现有最先进方法；3. 识别了各种方法的有效性和局限性。

Conclusion: 对现有记忆方法的深入理解能为未来研究提供宝贵新见解，论文提出的统一框架、系统性比较和新方法设计为LLM智能体记忆研究奠定了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 越南语语音情感识别面临挑战：声学模式模糊、标注数据不足、真实场景中情感边界不清晰。传统数据驱动模型难以处理这些模糊样本，需要结合人类知识。

Method: 提出人机协作框架，核心是LLM推理机制。使用声学特征模型提供置信度和特征级证据；引入置信度路由区分简单和模糊样本；模糊样本交由LLM进行基于结构化规则的深度推理；采用迭代优化策略通过错误分析和规则更新持续改进系统。

Result: 在包含2,764个样本的越南语语音数据集上测试，涵盖平静、愤怒、恐慌三类情感，标注者间一致性高（Fleiss Kappa=0.8574）。方法达到86.59%准确率，Macro F1约0.85-0.86，有效处理模糊和难分类样本。

Conclusion: 该工作强调了数据驱动模型与人类推理结合的重要性，为低资源环境下的语音情感识别提供了鲁棒且模型无关的方法。人机协作框架能有效处理模糊样本，在标注数据有限的情况下实现高性能。
...
TL;DR: 本文提出了一种更细致的保加利亚语文本毒性检测方法，通过构建本体论模型和标注数据集，训练了BERT分类模型，在保留医疗术语和少数群体相关文本的同时准确识别毒性内容。


<details>
  <summary>Details</summary>
Motivation: 当前在线内容毒性检测方案常常无意中屏蔽有价值信息，包括医疗术语和少数群体相关文本。为了解决这一问题，需要开发一种更细致的方法来识别保加利亚语文本中的毒性内容，同时保留重要信息的访问权限。

Method: 研究探索了两种不同的毒性内容检测方法：1) 提出一个建模保加利亚语潜在毒性词汇的本体论；2) 构建包含4,384个手动标注句子的数据集，涵盖毒性语言、医学术语、非毒性语言和少数群体相关术语四个类别。基于此数据集训练BERT模型进行毒性语言分类。

Result: 训练的BERT模型达到了0.89的宏观F1分数。该模型可直接应用于实际环境，并可作为毒性内容检测系统的组件进行集成。

Conclusion: 本文提出的方法为保加利亚语毒性内容检测提供了更细致的解决方案，能够在准确识别毒性内容的同时避免屏蔽医疗术语和少数群体相关文本等有价值信息，具有在各种在线平台和内容审核系统中应用的潜力。
...
TL;DR: LiveMathematicianBench是一个基于arXiv最新论文构建的动态多选基准，用于评估大语言模型的研究级数学推理能力，通过证明草图引导的干扰项设计和替换抵抗机制来区分真实理解与模式匹配。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准存在合成设置和数据污染问题，无法真实评估大语言模型的研究级数学能力。随着大语言模型越来越多地融入科学工作流，对其数学能力进行严格评估成为实际需求。

Method: 1. 基于模型训练截止后发布的arXiv论文构建动态基准；2. 引入13类逻辑分类法（蕴含、等价、存在、唯一性等）；3. 采用证明草图引导的干扰项生成管道，利用高层次证明策略构建看似合理但无效的答案选项；4. 引入替换抵抗机制以区分答案识别与实质性推理；5. 采用双模式协议评估模型利用证明草图的能力。

Result: 基准远未饱和：最佳模型Gemini-3.1-pro-preview仅达到43.5%准确率。在替换抵抗评估下，准确率大幅下降：GPT-5.4最高为30.6%，Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）。证明草图访问带来一致的准确率提升，表明模型能利用高层次证明策略进行推理。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的研究级数学推理测试平台，能够更真实地评估大语言模型的数学推理能力，揭示当前模型在实质性数学理解方面的局限性。
...
TL;DR: 本文提出基于指令微调和离散控制令牌的领域无关可控自动文本简化框架，发现小模型可竞争但可控性依赖训练数据中目标属性的充分变化，标准评估指标不足，需基于误差的度量方法。


<details>
  <summary>Details</summary>
Motivation: 当前可控自动文本简化（CATS）通常被视为解码问题，且评估指标不能有效反映控制程度。研究发现CATS的可控性受到数据和评估方法的显著限制。

Method: 提出基于指令微调和离散控制令牌的领域无关CATS框架，引导开源模型达到目标可读性水平和压缩率。在三个模型家族（Llama、Mistral、Qwen，1-14B）和四个领域（医学、公共管理、新闻、百科全书文本）进行实验。

Result: 较小模型（1-3B）可具有竞争力，但可靠的可控性强烈依赖于训练数据是否编码足够的目标属性变化。可读性控制（FKGL、ARI、Dale-Chall）学习一致，而压缩控制因现有语料库中信号变异性有限而表现不佳。标准简化和平行度指标不足以衡量控制效果。

Conclusion: 需要基于误差的度量方法来衡量目标-输出对齐，朴素的数据分割会引入分布不匹配问题，影响训练和评估。数据质量和评估方法是实现可靠可控性的关键因素。
...
Method: 提出DEFT框架：1) 基于语言模型输出分布和偏好数据差异分布计算微分分布奖励；2) 使用该奖励从原始数据中筛选高质量子集；3) 将筛选数据融入现有对齐方法指导模型输出分布
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM偏好学习方法存在性能提升有限、计算成本高、超参数敏感以及缺乏全局token级关系建模等问题，需要更有效且理论完备的对齐方法。

Method: PLOT将偏好学习构建为最优传输问题，从最优传输推导出token级损失函数，利用token嵌入捕捉语义关系，实现全局感知的优化，同时保持LLM原始分布。

Result: 在人类价值观与逻辑问题解决两大类、七个子偏好的实验中，PLOT一致提升了对齐性能，同时保持了流畅性和连贯性，验证了最优传输作为偏好学习原则性方法的有效性。

Conclusion: 最优传输为LLM偏好学习提供了理论基础框架，PLOT方法在提升对齐性能的同时保持了模型稳定性，为LLM偏好学习提供了新的理论见解。
...
TL;DR: 论文提出自适应占位符补全(APC)框架，通过在高熵位置输出显式占位符来改进传统硬补全，降低编辑成本而不牺牲传统补全质量。


<details>
  <summary>Details</summary>
Motivation: 传统LLM代码补全采用硬补全范式，即使在上下文不足时也强制生成完全具体的代码。对300万真实交互的分析显示：61%的生成建议在接收后被编辑或拒绝，尽管与用户后续代码有80%以上相似度，表明模型在特定token位置经常做出错误预测。

Method: 提出自适应占位符补全(APC)协作框架，在硬补全基础上扩展，在高熵位置战略性地输出显式占位符，允许用户通过IDE导航直接填充。理论上将代码补全形式化为不确定性下的成本最小化问题。基于填充占位符比纠正错误成本更低的观察，证明存在临界熵阈值，超过该阈值APC的期望成本严格低于硬补全。通过从过滤的真实编辑日志构建训练数据，并设计基于成本的奖励函数进行强化学习来实例化该框架。

Result: 在1.5B到14B参数模型上的广泛评估表明，APC将期望编辑成本降低了19%到50%，同时保持了标准硬补全性能。提供了不确定性感知代码补全的理论基础和实践训练框架。

Conclusion: 自适应弃权可以通过端到端学习实现，无需牺牲传统补全质量。APC框架在降低编辑成本方面显著优于传统硬补全方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法通常强调融合但忽视了噪声特征的不确定性和细粒度推理，需要更鲁棒且能建模上下文推理的方法
...
TL;DR: 本文评估了基于transformer的模型在法国心衰队列中的短期死亡率预测，比较了纯文本、纯结构化数据、多模态和LLM方法，发现实体感知的多模态transformer性能最佳。


<details>
  <summary>Details</summary>
Motivation: 心衰患者的短期死亡率预测具有挑战性，特别是仅依赖结构化电子健康记录数据时。需要探索如何有效整合临床文本和结构化数据来提升预测准确性。

Method: 在法国心衰队列上评估了多种基于transformer的模型：1）纯文本模型；2）纯结构化数据模型；3）多模态融合模型；4）大型语言模型方法。特别比较了CLS嵌入与实体级表示，以及不同模态融合策略。

Result: 1）用实体级表示丰富临床文本比单独使用CLS嵌入能提升预测性能；2）文本和结构化变量的监督多模态融合获得最佳整体性能；3）大型语言模型在不同模态和解码策略上表现不一致，纯文本提示优于结构化或多模态输入。

Conclusion: 实体感知的多模态transformer为心衰短期结局预测提供了最可靠的解决方案，而当前的LLM提示方法在临床决策支持方面仍有局限，需要进一步改进。
...
Method: 提出神经符号方法，结合非上下文文本嵌入（fastText）与符号概念特征，包括体裁、主题和说服技巧等特征，增强分类的鲁棒性和泛化能力。

Result: 结果显示该方法优于等效的纯文本方法，消融研究和可解释性分析证实了添加特征的好处。

Conclusion: 神经符号方法通过结合文本嵌入和符号概念特征，能够有效提高宣传性新闻检测的鲁棒性和泛化能力，为信息失序检测提供了更稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究语言词序是否遵循交换距离最小化原则，即给定源顺序时，在排列图中距离更近的词序成本更低、更可能出现。需要建立测量词序变异最优性的数学框架。

Method: 使用排列图（permutohedron）表示序列所有排列的结构，顶点为排列，相邻元素交换产生边连接。开发测量词序变异相对于交换距离最小化最优性的数学框架。引入二次分配问题（QAP）作为语言优化研究的统一框架。

Result: 跨语言手势至少达到77%的最优性，多次达到最优的情况不太可能是偶然。建立了词序或手势顺序相对于交换距离最小化最优性的理论基础。

Conclusion: 提出了测量词序变异最优性的数学框架，证明跨语言手势高度优化。引入二次分配问题作为语言优化研究的统一框架，并提出最优分配的一般原则，统一包括交换距离最小化在内的多种语言原则。
...
TL;DR: 论文提出针对机器翻译基准数据集的三步自动化质量保证方法，通过结构审计、质量分析和错误分析来评估翻译可靠性，并发布清理后的EU20数据集


<details>
  <summary>Details</summary>
Motivation: 机器翻译基准数据集虽然降低了成本并提供了规模，但噪声、结构丢失和质量不均等问题削弱了对其可靠性的信心。研究旨在开发可扩展的翻译质量测量和验证方法

Method: 采用三步自动化质量保证方法：1) 结构语料库审计与针对性修复；2) 使用神经度量COMET（无参考和基于参考）进行质量分析，并与DeepL/ChatGPT/Google翻译服务比较；3) 基于LLM的跨度级翻译错误分析

Result: 趋势一致：COMET分数较低的数据集在跨度级别表现出更高的准确性/误译错误比例（特别是HellaSwag；ARC相对干净）。基于参考的COMET在MMLU上与人编辑样本对比也指向相同方向。研究发布了清理/校正后的EU20数据集和可复现代码
...
TL;DR: SAFE框架通过可验证的实体序列替换无基础的思维链，在训练时验证消除噪声监督，在推理时实时检测无基础步骤，显著提升多跳问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有多跳问答基准经常奖励大型语言模型虚假的正确性，掩盖了无基础或有缺陷的推理步骤，需要转向更严谨的推理评估。

Method: 提出SAFE动态基准框架：1) 训练时验证：建立原子错误分类法和知识图谱基础验证管道，消除标准基准中的噪声监督；2) 推理时验证：在已验证数据集上训练反馈模型，实时动态检测无基础步骤。

Result: 识别出高达14%的实例为不可回答，显著优于标准基线，平均准确率提升8.4个百分点，同时保证推理时可验证的轨迹。

Conclusion: SAFE框架不仅暴露了现有基准的关键缺陷，还通过严格的验证机制实现了更可靠的多跳问答评估，推动了向严谨推理的转变。
...
TL;DR: 扩散模型在连续域生成建模中表现优异，但在离散数据上应用困难。研究发现DDPM求解器在从离散分布采样时，在临界区间内数据密度变为多模态，导致采样进入低密度区域，影响生成质量。通过自条件化和q-sampling可缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为连续域生成建模的标准方法，但在离散数据上的应用仍面临挑战。本研究旨在探究高斯扩散模型（使用DDPM求解器）为何难以从离散分布中采样，这些离散分布在连续空间中表示为δ分布的混合。

Method: 使用玩具随机层次模型分析问题，识别出数据密度变为多模态的临界采样区间。提出结合自条件化和q-sampling求解器的方法，在临界区间内从DDPM切换到q-sampling以改善生成质量。

Result: 研究发现DDPM在临界区间内会进入模态间的低密度区域，产生模型分布外的输入，从而降低样本质量。自条件化和q-sampling有助于缓解此问题。在真实数据上，结合自条件化并在临界区间内从DDPM切换到q-sampling能显著提升生成质量。

Conclusion: 扩散模型在离散数据采样中存在临界多模态区间问题，导致DDPM采样质量下降。通过自条件化和q-sampling的组合策略可以有效解决这一问题，在文本、编程代码和蛋白质等多个领域的条件和非条件任务中都验证了该方法的有效性。
...
TL;DR: 研究自监督语音模型训练过程中不同层次语言结构的涌现时机与模式，分析Wav2Vec2和HuBERT模型在荷兰语语音数据上的层间分布和学习轨迹


<details>
  <summary>Details</summary>
Motivation: 探究自监督语音模型在训练过程中何时以及如何习得语言结构，理解不同层次语言表征的涌现模式与预训练目标的影响

Method: 分析六个Wav2Vec2和HuBERT模型在荷兰语语音数据上的中间检查点，研究多种语言结构在不同层级的编码模式和学习轨迹，考察预训练目标定义层级对结构组织的影响

Result: 发现不同层次语言结构呈现显著不同的层间模式和学习轨迹，这些差异部分可由其与声学信号的抽象程度以及输入信息整合的时间尺度解释；高阶预测任务（迭代精化的伪标签）能诱导更大的并行性

Conclusion: 预训练目标的定义层级强烈影响语言结构的层间组织和学习轨迹，模型训练过程中语言结构的涌现模式具有层次特异性，为理解自监督语音表征学习提供了重要见解
...
TL;DR: 提出BidirLM方法，将因果生成语言模型转换为双向编码器，通过系统消融实验确定关键因素，引入双重策略缓解灾难性遗忘，并通过模型合并增强多模态能力。


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成语言模型转换为双向编码器的方法存在三个主要问题：缺乏对最优训练目标的共识、在大规模应用时出现灾难性遗忘、以及无法灵活整合专业化的生成模型生态系统。需要一种系统化的解决方案来克服这些限制。

Method: 1. 在Gemma3和Qwen3模型家族上进行系统消融实验，识别成功适应的关键因素，特别强调常被忽略的先验掩码阶段的重要性。2. 引入双重策略：线性权重合并和轻量级多领域数据混合，以缓解灾难性遗忘问题。3. 通过将编码器与专业因果模型合并，实现模态和领域特定能力的无缝转移。

Result: 开发出BidirLM，一个包含五个编码器的开源家族，在文本、视觉和音频表示基准测试中优于现有替代方案。该方法适用于任何因果解码器LLM。

Conclusion: 提出了一种系统化的方法，成功将因果生成语言模型转换为高性能双向编码器，解决了当前方法的局限性，并通过开源配方为社区提供了可复现的解决方案。
...
TL;DR: GOOSE提出了一种基于上下文匹配和统计预测两种token来源质量差异的自适应脊柱树结构，通过将高接受率的上下文匹配token组成深链，低接受率的统计预测token作为宽分支，在固定验证预算下实现更优的解码加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法通常使用单一token来源构建树结构，没有区分不同来源token的质量差异。作者观察到上下文匹配token和统计预测token的接受率存在显著差异（中位数约6倍差距），这为优化树结构设计提供了机会。

Method: GOOSE采用训练无关框架，构建自适应脊柱树：将高接受率的上下文匹配token组成深链作为"脊柱"，在每个节点上为低接受率的统计预测token添加宽分支。这种各向异性结构突破了平衡树的深度限制，在固定验证预算下最大化接受token数量。

Result: 在五个LLM（7B-33B）和五个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下比平衡树基线提升12-33%。理论证明显示，接受token数量至少与单独使用任一来源相当。

Conclusion: 通过利用不同token来源的质量差异设计各向异性树结构，GOOSE显著提升了推测解码效率，证明了区分token来源质量并相应优化树拓扑的重要性。
...
Result: 在知识密集型基准测试中，RRPO显著优于包括强大的列表式重排序器RankZephyr在内的强基线，且框架具有良好泛化性、正交集成能力和噪声鲁棒性

Conclusion: RRPO通过直接对齐重排序与LLM生成质量，有效解决了当前重排序模型与下游生成过程脱节的问题，为检索增强生成系统提供了更有效的重排序方案
...
TL;DR: 提出"韵律ABX"框架，扩展传统ABX任务来评估自监督语音模型对韵律对比的敏感性，无需显式标签


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型已知对音位对比敏感，但其对韵律对比的敏感性尚未直接测量。传统ABX任务主要用于音位对比评估，需要扩展到韵律领域

Method: 1) 提出韵律ABX框架，扩展ABX判别任务来评估韵律对比；2) 构建并发布英语和日语最小对立对数据集；3) 结合普通话数据集评估英语重音、日语音高重音和普通话声调的对比敏感性；4) 分析模型和层级排名在不同实验条件下的稳定性

Result: 1) 成功评估了自监督语音模型对英语重音、日语音高重音和普通话声调的对比敏感性；2) 发现模型和层级排名在多种实验条件下通常保持稳定，使其适用于低资源场景

Conclusion: 韵律ABX框架为评估自监督语音模型的韵律对比敏感性提供了有效方法，无需大量数据或显式标签，且结果在低资源设置下具有实用性
...
TL;DR: 论文提出稳定性过滤方法，通过筛选行为稳定的边界构建更有效的引导向量，解决现有方法因行为不稳定边界导致的信号稀释问题，在数学推理任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于关键词匹配的方法假设检测到的边界都编码了真实的行为信号，但研究发现大多数边界（93.3%）是行为不稳定的，无法在相同前缀下重现目标行为，导致引导信号被稀释。

Method: 1) 提出概率模型将内在推理行为形式化为具有上下文依赖触发概率的随机事件；2) 开发稳定性过滤方法，仅保留模型能一致重现目标行为的边界；3) 结合内容子空间投影去除残差问题特定噪声。

Result: 在MATH-500上达到0.784准确率（比最强基线提升5.0%）。构建的引导向量可在同架构模型间迁移而无需重新提取，提升Nemotron-Research-Reasoning-1.5B（+5.0）和DeepScaleR-1.5B-Preview（+6.0）。

Conclusion: 稳定性过滤能有效识别真实的行为信号，构建更纯净的引导向量，显著提升推理行为控制效果，且具有跨模型迁移能力，为训练自由的行为控制提供了更可靠的方法。
...
TL;DR: GaelEval是首个针对苏格兰盖尔语的多维度基准测试，包含形态句法MCQA、文化翻译和文化知识问答三个任务，评估19个LLM后发现前沿模型在盖尔语语法多个维度上超越人类基线，专有模型优于开源模型，盖尔语提示带来小幅稳定优势。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在非官方支持语言中常表现出不均衡的"影子"能力，对于形态句法丰富的少数语言如苏格兰盖尔语，现有翻译基准无法捕捉其结构能力，需要更全面的评估框架。

Method: 开发GaelEval基准测试，包含三个维度：1)专家编写的形态句法多项选择题任务；2)文化基础翻译基准；3)大规模文化知识问答任务。评估19个LLM，与30名流利说话者的人类基线对比，分析专有模型与开源模型表现差异，以及盖尔语提示效果。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超越人类基线(78.1%)。专有模型持续优于开源系统，盖尔语提示带来+2.4%稳定优势。在文化任务上，领先模型超过90%准确率，但多数系统在盖尔语提示下表现更差，绝对分数相对于手动基准有所膨胀。

Conclusion: GaelEval揭示前沿模型在盖尔语语法多个维度上达到超人类表现，证明了盖尔语提示的效果，并显示专有模型相对于开源模型存在一致性能差距，为少数语言评估提供了更全面的基准框架。
...
TL;DR: 该论文介绍了在SOMD 2026跨文档软件提及共指消解共享任务中获得第二名的方法，比较了无需微调的模糊匹配和上下文感知表示两种方法，分析了它们在噪声条件下的表现差异和计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 软件提及共指消解是一个研究不足的任务，需要开发有效的方法来处理跨文档中软件名称的共指关系。作者旨在比较两种无需微调的方法在真实场景下的性能表现。

Method: 提出了两种无需微调的方法：1) 模糊匹配(FM)：基于词汇字符串相似性的方法；2) 上下文感知表示(CAR)：结合提及级别和文档级别嵌入的方法。在SOMD 2026共享任务的三个子任务上进行了评估。

Result: 两种方法在所有子任务上都取得了竞争性性能（CoNLL F1 0.94-0.96），CAR在官方测试集上始终比FM高出1个百分点。噪声注入研究显示互补的失败模式：边界噪声下CAR更稳健（仅损失0.07 F1），而提及替换下FM更稳健（损失0.52 vs 0.63）。CAR的计算复杂度近似线性，FM超线性。

Conclusion: 系统选择应考虑上游提及检测器的噪声特征和目标语料库的规模。CAR在大规模语料上更高效，而FM在特定噪声条件下表现更好。软件名称的高表面规律性降低了对复杂语义推理的需求。
...
<details>
  <summary>Details</summary>
Motivation: 文本频率已被验证与人类阅读速度相关，但其与大型语言模型（LLMs）的相关性研究较少。本文旨在探索文本数据频率这一未充分研究的话题，提出文本频率对LLM提示和微调的重要性。

Method: 1. 提出文本频率定律（TFL）：优先使用高频文本数据进行LLM提示和微调，使用在线资源估计句子级频率，通过输入改写器将输入改写为更频繁的文本表达。2. 提出文本频率蒸馏（TFD）：查询LLM进行故事补全以扩展数据集句子，用生成语料调整初始频率估计。3. 提出课程文本频率训练（CTFT）：按句子级频率递增顺序微调LLMs。

Result: 在自建数据集TFPD（包含数学推理、机器翻译、常识推理和智能体工具调用任务）上进行实验，结果显示所提框架的有效性。

Conclusion: 文本频率是影响LLM性能的重要因素，提出的三单元框架（TFL、TFD、CTFT）能够有效利用文本频率优化LLM在多种任务上的表现，为LLM优化提供了新的研究方向。
...
TL;DR: 提出Neuro-RIT框架，通过神经元引导的鲁棒指令调优，解决检索增强语言模型在噪声上下文下的性能退化问题，从密集适应转向精确神经元对齐


<details>
  <summary>Details</summary>
Motivation: 检索增强语言模型在知识密集型任务中表现良好，但在面对不相关或噪声检索上下文时性能会显著下降。现有方法通常在层或模块级别进行粗粒度参数更新，忽略了大型语言模型固有的神经元级稀疏性

Method: 提出Neuro-RIT框架：1) 使用基于归因的神经元挖掘显式分离处理相关与不相关上下文的神经元；2) 采用两阶段指令调优策略：通过功能失活专用于不相关上下文的神经元实现直接噪声抑制，同时优化目标层进行证据蒸馏

Result: 在多样化QA基准测试上的广泛实验表明，Neuro-RIT始终优于强基线方法和鲁棒性增强方法

Conclusion: 通过从密集适应转向精确神经元对齐，Neuro-RIT框架有效解决了检索增强语言模型在噪声上下文下的鲁棒性问题，为提升模型抗干扰能力提供了新范式
...
TL;DR: L3TR：一个基于大语言模型的列表式人才推荐框架，通过块注意力机制和局部位置编码解决位置偏差和并发令牌偏差问题，相比点式方法显著提升推荐效果


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的人才推荐系统大多采用点式范式，需要重复处理文本且无法捕捉候选人之间的关系，导致令牌消耗高且推荐效果不佳。此外，LLM在处理多项选择题和长文档时存在位置偏差和"迷失在中间"问题。

Method: 提出L3TR框架：1)采用隐式策略利用LLM的潜在输出；2)设计块注意力机制增强文档间处理；3)引入局部位置编码方法缓解位置偏差和并发令牌偏差；4)提出ID采样方法解决训练和推理阶段候选集规模不一致问题；5)设计位置偏差和令牌偏差检测方法及无训练去偏方法。

Result: 在两个真实世界数据集上的广泛实验验证了L3TR的有效性，相比现有基线方法取得了持续改进。

Conclusion: L3TR框架通过列表式处理、块注意力机制和局部位置编码，有效解决了基于LLM的人才推荐系统中的位置偏差、令牌偏差和效率问题，为人才推荐任务提供了更优的解决方案。
...
Method: 1. 创建CV-18 NER数据集：基于阿拉伯语Common Voice 18语料库，按照Wojood模式（21种实体类型）进行手动NER标注；2. 基准测试：比较级联系统（ASR + 文本NER）和基于Whisper、AraBEST-RQ的端到端模型；3. 分析阿拉伯语特定自监督预训练与多语言弱监督对任务的影响。

Result: 端到端系统在测试集上显著优于最佳级联配置：AraBEST-RQ 300M达到37.0% CoER，Whisper-medium达到38.0% CVER。分析表明：阿拉伯语特定自监督预训练在ASR上表现强劲，而多语言弱监督在联合语音到实体学习上转移更有效，且在此低资源设置中，更大模型可能更难适应。

Conclusion: 端到端方法在阿拉伯语语音NER任务上优于传统级联管道，CV-18 NER数据集和模型的公开发布为阿拉伯语语音端到端命名实体识别提供了首个开放基准。
...
TL;DR: 论文研究如何从多个LLM中生成全面的答案集，提出多样性覆盖率指标评估模型生成多样化答案的能力，并开发了基于查询的路由器选择最佳模型。


<details>
  <summary>Details</summary>
Motivation: 当面对允许多个有效答案的提示时，全面生成这些答案是满足多样化用户需求的第一步。现有LLM在生成全面答案集方面存在差异，需要系统评估和优化方法。

Method: 1) 提出多样性覆盖率指标，衡量预测答案集中每个唯一答案的质量得分相对于相同数量最佳可能答案集的比例；2) 评估18个LLM的多样性表现；3) 开发基于查询的路由器，预测每个查询的最佳模型。

Result: 1) 没有单一模型在所有开放提示上主导多样化响应生成；2) 每个提示都存在显著优于其他模型的特定模型；3) 在NB-Wildchat数据集上，训练的路由器优于单一最佳模型基线（26.3% vs 23.8%）；4) 方法在NB-Curated数据集和不同答案生成提示策略上具有泛化能力。

Conclusion: 该研究为在拥有多个模型的情况下生成全面答案集奠定了基础，证明了基于查询的路由器在优化多样化答案生成方面的有效性，为多模型系统设计提供了新思路。
...
TL;DR: 研究发现语言模型扩展新词汇时，均值初始化方法存在缺陷，提出基于语言基础的初始化方法GTI，在多个生成推荐基准测试中表现更优


<details>
  <summary>Details</summary>
Motivation: 语言模型在扩展新词汇时通常采用均值初始化方法，但这种方法存在缺陷，导致新token在嵌入空间中坍缩到退化子空间，消除token间差异，使得后续微调难以完全恢复

Method: 通过谱分析和几何诊断分析均值初始化的问题，提出"基于语言的token初始化假设"，并实现为GTI方法：在微调前，仅使用配对的语言监督将新token映射到预训练嵌入空间中具有语义意义的独特位置

Result: GTI在大多数评估设置中优于均值初始化和现有的辅助任务适应方法，在行业规模和公共数据集上的生成推荐基准测试中表现更好；分析显示基于语言的嵌入产生更丰富的token间结构，这些结构在微调过程中得以保持

Conclusion: 初始化质量是词汇扩展的关键瓶颈，基于语言的token初始化假设得到验证，GTI作为一种轻量级方法能有效改善语言模型扩展新词汇时的性能
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为智能体AI系统的核心控制组件，可靠的结构化路由需要在正确性、延迟和实施成本之间取得平衡。当前研究通常将结构化路由视为提示工程问题，但作者认为这本质上是系统级负载分配问题，需要从系统层面考虑结构工作在整个生成栈中的分配方式。

Method: 采用全面的全因子基准测试，涵盖48种部署配置和15,552个请求，覆盖OpenAI、Gemini和Llama三种后端。评估不同路由模式（模型直接输出结构、传输中压缩、生成后本地重建）在异构后端条件下的性能表现。

Result: 核心发现：不存在通用的最佳路由模式，后端特定的交互效应主导性能。在Gemini和OpenAI上高度可靠的路由模式在Llama上可能出现显著的正确性下降，压缩实现的效率增益也强烈依赖于后端。不同后端之间存在明显的性能差异和权衡关系。
...
Method: 提出数字孪生反事实框架：1) 将数字孪生模拟器形式化为潜在结果框架中的随机映射；2) 建立孪生保真度假设层次(边际保真度、联合保真度、结构保真度)；3) 设计五层验证架构将不可证伪主张转化为可检验测试；4) 形式化分解将因果量分为可边际验证和依赖copula的两类；5) 提供边界、敏感性和不确定性量化工具

Result: 建立了系统框架：1) 边际因果主张(ATE、CATE、QTE)可通过可观测臂比较进行检验；2) 联合因果主张(ITE分布、获益/损害概率、处理效应方差)明确依赖不可观测的个体内依赖结构；3) copula依赖关系通过边界和敏感性分析变得显式；4) 在边际因果主张可检验性增强的同时，明确表征了边际与联合主张之间的差距

Conclusion: DTCF不解决因果推断的根本问题，但提供了系统框架：边际因果主张变得可检验，联合因果主张明确依赖假设索引，两者之间的差距被形式化表征。该框架使因果推断更加透明和可验证
...
TL;DR: IDEA2是一个结合大型语言模型和专家在环的半自动化工作流，用于从需求文档中提取能力问题，通过迭代反馈提高接受度和相关性。
...
Method: IDEA2采用半自动化工作流，包含核心迭代循环：1) LLM从需求文档中提取初始CQ；2) 领域专家在协作平台上进行共创式评审和反馈；3) LLM根据反馈迭代重新表述被拒绝的CQ直至达成共识。使用溯源模型跟踪每个CQ的完整生命周期。

Result: 在两个真实场景（科学数据、文化遗产）中验证，表明IDEA2能加速需求工程过程，提高最终CQ的接受度和相关性，在领域专家中表现出高可用性和有效性。

Conclusion: IDEA2通过整合LLM和专家协作，有效解决了能力问题提取的瓶颈问题，提供了透明、可复现的工作流，显著提升了本体工程需求阶段的效率和质量。
...
Method: 提出世界中心多智能体系统架构，其中学习和协调操作基于共享的世界模型而非孤立的智能体本地表示；引入语义模型作为表示这类世界的数学形式化方法；开发Ontobox平台作为WMAS的实现。

Result: 建立了世界中心多智能体系统的理论框架，包括世界分类维度（本体显式性、规范性等），提出了语义模型作为形式化表示，并实现了Ontobox平台作为具体实现方案。

Conclusion: 世界中心多智能体系统为结构化领域提供了一种更优的架构选择，通过共享的显式世界表示确保了语义一致性、可解释性和系统行为的可验证性，为企业和制度系统的多智能体应用提供了理论基础和实现平台。
...
TL;DR: 该研究提出AI自动化是一个连续谱：崩溃浪潮（AI能力在少量任务上突然激增）和上升潮汐（AI能力持续广泛提升）。基于对3000多个文本任务的评估，研究发现上升潮汐是主要形式，AI在广泛任务上表现良好且快速改进，预计到2029年LLM能以80%-95%成功率完成大多数文本任务。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解AI自动化如何影响经济与劳动力市场。现有研究关注AI在特定任务上的突然突破（崩溃浪潮），但缺乏对AI能力广泛持续提升（上升潮汐）的系统分析。需要实证检验哪种模式主导AI自动化进程，以预测其对就业和经济的影响。

Method: 研究方法包括：1）提出AI自动化连续谱理论框架；2）构建包含3000多个文本任务的评估集，基于美国劳工部O*NET职业分类；3）收集超过17,000份来自相关职业工作者的评估；4）分析AI模型在2024-Q2至2025-Q3期间的任务完成成功率；5）基于趋势预测未来AI能力发展。

Result: 研究发现：1）几乎没有崩溃浪潮的证据（与METR研究相反）；2）上升潮汐是AI自动化的主要形式；3）2024-Q2 AI模型能以约50%成功率完成人类需3-4小时的任务；4）到2025-Q3成功率提升至约65%；5）若趋势持续，到2029年LLM能以80%-95%平均成功率完成大多数文本任务。

Conclusion: 结论是AI自动化主要通过上升潮汐模式发生，AI能力在广泛任务上持续提升。虽然技术能力快速发展，但经济影响可能滞后，因为组织采纳AI需要更长时间。研究强调了理解AI能力广泛持续提升对劳动力市场预测的重要性。
...
TL;DR: LLMs存在系统性认知偏差，这些偏差在模型激活空间中呈线性可分方向，可通过激活引导技术有效减少偏差同时保持下游任务能力。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地应用于高风险决策场景，虽然已有研究表明LLMs表现出认知偏差行为，但这些偏差是否对应可识别的内部表征以及能否通过针对性干预来缓解仍然是一个开放性问题。

Method: 1) 定义LLM认知偏差为在可计算基准任务中系统性、可复现的偏离正确答案；2) 引入LLM CogBias基准，围绕四个认知偏差家族组织：判断、信息处理、社会和反应偏差；3) 评估三个LLMs；4) 使用对比设计下的线性探针分析偏差在激活空间中的表征；5) 应用激活引导技术调节偏差行为。

Result: 1) 认知偏差在所有四个家族中系统性出现，幅度和去偏响应强烈依赖于偏差家族：提示级去偏显著减少反应偏差但对判断偏差产生反效果；2) 认知偏差在模型激活空间中编码为线性可分方向；3) 激活引导可将偏差分数（偏差响应比例）减少26-32%，同时在25个基准测试中保持下游能力（Llama：性能下降可忽略；Qwen：判断偏差最多下降19.0个百分点）；4) 尽管不同模型间的偏差表征接近正交（平均余弦相似度0.01），但引导在不同架构中减少偏差的效果相似，表明存在共享的功能组织。

Conclusion: LLMs确实表现出系统性认知偏差，这些偏差在内部表征中是可识别的，并且可以通过激活引导技术有效缓解。不同模型间偏差表征的差异与去偏效果的相似性表明，尽管具体表征不同，但认知偏差在LLMs中具有共享的功能组织，这为理解和干预LLM认知偏差提供了理论基础和实践方法。
...
TL;DR: 提出RIFT（RubrIc Failure mode Taxonomy）分类法，用于系统诊断评分标准（rubric）的质量问题，包含8种失效模式，分为3大类，并通过自动化指标实现可扩展的诊断。
...
Method: 采用扎根理论方法，从五个多样化基准（通用指令遵循、代码生成、创意写作、专家级深度研究）中迭代标注评分标准，直至不再发现新的失效模式。开发了包含8种失效模式的RIFT分类法，分为可靠性失效、内容有效性失效和后果有效性失效三大类。评估了分类法的一致性（人工标注者间一致性），并提出了自动化评分标准质量指标。

Result: 观察到整体公平的一致性（87%的成对一致性和0.64的平均Cohen's kappa）。自动化指标与人工失效模式标注对齐，达到最高0.86的F1分数，支持可扩展的诊断。

Conclusion: RIFT分类法为系统诊断评分标准质量问题提供了框架，自动化指标能够有效识别失效模式，有助于改进LLM评估和训练中评分标准的设计质量。
...
TL;DR: 提出VOIMCP算法，通过基于信息价值选择性处理观测来高效分配计算资源，在有限规划时间内提升POMDP求解性能


<details>
  <summary>Details</summary>
Motivation: POMDP在状态和转移不确定性下的规划具有挑战性，特别是在有限规划时间内获得高性能策略。许多POMDP问题中，信息价值在信念空间中的变化为优化计算分配提供了机会。

Method: 提出动态规划框架，基于每个信念的信息价值有条件地处理观测；在此基础上提出VOIMCP算法，当信息价值低时选择性忽略观测信息，避免不必要的观测分支。

Result: 提供了VOI推理框架的近似最优性理论保证，推导了VOIMCP的非渐近收敛界；仿真评估显示VOIMCP在多个POMDP基准测试中优于基线方法。

Conclusion: 通过利用信息价值在信念空间中的变化性，选择性处理观测信息能够更高效地分配计算资源，在有限规划时间内获得更好的POMDP求解性能。
...
<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药学领域应用日益广泛，但在高风险用药管理场景中，即使单个错误推荐也可能导致严重患者伤害。当前评估主要依赖传统性能指标，对系统在实际决策中的可靠性理解不足，需要关注错误发生机制及其临床影响。

Method: 通过一系列受控模拟场景（涉及药物相互作用和剂量决策），分析不同类型的系统故障，包括遗漏相互作用、错误风险标记和不适当剂量推荐。研究采用可靠性导向的评估框架，而非仅依赖聚合性能指标。

Result: 研究发现AI在用药相关场景中的错误可能导致药物不良反应、治疗无效或延迟护理，特别是在缺乏充分人工监督的情况下使用系统时。研究识别了特定故障模式及其潜在临床后果，并揭示了过度依赖AI推荐和决策过程透明度有限带来的风险。

Conclusion: 该研究为医疗AI评估提供了可靠性导向的视角，强调理解故障行为和实际影响的重要性。在药房实践等安全关键领域，需要将传统性能指标与风险感知的评估方法相结合，并加强人工监督和系统透明度。
...
<details>
  <summary>Details</summary>
Motivation: 科学发现因文献碎片化而减缓，现有AI工具缺乏结构化、多步骤的方法来从科学文献中提取深度见解，LLM存在幻觉和不完整提取的问题。
...
Method: 1) 提出新的数学规划公式和可证明的零阶段不可行性筛选；2) 构建可验证的训练数据集（可行带结构化证书或可证明不可行）；3) 使用精确优化流程生成的数据对8B参数LLM进行微调；4) 将LLM输出作为下游局部搜索的初始解。

Result: 微调模型相比GPT-5.2整体准确率提升高达30%；LLM引导的初始解为下游局部搜索提供高达2倍的加速。

Conclusion: 提出的不可行性感知框架能够有效提升LLM在组合优化问题中的性能，既能生成解决方案又能检测不可行性，并通过LLM辅助的初始解加速下游优化过程。
...
TL;DR: P-COD通过文档间关系检测异常值，在科学文献摘要中减少LLM幻觉，实现98%的检测精度


<details>
  <summary>Details</summary>
Motivation: 当前方法（如提示工程、思维链）仅关注单个文档，忽略了语料库中文档间的关系，导致LLM在从大型文本语料库中提取数据时产生幻觉，影响准确性

Method: 提出Peer Context Outlier Detection (P-COD)方法，利用文档间关系改进提取准确性。在科学文献摘要领域，通过比较提取数据与语料库中已验证的同行信息，调整置信度分数并标记低置信结果供专家审查

Result: 实验显示在6个科学领域中异常值检测精度高达98%，表明该方法能有效减少幻觉，增强对自动化系统的信任，让研究人员专注于模糊案例

Conclusion: P-COD通过利用文档间关系显著减少了LLM的幻觉，提高了数据提取工作流的效率，使研究人员能够专注于需要人工审查的模糊案例
...
TL;DR: 提出一个基于技能演化的智能体框架，用于超表面逆向设计，通过迭代优化特定求解器策略来积累可重用工作流知识，提高设计成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 超表面逆向设计需要复杂的计算电磁学专业知识和特定求解器软件工程技能，现有基于大语言模型的系统缺乏跨任务的可重用工作流知识积累能力，限制了设计流程的自主性和可访问性。

Method: 提出一个智能体框架，包含编码智能体、演化技能工件和基于物理仿真的确定性评估器。该框架通过迭代优化特定求解器策略，在不修改模型权重或底层物理求解器的情况下，跨任务积累可重用工作流知识。

Result: 在包含多个超表面逆向设计任务类型的基准测试中，演化技能将分布内任务成功率从38%提升至74%，标准通过率从0.510提升至0.870，平均尝试次数从4.10降至2.30。在分布外任务上，虽然二进制成功率变化不大，但最佳边际改进、错误构成变化和智能体行为表明工作流知识实现了部分迁移。
...
TL;DR: 提出基于角色的LLM框架，通过分配专门角色（政策分析师、法律策略专家、食品系统专家）自动化健康食品政策的信息提取，解决现有方法中的幻觉、误分类和遗漏问题。
...
Conclusion: 该基于角色的LLM框架为健康政策信息提取提供了可靠且透明的方法论，能够有效自动化信息提取过程，优于传统基线方法。
...
TL;DR: PHMForge是首个专门为预测与健康管理（PHM）任务设计的LLM智能体基准测试，包含75个工业场景、65个专用工具，评估显示当前最佳配置仅完成68%任务，存在工具编排、多资产推理和跨设备泛化等系统性缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法满足工业领域对LLM智能体的严格要求，工业领域的错误决策会带来重大的安全和财务后果。需要专门针对预测与健康管理（PHM）任务的基准测试来评估LLM智能体在真实工业场景中的表现。

Method: 构建PHMForge基准测试，包含75个专家策划的工业场景，涵盖7类工业资产（涡扇发动机、轴承、电机、变速箱、航空发动机）和5个核心任务类别。开发65个专用工具分布在两个MCP服务器上，实现基于执行的评估器，使用任务匹配的评估指标（MAE/RMSE用于回归，F1分数用于分类，分类匹配用于健康评估）。

Result: 评估领先框架（ReAct、Cursor Agent、Claude Code）与前沿LLM（Claude Sonnet 4.0、GPT-4o、Granite-3.0-8B）的组合，发现即使最佳配置也仅完成68%的任务。存在系统性失败：23%的工具编排错误序列，多资产推理性能下降14.9个百分点，在未见数据集上的跨设备泛化能力仅为42.7%。

Conclusion: PHMForge填补了工业领域LLM智能体评估的关键空白，揭示了当前LLM智能体在PHM任务中的显著局限性，特别是在工具编排、多资产推理和跨设备泛化方面。开源基准测试将推动工业AI智能体研究的发展。
...
TL;DR: 该论文研究了将高维表示自编码器（RAE）整合到连续自回归（AR）模型中的挑战，提出了分布归一化和高斯噪声注入两种方法来解决建模难度和曝光偏差问题，使RAE在AR模型中达到与VAE相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统上，生成模型的潜在空间由VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法显示表示自编码器也能达到与VAE编码器竞争的性能，但将其整合到连续自回归模型中的研究仍然不足。本文旨在探索在AR范式下使用高维表示自编码器（RAE-AR）面临的挑战。

Method: 论文针对AR模型的两个主要挑战提出解决方案：1）通过分布归一化进行标记简化，降低复杂标记级分布建模的难度并改善收敛性；2）在训练期间加入高斯噪声注入以增强预测鲁棒性，缓解高维度放大的训练-推理差距（曝光偏差）。

Result: 实证结果表明，这些修改显著缩小了性能差距，使表示自编码器在AR模型中能够达到与传统VAE相当的结果。这为视觉理解和生成建模之间更统一的架构铺平了道路。

Conclusion: 本文成功解决了将高维表示自编码器整合到连续自回归模型中的关键挑战，通过分布归一化和噪声注入技术，使RAE-AR系统能够达到与基于VAE的AR模型竞争的性能，推动了视觉理解与生成建模架构的统一。
...
TL;DR: 研究发现LLM训练中归一化层和优化器不应视为独立设计选择：Derf归一化与Muon优化器存在显著负交互作用，导致性能下降0.66 nats，而DyT归一化无此问题


<details>
  <summary>Details</summary>
Motivation: 传统LLM训练中将归一化层和优化器视为独立设计选择，但研究发现这种假设可能不成立，特定组合会产生显著的负交互作用，影响模型性能

Method: 在10亿参数规模上进行3x2因子实验，训练1000步，比较Derf、RMSNorm和DyT三种归一化层与AdamW、Muon两种优化器的组合效果，分析Derf与Muon负交互的失效模式

Result: Derf与Muon组合存在显著负交互：性能差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats；DyT无此问题；失效模式包括饱和和尺度盲区；通过EMA混合或降低alpha参数可恢复大部分性能差距

Conclusion: 归一化层和优化器选择存在重要交互作用，不应独立设计；Derf与Muon组合存在隐藏的0.66 nats性能损失；使用EMA混合或调整alpha参数可缓解此问题
...
Method: 提出NED-Tree系统框架：(a)采用逐句提取策略确保稳健的参数映射和可追溯性；(b)使用递归树结构自适应地将复杂非线性项分解为求解器兼容的子元素。同时创建NEXTOR基准测试用于复杂非线性、多约束OR问题

Result: 在10个基准测试中，NED-Tree达到72.51%的平均准确率，建立了新的最先进水平。这是首个通过元素分解驱动LLM解决非线性建模困难的框架

Conclusion: NED-Tree成功弥合了建模语义与代码语义之间的差距，为自动化OR问题翻译提供了有效解决方案，框架和基准测试已公开可用
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习与人类反馈（RLHF）方法通常需要人工标注或正确性信号来训练模型进行自我精炼。本文旨在开发一种无需额外标注或正确性信号的框架，能够联合优化推理和精炼能力，提高模型在复杂推理任务上的表现。

Method: 提出ThinkTwice两阶段框架：1）第一阶段优化模型解决推理问题；2）第二阶段优化模型精炼自己的解决方案。两个阶段使用相同的二元正确性奖励，基于Group Relative Policy Optimization（GRPO）进行训练，无需正确性信号或批判性标注。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性的在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME基准上比GRPO在精炼前提升5个百分点，在自我精炼一步后提升11.5个百分点（pass@4）。训练动态分析显示隐含的"纠正-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保持已正确的解决方案。

Conclusion: ThinkTwice框架为RLVR（强化学习与验证反馈）提供了一种原则性且有效的方法论，通过联合训练推理和自我精炼能力，无需额外标注即可显著提升模型性能，揭示了训练动态中的隐含课程学习机制。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLMs在教学决策时是否像人类一样通过推理学习者知识状态来制定教学策略，还是仅使用简单的启发式规则。这有助于理解LLMs的教学能力本质。

Method: 使用先前研究人类教学策略的受控任务：教师LLM观察学习者在奖励标注有向图中的轨迹，揭示单一边缘以帮助学习者重新规划更好路径。将LLMs作为模拟教师，用与人类研究相同的认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心理化效用模型）拟合其逐次选择。进行基线实验和支架干预实验。

Result: 在基线实验中，大多数LLMs表现良好，策略变化小，图间表现与人类相似。模型比较显示贝叶斯最优教学最能解释多数模型的决策。支架干预中，模型遵循辅助推理或奖励导向提示，但这些支架在启发式不一致测试图上不能可靠改善教学，有时反而降低性能。

Conclusion: 认知模型拟合揭示了LLMs的教学策略本质上是贝叶斯最优推理，而非简单启发式。提示合规性不能保证更好的教学决策，表明LLMs的教学能力有其内在机制，外部干预不一定能有效改善。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖监督微调，缺乏代码执行环境暴露，且即使最先进的MLLMs也难以有效进行自校正。需要一种能够通过环境交互和迭代自校正来提高代码生成质量的方法。
...
Conclusion: MM-ReCoder通过强化学习和自校正能力有效解决了现有MLLMs在图表到代码生成任务中的局限性，为多模态编码任务提供了一种有前景的新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有拒绝特征选择方法依赖特征在有害提示上的激活强度，这倾向于捕捉表面信号而非拒绝决策的因果因素。需要更可靠的方法来识别因果性拒绝特征

Method: 提出CRaFT框架，通过模型拒绝-合规决策边界附近的提示来评估特征对决策的影响度，以此排名特征，而非依赖激活强度

Result: 在Gemma-3-1B-it模型上，CRaFT将攻击成功率从6.7%提升至48.2%，在多个越狱基准测试中优于基线方法

Conclusion: 电路影响度比激活强度更可靠，可作为识别因果性拒绝特征的标准，有助于理解LLM拒绝行为的内在机制
...
TL;DR: 论文提出Metric Freedom (F)作为技能效用的先验预测器，通过度量评分地形的拓扑刚性来预测多智能体系统向单智能体技能蒸馏的效果，并基于此设计自适应蒸馏框架。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统(MAS)在处理复杂任务时存在协调开销大、上下文碎片化和脆弱的阶段排序等问题。将MAS蒸馏为单智能体技能可以规避这些成本，但缺乏关于何时以及蒸馏什么的原则性指导，导致实证结果不一致（技能提升从28%改进到2%退化）。

Method: 提出Metric Freedom (F)作为技能效用的先验预测器，通过Mantel测试量化输出多样性与得分方差之间的耦合程度，测量度量评分地形的拓扑刚性。基于F设计两阶段自适应蒸馏框架：阶段1作为选择性提取机制，在"自由"度量上提取工具和知识同时丢弃限制性结构以保持探索；阶段2专门针对"刚性"度量(F ≤ 0.6)进行计算密集的迭代优化，消除轨迹局部过拟合。
...
Conclusion: 技能效用本质上是度量层面的属性，而非任务层面的。Metric Freedom (F)作为第一个技能效用的先验预测器，能有效指导自适应蒸馏，在保持或提升性能的同时显著降低多智能体系统的协调成本和延迟。
...
TL;DR: GraphWalk是一个无需训练、基于工具的框架，使现成LLM能够通过顺序图导航进行推理，显著提升大规模知识图谱上的性能。


<details>
  <summary>Details</summary>
Motivation: 企业级知识图谱规模庞大，无法放入现有最大上下文窗口，标准方法（如提示技术或检索增强生成）在处理复杂查询时面临严重限制，需要多跳推理能力。

Method: GraphWalk为LLM配备一组最小正交图操作（工具），使模型能够遍历任意图结构，通过顺序工具调用进行多步推理，每个工具调用代表可验证步骤并创建透明执行轨迹。

Result: 在迷宫遍历和合成企业知识图谱上的评估显示，基于工具的遍历方法在所有测试模型家族中都带来显著且一致的性能提升，尤其在大规模场景下优势更明显，而上下文方法则完全失败。

Conclusion: GraphWalk框架使LLM能够通过最小工具集进行顺序图导航，有效解决大规模知识图谱上的复杂推理问题，提供透明执行轨迹，且性能随模型规模增加而提升。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是评估大型语言模型在生产环境中的收据项目分类应用，特别关注不同模型在准确性、响应稳定性和成本效率方面的表现，以及零样本与少样本提示方法的适用性。

Method: 研究方法包括：1）在AWS Bedrock平台上测试四种指令调优模型（Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct、Mistral 7B Instruct）；2）评估准确性、响应稳定性和令牌级成本；3）比较零样本与少样本提示方法的效果。

Result: 实验结果表明：Claude 3.7 Sonnet在分类准确性和成本效率之间达到了最有利的平衡。研究还揭示了不同提示方法在准确性和成本方面的适用性差异。

Conclusion: 结论是Claude 3.7 Sonnet在生产环境的收据项目分类任务中表现出最佳的综合性能，为实际应用提供了成本效益高的解决方案。研究强调了在LLM部署中同时考虑性能指标和经济成本的重要性。
...
TL;DR: OSCAR是一个无需训练、基于推理时间的框架，通过并行去噪链和交叉链熵检测来定位和修正扩散语言模型中的幻觉，显著减少幻觉内容并提高事实准确性。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型(DLMs)的去噪轨迹为推理时间控制提供了天然信号，理想的幻觉缓解框架应利用这一模型原生信号进行干预，而非依赖外部训练的幻觉分类器。

Method: 提出承诺不确定性定位：给定去噪轨迹，识别交叉链熵超过无监督阈值的token位置。引入OSCAR框架：运行N个并行去噪链（随机揭示顺序），计算交叉链香农熵检测高不确定性位置，然后基于检索证据进行针对性重掩码。

Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA数据集上，使用LLaDA-8B和Dream-7B模型，OSCAR显著减少幻觉内容并提高事实准确性。消融实验确认定位和修正具有互补增益，且对N∈{4,8,16}具有鲁棒性。

Conclusion: 扩散语言模型的基于熵的不确定性信号优于专门的训练检测器，突显了DLMs识别事实不确定性的内在能力，这是自回归模型所不具备的。OSCAR为DLMs中的定位和不确定性感知生成研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的进化方法主要依赖固定启发式和硬编码探索规则，限制了智能体的自主性。需要开发能够支持持续搜索和知识积累的自主进化框架来解决开放式发现问题。

Method: CORAL采用多智能体架构，包含持久共享记忆、异步多智能体执行、心跳干预机制。提供隔离工作空间、评估器分离、资源管理、智能体会话和健康管理等安全保障措施。

Result: 在数学、算法和系统优化等多样化任务上，CORAL在10个任务上取得新的SOTA结果，改进率比固定进化搜索基线高3-10倍，且评估次数更少。在Anthropic内核工程任务中，四个协同进化智能体将最佳得分从1363周期提升到1103周期。

Conclusion: 增强智能体自主性和多智能体进化能显著改善开放式发现。CORAL框架通过知识重用和多智能体探索通信机制实现了这一目标，为自主进化系统提供了实用框架。
...
TL;DR: 提出七种面向临床AI系统的本体感知设计模式，用于构建对本体扭曲具有弹性的数据管道，包括数据验证、信号保留、漂移监控、双本体层、反馈中断、术语版本管理和合规适配等模式。
...
Method: 采用Gang-of-Four模式语言提出七种本体感知设计模式：本体检查点、休眠感知管道、漂移哨兵、双本体层、物化断路器、术语版本门、监管合规适配器，并通过初级保健AI系统的参考架构和糖尿病风险预测场景进行说明。

Result: 建立了七种设计模式的完整规范（问题、驱动力、解决方案、后果、已知用例、相关模式），其中三种模式在现有系统中有部分先例，四种模式为首次正式描述，提供了可组合的参考架构。

Conclusion: 论文提供了基于理论分析的设计词汇表，为构建对本体扭曲具有弹性的临床AI系统提供了系统化架构方案，但缺乏运行时基准测试且限于德国和欧盟监管环境，需要未来在生产系统中进行实证评估。
...
### [184] [ContextBudget: Budget-Aware Context Management for Long-Horizon Search Agents](https://arxiv.org/abs/2604.01664)
...
Result: 在组合式多目标QA和长程网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法，在高复杂度设置中达到基线方法1.6倍以上的性能提升，且在预算缩减时保持优势
...
TL;DR: HMO框架通过三层内存架构优化智能体长期记忆管理，基于用户画像动态分配记忆优先级，在保持性能的同时减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 智能体需要长期记忆来维持历史感知一致性，但大量交互数据的积累会导致性能瓶颈：存储扩展增加检索噪声和计算延迟，在受限的个人设备上会超出模型的推理能力。

Method: 提出分层内存编排（HMO）框架，将交互历史组织成三层目录结构：紧凑的主缓存（存储近期和关键记忆+演化用户画像）、高优先级次级层、全局完整历史档案。用户画像驱动记忆在层次间的重新分配，将与长期模式相关的记录提升到更活跃的层级，同时降级相关性较低的信息。

Result: 在多个基准测试中达到最先进性能。在OpenClaw等生态系统中的实际部署表明，HMO显著增强了智能体的流畅性和个性化程度。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要针对同源模型（共享相同预训练骨干），但在开放模型生态系统中，有用专家模型通常基于不同架构（如Llama、Qwen、Mistral），异构设置下的直接权重空间融合因架构不匹配、潜在基不对齐和跨源冲突而变得病态。

Method: HeteroFusion包含两个关键组件：1）基于拓扑的对齐，通过匹配功能模块结构而非原始张量坐标在异构骨干间转移知识；2）冲突感知去噪，在融合过程中抑制不兼容或噪声转移信号。该方法还通过分析证明，在预测结构化更新的同时保持目标适配器基可导致稳定且良态的转移过程。

Result: 在异构转移、多源融合、噪声源鲁棒性和跨家族泛化设置中，HeteroFusion始终优于强大的融合、合并和集成基线方法。

Conclusion: HeteroFusion为异构语言模型融合提供了有效解决方案，通过拓扑对齐和冲突感知去噪克服了架构不匹配问题，在开放模型生态系统中具有重要应用价值。
...
TL;DR: 研究基于中国视频平台数据，发现AIGC通过高产量获得与HGC相当的总体参与度，但消费者明显偏好HGC，算法分发机制能调节这种矛盾。
...
<details>
  <summary>Details</summary>
Motivation: 通用航空故障诊断对飞行安全至关重要，但在资源受限的边缘设备上部署深度学习模型面临计算能力和可解释性的双重挑战。需要设计轻量级、可解释且高效的故障诊断框架。

Method: 采用两阶段级联架构：第一阶段高召回率故障检测，第二阶段异常样本细粒度故障分类。提出基于互信息、梯度分析和SE注意力权重的多方法融合策略压缩输入传感器通道。设计1+1分支LiteInception架构压缩InceptionTime参数。引入知识蒸馏作为精确率-召回率调节机制。构建集成四种归因方法的双层可解释性框架。

Result: 在NGAFID数据集上，故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%。模型参数压缩70%，CPU推理加速超过8倍，F1损失小于3%。输入传感器通道从23个减少到15个。

Conclusion: LiteInception框架在效率、准确性和可解释性之间取得了良好平衡，通过轻量化设计、自适应调节和可解释性增强，为边缘设备上的航空故障诊断提供了实用解决方案。
...
TL;DR: 开发AnIML本体论，通过OWL 2形式化AnIML语义并与Allotrope数据格式对齐，使用专家参与和LLM辅助方法，通过多层验证确保跨系统互操作性。


<details>
  <summary>Details</summary>
Motivation: 异构实验数据系统间的语义互操作性仍是数据驱动科学发现的主要障碍。AnIML作为分析化学和生物学的XML标准，在工业研发实验室中广泛使用，但其XML模式的表达性导致不同利益相关者的解释分歧，破坏了原本设计的互操作性。
...
Result: 成功开发了AnIML本体论，形式化了AnIML语义并与Allotrope数据格式对齐。通过数据驱动转换、SPARQL验证和基于SHACL约束的对抗性验证协议，验证了本体的有效性和可靠性。

Conclusion: AnIML本体论通过形式化语义和与Allotrope数据格式的对齐，解决了AnIML XML模式解释分歧的问题，为跨系统和跨实验室的互操作性提供了可靠基础，支持未来的数据驱动科学发现。
...
TL;DR: SAT框架解决二维单规格切割问题，通过需求展开、分配变量和不可行方向消除规则，在Cui-Zhao基准测试中优于OR-Tools、CPLEX和Gurobi


<details>
  <summary>Details</summary>
Motivation: 二维单规格切割问题(2D-CSSP)是制造业中的核心任务，需要从标准板材上切割矩形零件以满足需求并最小化浪费。该问题比装箱问题更复杂，因为每种零件类型需要多个副本，导致组合爆炸。现有求解器在处理此类问题时存在局限性。

Method: 提出基于SAT的框架：1) 将零件类型按需求展开为多个副本；2) 为每个副本分配板材变量；3) 仅对分配到同一板材的副本激活非重叠约束；4) 引入不可行方向消除规则，当零件只能以一种方向放入板材时固定旋转变量。比较三种最小化板材数量的方法：非增量SAT加二分搜索、增量SAT跨迭代重用子句、加权部分MaxSAT。
...
Conclusion: SAT方法在解决二维切割问题上优于传统优化求解器，特别是在处理旋转选项时表现出不同的性能特征。该框架通过需求展开和智能约束管理有效处理组合复杂性，为制造业切割优化提供了新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 通用LLM将生成视为单次文本补全，无法满足安全关键工程工作流中固有的顺序多门约束，导致在生成高超音速热防护系统仿真工件时出现级联约束违反问题。

Method: 提出约束闭环生成框架，将TPS工件生成组织为迭代工作流：生成、验证、CDG引导修复、执行和审计。约束依赖图编码约束类别间的经验共解决结构，基于生命周期排序先验和经验共解决概率指导修复。

Result: AeroTherm-GPT在HyTPS-Bench上达到88.7%端到端成功率，比非CDG消融基线提升12.5个百分点，根因修复效率达4.16（vs. 1.76），在科学推理和代码生成任务上无灾难性遗忘。

Conclusion: CCLG框架通过约束依赖图指导的迭代修复机制，显著提升了LLM在复杂工程约束下的工件生成成功率，为安全关键领域的LLM应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱能高效存储大量关系，但难以处理概念在不同领域中语义变化的问题。当前系统通常将领域信息作为元数据、限定符或图级组织，这些机制仅用于过滤和溯源，不改变断言的形式状态。许多知识系统的实际失败源于将领域视为断言的外部因素。

Method: 提出领域情境化概念图(DCG)框架，将领域直接写入关系中作为模态世界约束。采用形式为(C, R at D, C')的三元组，其中at D标识关系成立的世界。通过领域索引的必然性算子解释关系，使真值、推理和冲突检查都限定在相关世界中。开发了Kripke风格语义、紧凑谓词系统、Prolog实现，并映射到RDF、OWL和关系数据库。

Result: DCG框架实现了三个重要结果：1)模糊概念可在表示点进行消歧；2)无效断言可根据其领域进行挑战；3)跨领域关系可通过显式谓词连接。为领域提供了结构化和可计算的角色，改变了领域在知识表示中的传统地位。

Conclusion: 领域应被视为知识表示的内在组成部分而非补充注释。DCG通过将领域作为模态世界约束纳入表示结构，解决了概念语义漂移问题，为知识系统提供了更精确和实用的表示框架。这一贡献是对领域本身的表示学重新解释。
...
TL;DR: PGPO提出了一种细粒度的信用分配框架，通过量化视觉依赖度并动态重塑token级优势，解决多模态推理中视觉依赖token学习信号稀释的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习框架存在方法学缺陷：通过在所有生成token上分配相同优势，稀释了多模态推理中关键视觉依赖步骤的学习信号，阻碍了感知基础推理的优化。
...
Method: 提出了AWARE框架，包含：1）使用监督嵌入学习进行任务对齐的检索；2）轻量级适配器；3）在多队列EHR基准上比较经典、深度表格和TICL模型；4）评估不同数据规模、特征维度、结果稀有性和跨队列泛化能力。

Result: PFN-based TICL模型在低数据情况下样本效率高，但随着异质性和不平衡增加，基于朴素距离的检索性能下降。AWARE框架在极端不平衡情况下将AUPRC提升高达12.2%，且随着数据复杂性增加，增益更加明显。

Conclusion: 检索质量和检索-推理对齐是部署表格上下文学习于临床预测的关键瓶颈。AWARE框架通过任务对齐的检索机制有效解决了这些问题，为临床预测中的TICL应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有随机最短路径问题算法虽然使用启发式函数筛选有希望的状态，但在每个Bellman备份中仍考虑所有适用动作，即使启发式表明某些动作过于昂贵，导致计算资源浪费。

Method: 将启发式搜索重新表述为线性规划问题，为SSP引入高效的约束生成实现，开发CG-iLAO*算法，该算法基于iLAO*但使用新方法避免考虑昂贵动作。

Result: CG-iLAO*在许多问题上仅考虑iLAO*动作的40%，在某些问题上低至1%；平均减少3.5倍的动作成本计算，比iLAO*和LRTDP分别快2.8倍和3.7倍。

Conclusion: 通过约束生成技术避免考虑启发式指示的昂贵动作能显著提高随机最短路径问题求解效率，CG-iLAO*在减少计算量和加速求解方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 在多类分类中，传统的硬标签监督无法表达类别间的模糊性和不确定性。可能性监督提供了一种更灵活的监督形式，允许表达类别间的分级可信度，但需要将其转化为可训练的约束条件。

Method: 1. 从归一化的可能性分布构建非空闭凸可接受概率分布集，结合概率兼容性要求和线性形状约束；2. 使用Dykstra算法计算模型输出概率向量到可接受集的KL投影；3. 通过最小化预测与其投影之间的散度来训练模型。

Result: 提出的投影算法在实际应用中足够高效，基于投影的学习目标在合成数据和真实世界自然语言推理任务（基于ChaosNLI数据集）上都能提升预测性能。

Conclusion: 该方法为可能性监督的多类分类提供了一种有效的学习框架，通过KL投影将定性约束转化为可优化的目标，在保持可能性分布结构的同时提升了模型性能。
...
Method: 基于一阶逻辑构建Qiana框架，使其可有限公理化；支持对公式和上下文进行量化；允许在上下文中使用超协调逻辑；设计兼容现有一阶逻辑定理证明器。

Result: 成功开发出Qiana逻辑框架，能够表示时间性、事件演算和模态逻辑；讨论了不同的设计替代方案；证明了框架与现有一阶逻辑定理证明器的兼容性。

Conclusion: Qiana提供了一个强大而灵活的逻辑框架，用于在特定上下文中进行推理，支持量化表达和矛盾容忍，同时保持与标准一阶逻辑工具的兼容性。
...
Method: 使用多LLM提取框架从1,430篇同行评审出版物中构建NeuroPose-AHM数据集。对数据集进行了四任务分析框架验证，包括：1) 多标签AHM类型分类；2) 构建头颈严重程度指数(HNSI)；3) 用真实世界数据验证HNSI；4) 运动类型概率与HNSI分数的桥接分析。

Result: 数据集包含2,756条记录，涵盖57种神经系统疾病。LLM提取可靠性高(kappa=0.822)。在颈椎肌张力障碍分析中：AHM类型分类F1=0.856；HNSI与真实数据严重带比例一致(6.7%)；运动类型概率与HNSI显著相关(p<0.001)。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的资源，具有分析实用性，可用于神经系统异常头部运动研究。数据集已在Zenodo公开可用，为AI驱动诊断工具开发提供了重要基础。
...
TL;DR: SenseMath基准测试评估LLMs的数字感知能力，发现模型能使用计算捷径但缺乏对其适用性的结构性理解，表现出程序性流畅而非真正的数字感知。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常默认逐步计算，即使存在高效数值捷径。这引发了一个基本问题：它们是否表现出类似人类行为意义上的数字感知能力，即识别数值结构、在适当时应用捷径、在不适用时避免使用捷径的能力。

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字尺度，配有匹配的强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用（模型能否在适用捷径的问题上应用捷径）、适用性判断（能否识别捷径何时适用或误导）、问题生成（能否生成正确允许特定类型捷径的新问题）。评估了五个LLM，从GPT-4o-mini到Llama-3.1-8B。
...
Conclusion: 当前LLMs表现出程序性捷径流畅性，但缺乏支撑人类数字感知的对捷径何时及为何有效的结构性理解。模型能执行捷径但无法理解其适用条件，表明它们缺乏真正的数字感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法大多依赖有监督分类器，需要疾病标签数据，难以泛化到异质性病理表现。需要一种无需疾病标签就能检测和校正步态异常的方法。
...
<details>
  <summary>Details</summary>
Motivation: 强化学习能增强大语言模型的推理能力，但在多轮智能体任务中存在挑战：长时程交互和环境反馈的随机性导致次优动作产生误导性上下文，形成错误累积的反馈循环，使标准探索策略失效

Method: 提出ProCeedRL（Process Critic with Explorative Demonstration RL），将探索从被动选择转向主动干预。采用过程级批评器实时监控交互，结合基于反思的演示来指导智能体停止错误累积。通过从探索性演示和同策略样本中学习来提升探索效率
...
TL;DR: ATBench是一个轨迹级基准测试，用于评估LLM智能体的安全性，通过三维风险分类、异构工具池和长上下文延迟触发协议，在多样化、结构化的交互中检测安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败观测粗糙、长时域真实性弱等局限性，无法充分评估LLM智能体在现实部署中多步交互中涌现的风险。

Method: 基于风险来源、失败模式和现实危害三维分类法构建轨迹；使用异构工具池和长上下文延迟触发协议模拟真实风险涌现；包含1000个轨迹（503安全/497不安全），平均9.01轮对话和3.95k tokens；采用基于规则和LLM的过滤加全人工审核确保数据质量。

Result: ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性；支持分类分层分析、跨基准比较和长时域失败模式诊断；包含1954个调用的工具，来自2084个可用工具池。

Conclusion: ATBench提供了一个结构化、多样化且真实的轨迹级基准测试，能够全面评估LLM智能体在多步交互中的安全性，填补了现有基准在交互多样性和长时域真实性方面的不足。
...
TL;DR: 该论文是一篇关于语言模型中潜在空间的系统性综述，从基础、演化、机制、能力和展望五个视角全面梳理了该领域的研究进展。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，潜在空间正成为语言模型的原生计算基底。传统基于显式token生成的理解方式存在局限性，许多关键内部过程更适合在连续潜在空间中执行。该综述旨在为语言模型中的潜在空间研究提供统一、最新的全景图。

Method: 采用五维框架组织综述：1) 基础：界定潜在空间范围，区分显式/言语空间和视觉生成模型的潜在空间；2) 演化：追溯从早期探索到当前大规模扩展的发展历程；3) 机制：从架构、表示、计算、优化四个技术维度分析；4) 能力：展示潜在空间支持的推理、规划、建模、感知、记忆、协作、具身等能力谱系；5) 展望：讨论开放挑战和未来研究方向。

Result: 构建了语言模型潜在空间的系统性分类框架，识别了该领域的关键技术发展脉络和能力维度，为理解潜在空间作为下一代智能的通用计算和系统范式奠定了基础。

Conclusion: 潜在空间正成为语言模型的核心计算范式，超越了传统的显式token处理。该综述不仅为现有研究提供参考框架，更为理解潜在空间作为下一代智能的通用计算范式奠定基础，指出了该领域的关键挑战和未来发展方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统保险问卷冗长标准化，难以捕捉个体差异，且依赖用户诚实回答易导致欺诈。需要更智能、个性化、可信的承保方法。

Method: 采用大型语言模型（LLM）和替代数据源（社交媒体图像分析、地理数据分类、检索增强生成RAG）提取用户洞察，生成个性化自适应问卷。

Result: 在行业合作伙伴移动应用中测试：传统问卷风险评估准确度略高，但GPT驱动的自适应版本问题更少，用户体验更流畅、更具吸引力，用户偏好明显。

Conclusion: ARQuest在提升用户满意度和简化保险流程方面潜力巨大，进一步开发可能超越传统方法的风险准确性，推动保险行业创新。
...
Conclusion: Diff-KD通过整合扩散生成式精炼与知识蒸馏，能够主动恢复损坏语义，显著提升了多智能体协同感知在现实损坏条件下的鲁棒性
...
TL;DR: 该论文研究LLM在时间序列解释生成与评估中的表现，发现LLM作为评估者比作为生成者更可靠，即使在自身生成错误时也能正确评估他人解释。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估LLM生成的时间序列解释的事实正确性的通用方法。现有方法要么需要真实解释作为参考，要么只能处理数值信号而无法评估自由形式的文本推理。需要一种无需预定义参考或任务特定规则的直接验证方法。

Method: 研究LLM作为时间序列解释的生成者和评估者。在无参考设置下，给定时间序列、问题和候选解释，评估者基于模式识别、数值准确性和答案忠实性分配三元正确性标签。构建包含350个时间序列案例的合成基准，涵盖七种查询类型，每种都配有正确、部分正确和错误的解释。评估模型在四个任务上的表现：解释生成、相对排序、独立评分和多异常检测。

Result: 结果显示明显的不对称性：生成高度依赖模式，在某些查询类型上存在系统性失败（季节性下降和波动率变化的准确率仅为0.00-0.12，而结构性断点的准确率为0.94-0.96）。评估则更稳定，即使模型自身输出错误，也能正确排序和评分解释。这表明LLM作为时间序列解释的评估者具有可行性。

Conclusion: LLM作为数据驱动的评估者在时间序列领域具有潜力，能够可靠评估数据驱动的推理。虽然生成能力存在局限性，但评估能力相对稳定，为时间序列解释的事实正确性验证提供了可行方法。
...
TL;DR: MTI是一个基于行为的AI智能体气质测评系统，通过四个独立轴（反应性、顺从性、社交性、韧性）测量AI的行为模式差异，而非自我报告，证明了气质与模型大小无关，RLHF会重塑气质结构。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么借用人类人格维度并依赖自我报告（这与LLM的实际行为存在偏差），要么将行为变异视为缺陷而非特质。缺乏标准化的工具来测量AI模型之间的气质差异。

Method: 引入模型气质指数（MTI），基于行为而非自我报告，采用四轴框架（反应性、顺从性、社交性、韧性），使用结构化检查协议和两阶段设计（分离能力与气质），对10个小型语言模型进行测评。

Result: 五个主要发现：1）四轴在指令调优模型中基本独立；2）轴内面解离得到证实；3）顺从性-韧性悖论；4）RLHF通过创造轴内面分化重塑气质；5）气质与模型大小无关。

Conclusion: MTI提供了一个标准化的行为基础测评系统，能够可靠测量AI智能体的气质差异，这些差异与模型能力无关，且RLHF等训练范式会显著改变气质结构。
...
### [209] [From High-Dimensional Spaces to Verifiable ODD Coverage for Safety-Critical AI-based Systems](https://arxiv.org/abs/2604.02198)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统在安全关键领域部署面临EASA认证挑战，需要证明ODD的完整覆盖，但现有方法缺乏可扩展性和形式化基础，无法满足完整性要求，且缺乏标准化工程方法连接抽象ODD定义与可验证证据

Method: 集成参数离散化、基于约束的过滤和基于关键性的维度约减，形成结构化多步骤ODD覆盖验证流程，基于先前AI空中防撞研究的仿真数据进行验证

Result: 该方法能够系统化定义和实现满足EASA完整性要求的覆盖度量，实现在更高维度验证ODD覆盖，推进安全设计方法

Conclusion: 提出的方法填补了现有空白，为AI系统在安全关键领域认证提供了可扩展、形式化的ODD覆盖验证框架，符合EASA标准并支持安全设计理念
...
TL;DR: 研究评估LLM生成的胸部CT报告日文翻译的教育适用性，发现放射科医生与LLM评估者之间几乎无一致性，LLM评估者强烈偏好LLM输出，表明自动LLM评估不足以替代专家评审


<details>
  <summary>Details</summary>
Motivation: 放射学报告的多语言翻译对研究、临床沟通和教育很重要，但LLM评估的有效性尚不明确。需要评估LLM生成翻译的教育适用性，并比较放射科医生评估与LLM作为评估者的差异。

Method: 分析150份胸部CT报告，比较人工编辑的日文翻译与DeepSeek-V3.2生成的LLM翻译。由认证放射科医生和放射科住院医师进行盲法配对评估（术语准确性、可读性、整体质量、放射科医生风格真实性）。同时使用3个LLM评估者（DeepSeek-V3.2、Mistral Large 3、GPT-5）评估相同配对。使用QWK和百分比一致性评估一致性。

Result: 放射科医生与LLM评估者之间一致性接近零（QWK=-0.04至0.15）。两名放射科医生之间一致性也差（QWK=0.01至0.06）。放射科医生1认为59%的病例术语相当，在可读性（51%）和整体质量（51%）上偏好LLM翻译。放射科医生2认为75%的病例可读性相当，在整体质量上偏好人工编辑翻译（40% vs 21%）。所有3个LLM评估者强烈偏好LLM翻译（70%-99%），并在>93%的病例中认为其更具放射科医生风格。

Conclusion: LLM生成的翻译常被认为自然流畅，但两名放射科医生评估差异显著。LLM作为评估者表现出对LLM输出的强烈偏好，与放射科医生几乎无一致性。对于放射学报告翻译的教育用途，仅依赖自动LLM评估不足够；专家放射科医生评审仍然重要。
...
Method: 采用基于扰动的策略结合三矩阵分析框架：1)角度偏差矩阵捕捉语义方向变化；2)幅度偏差矩阵测量语义强度变化；3)维度重要性矩阵评估各向量维度贡献。通过系统移除每个token并测量这三个互补维度的影响，得到复合重要性分数。

Result: 开发了一种模型无关的token重要性可视化技术，能够生成显示token级贡献的相关性图，提供数学基础扎实的token重要性度量，且不增加计算成本。

Conclusion: 提出的方法为理解生成式AI系统如何感知和优先处理输入文本信息提供了轻量级、模型无关的解决方案，所有可解释性技术已开源实现，促进可复现性和广泛应用。
...
### [212] [When to ASK: Uncertainty-Gated Language Assistance for Reinforcement Learning](https://arxiv.org/abs/2604.02226)
*Juarez Monteiro,Nathan Gavenski,Gianlucca Zuin,Adriano Veloso*

Main category: cs.AI

TL;DR: ASK方法结合小语言模型与RL策略，通过选择性查询语言模型来增强OOD泛化能力，无需重新训练


<details>
  <summary>Details</summary>
Motivation: RL智能体在分布外场景中表现不佳，而大语言模型计算成本高且自主规划能力有限，需要一种高效结合方法

Method: 使用蒙特卡洛Dropout评估不确定性，仅当不确定性超过阈值时查询语言模型获取行动建议，选择性结合语言模型推理与RL策略

Result: 在FrozenLake环境中，域内无改进，但在迁移任务中实现稳健导航，获得0.95的奖励

Conclusion: 有效的神经符号集成需要精心编排而非简单组合，需要足够模型规模和有效混合机制来实现成功的OOD泛化
...
<details>
  <summary>Details</summary>
Motivation: 监管文档包含LLM系统必须遵守的法律义务，但将密集、层次化的法律文本转换为机器可读规则仍然是一个昂贵、专家密集型的过程，需要自动化解决方案来降低成本和提高可扩展性。

Method: De Jure采用四阶段管道：1) 源文档规范化到结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 多标准LLM-as-a-judge评估（19个维度）；4) 在有限再生预算内迭代修复低质量提取，优先修复上游组件。

Result: 在金融领域，De Jure在三个法官引导迭代内达到峰值性能；在医疗和AI治理领域保持高性能；在RAG合规问答评估中，基于De Jure提取规则的响应在单规则检索深度下73.8%优于先前工作，在更广泛检索下达到84.0%。
...
TL;DR: 提出Trace Inversion方法，通过分析推理模型的推理轨迹来重构其可能回答的问题，通过比较原始问题与重构问题的相似度来提升模型拒绝回答的能力


<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要具备有效的拒绝回答能力，但推理模型在这方面表现较差。模型幻觉导致的拒绝失败可以重新解释为模型回答了错误的问题，而非错误地回答了问题

Method: 提出查询错位框架和Trace Inversion方法：1)生成模型的推理轨迹；2)仅基于轨迹重构模型最可能回答的查询；3)比较初始查询与重构查询的相似度，低相似度则标记为需要拒绝回答

Result: 在四个前沿LLM和九个拒绝回答QA数据集上的广泛实验表明，Trace Inversion在36个设置中的33个中击败了竞争基线，有效提升了拒绝回答性能

Conclusion: Trace Inversion通过分析推理轨迹来检测查询错位，为提升推理模型的拒绝回答能力提供了一种有效的新方法，显著优于现有基线
...
<details>
  <summary>Details</summary>
Motivation: 研究情感语气在用户查询中对大语言模型行为的影响，探索情感框架是否可作为有效的干预手段

Method: 在六个基准领域（数学推理、医学问答、阅读理解、常识推理和社交推理）测试静态情感前缀的影响，引入EmotionRL自适应情感提示框架进行动态选择

Result: 静态情感前缀通常只产生小的准确性变化；情感影响在社交任务中更可变；自适应情感选择比固定情感提示产生更可靠的增益
...
TL;DR: 该报告综述了变形形状对应估计的三种主要方法：基于功能映射的谱方法、施加离散约束的组合公式化方法以及直接恢复全局对齐的变形方法，并讨论了各方法的优缺点、最新进展和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 变形形状实例之间的对应估计是计算机图形学中长期存在的问题，从纹理传输到统计建模等多种应用都依赖于恢复准确的对应映射。由于不同下游应用的需求，出现了多种方法从不同角度解决这一挑战性问题。本报告旨在帮助研究人员、从业者和学生理解该领域的最新趋势和进展。

Method: 报告将发展分为三种范式：1) 基于功能映射的谱方法；2) 施加离散约束的组合公式化方法；3) 直接恢复全局对齐的变形方法。报告系统讨论了每种方法的优缺点，并强调了各领域的最新进展。

Result: 报告提供了对变形形状对应估计领域的全面综述，分析了三种主要方法范式的技术特点、优势和局限性。同时指出了新兴挑战和机遇，包括使用视觉基础模型进行零样本对应以及部分形状匹配等特别具有挑战性的任务。

Conclusion: 该领域呈现出多样化的发展趋势，三种主要方法范式各有其适用场景和优势。未来研究方向包括利用视觉基础模型实现零样本对应、解决部分形状匹配等挑战性问题，以及探索不同方法的融合与互补。
...
TL;DR: 提出用于3D高斯泼溅的实时交互式色彩编辑框架，支持调色板重着色、色彩感知亮度调整和像素级约束，通过视图空间分解解决现有方法在alpha混合后的稀疏性失效问题。


<details>
  <summary>Details</summary>
Motivation: 专业色彩编辑需要精确控制色相、饱和度和亮度，现有基于调色板的3DGS方法在原始级别分解颜色，但alpha混合后无法保证像素级稀疏性，导致编辑影响非目标区域，降低编辑质量。

Method: 采用视图空间调色板分解方法，泼溅权重而非颜色以优化场景可观察外观；引入基于逆重心坐标的几何损失来强制一致的稀疏模式，确保相似颜色具有相似分解。

Result: 相比原始空间方法获得更优的编辑质量，支持专业色彩分级工作流程，实现3DGS场景的实时交互编辑。

Conclusion: 提出的视图空间分解方法解决了现有调色板编辑中的稀疏性失效问题，实现了高质量的实时交互色彩编辑，为3DGS场景提供了专业色彩分级能力。
...
TL;DR: 提出一种保证B-Rep拓扑正确性的网格生成方法，将拓扑作为网格化过程的不变量，通过单一容差控制几何精度，无需后处理修复
...
Method: 将B-Rep拓扑作为网格化过程的不变量，算法强制执行精确的B-Rep拓扑，同时允许用户定义单一容差来控制网格与底层参数曲面的偏差

Result: 在ABC和Fusion 360存储库的数千个真实CAD模型上评估，包括标准网格工具失败的实例，证明算法级拓扑保证能实现可靠的网格生成

Conclusion: 拓扑与几何的分离消除了后处理修复需求，即使在底层几何不一致或高度近似的情况下也能生成鲁棒的网格，适合下游应用
